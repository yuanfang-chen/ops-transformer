...
// check sink dtype
bool LearnableSinkChecker::CheckSinkDtypeSupport(const FiaTilingInfo &fiaInfo)
{
    ...
    if (learnableSinkDesc != nullptr) {
        OP_CHECK_IF(learnableSinkDesc->GetDataType() != DT_BF16 && learnableSinkDesc->GetDataType() != DT_FLOAT16,
...
// CheckFeature
bool LearnableSinkChecker::CheckFeatureSupport(const FiaTilingInfo &fiaInfo)
{
    if (!fiaInfo.learnableSinkFlag) {
        return GRAPH_SUCCESS;
    }
...
// check sink shape
bool LearnableSinkChecker::CheckSinkShapeSupport(const FiaTilingInfo &fiaInfo)
{
    if (!fiaInfo.learnableSinkFlag) {
        return GRAPH_SUCCESS;
    }
...
// check axis
bool LearnableSinkChecker::CheckAxisSupport(const FiaTilingInfo &fiaInfo)
{
    if (!fiaInfo.learnableSinkFlag) {
        return GRAPH_SUCCESS;
    }
...
bool LearnableSinkChecker::CheckSinglePara(const FiaTilingInfo &fiaInfo)
{
    if (GRAPH_SUCCESS != CheckSinkDtypeSupport(fiaInfo) ||
        GRAPH_SUCCESS != CheckSinkShapeSupport(fiaInfo)) {
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
}

bool LearnableSinkChecker::CheckParaExistence(const FiaTilingInfo &fiaInfo)
{
    return GRAPH_SUCCESS;
}

bool LearnableSinkChecker::CheckFeature(const FiaTilingInfo &fiaInfo)
{
...
bool LearnableSinkChecker::CheckMultiPara(const FiaTilingInfo &fiaInfo)
{
    return GRAPH_SUCCESS;
}
