...
// check rope dtype
bool RopeChecker::CheckRopeDtype(const FiaTilingInfo &fiaInfo)
{
    if (fiaInfo.ropeMode != RopeMode::ROPE_SPLIT) {
        return GRAPH_SUCCESS;
    }
    OP_CHECK_IF((fiaInfo.inputQRopeType != DT_BF16 && fiaInfo.inputQRopeType != DT_FLOAT16),
    ...
    OP_CHECK_IF((fiaInfo.inputKRopeType != DT_BF16 && fiaInfo.inputKRopeType != DT_FLOAT16),
        OP_LOGE(fiaInfo.opName,
...
// check rope DSize
bool RopeChecker::CheckRopeDSizeSupport(const FiaTilingInfo &fiaInfo)
{
    if (fiaInfo.ropeMode != RopeMode::ROPE_SPLIT) {
        return GRAPH_SUCCESS;
    }
...
bool RopeChecker::CheckQDsizeSupport(const FiaTilingInfo &fiaInfo)
{
...
bool RopeChecker::CheckShapeSupport(const FiaTilingInfo &fiaInfo)
{
    // check qk head dim and rope head dim
    if (GRAPH_SUCCESS != CheckQDsizeSupport(fiaInfo)) {
            return GRAPH_FAILED;
    }
    ...
    if (GRAPH_SUCCESS != CheckQKAndQKRopeShapeConsistency(fiaInfo, queryShape, queryRopeShape, "query")) {
            return GRAPH_FAILED;
    }
    // check keyRope shape
    if (GRAPH_SUCCESS != CheckQKAndQKRopeShapeConsistency(fiaInfo, keyShape, keyRopeShape, "key") ||
        GRAPH_SUCCESS != CheckPAKeyAndKeyRopeShapeConsistency(fiaInfo, keyShape, keyRopeShape) ||
        GRAPH_SUCCESS != CheckTensorlistKeyAndKeyRopeShapeConsistency(fiaInfo)) {
            return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
}
...
bool RopeChecker::CheckRopeDtypeConsistency(const FiaTilingInfo &fiaInfo)
{
    if (enableNonQuant_) {
...
bool RopeChecker::CheckQKAndQKRopeShapeConsistency(const FiaTilingInfo &fiaInfo,
...
bool RopeChecker::CheckPAKeyAndKeyRopeShapeConsistency(const FiaTilingInfo &fiaInfo,
...
bool RopeChecker::CheckTensorlistKeyAndKeyRopeShapeConsistency(const FiaTilingInfo &fiaInfo)
{
...
bool RopeChecker::CheckRopeExistence(const FiaTilingInfo &fiaInfo)
{
...
bool RopeChecker::CheckFeatureSupport(const FiaTilingInfo &fiaInfo)
{
    if (fiaInfo.ropeMode != RopeMode::ROPE_SPLIT) {
        return GRAPH_SUCCESS;
    }
...
bool RopeChecker::CheckFeatureDecodeMLA(const FiaTilingInfo &fiaInfo)
{
...
bool RopeChecker::CheckFeatureAntiQuant(const FiaTilingInfo &fiaInfo)
{
    // 不支持伪量化
...
bool RopeChecker::CheckAxisSupport(const FiaTilingInfo &fiaInfo)
{
    if (fiaInfo.mlaMode != MlaMode::ROPE_SPLIT_D512) {
        return GRAPH_SUCCESS;
    }
...
bool RopeChecker::CheckSinglePara(const FiaTilingInfo &fiaInfo)
{
    if (GRAPH_SUCCESS != CheckRopeDtype(fiaInfo) ||
        GRAPH_SUCCESS != CheckRopeDSizeSupport(fiaInfo)) {
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
}

bool RopeChecker::CheckParaExistence(const FiaTilingInfo &fiaInfo)
{
    if (GRAPH_SUCCESS != CheckRopeExistence(fiaInfo)) {
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
}

bool RopeChecker::CheckFeature(const FiaTilingInfo &fiaInfo)
{
    if (fiaInfo.ropeMode != RopeMode::ROPE_SPLIT) {
        return GRAPH_SUCCESS;
    }
    if (GRAPH_SUCCESS != CheckFeatureSupport(fiaInfo) ||
        GRAPH_SUCCESS != CheckFeatureDecodeMLA(fiaInfo)) {
            return GRAPH_FAILED;
    }

    if (GRAPH_SUCCESS != CheckShapeSupport(fiaInfo) ||
        GRAPH_SUCCESS != CheckRopeDtypeConsistency(fiaInfo) ||
        GRAPH_SUCCESS != CheckAxisSupport(fiaInfo)) {
            return GRAPH_FAILED;
    }
...
bool RopeChecker::CheckMultiPara(const FiaTilingInfo &fiaInfo)
{
    if (fiaInfo.ropeMode != RopeMode::ROPE_SPLIT) {
        return GRAPH_SUCCESS;
    }
