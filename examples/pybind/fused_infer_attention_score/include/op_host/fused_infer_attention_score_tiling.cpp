...
static void ConvertDataTypePFA(optiling::TilingContext &context, ContextParamsForPFATiling &contextKeyParams)
{
    ...
    contextKeyParams.quantScale2Type = (context.GetOptionalInputDesc(QUANT_SCALE2_INDEX) != nullptr) ?
        context.GetOptionalInputDesc(QUANT_SCALE2_INDEX)->GetDataType() : optiling::DT_FLOAT;
    contextKeyParams.quantOffset2Type = (context.GetOptionalInputDesc(QUANT_OFFSET2_INDEX) != nullptr) ?
        context.GetOptionalInputDesc(QUANT_OFFSET2_INDEX)->GetDataType() : optiling::DT_FLOAT;
    contextKeyParams.blockTableType = (context.GetOptionalInputDesc(BLOCK_TABLE_INDEX) != nullptr) ?
        context.GetOptionalInputDesc(BLOCK_TABLE_INDEX)->GetDataType() : optiling::DT_INT32;
...
static void ConvertShapePFA(optiling::TilingContext &context, ContextParamsForPFATiling &contextKeyParams)
{
...
static bool ConvertAttrsPFA(optiling::TilingContext &context, ContextParamsForPFATiling &contextKeyParams)
{
...
static bool GetCumulativeKeyValueSInBSH(optiling::TilingContext &context,
...
static bool GetCumulativeKeyValueSInBNSD(optiling::TilingContext &context,
    ContextParamsForPFATiling &contextKeyParams, int64_t &cumulativeKeyS, int64_t &cumulativeValueS,
    const int64_t validBatchOfK)
{
...
static bool GetCumulativeKeyValueSInBSND(optiling::TilingContext &context,
    ContextParamsForPFATiling &contextKeyParams, int64_t &cumulativeKeyS, int64_t &cumulativeValueS,
    const int64_t validBatchOfK)
{
...
static bool CheckCumulativeKeyValue(optiling::TilingContext &context,
    ContextParamsForPFATiling &contextKeyParams, int64_t &cumulativeKeyS, int64_t &cumulativeValueS,
    const int64_t validBatchOfK)
{
    ...
    if (layoutStr == "BSH") {
        ...
        OP_CHECK_IF(GetCumulativeKeyValueSInBSH(
            context, contextKeyParams, cumulativeKeyS, cumulativeValueS, validBatchOfK)!= GRAPH_SUCCESS,
            OPS_REPORT_VECTOR_INNER_ERR(context.GetNodeName(),
            "get cumulativeKeyS and cumulativeValueS in BSH failed"), return GRAPH_FAILED);
        ...
        OP_CHECK_IF(GetCumulativeKeyValueSInBNSD(
            context, contextKeyParams, cumulativeKeyS, cumulativeValueS, validBatchOfK) != GRAPH_SUCCESS,
            OPS_REPORT_VECTOR_INNER_ERR(context.GetNodeName(),
            "get cumulativeKeyS and cumulativeValueS in BNSD/BNSD_BSND failed"), return GRAPH_FAILED);
    } else {
        // check N and D, respectively, are the same across batches and KVs under BSND
        OP_CHECK_IF(GetCumulativeKeyValueSInBSND(
            context, contextKeyParams, cumulativeKeyS, cumulativeValueS, validBatchOfK) != GRAPH_SUCCESS,
            OPS_REPORT_VECTOR_INNER_ERR(context.GetNodeName(),
            "get cumulativeKeyS and cumulativeValueS in BSND failed"), return GRAPH_FAILED);
    }

    OP_CHECK_IF((contextKeyParams.emptyTensor == 1) && (cumulativeKeyS != 0) && (cumulativeValueS != 0),
        OPS_REPORT_VECTOR_INNER_ERR(context.GetNodeName(),
        "Got empty tensor in key and value which is not continuous.!!"), return GRAPH_FAILED);

    return GRAPH_SUCCESS;
}

static bool CheckKvPFA(optiling::TilingContext &context, ContextParamsForPFATiling &contextKeyParams)
{
    const string layoutStr = string(contextKeyParams.layout);
    auto batchOfQ = 1;
    auto batchOfK = 1;
...
static bool CheckParamsPFA(optiling::TilingContext &context, ContextParamsForPFATiling &contextKeyParams)
{
    OP_CHECK_IF(CheckKvPFA(context, contextKeyParams) != GRAPH_SUCCESS,
        OPS_REPORT_VECTOR_INNER_ERR(context.GetNodeName(), "check kv failed"), return GRAPH_FAILED);
    ...
    OP_CHECK_IF(((contextKeyParams.kvPaddingSize != nullptr) &&
        ...
        OPS_REPORT_VECTOR_INNER_ERR(context.GetNodeName(), "KV PaddingSize input is invalid!"), return GRAPH_FAILED);

    OP_CHECK_IF(((contextKeyParams.blockTable != nullptr) &&
        ((contextKeyParams.queryPaddingSize != nullptr) || (contextKeyParams.kvPaddingSize != nullptr))),
        OPS_REPORT_VECTOR_INNER_ERR(context.GetNodeName(),
        "when page attention is used, left padding is not supported!"), return GRAPH_FAILED);

    OP_CHECK_IF(((contextKeyParams.queryPaddingSize != nullptr) && (contextKeyParams.actualSequenceLengthQ == nullptr)),
        OPS_REPORT_VECTOR_INNER_ERR(context.GetNodeName(),
        "if Query has leftpadding, the query's actual sequence lengths are required!"), return GRAPH_FAILED);
    OP_CHECK_IF(((contextKeyParams.kvPaddingSize != nullptr) && (contextKeyParams.actualSequenceLengthKV == nullptr)),
        OPS_REPORT_VECTOR_INNER_ERR(context.GetNodeName(),
        "if KV has leftpadding, the key/value's actual sequence lengths are required!"), return GRAPH_FAILED);

    return GRAPH_SUCCESS;
}

static bool ConvertContextToParamsPFA(optiling::TilingContext &context,
    ContextParamsForPFATiling &contextKeyParams)
{
    ...
    OP_CHECK_IF((contextKeyParams.attentionMask != nullptr) &&
        (context.GetOptionalInputDesc(ATTEN_MASK_INDEX)->GetDataType() != optiling::DT_BOOL) &&
        (context.GetOptionalInputDesc(ATTEN_MASK_INDEX)->GetDataType() != optiling::DT_INT8) &&
        (context.GetOptionalInputDesc(ATTEN_MASK_INDEX)->GetDataType() != optiling::DT_UINT8),
        OPS_REPORT_VECTOR_INNER_ERR(context.GetNodeName(),
        "Invalid attention mask datatype! Only support BOOL, INT8 and UINT8"), return GRAPH_FAILED);
    ...
    OP_CHECK_IF(ConvertAttrsPFA(context, contextKeyParams) != GRAPH_SUCCESS,
    ...
    OP_CHECK_IF(CheckParamsPFA(context, contextKeyParams) != GRAPH_SUCCESS,
        OPS_REPORT_VECTOR_INNER_ERR(context.GetNodeName(), "check params failed"), return GRAPH_FAILED);
...
static bool CheckDequantParams(optiling::TilingContext &context, const int64_t s)
{
...
static bool CheckLseShape(optiling::TilingContext &context, bool lseFlag, const int64_t b,
    const int64_t s, const int64_t n)
{
    ...
    if (inputLayoutStr == "TND" || inputLayoutStr == "NTD_TND") {
        OP_CHECK_IF(((lseFlag != false) && (tempLse->GetStorageShape().GetDimNum() != 3)),
            ...
            inputLayoutStr.c_str(), tempLse->GetStorageShape().GetDimNum()), return GRAPH_FAILED);
...
static bool SetPlatformInfo(optiling::TilingContext &context, PromptFlashAttentionCompileInfo &compileInfoPtr)
{
    auto platformInfoPtr = context.GetPlatformInfo();
    OP_CHECK_IF(platformInfoPtr == nullptr,
        OPS_REPORT_VECTOR_INNER_ERR(context.GetNodeName(), "platformInfoPtr is null"), return GRAPH_FAILED);
...
static bool TilingProcess4PFA(optiling::TilingContext *context, const uint32_t tempD, const int64_t b,
    const int64_t s, const int64_t n)
{
    ...
    OP_CHECK_IF((((contextParamsForPFATiling.inputDataType == optiling::DT_INT8) ||
        (contextParamsForPFATiling.kDataType == optiling::DT_INT8) ||
        (contextParamsForPFATiling.outputDataType == optiling::DT_INT8)) && (tempD % D_ALIGN_32 != 0)),
    ...
    OP_CHECK_IF((tempD % D_ALIGN_16 != 0), OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(),
        "D should be 16 elements aligned when with FP16/BF16 dtype!"), return GRAPH_FAILED);
...
bool CheckFAISeqlenDataInTND(
    const optiling::TilingContext *context, bool isPageAttention, int64_t actSeqLenDims, int64_t actSeqLenKVDims)
{
...
bool CheckSparseModeParams(const optiling::TilingContext *context, int64_t actSeqLenDims)
{
    auto actSeqLenData = context->GetOptionalInputTensor(ACTUAL_SEQ_Q_INDEX);
    auto actSeqLenDataKV = context->GetOptionalInputTensor(ACTUAL_SEQ_KV_INDEX);
...
bool CheckFAIIsTND(optiling::TilingContext *context, bool isPageAttention)
{
    ...
    OP_CHECK_IF((actSeqLenKVDims == 0),
        ...
        return GRAPH_FAILED);
    if (CheckFAISeqlenDataInTND(context, isPageAttention, actSeqLenDims, actSeqLenKVDims) != GRAPH_SUCCESS) {
        return GRAPH_FAILED;
    }
    int32_t sparseMode = *(context->GetAttrs()->GetAttrPointer<int32_t>(ATTR_SPARSE_MODE_INDEX));
    if (sparseMode == 4U && CheckSparseModeParams(context, actSeqLenDims) != GRAPH_SUCCESS) {
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
}

bool CheckFAIQKV(optiling::TilingContext *context, bool isPageAttention)
{
    ...
    OP_CHECK_IF((qDataType != kDataType) || (qDataType != vDataType),
        OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "Input dtype of Q, K, and V must be consitent"),
            return GRAPH_FAILED);
    OP_CHECK_IF((qDataType != optiling::DT_FLOAT16) && (qDataType != optiling::DT_BF16),
    ...
    if (inputLayoutStr == "TND") {
        if (CheckFAIIsTND(context, isPageAttention) != GRAPH_SUCCESS) {
            return GRAPH_FAILED;
        }
    }
    return GRAPH_SUCCESS;
...
bool CheckFAILearnableSink(const optiling::TilingContext *context) {
 	         auto qDataType = context->GetInputDesc(QUERY_INDEX)->GetDataType();
 	        ...
 	        OP_CHECK_IF(((sinkDataType != optiling::DT_FLOAT16) && (sinkDataType != optiling::DT_BF16)),
...
bool CheckFAISinglePara(const optiling::TilingContext *context, bool isPageAttention)
{
    auto attrs = context->GetAttrs();
    auto tempQ = context->GetInputShape(QUERY_INDEX);
...
bool CheckFAIMaskShape(const optiling::TilingContext *context)
{
...
bool CheckFAIMask(optiling::TilingContext *context)
{
    auto tempAttnMaskShape = context->GetOptionalInputShape(ATTEN_MASK_INDEX);
    auto attrs = context->GetAttrs();
    int32_t sparseMode = *(attrs->GetAttrPointer<int32_t>(ATTR_SPARSE_MODE_INDEX));
    ...
    if (tempAttnMaskShape == nullptr) {
        ...
        OP_CHECK_IF(maskDimNum != 2U && maskDimNum != 3U && maskDimNum != 4U,
            OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(),
                ...
                return GRAPH_FAILED);
        if (CheckFAIMaskShape(context) != GRAPH_SUCCESS) {
            return GRAPH_FAILED;
        }
    }
    return GRAPH_SUCCESS;
...
static bool CheckFAILseOutput(const optiling::TilingContext *context)
{
    bool lseFlag = *(context->GetAttrs()->GetAttrPointer<bool>(SOFTMAX_LSE_FLAG_INDEX));
    ...
    OP_CHECK_IF(((lseFlag != false) && (lseShape->GetStorageShape().GetDimNum() != 3U)),
        OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(),
        "When layout is TND, SoftmaxLse shape dim must be 3"), return GRAPH_FAILED);
    ...
    OP_CHECK_IF((lseFlag != false) && ((lseDim0 != t) || (lseDim1 != n) || (lseDim2 != 1U)),
        ...
        t, n), return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}

bool CheckFAIAvailability(optiling::TilingContext *context)
{
    bool isPageAttention = context->GetOptionalInputShape(BLOCK_TABLE_INDEX) != nullptr ? true : false;
    if (CheckFAIQKV(context, isPageAttention) != GRAPH_SUCCESS ||
        CheckFAIMask(context) != GRAPH_SUCCESS ||
        CheckFAISinglePara(context, isPageAttention) != GRAPH_SUCCESS ||
        CheckFAILseOutput(context) != GRAPH_SUCCESS) {
        return GRAPH_FAILED;
    }
    return GRAPH_SUCCESS;
}

static bool ConvertContextToParamsFAI(optiling::TilingContext *context, FAInferContext& faInfo, uint32_t aicoreNum)
{
    ...
    faInfo.dataType = static_cast<DataType>(qDataType == optiling::DT_BF16);
...
static bool IsUsingFAI(optiling::TilingContext &context, const string inputLayoutStr, const uint32_t tempD)
{
    bool isPageAttention = context.GetOptionalInputShape(BLOCK_TABLE_INDEX) != nullptr ? true : false;
    auto tempAttnMaskShape = context.GetOptionalInputShape(ATTEN_MASK_INDEX);
    auto qDataType = context.GetInputDesc(QUERY_INDEX)->GetDataType();
    ...
    if (isLearnableSink && inputLayoutStr == "TND") {
        ...
        int64_t tempQD = tempQ->GetStorageShape().GetDim(DIM_2);
        auto sinkDataType = context.GetOptionalInputDesc(LEARNABLE_SINK_INDEX)->GetDataType();
        if (tempQD == QUERY_HEAD_DIM_64 && sinkDataType == optiling::DT_BF16) {
    ...
    bool mhaConditions = isMha && !((qDataType == optiling::DT_BF16) && (innerPrecise == 1)) && 
...
static bool TilingProcess4SplitFuse(optiling::TilingContext *context)
{
    OP_CHECK_IF(CheckFAIAvailability(context) != GRAPH_SUCCESS,
...
bool IsGqaIfa(optiling::TilingContext &context, const string inputLayoutStr, const int64_t queryS, const int64_t queryD)
{
...
int64_t GetTndQueryS(optiling::TilingContext &context)
{
    auto queryShape = context.GetInputShape(QUERY_INDEX);
...
bool IsGqaMtp(optiling::TilingContext &context, const string inputLayoutStr, const int64_t queryS, const int64_t queryD)
{
    if (context.GetOptionalInputTensor(QUERY_ROPE_INDEX) != nullptr) {
        return false;
    }
    ...
    if (inputLayoutStr == "TND") {
        ...
        if (context.GetInputDesc(QUERY_INDEX)->GetDataType() != optiling::DT_BF16 ||
            context.GetInputDesc(KEY_INDEX)->GetDataType() != optiling::DT_INT8 ||
            context.GetInputDesc(VALUE_INDEX)->GetDataType() != optiling::DT_INT8) {
            return false;
        }
    }
    return true;
...
bool IsAtbIfa(optiling::TilingContext &context, const string inputLayoutStr, const int64_t queryD)
{
    bool isIsAtbIfaLayout = (inputLayoutStr == "TND_NTD") || (inputLayoutStr == "TND");
    if (!isIsAtbIfaLayout) {
        return false;
    }
    if (!((context.GetInputDesc(KEY_INDEX)->GetDataType() == optiling::DT_INT8) &&
        (context.GetInputDesc(QUERY_INDEX)->GetDataType() != optiling::DT_INT8))) { // KV antiquant
        return false;
    }
    ...
    bool isAntiquantParamFloat = (context.GetOptionalInputTensor(ANTIQUANT_SCALE_INDEX) != nullptr) &&
                                 ...
                                 (context.GetOptionalInputDesc(ANTIQUANT_SCALE_INDEX)->GetDataType() == optiling::DT_FLOAT) &&
                                 (context.GetOptionalInputDesc(ANTIQUANT_OFFSET_INDEX)->GetDataType() == optiling::DT_FLOAT);
...
bool IsMlaIfaOrMtp(optiling::TilingContext &context, const string inputLayoutStr, const int64_t queryS, const int64_t queryD)
{
...
bool IsSlidingAttention(optiling::TilingContext &context, const string inputLayoutStr, const int64_t queryD)
{
    if (context.GetOptionalInputTensor(QUERY_ROPE_INDEX) == nullptr) { // mla
        return false;
    }
...
static bool IsUsingIFA(optiling::TilingContext &context, const string inputLayoutStr, const uint32_t queryD, 
...
static bool TilingProcess4IFA(optiling::TilingContext *context)
{
    // IFA tiling path
...
static bool CheckOutShapeInDim3(const optiling::TilingContext *context, const string &outputLayoutStr, const gert::Shape outShape,
    const gert::Shape exceptOutShape)
{
    OP_CHECK_IF((outShape.GetDimNum() != DIM_NUM_3),
            ...
            outputLayoutStr.c_str(), outShape.GetDimNum()), return GRAPH_FAILED);
...
static bool CheckOutShapeInDim4(const optiling::TilingContext *context, const string &outputLayoutStr, const gert::Shape outShape,
    const gert::Shape exceptOutShape)
{
...
static bool CheckOutShape(optiling::TilingContext *context, const string &outputLayoutStr, const gert::Shape outShape, const gert::Shape qkvShapeInfo)
{
...
static bool GetB(const optiling::TilingContext *context, const string inputLayoutStr, int64_t &b)
{
    auto tempQ = context->GetInputShape(QUERY_INDEX);
...
static bool GetQueryN(const optiling::TilingContext *context, const string inputLayoutStr, int64_t &queryN)
{
    auto tempQ = context->GetInputShape(QUERY_INDEX);
    if (inputLayoutStr == "NSD" || 
    ...
    OP_CHECK_IF(queryN == 0, 
            OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "Q numhead is 0!"), return GRAPH_FAILED);
    return GRAPH_SUCCESS;
}

static bool GetQueryS(const optiling::TilingContext *context, const string inputLayoutStr, int64_t &queryS)
{
    auto tempQ = context->GetInputShape(QUERY_INDEX);
    if (inputLayoutStr == "NSD" || 
        inputLayoutStr == "BSH" || 
...
static bool GetQueryT(const optiling::TilingContext *context, const string inputLayoutStr, int64_t &queryT)
{
    auto tempQ = context->GetInputShape(QUERY_INDEX);
    if (inputLayoutStr == "TND" || 
...
static bool GetQueryD(const optiling::TilingContext *context, const string inputLayoutStr, int64_t &queryD)
{
    auto tempQ = context->GetInputShape(QUERY_INDEX);
    if (inputLayoutStr == "NSD" || 
        inputLayoutStr == "TND" || 
...
static bool GetPAValueD(const optiling::TilingContext *context, int64_t &valueD)
{
    auto tempV = context->GetInputShape(VALUE_INDEX);
...
static bool GetValueD(optiling::TilingContext *context, const string inputLayoutStr, int64_t &valueD, bool isPageAttention)
{
    if (isPageAttention) {
    ...
    if (inputLayoutStr == "NSD" || 
        ...
        OP_CHECK_IF((tempV->GetStorageShape().GetDimNum() != DIM_NUM_3),
                    ...
                    inputLayoutStr.c_str(), tempV->GetStorageShape().GetDimNum()), return GRAPH_FAILED);
...
static bool IsMla(const optiling::TilingContext *context)
{
    auto qRope = context->GetOptionalInputTensor(QUERY_ROPE_INDEX);
    auto kRope = context->GetOptionalInputTensor(KEY_ROPE_INDEX);
...
static bool CheckInputLayoutMla(const optiling::TilingContext *context, const string inputLayoutStr, const int64_t queryD, const bool isPageAttention)
{
    // MLA support layout
...
static bool CheckInputLayout(optiling::TilingContext *context, const string inputLayoutStr,const int64_t queryS, const int64_t queryD, const bool isPageAttention)
{
    auto tempQ = context->GetInputShape(QUERY_INDEX);
    auto tempK = context->GetInputShape(KEY_INDEX);
    auto tempV = context->GetInputShape(VALUE_INDEX);
    // mla check
    auto qRope = context->GetOptionalInputTensor(QUERY_ROPE_INDEX);
    if (IsMla(context) != GRAPH_SUCCESS) {
        return GRAPH_FAILED;
    }
    if (qRope != nullptr) {
        if (CheckInputLayoutMla(context, inputLayoutStr, queryD, isPageAttention) != GRAPH_SUCCESS) {
            return GRAPH_FAILED;
        }
    }
    ...
    if (inputLayoutStr == "BSH" || inputLayoutStr == "BSH_NBSD" || inputLayoutStr == "TND" || 
        ...
        OP_CHECK_IF((tempQ->GetStorageShape().GetDimNum() != DIM_NUM_3),
            ...
            inputLayoutStr.c_str(), tempQ->GetStorageShape().GetDimNum()), return GRAPH_FAILED);
    }
    else {
    ...
    if (isPageAttention == false) {
        if (inputLayoutStr == "TND" || inputLayoutStr == "NTD_TND") {
            OP_CHECK_IF(((tempK->GetStorageShape().GetDimNum() != DIM_NUM_3) || (tempV->GetStorageShape().GetDimNum() != DIM_NUM_3)),
                ...
                inputLayoutStr.c_str(), DIM_NUM_3, tempK->GetStorageShape().GetDimNum(), tempV->GetStorageShape().GetDimNum()), return GRAPH_FAILED);
        }
    }
    return GRAPH_SUCCESS;
}
...
bool TilingFusedInferAttentionScore(optiling::TilingContext *context)
{
    ...
    OP_CHECK_IF(attrs == nullptr, OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(),
        "Attributes returned from GetAttrs() is a nullptr"), return GRAPH_FAILED);
    ...
    int64_t valueD = 0;
    bool isPageAttention = context->GetOptionalInputShape(BLOCK_TABLE_INDEX) != nullptr ? true : false;
    if ((GetB(context, inputLayoutStr, b) != GRAPH_SUCCESS) ||
        (GetQueryN(context, inputLayoutStr, queryN) != GRAPH_SUCCESS) ||
        (GetQueryS(context, inputLayoutStr, queryS) != GRAPH_SUCCESS) ||
        (GetQueryT(context, inputLayoutStr, queryT) != GRAPH_SUCCESS) ||
        (GetValueD(context, inputLayoutStr, valueD, isPageAttention) != GRAPH_SUCCESS) ||
        (GetQueryD(context, inputLayoutStr, queryD) != GRAPH_SUCCESS)) {
        return GRAPH_FAILED;
    }
    // 校验intput
    OP_CHECK_IF(CheckInputLayout(context, inputLayoutStr, queryS, queryD, isPageAttention) != GRAPH_SUCCESS,
        OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "check InputLayout failed"), return GRAPH_FAILED);
    // 校验OutShape
    ...
    OP_CHECK_IF(CheckOutShape(context, outputLayoutStr, outShape, qkvShapeInfo) != GRAPH_SUCCESS,
        OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "check output shape failed"), return GRAPH_FAILED);
    // 是否路由到IFA
    bool usingIFA = IsUsingIFA(*context, inputLayoutStr, queryD, queryS);
    bool usingFAI = IsUsingFAI(*context, inputLayoutStr, queryD);
    if (usingFAI) {
        OP_CHECK_IF(TilingProcess4SplitFuse(context) != GRAPH_SUCCESS,
            OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "tiling process for split fuse failed"),
            return GRAPH_FAILED);
    } else if (usingIFA) { // IFA tiling process
        OP_CHECK_IF(TilingProcess4IFA(context) != GRAPH_SUCCESS,
            OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "tiling process for ifa failed"),
            return GRAPH_FAILED);
    } else { // PFA tiling process
        OP_CHECK_IF(TilingProcess4PFA(context, static_cast<uint32_t>(queryD), b, queryS, queryN) != GRAPH_SUCCESS,
...
extern "C" {
__attribute__((visibility("default"))) bool DeviceDoOpTilingIncreFlashAttention(optiling::TilingContext *context)
{
    return TilingIncreFlashAttention(context);
}
__attribute__((visibility("default"))) bool DeviceDoOpTilingFusedInferAttentionScore(
