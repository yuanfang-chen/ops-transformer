...
{
    m.def("updateContext(c10::string_view group_ep, int64_t ep_world_size) -> Tensor");
}
...
*/
at::Tensor update_context(c10::string_view group_ep, int64_t ep_world_size)
{
    std::string group_ep_str = std::string(group_ep);
    Mc2ContextStru mc2ContextHost[2];
    int32_t ret = GetMc2Context(mc2ContextHost, ep_world_size, group_ep_str.c_str());
