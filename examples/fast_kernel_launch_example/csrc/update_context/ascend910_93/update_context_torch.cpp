...
static int32_t CreatMc2Context(HcclComm &comm, int64_t worldSize, int64_t cclBufferSize, Mc2ContextStru &mc2Context)
{
    ...
    for (uint64_t remoteRankId = 0; remoteRankId < worldSize; remoteRankId++) {
        ...
        if (rankId == remoteRankId) {
            ret = static_cast<HcclResult>(HcclGetHcclBuffer(comm, &remoteAddr, &commSize));  // 获取本卡地址
        } else {
            ret = static_cast<HcclResult>(HcclGetRemoteIpcHcclBuf(comm, remoteRankId, &remoteAddr, &commSize)); // 获取远端地址
        }
...
{
    HcclResult ret = static_cast<HcclResult>(HcclKfcOpArgsSetAlgConfig(opArgs, const_cast<char *>(algConfig.c_str())));  // 设置通信类型
    TORCH_CHECK(ret == 0, "HcclKfcOpArgsSetAlgConfig failed, ret:", ret);
    ret = static_cast<HcclResult>(HcclCommGetHandleWithName(groupName, &commHandle));  // 通过groupName获取groupHandle
    TORCH_CHECK(ret == 0, "HcclGetCommHandle failed, ret:", ret);
    void *opsResCtx;
    ret = static_cast<HcclResult>(HcclCreateOpResCtx(commHandle, opType, opArgs, &opsResCtx)); // 创建HcclContext
    TORCH_CHECK(ret == 0, "HcclCreateOpResCtx failed, ret:", ret);

    // Get Comm world Size and rank
    uint32_t rankId = 0;
    uint32_t worldSizeHccl = 0;
    ret = static_cast<HcclResult>(HcclGetRankSize(commHandle, &worldSizeHccl)); // 获取通信域大小
    TORCH_CHECK(ret == HCCL_SUCCESS, "HcclGetRankSize failed, ret:", ret);
    ret = static_cast<HcclResult>(HcclGetRankId(commHandle, &rankId)); // 获取卡号
...
{
    void* opArgs = nullptr;
    HcclResult ret = static_cast<HcclResult>(HcclKfcAllocOpArgs(&opArgs));  // 通信配置对象创建
    TORCH_CHECK(ret == 0, "HcclKfcAllocOpArgs failed, ret:", ret);
    uint8_t commEngine = COMM_ENGINE_AIV;
    ret = static_cast<HcclResult>(HcclKfcOpArgsSetCommEngine(opArgs, (uint8_t)commEngine)); // 设置通信方式
    ...
    ret = static_cast<HcclResult>(HcclKfcFreeOpArgs(opArgs)); // 释放通信配置对象
