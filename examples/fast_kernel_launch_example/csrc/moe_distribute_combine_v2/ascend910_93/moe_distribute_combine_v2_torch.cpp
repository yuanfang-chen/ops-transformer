...
{
    m.def("MoeDistributeCombineV2(Tensor expand_x, Tensor expert_ids, Tensor expand_idx, Tensor ep_send_counts, " \
            "Tensor expert_scales, Tensor mc2_context, str group_ep, " \
            "int ep_world_size, int ep_rank_id, Tensor? tp_send_counts, Tensor? x_active_mask, " \
            "Tensor? shared_expert_x, Tensor? elastic_info, Tensor? ori_x, " \
            "Tensor? const_expert_alpha_1, Tensor? const_expert_alpha_2, Tensor? const_expert_v, " \
            "Tensor? performance_info, str group_tp, int tp_world_size, int tp_rank_id, " \
            "int expert_shard_type, int shared_expert_num, int shared_expert_rank_num, " \
            "int global_bs, int comm_quant_mode, " \
...
static void CalTilingKey(int32_t &tilingKey, at::ScalarType xType, const bool isScales, const uint32_t quantMode)
{
...
void MoeDistributeCombineV2_api(aclrtStream stream,
                                ...
                                at::Tensor &y_out, int64_t comm_quat_mode,
                                MoeDistributeCombineV2Info tilingData)
{
    ...
    void* expandScales_ptr = nullptr;
    if(expand_scales.has_value()) {
        expandScales_ptr = get_first_tensor_address<c10::optional<at::Tensor>>(expand_scales->scalar_type(), expand_scales, false);
    }
    ...
    auto yOut_ptr = get_first_tensor_address<at::Tensor>(y_out.scalar_type(), y_out, false);
    ...
    (GM_ADDR)elasticInfo_ptr, (GM_ADDR)oriX_ptr, (GM_ADDR)constExpertAlpha1_ptr, (GM_ADDR)constExpertAlpha2_ptr,
    (GM_ADDR)constExpertV_ptr, (GM_ADDR)performanceInfo_ptr, (GM_ADDR)yOut_ptr,
    ...
    c10::string_view comm_alg, int64_t zero_expert_num, int64_t copy_expert_num, int64_t const_expert_num)
{
    ...
    tilingData.moeExpertPerRankNum = moe_expert_per_rank_num;
    ...
    tilingData.hasElasticInfo = elastic_info.has_value();
    ...
    auto acl_call = [=]() mutable -> int {
        MoeDistributeCombineV2_api(stream, expand_x, expert_ids, assist_info_for_combine, ep_send_counts,
            ...
            expand_scales, shared_expert_x, elastic_info, ori_x, const_expert_alpha_1, const_expert_alpha_2,
...
at::Tensor npu_moe_distribute_combine_v2_meta(
    ...
    const at::Tensor &ep_send_counts, const at::Tensor &mc2_context,
