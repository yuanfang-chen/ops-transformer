...
TORCH_LIBRARY_FRAGMENT(EXTENSION_MODULE_NAME, m)
{
    m.def("MoeDistributeDispatchV2(Tensor x, Tensor expert_ids, Tensor mc2_context, str group_ep, int ep_world_size, " \
            ...
            "Tensor? expert_scales=None, Tensor? performance_info=None, str group_tp=\"\", "\
            "int tp_world_size=0, int tp_rank_id=0, int expert_shard_type=0, int shared_expert_num=0, " \
...
static void calculate_tilingkey(int32_t &tilingKey, at::ScalarType xType, const bool isScales, const uint32_t quantMode, const bool isSetCommAlg)
{
...
void moe_distribute_dispatch_v2_api(aclrtStream stream, bool is_fullmesh_v2, const at::Tensor &x, const at::Tensor &expert_ids, const at::Tensor &new_workspace, const at::Tensor &mc2_context,
    ...
    const c10::optional<at::Tensor> &performance_info)
{
    ...
    tilingData.tpWorldSize = tp_world_size;                // tpWorldSize
    tilingData.epRankId = ep_rank_id;                   // epRankId
    tilingData.tpRankId = tp_rank_id;                   // tpRankId
                                           ...
                                           c10::string_view comm_alg, int64_t zero_expert_num, int64_t copy_expert_num, int64_t const_expert_num)
{
    ...
    calculate_tilingdata(tilingData, ep_world_size, ep_rank_id, moe_expert_num, total_winsize_ep, tp_world_size, tp_rank_id,
    ...
    auto acl_call = [=]() mutable -> int {
        moe_distribute_dispatch_v2_api(stream, is_fullmesh_v2, x, expert_ids, new_workspace, mc2_context, group_ep, ep_world_size, ep_rank_id, moe_expert_num, scales,
                                    x_active_mask, expert_scales, performance_info, expand_x,
