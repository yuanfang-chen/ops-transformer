                                           ...
                                           c10::string_view comm_alg, int64_t zero_expert_num, int64_t copy_expert_num, int64_t const_expert_num)
{
    ...
    tilingData.moeDistributeDispatchV2Info.aivNum = ascendcPlatform->GetCoreNumAic();                     // aivNum
    ...
    tilingData.moeDistributeDispatchV2Info.totalWinSizeEp = totalWinSizeEp;
    ...
    // 初始化通信域配置
    AscendC::Mc2CcTilingConfig mc2CcTilingConfig(group_ep_ptr, ep_world_size, "AlltoAll=level0:fullmesh;level1:pairwise");
    // SetCommEngine: 3 (MTE通信方式需要配置)
    mc2CcTilingConfig.SetCommEngine(3);
    int ret = mc2CcTilingConfig.GetTiling(tilingData.mc2InitTiling);
    TORCH_CHECK(ret == ACL_SUCCESS, "[ERROR] GetTiling mc2InitTiling failed. ret = %d\n", ret);
    ret = mc2CcTilingConfig.GetTiling(tilingData.mc2CcTiling);
    TORCH_CHECK(ret == ACL_SUCCESS, "[ERROR] GetTiling mc2CcTiling failed. ret = %d\n", ret);
    ret = mc2CcTilingConfig.GetTiling(tilingData.mc2CcTiling1);
    TORCH_CHECK(ret == ACL_SUCCESS, "[ERROR] GetTiling mc2CcTiling1 failed. ret = %d\n", ret);
