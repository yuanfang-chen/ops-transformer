...
{
    m.def("MoeDistributeDispatchV2(Tensor x, Tensor expert_ids, str group_ep, int ep_world_size, " \
...
static void CalTilingKey(uint64_t &tilingKey, at::ScalarType xType, const bool isScales, const uint32_t quantMode, const bool isSetCommAlg)
{
...
void MoeDistributeDispatchV2_api(aclrtStream stream, bool is_fullmesh_v2, const at::Tensor &x, const at::Tensor &expert_ids,
                                ...
                                at::Tensor mc2Context, MoeDistributeDispatchV2TilingData tilingData)
{
    ...
    auto expandScalesOut_ptr =
        get_first_tensor_address<at::Tensor>(assist_info_forcombine.scalar_type(), assist_info_forcombine, false);
    auto mc2Context_ptr =
        get_first_tensor_address<at::Tensor>(mc2Context.scalar_type(), mc2Context, false);
    ...
    CalTilingKey(tilingKey, scales.has_value(), tilingData.moeDistributeDispatchV2Info.quantMode, is_fullmesh_v2);
    void *tilingDataGM = nullptr;
    aclError ret = aclrtMalloc(&tilingDataGM, sizeof(MoeDistributeDispatchV2TilingData), ACL_MEM_MALLOC_HUGE_FIRST);
    if (ret != ACL_SUCCESS) {
        /* 错误处理 */
    }
    ret = aclrtMemcpy(
        tilingDataGM, 
        sizeof(MoeDistributeDispatchV2TilingData),
        &tilingData,  // 主机内存地址
        sizeof(MoeDistributeDispatchV2TilingData),
    ...
    moe_distribute_dispatch_v2_demo(tilingKey, tilingData.moeDistributeDispatchV2Info.aivNum, (void*)stream, (GM_ADDR)x_ptr,
        ...
        (GM_ADDR)expandScalesOut_ptr, (GM_ADDR)workspace_ptr, (GM_ADDR)mc2Context_ptr, (GM_ADDR)tilingDataGM);
}

std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> npu_moe_distribute_dispatch_v2(const at::Tensor &x, const at::Tensor &expert_ids,
                                           c10::string_view group_ep, int64_t ep_world_size, int64_t ep_rank_id,
                                           ...
                                           c10::string_view comm_alg, int64_t zero_expert_num, int64_t copy_expert_num, int64_t const_expert_num)
{
    ...
    auto context = update_context(group_ep_ptr, ep_world_size);
    // at::Tensor context;
    uint64_t workspaceSize = 16U * 1024U * 1024U;
    void *workspace_ptr = nullptr;
    ...
    auto acl_call = [=]() mutable -> int {
        MoeDistributeDispatchV2_api(stream, is_fullmesh_v2, x, expert_ids, group_ep, ep_world_size, ep_rank_id, moe_expert_num, scales,
