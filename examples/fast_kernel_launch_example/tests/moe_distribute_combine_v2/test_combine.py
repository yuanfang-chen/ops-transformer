...
class Config:
    ...
    def __init__(self):
        ...
        self.ep_world_size = self.world_size // self.tp_world_size
        ...
        self.total_winsize_ep = HCCL_BUFFER_SIZE * 1024 * 1024
...
def init_dist(rank):
    ...
    ep_group, tp_group = create_process_groups(global_rank)
    return get_hccl_name(global_rank, ep_group), get_hccl_name(global_rank, tp_group), global_rank


def run_npu_process(rank):
    """Run MoE operator test on NPU."""
    torch_npu.npu.set_device(rank)
    ep_hcomm, tp_hcomm, global_rank = init_dist(rank)
    
    workspace = torch.empty(TARGET_SIZE_BYTES // 4, dtype=torch.float32)
    mc2_context = torch.ops.ascend_ops.updateContext(
        workspace.to(torch.float32).npu(), ep_hcomm, 12 * 1024 * 1024 * 1024, cfg.ep_world_size
    )
    ...
    dispatch_fn = torch.ops.ascend_ops.MoeDistributeDispatchV2
    expand_x, _, assist_info, _, ep_recv_counts, _, _ = dispatch_fn(
        x=x, group_ep=ep_hcomm, expert_ids=topk_idx,
        ep_world_size=cfg.ep_world_size, ep_rank_id=0,
        shared_expert_num=0, shared_expert_rank_num=cfg.shared_expert_rank_num,
        ...
        mc2_context=mc2_context, total_winsize_ep=cfg.total_winsize_ep,
    )
    
    if cfg.is_quant:
        expand_x = expand_x.to(cfg.input_dtype)
    
    combine_fn = torch.ops.ascend_ops.MoeDistributeCombineV2
    _ = combine_fn(
        expand_x=expand_x, expert_ids=topk_idx, group_ep=ep_hcomm,
        ep_world_size=cfg.ep_world_size, ep_rank_id=0,
        ...
        copy_expert_num=cfg.copy_expert_num, const_expert_num=cfg.const_expert_num,
        mc2_context=mc2_context, total_winsize_ep=12 * 1024 * 1024 * 1024,
    )
    
    print(f"rank {global_rank} epid {global_rank // cfg.tp_world_size} tpid {global_rank % cfg.tp_world_size} npu finished!\n")


def validate_config():
    """Validate configuration parameters."""
    if cfg.tp_world_size != 1 and cfg.local_num_experts > 1:
        print("Unsupported: tp = 2 and local moe > 1"); return False
...
if __name__ == "__main__":
    ...
    print(f"quant_mode={cfg.quant_mode}, tp={cfg.tp_world_size}, ep={cfg.ep_world_size}")
