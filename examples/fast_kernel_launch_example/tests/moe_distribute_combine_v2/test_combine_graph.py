...
TARGET_SIZE_BYTES = 16 * BYTES_PER_MB
...
class Config:
    ...
    dispatch_comm_alg: str = "fullmesh_v2"
    total_winsize_ep: int = HCCL_BUFFER_SIZE * 1024 * 1024
    aic_core_num: int = 24
    aiv_core_num: int = 48
    tp_world_size: int = 1
    rank_per_dev: int = 16
    ...
    def forward(self, **kwargs) -> torch.Tensor:
        with torchair.scope.limit_core_num(cfg.aic_core_num, cfg.aiv_core_num):
            ...
            out = dispatch_fn(
                ...
                x_active_mask=kwargs['x_active_mask'], mc2_context=kwargs['mc2_context'],
                group_ep=kwargs['group_ep'], group_tp=kwargs['group_tp'],
                ep_rank_id=kwargs['ep_rank_id'], tp_rank_id=kwargs['tp_rank_id'],
                ep_world_size=cfg.ep_world_size, tp_world_size=cfg.tp_world_size,
                ...
                total_winsize_ep=cfg.total_winsize_ep,
            ...
            return combine_fn(
                ...
                ep_send_counts=out[4], tp_send_counts=out[5],
                expert_scales=kwargs['expert_scales'],
                x_active_mask=kwargs['x_active_mask'],
                ori_x=None, group_ep=kwargs['group_ep'],
                group_tp=kwargs['group_tp'],
                ep_rank_id=kwargs['ep_rank_id'],
                tp_rank_id=kwargs['tp_rank_id'],
                ...
                mc2_context=kwargs['mc2_context'],
                total_winsize_ep=cfg.total_winsize_ep,
                comm_alg="",
            )
...
def build_model_kwargs(x, expert_ids, x_active_mask, group_ep, group_tp, ep_rank_id, tp_rank_id, mc2_context, expert_scales):
...
def run_cascade_npu(queue, rank, x, expert_ids, x_active_mask, expert_scales, ori_x):
    ...
    ep_hcomm, tp_hcomm, _, _ = init_hccl_comm(rank, ep_ranks, tp_ranks)
    
    x_context = torch.empty(TARGET_SIZE_BYTES // 4, dtype=torch.int32).npu()
    mc2_context = torch.ops.ascend_ops.updateContext(x_context, ep_hcomm, cfg.total_winsize_ep, cfg.ep_world_size)
    
    kwargs = build_model_kwargs(x, expert_ids, x_active_mask, ep_hcomm, tp_hcomm,
                                 rank // cfg.tp_world_size, rank % cfg.tp_world_size, mc2_context, expert_scales)
    
    model = compile_model(MOECascadeModel())
    _ = model(**kwargs)
