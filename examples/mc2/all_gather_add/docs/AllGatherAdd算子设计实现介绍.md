...
### 1.1 算子逻辑
算子逻辑为：对输入的通信操作数a做AllGather通信，得到Add计算的被加数，即通信结果gather_out，将gather_out和加数b做Add运算得到输出c。
- 对应的数学表达式：
...
根据对算子输入输出的分析，确定核函数的参数aGM，bGM，cGM，gatherOutGM；aGM，bGM为输入在Global Memory上的内存地址，cGM，gatherOutGM为输出在Global Memory上的内存地址。注意，核函数的参数和单算子API调用的输入输出在命名上存在区别，原因是核函数的参数是输入输出在Global Memory上的内存地址，而单算子API调用时输入输出的类型是aclTensor，两者并不完全一致，例如：
...
## 2. 数据流分析
AllGather的功能为将通信域内所有卡的输入按照卡id重新排序，然后拼接起来，再将结果发送到所有卡。Add操作将每张卡收到的AllGather结果与本卡的b输入数据进行相加。
本样例通信域内卡数rank_size固定为2，若通信不切分轮次，通信计算串行进行，算子语义示意图如下：

![alt text](../docs/figures/AllGatherAdd算子语义示意图.png)

AllGatherAdd算子的数据在卡间进行AllGather通信，在卡内进行Add计算，通信计算部分可以并行进行互不影响。因此，可以将通信数据切分为块，每次计算对前一轮的通信结果进行操作，流水互相掩盖，则可得到通信计算掩盖示意图如下：
...
AllGatherAdd算子逻辑：（comm_turn=2时）

- AI Core将要执行的通信信息写入Global Memory中的消息区，实现任务下发。消息区是特定地址的Global Memory，AI Core和AI CPU通过向其写入和轮询读取来实现消息在两者间的传递，这些操作统一封装于Hccl高阶API中。

- AI CPU从消息区读取到所有通信任务信息，开始基于HCCS（华为缓存一致性系统，用于CPU/NPU之间的高速互联）或RoCE（承载在融合以太网上的RDMA技术，即跨越以太网的RDMA通信方式）等链路执行第一轮AllGather集合通信任务。下图为第一轮AllGather通信示意图。

    ![alt text](../docs/figures/第一轮AllGather通信示意图.png)

- AI CPU完成第一轮通信任务后，向消息区写入第一轮通信任务已完成的消息，并开始执行第二轮通信任务。同时，AI Vector开始对第一轮AllGather通信结果进行Add计算。下图为第二轮通信和rank0上第一轮Add计算的示意图。

    ![alt text](../docs/figures/第二轮通信和第一轮Add计算示意图.png)

- 按照通信切分轮次逐步完成所有数据块的通信和计算。
...
Add多核切分:
多核切分: 根据当前核数(rank_size = 2)，对输入shape的X轴进行多核切分，得到单核内shape的大小。
...
```
context为[TilingContext](https://www.hiascend.com/document/detail/zh/canncommercial/850/API/basicdataapi/atlasopapi_07_00555.html)的对象指针，该指针由/op_host/all_gather_add_tiling文件的Tiling入口函数AllGatherAddTilingFunc传入，用于保存算子Tiling计算的上下文。在AllGatherAdd算子的Tiling实现中，通过该上下文context获取计算Tiling所需要的输入输出shape、输入属性等参数，然后将Tiling结果（例如TilingKey、TilingData）保存至上下文中，供后续算子执行时使用。
...
## 6. Kernel实现
在AllGatherAdd算子的Kernel实现中，需要按照通信切分轮次对所有卡的数据执行AllGather通信计算，同时需要通信分块数据分轮次进行Add运算。多轮通信场景下，多张卡的数据拼接到一张卡时，相邻数据块地址并不连续，而每轮Add运算仅对上一轮的通信结果进行计算，因此需要考虑Add计算所需的基本信息。Kernel侧计算所需的信息如下：
- 输入a、b操作数和输出c的地址。
...
```
- Add计算依赖AIV核，因此算子逻辑仅运行于AIV核中。通过KERNEL_TYPE_MIX_AIV_1_0宏[设置kernel类型](https://www.hiascend.com/document/detail/zh/canncommercial/850/API/ascendcopapi/atlasascendc_api_07_0218.html)，AIC、AIV混合场景下，控制算子执行时仅启动AI Core上的Vector核。
    ```Cpp
...
通过工程编译脚本完成算子的编译后，会自动生成单算子API，可以直接在应用程序中调用。本节介绍op_host目录下如何对API生成的两段式aclnn接口进行封装，并在examples目录下的test_aclnn_all_gather_add.cpp文件中进行调用并完成算子执行的。

首先，对框架自动生成的aclnnInnerAllGatherAddGetWorkspaceSize和aclnnInnerAllGatherAdd函数进行封装，在[aclnn_all_gather_add.h](../op_host/op_api/aclnn_all_gather_add.h)和[aclnn_all_gather_add.cpp](../op_host/op_api/aclnn_all_gather_add.cpp)编写两段式接口。
