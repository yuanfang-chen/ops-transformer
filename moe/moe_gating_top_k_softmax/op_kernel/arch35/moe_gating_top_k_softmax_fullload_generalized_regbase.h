    ...
    if (k_ == 1 && expertCount_ <= B32_BLOCK_COUNT) {
        __VEC_SCOPE__ {
            ...
            for (uint16_t i = 0; i < static_cast<uint16_t>(rowCount); i++) {
                ...
                DataCopy(sortedTensorAddr + expertCountAlign_ * KEY_VALUE_FACTOR * i, valueAndIndexReg, maskForValueAndIndex);
            }
        }
    } else {
    ...
    int64_t rowCount)
{
    ...
    if (expertCountAlign_ == ONE_REPEAT_SORT_NUM) {
        HandleOneRepeatSortNum(rowCount, softmaxTensor, sortedTensor, sortTmpTensor, expertIdxTensor);
    } else {
