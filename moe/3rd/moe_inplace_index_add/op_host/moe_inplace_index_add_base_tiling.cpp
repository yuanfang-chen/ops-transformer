...
{
    // OP_LOG_FULL(DLOG_DEBUG, context_->GetNodeName(), "Tiling: %s", optiling::DebugTilingContext(context_).c_str());
...
ge::graphStatus MoeInplaceIndexAddTiling::CheckInputDtype()
{
    ...
    OP_TILING_CHECK((VAR_DTYPE.find(dtype_) == VAR_DTYPE.end()),
...
int16, bool, int64, bfloat16 currently, but got [%s].",
    ...
    OP_TILING_CHECK(varTypeSize_ <= 0, VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(), "get dataType size fail."),
                    return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(
        (updatesDtype != dtype_),
        VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(), "updates [%s] and var [%s] must have the same dtype.",
...
ge::graphStatus MoeInplaceIndexAddTiling::CheckInputShape()
{
    ...
    if (alphaShapePtr != nullptr) {
        ...
        OP_TILING_CHECK(alphaShape.GetShapeSize() != 1,
                        VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(), "the alpha size must be 1."),
                        return ge::GRAPH_FAILED);
    }

    OP_TILING_CHECK((indicesDimNum > 1),
                    VECTOR_INNER_ERR_REPORT_TILIING(
