...
int64_t MoeInplaceIndexAddSimdSortTiling::GetIndicesAlignBlockSize(int64_t indicesFactor)
{
    ...
    */ 
    ...
    int64_t occupy = CeilAlign(indicesFactor * indicesTypeSize_, ubBlock) +
                    (CeilAlign(indicesFactor * (indicesTypeSize_), ubBlock) + SIZE_TWO * ALIGN_SIZE) +
                    (CeilAlign(indicesFactor * INT32_BYTES, ubBlock) + ALIGN_SIZE) +
                    (CeilAlign(indicesFactor * indicesTypeSize_, ubBlock) + ALIGN_SIZE) + 
                     CeilAlign(indicesFactor * INT32_BYTES, ubBlock) +
...
int64_t MoeInplaceIndexAddSimdSortTiling::GetAfterAlignBlockSize(int64_t indicesFactor, int64_t afterFactor)
{
    ...
    auto ubBlock = Ops::Base::GetUbBlockSize(context_);
    int64_t occupy = indicesFactor * CeilAlign((varTypeSize_) * afterFactor, ubBlock) + 
                     indicesFactor * CeilAlign((FP32_BYTES) * afterFactor, ubBlock);
    return occupy;
}
...
void MoeInplaceIndexAddSimdSortTiling::DoOpTilingSplitAfterSort()
{
    ...
    */
    int64_t occupy = CeilAlign(indicesTypeSize_, ubBlock) +
                     CeilAlign(indicesTypeSize_ + SIZE_TWO * ALIGN_SIZE, ubBlock) + 
                     CeilAlign(INT32_BYTES, ubBlock) + 
                     CeilAlign(INT32_BYTES * SIZE_TWO, ubBlock) +
                     CeilAlign(indicesTypeSize_, ubBlock) + 
                     CeilAlign(varTypeSize_ * eachCoreAfterAxisCount_, ubBlock) + 
                     CeilAlign(FP32_BYTES * eachCoreAfterAxisCount_, ubBlock) + 
                     GetSortTmpSize(indicesDtype_, 1, false);
    int64_t indicesSize = CeilAlign(indicesTypeSize_, ubBlock) +
                          CeilAlign(indicesTypeSize_ + SIZE_TWO * ALIGN_SIZE, ubBlock) + 
                          CeilAlign(INT32_BYTES, ubBlock) + 
                          CeilAlign(INT32_BYTES * SIZE_TWO, ubBlock) +
                          CeilAlign(indicesTypeSize_, ubBlock) +
    ...
    if (occupy > halfUbSize) {
        ...
        while (restSize <= 0) {
            --ubIndexFactor_;
            restSize = halfUbSize - (CeilAlign(ubIndexFactor_ * indicesTypeSize_, ubBlock) +
                                    CeilAlign(ubIndexFactor_ * (indicesTypeSize_ + SIZE_TWO * ALIGN_SIZE), ubBlock) + 
                                    CeilAlign(ubIndexFactor_ * INT32_BYTES, ubBlock) + 
                                    CeilAlign(ubIndexFactor_ * (INT32_BYTES * SIZE_TWO), ubBlock) +
                                    CeilAlign(ubIndexFactor_ * indicesTypeSize_, ubBlock) + 
                                    ubIndexFactor_ * CeilAlign((FP32_BYTES) * eachCoreAfterAxisCount_, ubBlock) + 
                                    ubIndexFactor_ * CeilAlign((varTypeSize_) * eachCoreAfterAxisCount_, ubBlock) + 
...
void MoeInplaceIndexAddSimdSortTiling::DoOpTilingSplitPreSort()
{
    ...
    */
    ...
    int64_t occupy = CeilAlign(indicesTypeSize_, ubBlock) +
                     ...
                     CeilAlign(INT32_BYTES * SIZE_TWO, ubBlock) +
                     CeilAlign(indicesTypeSize_, ubBlock) + 
                     CeilAlign(varTypeSize_ * afterAxis_, ubBlock) + 
                     CeilAlign(FP32_BYTES * afterAxis_, ubBlock) + 
                     GetSortTmpSize(indicesDtype_, 1, false);
    if (occupy > halfUbSize) {
        int64_t indicesUbSize = std::min(INDICES_MIN_BLOCK_SIZE, indicesAxis_ * indicesSize);
        /* indicesBuf_ + outOfstBuf_ */
