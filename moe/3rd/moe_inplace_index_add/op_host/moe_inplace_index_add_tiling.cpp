...
static int64_t GetCeilInt(int64_t value1, int64_t value2) {
  OP_TILING_CHECK(value2 == 0,
                  VECTOR_INNER_ERR_REPORT_TILIING("MoeInplaceIndexAdd",
...
static bool CalRunningCaseAtomicAdd(const MoeInplaceIndexAddCompileInfo *compile_info,
  ...
  if (params->indices_num < compile_info->core_num) {
    ...
    params->tail_num = params->full_num_per_block;
    OP_TILING_CHECK(params->block_num == 0,
                  VECTOR_INNER_ERR_REPORT_TILIING("MoeInplaceIndexAdd",
...
static bool InitRunningInfo(MoeInplaceIndexAddTilingData* params,
  ...
  OP_TILING_CHECK(params->indices_num == 0,
                  VECTOR_INNER_ERR_REPORT_TILIING("MoeInplaceIndexAdd",
...
static bool CalRunningInfo(const MoeInplaceIndexAddCompileInfo *compile_info,
  ...
  if (dtype == DT_BF16) {
    ...
    OP_TILING_CHECK(!CalRunningCaseAtomicAdd(compile_info, params),
                    VECTOR_INNER_ERR_REPORT_TILIING("MoeInplaceIndexAdd", "CalRunningCaseAtomicAdd failed."),
                    return false);
...
ge::graphStatus Tiling4MoeInplaceIndexAdd(gert::TilingContext* context) {
  ...
  if (alpha_shape != nullptr) {
    ...
    OP_TILING_CHECK(!(alpha_shape_val.GetDimNum() == 1 && alpha_shape_val.GetDim(0) == 1) || alpha_shape_val.IsScalar(),
                    VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "shape of alpha must be (1, )"),
  ...
  OP_TILING_CHECK(!InitRunningInfo(params, indices_shape_val, axis_val),
                  VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "InitRunningInfo failed."),
                  return ge::GRAPH_FAILED);
  OP_TILING_CHECK(!CalRunningInfo(compile_info, params, var_shape_val, updates_shape_val, axis_val, dtype),
  ...
  OP_TILING_CHECK(workspaces == nullptr,
              VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "workspaces is null."),
              return ge::GRAPH_FAILED);
  if (dtype == ge::DT_BF16) {
...
ge::graphStatus TilingPrepareMoeInplaceIndexAddForAscendC(gert::TilingParseContext *context)
{
    ...
    OP_TILING_CHECK((compileInfo->coreNum <= 0),
        VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "Failed to core num."), return ge::GRAPH_FAILED);
    uint64_t ubSize;
    ascendcPlatform.GetCoreMemSize(platform_ascendc::CoreMemType::UB, ubSize);
    compileInfo->ubSize = static_cast<int64_t>(ubSize);
    OP_TILING_CHECK((compileInfo->ubSize <= 0),
        VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "Failed to get ub size."), return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus TilingPrepare4MoeInplaceIndexAdd(gert::TilingParseContext* context) {
  ...
  OP_TILING_CHECK(compile_info == nullptr || parsed_object_cinfo == nullptr,
                  VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "compile_info or json_str nullptr!"),
                  return ge::GRAPH_FAILED);
  ...
  OP_TILING_CHECK(vars.empty(), VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "get vars failed."),
                  return ge::GRAPH_FAILED);
  uint32_t core_num = 0;
  OP_TILING_CHECK(!MoeGetTilingCoreNum(context, core_num),
  ...
  OP_TILING_CHECK(!optiling::MoeReadCompileItem(vars, "ub_size", compile_info->ub_size),
      VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "GetCompileParams, get ub_size failed."),
      return ge::GRAPH_FAILED);
  OP_TILING_CHECK(!optiling::MoeReadCompileItem(vars, "var_size", compile_info->var_size),
      VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "GetCompileParams, get var_size failed."),
      return ge::GRAPH_FAILED);
  OP_TILING_CHECK(!optiling::MoeReadCompileItem(vars, "var_data_each_block", compile_info->var_data_each_block),
      VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "GetCompileParams, get var_data_each_block failed."),
      return ge::GRAPH_FAILED);
  OP_TILING_CHECK(!optiling::MoeReadCompileItem(vars, "indices_size", compile_info->indices_size),
      VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "GetCompileParams, get indices_size failed."),
      return ge::GRAPH_FAILED);
  OP_TILING_CHECK(!optiling::MoeReadCompileItem(vars, "indices_data_each_block", compile_info->indices_data_each_block),
      VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "GetCompileParams, get indices_data_each_block failed."),
      return ge::GRAPH_FAILED);
  OP_TILING_CHECK(!optiling::MoeReadCompileItem(vars, "soc_version", compile_info->soc_version),
      VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "GetCompileParams, get soc_version failed."),
      return ge::GRAPH_FAILED);
  OP_TILING_CHECK(!optiling::MoeReadCompileItem(vars, "atomic_add", compile_info->atomic_add),
      VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(), "GetCompileParams, get atomic_add failed."),
      return ge::GRAPH_FAILED);
  return ge::GRAPH_SUCCESS;
}
