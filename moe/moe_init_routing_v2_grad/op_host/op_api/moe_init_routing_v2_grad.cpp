...
const aclTensor* MoeInitRoutingV2Grad(
    ...
    L0_DFX(MoeInitRoutingV2Grad, grad_expanded_x, expanded_row_idx, top_k, drop_pad_mode, active_num);

    // Get input shapes
    op::Shape gradExpandedXShape = grad_expanded_x->GetViewShape();
    op::Shape expandedRowIdxShape = expanded_row_idx->GetViewShape();

    // Get dimensions
    int64_t expandedRowIdxDim0 = (expandedRowIdxShape.GetDim(0) == ge::UNKNOWN_DIM_NUM) ?
        -1 : expandedRowIdxShape.GetDim(0);
    ...
    // Infer grad_x shape
    op::Shape outShapeGradX;
    outShapeGradX.SetDimNum(0);

    int64_t gradXDim0 = (expandedRowIdxDim0 == -1) ? -1 : expandedRowIdxDim0 / top_k;
    int64_t gradXDim1 = gradExpandedXDim1;

    outShapeGradX.AppendDim(gradXDim0);
    outShapeGradX.AppendDim(gradXDim1);

    auto grad_x = executor->AllocTensor(outShapeGradX, grad_expanded_x->GetDataType(), op::Format::FORMAT_ND);
