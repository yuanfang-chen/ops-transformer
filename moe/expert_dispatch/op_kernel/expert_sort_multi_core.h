...
/*!
 * \file expert_sort_multi_core.h
 * \brief
 */
#ifndef EXPERT_SORT_MULTI_CORE_H
#define EXPERT_SORT_MULTI_CORE_H

#include "expert_sort_base.h"
...
class ExpertSortMultiCore : public ExpertSortBase
{
public:
    __aicore__ inline ExpertSortMultiCore(){};
    __aicore__ inline void Init(GM_ADDR expertId, GM_ADDR expendedRowIdx, GM_ADDR workspace,
...
private:
    ...
    __aicore__ inline void InitExpertMrgSort(ExpertMrgsort* sorter, int64_t listNum, int64_t coreOffset,
                                             int64_t loopOffset);
    __aicore__ inline void InitExpertMrgSortOut(ExpertMrgsortOut* sorter, int64_t listNum, int64_t coreOffset);

private:
    GlobalTensor<float> workspaceGms[2];

    const ExpertVBSComputeTilingData* vbsTilingData;
    const ExpertVMSMiddleComputeTilingData* vmsTilingData;
    const ExpertSortOutComputeTilingData* sortOutTilingData;

    // for ExpertMrgsort
    ExpertMrgsort mrgsorter;
    ExpertMrgsortParam mrgsortParam;
...
__aicore__ inline void ExpertSortMultiCore::VBSCopyIn(int64_t progress, int64_t size, int64_t sortNum)
{
    ...
    DataCopyPad(inLocal[0], expertIdGm[inOffset], dataCopyParams, dataCopyPadParams);
...
__aicore__ inline void ExpertSortMultiCore::UBSortCompute(int64_t progress, int64_t size, int64_t sortNum)
{
...
__aicore__ inline void ExpertSortMultiCore::VBSCopyOut(int64_t progress, int64_t size, int64_t sortNum)
{
...
__aicore__ inline void ExpertSortMultiCore::InitExpertMrgSort(ExpertMrgsort* sorter, int64_t listNum,
...
__aicore__ inline void ExpertSortMultiCore::InitExpertMrgSortOut(ExpertMrgsortOut* sorter, int64_t listNum,
                                                                 int64_t coreOffset)
{
    ...
    sorter->SetOutput(this->sortedExpertIdGm, this->expendedRowIdxGm, outLocal, outLocalV);
...
__aicore__ inline void ExpertSortMultiCore::OneCoreVMSProcess(int64_t listNum, int64_t perListElements,
                                                              int64_t lastListElements)
{
    ...
    for (int64_t i = 0; listNum >= 1; i++) {
        ...
        for (int64_t loop = 0; loop < loops - 1; loop++) {
            InitExpertMrgSort(&mrgsorter, MAX_MRGSORT_LIST, coreOffset, loop * loopOffset);
        ...
        InitExpertMrgSort(&mrgsorter, remainListNum, coreOffset, (loops - 1) * loopOffset);
        mrgsorter.Init(&mrgsortParam);
        mrgsorter.Process();
...
__aicore__ inline void ExpertSortMultiCore::UBSortProcess(int64_t progress, int64_t size, int64_t sortNum)
{
...
__aicore__ inline void ExpertSortMultiCore::VBSProcess()
{
...
__aicore__ inline void ExpertSortMultiCore::VMSProcess()
{
    ...
    for (; listNum > MAX_MRGSORT_LIST;) {
        ...
        if (this->blockIdx < currentStageNeedCoreNum - 1) {
            ...
            mrgsortParam.lastListElements = perListElements;
            mrgsortParam.oneLoopMaxElements = this->sortOutTilingData->oneLoopMaxElements;
            InitExpertMrgSort(&mrgsorter, MAX_MRGSORT_LIST, coreOffset, 0);
            mrgsorter.Init(&mrgsortParam);
            mrgsorter.Process();
            ...
            mrgsortParam.lastListElements = lastListElements;
            mrgsortParam.oneLoopMaxElements = this->sortOutTilingData->oneLoopMaxElements;
            InitExpertMrgSort(&mrgsorter, remainListNum, coreOffset, 0);
            mrgsorter.Init(&mrgsortParam);
            mrgsorter.Process();
        }
...
__aicore__ inline void ExpertSortMultiCore::SortOutProcess()
{
    if (this->blockIdx < 1) {
        mrgsortParam.perListElements = perListElements;
        mrgsortParam.lastListElements = lastListElements;
        ...
        ExpertMrgsortOut sorter;
        InitExpertMrgSortOut(&sorter, listNum, GetSortLen<float>(perListElements));
...
__aicore__ inline void ExpertSortMultiCore::Init(GM_ADDR expertId, GM_ADDR expendedRowIdx, GM_ADDR workspace,
                                                 const ExpertDispatchTilingData* tilingData, TPipe* tPipe)
{
    ...
    this->pipe = tPipe;
    expertIdGm.SetGlobalBuffer(
        (__gm__ int32_t*)expertId + this->blockIdx * tilingData->vbsComputeParamsOp.perCoreElements,
        this->sortTotalLength);
    sortedExpertIdGm.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t*>(workspace),
...
__aicore__ inline void ExpertSortMultiCore::Process()
{
    VBSProcess();
    VMSProcess();
    SortOutProcess();
