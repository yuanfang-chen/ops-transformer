...
/*!
 * \file expert_sort_one_core.h
 * \brief
 */
#ifndef EXPERT_SORT_ONE_CORE_H
#define EXPERT_SORT_ONE_CORE_H
...
class ExpertSortOneCore : public ExpertSortBase
{
public:
    __aicore__ inline ExpertSortOneCore(){};
...
__aicore__ inline void ExpertSortOneCore::CopyIn()
{
    ...
    DataCopyPad(inLocal[0], expertIdGm, dataCopyParams, dataCopyPadParams);
    LocalTensor<int32_t> rowIdxLocal = inLocal[this->sortNum];
    ArithProgression<int32_t>(rowIdxLocal, 0, 1, this->sortNum);
    sortDataCopyInQueue.EnQue(inLocal);
}

__aicore__ inline void ExpertSortOneCore::SortCompute()
{
    LocalTensor<int32_t> inLocal = sortDataCopyInQueue.DeQue<int32_t>();
    LocalTensor<int32_t> expertId = inLocal[0];
    LocalTensor<float> expertIdFp32 = expertId.ReinterpretCast<float>();
    Cast(expertIdFp32, expertId, RoundMode::CAST_ROUND, this->tileLength);
    LocalTensor<uint8_t> maskLocalTensor = sortedBuffer.Get<uint8_t>();
    AscendC::CompareScalar(
        maskLocalTensor, expertIdFp32, static_cast<float>(expertStart_), AscendC::CMPMODE::LT,
        (this->totalLength + ONE_REPEAT_COMPARE_NUM - 1) / ONE_REPEAT_COMPARE_NUM * ONE_REPEAT_COMPARE_NUM);

    Muls(expertIdFp32, expertIdFp32, (float)-1, this->tileLength);

    LocalTensor<float> floatMinLocalTensor = tempBuffer.Get<float>();
    Duplicate(floatMinLocalTensor, MIN_FP32, this->tileLength);
    Select(expertIdFp32, maskLocalTensor, floatMinLocalTensor, expertIdFp32, SELMODE::VSEL_TENSOR_TENSOR_MODE,
           this->totalLength);
    ...
    if (duplicateNum > 0) {
        ...
        Duplicate(expertIdFp32[duplicateIndex], MIN_FP32, mask, 1, DST_BLK_STRIDE, DST_REP_STRIDE);
    }

    LocalTensor<float> concatLocal;
    LocalTensor<float> tempTensor = tempBuffer.Get<float>(GetSortLen<float>(this->sortNum));
    Concat(concatLocal, expertIdFp32, tempTensor, this->sortNum / ONE_REPEAT_SORT_NUM);
...
__aicore__ inline void ExpertSortOneCore::CopyOut()
{
    ...
    DataCopyPad(sortedExpertIdGm, outLocal[0], intriParams);
    DataCopyPad(expendedRowIdxGm, outLocal[this->sortNum], intriParams);
    sortDataCopyOutQueue.FreeTensor(outLocal);
}

__aicore__ inline void ExpertSortOneCore::Init(GM_ADDR expertId, GM_ADDR expendedRowIdx, GM_ADDR workspace,
                                               const ExpertDispatchTilingData* tilingData, TPipe* tPipe)
{
    this->pipe = tPipe;
    ...
    expertIdGm.SetGlobalBuffer((__gm__ int32_t*)expertId, this->tileLength);
...
__aicore__ inline void ExpertSortOneCore::Process()
{
    if (GetBlockIdx() < 1) {
        CopyIn();
        SortCompute();
