...
extern "C" __global__ __aicore__ void moe_init_routing_v2(
    uint8_t* x, uint8_t* expertIdx, uint8_t* expandedX, uint8_t* expandedRowIdx,
    uint8_t* expertTokensCountOrCumsum, uint8_t* expertTokensBeforeCapacity,
    uint8_t* workspace, uint8_t* tiling);

class moe_init_routing_v2_test : public testing::Test {
 protected:
  static void SetUpTestCase() {
    cout << "moe_init_routing_v2_test SetUp\n" << endl;
  }
  static void TearDownTestCase() {
    cout << "moe_init_routing_v2_test TearDown\n" << endl;
  }
};

TEST_F(moe_init_routing_v2_test, test_case_0) {
  size_t num_rows = 80;
  size_t k = 60;
  size_t cols = 3000;
  ...
  size_t tiling_FileSize = sizeof(MoeInitRoutingV2TilingData);
  ...
  system("cp -r ../../../../../moe/moe_init_routing_v2/tests/ut/op_kernel/moe_init_routing_v2_data ./");
  system("chmod -R 755 ./moe_init_routing_v2_data/");
  system("cd ./moe_init_routing_v2_data/ && rm -rf ./*bin");
  system("cd ./moe_init_routing_v2_data/ && python3 gen_data.py 80 60 3000 0 6 8 0 1 false 0 float32");
  system("cd ./moe_init_routing_v2_data/ && python3 gen_tiling.py case0");

  char* path_ = get_current_dir_name();
  string path(path_);
  ReadFile(path + "/moe_init_routing_v2_data/input_x.bin", x_FileSize, x, x_FileSize);
  ReadFile(path + "/moe_init_routing_v2_data/input_expertIdx.bin", expertIdx_FileSize, expertIdx,
           expertIdx_FileSize);
  ReadFile(path + "/moe_init_routing_v2_data/tiling.bin", tiling_FileSize, tiling, tiling_FileSize);

  ICPU_SET_TILING_KEY(tilingKey);
  ICPU_RUN_KF(moe_init_routing_v2, blockDim, x, expertIdx, expandedX, expandedRowIdx,
              expertTokensCountOrCumsum, nullptr, workspace, tiling);
