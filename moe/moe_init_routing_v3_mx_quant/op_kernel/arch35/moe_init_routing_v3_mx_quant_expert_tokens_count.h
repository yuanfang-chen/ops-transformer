...
/*!
 * \file moe_init_routing_v3_mx_quant_expert_tokens_count.h
 * \brief ExpertTokensCount stage for MoeInitRoutingV3MxQuant
 */
#ifndef MOE_INIT_ROUTING_V3_MX_QUANT_EXPERT_TOKENS_COUNT_H
#define MOE_INIT_ROUTING_V3_MX_QUANT_EXPERT_TOKENS_COUNT_H
...
__aicore__ inline void ExpertTokensCount::Process()
{
    ...
    SyncAll();
    /* copy expert tokens count result from workspace to output GM. */
    if (blockIdx_ == 0) {
        expertCountCopyIn();
        expertCountCompute();
        expertCountCopyOut();
...
#endif // MOE_INIT_ROUTING_V3_MX_QUANT_EXPERT_TOKENS_COUNT_H
