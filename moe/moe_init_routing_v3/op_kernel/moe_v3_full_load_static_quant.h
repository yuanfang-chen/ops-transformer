...
/*!
 * \file moe_v3_static_quant_full_load.h
 * \brief
 */
...
template <typename T>
__aicore__ inline void MoeV3FullLoadStaticQuant<T>::ComputeQuant(int64_t xLocalLength)
{
...
template <typename T>
__aicore__ inline void MoeV3FullLoadStaticQuant<T>::CopyOutXStaticQuant()
{
...
template <typename T>
__aicore__ inline void MoeV3FullLoadStaticQuant<T>::FreeLocalTensor()
{
    if (!this->ep_) {
        LocalTensor<int32_t> expandedRowIdx = this->expandedRowIdxCopyOutQueue_.template DeQue<int32_t>();
