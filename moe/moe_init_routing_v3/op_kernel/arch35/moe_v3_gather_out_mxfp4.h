...
template <typename T>
class MoeGatherOutFp4 {
public:
    __aicore__ inline MoeGatherOutFp4(){};
...
template <typename T>
__aicore__ inline void MoeGatherOutFp4<T>::Init(GM_ADDR x, GM_ADDR scale, GM_ADDR workspace, GM_ADDR expandedRowIdx,
                                             ...
                                             const MoeInitRoutingV3Arch35TilingData *tilingData, TPipe *tPipe)
{
    ...
    xUint8tGm_.SetGlobalBuffer((__gm__ uint8_t *)x, n_ * cols_ / 2);
    xGscaleGm_.SetGlobalBuffer((__gm__ uint8_t *)scale, n_ * cols_ / UB_BLOCK_SIZE);
    expandedXGm_.SetGlobalBuffer((__gm__ uint8_t *)expandedX + blockIdx_ * perCoreIndicesElements_ * cols_ / 2,
                                 curCoreIndicesElements_ * cols_ / 2);
    expandedScaleGm_.SetGlobalBuffer((__gm__ uint8_t *)expandedScale + blockIdx_ * perCoreIndicesElements_,
                                     curCoreIndicesElements_ * cols_ / UB_BLOCK_SIZE);
    ...
    pipe_->InitBuffer(scaleCopyInQueue_, GATHER_OUT_BUFFER_NUM, AlignBytes(perLoopCols_ / UB_BLOCK_SIZE, sizeof(uint8_t)));
...
template <typename T>
__aicore__ inline void MoeGatherOutFp4<T>::CopyExpertIn(int64_t curExpertLoopOffset, int64_t curLoopElements)
{
...
template <typename T>
__aicore__ inline void MoeGatherOutFp4<T>::CopyXIn(int64_t xSrcOffset, int64_t scaleSrcOffset, int64_t curLoopCols)
{
...
template <typename T>
__aicore__ inline void MoeGatherOutFp4<T>::CopyXOut(int64_t xDstOffset, int64_t scaleDstOffset, int64_t curLoopCols)
{
...
template <typename T>
__aicore__ inline void MoeGatherOutFp4<T>::CopyScaleIn(int64_t scaleSrcOffset)
{
...
template <typename T>
__aicore__ inline void MoeGatherOutFp4<T>::CopyScaleOut(int64_t scaleDstOffset)
{
...
template <typename T>
__aicore__ inline void MoeGatherOutFp4<T>::Process()
{   
    if (blockIdx_ < needCoreNum_) {
        ...
        for (int64_t indicesLoop = 0; indicesLoop < indicesLoops_; indicesLoop++) {
            ...
            for (int64_t indicesIndex = 0; indicesIndex < curLoopElements; indicesIndex++) {
                ...
                int64_t xSrcOffset = rowIdx / k_ * cols_ / 2;
                int64_t scaleSrcOffset = rowIdx / k_;
                int64_t xDstOffset = (curExpertLoopOffset + indicesIndex) * cols_ / 2;
                SetWaitFlag<HardEvent::S_MTE2>(HardEvent::S_MTE2);
                if (isInputScale_ == 1) {
                ...
                for (int64_t colsLoop = 0; colsLoop < colsLoops_; colsLoop++) {
                    ...
                    int64_t colsLoopOffset = colsLoop * perLoopCols_ / 2;
                    CopyXIn(xSrcOffset + colsLoopOffset, scaleSrcOffset, curLoopCols / 2);
                    CopyXOut(xDstOffset + colsLoopOffset, indicesIndex, curLoopCols / 2);
                }
            }
