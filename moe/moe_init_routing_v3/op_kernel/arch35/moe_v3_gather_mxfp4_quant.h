...
constexpr uint16_t FP16_EMASK_AND_INF_VAL = 0x7c00; // 0x 0111 1100 0000 0000
// bf16中指数部分Mask，同时也表示bf16的INF值
constexpr uint16_t BF16_EMASK_AND_INF_VAL = 0x7f80; // 0x 0111 1111 1000 0000
// bfloat16的nan值（与inf值不同）
constexpr uint16_t BF16_NAN_VAL = 0x7f81;           // 0x 0111 1111 1000 0001
                                        ...
                                        uint16_t vfLoopNum, uint32_t vlForT, uint32_t numVRegBlocks)
{
    ...
    // 存储FP32/FP16/BF16的指数位为1的mask
    RegTensor<uint16_t> emaskFP16, emaskBF16, emaskFP32;
    ...
    for (uint16_t i = 0; i < vfLoopNum; i++) {
        ...
        if constexpr (IsSameType<T, float>::value) {
            // 双搬入，奇数位放x0、偶数位放x1
            DataCopy<T, PostLiteral::POST_MODE_UPDATE, LoadDist::DIST_DINTLV_B32>(x0, x1, xAddr, vlForT * 2);
            // fp32要先转成bf16
                                       ...
                                       uint32_t vlForT, uint16_t expLowerBoundValue)
{
    ...
    Duplicate(infBF16, BF16_EMASK_AND_INF_VAL);         // 0x7f80
    // zeroB16存放B16长度的0值
...
/*
        // Compare<uint64_t, CMPMODE::NE>(maskInfBF16, maxExp, infBF16, maskValid);
        // Compare<uint64_t, CMPMODE::NE>(maskZero, maxExp, zeroB16, maskValid);
        // Compare<uint64_t, CMPMODE::LE>(maskLowExp, maxExp, expLowerBound, maskValid);
        ...
        // Select<uint16_t>(mxScale, mxScale, nanForE8M0, maskInfBF16);
        // Select<uint16_t>(mxScale, mxScale, maskZero, maskZero);

        // DataCopy<<uint16_t, PostLiteral::POST_MODE_UPDATE, StoreDist::DIST_PACK_B16>(mxScaleOutAddr, mxScale, vlForT / 2, maskLoop);
*/
                                                          ...
                                                          const MoeInitRoutingV3Arch35TilingData *tilingData, TPipe *tPipe)
{
    ...
    } else if constexpr (IsSameType<U, fp8_e5m2>::value) {
