                                       ...
                                       uint32_t vlForT, uint16_t expLowerBoundValue)
{
    ...
    Duplicate(expLowerBound, expLowerBoundValue);           // 0x0100
    ...
    Duplicate(nanBF16, BF16_NAN_VAL);                       // 0x7f81
    ...
    for (uint16_t i = 0; i < vfLoopNum; i++) {
        ...
        Compare<uint16_t, CMPMODE::LT>(maskLowExp, maxExp, expLowerBound, maskValid);   // 0x0100
        ...
        Compare<uint16_t, CMPMODE::EQ>(maskInfBF16, maxExp, infBF16, maskValid);        // 0x7f80
        Select<uint16_t>(mxScale, nanForE8M0, mxScale, maskInfBF16);                    // 0x00ff
        // mxScale[maxExp==0]=0
        ...
        Select<uint16_t>(invScale, nanBF16, invScale, maskInfBF16);                     // 0x7f81
        // invScale[maxExp==0]=0
        Select<uint16_t>(invScale, zeroB16, invScale, maskZero);
        // invScale[(invScale=invSub-sharedExp)==0]=FP8_E8M0_SPECIAL_MIN
        Compare<uint16_t, CMPMODE::EQ>(maskSpecialMin, invSub, sharedExp, maskValid);   // 0x7f00
        Select<uint16_t>(invScale, specialMinE8M0, invScale, maskSpecialMin);           // 0x0040
...
template <typename T, typename U>
__aicore__ inline void FP16Convert(RegTensor<half>& output, RegTensor<half>& input, MaskReg& mask)
{
    __VEC_SCOPE__
    {
                                      ...
                                      uint32_t vlForT, uint32_t numVRegBlocks)
{
    ...
    static constexpr CastTrait traitFP16ToBF16 = {RegLayout::UNKNOWN, SatMode::UNKNOWN, MaskMergeMode::ZEROING,
                                                  RoundMode::CAST_TRUNC};
    // bf16转b4
    static constexpr CastTrait traitB16ToB4 = {RegLayout::ZERO, SatMode::SAT, MaskMergeMode::ZEROING,
                                                RoundMode::CAST_RINT};
