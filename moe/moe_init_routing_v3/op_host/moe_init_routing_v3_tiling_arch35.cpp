...
uint64_t MoeInitRoutingV3Arch35TilingClass::GetTilingKey() const
{
    ...
    if (quantMode_ == QUANT_MODE_MXFP8_E5M2 || quantMode_ == QUANT_MODE_MXFP8_E4M3FN || QUANT_MODE_MXFP4_E2M1) {
        ...
        // quantMode >= 9时，都可以走这个TilingKey的计算
...
ge::graphStatus MoeInitRoutingV3Arch35TilingClass::CheckSetAttrs()
{
    ...
    OP_CHECK_IF(quantMode_ != QUANT_MODE_UNQUANT && quantMode_ != QUANT_MODE_DYNAMIC &&
                ...
                OP_LOGE(context_, "Attr quant_mode currently supports (%ld, %ld, %ld, %ld, %ld, %ld, %ld, %ld), but got %ld",
                        ...
                        QUANT_MODE_HIF8_CAST, QUANT_MODE_HIF8_PERTENSOR, QUANT_MODE_HIF8_PERTOKEN, QUANT_MODE_MXFP4_E2M1, 
                        quantMode_),
                return ge::GRAPH_FAILED);
...
ge::graphStatus MoeInitRoutingV3Arch35TilingClass::CheckInputX()
{
    ...
    static const unordered_set<DataType>  MXFP4QUANT_SUPPORTED_DTYPES = {DataType::DT_FLOAT, DataType::DT_FLOAT16, 
                                                                        DataType::DT_BF16};
