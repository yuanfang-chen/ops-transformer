...
const static int64_t FP4_BLOCK_SIZE = 64LL;
...
ge::graphStatus MoeInitRoutingV3Arch35TilingClass::CheckInputScale()
{
    ...
    if (quantMode_ == QUANT_MODE_UNQUANT) {
        ...
        if (rankScale == 3) {
            ...
            expectedDim0 = xShape_.GetDim(0);
            expectedDim1 = xShape_.GetDim(1) / FP4_BLOCK_SIZE;
...
ge::graphStatus MoeInitRoutingV3Arch35TilingClass::CheckOutputExpandedScale()
{
    ...
    int64_t expectedRank{-1}, expectedDim0{-1}, expectedDim1{-1}, expectedDim2{-1};
...
PerLoopParams MoeInitRoutingV3Arch35TilingClass::GetPerLoopParams(MultipleParams& multipleParams)
{
    ...
    if (quantMode_ == QUANT_MODE_HIF8_PERTENSOR) {
        ...
        int64_t tmp = (inputXDtypeSize_ > 1000) ? 1 : inputXDtypeSize_;
        OP_LOGD(context_, "233333333333333333333333     inputScaleDTypeSize_=%ld\n", inputScaleDTypeSize_);
        inputXDtypeSize_ = 1;//(inputXDtypeSize_ > 1000) ? 1 : inputXDtypeSize_;
        inputScaleDTypeSize_ = 1;
        OP_LOGD(context_, "233333333333333333333333     tmp=%ld\n", tmp);
        perLoopParams.perLoopMaxIndicesElements =
            (availUbSize_ - Align(perLoopParams.perLoopCols, inputXDtypeSize_) * multipleParams.colMultiple -
        ...
        OP_LOGD(context_, "availUbSize_=%ld, Align(perLoopParams.perLoopCols, inputXDtypeSize_)=%ld, multipleParams.colMultiple=%ld\n",
                availUbSize_, Align(perLoopParams.perLoopCols, inputXDtypeSize_), multipleParams.colMultiple);
        OP_LOGD(context_, "Align(perLoopParams.perLoopCols / 32, inputScaleDTypeSize_)=%ld, multipleParams.rowMultiple=%ld",
                Align(perLoopParams.perLoopCols / 32, inputScaleDTypeSize_), multipleParams.rowMultiple);

        while (perLoopParams.perLoopMaxIndicesElements <= 0) {
            OP_LOGD(context_, "4555555555555555555555555555555555555555555555\n");
            perLoopParams.perLoopCols = Ops::Base::CeilDiv(perLoopParams.perLoopCols, NUM_TWO);
            perLoopParams.perLoopMaxIndicesElements =
                (availUbSize_ - Align(perLoopParams.perLoopCols, inputXDtypeSize_) * multipleParams.colMultiple -
                Align(perLoopParams.perLoopCols / 32, inputScaleDTypeSize_) * inputScaleDTypeSize_ * NUM_TWO) / multipleParams.rowMultiple / static_cast<int64_t>(sizeof(int32_t));
