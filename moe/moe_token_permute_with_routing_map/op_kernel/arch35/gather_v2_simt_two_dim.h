...
class Gatherv2SimtTwoDim {
 public:
  ...
  __aicore__ inline void Init(GM_ADDR x, GM_ADDR indices, GM_ADDR y, const MoeTokenPermuteWithRoutingMapTilingData* tilingData, bool isprob);
 ...
 private:
  ...
  template <const bool NIS>
  static __simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM_LAUNCH_BOUND_TWO_DIM) inline void GatherSimtProb(const INDEX_SIZE_T yIndexBase,
  INDEX_SIZE_T currentCoreElements, INDEX_SIZE_T m0, INDEX_SIZE_T shift0, INDEX_SIZE_T tokenNum,
  __gm__ X_T* x, __gm__ INDICES_T* indices, __gm__ volatile X_T* y);
...
template <typename X_T, typename INDICES_T, typename INDEX_SIZE_T>
template <const bool NIS>
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM_LAUNCH_BOUND_TWO_DIM) inline void Gatherv2SimtTwoDim<X_T, INDICES_T, INDEX_SIZE_T>::GatherSimtProb(const INDEX_SIZE_T yIndexBase,
  INDEX_SIZE_T currentCoreElements, INDEX_SIZE_T m0, INDEX_SIZE_T shift0, INDEX_SIZE_T tokenNum,
  __gm__ X_T* x, __gm__ INDICES_T* indices, __gm__ volatile X_T* y) {
  for (INDEX_SIZE_T index = static_cast<INDEX_SIZE_T>(Simt::GetThreadIdx()); index < currentCoreElements;
      index += static_cast<INDEX_SIZE_T>(Simt::GetThreadNum())) {
...
__aicore__ inline void Gatherv2SimtTwoDim<X_T, INDICES_T, INDEX_SIZE_T>::ProbProcess() {
  if (blockIdx < needCoreNum) {
    ...
    AscendC::Simt::VF_CALL<GatherSimtProb<false>>(Simt::Dim3(threadNum), yIndexBase, currentCoreElements, m0, shift0,
                tokenNum, (__gm__ X_T*) (xGm_.GetPhyAddr()),
                (__gm__ INDICES_T*) (indicesGm_.GetPhyAddr()), (__gm__ volatile X_T*) (yGm_.GetPhyAddr()));
