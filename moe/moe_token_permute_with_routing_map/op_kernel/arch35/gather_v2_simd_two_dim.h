...
public:
  ...
  __aicore__ inline void Init(GM_ADDR x, GM_ADDR indices, GM_ADDR y, const MoeTokenPermuteWithRoutingMapTilingData* tilingData);
...
template <typename INDICES_T>
__aicore__ inline void Gatherv2SimdTwoDim<INDICES_T>::Init(GM_ADDR x, GM_ADDR indices,
...
this->indexCopyTilingData = &(tilingData->indexCopyComputeParamsOp);
  ...
  pipe_->InitBuffer(inQueue_, BUFFER_NUM_SIMD_TWO_DIM, indexCopyTilingData->tokenUB * indexCopyTilingData->oneTokenBtypeSize);
...
__aicore__ inline INDICES_T Gatherv2SimdTwoDim<INDICES_T>::GetIndex(int64_t idx, int64_t endIdx)
{
    if (idx >= indicesOffsetBase_ + curIndexSize_) {
      ...
      int64_t copyLen = (endIdx - idx) > indexCopyTilingData->onceIndices ? indexCopyTilingData->onceIndices  : endIdx - idx;
  ...
  for (int64_t i = 0; i < loopNum; i++) {
    ...
    for (int64_t j = 0; j < rows; j++) {
      ...
      if (unlikely(xIndex == preIdx0 || xIndex == preIdx1 ||xIndex == preIdx2)) {
          ...
          Copy(xLocal[j * colsAlign], xLocal[backStep], indexCopyTilingData->onceUbTokenNums * indexCopyTilingData->oneTokenBtypeSize);
      } else {
          CopyIn(xLocal[j * colsAlign], xGm_, offset, 1, indexCopyTilingData->onceUbTokenNums * indexCopyTilingData->oneTokenBtypeSize);
          preIdx2 = preIdx1;
          preIdx1 = preIdx0;
          preIdx0 = xIndex;
    ...
    int64_t yOffset = (yStart + i * onceMaxRows) * indexCopyTilingData->onceUbTokenNums * indexCopyTilingData->oneTokenBtypeSize;
...
if (GetBlockIdx() < indexCopyTilingData->tailCoreNum) {
  ...
  int64_t indiceEndIdx = yEnd;
  int64_t loopSize = (indexCopyTilingData->onceUbTokenNums + indexCopyTilingData->tokenUB - 1) / indexCopyTilingData->tokenUB;
  for (int64_t i = yStart; i < yEnd; i++) {
    ...
    for (int64_t j = 0; j < loopSize; j++) {
      int64_t cols = (j == loopSize - 1) ? (indexCopyTilingData->onceUbTokenNums - j * indexCopyTilingData->tokenUB) : indexCopyTilingData->tokenUB;
      LocalTensor<int8_t> xLocal = inQueue_.AllocTensor<int8_t>();
...
__aicore__ inline void Gatherv2SimdTwoDim<INDICES_T>::Process() {
  ...
  int64_t colsAlign = (indexCopyTilingData->onceUbTokenNums * indexCopyTilingData->oneTokenBtypeSize + ubBlockSize - 1) / ubBlockSize * ubBlockSize;
