...
public:
    ...
    __aicore__ inline MoeGatingTopKSoftmax<T1, T1, T2>(){};

    __aicore__ inline void Init(
            GM_ADDR x, GM_ADDR y, GM_ADDR expertIdx, GM_ADDR workspace,
            const MoeGatingTopKSoftmaxTilingData &tilingData, AscendC::TPipe *pipeIn)
    {
        ...
        gmX_.SetGlobalBuffer((__gm__ T1 *)x + AscendC::GetBlockIdx() * gmXOffset_, gmXOffset_); //  每一个 core 起始address
        gmY_.SetGlobalBuffer((__gm__ T1 *)y + AscendC::GetBlockIdx() * gmYOffset_, gmYOffset_);
        gmExpertIdx_.SetGlobalBuffer((__gm__ T2 *)expertIdx + AscendC::GetBlockIdx() * gmExpertIdxOffset_, gmExpertIdxOffset_);


        gmSync_.SetGlobalBuffer((__gm__ int32_t *)workspace, 8 * 32 * sizeof(int32_t));
        gmYTmp_.SetGlobalBuffer((__gm__ T1*)workspace + 8 * 32 * sizeof(int32_t) / sizeof(T1) + GetBlockIdx() * oneCoreRow_ * kAlign_, rowWork_ * kAlign_);
        gmExpertTmp_.SetGlobalBuffer((__gm__ T2*)workspace + 8 * 32 * sizeof(int32_t) / sizeof(T2) + numRow_ * kAlign_ * sizeof(T1) / sizeof(T2) + GetBlockIdx() * oneCoreRow_ * kAlign_, rowWork_ * kAlign_);
        ...
        pipe->InitBuffer(bufUb_, MAX_UB_SIZE - 8 * 32 * sizeof(int32_t)); 
        pipe->InitBuffer(syncIn, 8 * 32 * sizeof(int32_t));
    ...
    __aicore__ inline void ParesTiling(const MoeGatingTopKSoftmaxTilingData* __restrict tilingData){
    ...
    __aicore__ inline  void Compute()
    {
        ...
        SoftMaxTiling* softmaxTilingData = (AscendC::GetBlockIdx() < activateCore_ - 1) ? &FormerSoftmaxTilingData : &TailSoftmaxTilingData;
        
        TopkTiling* TopkTilingData = (AscendC::GetBlockIdx() < activateCore_ - 1) ? &FormerTopkTilingData : &TailTopkTilingData;
        ...
        SoftMax<T1, false, false>(yTmpLT, sumTmpLT, maxTmpLT, xLT, softmaxTmpUint8LT, *softmaxTilingData, softmaxShapeInfoData);  // pay attention to softmax -- tiling data
    ...
    __aicore__ inline  void CopyOut()
    {
        ...
        PipeBarrier<PIPE_ALL>();
        //AscendC::SyncAll(gmSync_, syncLT, 256);
...
extern "C" __global__ __aicore__ void moe_gating_top_k_softmax(GM_ADDR x, GM_ADDR finished, GM_ADDR y, GM_ADDR expertIdx, GM_ADDR rowIdx,GM_ADDR workspace, GM_ADDR tiling) {\


    GET_TILING_DATA(tilingData, tiling);
    
        AscendC::TPipe pipe;
        MoeGatingTopKSoftmax<half, int32_t> kernel;
