...
@register("ascend_aclnn_moe_token_unpermute_with_ep_grad_probs_none")
class FunctionUnpermuteGradApi(BaseApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        rangeattr = input_data.kwargs["range"]
        output_grad = self.output_grad
        sorted_indices = self.origin_sorted_indices
        permuted_tokens = self.permuted_tokens
        range_list = rangeattr

        topk = input_data.kwargs["topk_num"]
        d = output_grad.size(1)
        n = sorted_indices.size(0)
        unpermuted_tokens = self.unpermuted_tokens
        unpermuted_tokens = \
            permuted_tokens.index_select(0, sorted_indices).reshape(-1, topk, permuted_tokens.size(-1))
        self.permuted_tokens_grad1.index_copy_(0, sorted_indices, output_grad)
        if len(range_list) == 2:
            self.permuted_tokens_grad1 = self.permuted_tokens_grad1[range_list[0]:range_list[1]]
        return self.permuted_tokens_grad1.bfloat16(), torch.tensor([1]).bfloat16().to(self.permuted_tokens_grad1.device)

    def init_by_input_data(self, input_data: InputDataset):
        sorted_indices = input_data.kwargs['sorted_indices']
        permutedtokens = input_data.kwargs["permutedTokens"]
        probs = input_data.kwargs["probs"]
        input_data.kwargs["range"] = (0, sorted_indices.shape[0])
...
@register("AclnnBaseApi_aclnn_moe_token_unpermute_with_ep_grad_probs_none")
