...
at::Tensor reset_npu(const at::Tensor &expand_x, int64_t ep_world_size,
                               int64_t ep_rank_id,
    ...
    at::Tensor input_elastic_rank = torch::full({detection_world_size}, 0, torch::kInt); // 应该是外界输入的
    ...
    SimpleSingleThread& threadPool = SimpleSingleThread::getInstance(); // 这是啥作用
    threadPool.executeAndWait(std::bind(ResetThreadFun, elastic_rank_table, elastic_rank_cpu, input_elastic_rank, output_elastic_rank_npu,
        ep_world_size, ep_rank_id, group_detection, detection_world_size, detection_rank_id,
