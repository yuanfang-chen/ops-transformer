...
@GENERATOR_REGISTRY.register("generate_block_sparse_attention")
class RainFusionAttentionGenerator(CaseGenerator):
    ...
    def after_input_config(
        ...
        if index == 0 and input_case.name == "query":
            self.query_total = input_case.shape[0]
            self.head_num = input_case.shape[1]
            self.head_dim = random.choice(self.headDim_range)
    ...
    def after_case_config(self, case_config: CaseConfig) -> CaseConfig:
        ...
        for b in range(self.batch):
            q_seqlen = case_config.inputs[6][b].range_values
            kv_seqlen = case_config.inputs[7][b].range_values
            ...
            max_q_block_num = max(max_q_block_num, s_block_num_q)
        scale = case_config.inputs[13].range_values
        if scale == 0.0:
        ...
        if self.q_input_layout == "TND":
            ...
            case_config.inputs[19].shape = [query_total, self.head_num, self.head_dim]
            case_config.inputs[19].dtype = self.dtype
            case_config.inputs[20].shape = [query_total, self.head_num, 1]
            case_config.inputs[20].dtype = 'fp32'
            print("testlog===", case_config.inputs[19])
            print("testlog===", case_config.inputs[20])
            ...
            case_config.inputs[19].shape = [self.batch, self.head_num, self.maxQSeqlen, self.head_dim]
            case_config.inputs[19].dtype = self.dtype
            case_config.inputs[20].shape = [self.batch, self.head_num, self.maxQSeqlen, 1]
            case_config.inputs[20].dtype = 'fp32'
            print("testlog===", case_config.inputs[19])
            print("testlog===", case_config.inputs[20])
            
        # case_config.inputs[3].shape = [blocks_total, self.head_num, max_kv_block_num]
        case_config.inputs[3].shape = [self.batch, self.head_num, max_q_block_num, max_kv_block_num]
        ...
        case_config.inputs[11].range_values = self.kv_num
        return case_config
