...
def parse_txt_content(content):
    """解析txt文件内容，提取所需字段"""
    # 使用正则表达式匹配内容
    # Build [{self.soc}] op_name [{self.op_file}] index [{str(index)}] bin_file [{bin_file}] duration [$duration] size [$kernel_size] start_time [$start_time] end_time [$end_time]"
...
def main():
    ...
    for txt_file in txt_files:
        try:
            ...
            # 统计{capital_name}-{op_name}-{index}.sh文件的总个数
            ops_kernel_num = count_sh_files(base_dir, capital_name, op_name)
            new_kernel_name = f"{op_name}-{ops_kernel_num}-{index}"   
    ...
    # 写入CSV文件
    if data_list:
        csv_filename = 'ops_cost_{soc_version}.csv'
