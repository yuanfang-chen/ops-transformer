...
namespace AllGatherMatmulInnerImpl
{
using namespace AscendC;

template <typename AType, typename BType, typename BiasType, typename CType>
class AllGatherMatmulInner
{
public:
    __aicore__ inline AllGatherMatmulInner()
    {
    }
    __aicore__ inline void Init(GM_ADDR aGM, GM_ADDR bGM, GM_ADDR biasGM, GM_ADDR contextGM, GM_ADDR cGM,
                                GM_ADDR gatherOut, GM_ADDR workspaceGM,  Mc2Tiling::AllGatherMatmulInnerTilingData* tilingData,
...
private:
    ...
    Hccl<HcclServerType::HCCL_SERVER_TYPE_AICPU> hccl_;         // CCU模式
...
__aicore__ inline void AllGatherMatmulFP16BF16<AType, BType, BiasType, CType>::Init(
    GM_ADDR aGM, GM_ADDR bGM, GM_ADDR contextGM,  GM_ADDR biasGM, GM_ADDR cGM, GM_ADDR gatherOut, GM_ADDR workspaceGM, 
    Mc2Tiling::AllGatherMatmulTilingDataV2* tilingData, __gm__ void* mc2InitTiling, __gm__ void* mc2CcTiling, TPipe* tPipe)
{
    ...
    // 获取 context
    ctx_ = *(__gm__ uint64_t *)contextGM;
    // 初始化Hccl类
    hccl_.Init((GM_ADDR)contextGM, mc2InitTiling);
    // 管道初始化
    tPipe_ = tPipe;
    ...
    cGM_ = cGM;
    biasGM_ = biasGM;
    rankId_ = CommKfcContext(ctx_).apiCtx.rankId;
