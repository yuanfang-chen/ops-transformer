...
{
     const gert::StorageShape *dynamicScalesStorageShape = context->GetOutputShape(MoeDispatchExtend::OUTPUT_DYNAMIC_SCALES_INDEX);
...
{
    if (isScales) {
        const gert::StorageShape *scalesStorageShape = context->GetOptionalInputShape(MoeDispatchExtend::SCALES_INDEX);
        ...
        if (quantMode != static_cast<uint32_t>(QuantModeA5::STATIC_QUANT)) {
            ...
            auto expandXDesc = context->GetOutputDesc(MoeDispatchExtend::OUTPUT_EXPAND_X_INDEX);
...
static bool CheckInputTensorDim(const gert::TilingContext *context, const char *nodeName,
    const bool isScales, const uint32_t quantMode, const bool isLayered)
{
    const gert::StorageShape *xStorageShape = context->GetInputShape(MoeDispatchExtend::X_INDEX);
    ...
    const gert::StorageShape *expertIdStorageShape = context->GetInputShape(MoeDispatchExtend::EXPERT_IDS_INDEX);
    if (isLayered) {
        const gert::StorageShape *expertScaleStorageShape = context->GetOptionalInputShape(MoeDispatchExtend::EXPERT_SCALES_INDEX);
    ...
    OP_TILING_CHECK(!CheckScaleTensorDim(context, nodeName, isScales, quantMode), 
...
{
    const gert::StorageShape *expandXStorageShape = context->GetOutputShape(MoeDispatchExtend::OUTPUT_EXPAND_X_INDEX);
    ...
    const gert::StorageShape *assistInfoStorageShape = context->GetOutputShape(MoeDispatchExtend::OUTPUT_ASSIST_INFO_INDEX);
    ...
    const gert::StorageShape *expertTokenNumsStorageShape = context->GetOutputShape(MoeDispatchExtend::OUTPUT_EXPERT_TOKEN_NUMS_INDEX);
    ...
    const gert::StorageShape *epRecvCountStorageShape = context->GetOutputShape(MoeDispatchExtend::OUTPUT_EP_RECV_COUNTS_INDEX);
    ...
    const gert::StorageShape *tpRecvCountStorageShape = context->GetOutputShape(MoeDispatchExtend::OUTPUT_TP_RECV_COUNTS_INDEX);
...
static bool CheckTensorDim(const gert::TilingContext *context, const char *nodeName,
    const bool isScales, const uint32_t quantMode, const bool isActiveMask, const bool hasElasticInfo, const bool isPerformance, const bool isLayered)
{
    OP_TILING_CHECK(!CheckInputTensorDim(context, nodeName, isScales, quantMode, isLayered), 
    ...
    if (isActiveMask) {
        ...
        const gert::StorageShape *expertIdStorageShape = context->GetInputShape(MoeDispatchExtend::EXPERT_IDS_INDEX);
        const int64_t expertIdDim1 = expertIdStorageShape->GetStorageShape().GetDim(1);
        const gert::StorageShape *xActiveMaskStorageShape = context->GetOptionalInputShape(MoeDispatchExtend::X_ACTIVE_MASK_INDEX);
    ...
    if (hasElasticInfo) {
        const gert::StorageShape *elasticInfoStorageShape = context->GetOptionalInputShape(MoeDispatchExtend::ELASTIC_INFO_INDEX);
    ...
    if (isPerformance) {
        const gert::StorageShape *performanceInfoStorageShape = context->GetOptionalInputShape(MoeDispatchExtend::PERFORMANCE_INFO_INDEX);
    ...
    bool isScales, const uint32_t quantMode)
{
    ...
    OP_TILING_CHECK(isScales && (quantMode == static_cast<uint32_t>(QuantModeA5::MX_QUANT)),
        ...
        quantMode), return ge::GRAPH_FAILED);
    auto xDesc = context->GetInputDesc(MoeDispatchExtend::X_INDEX);
    ...
    const char *nodeName, const bool isScales)
{
    ...
    if (isScales) {
        auto scalesDesc = context->GetOptionalInputDesc(MoeDispatchExtend::SCALES_INDEX);
        OP_TILING_CHECK(scalesDesc == nullptr, OP_LOGE(nodeName, "scalesDesc is null."), return false);
        auto dynamicScalesDesc = context->GetOutputDesc(MoeDispatchExtend::OUTPUT_DYNAMIC_SCALES_INDEX);
...
{
    auto assistInfoDesc = context->GetOutputDesc(MoeDispatchExtend::OUTPUT_ASSIST_INFO_INDEX);
    ...
    auto expertTokenNumsDesc = context->GetOutputDesc(MoeDispatchExtend::OUTPUT_EXPERT_TOKEN_NUMS_INDEX);
    ...
    auto epRecvCountsDesc = context->GetOutputDesc(MoeDispatchExtend::OUTPUT_EP_RECV_COUNTS_INDEX);
    ...
    auto tpRecvCountsDesc = context->GetOutputDesc(MoeDispatchExtend::OUTPUT_TP_RECV_COUNTS_INDEX);
...
static bool CheckQuantModeAndExpandXType(const gert::TilingContext *context, const char *nodeName)
{
    ...
    auto quantModePtr = attrs-> GetAttrPointer<int64_t>(MoeDispatchExtend::ATTR_QUANT_MODE_INDEX);
    auto expandXDesc = context-> GetOutputDesc(MoeDispatchExtend::OUTPUT_EXPAND_X_INDEX);
    ...
    const bool isScales, const uint32_t quantMode, const bool isActiveMask, const bool hasElasticInfo, const bool isPerformance)
{
    ...
    auto expertIdDesc = context->GetInputDesc(MoeDispatchExtend::EXPERT_IDS_INDEX);
    ...
    if (isActiveMask) {
        auto xActiveMaskDesc = context->GetOptionalInputDesc(MoeDispatchExtend::X_ACTIVE_MASK_INDEX);
    ...
    if (hasElasticInfo) {
        auto elasticInfoDesc = context->GetOptionalInputDesc(MoeDispatchExtend::ELASTIC_INFO_INDEX);
    ...
    if (isPerformance) {
        auto performanceInfoDesc = context->GetOptionalInputDesc(MoeDispatchExtend::PERFORMANCE_INFO_INDEX);
    ...
    bool &isLayered)
{
    ...
    auto groupEpPtr = attrs->GetAttrPointer<char>(static_cast<int>(MoeDispatchExtend::ATTR_GROUP_EP_INDEX));
    auto groupTpPtr = attrs->GetAttrPointer<char>(static_cast<int>(MoeDispatchExtend::ATTR_GROUP_TP_INDEX));
    auto epWorldSizePtr = attrs->GetAttrPointer<int64_t>(MoeDispatchExtend::ATTR_EP_WORLD_SIZE_INDEX);
    auto tpWorldSizePtr = attrs->GetAttrPointer<int64_t>(MoeDispatchExtend::ATTR_TP_WORLD_SIZE_INDEX);
    auto epRankIdPtr = attrs->GetAttrPointer<int64_t>(MoeDispatchExtend::ATTR_EP_RANK_ID_INDEX);
    auto tpRankIdPtr = attrs->GetAttrPointer<int64_t>(MoeDispatchExtend::ATTR_TP_RANK_ID_INDEX);
    auto expertShardPtr = attrs->GetAttrPointer<int64_t>(MoeDispatchExtend::ATTR_EXPERT_SHARD_TYPE_INDEX);
    auto sharedExpertNumPtr = attrs->GetAttrPointer<int64_t>(static_cast<int>(MoeDispatchExtend::ATTR_SHARED_EXPERT_NUM_INDEX));
    auto sharedExpertRankNumPtr = attrs->GetAttrPointer<int64_t>(MoeDispatchExtend::ATTR_SHARED_EXPERT_RANK_NUM_INDEX);
    auto moeExpertNumPtr = attrs->GetAttrPointer<int64_t>(MoeDispatchExtend::ATTR_MOE_EXPERT_NUM_INDEX);
    auto quantModePtr = attrs->GetAttrPointer<int64_t>(MoeDispatchExtend::ATTR_QUANT_MODE_INDEX);
    auto expertTokenNumsTypePtr = attrs->GetAttrPointer<int64_t>(static_cast<int>(MoeDispatchExtend::ATTR_EXPERT_TOKEN_NUMS_TYPE_INDEX));
    auto commAlgPtr = attrs->GetAttrPointer<char>(static_cast<int>(MoeDispatchExtend::ATTR_COMM_ALG_INDEX));
    auto zeroExpertNumPtr = attrs->GetAttrPointer<int64_t>(static_cast<int>(MoeDispatchExtend::ATTR_ZERO_EXPERT_NUM_INDEX));
    auto copyExpertNumPtr = attrs->GetAttrPointer<int64_t>(static_cast<int>(MoeDispatchExtend::ATTR_COPY_EXPERT_NUM_INDEX));
    auto constExpertNumPtr = attrs->GetAttrPointer<int64_t>(static_cast<int>(MoeDispatchExtend::ATTR_CONST_EXPERT_NUM_INDEX));
    ...
    bool isLayered)
{
    ...
    auto attrs = context->GetAttrs();
    OP_TILING_CHECK(attrs == nullptr, OP_LOGE(nodeName, "attrs is nullptr."), return ge::GRAPH_FAILED);
    auto globalBsPtr = attrs->GetAttrPointer<int64_t>(MoeDispatchExtend::ATTR_GLOBAL_BS_INDEX);
...
{
    OP_TILING_CHECK(!CheckTensorDim(context, nodeName, isScales, quantMode, isActiveMask, hasElasticInfo, isPerformance, isLayered),
    ...
    uint64_t tokenNeedSizeDispatch, uint64_t tokenNeedSizeCombine)
{
    ...
    if (tpWorldSize == TP_WORLD_SIZE_TWO) {
        ...
        auto groupTpHccl = attrs->GetAttrPointer<char>(static_cast<int>(MoeDispatchExtend::ATTR_GROUP_TP_INDEX));
    ...
    const char *nodeName, const bool isSetFullMeshV2, uint32_t &localMoeExpertNum, bool isLayered)
{
    ...
    OP_TILING_CHECK(
        mc2tiling::GetEpWinSize(context, nodeName, hcclBufferSizeEp, maxWindowSizeEp, MoeDispatchExtend::ATTR_GROUP_EP_INDEX, isLayered) != ge::GRAPH_SUCCESS,
...
static ge::graphStatus MoeDistributeDispatchA3TilingFuncImpl(gert::TilingContext *context)
{
    ...
    // 获取入参属性
    OP_TILING_CHECK(GetAttrAndSetTilingData(context, nodeName, *tilingData, groupEp, groupTp, isSetFullMeshV2, isLayered) != ge::GRAPH_SUCCESS,
    ...
    OP_TILING_CHECK(
        TilingCheckMoeDistributeDispatch(context, nodeName, isActiveMask, isScales, hasElasticInfo, isPerformance, quantMode, isLayered) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Tiling check param failed."), return ge::GRAPH_FAILED);

    // 检查属性的取值是否合法
    OP_TILING_CHECK(CheckAttrs(context, nodeName, *tilingData, localMoeExpertNum, isActiveMask, isSetFullMeshV2, isLayered) != ge::GRAPH_SUCCESS,
    ...
    OP_TILING_CHECK(CheckTensorShape(context, nodeName, *tilingData, quantMode, isScales,
        isSharedExpert, hasElasticInfo, isPerformance, static_cast<int64_t>(localMoeExpertNum), isLayered) != ge::GRAPH_SUCCESS,
...
static ge::graphStatus MoeDistributeDispatchA2CheckAttrAndSetTiling(const gert::TilingContext *context, MoeDistributeDispatchA2Info& info, const bool isLayered)
{
    ...
    auto groupEpPtr = attrs->GetAttrPointer<char>(static_cast<int>(MoeDispatchExtend::ATTR_GROUP_EP_INDEX));
    auto epWorldSizePtr = attrs->GetAttrPointer<int>(MoeDispatchExtend::ATTR_EP_WORLD_SIZE_INDEX);
    auto epRankIdPtr = attrs->GetAttrPointer<int>(MoeDispatchExtend::ATTR_EP_RANK_ID_INDEX);
    auto moeExpertNumPtr = attrs->GetAttrPointer<int>(MoeDispatchExtend::ATTR_MOE_EXPERT_NUM_INDEX);
    auto tpWorldSizePtr = attrs->GetAttrPointer<int>(MoeDispatchExtend::ATTR_TP_WORLD_SIZE_INDEX);
    auto tpRankIdPtr = attrs->GetAttrPointer<int>(MoeDispatchExtend::ATTR_TP_RANK_ID_INDEX);
    auto expertSharedTypePtr = attrs->GetAttrPointer<int>(MoeDispatchExtend::ATTR_EXPERT_SHARD_TYPE_INDEX);
    auto sharedExpertRankNumPtr = attrs->GetAttrPointer<int>(MoeDispatchExtend::ATTR_SHARED_EXPERT_RANK_NUM_INDEX);
    auto quantModePtr = attrs->GetAttrPointer<int>(MoeDispatchExtend::ATTR_QUANT_MODE_INDEX);
    auto globalBsPtr = attrs->GetAttrPointer<int>(MoeDispatchExtend::ATTR_GLOBAL_BS_INDEX);
    auto expertTokenNumsTypePtr = attrs->GetAttrPointer<int>(MoeDispatchExtend::ATTR_EXPERT_TOKEN_NUMS_TYPE_INDEX);
    auto zeroExpertNumPtr = attrs->GetAttrPointer<int64_t>(static_cast<int>(MoeDispatchExtend::ATTR_ZERO_EXPERT_NUM_INDEX));
    auto copyExpertNumPtr = attrs->GetAttrPointer<int64_t>(static_cast<int>(MoeDispatchExtend::ATTR_COPY_EXPERT_NUM_INDEX));
    auto constExpertNumPtr = attrs->GetAttrPointer<int64_t>(static_cast<int>(MoeDispatchExtend::ATTR_CONST_EXPERT_NUM_INDEX));
                                                                     ...
                                                                     bool isLayered)
{
    ...
    const gert::StorageShape *scalesStorageShape = context->GetOptionalInputShape(MoeDispatchExtend::SCALES_INDEX);
    const gert::StorageShape *xActiveMaskStorageShape = context->GetOptionalInputShape(MoeDispatchExtend::X_ACTIVE_MASK_INDEX);
    const gert::StorageShape *expertScalesStorageShape = context->GetOptionalInputShape(MoeDispatchExtend::EXPERT_SCALES_INDEX);
    const gert::StorageShape *elasticInfoStorageShape = context->GetOptionalInputShape(MoeDispatchExtend::ELASTIC_INFO_INDEX);
    const gert::StorageShape *performanceInfoStorageShape = context->GetOptionalInputShape(MoeDispatchExtend::PERFORMANCE_INFO_INDEX);
    const gert::StorageShape *expandScalesStorageShape = context->GetOutputShape(MoeDispatchExtend::OUTPUT_EXPAND_SCALES_INDEX);
...
static ge::graphStatus MoeDistributeDispatchA2CheckCommAlg(const gert::TilingContext *context, bool &isLayered)
{
    ...
    auto commAlg = attrs->GetAttrPointer<char>(static_cast<int>(MoeDispatchExtend::ATTR_COMM_ALG_INDEX));
...
static ge::graphStatus MoeDistributeDispatchA2TilingFuncImpl(gert::TilingContext *context)
{
    ...
    // 3. communication
    auto attrs = context->GetAttrs();
    auto group = attrs->GetAttrPointer<char>(static_cast<int>(MoeDispatchExtend::ATTR_GROUP_EP_INDEX));
    auto epWorldSizePtr = attrs->GetAttrPointer<int>(MoeDispatchExtend::ATTR_EP_WORLD_SIZE_INDEX);
