                                    ...
                                    uint64_t &groupSizeN, uint64_t &groupSizeK)
{
  ...
  if (groupSizeM == 0) {
    ...
    if (mmInfo.isMxfp) {
      ...
      scaleMValue = mmInfo.x1ScaleShape->GetStorageShape().GetDim(mmInfo.x1Shape->GetStorageShape().GetDimNum() - 3);
    } else {
      scaleMValue = mmInfo.x1ScaleShape->GetStorageShape().GetDim(mmInfo.x1Shape->GetStorageShape().GetDimNum() - 2);
    }
    ...
    OP_TILING_CHECK((mValue % scaleMValue) != 0,
                     OP_LOGE(mmInfo.opName, "The groupSize in m dimension is 0 and the m dimension of x1 \
  ...
  if (groupSizeN == 0) {
    ...
    OP_TILING_CHECK((nValue % scaleNValue) != 0,
                     OP_LOGE(mmInfo.opName, "The groupSize in n dimension is 0 and the n dimension of x2 \
  ...
  if (groupSizeK == 0) {
    ...
    auto kValue = mmInfo.x1Shape->GetStorageShape().GetDim(mmInfo.x1ScaleShape->GetStorageShape().GetDimNum() - 1);
    ...
    OP_TILING_CHECK((kValue % scaleKValue) != 0,
                     OP_LOGE(mmInfo.opName, "The groupSize in k dimension is 0 and the k dimension of x1 \
