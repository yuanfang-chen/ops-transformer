...
HcclCommunication<AICSync, ServerType, ContextType, TilingDataType, Primitive, SendCnt, RecvCnt>::Init()
{
    notifyFlag_ = false;
    if ASCEND_IS_AIV {
...
template <bool AICSync, AscendC::HcclServerType ServerType, typename ContextType, typename TilingDataType,
          template <typename> class Primitive, uint32_t SendCnt, uint32_t RecvCnt>
__aicore__ inline void
HcclCommunication<AICSync, ServerType, ContextType, TilingDataType, Primitive, SendCnt, RecvCnt>::ProcessWithPrepare(
    uint32_t taskIndex)
{
    // 只有通信核参与通信
    if (!notifyFlag_) {
...
template <bool AICSync, AscendC::HcclServerType ServerType, typename ContextType, typename TilingDataType,
          template <typename> class Primitive, uint32_t SendCnt, uint32_t RecvCnt>
__aicore__ inline void
HcclCommunication<AICSync, ServerType, ContextType, TilingDataType, Primitive, SendCnt, RecvCnt>::AddIndex(
    uint32_t taskCnt)
{
    // 只有通信核参与通信
    if (!notifyFlag_) {
...
template <bool AICSync, AscendC::HcclServerType ServerType, typename ContextType, typename TilingDataType,
          class Primitive<>, uint32_t SendCnt, uint32_t RecvCnt>
__aicore__ inline void
HcclCommunication<AICSync, ServerType, ContextType, TilingDataType, Primitive<>, SendCnt, RecvCnt>::PrepareAll() {
    // 只有通信核参与通信
    if (!notifyFlag_) {
...
template <bool AICSync, AscendC::HcclServerType ServerType, typename ContextType, typename TilingDataType,
          template <typename> class Primitive, uint32_t SendCnt, uint32_t RecvCnt>
__aicore__ inline void
HcclCommunication<AICSync, ServerType, ContextType, TilingDataType, Primitive, SendCnt, RecvCnt>::End()
{
    // 全量等待通信结果
    if (notifyFlag_) {
...
#define DEFINE_MC2_HCCL_FOR_COMMUNICATION(AICSync, ServerType, HcclContextType, TilingDataType, Primitive,             \
    ...
    using CommunicationType =                                                                                          \
        MC2KernelTemplate::HcclCommunication<AICSync, ServerType, HcclContextType, TilingDataType, Primitive,          \
