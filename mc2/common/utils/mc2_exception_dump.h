...
inline void Mc2ExceptionImpl(aclrtExceptionInfo *args, void *userdata, const char *op)
{
    ...
    if ((std::strstr(socName, "Ascend910_93") != nullptr) &&
        ((std::strstr(op, "MoeDistributeDispatchV2") != nullptr) ||
        (std::strstr(op, "MoeDistributeCombineV2") != nullptr))) {
        OP_LOGE(OP_NAME, "The soc version is %s, The operator is %s, skip dump process", socName, op);
        return;
    }
