    ...
    uint64_t aivNum) 
{
    ...
    uint64_t maxPerBlockNum_ = CeilAlign::FloorAlign(
        TOTAL_UB_SIZE / UB_BUFFER_NUM,
        UB_ALIGN_BYTES
    ) / sizeof(DataType);

    // 限制UB每次搬运数据块大小。
    perBlockNum_ = CeilAlign::MIN(MAX_PER_BLOCK_NUM, maxPerBlockNum_);

    // 分核
    totalBlockNums_ = AiVReduceSumImplUtil::CeilDiv(dataSize, perBlockNum_); // 数据需要搬运的总块数

    // 尾块搬运大小
    tailBytes_ = CeilAlign::BlockAlignMod(dataSize, perBlockNum_) * sizeof(DataType); // 即计算分卡后每片的最后一个搬运数据块的字节大小
    tailPerBlockNumAlign_ = CeilAlign::CeilAlign(tailBytes_, UB_ALIGN_BYTES) / sizeof(DataType); // 即计算分卡后每片的最后一个搬运数据块的大小, 向上32B对齐
