...
public:
    ...
    const void EAxisCut(uint64_t tmpCnt, uint64_t eSize, CutResult &eCut);
...
public:
    ...
    explicit OneCalcTwoCommShardHBase(const mc2tiling::TilingArgs& args, uint64_t inputEpDim, uint64_t inputTpDim,
        ...
        // TilingMethods
        const void InitCutResult(CutResult& tmpCut, uint64_t totalLen);
        const void CutAxisE(CutResult& cutRes, uint64_t maxCutNum, uint64_t minLen, double unbalanceRatio);
        ...
        const void TrimCutResult(CutResult& tmpCut, bool setShortFlag);
