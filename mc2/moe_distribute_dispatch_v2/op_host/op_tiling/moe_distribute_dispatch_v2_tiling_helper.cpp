...
#include "moe_distribute_dispatch_v2_tiling_helper.h"
...
bool MoeDistributeDispatchV2TilingHelper::CheckInputTensorDim(const gert::TilingContext *context, const char *nodeName,
    const bool isScales, const uint32_t quantMode)
{
    const gert::StorageShape *xStorageShape = context->GetInputShape(X_INDEX + context_offset);
    ...
    const gert::StorageShape *expertIdStorageShape = context->GetInputShape(EXPERT_IDS_INDEX + context_offset);
    ...
    if (isScales) {
        const gert::StorageShape *scalesStorageShape = context->GetOptionalInputShape(SCALES_INDEX + context_offset);
...
bool MoeDistributeDispatchV2TilingHelper::CheckTensorDim(const gert::TilingContext *context, const char *nodeName,
    const bool isScales, const uint32_t quantMode, const bool isActiveMask, const bool hasElasticInfo, const bool isPerformance)
{
    ...
    if (isActiveMask) {
        ...
        const gert::StorageShape *expertIdStorageShape = context->GetInputShape(EXPERT_IDS_INDEX + context_offset);
        const int64_t expertIdDim1 = expertIdStorageShape->GetStorageShape().GetDim(1);
        const gert::StorageShape *xActiveMaskStorageShape = context->GetOptionalInputShape(X_ACTIVE_MASK_INDEX + context_offset);
    ...
    if (hasElasticInfo) {
        const gert::StorageShape *elasticInfoStorageShape = context->GetOptionalInputShape(ELASTIC_INFO_INDEX + context_offset);
    ...
    if (isPerformance) {
        const gert::StorageShape *performanceInfoStorageShape = context->GetOptionalInputShape(PERFORMANCE_INFO_INDEX + context_offset);
...
ge::graphStatus MoeDistributeDispatchV2TilingHelper::CheckQuantModeAndScales(const gert::TilingContext *context,
    const char *nodeName, bool isScales, const uint32_t quantMode)
{
    ...
    OP_TILING_CHECK(isScales && (quantMode == static_cast<uint32_t>(QuantModeA5::MX_QUANT)),
        ...
        quantMode), return ge::GRAPH_FAILED);
    auto xDesc = context->GetInputDesc(X_INDEX + context_offset);
...
bool MoeDistributeDispatchV2TilingHelper::CheckTensorDataTypeNonQuant(const gert::TilingContext *context,
    const char *nodeName, const bool isScales)
{
    ...
    if (isScales) {
        auto scalesDesc = context->GetOptionalInputDesc(SCALES_INDEX + context_offset);
...
bool MoeDistributeDispatchV2TilingHelper::CheckTensorDataTypeStaticOrDynamic(
...
bool MoeDistributeDispatchV2TilingHelper::CheckTensorDataTypeMxfp8(
...
bool MoeDistributeDispatchV2TilingHelper::CheckDistinctTensorDataType(const gert::TilingContext *context, const char *nodeName,
    const bool isScales, const uint32_t quantMode)
{  
...
bool MoeDistributeDispatchV2TilingHelper::CheckTensorDataType(const gert::TilingContext *context, const char *nodeName,
    const bool isScales, const uint32_t quantMode, const bool isActiveMask, const bool hasElasticInfo, const bool isPerformance)
{
    ...
    auto expertIdDesc = context->GetInputDesc(EXPERT_IDS_INDEX + context_offset);
    ...
    if (isActiveMask) {
        auto xActiveMaskDesc = context->GetOptionalInputDesc(X_ACTIVE_MASK_INDEX + context_offset);
    ...
    if (hasElasticInfo) {
        auto elasticInfoDesc = context->GetOptionalInputDesc(ELASTIC_INFO_INDEX + context_offset);
    ...
    if (isPerformance) {
        auto performanceInfoDesc = context->GetOptionalInputDesc(PERFORMANCE_INFO_INDEX + context_offset);
...
//校验输入输出数据格式
bool MoeDistributeDispatchV2TilingHelper::CheckTensorFormat(const gert::TilingContext *context, const char *nodeName,
...
ge::graphStatus MoeDistributeDispatchV2TilingHelper::CheckAttrs(const gert::TilingContext *context, const char *nodeName,
...
ge::graphStatus MoeDistributeDispatchV2TilingHelper::TilingCheckMoeDistributeDispatch(gert::TilingContext *context, const char *nodeName,
...
ge::graphStatus MoeDistributeDispatchV2TilingHelper::MoeDistributeDispatchA3TilingImpl(gert::TilingContext *context)
{
    const char *nodeName = context->GetNodeName();
...
MoeDistributeDispatchV2TilingHelper::MoeDistributeDispatchV2TilingHelper(bool is_context) {
    is_context_ = is_conext;
    context_offset = is_context ? 1 : 0;
}

ge::graphStatus MoeDistributeDispatchA3TilingFuncImpl(gert::TilingContext *context)
{
    // dispatch V2 does not have context as input
    MoeDistributeDispatchV2TilingHelper tilingHelper(false);
    auto ret = tilingHelper.MoeDistributeDispatchA3TilingImpl(context);
    int32_t groupEp = tilingHelper.GetGroupEp();
    int32_t groupTp = tilingHelper.GetGroupTp();
    int32_t tpWorldSize = tilingHelper.GetTpWorldSize();
    MoeDistributeDispatchV2TilingData *tilingData = context->GetTilingData<MoeDistributeDispatchV2TilingData>();
...
ge::graphStatus MoeDistributeDispatchA5TilingFuncImpl(gert::TilingContext* context)
{
    ...
    // MTE 调用 A3 tiling 实现
    MoeDistributeDispatchV2TilingHelper tilingHelper(false);
    return tilingHelper.MoeDistributeDispatchA3TilingFuncImpl(context);
}
