...
namespace {
    ...
    constexpr uint32_t H_BASIC_BLOCK_LAYERED = 256;
    constexpr uint32_t MAX_LOCAL_EXPERTNUM_LAYERED = 24;
    ...
    const bool isScales, const uint32_t quantMode, const bool isLayered)
{
    ...
    const gert::StorageShape *expertIdStorageShape = context->GetOptionalInputShape(EXPERT_IDS_INDEX);
    if (isLayered) {
        const gert::StorageShape *expertScaleStorageShape = context->GetInputShape(EXPERT_SCALES_INDEX);
        OP_TILING_CHECK(expertScaleStorageShape == nullptr, OP_LOGE(nodeName, "expertScaleShape is null."), return GRAPH_FAILED);
    ...
    bool isLayered)
{
    ...
    if (isLayered) {
        OP_TILING_CHECK((localMoeExpertNum > MAX_LOCAL_EXPERTNUM_LAYERED), OP_LOGE(nodeName,
            "Cannot support localMoeExpertNum > %u, but get localMoeExpertNum = %u > 1", MAX_LOCAL_EXPERTNUM_LAYERED,
    ...
    bool isLayered)
{
    ...
    if (isLayered) {
        OP_TILING_CHECK((xDim1 % H_BASIC_BLOCK_LAYERED), OP_LOGE(nodeName,
    ...
    const uint32_t tpWorldSize, const bool isSetFullMeshV2, bool isLayered)
{
    ...
    if (isLayered) {
        fullMesh = TILINGKEY_ENABLE_HIERARCHY;
    }
    if (mc2tiling::GetNpuArch(context) == NpuArch::DAV_3510) {
    ...
    const char *nodeName, const bool isSetFullMeshV2, uint32_t &localMoeExpertNum, bool isLayered)
{
    ...
    OP_TILING_CHECK((actualSize > maxWindowSizeEp),
        OP_LOGE(nodeName, "HCCL_BUFFSIZE_EP is too SMALL, maxBs = %lu, h = %lu, epWorldSize = %lu,"
            ...
            "sizeof(uint32_t) + 64) + 104 * 1024 * 1024)) = %luMB,"
