...
static ge::graphStatus MoeDistributeDispatchA2CheckAttrAndSetTiling(const gert::TilingContext *context, MoeDistributeDispatchA2Info& info, const bool isLayered)
{
    ...
    auto zeroExpertNumPtr = attrs->GetAttrPointer<int64_t>(ATTR_ZERO_EXPERT_NUM_INDEX);
    auto copyExpertNumPtr = attrs->GetAttrPointer<int64_t>(ATTR_COPY_EXPERT_NUM_INDEX);
    auto constExpertNumPtr = attrs->GetAttrPointer<int64_t>(ATTR_CONST_EXPERT_NUM_INDEX);
...
static ge::graphStatus MoeDistributeDispatchA2TilingFuncImpl(gert::TilingContext *context)
{
    ...
    // 3. communication
    auto attrs = context->GetAttrs();
    auto group = attrs->GetAttrPointer<char>(ATTR_GROUP_EP_INDEX);
    auto epWorldSizePtr = attrs->GetAttrPointer<int64_t>(ATTR_EP_WORLD_SIZE_INDEX);
