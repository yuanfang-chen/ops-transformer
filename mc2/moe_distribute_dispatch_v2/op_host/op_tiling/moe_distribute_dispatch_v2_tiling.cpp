    ...
    const char *nodeName)
{
    ...
    if (ubSize > ubRealSize) {
        OP_LOGE(nodeName, "Current scenario is exceeds the size limit, BS = %u, H = %u, topK = %u, moeExpertNum = %u,
            aivNum available = %u, used UBsize = %u", bsSize, hSize, kSize, 
            tilingData.moeDistributeDispatchV2Info.moeExpertNum, ascendcPlatform.GetCoreNumAiv(), ubSize);
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
