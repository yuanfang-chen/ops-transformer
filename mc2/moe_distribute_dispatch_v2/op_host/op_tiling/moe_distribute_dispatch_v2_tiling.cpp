...
static ge::graphStatus CheckUBSize(const gert::TilingContext *context, MoeDistributeDispatchV2TilingData &tilingData,
    const char *nodeName, const bool isSetFullMeshV2)
{
    if (!isSetFullMeshV2) {
        return ge::GRAPH_SUCCESS;
    }
    ...
    uint32_t hOutSizeAlign = ceil(hOutSize, UB_ALIGN);
    ...
    ubSize = ubSize + hCommuSize * BUFFER_NUM;
    ...
    uint32_t expertIdsBufSize = ceil(expertIdsCnt * sizeof(int32_t), SIZE_ALIGN_256);
    ubSize = ubSize + expertIdsBufSize;
    ...
    ubSize = ubSize + expertMaskBufSize; // expertMaskBuf
    ubSize = ubSize + ceil(moeExpertNum * sizeof(int32_t), UB_ALIGN); // tokenNumToExpertBuf
...
static ge::graphStatus MoeDistributeDispatchA3TilingFuncImpl(gert::TilingContext *context)
{
    ...
    // 校验UB大小
    OP_TILING_CHECK(CheckUBSize(context, *tilingData, nodeName, isSetFullMeshV2) != ge::GRAPH_SUCCESS,
