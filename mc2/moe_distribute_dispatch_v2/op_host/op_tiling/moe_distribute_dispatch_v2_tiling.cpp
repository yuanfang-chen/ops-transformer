...
{
    uint32_t template = TILINGKEY_NO_FULLMESH;
    bool tp = false;
    ...
    if (isSetFullMeshV2) {
        template = TILINGKEY_ENABLE_FULLMESH;
    }
    if (isLayered) {
        template = TILINGKEY_ENABLE_HIERARCHY;
    }
    if (mc2tiling::GetNpuArch(context) == NpuArch::DAV_3510) {
        tilingKey = GET_TPL_TILING_KEY(tp, tilingKeyQuantMode, scaleMode,
                                                template, commMode, TILINGKEY_TPL_A5);
    } else {
        if (tpWorldSize == MAX_TP_WORLD_SIZE) {
            tp = true;
        }
        tilingKey = GET_TPL_TILING_KEY(tp, tilingKeyQuantMode, scaleMode, 
                                                template, commMode, TILINGKEY_TPL_A3);
    }
    return tilingKey;
}
