...
static constexpr int64_t RANK_NUM_PER_A3_NODE = 16;
...
static ge::graphStatus InferShapeMoeDistributeDispatchV2(gert::InferShapeContext *context)
{
    ...
    if (expertScalesShape != nullptr) {
        assist_info_shape =  std::max(static_cast<int64_t>(globalBsReal * 2 * k * ((*epWorldSize) / RANK_NUM_PER_A3_NODE)), a * ASSIST_INFO_NUM_PER_A);
    }
    assistInfoShape->SetDim(0U, assist_info_shape);
