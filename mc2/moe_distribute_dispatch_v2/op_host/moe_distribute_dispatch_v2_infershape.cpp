...
static ge::graphStatus InferShapeMoeDistributeDispatchV2(gert::InferShapeContext *context)
{
    ...
    int64_t a;
    int64_t assist_info_shape = a * ASSIST_INFO_NUM_PER_A;
    ...
    if (expertScalesShape != nullptr) {
        assist_info_shape =  std::max(static_cast<int64_t>(globalBsReal * 2 * k * ((*epWorldSize) / RANK_NUM_PER_NODE)), assist_info_shape);
    }
