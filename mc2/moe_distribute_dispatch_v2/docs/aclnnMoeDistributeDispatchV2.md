...
- **参数说明**
    ...
    - <term>Ascend 950PR/Ascend 950DT</term>：
        ...
        - `epWorldSize`取值范围[2, 768]。
        - `moeExpertNum`取值范围(0, 1024]。
        - `groupTp在UB-MEM`通信方式下支持字符串长度范围为[1, 128)，不能和groupEp相同；当前版本CCU通信方式不支持TP域通信，传空字符即可。
        ...
        - `tpRecvCountsOut`在UB-MEM通信方式下有TP域通信时为1D shape Tensor，shape为(tpWorldSize,)；当前版本CCU通信方式不支持TP域通信。
        - `expandScalesOut`当前版本不支持该输出。
