...
- <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：
    ...
    - 参数说明里shape格式说明：
        ...
        - `Bs`：表示batch sequence size，即本卡最终输出的token数量。
            ...
            - `commAlg` = "hierarchy"：取值范围(0, 10240]。
    - `HCCL_BUFFSIZE`：调用本算子前需检查`HCCL_BUFFSIZE`环境变量取值是否合理，该环境变量表示单个通信域占用内存大小，单位MB，不配置时默认为200MB。
        ...
        - `commAlg` = "hierarchy"：要求 >= `moeExpertNum` * `maxBs` * (`H` * 2 + 16 * Align8(`K`))B + `serverNum` * `maxBs` * (`H` * 2 + 16 * Align8(`K`))B * 2 + 4MB，不要求`moeExpertNum` / `epWorldSize` <= 24，其中Align8(x) = ((x + 8 - 1) / 8) * 8。
