...
public:
    ...
    __aicore__ inline void LogInfo(const __gm__ char * func, const uint32_t line, const __gm__ char *msg, const GM_ADDR addr);
    __aicore__ inline void LogInfo(const __gm__ char * func, const uint32_t line, const __gm__ char *msg);
    template<typename T>
    __aicore__ inline void LogInfo(const __gm__ char * func, const uint32_t line, const __gm__ char *msg,   const T num);
    ...
    template <typename T,size_t N>
    __aicore__ inline void LogInfo(const __gm__ char * func, uint32_t line, const __gm__ char *msg, LocalTensor<T>& tensor,const uint32_t (&shape)[N]); //按指定shap打

    template <typename T,size_t N>
    __aicore__ inline void LogInfo(const __gm__ char * func, uint32_t line, const __gm__ char *msg, GlobalTensor<T>& tensor,const uint32_t (&shape)[N]); //按指定shap打
...
__aicore__ inline void Log::LogInfo(const __gm__ char * func, const uint32_t line, const __gm__ char *msg, const GM_ADDR addr){
    const __gm__ void * addrtemp = static_cast<const __gm__ void*>(addr);
    printf("[rankId: %d][aivId: %d][line: %d]", rankId_, aivId_, line);
    printf("%s: %p\n", msg, addrtemp);
}
__aicore__ inline void Log::LogInfo(const __gm__ char * func, const uint32_t line, const __gm__ char *msg){
    printf("[rankId: %d][aivId: %d][line: %d]%s\n", rankId_, aivId_, line, msg);
}

template<typename T>
__aicore__ inline void Log::LogInfo(const __gm__ char * func, const uint32_t line, const __gm__ char *msg,   const T num){
...
template <typename T,size_t N>
__aicore__ inline void Log::LogInfo(const __gm__ char * func, const uint32_t line, const __gm__ char *msg, LocalTensor<T>& tensor,const uint32_t (&shape)[N]) {//按指定shap打
    printf("[rankId: %d][aivId: %d][line: %d]", rankId_, aivId_, line);
    printf("%s\n", msg);
    if(tensor.GetSize() == 0){
...
template <typename T,size_t N>
__aicore__ inline void Log::LogInfo(const __gm__ char * func, const uint32_t line, const __gm__ char *msg, GlobalTensor<T>& tensor,const uint32_t (&shape)[N]) {//按指定shap打
    printf("[rankId: %d][aivId: %d][line: %d]", rankId_, aivId_, line);
    printf("%s\n", msg);
    bool cast = !is_decimal_integer_v<T> && !is_unsigned_integer_v<T> && !is_fp_point_v<T>;
    if (cast) {
