...
public:
    ...
    __aicore__ inline void LogInfo(const  char * func, const uint32_t line, const __gm__ char *msg, const GM_ADDR addr);
    __aicore__ inline void LogInfo(const  char * func, const uint32_t line, const __gm__ char *msg);
    template<typename T>
    __aicore__ inline void LogInfo(const  char * func, const uint32_t line, const __gm__ char *msg,   const T num);
    ...
    template <typename T,size_t N>
    __aicore__ inline void LogInfo(const  char * func, uint32_t line, const __gm__ char *msg, LocalTensor<T>& tensor,const uint32_t (&shape)[N]); //按指定shap打

    template <typename T,size_t N>
    __aicore__ inline void LogInfo(const  char * func, uint32_t line, const __gm__ char *msg, GlobalTensor<T>& tensor,const uint32_t (&shape)[N]); //按指定shap打
...
__aicore__ inline void Log::LogInfo(const  char * func, const uint32_t line, const __gm__ char *msg, const GM_ADDR addr){
    ...
    const __gm__ char * function = reinterpret_cast<const __gm__ char*>(func);
