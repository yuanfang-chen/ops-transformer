...
#define TemplateMC2TypeClass typename XType, typename ExpandXOutType, int32_t QuantMode, bool IsSmoothScaleExist, bool IsNeedAllgather
...
private:
    ...
    __aicore__ inline GM_ADDR GetWindAddrByRankId(uint8_t ctxIdx, const int32_t rankId)
    {
        ...
        if (ctxIdx) {
            return winContext.commTp.windowInAddr[rankId] + winDataSizeOffset;
        } else {
            return winContext.commEp.windowInAddr[rankId] + winDataSizeOffset;
        }
    }
    ...
    {
        if (ctxIdx) {
            return winContext.commTp.windowExpAddr[rankId] + dataState_ * WIN_STATE_OFFSET;
        } else {
            return winContext.commEp.windowExpAddr[rankId] + dataState_ * WIN_STATE_OFFSET;
        }
    }
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatchV2<TemplateMC2TypeFunc>::ProcessToken(
...
__aicore__ inline void MoeDistributeDispatchV2<TemplateMC2TypeFunc>::BufferInit()
{
    ...
    tpipe_->InitBuffer(tokenNumBuf_, moeExpertNumPerRank_ * UB_ALIGN);  
    uint32_t statusBufCntAlign = Ceil(Ceil(totalExpertNum_, aivNum_), 8) * 8;   // 8 = UB_ALIGN / sizeof(int32_t)
    uint32_t statusBufSize = statusBufCntAlign * UB_ALIGN;
    tpipe_->InitBuffer(statusBuf_, statusBufSize);  
}
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatchV2<TemplateMC2TypeFunc>::PerformanceInfoCopyOut()
{
    SyncFunc<AscendC::HardEvent::S_MTE3>();
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatchV2<TemplateMC2TypeFunc>::SetExpertTokenNums()
{
    ...
    for (uint32_t localExpertIdx = 0; localExpertIdx < localExpertNum; ++localExpertIdx) {
        ...
        uint32_t mask = 2;
        SyncFunc<AscendC::HardEvent::S_V>();
        ReduceSum(expertTokenNumTensor, expertStatusTensor, gatherMaskOutTensor, mask, epWorldSize_, 1);
        SyncFunc<AscendC::HardEvent::V_S>();
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatchV2<TemplateMC2TypeFunc>::AllGatherSetExpertTokenNumsAndTpRecvCount()
{
    if (aivId_ != lastCore_) {
        return;
    }
