...
private:
    ...
    __aicore__ inline void CopyExpertAndWeight(uint32_t currentTokenNum, uint32_t startTokenId,
                                               LocalTensor<uint8_t> tokenTensorU8,
                                               GlobalTensor<uint8_t> expertIdsGMTensorU8,
                                               GlobalTensor<uint8_t> weightGMTensor);
    __aicore__ inline void CalSendServerInfo(uint32_t currentTokenNum, uint32_t startTokenId,
                                             LocalTensor<uint64_t> tokenTempTensorU64);
    ...
    __aicore__ inline uint32_t GetArrivedTokenInfo(uint32_t serverIdx, uint32_t tokenIdx, bool justExpInfo,
                                                   LocalTensor<uint8_t> receiveTokenStructU8Lt);
    ...
    __aicore__ inline void SendTokenToIpc(uint32_t expStartId, uint32_t expEndId, uint32_t formServerId,
                                          LocalTensor<uint8_t> tokenStructU8Lt, LocalTensor<int32_t> tokenStructI32Lt,
                                          bool justExpInfo);
    __aicore__ inline void WriteTokenNumToIpc(uint32_t expStartId, uint32_t expEndId, uint32_t formServerId,
                                              uint32_t coresPerServer, uint32_t logicAivId);
...
{
    bufferChosenGMTensor_.SetGlobalBuffer(
        (__gm__ uint32_t *)(windowInGM_ + dataSpaceSize_ + worldSize_ * STATE_OFFSET));
...
__aicore__ inline void MoeDistributeDispatchV2Layered<TemplateMC2TypeV2LayeredFunc>::InitIPCBuffer()
{
    for (int i = 0; i < SERVER_RANK_SIZE; i++) {
        shareIPCAddrsGM_[i] = (__gm__ uint8_t *)(reinterpret_cast<uint64_t>(
...
__aicore__ inline void MoeDistributeDispatchV2Layered<TemplateMC2TypeV2LayeredFunc>::InitGlobalTensor(
    GM_ADDR expertIds, GM_ADDR expandXOut, GM_ADDR dynamicScalesOut, GM_ADDR expandScales, GM_ADDR xActiveMask,
    GM_ADDR workspaceGM)
{
    ...
    tokenDstServerMaskWorkspaceGMTensor_.SetGlobalBuffer((__gm__ uint64_t *)(workspaceGM), axisBS_ * FLAG_SIZE);
...
__aicore__ inline void MoeDistributeDispatchV2Layered<TemplateMC2TypeV2LayeredFunc>::ProcessExpandX(
    uint32_t tokenCntInBatch, uint32_t srIdx, uint32_t sumTokenCnt, uint32_t batchIdx, uint32_t tokenNumsInUB,
    GlobalTensor<uint8_t> &srcIpcGt)
{
    ...
    DataCopyPad(expandXOutGMTensor_[(sumTokenCnt + batchIdx * tokenNumsInUB) * axisH_],
    ...
    uint32_t tokenCntInBatch, uint32_t curExpIdx, uint32_t sumTokenCnt, uint32_t batchIdx, uint32_t tokenNumsInUB)
{
    ...
    for (uint32_t tokenIdx = 0; tokenIdx < tokenCntInBatch; tokenIdx++) {
        for (uint32_t expIdx = 0; expIdx < axisK_; expIdx++) {
            ...
            if (curExpIdx + rankId_ * localMoeExpertNum_ ==
                outExpIdxTensor_(expOffset)) {               // 本aiv处理的专家是当前topK Id
...
{
    DataCopyExtParams quantTokenParams{static_cast<uint16_t>(tokenCntInBatch), static_cast<uint32_t>(sizeof(float)),
    ...
    DataCopyPad(dynamicScalesOutGMTensor_[(sumTokenCnt + batchIdx * tokenNumsInUB)], quantTempUB, quantTokenParams);
}
    ...
    uint32_t srPreCnt, uint32_t srCntCurCore, uint32_t curExpIdx)
{
    ...
    outTokenTensor_ = tBuf_.GetWithOffset<uint8_t>(tokenUbSize_ / sizeof(uint8_t),
                                                   TBUF_TEMP_OFFSET); // 用int8 方便和偏移地址的单位B对齐
    ...
    int32_t sumTokenCnt =
        (srPreCnt == 0) ?
            0 :
            tokenCntTensor_(srPreCnt -
                            1); // 之前rank共有多少个token。如果当前是s0r0就是0，否则取上一个sr的tokenCntSum值
    for (uint32_t idx = 0; idx < srCntCurCore; ++idx) {
        // 循环本Core需要处理的Rank数
        uint32_t srIdx = srPreCnt + idx;
        int32_t curSrTokenCnt =
            tokenCntTensor_(srIdx) - (srIdx == 0 ? 0 : tokenCntTensor_(srIdx - 1)); // 差值为当前Server_xRank_y的token数
        if (curSrTokenCnt == 0) {
...
__aicore__ inline void MoeDistributeDispatchV2Layered<TemplateMC2TypeV2LayeredFunc>::Ipc2Out()
{
    ...
    tokenCntIpcGt.SetGlobalBuffer(
        (__gm__ int32_t *)(shareIPCAddrsGM_[rankId_ % SERVER_RANK_SIZE] + IPC_TOKEN_CNT_OFFSET));
    ...
    DataCopyExtParams copyExpertIdsParams{1,
                                          static_cast<uint32_t>(serverNum_ * SERVER_RANK_SIZE * localMoeExpertNum_ *
    ...
    // 当前core处理的rank起始位置 (最大是localMOE * worldSize，这里就是 moeIdx*worldSize 表示之前专家占用rank) +
    // 当前core在专家内的偏移 + 剩余rank偏移
    ...
    if (aivId_ == 0) {
        ...
        DataCopyExtParams countsParams{
            1, static_cast<uint32_t>(localMoeExpertNum_ * serverNum_ * SERVER_RANK_SIZE * sizeof(int32_t)), 0, 0, 0};
        SyncFunc<AscendC::HardEvent::S_MTE3>();
...
MoeDistributeDispatchV2Layered<TemplateMC2TypeV2LayeredFunc>::CreateZeroInnerCnt(uint32_t curServerId)
{
    ...
    combineInnerCntGMTensor.SetGlobalBuffer(
        (__gm__ int16_t *)(epRecvCountsOutGM_ + combineInnerCntOffset_ + globalBs_ * curServerId * sizeof(int16_t)));
...
MoeDistributeDispatchV2Layered<TemplateMC2TypeV2LayeredFunc>::CreateInnerReduceInfo(uint32_t serverIdx)
{
    ...
    LocalTensor<uint8_t> tokenTopKInfoU8Lt =
        tBuf_.GetWithOffset<uint8_t>(MAX_BS_NUM * alignK_ * sizeof(int32_t), IPC_BUFF_ALIGN);
    ...
    while (tokenStatus != FINISH_STATUS) {
        if (serverId_ == serverIdx) {
            ...
            tokenStatus =
                GetArrivedTokenInfo(curServerId, tokenIdx, true, tokenTopKInfoU8Lt[tokenIdx * expLenInStruct_]);
        }
    ...
    for (uint32_t tokenIdx = 0; tokenIdx < realBS; tokenIdx++) {
        for (uint32_t expIdx = 0; expIdx < axisK_; expIdx++) {
            ...
            if (expId >= currServerExpBegin && expId < currServerExpEnd) {
                ...
                expRecvTokenIdxMap_(expIdInServer * realBS + offsetInExp) =
                    static_cast<uint16_t>(tokenIdx); // expRecvTokenIdxMap_记录exp对应偏移位置上的token序号
                tokenOffsetLt_(tokenIdx * axisK_ + expIdx) =
                    offsetInExp; // tokenOffsetLt_记录每个token在其topk专家上的偏移值
            }
        }
    }
    ...
    uint32_t realBS, LocalTensor<int32_t> tokenTopKInfoI32Lt)
{
    ...
    for (uint32_t expBlockId = 0; expBlockId < moeExpertNumInServer_; expBlockId++) {
        uint32_t validCnt = (expBlockId % localMoeExpertNum_ == 0) ?
                                expCntMap_(expBlockId) :
                                (expCntMap_(expBlockId) - expCntMap_(expBlockId - 1));
        for (uint32_t tokenIdx = 0; tokenIdx < validCnt; tokenIdx++) {
            ...
            for (uint32_t expIdx = 0; expIdx < axisK_; expIdx++) {
                ...
                if (expId >= currServerExpBegin && expId < currServerExpEnd) {
                    ...
                    innerOffsetLt_(expOccurNum) =
                        (expIdInServer % localMoeExpertNum_ == 0) ? 0 : expCntMap_(expIdInServer - 1);
                    ...
                    expOccurNum += 1;
                    expRecvTokenIdxMap_(expIdInServer * realBS + tokenOffsetLt_(tokenId * axisK_ + expIdx)) =
                        -1; // 记录处理过的token不再重复处理
                }
            }
            tokenOccurNum += 1;
    ...
    for (uint32_t tokenIdx = 0; tokenIdx < activeMaskBsCnt_; ++tokenIdx) {
        ...
        for (uint32_t serverIdx = 0; serverIdx < serverNum_; ++serverIdx) {
            ...
            uint32_t tokenOffset =
                preTokenCnt - preServerCnt + combineOffsetIdx_(tokenIdx * serverNum_ + serverIdx) + serverBaseCnt;
...
__aicore__ inline void MoeDistributeDispatchV2Layered<TemplateMC2TypeV2LayeredFunc>::CreateOuterReduceInfo()
{
    ...
    miniServerExpIds_ =
        tBuf_.GetWithOffset<int32_t>(RoundUp(activeMaskBsCnt_ * serverNum_, B32_PER_BLOCK), baseBuffOffset);
    ...
    expertIdsI32Tensor_ =
        tBuf_.GetWithOffset<int32_t>(RoundUp(activeMaskBsCnt_ * axisK_, B32_PER_BLOCK), baseBuffOffset);
    ...
    Duplicate<int32_t>(miniServerExpIds_, int32_t(moeExpertNum_),
                       RoundUp(activeMaskBsCnt_ * serverNum_, B32_PER_BLOCK));
                                                                                        ...
                                                                                        uint32_t sendTokenNum)
{
    ...
    LocalTensor<uint8_t> tokenTensorU8 =
        tBuf_.GetWithOffset<uint8_t>(maxTokenNumInUB * quantTokenUBSize, TBUF_TEMP_OFFSET);
...
{
    // Expert进行拷贝
    DataCopyExtParams expCopyParams{static_cast<uint16_t>(currentTokenNum), static_cast<uint32_t>(realLenInStruct_), 0,
                                    static_cast<uint32_t>(infoGapInStruct_), 0};
    DataCopyPadExtParams<uint8_t> expPadParams;
    DataCopyPad(tokenTensorU8[expOffsetInStruct_], expertIdsGMTensorU8[startTokenId * realLenInStruct_], expCopyParams,
                expPadParams);
    // Weights进行拷贝
    DataCopyExtParams weightCopyParams{static_cast<uint16_t>(currentTokenNum), static_cast<uint32_t>(realLenInStruct_),
                                       0, static_cast<uint32_t>(infoGapInStruct_), 0};
    DataCopyPadExtParams<uint8_t> weightPadParams;
    DataCopyPad(tokenTensorU8[weightOffsetInStruct_], weightGMTensor[startTokenId * realLenInStruct_], weightCopyParams,
                weightPadParams);
    SyncFunc<AscendC::HardEvent::MTE2_MTE3>();
}
...
MoeDistributeDispatchV2Layered<TemplateMC2TypeV2LayeredFunc>::SendDataToServer(uint32_t dstServerId)
{
    ...
    LocalTensor<uint64_t> sendTokenInfoLocalTensor =
        tBuf_.GetWithOffset<uint64_t>((activeMaskBsCnt_ * FLAG_SIZE) / sizeof(uint64_t), 0);
    DataCopy(sendTokenInfoLocalTensor, tokenDstServerMaskWorkspaceGMTensor_,
             (activeMaskBsCnt_ * FLAG_SIZE) / sizeof(uint64_t));
    SyncFunc<AscendC::HardEvent::MTE2_S>();
    ...
    for (uint32_t tokenIdx = 0; tokenIdx < activeMaskBsCnt_; ++tokenIdx) {
        ...
        if ((dstServerInfo & dstServerMask) != 0) { // 当前有需要发送的token立即发送
            ...
            AIVRDMAPostSend((GM_ADDR)srcRdmaAddr, (GM_ADDR)dstRdmaAddr, dstRankId, tokenStructLen_, qpInfo_,
                            ubLocalTensor_, ubLocalHeadTensor_);
    ...
    AIVRDMAPostSend((GM_ADDR)srcFlagRdmaAddr, (GM_ADDR)dstFlagRdmaAddr, dstRankId, FLAG_SIZE, qpInfo_, ubLocalTensor_,
                    ubLocalHeadTensor_);
}
    ...
    uint32_t serverIdx, uint32_t tokenIdx, bool justExpInfo, LocalTensor<uint8_t> receiveTokenStructU8Lt)
{
    ...
    if (endFlagValue != END_OF_WRITE_FLAG_VALUE) {
        ...
        return WAIT_STATUS;
    } else { // 得到上个token->可以处理 读到结束符
             // 先搬运当前最后一个token，再走进来，越界读，读到不存在的token，返回finish
    ...
    uint32_t tokenIdx, bool justExpInfo, LocalTensor<uint8_t> receiveTokenStructU8Lt)
{
    ...
    if ((sendFlag & dstServerMask) == 0) {
        ...
        if (justExpInfo) {
            DataCopy(receiveTokenStructU8Lt, TokensGtU8[tokenIdx * tokenStructLen_ + expOffsetInStruct_],
                     expLenInStruct_);
        } else {
...
__aicore__ inline void MoeDistributeDispatchV2Layered<TemplateMC2TypeV2LayeredFunc>::Win2Ipc()
{
    ...
    tokenNumPerExp_ = tBuf_.GetWithOffset<int32_t>(SERVER_RANK_SIZE * localMoeExpertNum_ * EXP_TOKEN_COUNT_FLAG_CNT,
                                                   TBUF_TEMP_OFFSET);
    LocalTensor<uint8_t> tokenStructU8Lt = tBuf_.GetWithOffset<uint8_t>(
        tokenStructLen_ / sizeof(uint8_t), RoundUp(tokenNumPerExpInfoSize + TBUF_TEMP_OFFSET, IPC_BUFF_ALIGN));
    LocalTensor<int32_t> tokenStructI32Lt = tBuf_.GetWithOffset<int32_t>(
    ...
    uint32_t expStartId, uint32_t expEndId, uint32_t formServerId, uint32_t coresPerServer, uint32_t logicAivId)
{
    ...
    for (uint32_t batch = 0; batch < batchNum; batch++) {
        ...
        targetCntIpcGt.SetGlobalBuffer(
            (__gm__ int32_t *)(shareIPCAddrsGM_[targetRankId % SERVER_RANK_SIZE] + IPC_TOKEN_CNT_OFFSET));
    ...
    LocalTensor<int32_t> tokenStructI32Lt, bool justExpInfo)
{
    ...
    for (int32_t expIndex = 0; expIndex < axisK_; ++expIndex) {
        ...
        targetTokenIpcGt.SetGlobalBuffer(
            (__gm__ uint8_t *)(shareIPCAddrsGM_[targetRankId % SERVER_RANK_SIZE] + IPC_DATA_OFFSET + targetOffset));
