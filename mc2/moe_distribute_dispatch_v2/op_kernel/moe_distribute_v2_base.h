                                           ...
                                           uint32_t moeExpertNum, uint32_t epWorldSizeOriginal, uint32_t globalBS, TBuf<> dataStateBuf)
{
 ...
 __aicore__ inline void AIVRDMAPostDoorBell(__gm__ HcclAiRMAWQ* qp_ctx_entry, uint64_t curHead, __gm__ uint32_t* curHardwareHead, 
    LocalTensor<uint64_t>& ubLocalTensor_, LocalTensor<uint32_t>& ubLocalHeadTensor_)
{
    ...
    ubLocalTensor_.SetValue(0, doorBellInfo);
    ...
    PipeBarrier<PIPE_ALL>();
    AscendC::DataCopyPad(DBGlobalTensor, ubLocalTensor_, copyParams);
    PipeBarrier<PIPE_ALL>();

    ubLocalHeadTensor_.SetValue(0, (uint32_t)curHead);
    ...
    PipeBarrier<PIPE_ALL>();
    AscendC::DataCopyPad(HeadGlobalTensor, ubLocalHeadTensor_, copyParamsHead);
    PipeBarrier<PIPE_ALL>();
}
    ...
    LocalTensor<uint64_t>& ubLocalTensor_, LocalTensor<uint32_t>& ubLocalHeadTensor_)
{
    ...
    *(__gm__ uint64_t*)(sgeAddr + 2 * sizeof(uint32_t)) = (uint64_t)srcDmaAddr; // src VA addr memory registered by RNIC
    ...
    PipeBarrier<PIPE_ALL>();
    curHead++;
    AIVRDMAPostDoorBell(qp_ctx_entry, curHead, (__gm__ uint32_t*) curHardwareHead, ubLocalTensor_, ubLocalHeadTensor_);
}
