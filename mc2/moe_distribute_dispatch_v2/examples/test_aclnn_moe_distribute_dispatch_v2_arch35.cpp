...
int LaunchOneProcessForDispatchV2AndCombineV2(Args &args)
{
    int64_t g_bs = 8;
    int64_t g_h = 7168;
    int64_t g_k = 2;
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("[ERROR] CreateAclTensorX failed. ret: %d\n", ret); return ret);

    ret = CreateAclTensor(expertIdsHostData, expertIdsShape, & expertIdsDeviceAddr, aclDataType::ACL_INT32, &expertIds,
                        args.rankId, "expertIds");
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("[ERROR] CreateAclTensorExpertIds failed. ret: %d\n", ret); return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("[ERROR] CreateAclTensorScales failed. ret: %d\n", ret); return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("[ERROR] CreateAclTensorExpertScales failed. ret: %d\n", ret); return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("[ERROR] CreateAclTensorExpandX failed. ret: %d\n", ret); return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("[ERROR] CreateAclTensorDynamicScales failed. ret: %d\n", ret); return ret);

    ret = CreateAclTensor(expandIdxHostData, expandIdxShape, &expandIdxDeviceAddr, aclDataType::ACL_INT32, &expandIdx,
                        args.rankId, "expandIdx");
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("[ERROR] CreateAclTensorExpandIdx failed. ret: %d\n", ret); return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("[ERROR] CreateAclTensorExpertTokenNums failed. ret: %d\n", ret); return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("[ERROR] CreateAclTensorEpRecvCounts failed. ret: %d\n", ret); return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("[ERROR] CreateAclTensorTpRecvCounts failed. ret: %d\n", ret); return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("[ERROR] CreateAclTensorExpandScales failed. ret: %d\n", ret); return ret);
...
int main(int argc, char *argv[])
{
    ...
    for (uint32_t rankId = 0; rankId < DEV_NUM; rankId++) {
        ...
        args[rankId].tpRankId = 0;
        args[rankId].hcclEpComm = comms[rankId];
        args[rankId].context = context[rankId];
        args[rankId].stream = stream[rankId];
        threads[rankId].reset(new(std::nothrow) std::thread(&LaunchOneProcessForDispatchV2AndCombineV2, std::ref(args[rankId])));
    }
    for (uint32_t rankId = 0; rankId < DEV_NUM; rankId++) {
        threads[rankId]->join();
    }
