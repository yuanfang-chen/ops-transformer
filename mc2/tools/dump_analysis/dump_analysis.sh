...
if [ ! -n "$TARGET_DIR" ]; then
    echo "warning:TARGET_DIR undefind"
fi
if [ ! -d "$TARGET_DIR" ]; then
    echo "warning: unfind TARGET_DIR:$TARGET_DIR"
fi
#判断TOOL_PATH
if [ ! -n "$TOOL_PATH" ]; then
...
if [ ! -n "$SOC_VERSION" ]; then
    echo "warning:SOC_VERSION undefind"
fi
...
if [ ! -d "$PROFILING_PATH" ]; then
    echo "error: 指定的路径 $PROFILING_PATH 不存在"
    exit 1
fi
...
if [ "$floder_count" -eq 0 ]; then
    echo "error: 指定的路径 $PROFILING_PATH 下未找到profiling文件"
else
