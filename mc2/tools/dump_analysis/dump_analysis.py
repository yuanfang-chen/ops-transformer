...
def compare_run_num(dis_run_num_list_func: list, com_run_num_list_func: list, dis_diff_indices_func: list, 
                    com_diff_indices_func: list, max_dis_run_num_func: int, max_com_run_num_func: int,
                    card_num_func: int) -> dict:
    run_num_error_dict = {}
...
def get_dis_status_num(dis_epworldsize_func: int, dis_moe_num_func: int, all_card_num_func: int, card_num_func: int,
                       share_expert_card_count_func: int, len_status: int):
    status_num = 0
    ...
    if len_status < (status_num * 8):
        logging.error("2.1 计算得出的总状态位*8(%d) 大于dispatch状态区数据(int32)shape(%d)", (status_num * 8), len_status)
    return status_num
...
def dis_status_analysis(int32_status_data: np.ndarray, dis_status_list_func: list, dis_core_num_func: int, 
                        card_num_func: int, dis_status_num_func: int, dis_0_1_func: int, dis_unwait_index_func: list):
...
def com_status_analysis(int32_status_data: np.ndarray, com_status_list_func: list, com_core_num_func: int, bs_func: int,
                        k_func: int, share_expert_num_func: int, com_status_num_func: int, com_0_1_func: int,
                        card_num_func: int, com_unwait_index_func: list):
...
for filename in tqdm(os.listdir(os.path.join(floder_path))):
    if filename.startswith(perfix) and filename.endswith(endfix):
        ...
        if (dis_core_num != 0 and com_core_num != 0):
            error_dict.update(compare_run_num(dis_run_num_list, com_run_num_list, dis_diff_indices, com_diff_indices,
                                             max_dis_run_num, max_com_run_num, card_num))
        ...
        if (dis_core_num != 0):
            ...
            dis_status_num = get_dis_status_num(dis_epworldsize, dis_moe_num, all_card_num, card_num,
                                                share_expert_card_count, len(int32_dis_0_status))
            if dis_0_1 == 0:
                ...
                error_dict.update(dis_status_analysis(int32_dis_0_status, dis_status_list, dis_core_num, card_num,
                                                      dis_status_num, dis_0_1, dis_unwait_index))
                ...
                error_dict.update(dis_status_analysis(int32_dis_1_status, dis_status_list, dis_core_num, card_num,
                                                      dis_status_num, dis_0_1, dis_unwait_index))
                logging.info("2.3 dispatch状态区分析完成\n")
            else:
        ...
        if (com_core_num != 0):
            ...
            if com_0_1 == 0:
                ...
                error_dict.update(com_status_analysis(int32_com_0_status, com_status_list, com_core_num, bs, k,
                                                      share_expert_num, com_status_num, com_0_1, card_num,
                                                      com_unwait_index))
                ...
                error_dict.update(com_status_analysis(int32_com_1_status, com_status_list, com_core_num, bs, k,
                                                      share_expert_num, com_status_num, com_0_1, card_num,
                                                      com_unwait_index))
                logging.info("3.3 combine状态区分析完成\n")
            else:
        ...
        if (dis_core_num != 0 and com_core_num != 0):
            ...
            dis_run_num_info = pd.DataFrame([dis_run_num_list], index=[f"d{card_num}_dispatch各核执行次数数据"])
            dis_run_num_info.to_csv("win_data_list.csv", index=True,
                                    mode='a', header=not file_list_eixsts, encoding="gbk")
            com_run_num_info = pd.DataFrame([com_run_num_list], index=[f"d{card_num}_combine各核执行次数数据"])
            com_run_num_info.to_csv("win_data_list.csv", index=True, mode='a', header=False, encoding="gbk")
        ...
        if (dis_core_num != 0):
            ...
            if dis_0_1 == 0:
                ...
                int32_dis_0_status_info.to_csv("win_status_list.csv", index=True, mode='a', header=False,
                                                encoding="gbk")
            else:
                int32_dis_1_status_info = pd.DataFrame([int32_dis_1_status],
                                                        index=[f"d{card_num}_dispatch 1区状态区数据"])
                int32_dis_1_status_info.to_csv("win_status_list.csv", index=True, mode='a', header=False,
                                                encoding="gbk")
        ...
        if (com_core_num != 0):
            ...
            com_core_info.to_csv("win_data.csv", index=True, mode='a', header=not file_eixsts)
            ...
            if com_0_1 == 0:
                ...
                int32_com_0_status_info.to_csv("win_status_list.csv", index=True, mode='a', header=False,
                                                encoding="gbk")
            else:
                int32_com_1_status_info = pd.DataFrame([int32_com_1_status],
                                                        index=[f"d{card_num}_combine 1区状态区数据"])
                int32_com_1_status_info.to_csv("win_status_list.csv", index=True, mode='a', header=False,
                                                encoding="gbk")
