...
def check_dis_com(target_path: str) -> str:
    for filename in os.listdir(os.path.join(target_path)):
        if filename.lower().endswith('host.o'):
            if 'dispatch' in filename.lower():
                logging.info('1.2 根据dump数据文件名%s,判断该卡挂在dispatch', filename)
                return 'dispatch'
            elif 'combine' in filename.lower():
                logging.info('1.2 根据dump数据文件名%s,判断该卡挂在combine', filename)
                return 'combine'
            else:
                logging.warning('1.2 根据dump数据文件名%s,该卡没有挂在dispatch/combine算子上', filename)
                return filename

#判断topk是否超范围(<0 or >moe专家数)
def check_topk(target_path: str, moe_num_func: int, bs_func: int, sp_moe_num: int):
    k = 0
    for filename in os.listdir(os.path.join(target_path)):
        if 'input.1.bin' in filename:
            file_path = os.path.join(target_path, filename)
            expert_ids = np.fromfile(file_path, dtype = np.int32)
            k = int(expert_ids.shape[0] / bs_func)
            expert_ids_reshape = expert_ids.reshape(bs, k)
            logging.info('1.3 该卡的输入expertids为%s\n', expert_ids_reshape)
            mask = (expert_ids_reshape <0) | (expert_ids_reshape >= (moe_num_func + sp_moe_num))
            if np.any(mask):
                row,col = np.where(mask)
                logging.warning('1.3 检测到expertids中有%d个异常输入, expertids中的值应该大于0 且小于moe专家数%d', 
                                len(row), moe_num_func)
                for r, c in zip(row, col):
                    logging.warning('1.3 expertids中下标[%d,%d]的值:%d超范围', r, c, expert_ids_reshape[r][c])
            else:
                logging.info('1.3 未检测到expertids中有异常输入')
            return k, expert_ids_reshape
    if k == 0:
...
def get_a(target_path: str):
    ...
    for filename in os.listdir(os.path.join(target_path)):
        if 'input.2.bin' in filename:
            assist_info_for_combine_path = os.path.join(target_path, filename)
            assist_info_for_combine = np.fromfile(assist_info_for_combine_path, dtype = np.int32)
            a = int(assist_info_for_combine.shape[0] / 128)
            break
    return a

#获取h
def get_h(target_path: str, a: int, tp_worldsize_func: int):
    h = 0
    for filename in os.listdir(os.path.join(target_path)):
        if 'input.0.bin' in filename:
            expands_path = os.path.join(target_path, filename)
            expands = np.fromfile(expands_path, dtype = np.int16)
            h = int(expands.shape[0] / (tp_worldsize_func * a))
            break
    return h
#获取本卡专家数
...
def count_epsendcnt(ep_worldsize_func: int, tp_worldsize_func: int, local_expert_num_func: int, expertids_func):
    ...
    for expert_id in all_expertids:
        if 0 <= expert_id < shape_func:
...
def compare_epsendcnt(target_path: str, epsendcnt_func):
    for filename in os.listdir(os.path.join(target_path)):
        if 'input.3.bin' in filename:
            file_path = os.path.join(target_path, filename)
            epsendcnt_input = np.fromfile(file_path, dtype = np.int32)
...
def get_dump_expandidx(target_path: str):
    ...
    for filename in os.listdir(os.path.join(target_path)):
        if 'input.2.bin' in filename:
            file_path = os.path.join(target_path, filename)
            expandidx_dump = np.fromfile(file_path, dtype = np.int32).tolist()
...
single_mul_card = int(sys.argv[9])
error_dict = {}
max_dis_run_num = 0
max_com_run_num = 0
dis_moe_num = 0
...
for filename in os.listdir(os.path.join(floder_path)):
    if filename.startswith(perfix) and filename.endswith(endfix):
        ...
        if card_num == (all_card_num - 1):
            ...
            if dis_run_judge and com_run_judge:
                logging.info("6. 多卡的dispatch、combine的执行次数完全相同")
            else:
                if not dis_run_judge:
                    logging.warning("6. 多卡对比中,有卡的dispatch执行次数与其他卡不相同")
                if not com_run_judge:
                    logging.warning("6. 多卡对比中,有卡的combine执行次数与其他卡不相同")

            if dis_moe_judge and com_moe_judge:
                logging.info("6. 多卡的dispatch、combine的moe专家数完全相同")
            else:
                if not dis_moe_judge:
                    logging.warning("6. 多卡对比中,有卡的dispatchmoe专家数与其他卡不相同")
                if not com_moe_judge:
                    logging.warning("6. 多卡对比中,有卡的combinemoe专家数与其他卡不相同")

            if dis_bs_judge and com_bs_judge:
                logging.info("6. 多卡的dispatch、combine的globalbs完全相同")
            else:
                if not dis_bs_judge:
                    logging.warning("6. 多卡对比中,有卡的dispatchglobalbs与其他卡不相同")
                if not com_bs_judge:
                    logging.warning("6. 多卡对比中,有卡的combineglobalbs与其他卡不相同")
            logging.info("6. 各卡dispatch的执行次数:%s", all_card_dis_run_num[:all_card_num])
            logging.info("6. 各卡combine的执行次数:%s", all_card_com_run_num[:all_card_num])
            logging.info("6. 各卡dispatch的moe专家数:%s", all_card_dis_moe_num[:all_card_num])
            logging.info("6. 各卡combine的moe专家数:%s", all_card_com_moe_num[:all_card_num])
            logging.info("6. 各卡dispatch的globalbs:%s", all_card_dis_globalbs_num[:all_card_num])
            logging.info("6. 各卡combine的globalbs:%s", all_card_com_globalbs_num[:all_card_num])
            logging.info("6. 各卡dispatch、combine执行次数、moe专家数、globalbs数据已归档至win_all_card_data.csv")
                ...
                if all_card_dis_com[card_id] == 'combine':
                    ...
                    for expand_idx, (triple1, triple2) in enumerate(zip(compare_expandidx_final, dump_expandidx)):
                        if triple1 != triple2:
                            ...
                            logging.warning('7. 第%d个三元组不一致:计算的expandidx:%s,dump数据取出的expandidx',
                                            triple1, triple2)
