...
# 获取每张卡对应的csv表的绝对路径
def find_device_and_csv_by_num(profiling_path, card_num_func):
    ...
    for root, dirs, _ in os.walk(profiling_path):
        for dir_name in dirs:
            match = device_pattern.match(dir_name)
            if match:
                ...
                if 0 <= device_num < card_num_func:
                    ...
                    csv_search_root_mapping[device_num] = csv_search_root
...
def get_card_data_by_cprofiling(csv_path_list_func):
    ...
    for card_num in range(1, card_num_func):
        ...
        if card_path is None or not os.path.exists(card_path):
            logging.warning("%d卡的profiling数据文件不存在, 跳过", card_num)
            continue
        try:
            df_card = pd.read_csv(card_path)
            for col in ['Type', 'Duration(us)']:
                if col not in df_card.columns:
                    logging.warning("%d卡的profiling数据文件缺少列%s", card_num, col)
            current_type_count = {}
            card_map = {}
            ...
            logging.warning("%d卡的profiling数据文件读取失败", card_num)
            continue
    return reslut_dict
...
def get_all_data(reslut_dict):
    ...
    logging.info("profiling详细数据已存储至profiling_all_data.csv")


def calculate_avg(reslut_dict):
    type_total_count = {}
    logging.info("计算算子平均抖动时, 不计算算子第0次出现时的抖动值")
    for (type_name, count) in reslut_dict.keys():
    ...
    logging.info("所有算子的平均抖动已存储至profiling_avg_data.csv")
        ...
        for typ in df["Type"].unique():
            ...
            for col in aiv_cols:
                ...
                if valid_vals.empty:
                    ...
                    type_avg[typ][col] = round(valid_vals.mean(), 6)
            ...
            for col in aiv_cols:
                ...
                if not isinstance(value, (int, float)) or value <= 0:
                    logging.warning("%d 卡| Type=%s| 第%d次出现| 列名=%s| 值=%s | 非正数", card_id, current_type, seq_num,
                                    col, str(value))
                    continue
                avg_val = type_avg[current_type].get(col)
                if avg_val is None or not isinstance(avg_val, (int, float)):
                    continue
                lower = avg_val * 0.5
                upper = avg_val * 1.5
                if not (lower <= value <= upper):
                    logging.warning("%d 卡| Type=%s| 第%d次出现| 列名=%s| 值=%.2f| 平均值=%.2f| 超出范围 %.2f - %.2f",
