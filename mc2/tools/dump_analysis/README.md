...
- dump数据解析:对指定的dump数据进行解析，获得该dump数据的输入、输出以及win区数据，判断出每张卡对应的moe专家数、使用核数、执行次数、epworldsize以及0/1标识位，并判断每个核上的这些参数是否一致，并将分析出有异常的点输出。

- 执行序分析:根据每张卡的dispatchv2&combinev2的执行次数关系，判断该卡挂在dispatchv2/combinev2算子上

- 状态位分析:获取每个卡中每个核的执行位置信息，判断哪些核没有等到状态位，并根据对应的dispatchv2&combinev2的0/1标识位，到对应的0/1状态区找出没等到状态的核里面具体是第几个状态位没有等到。
...
<table style="undefined;table-layout: fixed; width: 1392px"> <colgroup>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>输出</td>
   <td>存放win区中0/1标识区指定的dispatchv2/combinev2的0/1状态区的所有状态位数据，用于查看没等到状态的核里面具体是第几个状态位没有等到。</td>
   <td>csv表</td>
  </tr>
  <tr>                                                 
   <td>win_all_card_rum_num</td>
   <td>输出</td>
   <td>存放每张卡dispatchv2&combinev2的执行次数,用于对比每张卡的dispatchv2&combinev2的执行次数是否一致。</td>
   <td>csv表</td>
  </tr>
  <tr>
   <td>win_analysis_list</td>
   <td>输出</td>
   <td>存放分析出的异常点及异常位置，即把打屏日志中的Warning信息存储在该csv文件中</td>
   <td>csv表</td>
  </tr>
  <tr>
   <td>win_data</td>
   <td>输出</td>
   <td>存放解析出的各卡的moe专家数、使用核数、执行次数、epworldsize以及0/1标识位数据。</td>
   <td>csv表</td>
  </tr>
  <tr>
   <td>win_data_list</td>
   <td>输出</td>
   <td>存放解析出的各卡中每个核的moe专家数、使用核数、执行次数、epworldsize以及0/1标识位数据</td>
   <td>csv表</td>
  </tr>
 </tbody>
</table>
