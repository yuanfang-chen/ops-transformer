...
// 获取必选输入
graphStatus MoeDistributeDispatchGetOpInput(OpExecuteContext* hostApiCtx, OpInput &opInput)
{
    opInput.x = hostApiCtx->GetInputTensor(static_cast<size_t>(0));
...
// 获取可选输入
void MoeDistributeDispatchGetOpOptionalInput(OpExecuteContext* hostApiCtx, OpOptionalInput &opOptionalInput)
{
    opOptionalInput.scales = hostApiCtx->GetOptionalInputTensor(static_cast<size_t>(2));
    opOptionalInput.xActiveMask = hostApiCtx->GetOptionalInputTensor(static_cast<size_t>(3));
    opOptionalInput.expertScales = hostApiCtx->GetOptionalInputTensor(static_cast<size_t>(4));
}

// 获取输出
graphStatus MoeDistributeDispatchGetOpOutput(OpExecuteContext* hostApiCtx, OpOutput &opOutput)
{
    opOutput.expandX = hostApiCtx->GetOutputTensor(static_cast<size_t>(0));
    OP_CHECK_IF(opOutput.expandX == nullptr, OP_LOGE(moeDistributeDispatchInfo, "expandX is null"), return ge::GRAPH_FAILED);

    opOutput.dynamicScales = hostApiCtx->GetOutputTensor(static_cast<size_t>(1));
    ...
    opOutput.expandIdx = hostApiCtx->GetOutputTensor(static_cast<size_t>(2));
    OP_CHECK_IF(opOutput.expandIdx == nullptr, OP_LOGE(moeDistributeDispatchInfo, "expandIdx is null"), return ge::GRAPH_FAILED);

    opOutput.expertTokenNums = hostApiCtx->GetOutputTensor(static_cast<size_t>(3));
...
static graphStatus MoeDistributeDispatchExecuteFunc(OpExecuteContext* hostApiCtx)
{
    OP_LOGD(moeDistributeDispatchInfo, "start to fallback for moeDistributeDispatch");
    OP_CHECK_IF(hostApiCtx == nullptr, OP_LOGE(moeDistributeDispatchInfo, "hostApiCtx is null"), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(MoeDistributeDispatchGetOpInput(hostApiCtx, opInput) != ge::GRAPH_SUCCESS,
                OP_LOGE(moeDistributeDispatchInfo, "get input failed"), return ge::GRAPH_FAILED);
    MoeDistributeDispatchGetOpOptionalInput(hostApiCtx, opOptionalInput);
    OP_CHECK_IF(MoeDistributeDispatchGetOpOutput(hostApiCtx, opOutput) != ge::GRAPH_SUCCESS,
                OP_LOGE(moeDistributeDispatchInfo, "get output failed"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(MoeDistributeDispatchGetOpAttrs(hostApiCtx, opAttrs) != ge::GRAPH_SUCCESS,
