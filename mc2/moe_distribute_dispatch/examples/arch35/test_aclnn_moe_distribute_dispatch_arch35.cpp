...
int LaunchOneProcessForDispatchAndCombine(Args &args)
{
    int64_t g_bs = 8;
    int64_t g_h = 7168;
    int64_t g_k = 2;
...
int main(int argc, char *argv[])
{
    ...
    for (uint32_t rankId = 0; rankId < DEV_NUM; rankId++) {
        ...
        threads[rankId].reset(new(std::nothrow) std::thread(&LaunchOneProcessForDispatchAndCombine, std::ref(args[rankId])));
    }
    for (uint32_t rankId = 0; rankId < DEV_NUM; rankId++) {
        threads[rankId]->join();
    }
