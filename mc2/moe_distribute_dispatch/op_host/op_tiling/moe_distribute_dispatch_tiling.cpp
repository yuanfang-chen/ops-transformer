...
struct MoeDistributeDispatchAttrPointers {
...
static ge::graphStatus CheckAttrPointersNotNull(const char *nodeName, const MoeDistributeDispatchAttrPointers &attrPtrs)
{
...
static ge::graphStatus CheckAttrValuesValid(const char *nodeName, const MoeDistributeDispatchAttrPointers &attrPtrs, std::string &groupTp)
{
    ...
    MoeDistributeDispatchTilingData &tilingData, std::string &groupEp, std::string &groupTp)
{
    ...
    MoeDistributeDispatchAttrPointers attrPtrs;  
...
static ge::graphStatus MoeDistributeDispatchCheckExpertParams(gert::TilingContext *context, const char *nodeName,
...
static ge::graphStatus MoeDistributeDispatchCheckEpWorldSizeParams(gert::TilingContext *context, const char *nodeName,
...
static ge::graphStatus MoeDistributeDispatchCheckBatchParams(gert::TilingContext *context, const char *nodeName,
    MoeDistributeDispatchTilingData &tilingData) 
{
    uint32_t epWorldSize = tilingData.moeDistributeDispatchInfo.epWorldSize;
...
static ge::graphStatus CheckAttrs(gert::TilingContext *context, const char *nodeName,
    MoeDistributeDispatchTilingData &tilingData, uint32_t &localMoeExpertNum)
{
    OP_TILING_CHECK(MoeDistributeDispatchCheckExpertParams(context, nodeName, tilingData, localMoeExpertNum) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Check expert params (shared expert/K value/local expert num) failed."), return ge::GRAPH_FAILED);

    OP_TILING_CHECK(MoeDistributeDispatchCheckEpWorldSizeParams(context, nodeName, tilingData) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Check arch params (epWorldSize rule) failed."), return ge::GRAPH_FAILED);

    OP_TILING_CHECK(MoeDistributeDispatchCheckBatchParams(context, nodeName, tilingData) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Check batch params (bs/globalBs) failed."), return ge::GRAPH_FAILED);

    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus MoeDistributeDispatchCheckInputTensorShape(gert::TilingContext *context, const char *nodeName,
...
static ge::graphStatus MoeDistributeDispatchCheckCommTokenShape(gert::TilingContext *context, const char *nodeName,
...
static ge::graphStatus MoeDistributeDispatchCheckOutputTensorShape(gert::TilingContext *context, const char *nodeName,
    MoeDistributeDispatchTilingData &tilingData, const uint32_t quantMode, 
    const bool isSharedExpert, const int64_t localMoeExpertNum, const uint32_t A) 
{
    const gert::StorageShape *xStorageShape = context->GetInputShape(X_INDEX);
    const int64_t xDim0 = xStorageShape->GetStorageShape().GetDim(0);
    const int64_t xDim1 = xStorageShape->GetStorageShape().GetDim(1);
    ...
    // 调用通信域校验
    OP_TILING_CHECK(MoeDistributeDispatchCheckCommTokenShape(context, nodeName, tilingData, isSharedExpert, localMoeExpertNum) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Check comm token shape (epRecvCount/tpRecvCount) failed."),
        return ge::GRAPH_FAILED);
...
{
    // 输入Tensor校验
    OP_TILING_CHECK(MoeDistributeDispatchCheckInputTensorShape(context, nodeName, tilingData, isScales) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Check input tensor shape failed."),
        return ge::GRAPH_FAILED);
    ...
    // 输出Tensor校验
    OP_TILING_CHECK(MoeDistributeDispatchCheckOutputTensorShape(context, nodeName, tilingData, quantMode, isSharedExpert, localMoeExpertNum, A) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Check output tensor shape failed."),
        return ge::GRAPH_FAILED);
...
static ge::graphStatus MoeDistributeDispatchA3A5TilingFuncImpl(gert::TilingContext *context)
{
    ...
    OP_TILING_CHECK(MoeDistributeDispatchA3A5TilingCheckAttr(context, quantMode, isScales) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "MoeDistributeDispatchA3A5Tiling Check failed."),
        return ge::GRAPH_FAILED);
    ...
    MoeDistributeDispatchA2Info& info, const bool isLayered)
{
    ...
    auto expertTokenNumsTypePtr = attrs->GetAttrPointer<int>(ATTR_EXPERT_TOKEN_NUMS_TYPE_INDEX);
    const gert::StorageShape *expertIdStorageShape = context->GetInputShape(EXPERT_IDS_INDEX);
    OP_TILING_CHECK(expertIdStorageShape == nullptr, OP_LOGE(K_INNER_DEBUG, "expertIdShape is null."), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(epWorldSizePtr == nullptr || *epWorldSizePtr <= 0 || *epWorldSizePtr > MAX_EP_WORLD_SIZE_A2 ||
        *epWorldSizePtr % RANK_NUM_PER_NODE_A2 != 0,
        OP_LOGE(K_INNER_DEBUG, "epWorldSize is invalid."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(epRankIdPtr == nullptr || *epRankIdPtr < 0 || *epRankIdPtr >= *epWorldSizePtr,
        OP_LOGE(K_INNER_DEBUG, "epRankId is invalid."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(moeExpertNumPtr == nullptr || *moeExpertNumPtr % *epWorldSizePtr != 0 ||
        *moeExpertNumPtr <= 0 || *moeExpertNumPtr > MAX_MOE_EXPERT_NUMS_A2,
        OP_LOGE(K_INNER_DEBUG, "moeExpertNum is invalid."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(!isLayered && *moeExpertNumPtr / *epWorldSizePtr > UNLAYERED_EXP_NUM_PER_RANK_A2,
        OP_LOGE(K_INNER_DEBUG, "moeExpertNum is %d, in case of unlayered, it must no more than %d.", 
            *moeExpertNumPtr / *epWorldSizePtr, UNLAYERED_EXP_NUM_PER_RANK_A2), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(tpWorldSizePtr == nullptr, OP_LOGE(K_INNER_DEBUG, "tpWorldSize is null."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(tpRankIdPtr == nullptr, OP_LOGE(K_INNER_DEBUG, "tpRankId is null."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(expertSharedTypePtr == nullptr,
        OP_LOGE(K_INNER_DEBUG, "expertSharedType is null."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(sharedExpertRankNumPtr == nullptr,
        OP_LOGE(K_INNER_DEBUG, "sharedExpertRankNum is null."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(quantModePtr == nullptr || (*quantModePtr != UNQUANT_MODE && *quantModePtr != DYNAMIC_QUANT_MODE),
        OP_LOGE(K_INNER_DEBUG, "quantMode is invalid."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(globalBsPtr == nullptr, OP_LOGE(K_INNER_DEBUG, "globalBs is null."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(expertTokenNumsTypePtr == nullptr || *expertTokenNumsTypePtr < 0 || *expertTokenNumsTypePtr > 1,
        OP_LOGE(K_INNER_DEBUG, "expertTokenNumsType is invalid. Must be 0 or 1. "), return ge::GRAPH_FAILED);
                                                                     ...
                                                                     MoeDistributeDispatchA2Info &info)
{
    ...
    OP_TILING_CHECK(xStorageShape == nullptr,
        OP_LOGE(K_INNER_DEBUG, "xShape is null."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(expertIdStorageShape == nullptr,
        OP_LOGE(K_INNER_DEBUG, "expertIdShape is null."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(xStorageShape->GetStorageShape().GetDimNum() != TWO_DIMS,
        OP_LOGE(K_INNER_DEBUG, "x dims is invalid."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(expertIdStorageShape->GetStorageShape().GetDimNum() != TWO_DIMS,
        OP_LOGE(K_INNER_DEBUG, "expertId dims is invalid."), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(h % BLOCK_SIZE_A2 != 0 || h == 0 || h > MAX_HIDDEN_SIZE_A2,
        OP_LOGE(K_INNER_DEBUG, "hiddensize is invalid."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(bs == 0 || bs > MAX_BATCH_SIZE_A2,
        OP_LOGE(K_INNER_DEBUG, "batchsize is invalid."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(k == 0 || k > MAX_K_VALUE_A2,
        OP_LOGE(K_INNER_DEBUG, "k is invalid."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(*quantModePtr == UNQUANT_MODE && isScales,
        OP_LOGE(K_INNER_DEBUG, "scales should be null when quantMode is unQuant."), return ge::GRAPH_FAILED);
