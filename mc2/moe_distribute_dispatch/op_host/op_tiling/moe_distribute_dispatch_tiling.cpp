    ...
    MoeDistributeDispatchA2Info &info, bool isLayered)
{
    ...
    if (isLayered) {
        ...
        if (minHcclBuffSize > hcclBuffSize) {
            OP_LOGE(
                    "MoeDistributeDispatch：HCCL_BUFFSIZE is too small, min required HCCL_BUFFSIZE ((moeExpertNum + epWorldSize / 4) * maxBs "
        ...
        if (minHcclBuffSize > hcclBuffSize) {
            OP_LOGE(
                    "MoeDistributeDispatch：HCCL_BUFFSIZE is too small, min required HCCL_BUFFSIZE (%lu * (maxBs * epWorldSize * "
...
static ge::graphStatus MoeDistributeDispatchA2TilingCheckAll(gert::TilingContext *context, const bool isLayered)
{
    const char *nodeName = context->GetNodeName();
