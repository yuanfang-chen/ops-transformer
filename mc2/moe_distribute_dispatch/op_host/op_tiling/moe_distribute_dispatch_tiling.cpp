...
static ge::graphStatus CheckAttrPointersNotNull(const char *nodeName, const MoeDistributeDispatchAttrPointers &attrPtrs)
{
    ...
    OP_TILING_CHECK(attrPtrs.epWorldSizePtr == nullptr, OP_LOGE(nodeName, "epWorldSizePtr is null."), 
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(attrPtrs.tpWorldSizePtr == nullptr, OP_LOGE(nodeName, "tpWorldSizePtr is null."), 
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(attrPtrs.epRankIdPtr == nullptr, OP_LOGE(nodeName, "epRankIdPtr is null."), 
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(attrPtrs.tpRankIdPtr == nullptr, OP_LOGE(nodeName, "tpRankIdPtr is null."), 
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(attrPtrs.expertShardPtr == nullptr, OP_LOGE(nodeName, "expertShardPtr is null."), 
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(attrPtrs.sharedExpertRankNumPtr == nullptr, OP_LOGE(nodeName, "sharedExpertRankNumPtr is null."),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(attrPtrs.moeExpertNumPtr == nullptr, OP_LOGE(nodeName, "moeExpertNumPtr is null."), 
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(attrPtrs.quantModePtr == nullptr, OP_LOGE(nodeName, "quantModePtr is null."), 
        return ge::GRAPH_FAILED);
...
static ge::graphStatus CheckAttrValuesValid(const char *nodeName, 
    const MoeDistributeDispatchAttrPointers &attrPtrs, std::string &groupTp)
{
    ...
    if (*attrPtrs.tpWorldSizePtr > 1) {
        ...
        OP_TILING_CHECK(*attrPtrs.tpRankIdPtr != 0,
            ...
            *attrPtrs.tpRankIdPtr), return ge::GRAPH_FAILED);
    }
    ...
    OP_TILING_CHECK(*attrPtrs.sharedExpertNumPtr != 1,
        OP_LOGE(nodeName, "sharedExpertNum only support 1, but got sharedExpertNum=%ld.", *attrPtrs.sharedExpertNumPtr), 
        return ge::GRAPH_FAILED);
...
{
    OP_TILING_CHECK(
        MoeDistributeDispatchCheckExpertParams(context, nodeName, tilingData, localMoeExpertNum) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Check expert params (shared expert/K value/local expert num) failed."), 
        return ge::GRAPH_FAILED);

    OP_TILING_CHECK(
        MoeDistributeDispatchCheckEpWorldSizeParams(context, nodeName, tilingData) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Check arch params (epWorldSize rule) failed."), 
        return ge::GRAPH_FAILED);

    OP_TILING_CHECK(
        MoeDistributeDispatchCheckBatchParams(context, nodeName, tilingData) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName, "Check batch params (bs/globalBs) failed."), 
        return ge::GRAPH_FAILED);
    ...
    const bool isSharedExpert, const int64_t localMoeExpertNum, const uint32_t A) 
{
    ...
    // 调用通信域校验
    OP_TILING_CHECK(MoeDistributeDispatchCheckCommTokenShape(
            context, nodeName, tilingData, isSharedExpert, localMoeExpertNum) != ge::GRAPH_SUCCESS,
    ...
    const bool isSharedExpert, const int64_t localMoeExpertNum)
{
    // 输入Tensor校验
    OP_TILING_CHECK(
        MoeDistributeDispatchCheckInputTensorShape(context, nodeName, tilingData, isScales) != ge::GRAPH_SUCCESS,
    ...
    // 输出Tensor校验
    OP_TILING_CHECK(MoeDistributeDispatchCheckOutputTensorShape(
