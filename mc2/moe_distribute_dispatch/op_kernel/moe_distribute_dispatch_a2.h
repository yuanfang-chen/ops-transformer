...
__aicore__ inline void MoeDistributeDispatchA2<TemplateMC2TypeA2Func>::AllocTensor() {
    ...
    dataOffset_ = statusEntryCount_ * sizeof(int32_t);
...
{
    if (aivId_ == 0) {
        Duplicate<int32_t>(statusTensor_, 0, worldSize_ * dataOffset_ / sizeof(int32_t));
        SyncFunc<AscendC::HardEvent::V_MTE3>();
        uint32_t dstStrideU32 = dataSizePerRank_ - dataOffset_;
        auto copyStatusParams = DataCopyExtParams{static_cast<uint16_t>(worldSize_), dataOffset_, 0, dstStrideU32, 0};
