...
/*!
 * \file moe_distribute_combine_v2_extend_def.cpp
 * \brief
 */
...
namespace ops {
class MoeDistributeCombineV2Extend : public OpDef {
public:
  explicit MoeDistributeCombineV2Extend(const char* name) : OpDef(name) {
    this->Input("expand_x")
        .ParamType(REQUIRED)
...
OP_ADD(MoeDistributeCombineV2Extend);
