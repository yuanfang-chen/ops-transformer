                                               ...
                                               std::string &groupEp, std::string &groupTp, uint32_t &commQuantMode)
{
    ...
    OP_TILING_CHECK((*commQuantModePtr != 0) && (*commQuantModePtr != INT8_COMM_QUANT),
                    OP_LOGE(nodeName,
                            "commQuantMode only support 0(default) or 2(int8 comm quant), but got commQuantMode=%ld.",
                            *commQuantModePtr),
                    return ge::GRAPH_FAILED);
                             ...
                             const bool hasElasticInfo, const bool isPerformance)
{
    ...
    if (hasElasticInfo) {
        const gert::StorageShape *elasticInfoStorageShape =
            context->GetOptionalInputShape(Idx::input::ELASTIC_INFO_INDEX);
    ...
    if (isPerformance) {
        const gert::StorageShape *performanceInfoStorageShape =
            context->GetOptionalInputShape(Idx::input::PERFORMANCE_INFO_INDEX);
    ...
    if (tpWorldSize == TP_WORLD_SIZE_TWO) {
        const gert::StorageShape *tpSendCountStorageShape =
            context->GetOptionalInputShape(Idx::input::TP_SEND_COUNTS_INDEX);
    ...
    if (isActiveMask) {
        const gert::StorageShape *xActiveMaskStorageShape =
            context->GetOptionalInputShape(Idx::input::X_ACTIVE_MASK_INDEX);
    ...
    if (sharedExpertXShape != nullptr) {
        ...
        if (sharedExpertXShape->GetStorageShape().GetDimNum() == TWO_DIMS) {
            OP_TILING_CHECK(sharedExpertXDim0 != expertIdsDim0,
                            OP_LOGE(nodeName,
                                    "sharedExpertX's dim0 not equal to bs, sharedExpertX's dim0 = %ld, bs = %ld",
...
static bool CheckSharedAttrs(const char *nodeName, const MoeDistributeCombineV2TilingData &tilingData)
{
    ...
    OP_TILING_CHECK((sharedExpertNum == 0U) && (sharedExpertRankNum > 0U),
                    OP_LOGE(nodeName,
                            "sharedExpertRankNum is invalid, only support 0 when sharedExpertNum is 0, but got %u.",
                            sharedExpertRankNum),
                    return false);
    OP_TILING_CHECK((sharedExpertNum > 0U) && (sharedExpertRankNum == 0U),
                    OP_LOGE(nodeName,
                            "sharedExpertNum is invalid, only support 0 when sharedExpertRankNum is 0, but got %u.",
                            sharedExpertNum),
                    return false);
                       ...
                       const char *nodeName, uint32_t &localMoeExpertNum, bool isActiveMask)
{
    ...
    const gert::StorageShape *constExpertVStorageShape =
        context->GetOptionalInputShape(Idx::input::CONST_EXPERT_V_INDEX);
                                   ...
                                   const std::string groupEp, const std::string groupTp, const uint32_t tpWorldSize)
{
    ...
    if (tpWorldSize > 1) {
        ...
        OP_TILING_CHECK(mc2CcTilingConfig.GetTiling(tiling->mc2CcTiling2) != 0,
                        OP_LOGE(nodeName, "mc2CcTilingConfig mc2tiling2 GetTiling  mc2CcTiling2 failed"),
                        return ge::GRAPH_FAILED);
    }
    return ge::GRAPH_SUCCESS;
}
                      ...
                      MoeDistributeCombineV2TilingData *tilingData)
{
    ...
    uint32_t totalBufferSize = maxSizeTokenBuf + maxSizeRowTmpFloatBuf + mulBufSize + hFloatAlign32Size +
                               hExpandXAlign32Size * BUFFER_NUM + flagRcvCount * STATE_OFFSET * BUFFER_NUM + UB_ALIGN;
    if (*commQuantModePtr == INT8_COMM_QUANT) {
        uint32_t scaleNum = (hExpandXAlign32Size / sizeof(expandXDesc->GetDataType())) /
                            static_cast<uint32_t>(UB_ALIGN / sizeof(float));
                                    ...
                                    const char *nodeName, uint32_t localMoeExpertNum)
{
    ...
    OP_TILING_CHECK(mc2tiling::GetEpWinSize(context, nodeName, hcclBufferSizeEp, maxWindowSizeEp,
                                            Idx::attr::ATTR_GROUP_EP_INDEX) != ge::GRAPH_SUCCESS,
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE(
            ...
            actualSize / MB_SIZE + 1UL, hcclBufferSizeEp / MB_SIZE),
        return ge::GRAPH_FAILED);
    ...
    if (tpWorldSize == TP_WORLD_SIZE_TWO) {
        ...
        OP_TILING_CHECK(mc2tiling::GetCclBufferSize(groupTpHccl, &maxWindowSizeTp, nodeName) != ge::GRAPH_SUCCESS,
                        ...
                        return ge::GRAPH_FAILED);
        actualSize = static_cast<uint64_t>(tilingData->moeDistributeCombineV2Info.a) *
                     (tokenNeedSizeDispatch + tokenNeedSizeCombine) * DOUBLE_DATA_BUFFER;
        OP_TILING_CHECK(
            (actualSize > maxWindowSizeTp),
            OP_LOGE(nodeName,
                    "TP HCCL_BUFFSIZE is too SMALL, A = %u, tokenNeedSizeDispatch = %lu, tokenNeedSizeCombine = %lu,"
                    "NEEDED_HCCL_BUFFSIZE(A * (tokenNeedSizeDispatch + tokenNeedSizeCombine) * 2) = %luMB, TP "
                    "HCCL_BUFFSIZE= %luMB.",
                    tilingData->moeDistributeCombineV2Info.a, tokenNeedSizeDispatch, tokenNeedSizeCombine,
                    actualSize / MB_SIZE + 1UL, maxWindowSizeTp / MB_SIZE),
            return ge::GRAPH_FAILED);
...
static ge::graphStatus MoeDistributeCombineA3TilingFuncImpl(gert::TilingContext *context)
{
    ...
    uint32_t tpWorldSize = tilingData->moeDistributeCombineV2Info.tpWorldSize;
    OP_TILING_CHECK(TilingCheckMoeDistributeCombine<Idx>(context, nodeName, isActiveMask, hasElasticInfo, isPerformance,
                                                         tpWorldSize) != ge::GRAPH_SUCCESS,
                                                                   ...
                                                                   int32_t &commQuantMode, const bool isLayered)
{
    ...
    auto groupEpPtr = attrs->GetAttrPointer<char>(static_cast<int>(Idx::attr::ATTR_GROUP_EP_INDEX));
    auto epWorldSizePtr = attrs->GetAttrPointer<int>(Idx::attr::ATTR_EP_WORLD_SIZE_INDEX);
    auto epRankIdPtr = attrs->GetAttrPointer<int>(Idx::attr::ATTR_EP_RANK_ID_INDEX);
    auto moeExpertNumPtr = attrs->GetAttrPointer<int>(Idx::attr::ATTR_MOE_EXPERT_NUM_INDEX);
    auto tpWorldSizePtr = attrs->GetAttrPointer<int>(Idx::attr::ATTR_TP_WORLD_SIZE_INDEX);
    auto tpRankIdPtr = attrs->GetAttrPointer<int>(Idx::attr::ATTR_TP_RANK_ID_INDEX);
    auto expertSharedTypePtr = attrs->GetAttrPointer<int>(Idx::attr::ATTR_EXPERT_SHARD_TYPE_INDEX);
    auto sharedExpertRankNumPtr = attrs->GetAttrPointer<int>(Idx::attr::ATTR_SHARED_EXPERT_RANK_NUM_INDEX);
    auto globalBsPtr = attrs->GetAttrPointer<int>(Idx::attr::ATTR_GLOBAL_BS_INDEX);
    auto commQuantModePtr = attrs->GetAttrPointer<int>(Idx::attr::ATTR_COMM_QUANT_MODE_INDEX);
    ...
    const gert::StorageShape *expertIdStorageShape = context->GetInputShape(Idx::input::EXPERT_IDS_INDEX);
                                                                    ...
                                                                    MoeDistributeCombineA2Info &info, bool isLayered)
{
    ...
    OP_TILING_CHECK(elasticInfoStorageShape != nullptr,
                    OP_LOGE(K_INNER_DEBUG, "current does not support elasticInfo as Idx::input"), return GRAPH_FAILED);
    ...
    if (constExpertVStorageShape != nullptr) {
        ...
        OP_TILING_CHECK(constExpertVDim1 != static_cast<int64_t>(h),
                        OP_LOGE(K_INNER_DEBUG,
                                "const_expert_v's dim1 not equal to h, const_expert_v's dim1 = %ld, h = %ld",
    ...
    OP_TILING_CHECK(performanceInfoStorageShape != nullptr &&
                    ...
                    OP_LOGE(K_INNER_DEBUG, "When performanceInfo is not null, it needs to be one-dimensional."),
                    return GRAPH_FAILED);
...
static ge::graphStatus MoeDistributeCombineCheckCommAlg(const gert::TilingContext *context, bool &isLayered)
{
    ...
    auto attrs = context->GetAttrs();
    OP_TILING_CHECK(attrs == nullptr, OP_LOGE(K_INNER_DEBUG, "attrs is null."), return ge::GRAPH_FAILED);
    auto commAlg = attrs->GetAttrPointer<char>(static_cast<int>(Idx::attr::ATTR_COMM_ALG_INDEX));
    auto epWorldSizePtr = attrs->GetAttrPointer<int>(Idx::attr::ATTR_EP_WORLD_SIZE_INDEX);
...
static ge::graphStatus MoeDistributeCombineA2TilingFuncImpl(gert::TilingContext *context)
{
    ...
    // 3. communication
    auto attrs = context->GetAttrs();
    auto group = attrs->GetAttrPointer<char>(static_cast<int>(Idx::attr::ATTR_GROUP_EP_INDEX));
    auto epWorldSizePtr = attrs->GetAttrPointer<int>(Idx::attr::ATTR_EP_WORLD_SIZE_INDEX);
...
static ge::graphStatus MoeDistributeCombineA5TilingFuncImpl(gert::TilingContext *context)
{
    auto attrs = context->GetAttrs();
    const char *nodeName = context->GetNodeName();
    auto commAlgPtr = attrs->GetAttrPointer<char>(static_cast<int>(Idx::attr::ATTR_COMM_ALG_INDEX));
    // 检查 commAlg 参数合法性校验
    ...
    OP_TILING_CHECK(!(isNullOrEmpty || isMte || isCcu),
                    OP_LOGE(nodeName,
                            "Invalid parameter: 'commAlg'='%s'. Only 'mte' and 'ccu' are supported."
                            "Nullptr and empty char* are also allowed but will be interpreted as 'mte'.",
                            commAlgPtr),
                    return ge::GRAPH_FAILED);
    if (isCcu) {
        // CCU 调用 A5 tiling 实现
        return MoeDistributeCombineTilingImpl(context, Idx::OP_VERSION_2);
    }
    // 默认空指针和空字符走 MTE 方式
    if (isNullOrEmpty) {
