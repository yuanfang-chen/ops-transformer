...
namespace {
    ...
    constexpr int64_t MAX_EP_WORLD_SIZE_LAYERED = 128;
    ...
    constexpr int64_t MAX_LOCAL_EXPERTNUM_LAYERED = 24;
    ...
    uint32_t &commQuantMode, const CombineV2Config& config, bool &isLayered)
{
    ...
    if (isLayered) {
        OP_TILING_CHECK((*sharedExpertNumPtr != 0) || (sharedExpertRankNum != 0),
...
static bool CheckInputTensorDim(const gert::TilingContext *context, const char *nodeName, const bool isLayered)
{
    ...
    if (isLayered) {
        const gert::StorageShape *expandScaleStorageShape = context->GetInputShape(EXPAND_SCALES_INDEX);
        OP_TILING_CHECK(expandScaleStorageShape == nullptr, OP_LOGE(nodeName, "expandScales is null."), return false);
        OP_TILING_CHECK(expandScaleStorageShape->GetStorageShape().GetDimNum() != ONE_DIM,
            OP_LOGE(nodeName, "expandScales must be 1-dimension, but got %lu dim",
            expandScaleStorageShape->GetStorageShape().GetDimNum()), return false);
        OP_LOGD(nodeName, "expandScales dim0 = %ld", expandScaleStorageShape->GetStorageShape().GetDim(0));
        OP_LOGD(nodeName, "expandScales dim1 = %ld", expandScaleStorageShape->GetStorageShape().GetDim(1));
    }
...
{
    OP_TILING_CHECK(!CheckInputTensorDim(context, nodeName, isLayered),
    ...
    const bool isPerformance, const CombineV2Config& config, bool isLayered)
{
    ...
    if (isLayered) {
        OP_TILING_CHECK((expandXDim1 % H_BASIC_BLOCK_LAYERED), OP_LOGE(nodeName,
    ...
    OP_TILING_CHECK((expandXDim1 < hMin) || (expandXDim1 > hMax),
        ...
        hMin, hMax, expandXDim1), return false); // 32对齐
    ...
    if (isLayered) {
        ...
        OP_TILING_CHECK(expandScalesDim0 < minexpandScalesDim0, OP_LOGE(nodeName, "expandScales's dim0 <"
            " minexpandScalesDim0, expandScales's dim0 is %ld, minexpandScalesDim0 is %ld.", expandScalesDim0,
    ...
    const char *nodeName, uint32_t &localMoeExpertNum, bool isActiveMask, const CombineV2Config& config, bool isLayered)
{
    ...
    if (isLayered) {
        OP_TILING_CHECK((localMoeExpertNum > MAX_LOCAL_EXPERTNUM_LAYERED), OP_LOGE(nodeName,
            "Cannot support localMoeExpertNum > %d, but get localMoeExpertNum = %u > 1", MAX_LOCAL_EXPERTNUM_LAYERED,
    ...
    OP_TILING_CHECK((expertIdsDim0 <= 0) || (expertIdsDim0 > bsUpperBound),
        ...
        expertIdsDim0, bsUpperBound), return false);
...
static uint64_t CalTilingKey(const uint32_t tpWorldSize, uint32_t commQuantMode, bool isLayered)
{
    ...
    uint32_t hierarchy = TILINGKEY_NO_HIERARCHY;
    ...
    if (isLayered) {
        hierarchy = TILINGKEY_ENABLE_HIERARCHY;
    }
    uint64_t tilingKey = GET_TPL_TILING_KEY(tp, quantMode, layeredMode, hierarchy, TILINGKEY_TPL_A3);
    return tilingKey;
}
...
static ge::graphStatus CheckWinSize(const gert::TilingContext *context, MoeDistributeCombineV2TilingData* tilingData,
    const char *nodeName, uint32_t localMoeExpertNum, bool isLayered)
{
    auto attrs = context->GetAttrs();
    ...
    uint64_t moeExpertNum = static_cast<uint64_t>(tilingData->moeDistributeCombineV2Info.moeExpertNum);
    ...
    uint64_t actualSize = isLayered ? (moeExpertNum * maxBs * (h * MAX_OUT_DTYPE_SIZE + (3 * (k + 7) / 8 * 8) *
 	         sizeof(uint32_t) + 64) + 104 * MB_SIZE) : ((maxBs * tokenNeedSizeDispatch * epWorldSize * static_cast<uint64_t>(localMoeExpertNum))
    ...
    OP_TILING_CHECK((actualSize > maxWindowSizeEp),
        OP_LOGE(nodeName, "HCCL_BUFFSIZE_EP is too SMALL, maxBs = %lu, h = %lu, epWorldSize = %lu,"
            ...
            " (maxBs * tokenNeedSizeCombine * (k + sharedExpertNum))) * 2) or NEEDED_HCCL_BUFFSIZE_HIERARCHY((moeExpertNum * maxBs * (h * MAX_OUT_DTYPE_SIZE + (3 * (k + 7) / 8 * 8) *"
 	             "sizeof(uint32_t) + 64) + 104 * MB_SIZE))"
 	             "= %luMB,"
...
static ge::graphStatus MoeDistributeCombineV2TilingFuncNew(gert::TilingContext* context, const CombineV2Config& config)
{
