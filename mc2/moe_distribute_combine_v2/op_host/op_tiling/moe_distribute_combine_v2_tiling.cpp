...
static void UbUsedCal(const uint64_t ubSize, const gert::TilingContext* context, MoeDistributeCombineV2TilingData *tilingData, const CombineV2Config& config)
{
    ...
    auto expandXDesc = context->GetInputDesc(config.expandXIndex), attrs = context->GetAttrs();
...
void MoeDistributeCombineV2ConfigIndexSet(const CombineV2Config& config)
{
...
void MoeDistributeCombineARNConfigIndexSet(const CombineV2Config& config)
{
