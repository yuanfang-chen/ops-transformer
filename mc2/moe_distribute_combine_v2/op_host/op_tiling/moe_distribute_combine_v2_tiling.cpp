...
namespace {
    ...
    constexpr uint32_t RANK_NUM_PER_NODE_A3 = 16;
    // A2
    ...
    const CombineV2Config& config)
{
    ...
    if (isLayered) {
        minAssistInfoDim0 = std::max(minAssistInfoDim0, static_cast<int64_t>(globalBs * 2 * expertIdsDim1 * ((epWorldSize) / RANK_NUM_PER_NODE_A3)));
    }
    OP_TILING_CHECK(assistInfoDim0 < minAssistInfoDim0, OP_LOGE(nodeName, "assistInfoDim0 < minAssistInfoDim0,"
        " assistInfoDim0 is %ld, minAssistInfoDim0 is %ld.", assistInfoDim0, minAssistInfoDim0), return false);
