...
using namespace MoeDistributeCombineV2LayeredImpl;
...
template<bool HasTp, uint8_t QuantMode, uint8_t LayeredMode, uint8_t Hierarchy, uint8_t ArchTag>
...

{
...
#endif
    if constexpr ((ArchTag == TILINGKEY_TPL_A3) && (Hierarchy == TILINGKEY_NO_HIERARCHY)) {
        GET_TILING_DATA_WITH_STRUCT(MoeDistributeCombineV2TilingData, tilingData, tilingGM);
    ...
    } else if constexpr ((ArchTag == TILINGKEY_TPL_A3) && (Hierarchy == TILINGKEY_ENABLE_HIERARCHY)) {
