...
private:
    ...
    {
        if (isMc2Context_) {
            return (GM_ADDR)mc2Context_->epHcclBuffer_[toRankId] + WIN_ADDR_OFFSET + winDataSizeOffsetEp_;
        }
        if (domain == EP_DOMAIN) {
    ...
    {
        if (isMc2Context_) {
            return (GM_ADDR)mc2Context_->epHcclBuffer_[toRankId] + winStatusOffset_;
        }
        if (domain == EP_DOMAIN) {
    ...
    const MoeDistributeCombineV2TilingData *tilingData)
{
    ...
    if (isMc2Context_) {
        ...
        mc2Context_ = (Mc2MoeContext*)mc2Context;
        epRankIdHccl = winContext_->epRankId;
