...
__aicore__ inline void MoeDistributeCombineV2<CombineMC2TypeFunc>::InitAttrs(const MoeDistributeCombineV2TilingData *tilingData)
{
    ...
    if (isMc2Context_) {
        statusDataSpaceGm_ = mc2ContextGMTensor_.GetValue(EP_STATUS_DATA_SPACE_OFFSET);
    ...
    GM_ADDR XOut, GM_ADDR workspaceGM, TPipe *pipe, const MoeDistributeCombineV2TilingData *tilingData)
{
    ...
    if (mc2Context != nullptr) {
        isMc2Context_ = true;
        mc2ContextGMTensor_.SetGlobalBuffer((__gm__ int64_t*)mc2Context);
    ...
    if constexpr (IsNeedReduceScatter) {
        if (isMc2Context_) {
            CheckWindowSize(totalWinSizeTp_, mc2ContextGMTensor_.GetValue(TP_WIN_SIZE_OFFSET), tpipe_, expandXOut);
        } else {
