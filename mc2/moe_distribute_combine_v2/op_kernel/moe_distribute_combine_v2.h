...
private:
    ...
    __aicore__ inline float WaitDispatch(uint32_t tokenIndex, uint64_t performanceTimeStart, uint32_t copyCount);
...
template <TemplateMC2TypeClass>
__aicore__ inline float MoeDistributeCombineV2<TemplateMC2TypeFunc>::WaitDispatch(uint32_t tokenIndex,
...
__aicore__ inline void MoeDistributeCombineV2<TemplateMC2TypeFunc>::LocalWindowCopy()
{
    ...
    float target = static_cast<float>(0.0);
    float minTarget = static_cast<float>(0.0);
    float maxTarget = static_cast<float>(0.0);
    float tempCount = static_cast<float>(0.0);
    ...
    while (tokenNumCompleted != tokenPerAivNum) {
        ...
        for (uint32_t curIdx = beginIndex; curIdx < endIndex; curIdx++) {
            ...
            target = (float)1.0 * targetCount;
            minTarget = target - (float)0.5;
            maxTarget = target + (float)0.5;
            tempCount = WaitDispatch(tokenIndex, performanceTimeStart, copyCount);
            if ((tempCount < minTarget) || (tempCount > maxTarget)) {
                continue;
            }
            tokenNumCompleted++;
