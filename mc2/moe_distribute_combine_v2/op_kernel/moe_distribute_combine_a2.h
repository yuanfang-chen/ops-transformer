    ...
    GM_ADDR XOut, GM_ADDR workspaceGM, TPipe *pipe, const MoeDistributeCombineA2TilingData *tilingData)
{
    ...
    if (unlikely(needPerformanceInfo_)) {
        ...
        Duplicate<int32_t>(performanceInfoI32Tensor_, 0,
                           RoundUp(performanceInfoSize_, B64_PER_BLOCK) * sizeof(int64_t) / sizeof(int32_t));
    }
...
__aicore__ inline void MoeDistributeCombineA2<TemplateMC2TypeA2Func>::allocTensor()
{
    ...
    flagLocal_ = LocalTensor<uint32_t>{TPosition::LCM, flagLocalAddr, UB_ALIGN / sizeof(uint32_t)};
    uint32_t sendCountLocalAddr = flagLocalAddr + UB_ALIGN;
    sendCountLocalEleNum = RoundUp(moeExpertNum_, UB_ALIGN / sizeof(uint32_t));
...
__aicore__ inline void MoeDistributeCombineA2<TemplateMC2TypeA2Func>::TokenActiveMaskCal()
{
    ...
    auto xActiveMaskLocal = LocalTensor<int8_t>{TPosition::LCM, 0, xActiveMaskAlignSize};
    ...
    Cast(xActiveMaskHalfLocal, xActiveMaskLocal, RoundMode::CAST_NONE, axisBS_);
    PipeBarrier<PIPE_V>();
...
MoeDistributeCombineA2<TemplateMC2TypeA2Func>::ConstructBatchWriteInfo(LocalTensor<ExpandIdxType> &sendCountInfo)
{
    ...
    DataCopy(workspaceGlobal_[worldTaskInfo_.startTaskId * U64_PER_ITEM], batchWriteItemLocalB64,
...
__aicore__ inline void MoeDistributeCombineA2<TemplateMC2TypeA2Func>::Preload()
{
    ...
    uint32_t expertTileNum = taskInfo.taskNum / MAX_NUM_EXPERTS_PER_TILE;
    uint32_t numExpertsPerTail = taskInfo.taskNum % MAX_NUM_EXPERTS_PER_TILE;
    ...
    auto expertCopyParams = DataCopyExtParams{1, MAX_NUM_EXPERTS_PER_TILE * (uint32_t)sizeof(uint32_t), 0, 0, 0};
    auto maskCopyParams = DataCopyExtParams{1, MAX_NUM_EXPERTS_PER_TILE, 0, 0, 0};
    for (uint32_t i = 0u; i < expertTileNum; i++) {
        ...
        SyncFunc<AscendC::HardEvent::MTE2_S>();
        if (isInputExpertMaskFlag_) {
            Histograms<true>(recvCountLocal_, expertIdsLocal_, expertMaskLocal_, (int32_t)moeExpertNum_,
                             MAX_NUM_EXPERTS_PER_TILE);
        } else {
            Histograms<false>(recvCountLocal_, expertIdsLocal_, expertMaskLocal_, (int32_t)moeExpertNum_,
                              MAX_NUM_EXPERTS_PER_TILE);
        }
        SyncFunc<AscendC::HardEvent::S_MTE2>();
    }
    if (numExpertsPerTail > 0) {
        auto tailExpertCopyParams = DataCopyExtParams{1, numExpertsPerTail * (uint32_t)sizeof(uint32_t), 0, 0, 0};
        auto tailMaskCopyParams = DataCopyExtParams{1, numExpertsPerTail, 0, 0, 0};
        uint64_t EleOffset = taskInfo_.startTaskId + expertTileNum * MAX_NUM_EXPERTS_PER_TILE;
        DataCopyPad(expertIdsLocal_, expertIdsGlobal_[EleOffset], tailExpertCopyParams, expertNoPadParams);
        if (isInputExpertMaskFlag_) {
            DataCopyPad(expertMaskLocal_, xActiveMaskGlobal_[EleOffset], tailMaskCopyParams, maskNoPadParams);
        }
        SyncFunc<AscendC::HardEvent::MTE2_S>();
        if (isInputExpertMaskFlag_) {
            Histograms<true>(recvCountLocal_, expertIdsLocal_, expertMaskLocal_, (int32_t)moeExpertNum_,
    ...
    DataCopyPad(numRecvTokensPerRankGlobal_[worldTaskInfo_.startTaskId], numRecvTokensPerRankLocal_,
                {1, static_cast<uint32_t>(worldTaskInfo_.taskNum * sizeof(uint32_t)), 0, 0, 0});
    SyncAll<true>();
}
...
{
    if (worldTaskInfo_.startTaskId >= worldSize_) {
        SyncAll<true>();
        return;
    }
    SyncFunc<AscendC::HardEvent::MTE2_S>();
...
__aicore__ inline void MoeDistributeCombineA2<TemplateMC2TypeA2Func>::LocalWindowCopy()
{
    ...
    auto expertWindowOffsetLocal_ = LocalTensor<uint32_t>{};
    auto topkWeightsLocal_ = LocalTensor<uint32_t>{};
    auto expandIdxLocal_ = LocalTensor<uint32_t>{};
    auto expertIdsLocal_ = LocalTensor<int32_t>{};
    auto expertMaskLocal_ = LocalTensor<bool>{};


    DataCopyPad(expertWindowOffsetLocal_, expertWindowOffsetGlobal_,
                {1, static_cast<uint32_t>(moeExpertNum_ * sizeof(uint32_t)), 0, 0, 0}, {false, 0, 0, 0});
    DataCopyPad(topkWeightsLocal_, topkWeightsGlobal_[tokenTaskInfo_.startTaskId * axisK_],
                {1, tokenTaskInfo_.taskNum * axisK_ * sizeof(uint32_t), 0, 0, 0}, {false, 0, 0, 0});
    DataCopyPad(expandIdxLocal_, expandIdxGlobal_[tokenTaskInfo_.startTaskId * axisK_],
                {1, tokenTaskInfo_.taskNum * axisK_ * sizeof(ExpandIdxType), 0, 0, 0}, {false, 0, 0, 0});
    DataCopyPad(expertIdsLocal_, expertIdsGlobal_[tokenTaskInfo_.startTaskId * axisK_],
                {1, tokenTaskInfo_.taskNum * axisK_ * sizeof(uint32_t), 0, 0, 0}, {false, 0, 0, 0});


    expandScalesSegBaseOffset_ = tokenTaskInfo_.startTaskId * axisK_;
    indexCountsSegBaseOffset_ = tokenTaskInfo_.startTaskId * axisK_;
    expertIdsSegBaseOffset_ = tokenTaskInfo_.startTaskId * axisK_;

    for (uint32_t i = 0; i < tokenTaskInfo_.taskNum; i++) {
        ...
        for (uint32_t topKIdx = 0; topKIdx < axisK_; topKIdx++) {
            ...
            if (isInputExpertMaskFlag_) {
                bool maskExpertFlag = expertMaskLocal_(tokentopKIdx);
                if (!maskExpertFlag) {
                    continue;
                }
            }
            uint32_t expId = expertIdsLocal_(tokentopKIdx - expertIdsSegBaseOffset_);
...
template <TemplateMC2TypeA2Class>
__aicore__ inline void MoeDistributeCombineA2<TemplateMC2TypeA2Func>::ProcessMoeAndCopyExpert(uint32_t tokenIdx,
                                                                                              uint32_t topKIdx)
{
    GM_ADDR wAddr;
    uint32_t tokentopKIdx = tokenIdx * axisK_ + topKIdx;
    float topkWeight = topkWeightsLocal_(tokentopKIdx - expandScalesSegBaseOffset_);
    uint32_t expId = expertIdsLocal_(tokentopKIdx - expertIdsSegBaseOffset_);
    if (expId < moeExpertNum_) {
        uint32_t rank = expId / localMoeExpertNum_;
        wAddr = (__gm__ uint8_t *)(windowInGM_) + rankSizeOnWin_ * rank +
                expertWindowOffsetLocal_(expId) * axisHExpandXTypeSize_ +
                expandIdxLocal_(tokentopKIdx - indexCountsSegBaseOffset_) * axisHExpandXTypeSize_;
    } else {
        wAddr = (__gm__ uint8_t *)(oriXGM_) + tokenIdx * axisHExpandXTypeSize_;
    }
    rankWindow_.SetGlobalBuffer((__gm__ ExpandXType *)wAddr);
    SyncFunc<AscendC::HardEvent::V_MTE2>();
    DataCopy(xLocal[eventId], rankWindow_, axisH_);
    SyncFunc<AscendC::HardEvent::MTE2_V>();
    Cast(tokenFloatLocal_, xLocal[eventId], AscendC::RoundMode::CAST_NONE, axisH_);
    PipeBarrier<PIPE_V>();
