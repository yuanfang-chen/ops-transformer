...
__aicore__ inline void MoeDistributeCombineA2<TemplateMC2TypeA2Func>::LocalWindowCopy()
{
    ...
    auto expandIdxLocal_ = LocalTensor<int32_t>{TPosition::LCM, expandIdxLocalAddr, maxTotalRoutedTokensPerTile};
    uint32_t expertIdsLocalAddr = expandIdxLocalAddr + maxTotalRoutedTokensPerTile * sizeof(int32_t);
    auto expertIdsLocal_ = LocalTensor<int32_t>{TPosition::LCM, expertIdsLocalAddr, maxTotalRoutedTokensPerTile};
    uint32_t expertMaskLocalAddr = expertIdsLocalAddr + maxTotalRoutedTokensPerTile * sizeof(int32_t);
    auto expertMaskLocal_ = LocalTensor<bool>{TPosition::LCM, expertMaskLocalAddr, maxTotalRoutedTokensPerTile};
