...
__aicore__ inline void MoeDistributeCombineA2<TemplateMC2TypeA2Func>::CalRecvCountsAndOffsets()
{
    ...
    CopyGm2Ub(expertIdsLocal_, expertIdsGlobal_[taskInfo.startTaskId], taskInfo.taskNum);
    ...
    SyncFunc<AscendC::HardEvent::S_MTE3>();
    CopyUb2Gm(expertWindowOffsetGlobal_[worldTaskInfo_.startTaskId * localMoeExpertNum_], expertWindowOffsetLocal_,
              worldTaskInfo_.taskNum * localMoeExpertNum_);
    SyncAll<true>();
}
...
__aicore__ inline void MoeDistributeCombineA2<TemplateMC2TypeA2Func>::WaitDispatch()
{
    ...
    if (unlikely(needPerformanceInfo_)) {
        ...
        CopyUb2Gm(performanceInfoI32GMTensor_, performanceInfoI32Tensor_,
...
__aicore__ inline void MoeDistributeCombineA2<TemplateMC2TypeA2Func>::allocTensorForWindowCopy()
{
    ...
    uint32_t maxTotalRoutedTokensPerTile = 512;
    uint32_t maxTokenNumPerTile = maxTotalRoutedTokensPerTile / axisK_;
    topkWeightsLocal_ = LocalTensor<float>{TPosition::LCM, topkWeightsLocalAddr, maxTotalRoutedTokensPerTile};
    uint32_t expandIdxLocalAddr = topkWeightsLocalAddr + maxTotalRoutedTokensPerTile * sizeof(float);
    expandIdxLocal_ = LocalTensor<int32_t>{TPosition::LCM, expandIdxLocalAddr, maxTotalRoutedTokensPerTile};
    uint32_t expertIdsLocalAddr = expandIdxLocalAddr + maxTotalRoutedTokensPerTile * sizeof(int32_t);
    expertIdsLocal_ = LocalTensor<int32_t>{TPosition::LCM, expertIdsLocalAddr, maxTotalRoutedTokensPerTile};
    uint32_t expertMaskLocalAddr = expertIdsLocalAddr + maxTotalRoutedTokensPerTile * sizeof(int32_t);
    expertMaskLocal_ = LocalTensor<bool>{TPosition::LCM, expertMaskLocalAddr, maxTotalRoutedTokensPerTile};
}
