...
namespace MoeDistributeCombineV2LayeredImpl {
...
private:
    ...
    uint32_t offsetIndexs_[65];
    uint32_t copyNums_[65];
    uint32_t dataOffsets_[65];
};
...
__aicore__ inline void MoeDistributeCombineV2Layered<TemplateMC2TypeV2LayeredFunc>::SumToWindow()
{
    ...
    SyncFunc<AscendC::HardEvent::MTE2_S>();
    uint64_t rdmaAddr = (uint64_t)(GetWindOutAddrByRankId(rankId_) + halfWinSize_ * bufferId_ +
                                   serverId * rankSizeOnWin_ * SERVER_RANK_SIZE);
    ...
    for (uint32_t i = 0U; i < maxLocalBs_; i++) {
        if ((i % coreNumPerServer) == (coreIdx_ % coreNumPerServer)){
            ...
            uint32_t dataOffset = i * (axisH_ / DOUBLE_TYPE + scaleNumAlign_);
            if (i != 0U) {
    ...
    const uint32_t copyLen, const uint32_t copyLenAlign, const uint64_t srcrdmaAddr, const uint64_t dstrdmaAddr)
{
    ...
    while (!stopFlag) {
        while (true) {
            DataCopy(checkRdmaLocal_[CACHELINE_SIZE], shareFlagGlobal_[(checkServer_ + 1) * SUM_WINDOW_FLAG + copySum * B64_PER_BLOCK], B64_PER_BLOCK);
            PipeBarrier<PIPE_ALL>();
...
__aicore__ inline void MoeDistributeCombineV2Layered<TemplateMC2TypeV2LayeredFunc>::AlltoAllServerDispatch()
{
    ...
    if (rankId_ != tragRankId) {
        AIVRDMAPostSend((GM_ADDR)((uint64_t)(readStateGlobal_.GetPhyAddr())),
                        ...
                        (GM_ADDR)((uint64_t)(GetWindInAddrByRankId(tragRankId) +
