/**
 * Copyright (c) 2025 Huawei Technologies Co., Ltd.
 * This program is free software, you can redistribute it and/or modify it under the terms and conditions of
 * CANN Open Software License Agreement Version 2.0 (the "License").
 * Please refer to the License for details. You may not use this file except in compliance with the License.
 * THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * INCLUDING BUT NOT LIMITED TO NON-INFRINGEMENT, MERCHANTABILITY, OR FITNESS FOR A PARTICULAR PURPOSE.
 * See LICENSE in the root of the software repository for the full text of the License.
 */

/* !
 * \file moe_distribute_combine_v2_tiling_key.h
 * \brief
 */

#ifndef MOE_DISTRIBUTE_COMBINE_V2_TILING_KEY_H
#define MOE_DISTRIBUTE_COMBINE_V2_TILING_KEY_H
#include "ascendc/host_api/tiling/template_argument.h"

namespace Mc2Tiling {
#define TILINGKEY_TPL_A2 0
#define TILINGKEY_TPL_A3 1
#define TILINGKEY_TPL_A5 2
#define TILINGKEY_NO_QUANT 0
#define TILINGKEY_INT8_QUANT 1
#define TILINGKEY_TPL_MTE 0
#define TILINGKEY_TPL_AICPU 1
#define TILINGKEY_NO_HIERARCHY 0
#define TILINGKEY_ENABLE_HIERARCHY 1

ASCENDC_TPL_ARGS_DECL(MoeDistributeCombineV2,
    ASCENDC_TPL_BOOL_DECL(TILINGKEY_TP, 0, 1),  // A3
    ASCENDC_TPL_UINT_DECL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_2_BW, ASCENDC_TPL_UI_LIST, 
                        TILINGKEY_NO_QUANT, TILINGKEY_INT8_QUANT),  // A2 & A3
    ASCENDC_TPL_UINT_DECL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_2_BW, ASCENDC_TPL_UI_LIST,
                        TILINGKEY_TPL_MTE, TILINGKEY_TPL_AICPU),  // A2
    ASCENDC_TPL_UINT_DECL(TILINGKEY_HIERARCHY, ASCENDC_TPL_2_BW, ASCENDC_TPL_UI_LIST,
                        TILINGKEY_NO_HIERARCHY, TILINGKEY_ENABLE_HIERARCHY),  // A3
    ASCENDC_TPL_UINT_DECL(ARCH_TAG, ASCENDC_TPL_2_BW, ASCENDC_TPL_UI_LIST, 
                        TILINGKEY_TPL_A2, TILINGKEY_TPL_A3, TILINGKEY_TPL_A5),
);
ASCENDC_TPL_SEL( 
#ifdef __DAV_C310__
    // A5 - kernel侧
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_TP, 0),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_QUANT),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_MTE),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_HIERARCHY, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_HIERARCHY),
        ASCENDC_TPL_UINT_SEL(ARCH_TAG, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_A5),
        ASCENDC_TPL_TILING_STRUCT_SEL(MoeDistributeCombineV2TilingData)
    ),
    // A3 - kernel侧
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_TP, 0, 1),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_UI_LIST, 
                            TILINGKEY_NO_QUANT, TILINGKEY_INT8_QUANT),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_MTE),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_HIERARCHY, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_HIERARCHY),
        ASCENDC_TPL_UINT_SEL(ARCH_TAG, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_A3),
        ASCENDC_TPL_TILING_STRUCT_SEL(MoeDistributeCombineV2TilingData)
    ),
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_TP, 0),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_UI_LIST, 
                            TILINGKEY_NO_QUANT, TILINGKEY_INT8_QUANT),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_MTE),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_HIERARCHY, ASCENDC_TPL_UI_LIST, TILINGKEY_ENABLE_HIERARCHY),
        ASCENDC_TPL_UINT_SEL(ARCH_TAG, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_A3),
        ASCENDC_TPL_TILING_STRUCT_SEL(MoeDistributeCombineV2TilingData)
    ),
#elif defined(__CCE_AICORE__) && (__CCE_AICORE__ == 220)
    // A2 - kernel侧
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_TP, 0),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_QUANT),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_UI_LIST, 
                            TILINGKEY_TPL_MTE, TILINGKEY_TPL_AICPU),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_HIERARCHY, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_HIERARCHY),
        ASCENDC_TPL_UINT_SEL(ARCH_TAG, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_A2),
        ASCENDC_TPL_TILING_STRUCT_SEL(MoeDistributeCombineA2TilingData)
    ),
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_TP, 0),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_INT8_QUANT),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_AICPU),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_HIERARCHY, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_HIERARCHY),
        ASCENDC_TPL_UINT_SEL(ARCH_TAG, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_A2),
        ASCENDC_TPL_TILING_STRUCT_SEL(MoeDistributeCombineA2TilingData)
    ),
    // A3 - kernel侧
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_TP, 0, 1),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_UI_LIST, 
                            TILINGKEY_NO_QUANT, TILINGKEY_INT8_QUANT),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_MTE),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_HIERARCHY, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_HIERARCHY),
        ASCENDC_TPL_UINT_SEL(ARCH_TAG, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_A3),
        ASCENDC_TPL_TILING_STRUCT_SEL(MoeDistributeCombineV2TilingData)
    ),
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_TP, 0),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_UI_LIST, 
                            TILINGKEY_NO_QUANT, TILINGKEY_INT8_QUANT),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_MTE),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_HIERARCHY, ASCENDC_TPL_UI_LIST, TILINGKEY_ENABLE_HIERARCHY),
        ASCENDC_TPL_UINT_SEL(ARCH_TAG, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_A3),
        ASCENDC_TPL_TILING_STRUCT_SEL(MoeDistributeCombineV2TilingData)
    ),
#else
    // A5 - host侧
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_TP, 0),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_QUANT),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_MTE),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_HIERARCHY, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_HIERARCHY),
        ASCENDC_TPL_UINT_SEL(ARCH_TAG, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_A5),
        ASCENDC_TPL_TILING_STRUCT_SEL(MoeDistributeCombineV2TilingData)
    ),
    // A2 - host侧
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_TP, 0),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_QUANT),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_UI_LIST, 
                            TILINGKEY_TPL_MTE, TILINGKEY_TPL_AICPU),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_HIERARCHY, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_HIERARCHY),
        ASCENDC_TPL_UINT_SEL(ARCH_TAG, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_A2),
        ASCENDC_TPL_TILING_STRUCT_SEL(MoeDistributeCombineA2TilingData)
    ),
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_TP, 0),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_INT8_QUANT),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_AICPU),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_HIERARCHY, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_HIERARCHY),
        ASCENDC_TPL_UINT_SEL(ARCH_TAG, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_A2),
        ASCENDC_TPL_TILING_STRUCT_SEL(MoeDistributeCombineA2TilingData)
    ),
    // A3 - host侧
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_TP, 0, 1),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_UI_LIST, 
                            TILINGKEY_NO_QUANT, TILINGKEY_INT8_QUANT),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_MTE),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_HIERARCHY, ASCENDC_TPL_UI_LIST, TILINGKEY_NO_HIERARCHY),
        ASCENDC_TPL_UINT_SEL(ARCH_TAG, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_A3),
        ASCENDC_TPL_TILING_STRUCT_SEL(MoeDistributeCombineV2TilingData)
    ),
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_TP, 0),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_QUANT_MODE, ASCENDC_TPL_UI_LIST, 
                            TILINGKEY_NO_QUANT, TILINGKEY_INT8_QUANT),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_LAYERED_MODE, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_MTE),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_HIERARCHY, ASCENDC_TPL_UI_LIST, TILINGKEY_ENABLE_HIERARCHY),
        ASCENDC_TPL_UINT_SEL(ARCH_TAG, ASCENDC_TPL_UI_LIST, TILINGKEY_TPL_A3),
        ASCENDC_TPL_TILING_STRUCT_SEL(MoeDistributeCombineV2TilingData)
    ),
#endif  
);
} // namespace Mc2Tiling
#endif
