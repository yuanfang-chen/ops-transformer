                                ...
                                const std::vector<uint64_t> &expectedScalesDims)
{
    ...
    if (actualDimNum != expectedScalesDims.size()) {
        OP_LOGE(nodeName, 
                "Scales dimension mismatch. Expected %lu dimensions, but got %lu dimensions. "
                "Expected shape: %s, Actual shape: %s",
                expectedScalesDims.size(), actualDimNum,
    ...
    for (size_t i = 0; i < expectedScalesDims.size(); ++i) {
        ...
        if (expectedDim != actualDim) {
            OP_LOGE(nodeName,
                    "Scales dimension %lu mismatch. Expected %lu, but got %lu. "
                    "Expected shape: %s, Actual shape: %s",
                    i, expectedDim, actualDim,
                    FormatShape(expectedScalesDims).c_str(),
                    Ops::Base::ToString(scalesShape->GetStorageShape()));
            return false;
        }
...
static bool CheckInputTensorDim(const gert::TilingContext *context, TilingRunInfo &runInfo, const OpType opType)
{
    ...
    OP_TILING_CHECK(!CheckScalesValid(context, expectedScalesDims),
                    OP_LOGE(nodeName, "scales dimensions and shapes is invalid in the quantmode."), return false); // 校验scales.shape合法性
    return true;
}
