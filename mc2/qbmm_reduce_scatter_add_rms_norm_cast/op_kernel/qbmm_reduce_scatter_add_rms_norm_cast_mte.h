...
protected:
    ...
    uint16_t bf16UbToGmData_{0};
    uint16_t bf16UbToGmStride_{0};
...
__aicore__ inline void QbmmReduceScatterAddRmsNormCastMte<TemplateMC2TypeFunc>::DequantCompute(GlobalTensor<int32_t> &curMmOutGm, uint32_t curAicM, uint32_t curAicN, uint64_t row, uint64_t col)
{
    ...
    for (uint32_t mUbLoopIdx = 0; mUbLoopIdx < mUbLoops; ++mUbLoopIdx) {
        ...
        DataCopyParams gm2UbParams{static_cast<uint16_t>(curAivM), int32GmToUbData_, int32GmToUbStride_, 0};
        DataCopyParams ub2GmParams{static_cast<uint16_t>(curAivM), bf16UbToGmData_, 0, bf16UbToGmStride_};
...
__aicore__ inline void QbmmReduceScatterAddRmsNormCastMte<TemplateMC2TypeFunc>::ReadRemoteDataAdd()
{
    ...
    for(uint32_t tileIdx = 0; tileIdx < rowNum; ++tileIdx) {
        Duplicate<float>(sumFp32Tensor, (float)0.0, n_);
        ...
        for(int tpIndex = 0; tpIndex < tpWorldSize_; ++tpIndex) {
            ...
            SyncFunc<AscendC::HardEvent::MTE2_V>();
            Cast(tokenFp32Tensor, tokenTensor_, RoundMode::CAST_NONE, n_);
            PipeBarrier<PIPE_V>();
            Add(sumFp32Tensor, sumFp32Tensor, tokenFp32Tensor, n_);
...
__aicore__ inline void QbmmReduceScatterAddRmsNormCastMte<TemplateMC2TypeFunc>::Process()
{
    ...
    if ASCEND_IS_AIV {
        ...
        tpipe_->InitBuffer(resFp32Buf_, n_ * sizeof(float));
        tpipe_->InitBuffer(gammaBuf_, n_ * sizeof(float));
        tpipe_->InitBuffer(gammaFp32Buf_, n_ * sizeof(float));
        ReadRemoteDataAdd();
    }
}
