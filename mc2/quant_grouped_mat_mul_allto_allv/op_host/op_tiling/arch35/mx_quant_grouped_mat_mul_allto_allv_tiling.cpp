...
ge::graphStatus MxQuantGroupedMatmulAllToAllvTiling::CheckAndSetLocalParamsGmm()
{
    ...
    OP_LOGD(opName_, "gmmXDtype=%d, gmmWeightDtype=%d", localParams_.gmmXDtype, localParams_.gmmWeightDtype);
    ...
    OP_LOGD(opName_, "yDtype=%d", localParams_.yDtype);
    ...
    OP_LOGD(opName_, "A=%lu, H1=%lu, ep=%lu, BsK=%lu, N1=%lu",
            localParams_.A, localParams_.H1, localParams_.ep, localParams_.BsK, localParams_.N1);
    OP_LOGD(opName_, "gmmWeightDim1=%lu, gmmWeightDim2=%lu",
...
ge::graphStatus MxQuantGroupedMatmulAllToAllvTiling::CheckAndSetLocalParamsMm()
{
    ...
    OP_LOGD(opName_, "mmXDtype=%d, mmWeightDtype=%d", localParams_.mmXDtype, localParams_.mmWeightDtype);
    ...
    OP_TILING_CHECK(localParams_.Bs != mmYDim0,
        OP_LOGE(opName_, "The mmX DIM0 %lu and mmY DIM0 %lu is not equal!", localParams_.Bs, mmYDim0),
        return ge::GRAPH_FAILED);
...
ge::graphStatus MxQuantGroupedMatmulAllToAllvTiling::CheckParamsRelationGmm()
{
    ...
    OP_TILING_CHECK(localParams_.gmmXQuantMode != QUANT_MX,
        OP_LOGE(opName_, "The gmmXQuantMode should be MX mode, but actul mode is %ld !", QUANT_MX, localParams_.gmmXQuantMode),
        return ge::GRAPH_FAILED);
    ...
    if (!localParams_.isGmmWeightTrans) {
        ...
        OP_TILING_CHECK(localParams_.H1 != localParams_.gmmWeightDim2,
            OP_LOGE(opName_, "In the Transposed Scenario, gmmX dim1 is %lu, but gmmWeight dim2 is %lu !", localParams_.H1, localParams_.gmmWeightDim2),
            return ge::GRAPH_FAILED);
        OP_TILING_CHECK(localParams_.N1 != localParams_.gmmWeightDim1,
            OP_LOGE(opName_, "In the Transposed Scenario, y dim1 is %lu, but gmmWeight dim1 is %lu !", localParams_.N1, localParams_.gmmWeightDim1),
            return ge::GRAPH_FAILED);
    }
...
ge::graphStatus MxQuantGroupedMatmulAllToAllvTiling::CheckParamsRelationMm()
{
    ...
    if (!localParams_.isMmWeightTrans) {
        OP_TILING_CHECK(localParams_.H2 != localParams_.mmWeightDim0,
            OP_LOGE(opName_, "In the Non-Transposed Scenario, mmX dim1 is %lu, but mmWeight dim0 is %lu !", localParams_.H2, localParams_.mmWeightDim0),
            return ge::GRAPH_FAILED);
        OP_TILING_CHECK(localParams_.N2 != localParams_.mmWeightDim1,
            OP_LOGE(opName_, "In the Non-Transposed Scenario, mmY dim1 is %lu, but mmWeight dim1 is %lu !", localParams_.N2, localParams_.mmWeightDim1),
            return ge::GRAPH_FAILED);
    } else {
        OP_TILING_CHECK(localParams_.H2 != localParams_.mmWeightDim1,
            OP_LOGE(opName_, "In the Transposed Scenario, mmX dim1 is %lu, but mmWeight dim1 is %lu !", localParams_.H2, localParams_.mmWeightDim1),
            return ge::GRAPH_FAILED);
        OP_TILING_CHECK(localParams_.N2 != localParams_.mmWeightDim0,
            OP_LOGE(opName_, "In the Transposed Scenario, mmY dim1 is  %lu, but mmWeight dim0 is %lu !", localParams_.N2, localParams_.mmWeightDim0),
            return ge::GRAPH_FAILED);
    }
...
ge::graphStatus MxQuantGroupedMatmulAllToAllvTiling::CheckParamsAttrEpAndSetLocalParams()
{
    ...
    OP_TILING_CHECK(epWorldSizePtr == nullptr, OP_LOGE(opName_, "The epWorldSizePtr is nullptr."), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(((groupSizeM != MX_GROUP_SIZE_M) && (groupSizeM != 0)) ||
        ...
        CUBE_INNER_ERR_REPORT(
            opName_, "The groupSizeM, groupSizeN should be 0 or 1, and groupSizeK should be 0 or 32 in mxfp8 scene,"
...
ge::graphStatus MxQuantGroupedMatmulAllToAllvTiling::CheckMxQuantGmmScaleShapes()
{
    ...
    OP_TILING_CHECK((localParams_.A != gmmXScaleDim0) || (gmmxDivH1 != gmmXScaleDim1) || (gmmXScaleDim2 != EVEN_ALIGN),
        OP_LOGE(opName_, "In the Non-Transposed Scenario, Wrong shape of gmmXScale! "
            ...
            "gmmXScaleDim1 should be equal to (gmmXDim1(%lu) + MX_SCALE_GROUP(%lu) - 1) / MX_SCALE_GROUP(%lu), gmmXScaleDim2 should be equal to 2, "
    ...
    if (localParams_.isGmmWeightTrans){ // Transposed Scenario
        OP_TILING_CHECK((gmmWeightScaleDim0 != localParams_.ep) || (gmmWeightScaleDim1 != localParams_.N1) || (gmmWeightScaleDim2 != gmmxDivH1) || (gmmWeightScaleDim3 != EVEN_ALIGN),
            OP_LOGE(opName_, "In the Transposed Scenario, Wrong shape of gmmWeightScale! "
                ...
                "gmmWeightScaleDim2 should be equal to (gmmWeightDim2(%lu) + MX_SCALE_GROUP(%lu) - 1) / MX_SCALE_GROUP(%lu), gmmWeightScaleDim3 should be equal to 2, "
                "Expected Shape of gmmWeightScale = (%lu, %lu, %lu, %lu), Actual Shape of gmmWeightScale = (%lu, %lu, %lu, %lu).",
                localParams_.ep, localParams_.N1, gmmxDivH1, MX_SCALE_GROUP, MX_SCALE_GROUP, localParams_.ep, localParams_.N1, gmmxDivH1, EVEN_ALIGN, 
                gmmWeightScaleDim0, gmmWeightScaleDim1, gmmWeightScaleDim2, gmmWeightScaleDim3), return ge::GRAPH_FAILED);
    } else {
        OP_TILING_CHECK((gmmWeightScaleDim0 != localParams_.ep) || (gmmWeightScaleDim1 != gmmxDivH1) || (gmmWeightScaleDim2 != localParams_.N1) || (gmmWeightScaleDim3 != EVEN_ALIGN),
            OP_LOGE(opName_, "In the Non-Transposed Scenario, Wrong shape of gmmWeightScale! "
                "gmmWeightScaleDim0 should be equal to gmmWeightDim0(%lu), "
                "gmmWeightScaleDim1 should be equal to (gmmWeightDim1(%lu) + MX_SCALE_GROUP(%lu) - 1) / MX_SCALE_GROUP(%lu),"
                "gmmWeightScaleDim2 should be equal to gmmWeightDim2(%lu), gmmWeightScaleDim3 should be equal to 2, "
                "Expected Shape of gmmWeightScale = (%lu, %lu, %lu, %lu), Actual Shape of gmmWeightScale = (%lu, %lu, %lu, %lu).",
                localParams_.ep, gmmxDivH1, localParams_.N1, MX_SCALE_GROUP, MX_SCALE_GROUP, localParams_.ep, gmmxDivH1, localParams_.N1, EVEN_ALIGN, 
                gmmWeightScaleDim0, gmmWeightScaleDim1, gmmWeightScaleDim2, gmmWeightScaleDim3), return ge::GRAPH_FAILED);
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus MxQuantGroupedMatmulAllToAllvTiling::CheckMxQuantMmScaleShapes()
{
    ...
    OP_TILING_CHECK((localParams_.Bs != mmXScaleDim0) || (mmxDivH2 != mmXScaleDim1) || (mmXScaleDim2 != EVEN_ALIGN),
        OP_LOGE(opName_, "In the Non-Transposed Scenario, Wrong shape of mmXScale! "
            "mmXScaleDim0 should be equal to mmXDim0(%lu), "
            "mmXScaleDim1 should be equal to (mmXDim1(%lu) + MX_SCALE_GROUP(%lu) - 1) / MX_SCALE_GROUP(%lu), mmXScaleDim2 should be equal to 2, "
