...
ge::graphStatus MxQuantGroupedMatmulAllToAllvTiling::CheckParamsRelationGmm()
{
    ...
    OP_TILING_CHECK(localParams_.gmmXQuantMode != QUANT_MX,
                    OP_LOGE(opName_, "The gmmXQuantMode should be MX mode (value=%ld), but actual mode is %ld !", QUANT_MX,
                            localParams_.gmmXQuantMode),
                    return ge::GRAPH_FAILED);
    OP_TILING_CHECK(localParams_.gmmWeightQuantMode != QUANT_MX,
                    OP_LOGE(opName_, "The gmmWeightQuantMode should be MX mode (value=%ld), but actual mode is %ld !", QUANT_MX,
