...
{
    OP_LOGI(opName_, "[MX_TILING] Enter CheckParamsRelationGmm");
    ...
    OP_LOGI(opName_, "[MX_TILING] gmmXScaleDtype=%d, gmmWeightScaleDtype=%d",
            localParams_.gmmXScaleDtype, localParams_.gmmWeightScaleDtype);
    OP_LOGI(opName_, "[MX_TILING] isGmmWeightTrans=%d", localParams_.isGmmWeightTrans);
...
ge::graphStatus MxQuantGroupedMatmulAllToAllvTiling::CheckParamsAttrEpAndSetLocalParams()
{
    ...
    OP_TILING_CHECK(((groupSizeM != MX_GROUP_SIZE_M) && (groupSizeM != 0)) ||
        ...
        CUBE_INNER_ERR_REPORT(
            opName_,
            "GroupSizeM, groupSizeN should be 0 or 1, and groupSizeK should be 0 or 32 in mxfp scene,"
