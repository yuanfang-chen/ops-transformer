...
constexpr size_t INPUT_K_GMM_X_OFFSET = 8;
constexpr size_t INPUT_K_GMM_WEIGHT_OFFSET = 9;
constexpr size_t INPUT_K_MM_X_SCALE = 10;
constexpr size_t INPUT_K_MM_WEIGHT_SCALE = 11;
constexpr size_t INPUT_K_MM_X_OFFSET = 12;
constexpr size_t INPUT_K_MM_WEIGHT_OFFSET = 13;
constexpr size_t INPUT_K_COMM_QUANT_SCALE = 14;
...
struct QuantGroupedMatMulAlltoAllvInputs {
    ...
    const gert::Tensor* gmmXOffset;
    const gert::Tensor* gmmWeightOffset;
    const gert::Tensor* mmXScale;
    const gert::Tensor* mmWeightScale;
    const gert::Tensor* mmXOffset;
    const gert::Tensor* mmWeightOffset;
...
static ge::graphStatus GetInputs(gert::OpExecuteContext* host_api_ctx, QuantGroupedMatMulAlltoAllvInputs& inputs)
{
    ...
    inputs.gmmXOffset = host_api_ctx->GetOptionalInputTensor(INPUT_K_GMM_X_OFFSET);
    inputs.gmmWeightOffset = host_api_ctx->GetOptionalInputTensor(INPUT_K_GMM_WEIGHT_OFFSET);
    ...
    inputs.mmXOffset = host_api_ctx->GetOptionalInputTensor(INPUT_K_MM_X_OFFSET);
    inputs.mmWeightOffset = host_api_ctx->GetOptionalInputTensor(INPUT_K_MM_WEIGHT_OFFSET);
...
{
    const auto apiRet = EXEC_OPAPI_CMD(aclnnQuantGroupedMatMulAlltoAllv,
                                       inputs.gmmX, inputs.gmmWeight, inputs.gmmXScale, inputs.gmmWeightScale,
                                       inputs.gmmXOffset, inputs.gmmWeightOffset, inputs.sendCountsTensor,
                                       inputs.recvCountsTensor, inputs.mmX, inputs.mmWeight, inputs.mmXScale,
                                       inputs.mmWeightScale, inputs.mmXOffset, inputs.mmWeightOffset,
                                       inputs.commQuantScale, *attrsData.gmmXQuantMode, *attrsData.gmmWeightQuantMode,
                                       *attrsData.mmXQuantMode, *attrsData.mmWeightQuantMode, *attrsData.commQuantMode,
                                       *attrsData.groupSize, *attrsData.commQuantDtype, attrsData.group,
                                       *attrsData.epWorldSize, actSendCountsSeqArray, actRecvCountsSeqArray,
                                       *attrsData.transGmmWeight, *attrsData.transMmWeight, y, mmY);
...
static ge::graphStatus QuantGroupedMatMulAlltoAllvExecuteFunc(gert::OpExecuteContext* host_api_ctx)
{
    ...
    return ExecuteQuantGroupedMatMulAlltoAllv(inputs, attrsData, actSendCountsSeqArray, actRecvCountsSeqArray, y, mmY);
}
