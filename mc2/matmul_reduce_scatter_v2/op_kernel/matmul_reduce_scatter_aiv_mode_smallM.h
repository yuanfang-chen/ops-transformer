    ...
    for (int32_t p = 0; p < p_value; p++) {
        ...
        if (needPerChannel & needPerToken) {
            blockEpilogue(perChannelScale + blockLocCoord.n(), layoutPerChannelScale, perTokenScale + blockLocCoord.m(),
                          layoutPerTokenScale, reinterpret_cast<__gm__ biasType *>(biasGM_), layoutBias, workspace + gmOffsetC, layout_tmp, gmPeerMem + gmOffsetC, layout_tmp,
                          blockSizeCoord);
        } else if (needPerChannel) {
            blockEpilogue(perChannelScale + blockLocCoord.n(), layoutPerChannelScale, reinterpret_cast<__gm__ biasType *>(biasGM_), layoutBias, workspace + gmOffsetC, layout_tmp,
                          gmPeerMem + gmOffsetC, layout_tmp, blockSizeCoord);
        } else if (needPerToken) {
            blockEpilogue(perTokenScale + blockLocCoord.m(), layoutPerTokenScale, reinterpret_cast<__gm__ biasType *>(biasGM_), layoutBias, gmPeerMem + gmOffsetC, layout_tmp,
                          gmPeerMem + gmOffsetC, layout_tmp, blockSizeCoord);
        }
    }
}
