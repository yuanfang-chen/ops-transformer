...
#define TEMPLATE_FUNC_PARAMS AType, BType, CType, ScaleType, MMClass, IsPerToken, ATrans, BTrans
...
private:
    ...
    __aicore__ inline void MatMulComputReduceScatterPerChannel(GM_ADDR aGM, GM_ADDR cGM,
                                                     DequantBmm::Mc2QuantBatchMatmulV3TilingDataParams& qBMmtiling, uint32_t tileCnt,
                                                     GM_ADDR gmToFloat, bool isLast, bool isTail);
    __aicore__ inline void MatMulComputReduceScatterPerToken(GM_ADDR aGM, GM_ADDR cGM, GM_ADDR x1ScaleGM,
                                                     DequantBmm::Mc2QuantBatchMatmulV3TilingDataParams& qBMmtiling, uint32_t tileCnt,
...
QuantBMMReduceScatter<TEMPLATE_FUNC_PARAMS>::MatMulComputReduceScatterPerChannel(
...
TEMPLATE_CLASS_PARAMS
__aicore__ inline void
QuantBMMReduceScatter<TEMPLATE_FUNC_PARAMS>::MatMulComputReduceScatterPerToken(
    ...
    GM_ADDR gmToFloat, bool isTail)
{
    ...
    // 卡内偏移
    uint64_t x1ScaleOffset = sizeof(ScaleType) * tiling.M;
    for (uint32_t i = 0; i < tileCnt; i++) {
        this->tPipe_->Destroy();
        this->tPipe_->Init();
        MMClass op;
...
{
    if constexpr (IsPerToken) {
        MatMulComputReduceScatterPerToken(aGM, cGM, x1ScaleGM, qBMmtiling, tileCnt, gmToFloat, isTail);
    } else {
