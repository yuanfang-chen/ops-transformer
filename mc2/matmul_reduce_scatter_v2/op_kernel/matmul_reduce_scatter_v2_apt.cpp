        ...
        if (tilingData.debugMode != static_cast<uint8_t>(MC2_DEBUG_ONLY_AICPU)) {                                 \
            using mmClass =                                                                                           \
                Mc2MatmulV3::Mc2QuantBatchMatmulASWKernel<DTYPE_X1, DTYPE_X2, scaleType, DTYPE_BIAS, DTYPE_Y,         \
                                                         CubeFormat::ND, CubeFormat::ND, CubeFormat::ND, __VA_ARGS__>;\
            templateClass<DTYPE_X1, DTYPE_X2, DTYPE_Y, scaleType, float, mmClass, false, __VA_ARGS__> op;             \
            op.Init(aGM, bGM, biasGM, x1ScaleGM, x2ScaleGM, cGM, (GM_ADDR)context, workspaceGM, &tilingData,          \
                    mc2InitTiling, mc2CcTiling, &pipe);                                                               \
            op.Process();                                                                                             \
        }                                                                                                             \
...
#define INVOKE_QUANT_BATCHMM_PERTOKEN_REDUCE_SCATTER_OP_IMPL(templateClass, scaleType, isATrans, isBTrans, ...)                           \
    do {                                                                                                              \
        REGISTER_TILING_DEFAULT(Mc2Tiling::QuantBatchMatmulV3ReduceScatterTilingData);                                \
        auto tiling = (__gm__ Mc2Tiling::QuantBatchMatmulV3ReduceScatterTilingData*)tilingGM;                         \
        __gm__ void* mc2InitTiling = (__gm__ void*)(&(tiling->mc2InitTiling));                                        \
        ...
        if (tilingData.debugMode != static_cast<uint8_t>(MC2_DEBUG_ONLY_AICPU)) {                                 \
            using mmClass =                                                                                           \
                Mc2QuantBatchMatmulV3::Mc2QuantBmmPertokenRegbaseKernel<DTYPE_X1, DTYPE_X2, scaleType, DTYPE_BIAS, float, DTYPE_Y, CubeFormat::ND, CubeFormat::ND, CubeFormat::ND,   \
                                                    isATrans, isBTrans, DTYPE_LOC_LOCAL, Mc2QuantBatchMatmulV3::Mc2QuantBmmAswBlock, MM_CFG_NO_PRELOAD_OPEN_UNIT_FLAG>;              \
            templateClass<DTYPE_X1, DTYPE_X2, DTYPE_Y, scaleType, float, mmClass, true, isATrans, isBTrans> op;              \
            op.Init(aGM, bGM, biasGM, x1ScaleGM, x2ScaleGM, cGM, (GM_ADDR)context, workspaceGM, &tilingData,          \
                    mc2InitTiling, mc2CcTiling, &pipe);                                                               \
            op.Process();                                                                                             \
        }                                                                                                             \
