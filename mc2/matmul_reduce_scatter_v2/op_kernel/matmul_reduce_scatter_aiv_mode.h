...
__aicore__ inline void MatmulReduceScatterAivMode<TemplateMMReduceScatterV2Func, Derived>::CatlassMatmul()
{
    if ASCEND_IS_AIC {
        ...
        constexpr bool MC2_NON_QUANT = ((Atype == Btype) && (Atype == DT_FLOAT16 || Btype == DT_BF16));
 	    constexpr bool aicCalBias = MC2_NON_QUANT && hasBias;   // 如果计算量化后的矩阵乘，bias不由CatlassMatmul负责
