                                            ...
                                            const aclTensor* x1Scale, const aclTensor* x2Scale)
{
    ...
    if (x2->GetDataType() == op::DataType::DT_INT8 && !CheckEmptyTensor(x2Scale, "x2Scale")) {
        return CaseOption::INVALID;
    } else if (!CheckEmptyTensor(x1Scale, "x1Scale") || !CheckEmptyTensor(x2Scale, "x2Scale")) {
        return CaseOption::INVALID;
    }
    if (x1->GetDataType() == op::DataType::DT_INT8) {
        if ((!CheckEmptyTensor(x1Scale, "x1Scale")) && (x2Scale->GetViewShape().GetDimNum() == DIM_NUM_TWO)) {
