...
static const std::initializer_list<op::DataType> AIV_MODE_OUTPUT_SUPPORT_TYPE = {
    op::DataType::DT_FLOAT16, op::DataType::DT_BF16};
 static bool CheckAivModeDtypeValid(const aclTensor* x1, const aclTensor* x2, const aclTensor* bias, const aclTensor* output) 
{ 
...
static aclnnStatus CheckAivModeParams(const aclTensor* x1, const aclTensor* x2, const aclTensor* bias, int64_t streamMode, 
                                const aclTensor* output) 
{ 
    // 1. 检查参数是否为空指针 
    CHECK_RET(CheckNotNull(x1, x2, output), ACLNN_ERR_PARAM_NULLPTR); 
