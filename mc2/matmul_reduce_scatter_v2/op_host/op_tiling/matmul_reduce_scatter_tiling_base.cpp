...
ge::graphStatus MatmulReduceScatterTilingBase::CheckHCCLSize()
{
    ...
    OP_TILING_CHECK(sizeOfSingleM > mc2tiling::ALL_GATHER_HCCL_MEM_LIMIT,
        OP_LOGE(opName_, "Unsupported matmul output size. Even after splitting data matmul output into (1, n), the size still exceeds 256MB."),
    ...
    OP_TILING_CHECK(sizeOfSplitM > mc2tiling::ALL_GATHER_HCCL_MEM_LIMIT,
        OP_LOGE(opName_, "Unsupported x1 size. Even after splitting data M into 16 parts (rounded up), the size still exceeds 256MB."),
                return ge::GRAPH_FAILED);
