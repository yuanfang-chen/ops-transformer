    ...
    GM_ADDR output, TPipe *tPipe, const AllGatherTilingData *tilingData)
{
    ...
    xSize_ = tilingData->bs * tilingData->hiddenSize * sizeof(XType);
    scaleSize_ = tilingData->s * tilingData->scaleHiddenSize * sizeof(ScalesType);
    ...
    xSliceSize_ = xSize_ / (mteComm_.hcclContext_->rankSize ); // all2all过程，数据需要按卡数均分
    xSliceSizeNums_ = xSliceSize_ / sizeof(XType); // 按卡均分后每片x数据的个数
    scaleSliceNums_ = scaleSize_ / (mteComm_.hcclContext_->rankSize * sizeof(ScalesType)); // all2all过程，每张卡需要的scale数据个数
    ...
    mteComm_.round_ = totalBlockNums_ / tilingData->aivNum;  // 计算数据分核搬运需要的轮次数
...
template <TemplateTypeClass>
__aicore__ inline void AllGatherMte<TemplateType>::ReadDataBlockReduceSum(uint64_t curXOffset,
    uint64_t curScaleOffset)
{
    ...
    /* 反量化计算与ReduceSum求和 */
    // TODO: 此处去除ReduceSum，并且将反量化后数据写回win区
...
template <TemplateTypeClass>
__aicore__ inline void AllGatherMte<TemplateType>::ClearSumTensor()
{
...
__aicore__ inline void AllGatherMte<TemplateType>::Process()
{
    ...
    // TODO: 这一步不需要，ARN_DQ 之后已经在本地 win 区了
    // 一次性拷贝完所有数据到本地卡win区
