    ...
    TPipe *tPipe, const AddRmsNormDynamicQuantAllGatherQbmmInfo *tilingData)
{
    ...
    /* all_gather 自己的数据 */
    M_ = tilingData_->addRmsNormDynamicQuantAllGatherTilingData.M;
    K_ = tilingData_->addRmsNormDynamicQuantAllGatherTilingData.Ka;
    xNums_ = tilingData_->addRmsNormDynamicQuantAllGatherTilingData.xNums; // 总的x数据个数， M * h
    xSize_ = tilingData_->addRmsNormDynamicQuantAllGatherTilingData.xSize; // 总的x数据量，B
    scaleSize_ = tilingData_->addRmsNormDynamicQuantAllGatherTilingData.scaleSize;
    totalBlockNums_ = CeilDiv(xSize_, X_BLOCK_BYTES); // 按每次搬运x的数据量分块，得到的总块数
    sendCoreNumPerRank_ = tilingData_->addRmsNormDynamicQuantAllGatherTilingData.sendCoreNumPerRank;
    tileK_ = tilingData_->addRmsNormDynamicQuantAllGatherTilingData.mteTileK;
    ...
    // 按k方向奇偶切分成两部分
    uint32_t kDim = tilingData_->addRmsNormDynamicQuantAllGatherTilingData.mteKSplitNum;
    uint32_t mDim = tilingData_->addRmsNormDynamicQuantAllGatherTilingData.mteMSplitNum;
    ...
    mMteCoreM_ = tilingData_->addRmsNormDynamicQuantAllGatherTilingData.mteMSplitSize;
    kMteCoreK_ = tilingData_->addRmsNormDynamicQuantAllGatherTilingData.mteKSplitSize;
