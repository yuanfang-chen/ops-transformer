...
static void GetAddRmsNormDynamicQuantAllGatherQbmm(gert::TilingContext *context, AddRmsNormDynamicQuantAllGatherQbmmTilingData &tilingData)
{   
...
// matmul切分
static void GetMatmultiling(gert::TilingContext *context, AddRmsNormDynamicQuantAllGatherQbmmTilingData &tilingData)
{
    MmTilingHelper mmTilingHelper(context);
    OP_CHECK_IF(!mmTilingHelper.getMamtulArgs(), OP_LOGE(context, "MmTilingHelper get matmulArgs failed, return Failed."), return ge::GRAPH_FAILED);
    mmTilingHelper.InitCompileInfo();
    OP_CHECK_IF(!mmTilingHelper.InitTCubeTilingData(), OP_LOGE(context, "MmTilingHelper InitTCubeTilingData failed, return Failed."), return ge::GRAPH_FAILED);
}
/**
 * @brief 设置hcomm参数
 * @param context: 框架根据input，output，attrs等信息生成tiling需要的context
 ...
 * @return
 */
static ge::graphStatus SetHcommCfg(const gert::TilingContext *context, AddRmsNormDynamicQuantAllGatherQbmmTilingData *tilingData,
...
static ge::graphStatus AddRmsNormDynamicQuantAllGatherQbmmTilingFunc(gert::TilingContext *context)
{
    OP_TILING_CHECK(context == nullptr,
                    OP_LOGE("add_rms_norm_dynamic_quant_all_gather_qbmm", "failed to get tiling context in add_rms_norm_dynamic_quant_all_gather_qbmm."),
                    return ge::GRAPH_FAILED);
    const char *nodeName = context->GetNodeName();
    OP_TILING_CHECK(nodeName == nullptr,
                    OP_LOGE("add_rms_norm_dynamic_quant_all_gather_qbmm", "failed to get nodeName in add_rms_norm_dynamic_quant_all_gather_qbmm."),
                    return ge::GRAPH_FAILED);

    AddRmsNormDynamicQuantAllGatherQbmmTilingData *tilingData = context->GetTilingData<AddRmsNormDynamicQuantAllGatherQbmmTilingData>();
    OP_TILING_CHECK(tilingData == nullptr, OP_LOGE(nodeName, "tilingData is nullptr in add_rms_norm_dynamic_quant_all_gather_qbmm."),
                    return ge::GRAPH_FAILED);

    // addrmsnormdynamicquantallgatherqbmm 部分tiling切分
    GetAddRmsNormDynamicQuantAllGatherQbmm(context, tilingData);
    // 先切k= 2560，直接调用matmul做tiling切分
    GetMatmultiling(context, tilingData);
