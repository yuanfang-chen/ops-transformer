...
bool IsTransposeLastTwoDims(const aclTensor *tensor) {
    ...
    OP_LOGE(ACLNN_ERR_PARAM_INVALID, "tensor_dim0 is: %ld, tensor_dim1 is: %ld.", tensor_dim0, tensor_dim1);

    int64_t stride_dim0 = tensor->GetViewStrides()[0];
    int64_t stride_dim1 = tensor->GetViewStrides()[1];
    OP_LOGE(ACLNN_ERR_PARAM_INVALID, "stride_dim0 is: %ld, stride_dim1 is: %ld.", stride_dim0, stride_dim1);

    int64_t storageDim0 = tensor->GetStorageShape().GetDim(0);
    int64_t storageDim1 = tensor->GetStorageShape().GetDim(1);
    OP_LOGD(ACLNN_ERR_PARAM_INVALID, "In aclnn, storageDim0 is: %ld, storageDim1 is: %ld", storageDim0, storageDim1);
    ...
    if (tensor->GetViewStrides()[dim2] == 1
      && tensor->GetViewStrides()[dim1] == tensor->GetViewShape().GetDim(dim2)) {
        ...
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "current tensor is not contiguous.");
        return true;
      }
    return false;
}
