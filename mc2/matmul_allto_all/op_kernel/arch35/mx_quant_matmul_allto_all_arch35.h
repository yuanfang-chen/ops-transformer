...
#include "../mc2_templates/computation/matmul/mx_quant_matmul.h"
...
#ifndef DEFINE_AND_IMPL_MC2_MATMUL_FOR_MATMUL_COMPUTATION_MXQUANT
#define DEFINE_AND_IMPL_MC2_MATMUL_FOR_MATMUL_COMPUTATION_MXQUANT(TilingType, ComputationType)                         \
    using ComputationType =                                                                                            \
        MXQuantMatmul<AscendC::MatMulASWKernel<DTYPE_X1, DTYPE_X2, fp8_e8m0_t, DTYPE_BIAS, DTYPE_Y, CubeFormat::ND,    \
                                               CubeFormat::ND, CubeFormat::ND, false, X2TRANSPOSE>,                    \
                      MXQuantExtraData, TilingType>
#endif
    ...
    uint32_t taskCnt)
{
    ...
    // mx的scale大小为1B
    pipeLineContext_.extraData.x1_scale_offset =
    ...
    uint32_t taskCnt)
{
    ...
    pipeLineContext_.transposeSrcAddr =
        tempComputeOutGM_ + mc2Tiling_.tileCnt * mc2Tiling_.tileM * mc2Tiling_.rankN * sizeof(DTYPE_Y);
    pipeLineContext_.transposeDstAddr = transOutGM_ + mc2Tiling_.tileCnt * mc2Tiling_.tileM * mc2Tiling_.rankN *
                                                          sizeof(DTYPE_Y) / (uint64_t)mc2Tiling_.rankDim;
    pipeLineContext_.transposeSrcOffset = (uint64_t)mc2Tiling_.tailM * (uint64_t)mc2Tiling_.rankN * sizeof(DTYPE_Y);
    ...
    pipeLineContext_.sendBuffer = pipeLineContext_.transposeDstAddr;
    pipeLineContext_.recvBuffer =
        y_ + mc2Tiling_.tileCnt * mc2Tiling_.tileM * mc2Tiling_.rankN * sizeof(DTYPE_Y) / (uint64_t)mc2Tiling_.rankDim;
    pipeLineContext_.sendOffset =
        (uint64_t)mc2Tiling_.tailM * (uint64_t)mc2Tiling_.rankN * sizeof(DTYPE_Y) / (uint64_t)mc2Tiling_.rankDim;
    pipeLineContext_.recvOffset = pipeLineContext_.sendOffset;
...
template <typename SchedulerType, typename SchedulerContextType, typename MatmulAlltoAllTilingDataType>
__aicore__ inline void
MxQuantMatmulAlltoAllArch35<SchedulerType, SchedulerContextType, MatmulAlltoAllTilingDataType>::ProcessPipeLine(
    uint32_t taskCnt)
{
