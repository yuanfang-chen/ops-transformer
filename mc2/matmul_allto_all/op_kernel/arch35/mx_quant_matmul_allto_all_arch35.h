    ...
    uint32_t taskCnt)
{
    ...
    pipeLineContext_.computationContext->additionalData.x1ScaleOffset =
        ComputeByteOffset<AscendC::fp8_e8m0_t>((uint64_t)mc2Tiling_.tileM * CeilDiv(mc2Tiling_.rankK, MXFP_GROUP_SIZE) * NUM_TWO);
    ...
    uint32_t taskCnt)
{
    ...
    pipeLineContext_.computationContext->baseData.bGM = x2_;
    pipeLineContext_.computationContext->baseData.cGM = ComputeGMAddr<DTYPE_Y>(
        tempComputeOutGM_, (uint64_t)mc2Tiling_.tileM * mc2Tiling_.rankN * (uint64_t)mc2Tiling_.tileCnt);
    ...
    pipeLineContext_.computationContext->additionalData.x1Scale =
        ComputeGMAddr<AscendC::fp8_e8m0_t>(x1Scale_, (uint64_t)mc2Tiling_.tileM * CeilDiv(mc2Tiling_.rankK, MXFP_GROUP_SIZE) *
                                             NUM_TWO * mc2Tiling_.tileCnt);
    pipeLineContext_.computationContext->additionalData.x2Scale = x2Scale_;
