                                            ...
                                            GM_ADDR workspaceGM, GM_ADDR tilingGM)
{
    ...
    op.Process();
#else
    REGISTER_TILING_DEFAULT(MxQuantMatmulAlltoAllTilingData);
    GET_TILING_DATA_WITH_STRUCT(MxQuantMatmulAlltoAllTilingData, tilingData, tilingGM);
    ...
    DEFINE_MC2_TRANSPOSE_FOR_MATH_COMPUTATION(DTYPE_Y, TransposeType);
    TransposeType transposeImplName(&pipe);
    DEFINE_MC2_HCCL_FOR_COMMUNICATION(HcclServerType::HCCL_SERVER_TYPE_CCU, 1, 0, MxQuantMatmulAlltoAllTilingData, CommunicationType);
    CommunicationType commImplName(&tilingData);
    using SchedulerContextType = PipelineContext<MXQuantExtraData, DequantBmm::Mc2QuantBatchMatmulV3TilingDataParams>;
    using SchedulerType = MC2KernelPipelineTemplate<ComputationType, TransposeType, CommunicationType, SchedulerContextType>;
    SchedulerType SchedulerImpl(&matmulImplName, &transposeImplName, &commImplName);
    MxQuantMatmulAlltoAllArch35<SchedulerType, SchedulerContextType, MxQuantMatmulAlltoAllTilingData> op(&SchedulerImpl);
