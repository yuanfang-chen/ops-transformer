...
#define MATMUL_ALLTO_ALL_A3_FP_IMPL(tilingData, pipe)                                                                  \
    do {                                                                                                               \
        ...
        Mc2Kernel::MatmulAlltoAllArch35<SchedulerType,                                                                 \
            SchedulerContextType, MatmulAlltoAllTilingDataA3> op(&SchedulerImpl);                                      \
...
#define MATMUL_ALLTO_ALL_APT_FP_IMPL(tilingData, pipe)                                                                 \
    do {                                                                                                               \
        ...
        SchedulerType SchedulerImpl(&matmulImplName, &transposeImplName, &commImplName);                               \
        Mc2Kernel::MatmulAlltoAllArch35<SchedulerType,                                                                 \
            SchedulerContextType, MatmulAlltoAllTilingData> op(&SchedulerImpl);                                        \
        op.Init(x1, x2, bias, y, workspaceGM, &tilingData, &pipe);                                                     \
        op.Process();                                                                                                  \
    } while (0)
#endif
...
#define MATMUL_ALLTO_ALL_APT_KC_QUANT_IMPL(tilingData, pipe)                                                           \
    do {                                                                                                               \
        ...
        SchedulerType SchedulerImpl(&matmulImplName, &transposeImplName, &commImplName);                               \
        Mc2Kernel::KcQuantMatmulAlltoAllArch35<SchedulerType,                                                          \
            SchedulerContextType, QuantMatmulAlltoAllTilingData> op(&SchedulerImpl);                                   \
...
#define MATMUL_ALLTO_ALL_APT_MX_QUANT_IMPL(tilingData, pipe)                                                           \
    do {                                                                                                               \
        ...
        SchedulerType SchedulerImpl(&matmulImplName, &transposeImplName, &commImplName);                               \
        Mc2Kernel::MxQuantMatmulAlltoAllArch35<SchedulerType,                                                          \
            SchedulerContextType, QuantMatmulAlltoAllTilingData> op(&SchedulerImpl);                                   \
