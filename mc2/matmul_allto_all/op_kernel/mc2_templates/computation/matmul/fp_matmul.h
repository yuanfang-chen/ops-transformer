...
public:
    ...
    // 获取数据上下文引用
    __aicore__ inline MC2MMContext<FpMMAdditionalData, MMTilingType>* GetMMContextPtr();
    // 执行一次计算的方法
...
template <typename MMTilingType, typename MMType>
inline __aicore__ MC2MMContext<FpMMAdditionalData, MMTilingType> *MC2FpMMWrapper<MMTilingType, MMType>::GetMMContextPtr()
{
    return &MMcontext_;
}
