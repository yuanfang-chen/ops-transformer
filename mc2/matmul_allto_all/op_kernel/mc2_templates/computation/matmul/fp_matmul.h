...
public:
    __aicore__ inline void  Init(MC2MMBaseGmAddrs* baseDataPtr, FpMMAdditionalData* additionalDataPtr, MMType* MMImplPtr, AscendC::TPipe *tPipe);
    __aicore__ inline void  UpdateAdditionalData();
    __aicore__ inline void  InitMM(Mc2MatMulV3TilingData* tilingDataPtr);
...
template <typename MMType>
__aicore__ inline void FpMMControl<MMType>::InitMM(Mc2MatMulV3TilingData* tilingDataPtr)
{
    if ASCEND_IS_AIV {
        return;
    }
    tPipePtr_->Reset();
    MMImplPtr_->Init(baseDataPtr_->aGM, baseDataPtr_->bGM, baseDataPtr_->cGM, baseDataPtr_->biasGM, nullptr, nullptr, tilingDataPtr, tPipePtr_);
}
