...
public:
    __aicore__ inline void  Init(MC2MMBaseGmAddrs* baseDataPtr, KCQuantMMAdditionalData* additionalDataPtr, MMType* MMImplPtr, AscendC::TPipe *tPipe);
    __aicore__ inline void  UpdateAdditionalData();
    __aicore__ inline void  InitMM(DequantBmm::Mc2QuantBatchMatmulV3TilingDataParams* tilingDataPtr);
...
template <typename MMType>
__aicore__ inline void KCQuantMMControl<MMType>::InitMM(DequantBmm::Mc2QuantBatchMatmulV3TilingDataParams* tilingDataPtr)
{
    tPipePtr_->Reset();
    MMImplPtr_->Init(baseDataPtr_->aGM, baseDataPtr_->bGM, additionalDataPtr_->x2_scale, additionalDataPtr_->x2_offset, baseDataPtr_->biasGM, additionalDataPtr_->x1_scale, baseDataPtr_->cGM, nullptr, tilingDataPtr, tPipePtr_);
}
