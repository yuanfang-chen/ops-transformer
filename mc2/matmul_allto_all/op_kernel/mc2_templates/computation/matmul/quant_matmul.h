...
public:
    ...
    // 获取数据上下文引用
    __aicore__ inline MC2MMContext<KCQuantMMAdditionalData, MMTilingType>* GetMMContextPtr();
    // 执行一次计算的方法
...
MC2KCQuantMMWrapper<MMTilingType, MMType>::GetMMContextPtr()
{
    return &MMcontext_;
}

template <typename MMTilingType, typename MMType>
inline __aicore__ void MC2KCQuantMMWrapper<MMTilingType, MMType>::Process(uint32_t taskIndex)
{
