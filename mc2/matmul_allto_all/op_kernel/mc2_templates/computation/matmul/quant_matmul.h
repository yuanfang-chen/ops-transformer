...
protected:
    MC2MMContext<KCQuantMMAdditionalData, DequantBmm::Mc2QuantBatchMatmulV3TilingDataParams> MMcontext_;
    MMType MMImpl_;
...
public:
    ...
    // 获取数据上下文引用
    __aicore__ inline MC2MMContext<KCQuantMMAdditionalData, DequantBmm::Mc2QuantBatchMatmulV3TilingDataParams>* GetMMContextPtr();
    // 执行一次计算的方法
...
template <typename MMType>
inline __aicore__ void MC2KCQuantMMWrapper<MMType>::Init() {}
template <typename MMType>
inline __aicore__ MC2MMContext<KCQuantMMAdditionalData, DequantBmm::Mc2QuantBatchMatmulV3TilingDataParams> *
MC2KCQuantMMWrapper<MMType>::GetMMContextPtr()
{
    return &MMcontext_;
}
template <typename MMType>
inline __aicore__ void MC2KCQuantMMWrapper<MMType>::Process(bool isFirst)
{
    if (!isFirst) {
        MMcontext_.baseData.aGM = MMcontext_.baseData.aGM + MMcontext_.baseData.aOffset;
        MMcontext_.baseDatabGM = MMcontext_.baseData.bGM + MMcontext_.baseData.bOffset;
        MMcontext_.baseDatacGM = MMcontext_.baseData.cGM + MMcontext_.baseData.cOffset;
        MMcontext_.additionalData.x1Scale = MMcontext_.additionalData.x1Scale + MMcontext_.additionalData.x1ScaleOffset;
    }
    tPipePtr_->Reset();
    MMImpl_.Init(MMcontext_.baseData.aGM, MMcontext_.baseData.bGM, MMcontext_.additionalData.x2Scale,
        MMcontext_.additionalData.x2Offset, MMcontext_.baseData.biasGM, MMcontext_.additionalData.x1Scale,
        MMcontext_.baseData.cGM, nullptr, MMcontext_.tilingDataPtr, tPipePtr_);
    MMImpl_.Process();
}
template <typename MMType>
