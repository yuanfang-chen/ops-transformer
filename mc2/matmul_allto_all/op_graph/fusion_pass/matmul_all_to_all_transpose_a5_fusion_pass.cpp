                                                  ...
                                                  const std::unique_ptr<ge::fusion::MatchResult> &matchResult)
{
    ...
    int64_t inputIdx = 0;
    auto rX1 = replaceGraphBuilder.CreateInput(inputIdx, "x1", inputDTypes[inputIdx], inputFormats[inputIdx],
                                               inputShapes[inputIdx].GetDims());
    ++inputIdx;
    auto rX2 = replaceGraphBuilder.CreateInput(inputIdx, "x2", inputDTypes[inputIdx], inputFormats[inputIdx],
                                               inputShapes[inputIdx].GetDims());
    ++inputIdx;
    ...
    OPS_LOG_D(kPassName.c_str(), "Check Pattern Name is %s.", patternNameStr.c_str());
    size_t inputDTypeSize = inputDTypes.size();
    OPS_LOG_D(kPassName.c_str(), "Check input DType size is %zu.", inputDTypeSize);
    ...
    if (patternNameStr.find(kPatternBias) != std::string::npos) {
        ...
        rBias = replaceGraphBuilder.CreateInput(inputIdx, "bias", inputDTypes[inputIdx], inputFormats[inputIdx],
                                                inputShapes[inputIdx].GetDims());
        ++inputIdx;
    }
    auto rX1Scale = replaceGraphBuilder.CreateInput(inputIdx, "x1_scale", inputDTypes[inputIdx], inputFormats[inputIdx],
                                                    inputShapes[inputIdx].GetDims());
    ++inputIdx;
    auto rX2Scale = replaceGraphBuilder.CreateInput(inputIdx, "x2_scale", inputDTypes[inputIdx], inputFormats[inputIdx],
                                                    inputShapes[inputIdx].GetDims());
    ...
    auto mc2 =
        ge::es::MatmulAlltoAll(rX1, rX2, rBias, rX1Scale, rX2Scale, nullptr /*rCommScale*/, nullptr /*rX1Offset*/,
                               nullptr /*rX2Offset*/, group.GetString(), worldSize, all2allAxes, yDType, x1QuantMode,
