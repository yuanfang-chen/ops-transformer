                          ...
                          std::vector<ge::Format> &inputFormats)
{
    ...
    for (const auto &subGraphInput : subGraphInputs) {
        ...
        auto matchNode = matchNodes.at(0); // input node 被多个当前pattern的节点消费时, 序号递增
        OPS_LOG_D(kPassName.c_str(), "Check input node name %s.", matchNode.GetName().c_str());
...
MakePattern -->vector<bool> 对应inputs的顺序
