                          ...
                          std::vector<ge::Format> &inputFormats)
{
    ...
    for (const auto &subGraphInput : subGraphInputs) {
        ...
        matchNode.node.GetName(nodeName); // get到的是这个输入指向的计算算子节点名称，不是本身input data的名称
        OPS_LOG_D(kPassName.c_str(), "Check match node name %s.", nodeName.GetString());
        ge::GNode curNode = matchNode.node; 
        size_t inputSize = curNode.GetInputsSize(); // 这是算子节点？
        OPS_LOG_D(kPassName.c_str(), "Check cur node input size %zu.", inputSize);
        OPS_LOG_D(kPassName.c_str(), "Check match node index %zu.", matchNode.index);
        // 看一下这个matchnode.index 是不是就是对应可取inputNode的index
        std::pair<ge::GNodePtr, int32_t> inputDataPair = curNode.GetInDataNodesAndPortIndexs(matchNode.index);
        if (inputDataPair.first != nullptr) {
            ge::AscendString inputNodeName = "";
            inputDataPair.first->GetName(inputNodeName);
            OPS_LOG_D(kPassName.c_str(), "Check input node name %s.", inputNodeName.GetString());
            OPS_LOG_D(kPassName.c_str(), "Check input node index inside %d.", inputDataPair.second);
        }
