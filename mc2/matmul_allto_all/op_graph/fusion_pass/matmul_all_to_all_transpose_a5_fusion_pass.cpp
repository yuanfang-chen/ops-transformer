...
const std::string kPatternBias = "HasBias";
...
const int64_t kInputNumWithBias = 5;

static ge::fusion::PatternUniqPtr MakePatternForTranspose(bool hasBias)
{
    std::string patternName = kPassName + kPatternTranspose;
    if (hasBias) {
        patternName += kPatternBias;
    }
    ...
    auto mc2 = ge::es::MatmulAlltoAll(x1, transpose, hasBias ? bias : nullptr, x1Scale, x2Scale, nullptr, nullptr, nullptr,
                                   ...
                                   const std::vector<ge::fusion::SubgraphInput> &subgraphInputs)
{
    ...
    for (const auto &subgraphInput : subgraphInputs) {
        auto matchNodes = subgraphInput.GetAllInputs();
        if (matchNodes.empty()) {
            OPS_LOG_D(kPassName.c_str(), "InferShapeReplaceGraph: matchNodes is empty.");
            continue;
        }
                                                  ...
                                                  const std::unique_ptr<ge::fusion::MatchResult> &matchResult)
{
    ...
    int64_t inputIdx = 0;
    auto rX1 = replaceGraphBuilder.CreateInput(inputIdx, "x1", inputDTypes[inputIdx], inputFormats[inputIdx], inputShapes[inputIdx].GetDims());
    auto rX2 = replaceGraphBuilder.CreateInput(++inputIdx, "x2", inputDTypes[inputIdx], inputFormats[inputIdx], inputShapes[inputIdx].GetDims());
    ...
    OPS_LOG_D(kPassName.c_str(), "Check Pattern Name is %s.", patternNameStr);
    ge::es::EsTensorHolder rBias = nullptr;
    if (patternNameStr.find(kPatternBias) != std::string::npos) {
        if (inputShapes.size() < kInputNumWithBias) {
            OPS_LOG_E(kPassName.c_str(), "Input Nums do not meet requirement with bias.");
            return nullptr;
        }
        rBias = replaceGraphBuilder.CreateInput(++inputIdx, "bias", inputDTypes[inputIdx], inputFormats[inputIdx], inputShapes[inputIdx].GetDims());
    }
    auto rX1Scale =
        replaceGraphBuilder.CreateInput(++inputIdx, "x1_scale", inputDTypes[inputIdx], inputFormats[inputIdx], inputShapes[inputIdx].GetDims());
    auto rX2Scale =
        replaceGraphBuilder.CreateInput(++inputIdx, "x2_scale", inputDTypes[inputIdx], inputFormats[inputIdx], inputShapes[inputIdx].GetDims());
    ...
    auto mc2 = ge::es::MatmulAlltoAll(rX1, rX2, rBias, rX1Scale, rX2Scale, nullptr /*rCommScale*/,
...
REG_FUSION_PASS(MatmulAllToAllTransposeA5FusionPass).Stage(ge::CustomPassStage::kCompatibleInherited); // 兼容性场景下指定stage
} // namespace ops