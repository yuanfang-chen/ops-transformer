...
static ge::graphStatus MatmulAlltoAllExecuteFunc(gert::OpExecuteContext* host_api_ctx)
{
    ...
    if (x1QuantMode == 0 && x2QuantMode == 0) {
        ...
        OPS_CHECK(
            ...
            return ge::GRAPH_FAILED);
        const auto ret = EXEC_OPAPI_CMD(aclnnQuantMatmulAlltoAll, matmulParas.x1Acl, matmulParas.x2Acl, matmulParas.bias, quant_matmul_para.x1ScaleAcl, quant_matmul_para.x2ScaleAcl,
