...
// setup & teardown
class MatmulAlltoAllArch35TilingTest : public testing::TestWithParam<MatmulAlltoAllTestParam> {
protected:
...
TEST_P(MatmulAlltoAllArch35TilingTest, MatmulAlltoAllGeneralCasesTest)
{
    TestOneParamCase(GetParam());
}

TEST_F(MatmulAlltoAllArch35TilingTest, MatmulAlltoAllGeneralCasesMultiThread)
{
    TestExecMultiThread(g_testCases, sizeof(g_testCases) / sizeof(MatmulAlltoAllTestParam), 1);
}

INSTANTIATE_TEST_CASE_P(MatmulAlltoAllTilingUT, MatmulAlltoAllArch35TilingTest, testing::ValuesIn(g_testCases));
