...
static ge::graphStatus CheckBiasShapeForMatmulAlltoAll(const gert::InferShapeContext* context, AlltoAllMatmulShapeInfo& shape)
{
...
static ge::graphStatus CheckXScaleShapeForMatmulAlltoAll(const gert::InferShapeContext* context, AlltoAllMatmulShapeInfo& shape)
{
    const auto attrs = context->GetAttrs();
    ...
    if (*x1QuantMode == X1_MXFP8_QUANT_NUM && *x2QuantMode == X2_MXFP8_QUANT_NUM) {
        ...
        OPS_CHECK(x2ShapeScaleDimNum != DIM_THREE, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                  "In MatmulAlltoAll, x2Scale dim num must be %ld, but actual value is: %ld", DIM_THREE, x2ShapeScaleDimNum), return ge::GRAPH_FAILED);
        // x2Scale最后一维一定是2
        ...
        OPS_CHECK(x2ShapeScaleDimNum != DIM_ONE, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                  "In MatmulAlltoAll, x2Scale shape must be %ld, but actual value is: %ld", DIM_ONE, x2ShapeScaleDimNum), return ge::GRAPH_FAILED);
    }
    // x2Scale第0维与n轴一致
    ...
    if (x1ShapeScale != nullptr) {
        ...
        if (*x1QuantMode == X1_MXFP8_QUANT_NUM && *x2QuantMode == X2_MXFP8_QUANT_NUM) {
            ...
            OPS_CHECK(x1ShapeScaleDimNum != DIM_THREE, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                      "In MatmulAlltoAll, x1Scale dim num must be %ld, but actual value is: %ld", DIM_THREE, x1ShapeScaleDimNum), return ge::GRAPH_FAILED);
            // x1Scale最后一维一定是2
            ...
            OPS_CHECK(x1ShapeScaleDimNum != DIM_ONE, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                      "In MatmulAlltoAll, x1Scale shape must be %ld, but actual value is: %ld", DIM_ONE, x1ShapeScaleDimNum), return ge::GRAPH_FAILED);
        }
        // x1Scale第0维与m轴一致
...
static ge::graphStatus CheckAxisKShapeForMatmulAlltoAll(const gert::InferShapeContext* context, AlltoAllMatmulShapeInfo& shape)
{
...
static ge::graphStatus InferShapeMatmulAlltoAll(gert::InferShapeContext* context)
{
    ...
    OPS_CHECK(
        ...
        return ge::GRAPH_FAILED);
    OPS_CHECK(
        GetMatmulAxisInfoFor(context, shape) != ge::GRAPH_SUCCESS,
