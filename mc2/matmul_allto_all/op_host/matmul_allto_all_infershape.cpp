...
{
    OP_LOGI(INNER_DEBUG, "Check x shape in matmul_all_to_all.");
    ...
    OPS_CHECK(x2Shape->GetDimNum() != DIM_TWO, CUBE_INNER_ERR_REPORT(INNER_DEBUG,
              "In MatmulAlltoAll, x2 shape should be %ld, but the actual value is %ld.", DIM_TWO, x2Shape->GetDimNum()),
    ...
    OPS_CHECK(x1Shape->GetDimNum() != DIM_TWO, CUBE_INNER_ERR_REPORT(INNER_DEBUG,
              "In MatmulAlltoAll, x1 shape should be %ld, but the actual value is %ld.", DIM_TWO, x1Shape->GetDimNum()),
              return ge::GRAPH_FAILED);
...
{
    OP_LOGI(INNER_DEBUG, "Check all_to_all_axes shape in matmul_all_to_all.");
    ...
    if (alltoAllAxesPtr != nullptr) {
        ...
        OPS_CHECK((alltoAllAxes[0] != NUM_MINUS_ONE || alltoAllAxes[1] != NUM_MINUS_TWO), CUBE_INNER_ERR_REPORT(INNER_DEBUG,
                  "In MatmulAlltoAll, the value of alltoAllAxes should be [-1, -2], but the actual value is [%ld, %ld].",
...
static ge::graphStatus CheckBiasShapeForMatmulAlltoAll(const gert::InferShapeContext* context, MatmulAlltoAllShapeInfo& shape)
{
    OP_LOGI(INNER_DEBUG, "Check bias shape in matmul_all_to_all.");
    ...
    if (biasShape != nullptr) {
        ...
        OPS_CHECK(biasShape->GetDim(0) != shape.n, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                  "In MatmulAlltoAll, bias dim0 must be the same with matmul axis n, but actual bias dim0 is: %ld, axis n is: %ld",
...
static ge::graphStatus CheckXScaleShapeForMatmulAlltoAll(const gert::InferShapeContext* context, MatmulAlltoAllShapeInfo& shape)
{
    OP_LOGI(INNER_DEBUG, "Check x_scale shape in matmul_all_to_all.");
    ...
    if (*x1QuantMode == X1_MXFP8_QUANT_NUM && *x2QuantMode == X2_MXFP8_QUANT_NUM) {
        ...
        OPS_CHECK(x2ShapeScaleDimNum != DIM_THREE, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                  "In MatmulAlltoAll, x2Scale dim num must be %ld, but actual value is: %ld", DIM_THREE, x2ShapeScaleDimNum),
                  return ge::GRAPH_FAILED);
        // x2Scale最后一维一定是2
        ...
        OPS_CHECK(x2ShapeScaleDimNum != DIM_ONE, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                  "In MatmulAlltoAll, x2Scale shape must be %ld, but actual value is: %ld", DIM_ONE, x2ShapeScaleDimNum),
                  return ge::GRAPH_FAILED);
    }
    // x2Scale第0维与n轴一致
    ...
    if (x1ShapeScale != nullptr) {
        ...
        if (*x1QuantMode == X1_MXFP8_QUANT_NUM && *x2QuantMode == X2_MXFP8_QUANT_NUM) {
            ...
            OPS_CHECK(x1ShapeScaleDimNum != DIM_THREE, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                      "In MatmulAlltoAll, x1Scale dim num must be %ld, but actual value is: %ld", DIM_THREE, x1ShapeScaleDimNum),
                      return ge::GRAPH_FAILED);
            // x1Scale最后一维一定是2
            ...
            OPS_CHECK(x1ShapeScaleDimNum != DIM_ONE, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                      "In MatmulAlltoAll, x1Scale shape must be %ld, but actual value is: %ld", DIM_ONE, x1ShapeScaleDimNum),
                      return ge::GRAPH_FAILED);
        }
        // x1Scale第0维与m轴一致
        OPS_CHECK(x1ShapeScale->GetDim(0) != shape.m, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                  "In MatmulAlltoAll, x1Scale dim0 must be the same with matmul axis m, but actual x1Scale dim0 is: %ld, axis m is: %ld",
                  x1ShapeScale->GetDim(0), shape.m), return ge::GRAPH_FAILED);
    }
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus CheckAxisKShapeForMatmulAlltoAll(const gert::InferShapeContext* context, MatmulAlltoAllShapeInfo& shape)
{
    OP_LOGI(INNER_DEBUG, "Check axis k shape in matmul_all_to_all.");
...
static ge::graphStatus GetMatmulAxisInfoForMatmulAlltoAll(const gert::InferShapeContext* context, MatmulAlltoAllShapeInfo& shape)
{
    const auto attrs = context->GetAttrs();
