...
void AlltoAllMM::SetCommTimeFactor()
{
    if (socVersion_ == SocVersion::SOC910_93) {
        OP_LOAD("AlltoAllMatmul, Current socVersion is SOC910_93.");
        // A3上的时间因子AlltoAll暂时定义为4
        commPerf_.ChangeCommTimeFactorByDivision(FOUR); // 4x time of factor
    } else {
        OP_LOAD("AlltoAllMatmul, Current socVersion is SOC950.");
