...
{
    const uint64_t rankDim2D = 2; // 卡数为2、4、8时单独处理
    const uint64_t rankDim4D = 4;
    const uint64_t rankDim8D = 8;
    if (socVersion_ == SocVersion::SOC910_93) {
        OP_LOGD("AlltoAllMatmul, Current socVersion is SOC910_93.");
        if (rankDim_ == rankDim2D) {
            commPerf_.ChangeCommTimeFactorByDivision(COMM_TIME_FACTOR_TWO);
        } else if (rankDim_ == rankDim4D) {
            commPerf_.ChangeCommTimeFactorByDivision(COMM_TIME_FACTOR_FOUR);
        } else if (rankDim_ == rankDim8D) {
            commPerf_.ChangeCommTimeFactorByDivision(COMM_TIME_FACTOR_EIGHT);
        } else {
