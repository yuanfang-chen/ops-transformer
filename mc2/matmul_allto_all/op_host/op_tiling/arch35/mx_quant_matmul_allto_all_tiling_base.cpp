...
{
    const gert::RuntimeAttrs *attrs = context_->GetAttrs();
    const bool *isTransX2 = attrs->GetAttr<bool>(indexSchema.x2Transpose);
...
ge::graphStatus MxQuantMatmulAllToAllTilingBase::CheckMxQuantScaleShapes(const gert::TilingContext *context, const char *opName)
{
    ...
    OP_TILING_CHECK((x1ScaleDim0 != shapeInfo.x1Dim0) ||
    ...
    if (TransX2Flag){ // Transposed Scenario
        OP_TILING_CHECK((x2ScaleDim0 != shapeInfo.x2Dim0) ||
        ...
        OP_TILING_CHECK((x2ScaleDim0 != x2Dim0DivMxFp8Size) ||
                            (x2ScaleDim1 != shapeInfo.x2Dim1) || (x2ScaleDim2 != EVEN_ALIGN),
