...
static ge::graphStatus CheckAttrPointersNotNull(const char* nodeName, const MoeDistributeCombineAttrPointers& attrPtrs) 
{
    ...
    OP_TILING_CHECK(attrPtrs.epWorldSizePtr == nullptr, OP_LOGE(nodeName, "epWorldSize is null."), 
                    return ge::GRAPH_FAILED);
    OP_TILING_CHECK(attrPtrs.tpWorldSizePtr == nullptr, OP_LOGE(nodeName, "tpWorldSize is null."), 
                    return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(attrPtrs.expertShardPtr == nullptr, OP_LOGE(nodeName, "expertShardType is null."), 
                    return ge::GRAPH_FAILED);
    OP_TILING_CHECK(attrPtrs.sharedExpertRankNumPtr == nullptr, OP_LOGE(nodeName, "sharedExpertRankNum is null."),
                    return ge::GRAPH_FAILED);
    OP_TILING_CHECK(attrPtrs.moeExpertNumPtr == nullptr, OP_LOGE(nodeName, "moeExpertNum is null."), 
                    return ge::GRAPH_FAILED);
    OP_TILING_CHECK(attrPtrs.sharedExpertNumPtr == nullptr, OP_LOGE(nodeName, "sharedExpertNum is null."),
                    return ge::GRAPH_FAILED);
    OP_TILING_CHECK(attrPtrs.commQuantModePtr == nullptr, OP_LOGE(nodeName, "commQuantMode is null."), 
                    return ge::GRAPH_FAILED);
    ...
    const MoeDistributeCombineAttrPointers& attrPtrs, std::string& groupTp) 
{
    ...
    if (*attrPtrs.tpWorldSizePtr > 1) {
        ...
        OP_TILING_CHECK(*attrPtrs.tpRankIdPtr != 0, OP_LOGE(nodeName, 
            "tpRankId is invalid, NoTp mode only support 0, but got tpRankId=%ld.", *attrPtrs.tpRankIdPtr), 
            return ge::GRAPH_FAILED);
    }
    OP_TILING_CHECK(*attrPtrs.expertShardPtr != 0, OP_LOGE(nodeName, 
        "expertShardType is invalid, only support 0, but got expertShardType=%ld.", *attrPtrs.expertShardPtr),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK((*attrPtrs.sharedExpertRankNumPtr < 0) || 
                    (*attrPtrs.sharedExpertRankNumPtr >= *attrPtrs.epWorldSizePtr), OP_LOGE(nodeName,
    ...
    OP_TILING_CHECK(*attrPtrs.sharedExpertNumPtr != 1, OP_LOGE(nodeName, 
        "sharedExpertNum only support 1, but got sharedExpertNum=%ld.", *attrPtrs.sharedExpertNumPtr), 
        return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK((*attrPtrs.commQuantModePtr != 0) && (*attrPtrs.commQuantModePtr != 2), OP_LOGE(nodeName, 
        "commQuantMode only support 0 or 2, but got commQuantMode=%ld.", *attrPtrs.commQuantModePtr),
        return ge::GRAPH_FAILED);
