...
static ge::graphStatus MoeDistributeCombineA2TilingCheckAndSet(gert::TilingContext *context,
...
static ge::graphStatus MoeDistributeCombineA2TilingFuncImpl(gert::TilingContext *context)
{
    ...
    OP_TILING_CHECK(MoeDistributeCombineA2TilingCheckAndSet(context, isLayered, commQuantMode) != ge::GRAPH_SUCCESS,
                    VECTOR_INNER_ERR_REPORT_TILING(nodeName, "MoeDistributeCombineA2 CheckAndSet Failed"),
                    return ge::GRAPH_FAILED);
...
static ge::graphStatus CheckAttrPointersNotNull(const char* nodeName, const MoeDistributeCombineAttrPointers& attrPtrs) 
{
...
static ge::graphStatus CheckAttrValuesValid(const char* nodeName, 
    const MoeDistributeCombineAttrPointers& attrPtrs, std::string& groupTp) 
{
    ...
    if (*attrPtrs.tpWorldSizePtr > 1) {
        ...
        OP_TILING_CHECK((*attrPtrs.commQuantModePtr != 0), OP_LOGE(nodeName,
                    "commQuantMode only supports 0 when tpWorldSize > 1, but got commQuantMode=%ld, tpWorldSize=%ld.",
                    *attrPtrs.commQuantModePtr, *attrPtrs.tpWorldSizePtr), return ge::GRAPH_FAILED);
        groupTp = std::string(attrPtrs.groupTpPtr);
    } else {
        OP_TILING_CHECK(*attrPtrs.tpRankIdPtr != 0, OP_LOGE(nodeName, 
            "tpRankId is invalid, NoTp mode only support 0, but got tpRankId=%ld.", *attrPtrs.tpRankIdPtr), return ge::GRAPH_FAILED);
    }
    OP_TILING_CHECK(*attrPtrs.expertShardPtr != 0, OP_LOGE(nodeName, 
        "expertShardType is invalid, only support 0, but got expertShardType=%ld.", *attrPtrs.expertShardPtr), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK((*attrPtrs.commQuantModePtr != 0) && (*attrPtrs.commQuantModePtr != 2), OP_LOGE(nodeName, 
        "commQuantMode only support 0 or 2, but got commQuantMode=%ld.", *attrPtrs.commQuantModePtr), return ge::GRAPH_FAILED);
...
static bool MoeDistributeCombineCheckExpertParams(gert::TilingContext *context, 
...
static bool MoeDistributeCombineCheckEpWorldSizeParams(gert::TilingContext *context, 
...
static bool MoeDistributeCombineCheckBatchParams(gert::TilingContext *context, 
    MoeDistributeCombineTilingData &tilingData, const char *nodeName) 
{
    uint32_t epWorldSize = tilingData.moeDistributeCombineInfo.epWorldSize;
...
{
    OP_TILING_CHECK(!MoeDistributeCombineCheckExpertParams(context, tilingData, nodeName, localMoeExpertNum),
                    OP_LOGE(nodeName, "Check expert params failed."), return false);

    OP_TILING_CHECK(!MoeDistributeCombineCheckEpWorldSizeParams(context, tilingData, nodeName),
                    OP_LOGE(nodeName, "Check arch params failed."), return false);

    OP_TILING_CHECK(!MoeDistributeCombineCheckBatchParams(context, tilingData, nodeName),
