...
static graphStatus MoeDistributeCombineExecuteFunc(OpExecuteContext* host_api_ctx)
{
  ...
  OP_CHECK_IF(shared_expert_num == nullptr, OP_LOGE(MoeDistributeCombineInfo,"shared_expert_num is null"), return ge::GRAPH_FAILED);
  const auto *shared_expert_rank_num = attrs->GetInt(static_cast<size_t>(9));
  OP_CHECK_IF(shared_expert_rank_num == nullptr, OP_LOGE(MoeDistributeCombineInfo,"shared_expert_rank_num is null"), return ge::GRAPH_FAILED);
