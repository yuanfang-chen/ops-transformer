...
// 获取可选输入
graphStatus MoeDistributeCombineGetOpOptionalInput(OpExecuteContext* hostApiCtx, OpOptionalInput &opOptionalInput)
{
    opOptionalInput.tpSendCounts = hostApiCtx->GetOptionalInputTensor(static_cast<size_t>(5));
    ...
    opOptionalInput.xActiveMask = hostApiCtx->GetOptionalInputTensor(static_cast<size_t>(6));
    opOptionalInput.activationScale = hostApiCtx->GetOptionalInputTensor(static_cast<size_t>(7));
    opOptionalInput.weightScale = hostApiCtx->GetOptionalInputTensor(static_cast<size_t>(8));
    opOptionalInput.groupList = hostApiCtx->GetOptionalInputTensor(static_cast<size_t>(9));
    opOptionalInput.expandScales = hostApiCtx->GetOptionalInputTensor(static_cast<size_t>(10));
    return ge::GRAPH_SUCCESS;
}

// 获取输出
graphStatus MoeDistributeCombineGetOpOutput(OpExecuteContext* hostApiCtx, OpOutput &opOutput)
{
    opOutput.y = hostApiCtx->GetOutputTensor(static_cast<size_t>(0));
    OP_CHECK_IF(opOutput.y == nullptr, OP_LOGE(moeDistributeCombineInfo,"y is null"), return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}

// 获取属性
graphStatus MoeDistributeCombineGetOpAttrs(OpExecuteContext* hostApiCtx, OpAttrs &opAttrs)
{
    const auto attrs = hostApiCtx->GetAttrs();
...
static graphStatus MoeDistributeCombineExecuteFunc(OpExecuteContext* hostApiCtx)
{
    OP_LOGD(moeDistributeCombineInfo, "start to fallback for moeDistributeCombine");
    OP_CHECK_IF(hostApiCtx == nullptr, OP_LOGE(moeDistributeCombineInfo,"hostApiCtx is null"), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(MoeDistributeCombineGetOpInput(hostApiCtx, opInput) != ge::GRAPH_SUCCESS,
                OP_LOGE(moeDistributeCombineInfo,"get input failed"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(MoeDistributeCombineGetOpOptionalInput(hostApiCtx, opOptionalInput) != ge::GRAPH_SUCCESS,
                OP_LOGE(moeDistributeCombineInfo,"get optional input failed"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(MoeDistributeCombineGetOpOutput(hostApiCtx, opOutput) != ge::GRAPH_SUCCESS,
                OP_LOGE(moeDistributeCombineInfo,"get output failed"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(MoeDistributeCombineGetOpAttrs(hostApiCtx, opAttrs) != ge::GRAPH_SUCCESS,
