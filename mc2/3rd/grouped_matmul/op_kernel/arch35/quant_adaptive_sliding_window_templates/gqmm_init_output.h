 ...
 * \brief
 */
#ifndef MC2_GQMM_INIT_OUTPUT_H
#define MC2_GQMM_INIT_OUTPUT_H
                                       ...
                                       TILING_TYPE *gmmArrayAddrIn, int32_t usedCoreNum, TPipe *pipe)
{
    ...
    if (groupListPtr == nullptr || gmmQuantParams->groupType != Mc2QuantUtils::SPLIT_K) {
        return;
    }
    ...
    yGm.SetGlobalBuffer(MC2_GROUPED_MATMUL::GetTensorAddr<T>(0, y));
    groupListGm.SetGlobalBuffer((__gm__ int64_t*)groupListPtr);
    if (GetSubBlockIdx() == 0) {
        pipe->InitBuffer(initBuff, Mc2QuantUtils::MAX_REPEAT_TIMES * Mc2QuantUtils::UB_ALIGN_SIZE);
    ...
    TILING_TYPE *kList = gmmArrayAddrIn + MC2_GROUPED_MATMUL::MKN_LIST_LEN;
    TILING_TYPE *nList = gmmArrayAddrIn + MC2_GROUPED_MATMUL::MKN_LIST_LEN * 2; // 2: mListGm_ + kListGm_

    bool isKZeroInit = false;
    for (uint32_t groupIdx = 0; groupIdx < gmmQuantParams->groupNum; ++groupIdx) {
        int32_t splitValue = Mc2QuantUtils::GetSplitValueFromGroupList(groupIdx, preOffset, gmmQuantParams->groupType,
        ...
        if (kSize == 0) {
            Mc2QuantUtils::InitOutputWithZero(yGm[yBaseOffset], initLocal, ySize, usedCoreNum, isKZeroInit);
        }
        yBaseOffset += ySize;
    }
}
