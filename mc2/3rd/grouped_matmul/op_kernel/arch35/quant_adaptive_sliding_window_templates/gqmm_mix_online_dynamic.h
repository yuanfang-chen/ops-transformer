 ...
 * \brief
 */
#ifndef MC2_GQMM_MIX_ONLINE_DYNAMIC_H
#define MC2_GQMM_MIX_ONLINE_DYNAMIC_H
...
public:
    ...
    MatmulImpl<aT, bT, cT, biasT, Mc2QuantUtils::MM_CFG_NO_PRELOAD_OPEN_UNIT_FLAG,
...
protected:
    ...
    __aicore__ inline void VFDoDequantWithoutAScale(__ubuf__ yType *dequantOutInUbAddr, __ubuf__ l0cType *l0cOutUbAddr,
                                                    uint16_t mSize);
    template <bool isPertensor, Mc2QuantUtils::QuantMode aQuantMode, bool isBiasEpilogue>
    ...
    {
        CrossCoreSetFlag<Mc2QuantUtils::SYNC_AIC_AIV_MODE, PIPE_V>(Mc2QuantUtils::AIV_SYNC_AIC_FLAG);
    }
    __aicore__ inline void WaitForVector()
    {
        CrossCoreWaitFlag<Mc2QuantUtils::SYNC_AIC_AIV_MODE, PIPE_FIX>(Mc2QuantUtils::AIV_SYNC_AIC_FLAG);
        CrossCoreWaitFlag<Mc2QuantUtils::SYNC_AIC_AIV_MODE, PIPE_FIX>(Mc2QuantUtils::AIV_SYNC_AIC_FLAG +
                                                                   Mc2QuantUtils::FLAG_ID_MAX);
    }
    __aicore__ inline void NotifyVector()
    {
        CrossCoreSetFlag<Mc2QuantUtils::SYNC_AIC_AIV_MODE, PIPE_FIX>(Mc2QuantUtils::AIC_SYNC_AIV_FLAG);
        CrossCoreSetFlag<Mc2QuantUtils::SYNC_AIC_AIV_MODE, PIPE_FIX>(Mc2QuantUtils::AIC_SYNC_AIV_FLAG +
                                                                  Mc2QuantUtils::FLAG_ID_MAX);
    }
    __aicore__ inline void WaitForCube()
    {
        CrossCoreWaitFlag<Mc2QuantUtils::SYNC_AIC_AIV_MODE, PIPE_V>(Mc2QuantUtils::AIC_SYNC_AIV_FLAG);
    }
    ...
    TILING_TYPE *gmmArrayAddrIn, TPipe *pipe)
{
    ...
    uint32_t mForSingleVec = Mc2QuantUtils::CeilDiv(mmTilingData_->baseM, GetTaskRation());
    pipe->InitBuffer(vecQueMMRes_, 1, mForSingleVec * mmTilingData_->baseN * sizeof(float)); // int32/fp32
    if (gmmQuantParams_->bQuantMode == static_cast<uint32_t>(Mc2QuantUtils::QuantMode::PERCHANNEL_MODE)) {
        pipe->InitBuffer(vecQueScale_, 1, mmTilingData_->baseN * sizeof(scaleType));
    }
    if (gmmQuantParams_->aQuantMode == static_cast<uint32_t>(Mc2QuantUtils::QuantMode::PERTOKEN_MODE)) {
        pipe->InitBuffer(vecQuePertokenScale_, 1,
                          Mc2QuantUtils::Align(mForSingleVec * sizeof(ptScaleType), Mc2QuantUtils::UB_ALIGN_SIZE));
    }
    isBiasEpilogue_ = Mc2QuantUtils::IsBiasEpilogue<xType, biasType>() && gmmQuantParams_->hasBias == 1;
    if (isBiasEpilogue_) {
        pipe->InitBuffer(vecQueBias_, 1, mmTilingData_->baseN * sizeof(biasType));
    }

    // fp16/bf16分两次输出，fp32分四次输出
    pipe->InitBuffer(vecQueOut_, Mc2QuantUtils::BUFFER_SWITCH,
                      Mc2QuantUtils::CeilDiv(mForSingleVec, Mc2QuantUtils::FP32_OUTPUT_TIMES) * mmTilingData_->baseN *
                          sizeof(yType));
    ...
    if ASCEND_IS_AIV {
        if (subBlockIdx_ == 0) {
            pipe->InitBuffer(initBuff_, Mc2QuantUtils::MAX_REPEAT_TIMES * Mc2QuantUtils::UB_ALIGN_SIZE);
...
__aicore__ inline void GQmmMixRegbaseKernel<LOCAL_TEMPLATE_FUNC_MIX_PARAMS>::UpdateMMGlobalAddr(uint32_t groupIdx)
{
    ...
    if (gmmQuantParams_->bQuantMode == static_cast<uint32_t>(Mc2QuantUtils::QuantMode::PERTENSOR_MODE)) {
        scaleType scaleBValue =
            *((__gm__ scaleType *)(MC2_GROUPED_MATMUL::GetTensorAddr<scaleType>(0, scaleTensorPtr_) + groupIdx));
        ...
        scaleBGlobal_.SetGlobalBuffer(MC2_GROUPED_MATMUL::GetTensorAddr<scaleType>(0, scaleTensorPtr_) +
...
__aicore__ inline void GQmmMixRegbaseKernel<LOCAL_TEMPLATE_FUNC_MIX_PARAMS>::Process()
{
    ...
    for (uint32_t groupIdx = 0; groupIdx < groupNum_; ++groupIdx) {
        ...
        if (kSize <= 0) {
            if (groupType_ == Mc2QuantUtils::SPLIT_K) { // k轴分组时，有(m,n)需要输出。int8输入无K轴分组。K轴分组无bias全0
                GlobalTensor<yType> yInitGlobal;
                yInitGlobal.SetGlobalBuffer(MC2_GROUPED_MATMUL::GetTensorAddr<yType>(0, yTensorPtr_) +
                                            block_.params_.cGroupAddrOffset);
                Mc2QuantUtils::InitOutputWithZero(yInitGlobal, initLocal_, static_cast<uint64_t>(mSize) * nSize,
...
{
    auto halfSingleM = Mc2QuantUtils::CeilDiv(block_.params_.singleCoreM, static_cast<uint64_t>(2)); // 分配给2个AIV计算
    ...
    auto mSizeForOnce = Mc2QuantUtils::CeilDiv(singleMInVec, static_cast<uint64_t>(splitNumOfOut));
    for (uint16_t i = 0; i < splitNumOfOut; i++) {
        // do dequant in vector
        uint64_t offsetL0c = i * mSizeForOnce *
                             Mc2QuantUtils::Align(block_.params_.singleCoreN,
                                               static_cast<uint64_t>(Mc2QuantUtils::UB_ALIGN_SIZE / sizeof(l0cType)));
        ...
        switch (gmmQuantParams_->aQuantMode) {
            case (static_cast<uint32_t>(Mc2QuantUtils::QuantMode::PERTOKEN_MODE)):
                {
                    uint64_t offsetPtScale = i * mSizeForOnce;
                    VFDoDequantWithAPertoken(dequantOutInUbAddr, l0cOutUbAddr, offsetPtScale, mSize);
                    break;
                }
            case (static_cast<uint32_t>(Mc2QuantUtils::QuantMode::PERTENSOR_MODE)):
                {
    ...
    __ubuf__ yType *dequantOutInUbAddr, __ubuf__ l0cType *l0cOutUbAddr, uint64_t offsetPtScale, uint16_t mSize)
{
    ...
    if (!isBiasEpilogue_) {
        if (gmmQuantParams_->bQuantMode == static_cast<uint32_t>(Mc2QuantUtils::QuantMode::PERTENSOR_MODE)) {
            VFDoDequant<true, Mc2QuantUtils::QuantMode::PERTOKEN_MODE, false>(
                dequantOutInUbAddr, l0cOutUbAddr, nullptr, ptScaleUbAddr, nullptr, mSize, block_.params_.singleCoreN);
        } else {
            VFDoDequant<false, Mc2QuantUtils::QuantMode::PERTOKEN_MODE, false>(
        ...
        if (gmmQuantParams_->bQuantMode == static_cast<uint32_t>(Mc2QuantUtils::QuantMode::PERTENSOR_MODE)) {
            VFDoDequant<true, Mc2QuantUtils::QuantMode::PERTOKEN_MODE, true>(
                dequantOutInUbAddr, l0cOutUbAddr, nullptr, ptScaleUbAddr, (__ubuf__ biasType *)biasUb_.GetPhyAddr(),
                mSize, block_.params_.singleCoreN);
        } else {
            VFDoDequant<false, Mc2QuantUtils::QuantMode::PERTOKEN_MODE, true>(
...
{
    VFDoDequant<false, Mc2QuantUtils::QuantMode::PERTENSOR_MODE, false>(
...
{
    if (!isBiasEpilogue_) {
        VFDoDequant<false, Mc2QuantUtils::QuantMode::DEFAULT, false>(dequantOutInUbAddr, l0cOutUbAddr,
                                                                  (__ubuf__ scaleType *)scaleUb_.GetPhyAddr(), nullptr,
                                                                  nullptr, mSize, block_.params_.singleCoreN);
    } else {
        if (gmmQuantParams_->bQuantMode == static_cast<uint32_t>(Mc2QuantUtils::QuantMode::PERTENSOR_MODE)) {
            VFDoDequant<true, Mc2QuantUtils::QuantMode::DEFAULT, true>(dequantOutInUbAddr, l0cOutUbAddr, nullptr, nullptr,
                                                                    (__ubuf__ biasType *)biasUb_.GetPhyAddr(), mSize,
                                                                    block_.params_.singleCoreN);
        } else {
            VFDoDequant<false, Mc2QuantUtils::QuantMode::DEFAULT, true>(
...
{
    uint32_t eleNumPerVf = Mc2QuantUtils::GetVRegSize() / sizeof(l0cType);
    uint32_t nSrcUbAligned =
        Mc2QuantUtils::Align(nSize, static_cast<uint16_t>(Mc2QuantUtils::UB_ALIGN_SIZE / sizeof(l0cType)));
    uint32_t nDstUbAligned = Mc2QuantUtils::Align(nSize, static_cast<uint16_t>(Mc2QuantUtils::UB_ALIGN_SIZE / sizeof(yType)));
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t mIdx = 0; mIdx < mSize; mIdx++) {
            ...
            for (uint16_t vfBlockIdx = 0; vfBlockIdx < nLoopCnt; vfBlockIdx++) {
                ...
                // out * perTokenScale
                if constexpr (aQuantMode == Mc2QuantUtils::QuantMode::PERTENSOR_MODE) {
                    AscendC::MicroAPI::Muls(mulPtScaleOutReg, mulScaleOutReg, perTokenScaleScalar_, maskN);
                } else if constexpr (aQuantMode == Mc2QuantUtils::QuantMode::PERTOKEN_MODE) {
...
#endif // MC2_GQMM_MIX_ONLINE_DYNAMIC_H