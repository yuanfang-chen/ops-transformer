...
ge::graphStatus MC2GMMTiling::CheckWeightNZShape(const gert::TilingContext* context, int64_t numInOneBlk) const {
...
ge::graphStatus MC2GMMTiling::CheckMKN(const gert::TilingContext* context) {
...
void MC2GMMTiling::SetTilingDataIsSingleTensor() {
...
ge::graphStatus MC2GMMTiling::PrepareTilingData(const gert::TilingContext* context) {
...
ge::graphStatus MC2GMMTiling::GMMGetTensorShapeSplitM(const gert::TilingContext* context, const gert::Shape &xShape,
...
ge::graphStatus MC2GMMTiling::GMMGetTensorShapeSplitK(const gert::TilingContext* context, const gert::Shape &xShape,
    const gert::Shape &wShape) {
...
*/
ge::graphStatus MC2GMMTiling::SplitMSingleXSingleWeightSingleY(const gert::Shape &xShape, const gert::Shape &wShape) {
...
*/
ge::graphStatus MC2GMMTiling::SplitMSingleXSeparatedWeight(const gert::TilingContext* context, const gert::Shape &xShape) {
  int64_t m = GMMGetBS(xShape);
  int64_t k = xShape.GetDim(xKDim_);
...
*/
ge::graphStatus MC2GMMTiling::SeparatedXSeparatedWeight(const gert::TilingContext* context) {
  for (uint32_t i = 0; i < MAX_TENSOR_CONT; i++) {
    auto wTensor = context->GetDynamicInputTensor(WEIGHT_INDEX, i);
...
*/
ge::graphStatus MC2GMMTiling::SeparatedXSingleWeight(const gert::TilingContext* context, const gert::Shape &wShape) {
  int64_t n = wShape.GetDim(weightNDim_) * nzFactor_;
  for (uint32_t i = 0; i < MAX_TENSOR_CONT; i++) {
    auto xTensor = context->GetDynamicInputTensor(X_INDEX, i);
...
*/
ge::graphStatus MC2GMMTiling::SplitKSingleXSingleWeightSingleY(const gert::TilingContext* context,
...
ge::graphStatus MC2GMMTiling::SplitKSingleXSeparatedWeight(const gert::TilingContext* context,
  const gert::Shape &xShape, const gert::Shape &wShape) {
  int64_t m = GMMGetBS(xShape);
  int64_t k = xShape.GetDim(xKDim_);
  int64_t n = wShape.GetDim(weightNDim_) * nzFactor_;
...
ge::graphStatus MC2GMMTiling::Init(const gert::TilingContext* context) {
...
ge::graphStatus MC2GMMTiling::GetPerGroupNum(const gert::TilingContext* context) {
...
void MC2GMMTiling::DivideUbAndSetWorkspaceAntiquant(size_t* workspaces, const uint32_t& aicNum, uint32_t &ubSize) {
    if (isA16W8Msd_) {
...
int32_t MC2GMMTiling::FindBestSingleN(const uint32_t& aicNum) {
...
bool MC2GMMTiling::TryFullLoadA(int32_t baseM,const GMMCompileInfo *compileInfoPtr) {
...
*/
ge::graphStatus MC2GMMTiling::IsOutputDisableL2Cache(gert::TilingContext* context, const GMMCompileInfo *compileInfoPtr) {
...
ge::graphStatus MC2GMMTiling::DynamicTilingSingleN(gert::TilingContext* context, const uint32_t& aicNum, const GMMCompileInfo *compileInfoPtr) {
  OP_CHECK_IF(compileInfoPtr == nullptr, OPS_REPORT_CUBE_INNER_ERR(
               context->GetNodeName(), "compileInfoPtr is nullptr."), return ge::GRAPH_FAILED);
...
ge::graphStatus MC2GMMTiling::DivideUbAndSetWorkspace(gert::TilingContext* context, const uint32_t& aicNum) {
...
int32_t MC2GMMTiling::FindBestSingleNPertoken(const uint32_t aicNum) const {
...
void MC2GMMTiling::FindBestUsedCoreNumOneGroup(const uint32_t aicNum) {
  if (groupNum_ > 1U) {
    return;
  }
...
ge::graphStatus MC2GMMTiling::SetWorkspscesPerTokenQuant(const uint32_t aicNum, size_t* workspaces) {
...
bool MC2GMMTiling::StaticTilingProcess(gert::TilingContext *context) {
  // cond.1 A8W8
...
bool MC2GMMTiling::CheckTilingMatchStaticValue() {
...
ge::graphStatus MC2GMMTiling::RunFusionKernelTiling(gert::TilingContext* context) {
...
void MC2GMMTiling::PrintTilingInfo(gert::TilingContext *context) {
...
ge::graphStatus MC2GMMTiling::GMMCalUbSize(const gert::TilingContext* context, uint32_t ubSize) {
...
int64_t MC2GMMTiling::GMMGetBS(const gert::Shape &xShape) const {
...
uint32_t MC2GMMTiling::GetTplDataType(const ge::DataType &dtype) {
...
bool MC2GMMTiling::IsAivAicRatioTwoRequired() {
...
bool MC2GMMTiling::IsFixedAxisMoveCondition() {
...
bool MC2GMMTiling::IsIntDataType() {
    return yDtype_ == ge::DT_INT8 || yDtype_ == ge::DT_INT32;
}

void MC2GMMTiling::GMMSetTplTilingKey(gert::TilingContext* context) {
...
ge::graphStatus MC2GMMTiling::GMMGetAttrs(const gert::TilingContext* context) {
...
ge::graphStatus MC2GMMTiling::GMMSetUbDivideBlkAntiquant() {
  if (isA16W8Msd_) {
...
ge::graphStatus MC2GMMTiling::GMMSetUbDivideBlkQuant() {
...
ge::graphStatus MC2GMMTiling::GMMSetUbDivideBlkA4W4() {
...
ge::graphStatus MC2GMMTiling::GMMSetUbDivideBlk() {
...
ge::graphStatus MC2GMMTiling::SetBias(const gert::TilingContext* context, matmul_tiling::MultiCoreMatmulTiling& mm) const {
...
void MC2GMMTiling::FullLoadK(const GMMCompileInfo* compileInfoPtr) {
...
ge::graphStatus MC2GMMTiling::CalcStepKaKb(const gert::TilingContext* context, const GMMCompileInfo* compileInfoPtr,
...
ge::graphStatus MC2GMMTiling::GMMSetMMTiling(const gert::TilingContext* context, const GMMCompileInfo* compileInfoPtr) {
...
void MC2GMMTiling::SetMMPreTiling() {
...
ge::graphStatus MC2GMMTiling::CalMMTiling(const gert::TilingContext* context, const GMMCompileInfo* compileInfoPtr) {
...
ge::graphStatus MC2GMMTiling::CheckA16W4MsdEnable(uint64_t mSize, uint64_t antiquantGroupNum,
...
uint64_t MC2GMMTiling::GetWithOffset(const gert::TilingContext *context)
{
...
ge::graphStatus MC2GMMTiling::A16W4MsdTiling(gert::TilingContext *context, const GMMCompileInfo *compileInfoPtr)
{
...
ge::graphStatus MC2GMMTiling::A8W4Tiling(gert::TilingContext* context, const GMMCompileInfo* compileInfoPtr) {
      auto yDesc = context->GetOutputDesc(Y_INDEX);
      OP_CHECK_NULL_WITH_CONTEXT(context, yDesc);
