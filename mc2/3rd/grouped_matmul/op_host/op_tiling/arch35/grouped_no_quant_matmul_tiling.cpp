...
{
    uint64_t mCnt = Mc2GroupedMatmul::CeilDiv(m_, baseM_);
    uint64_t nCnt = Mc2GroupedMatmul::CeilDiv(n_, baseN_);
    if (mCnt * nCnt >= remainCoreNum) {
        return;
    }
    if (mCnt <= nCnt) {
        baseM_ = Mc2GroupedMatmul::CeilAlign(Mc2GroupedMatmul::CeilDiv(m_, mCnt), BASIC_BLOCK_SIZE_16);
        mCnt = Mc2GroupedMatmul::CeilDiv(m_, baseM_);
        nCnt = remainCoreNum / mCnt;
        baseN_ = Mc2GroupedMatmul::CeilAlign(Mc2GroupedMatmul::CeilDiv(n_, nCnt), BASIC_BLOCK_SIZE_16);
    } else {
        baseN_ = Mc2GroupedMatmul::CeilAlign(Mc2GroupedMatmul::CeilDiv(n_, nCnt), BASIC_BLOCK_SIZE_16);
        nCnt = Mc2GroupedMatmul::CeilDiv(n_, baseN_);
    ...
    mCnt = Mc2GroupedMatmul::CeilDiv(m_, baseM_);
    nCnt = Mc2GroupedMatmul::CeilDiv(n_, baseN_);
    uint64_t kValueAlign = Mc2GroupedMatmul::CeilAlign(k_, BASIC_BLOCK_SIZE_16);
    uint64_t kValueMax =
        Mc2GroupedMatmul::FloorAlign(static_cast<uint64_t>(compileInfoPtr->l0ASize / DB_SIZE / GetSizeByDataType(xDType_) /
...
{
    uint64_t totalL1Size = compileInfoPtr->l1Size + 256UL; // 256Brpc
