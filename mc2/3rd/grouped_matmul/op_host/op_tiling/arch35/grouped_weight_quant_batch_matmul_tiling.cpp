...
bool GroupedWeightQuantBatchMatmulTiling::CheckTensorListSize(const gert::TilingContext *context)
{
    ...
    OP_CHECK_IF(
        numX_ >= Mc2GroupedMatmul::MAX_TENSOR_CONT,
        OP_LOGE(context->GetNodeName(),
    ...
    if (groupType_ == GroupType::SPLIT_M) {
        OP_CHECK_IF(groupListType_ != 0 && groupListType_ != 1,
                    OP_LOGE(context->GetNodeName(),
                            ...
                            groupListType_),
                    return false);
        auto groupListPtr = context->GetOptionalInputShape(Mc2GroupedMatmul::GROUPLIST_INDEX);
...
{
    uint16_t count = 0;
    for (int i = 0; i <= Mc2GroupedMatmul::MAX_TENSOR_CONT; i++) {
...
{
    for (uint16_t i = 0; i < Mc2GroupedMatmul::MAX_TENSOR_CONT; i++) {
        ...
        OP_CHECK_IF(xDimNum < MIN_X_DIM || xDimNum > MAX_X_DIM,
                    ...
                    return false);
        OP_CHECK_IF(wDimNum != Mc2GroupedMatmul::MIN_ND_DIM,
...
void GroupedWeightQuantBatchMatmulTiling::CalcNoFullBlockDimResplitTiling(uint64_t c0Size)
{
    ...
    uint64_t mainBlkCount = Mc2GroupedMatmul::CeilDiv(nSize_, BASIC_BLOCK_BASE_N);
    if (groupNum_ * mainBlkCount >= coreNum_) {
        // 按照BASIC_BLOCK_BASE_N分核，但是可以在group方向凑多个基本块，则按照mainBlkCount做均匀分核
        resplitParam_.firstTailBlockSize = Mc2GroupedMatmul::CeilAlign(Mc2GroupedMatmul::CeilDiv(nSize_, mainBlkCount), c0Size);
