...
bool CheckStreamKSKTilingNpuArch3510(const Mc2MatmulV3CompileInfo &compileInfo, const Mc2MatMulV3Args &args)
{
...
const static std::map<NpuArch, CheckStreamKSKTilingFunc> CheckStreamKSKTilingFuncMap = {
    {NpuArch::DAV_3510, CheckStreamKSKTilingNpuArch3510},
};
...
bool CheckStreamKDPSKTilingNpuArch3510(const Mc2MatmulV3CompileInfo &compileInfo, const Mc2MatMulV3Args &args)
{
    constexpr uint64_t STREAM_K_MIN_K_THRESHOLD = 8192UL;
    // 如果k轴小于32*256/DtypeSize_ 或 mn轴不是256对齐 或 输入是fp32类型，不走stream-k-dpsk
    if ((args.mValue % BASIC_BLOCK_SIZE_256 != 0UL) || 
        (args.nValue % BASIC_BLOCK_SIZE_256 != 0UL) ||
        (args.kValue < std::max(STREAM_K_MIN_K_THRESHOLD, compileInfo.aicNum * BASIC_BLOCK_K_128_BYTE) / 
        args.aDtypeSize) ||
...
const static std::map<NpuArch, CheckStreamKDPSKTilingFunc> CheckStreamKDPSKTilingFuncMap = {
    {NpuArch::DAV_3510, CheckStreamKDPSKTilingNpuArch3510},
};
...
Mc2MatMulV3L0C2Out GetL0C2OutFlagnpuArch3510(const Mc2MatMulV3Args &args)
{
...
const static std::map<NpuArch, GetL0C2OutFlagFunc> GetL0C2OutFlagFuncMap = {
    {NpuArch::DAV_3510, GetL0C2OutFlagnpuArch3510},
};
