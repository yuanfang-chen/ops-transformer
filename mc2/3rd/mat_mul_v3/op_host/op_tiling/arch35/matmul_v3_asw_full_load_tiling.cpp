...
bool Mc2MatMulV3AswFullLoadTiling::CheckBL1FullLoad(bool& isKFullLoad)
{
    ...
    auto iter = (CheckBL1FullLoadMap.find(compileInfo_.npuArch) == CheckBL1FullLoadMap.end() ||
                 args_.bFormat == ge::FORMAT_FRACTAL_NZ) ?
                    CheckBL1FullLoadDefault(isKFullLoad, kAlignedValue, nAlignedValue) :
                    (this->*CheckBL1FullLoadMap.at(compileInfo_.npuArch))(isKFullLoad, kAlignedValue, nAlignedValue);
    return iter;
}
