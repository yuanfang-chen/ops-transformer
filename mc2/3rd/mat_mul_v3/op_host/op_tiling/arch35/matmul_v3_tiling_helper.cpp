                                        ...
                                        const Mc2MatMulV3Args &args, Mc2MatMulV3RunInfo &runInfo)
{
    ...
    auto socversion = compileInfo.socVersion;
...
void Mc2MatMulV3TilingHelper::CalL1Tiling(const gert::TilingContext* context, const Mc2MatMulV3Args &args,
                                       Mc2MatMulV3RunInfo &runInfo)
{
    auto platformInfo = context->GetPlatformInfo();
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(platformInfo);
    NpuArch npuArchHelper = ascendcPlatform.GetCurNpuArch();
    auto iter = (CalL1TilingFuncMap.find(npuArchHelper) == CalL1TilingFuncMap.end())
                    ? CalL1TilingDefault
                    : CalL1TilingFuncMap.at(npuArchHelper);
    iter(compileInfo, args, runInfo);
}
