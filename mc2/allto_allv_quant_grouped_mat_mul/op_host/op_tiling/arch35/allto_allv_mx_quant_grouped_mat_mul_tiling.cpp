...
ge::graphStatus AlltoAllvMXQuantGmmTiling::CheckShareExpScaleShape() const
{
    ...
    size_t mmXScaleDumNum = context_->GetOptionalInputShape(MM_X_SCALE_INDEX)->GetStorageShape().GetDimNum();
    OP_TILING_CHECK(mmXScaleDumNum != context_->GetOptionalInputShape(GMM_X_SCALE_INDEX)->GetStorageShape().GetDimNum(), 
        OP_LOGE(context_->GetNodeName(), "When mx quant mode, mmXScaleDimNum input dimNum should be same as gmmXScale(3), but actual dimNum is %lu.", \
            mmXScaleDumNum), return ge::GRAPH_FAILED);
...
ge::graphStatus AlltoAllvMXQuantGmmTiling::GetPermuteScaleOutSize() const
{
    if (!permuteOutFlag_) {
        return;
    }
    uint64_t h1 = Ops::Base::CeilDiv(h1_, MX_BASIC_FACTOR);
    permuteScaleOutSize_ =  Ops::Base::CeilAlign((a_ * h1 * 2 * GetSizeByDataType(gmmWeightDataType_)), BASIC_BLOCK_SIZE_512);
}
