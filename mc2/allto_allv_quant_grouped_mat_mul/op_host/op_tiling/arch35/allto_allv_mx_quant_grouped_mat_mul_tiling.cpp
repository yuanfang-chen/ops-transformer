...
ge::graphStatus AlltoAllvMXQuantGmmTiling::CheckScaleShape() const
{
    ...
    OP_TILING_CHECK(gxSDimNum != DIM_THREE, OP_LOGE(context_->GetNodeName(), "When mx quant mode, "
                "gmmXScale input shape should be [3], but actual is %lu", gxSDimNum), 
                return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(gwtSDimNum != DIM_FOUR, OP_LOGE(context_->GetNodeName(), "When mx quant mode, "
                "gmmWeightScale input shape should be [4], but actual is %lu", gwtSDimNum), 
                return ge::GRAPH_FAILED);
...
ge::graphStatus AlltoAllvMXQuantGmmTiling::CheckShareExpScaleShape() const
{
    ...
    OP_TILING_CHECK(xSDimNum != context_->GetOptionalInputShape(GMM_X_SCALE_INDEX)->GetStorageShape().GetDimNum(), 
        OP_LOGE(context_->GetNodeName(), "When mx quant mode and has shared expert, xSDimNum input dimNum "
                "should be 3, but actual dimNum is %lu.", xSDimNum), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(wtSDimNum != DIM_THREE, 
        OP_LOGE(context_->GetNodeName(), "When mx quant mode and has shared expert, wtSDimNum input dimNum "
                "should be 3, but actual is %lu.", wtSDimNum), return ge::GRAPH_FAILED);
    ...
    if (transMmWeight_) {
        ...
        OP_TILING_CHECK((wtSDimN != n2_) || (wtSDimH != expectH) || (wtSDim2 != 2), 
                        ...
                        "is (%lu, %lu, 2)", expectH, n2_, wtSDimH, wtSDimN, wtSDim2), 
                        return ge::GRAPH_FAILED);
    }
