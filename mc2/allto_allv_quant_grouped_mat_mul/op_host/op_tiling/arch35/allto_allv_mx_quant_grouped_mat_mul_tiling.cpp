...
ge::graphStatus AlltoAllvMXQuantGmmTiling::CheckQuantMode() const
{
    ...
    if (hasSharedExpertFlag_) {
        ...
        OP_TILING_CHECK(mmWeightQuantMode != gmmWeightQuantMode,
            OP_LOGE(context_->GetNodeName(), "When mx quant mode, mmWeightQuantMode should be same as gmmWeightQuantMode(6), "
            "but actual is %ld.", mmWeightQuantMode), return ge::GRAPH_FAILED);
    }
...
ge::graphStatus AlltoAllvMXQuantGmmTiling::CheckScaleShape() const
{
    ...
    OP_TILING_CHECK((gmmXScaleBSKDim != bsk_) || (gmmXScaleHDim != expectedGHValue), 
        ...
        "(%lu, %lu, 2) but the actual is (%lu, %lu, 2)", 
        bsk_, expectedGHValue, gmmXScaleBSKDim, gmmXScaleHDim), return ge::GRAPH_FAILED);    
    ...
    if (transGmmWeight_) {
        OP_TILING_CHECK((gmmWeightScaleNDim != n1_) || (gmmWeightScaleHDim != expectedGHValue), 
            ...
            "shape of gmmWeightscale is (%lu, %lu, %lu, 2) but the actual is (%lu, %lu, %lu, 2)", 
            e_, n1_, expectedGHValue, e_, gmmWeightScaleNDim, gmmWeightScaleHDim), return ge::GRAPH_FAILED);
    } else {
        OP_TILING_CHECK((gmmWeightScaleNDim != n1_) || (gmmWeightScaleHDim != expectedGHValue), 
            OP_LOGE(context_->GetNodeName(), "When mx quant mode and not trans gmmWeight, the expected "
            "shape of gmmWeightscale is (%lu, %lu, %lu, 2) but the actual is (%lu, %lu, %lu, 2)", 
            e_, expectedGHValue, n1_, e_, gmmWeightScaleHDim, gmmWeightScaleNDim), return ge::GRAPH_FAILED);
    }
    if (hasSharedExpertFlag_) {
...
ge::graphStatus AlltoAllvMXQuantGmmTiling::CheckShareExpScaleShape() const
{
    ...
    OP_TILING_CHECK((mmXScaleBSDim != bs_) || (mmXScaleHDim != expectedHValue), 
        ...
        "of mmxscale is (%lu, %lu, 2) but the actual is (%lu, %lu, 2)", 
        bs_, expectedHValue, mmXScaleBSDim, mmXScaleHDim), return ge::GRAPH_FAILED);  
    ...
    if (transMmWeight_) {
        OP_TILING_CHECK((mmWeightScaleNDim != n2_) || (mmWeightScaleHDim != expectedHValue), 
            ...
            n2_, expectedHValue, mmWeightScaleNDim, mmWeightScaleHDim), return ge::GRAPH_FAILED);
    } else {
        OP_TILING_CHECK((mmWeightScaleNDim != n2_) || (mmWeightScaleHDim != expectedHValue), 
