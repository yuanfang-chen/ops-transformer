...
void AlltoAllvMXQuantGmmTiling::SetTilingParams(Mc2GroupedMatmulTilingData::GMMQuantTilingData &gmmQuantTilingData, uint64_t M, uint64_t N, uint64_t K) const
{
    ...
    int32_t tempM = std::min(static_cast<int32_t>(M), static_cast<int32_t>(BASIC_BLOCK_SIZE_256));
    mm.baseM = Ops::Base::CeilAlign(tempM, static_cast<int32_t>(CUBE_BLOCK));
    int32_t tempN = std::min(static_cast<int32_t>(N), static_cast<int32_t>(BASIC_BLOCK_SIZE_256));
    mm.baseN = Ops::Base::CeilAlign(tempN, static_cast<int32_t>(MX_BASIC_FACTOR));
    int32_t tempK = std::min(static_cast<int32_t>(K), static_cast<int32_t>(BASIC_BLOCK_SIZE_128));
    mm.baseK = Ops::Base::CeilAlign(tempK, static_cast<int32_t>(MX_BASIC_FACTOR));
    ...
    uint64_t baseScaleASize = Ops::Base::CeilDiv(static_cast<uint64_t>(mm.baseK), MX_BASIC_FACTOR);
    baseScaleASize = Ops::Base::CeilAlign(baseScaleASize, 2UL) * mm.baseM;
    uint64_t baseScaleBSize = Ops::Base::CeilDiv(static_cast<uint64_t>(mm.baseK), MX_BASIC_FACTOR) * mm.baseN;
    ...
    uint64_t depthInit = GetDepthA1B1(leftL1Size, baseL1Size, mm.baseK, 1UL);
    uint64_t leftL1SizeByDepthInit = leftL1Size - depthInit * baseL1Size;
    uint64_t depthASec = GetDepthA1B1(leftL1SizeByDepthInit, (baseASize + baseScaleASize) * depthInit, mm.baseK, depthInit);
    uint64_t depthBSec = GetDepthA1B1(leftL1SizeByDepthInit, (baseBSize + baseScaleBSize) * depthInit, mm.baseK, depthInit);
    ...
    mm.stepKa = std::min(mm.stepKa, static_cast<int32_t>(4));
    mm.stepKb = std::min(mm.stepKb, static_cast<int32_t>(4));
    if (mm.stepKa >= mm.stepKb && mm.stepKa * mm.baseK < mm.Ka) {
        mm.stepKa = mm.stepKa / mm.stepKb * mm.stepKb;
    }
    if (mm.stepKb > mm.stepKa && mm.stepKb * mm.baseK < mm.Kb) {
...
void AlltoAllvMXQuantGmmTiling::SetTilingMxTypeParam(Mc2GroupedMatmulTilingData::GMMQuantTilingData &gmmQuantTilingData) const
{
    auto &mm = gmmQuantTilingData.mmTilingData;
    ...
    uint32_t scaleFactorA = static_cast<uint32_t>(mm.Ka / (mm.stepKa * mm.baseK));
    uint32_t scaleFactorB = static_cast<uint32_t>(mm.Kb / (mm.stepKb * mm.baseK));
...
uint64_t AlltoAllvMXQuantGmmTiling::GetDepthA1B1(uint64_t leftSize, uint64_t perDepthSize, uint64_t baseKSize, uint64_t depthInit) const
{
...
ge::graphStatus AlltoAllvMXQuantGmmTiling::CheckScaleShape() const
{
    ...
    auto gmmXScaleBSKDim = static_cast<uint64_t>(context_->GetOptionalInputShape(GMM_X_SCALE_INDEX)->GetStorageShape().GetDim(DIM_ONE));
    auto gmmXScaleHDim = static_cast<uint64_t>(context_->GetOptionalInputShape(GMM_X_SCALE_INDEX)->GetStorageShape().GetDim(DIM_TWO));
    ...
    auto gmmWeightScaleDimOne = context_->GetOptionalInputShape(GMM_WEIGHT_SCALE_INDEX)->GetStorageShape().GetDim(DIM_ONE);
    auto gmmWeightScaleDimTwo = context_->GetOptionalInputShape(GMM_WEIGHT_SCALE_INDEX)->GetStorageShape().GetDim(DIM_TWO);
    ...
    if (hasSharedExpertFlag_) {
        ...
        auto mmXScaleDumNum = context_->GetOptionalInputShape(MM_X_SCALE_INDEX)->GetStorageShape().GetDimNum();
        OP_TILING_CHECK(mmXScaleDumNum != DIM_THREE, OP_LOGE(context_->GetNodeName(), "When mx pergroup mode, mmXScale input shape should be [3], \
                        but actual is %lu", mmXScaleDumNum), return ge::GRAPH_FAILED);
        auto expectedHValue = Ops::Base::CeilDiv(h2_, MX_BASIC_FACTOR);
        auto mmXScaleBSDim = static_cast<uint64_t>(context_->GetOptionalInputShape(MM_X_SCALE_INDEX)->GetStorageShape().GetDim(DIM_ONE));
        auto mmXScaleHDim = static_cast<uint64_t>(context_->GetOptionalInputShape(MM_X_SCALE_INDEX)->GetStorageShape().GetDim(DIM_TWO));
        ...
        OP_TILING_CHECK(mmWeightScaleDimNum != DIM_THREE, OP_LOGE(context_->GetNodeName(), "When mx pergroup mode, mmXScale input shape should be [3], \
                        but actual is %lu.", mmWeightScaleDimNum), return ge::GRAPH_FAILED);
        auto mmWeightScaleDimOne = context_->GetOptionalInputShape(MM_WEIGHT_SCALE_INDEX)->GetStorageShape().GetDim(DIM_ONE);
        auto mmWeightScaleDimTwo = context_->GetOptionalInputShape(MM_WEIGHT_SCALE_INDEX)->GetStorageShape().GetDim(DIM_TWO);
