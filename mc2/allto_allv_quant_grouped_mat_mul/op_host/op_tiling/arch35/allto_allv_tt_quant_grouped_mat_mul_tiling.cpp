...
ge::graphStatus AlltoAllvTTQuantGmmTiling::DoLibApiTiling()
{
...
ge::graphStatus AlltoAllvTTQuantGmmTiling::CheckGmmDType() const
{
    ...
    OP_TILING_CHECK(gmmXDataType != ge::DT_HIFLOAT8,
        OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, gmmX should be hifloat8, but actual is %s.", \
    ...
    OP_TILING_CHECK(gmmWeightDataType != ge::DT_HIFLOAT8,
        OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, gmmWeight should be hifloat8, but actual is %s.", \
    ...
    OP_TILING_CHECK(context_->GetOptionalInputDesc(GMM_X_SCALE_INDEX) == nullptr,
        OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, gmmXScale should not be null."), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(gmmXScaleDataType != ge::DT_FLOAT,
        OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, gmmXScale should be float32, but actual is %s.", \
    ...
    OP_TILING_CHECK(gmmWeightScaleDataType != ge::DT_FLOAT,
        OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, gmmWeightScale should be float32, but actual is %s.",  \
    ...
    OP_TILING_CHECK(gmmYDataType != ge::DT_FLOAT16 && gmmYDataType != ge::DT_BF16, OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, \
    ...
    if (permuteOutFlag_) {
        ...
        OP_TILING_CHECK(permuteOutDataType != gmmXDataType,
            OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, permuteOut should be same as gmmX dataType(hifloat8), but actual is %s.", \
                    ge::TypeUtils::DataTypeToSerialString(permuteOutDataType).c_str()), return ge::GRAPH_FAILED);
    }
...
ge::graphStatus AlltoAllvTTQuantGmmTiling::CheckMmDType() const
{
    ...
    OP_TILING_CHECK(mmXDataType != context_->GetOptionalInputDesc(GMM_X_INDEX)->GetDataType(),
        OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, mmX should be same as gmmX(hifloat8), but actual is %s." \
    ...
    OP_TILING_CHECK(mmWeightDataType != context_->GetOptionalInputDesc(GMM_WEIGHT_INDEX)->GetDataType(),
        OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, mmWeight should be same as gmmWeight(hifloat8), but actual is %s." \
                ge::TypeUtils::DataTypeToSerialString(mmWeightDataType).c_str()), return ge::GRAPH_FAILED);
    // check mmXScale dataType(same as gmmXScale)
    OP_TILING_CHECK(context_->GetOptionalInputDesc(MM_X_SCALE_INDEX) == nullptr,
        OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, mmXScale should not be null."), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(mmXScaleDataType != context_->GetOptionalInputDesc(GMM_X_SCALE_INDEX)->GetDataType(),
        OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, mmXScale should be same as gmmXScale(float32), but actual is %s.", \
    ...
    OP_TILING_CHECK(context_->GetOptionalInputDesc(MM_WEIGHT_SCALE_INDEX) == nullptr,
        OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, mmWeightScale should not be null."), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(mmYDataType != context_->GetOutputDesc(OUTPUT_GMM_Y_INDEX)->GetDataType(),
        OP_LOGE(context_->GetNodeName(), "When pertensor quant mode, mmY should be same as gmmY(%s), but actual is %s.", \
