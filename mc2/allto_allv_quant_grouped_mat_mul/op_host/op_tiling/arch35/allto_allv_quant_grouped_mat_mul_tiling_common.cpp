...
ge::graphStatus AlltoAllvQuantGmmTilingCommon::GetWorkspaceSize()
{
    ...
    permuteOutSize = Ops::Base::CeilAlign(permuteOutSize, BASIC_BLOCK_SIZE_512);
    permuteOutSize += permuteScaleOutSize_;
...
ge::graphStatus AlltoAllvQuantGmmTilingCommon::CheckInputNotNull() const
{
    ...
    OP_TILING_CHECK((context_->GetInputDesc(GMM_X_INDEX) == nullptr),
        OP_LOGE(context_->GetNodeName(), "input gmmX cat not be null."), return ge::GRAPH_FAILED);
    // gmmWeight
    OP_TILING_CHECK(context_->GetInputDesc(GMM_WEIGHT_INDEX) == nullptr == nullptr,
        OP_LOGE(context_->GetNodeName(), "input gmmWeight cat not be null."), return ge::GRAPH_FAILED);
    // gmmY
    OP_TILING_CHECK(context_->GetOutputDesc(OUTPUT_GMM_Y_INDEX) == nullptr,
        OP_LOGE(context_->GetNodeName(), "output gmmy cat not be null."), return ge::GRAPH_FAILED);
    // permuteOut
    if (permuteOutFlag_) {
    ...
    if (hasSharedExpertFlag_) {
        ...
        OP_TILING_CHECK((context_->GetOptionalInputDesc(MM_X_INDEX) == nullptr),
            OP_LOGE(context_->GetNodeName(), "When hasSharedExpertFlag is true, input mmX cat not be null."),
            return ge::GRAPH_FAILED);
        // gmmWeight
        OP_TILING_CHECK(context_->GetInputDesc(MM_WEIGHT_INDEX) == nullptr == nullptr,
            OP_LOGE(context_->GetNodeName(), "When hasSharedExpertFlag is true, input mmWeight cat not be null."),
            return ge::GRAPH_FAILED);
        // mmY
        OP_TILING_CHECK(context_->GetOutputDesc(OUTPUT_MM_Y_INDEX) == nullptr,
            OP_LOGE(context_->GetNodeName(), "When hasSharedExpertFlag is true, output mmy cat not be null."),
                    return ge::GRAPH_FAILED);
    }
