...
ge::graphStatus AlltoAllvQuantGmmTilingCommon::GetWorkspaceSize()
{
    ...
    if (isMxfp8_) {
        ...
        if (permuteScaleOutSize % tensorListSize != 0) {
            permuteScaleOutSize = (permuteScaleOutSize + tensorListSize - 1) & ~(tensorListSize - 1);
        }
...
ge::graphStatus AlltoAllvQuantGmmTilingCommon::CheckQuantMode() const
{
    OP_LOGD(context_->GetNodeName(), "start CheckQuantMode.");
    auto gmmXQuantMode = *gmmXQuantModePtr_;
    OP_TILING_CHECK(isHif8_ && gmmXQuantMode != PERTENSOR_QUANT_MODE,
        OP_LOGE(context_->GetNodeName(), "gmmXQuantMode should be 1(pertensor mode), but actual is %lu.", gmmXQuantMode), 
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(isMxfp8_ && gmmXQuantMode != MX_QUANT_MODE,
        OP_LOGE(context_->GetNodeName(), "gmmXQuantMode should be 6(mx quant mode), but actual is %lu.", gmmXQuantMode), 
                return ge::GRAPH_FAILED);
    // gmmWeightQuantMode
    auto gmmWeightQuantMode = *gmmWeightQuantModePtr_;
    OP_TILING_CHECK(isHif8_ && gmmWeightQuantMode != PERTENSOR_QUANT_MODE,
        OP_LOGE(context_->GetNodeName(), "gmmWeightQuantMode should be 1(pertensor mode), but actual is %lu.", gmmWeightQuantMode),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(isMxfp8_ && gmmWeightQuantMode != MX_QUANT_MODE,
        OP_LOGE(context_->GetNodeName(), "gmmWeightQuantMode should be 6(mx quant mode), but actual is %lu.", gmmWeightQuantMode),
        return ge::GRAPH_FAILED);
    if (hasSharedExpertFlag_) {
        // mmXQuantMode
        auto mmXQuantMode = *mmXQuantModePtr_;
        OP_TILING_CHECK(isHif8_ && mmXQuantMode != PERTENSOR_QUANT_MODE,
            OP_LOGE(context_->GetNodeName(), "mmXQuantMode should be 1(pertensor mode), but actual is %lu.", mmXQuantMode), 
            return ge::GRAPH_FAILED);
        OP_TILING_CHECK(isMxfp8_ && mmXQuantMode != MX_QUANT_MODE,
            OP_LOGE(context_->GetNodeName(), "mmXQuantMode should be 6(mx quant mode), but actual is %lu.", mmXQuantMode), 
                    return ge::GRAPH_FAILED);
        // mmWeightQuantMode
        auto mmWeightQuantMode = *mmWeightQuantModePtr_;
        OP_TILING_CHECK(isHif8_ && mmWeightQuantMode != PERTENSOR_QUANT_MODE,
            OP_LOGE(context_->GetNodeName(), "mmWeightQuantMode should be 1(pertensor mode), but actual is %lu.", mmWeightQuantMode),
            return ge::GRAPH_FAILED);
        OP_TILING_CHECK(isMxfp8_ && mmWeightQuantMode != MX_QUANT_MODE,
            OP_LOGE(context_->GetNodeName(), "mmWeightQuantMode should be 6(mx quant mode), but actual is %lu.", mmWeightQuantMode),
            return ge::GRAPH_FAILED);
        return ge::GRAPH_SUCCESS;
    }
...
void AlltoAllvQuantGmmTilingCommon::SetGMMQuantParams(
    Mc2GroupedMatmulTilingData::GMMQuantTilingData &gmmQuantTilingData) const
{
    ...
    gmmQuantTilingData.gmmQuantParams.aQuantMode = isMxfp8_ ? MX_QUANT_MODE : PERTENSOR_MODE;
    gmmQuantTilingData.gmmQuantParams.bQuantMode = isMxfp8_ ? MX_QUANT_MODE : PERTENSOR_MODE;
...
void AlltoAllvQuantGmmTilingCommon::SetTilingArray(Mc2GroupedMatmulTilingData::GMMQuantTilingData &gmmQuantTilingData, uint64_t M, uint64_t N, uint64_t K) const
{
