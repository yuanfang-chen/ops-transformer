    ...
    const bool* transGmmWeight)
{
    OPS_ERR_IF(gmmX == nullptr,
        OP_LOGE("AlltoAllvQuantGroupedMatMulFallback", "gmmX is null"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(gmmWeight == nullptr,
        OP_LOGE("AlltoAllvQuantGroupedMatMulFallback", "gmmWeight is null"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(group == nullptr,
        OP_LOGE("AlltoAllvQuantGroupedMatMulFallback", "group is null"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(epWorldSize == nullptr,
        OP_LOGE("AlltoAllvQuantGroupedMatMulFallback", "epWorldSize is null"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(transGmmWeight == nullptr,
        OP_LOGE("AlltoAllvQuantGroupedMatMulFallback", "transGmmWeight is null"), return ge::GRAPH_FAILED);
    ...
    std::vector<int64_t>& actRecvCountsSeqArray)
{
    OPS_ERR_IF(sendCounts == nullptr,
        OP_LOGE("AlltoAllvQuantGroupedMatMulFallback", "sendCounts is null"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(recvCounts == nullptr,
        OP_LOGE("AlltoAllvQuantGroupedMatMulFallback", "recvCounts is null"), return ge::GRAPH_FAILED);
...
{
    OPS_ERR_IF(gmmY == nullptr, OP_LOGE("AlltoAllvQuantGroupedMatMulFallback", "gmmY is null"), return ge::GRAPH_FAILED);
...
static ge::graphStatus AlltoAllvQuantGroupedMatMulExecuteFunc(gert::OpExecuteContext* host_api_ctx)
{
    ...
    const gert::Tensor* gmmX = host_api_ctx->
        GetInputTensor(static_cast<size_t>(AlltoAllvQuantGroupedMatMulInputIdx::K_GMM_X));
    const gert::Tensor* gmmWeight = host_api_ctx->
        GetInputTensor(static_cast<size_t>(AlltoAllvQuantGroupedMatMulInputIdx::K_GMM_WEIGHT));
    const gert::Tensor* sendCountsTensor = host_api_ctx->
        GetOptionalInputTensor(static_cast<size_t>(AlltoAllvQuantGroupedMatMulInputIdx::K_SEND_COUNTS_TENSOR));
    const gert::Tensor* recvCountsTensor = host_api_ctx->
        GetOptionalInputTensor(static_cast<size_t>(AlltoAllvQuantGroupedMatMulInputIdx::K_RECV_COUNTS_TENSOR));
    const gert::Tensor* mmX = host_api_ctx->
        GetOptionalInputTensor(static_cast<size_t>(AlltoAllvQuantGroupedMatMulInputIdx::K_MM_X));
    const gert::Tensor* mmWeight = host_api_ctx->
        GetOptionalInputTensor(static_cast<size_t>(AlltoAllvQuantGroupedMatMulInputIdx::K_MM_WEIGHT));

    // 获取属性
    const gert::RuntimeAttrs* attrs = host_api_ctx->GetAttrs();
    OPS_ERR_IF(attrs == nullptr, OP_LOGE("AlltoAllvQuantGroupedMatMulFallback", "attrs is null"), return ge::GRAPH_FAILED);
    const char* group = attrs->GetStr(static_cast<size_t>(AlltoAllvQuantGroupedMatMulAttrIdx::K_GROUP));
    const int64_t* epWorldSize = attrs->
        GetInt(static_cast<size_t>(AlltoAllvQuantGroupedMatMulAttrIdx::K_EP_WORLD_SIZE));
    const gert::TypedContinuousVector<int64_t>* sendCounts = attrs->
        GetListInt(static_cast<size_t>(AlltoAllvQuantGroupedMatMulAttrIdx::K_SEND_COUNTS));
    const gert::TypedContinuousVector<int64_t>* recvCounts = attrs->
        GetListInt(static_cast<size_t>(AlltoAllvQuantGroupedMatMulAttrIdx::K_RECV_COUNTS));
    const bool* transGmmWeight = attrs->
        GetBool(static_cast<size_t>(AlltoAllvQuantGroupedMatMulAttrIdx::K_TRANS_GMM_WEIGHT));
    const bool* transMmWeight = attrs->
        GetBool(static_cast<size_t>(AlltoAllvQuantGroupedMatMulAttrIdx::K_TRANS_MM_WEIGHT));
    const bool* permuteOutFlag = attrs->
        GetBool(static_cast<size_t>(AlltoAllvQuantGroupedMatMulAttrIdx::K_PERMUTE_OUT_FLAG));
    ...
    // 获取并校验输出参数
    auto gmmY = host_api_ctx->GetOutputTensor(static_cast<size_t>(AlltoAllvQuantGroupedMatMulOutputIdx::K_GMM_Y));
    auto mmY = host_api_ctx->GetOutputTensor(static_cast<size_t>(AlltoAllvQuantGroupedMatMulOutputIdx::K_MM_Y));
    auto permuteOut = host_api_ctx->
        GetOutputTensor(static_cast<size_t>(AlltoAllvQuantGroupedMatMulOutputIdx::K_PERMUTE_OUT));
