...
namespace {
    ...
    const char *K_INNER_DEBUG = "MoeDistributeCombineV3 Tiling Debug";
...
namespace optiling {
// a3专有
static void PrintTilingDataInfo(const char *nodeName, MoeDistributeCombineV3TilingData& tilingData)
{
    OP_LOGD(nodeName, "epWorldSize is %u.", tilingData.MoeDistributeCombineV3Info.epWorldSize);
    OP_LOGD(nodeName, "tpWorldSize is %u.", tilingData.MoeDistributeCombineV3Info.tpWorldSize);
    OP_LOGD(nodeName, "epRankId is %u.", tilingData.MoeDistributeCombineV3Info.epRankId);
    OP_LOGD(nodeName, "tpRankId is %u.", tilingData.MoeDistributeCombineV3Info.tpRankId);
    OP_LOGD(nodeName, "expertShardType is %u.", tilingData.MoeDistributeCombineV3Info.expertShardType);
    OP_LOGD(nodeName, "sharedExpertNum is %u.", tilingData.MoeDistributeCombineV3Info.sharedExpertNum);
    OP_LOGD(nodeName, "sharedExpertRankNum is %u.", tilingData.MoeDistributeCombineV3Info.sharedExpertRankNum);
    OP_LOGD(nodeName, "moeExpertNum is %u.", tilingData.MoeDistributeCombineV3Info.moeExpertNum);
    OP_LOGD(nodeName, "moeExpertPerRankNum is %u.", tilingData.MoeDistributeCombineV3Info.moeExpertPerRankNum);
    OP_LOGD(nodeName, "globalBs is %u.", tilingData.MoeDistributeCombineV3Info.globalBs);
    OP_LOGD(nodeName, "bs is %u.", tilingData.MoeDistributeCombineV3Info.bs);
    OP_LOGD(nodeName, "k is %u.", tilingData.MoeDistributeCombineV3Info.k);
    OP_LOGD(nodeName, "h is %u.", tilingData.MoeDistributeCombineV3Info.h);
    OP_LOGD(nodeName, "aivNum is %u.", tilingData.MoeDistributeCombineV3Info.aivNum);
    OP_LOGD(nodeName, "totalUbSize is %lu.", tilingData.MoeDistributeCombineV3Info.totalUbSize);
    OP_LOGD(nodeName, "totalWinSizeEP is %lu.", tilingData.MoeDistributeCombineV3Info.totalWinSizeEp);
    OP_LOGD(nodeName, "totalWinSizeTP is %lu.", tilingData.MoeDistributeCombineV3Info.totalWinSizeTp);
    OP_LOGD(nodeName, "hasElastic is %d.", tilingData.MoeDistributeCombineV3Info.hasElasticInfo);
    OP_LOGD(nodeName, "isPerformance is %d.", tilingData.MoeDistributeCombineV3Info.isPerformance);
}

static ge::graphStatus GetAttrAndSetTilingData(const gert::TilingContext *context,
    MoeDistributeCombineV3TilingData &tilingData, const char *nodeName, std::string &groupEp, std::string &groupTp,
    uint32_t &commQuantMode, const CombineV2Config& config)
{
    ...
    tilingData.MoeDistributeCombineV3Info.epWorldSize = static_cast<uint32_t>(epWorldSize);
    tilingData.MoeDistributeCombineV3Info.tpWorldSize = static_cast<uint32_t>(*tpWorldSizePtr);
    tilingData.MoeDistributeCombineV3Info.epRankId = static_cast<uint32_t>(*epRankIdPtr);
    tilingData.MoeDistributeCombineV3Info.tpRankId = static_cast<uint32_t>(*tpRankIdPtr);
    tilingData.MoeDistributeCombineV3Info.expertShardType = static_cast<uint32_t>(*expertShardPtr);
    tilingData.MoeDistributeCombineV3Info.sharedExpertNum = static_cast<uint32_t>(*sharedExpertNumPtr);
    tilingData.MoeDistributeCombineV3Info.sharedExpertRankNum = static_cast<uint32_t>(sharedExpertRankNum);
    if (tilingData.MoeDistributeCombineV3Info.sharedExpertRankNum == 0U) {
        if (tilingData.MoeDistributeCombineV3Info.sharedExpertNum == 1U) {
            tilingData.MoeDistributeCombineV3Info.sharedExpertNum = 0U;
        }
    }
    tilingData.MoeDistributeCombineV3Info.moeExpertNum = static_cast<uint32_t>(moeExpertNum);
    tilingData.MoeDistributeCombineV3Info.zeroExpertNum = static_cast<uint32_t>(zeroExpertNum);
    tilingData.MoeDistributeCombineV3Info.copyExpertNum = static_cast<uint32_t>(copyExpertNum);
    tilingData.MoeDistributeCombineV3Info.constExpertNum = static_cast<uint32_t>(constExpertNum);

    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus CheckARNAttrAndSetTilingData(const gert::TilingContext *context,
    MoeDistributeCombineV3TilingData &tilingData, const char *nodeName, const CombineV2Config& config)
{
    auto attrs = context->GetAttrs();
    OP_TILING_CHECK(attrs == nullptr, OP_LOGE(nodeName, "attrs is null."), return ge::GRAPH_FAILED);
    ...
    tilingData.MoeDistributeCombineV3Info.epsilon = *epsilonPtr;
...
static bool CheckOptionalInputTensorDim(const gert::TilingContext *context, const char *nodeName,
    const bool isActiveMask, const bool hasElasticInfo, const bool isPerformance, uint32_t tpWorldSize, const CombineV2Config& config)
{
...
static bool CheckTensorDim(gert::TilingContext *context, const char *nodeName, const bool isActiveMask,
                           const bool hasElasticInfo, const bool isPerformance, uint32_t tpWorldSize, const CombineV2Config& config)
{
    OP_TILING_CHECK(!CheckInputTensorDim(context, nodeName),
        OP_LOGE(nodeName, "param shape of input tensor is invalid"), return false);

    OP_TILING_CHECK(!CheckOptionalInputTensorDim(context, nodeName, isActiveMask, hasElasticInfo, isPerformance, tpWorldSize, config),
...
static bool CheckTensorShape(const gert::TilingContext *context, MoeDistributeCombineV3TilingData &tilingData,
    ...
    const CombineV2Config& config)
{
    ...
    int64_t expertIdsDim0 = expertIdsStorageShape->GetStorageShape().GetDim(0);
    int64_t expertIdsDim1 = expertIdsStorageShape->GetStorageShape().GetDim(1);
    int64_t moeExpertNum = static_cast<int64_t>(tilingData.MoeDistributeCombineV3Info.moeExpertNum);
    int64_t zeroExpertNum = static_cast<int64_t>(tilingData.MoeDistributeCombineV3Info.zeroExpertNum);
    int64_t copyExpertNum = static_cast<int64_t>(tilingData.MoeDistributeCombineV3Info.copyExpertNum);
    int64_t constExpertNum = static_cast<int64_t>(tilingData.MoeDistributeCombineV3Info.constExpertNum);
    ...
    tilingData.MoeDistributeCombineV3Info.k = static_cast<uint32_t>(expertIdsDim1);

    uint32_t A = 0U;
    uint32_t globalBs = tilingData.MoeDistributeCombineV3Info.globalBs;
    uint32_t sharedExpertNum = tilingData.MoeDistributeCombineV3Info.sharedExpertNum;
    uint32_t sharedExpertRankNum = tilingData.MoeDistributeCombineV3Info.sharedExpertRankNum;
    uint32_t rankNumPerSharedExpert = 0;
    uint32_t epWorldSizeU32 = tilingData.MoeDistributeCombineV3Info.epWorldSize;
    ...
    const int64_t epWorldSize = static_cast<int64_t>(tilingData.MoeDistributeCombineV3Info.epWorldSize);
    if (hasElasticInfo) {
        const gert::StorageShape *elasticInfoStorageShape = context->GetOptionalInputShape(config.elasticInfoIndex);
    ...
    tilingData.MoeDistributeCombineV3Info.a = A;
    ...
    // 校验expandX的维度并设h
    int64_t tpWorldSize = static_cast<int64_t>(tilingData.MoeDistributeCombineV3Info.tpWorldSize);
    const gert::StorageShape *expandXStorageShape = context->GetInputShape(EXPAND_X_INDEX);
    ...
    tilingData.MoeDistributeCombineV3Info.h = static_cast<uint32_t>(expandXDim1);
    if (expandXDim1 != 0) {
        tilingData.MoeDistributeCombineV3Info.armAvgFactor = 1.0f / expandXDim1;
    }
    ...
    int64_t moeExpertPerRankNum = static_cast<int64_t>(tilingData.MoeDistributeCombineV3Info.moeExpertPerRankNum);
    ...
    tilingData.MoeDistributeCombineV3Info.hasSharedExpertX = (sharedExpertXShape != nullptr);
    ...
    if (constExpertAlpha1Shape != nullptr) {
        ...
        OP_TILING_CHECK(
            constExpertAlpha1Dim0 != static_cast<int64_t>(tilingData.MoeDistributeCombineV3Info.constExpertNum),
            OP_LOGE(nodeName,
                "const_expert_alpha_1's dim0 not equal to const_expert_num, const_expert_alpha_1's dim0 = %ld, "
                "const_expert_num = %u",
                constExpertAlpha1Dim0, tilingData.MoeDistributeCombineV3Info.constExpertNum),
            return false);
    ...
    if (constExpertAlpha2Shape != nullptr) {
        ...
        OP_TILING_CHECK(
            constExpertAlpha2Dim0 != static_cast<int64_t>(tilingData.MoeDistributeCombineV3Info.constExpertNum),
            OP_LOGE(nodeName,
                "const_expert_alpha_2's dim0 not equal to const_expert_num, const_expert_alpha_2's dim0 = %ld, "
                "const_expert_num = %u",
                constExpertAlpha2Dim0, tilingData.MoeDistributeCombineV3Info.constExpertNum),
            return false);
    ...
    if (constExpertVShape != nullptr) {
        ...
        OP_TILING_CHECK(
            constExpertVDim0 != static_cast<int64_t>(tilingData.MoeDistributeCombineV3Info.constExpertNum),
            OP_LOGE(nodeName,
                "const_expert_v's dim0 not equal to const_expert_num, const_expert_v's dim0 = %ld, const_expert_num = %u.",
                constExpertVDim0, tilingData.MoeDistributeCombineV3Info.constExpertNum),
            return false);
...
static bool CheckSharedAttrs(const char *nodeName, const MoeDistributeCombineV3TilingData &tilingData)
{
    uint32_t sharedExpertNum = tilingData.MoeDistributeCombineV3Info.sharedExpertNum;
    uint32_t sharedExpertRankNum = tilingData.MoeDistributeCombineV3Info.sharedExpertRankNum;
...
static bool CheckAttrs(const gert::TilingContext *context, MoeDistributeCombineV3TilingData &tilingData,
    const char *nodeName, uint32_t &localMoeExpertNum, bool isActiveMask, const CombineV2Config& config)
{
    uint32_t epWorldSize = tilingData.MoeDistributeCombineV3Info.epWorldSize;
    uint32_t tpWorldSize = tilingData.MoeDistributeCombineV3Info.tpWorldSize;
    uint32_t moeExpertNum = tilingData.MoeDistributeCombineV3Info.moeExpertNum;
    uint32_t sharedExpertRankNum = tilingData.MoeDistributeCombineV3Info.sharedExpertRankNum;
    ...
    // 校验tp=2时是否没有动态缩容参数
    OP_TILING_CHECK((tpWorldSize > 1) && (tilingData.MoeDistributeCombineV3Info.hasElasticInfo), OP_LOGE(nodeName, "Cannot support elasticInfo "
        "when tpWorldSize = %u > 1", tpWorldSize), return false);
    tilingData.MoeDistributeCombineV3Info.moeExpertPerRankNum = localMoeExpertNum;
    ...
    tilingData.MoeDistributeCombineV3Info.bs = static_cast<uint32_t>(expertIdsDim0);
    ...
    OP_LOGD(nodeName, "MoeDistributeCombineV3 *globalBsPtr = %ld, bs = %ld, epWorldSize = %u\n",
    ...
    tilingData.MoeDistributeCombineV3Info.globalBs = static_cast<uint32_t>(*globalBsPtr);
    if (*globalBsPtr == 0) {
        tilingData.MoeDistributeCombineV3Info.globalBs = static_cast<uint32_t>(expertIdsDim0) * epWorldSize;
    }

    uint32_t copyExpertNum = tilingData.MoeDistributeCombineV3Info.copyExpertNum;
    uint32_t constExpertNum = tilingData.MoeDistributeCombineV3Info.constExpertNum;
...
{
    // 检查参数shape信息
    OP_TILING_CHECK(!CheckTensorDim(context, nodeName, isActiveMask, hasElasticInfo, isPerformance, tpWorldSize, config),
...
static ge::graphStatus SetHCommCfg(const gert::TilingContext *context, MoeDistributeCombineV3TilingData *tiling,
    const std::string groupEp, const std::string groupTp, const uint32_t tpWorldSize)
{
    const char* nodeName = context->GetNodeName();
    OP_LOGD(nodeName, "MoeDistributeCombineV3 groupEp = %s", groupEp.c_str());
    ...
    if (tpWorldSize > 1) {
        OP_LOGD(nodeName, "MoeDistributeCombineV3 groupTp = %s", groupTp.c_str());
...
static void UbUsedCal(const uint64_t ubSize, const gert::TilingContext* context, MoeDistributeCombineV3TilingData *tilingData, const CombineV2Config& config)
{
    uint32_t axisH = tilingData->MoeDistributeCombineV3Info.h;
    uint32_t axisBS = tilingData->MoeDistributeCombineV3Info.bs;
    uint32_t axisK = tilingData->MoeDistributeCombineV3Info.k;
    uint32_t zeroExpertNum = tilingData->MoeDistributeCombineV3Info.zeroExpertNum;
    uint32_t copyExpertNum = tilingData->MoeDistributeCombineV3Info.copyExpertNum;
    uint32_t constExpertNum = tilingData->MoeDistributeCombineV3Info.constExpertNum;
    bool isInputExpertMaskFlag = tilingData->MoeDistributeCombineV3Info.isExpertMask;
    bool isInputTokenMaskFlag = tilingData->MoeDistributeCombineV3Info.isTokenMask;
    ...
    uint32_t flagRcvCount = axisK + tilingData->MoeDistributeCombineV3Info.sharedExpertNum;
    ...
    tilingData->MoeDistributeCombineV3Info.bufferNum = totalBufferSize > ubSize ? BUFFER_SINGLE : BUFFER_NUM;
}

static ge::graphStatus CheckWinSize(const gert::TilingContext *context, MoeDistributeCombineV3TilingData* tilingData,
    const char *nodeName, uint32_t localMoeExpertNum)
{
    ...
    uint64_t h = static_cast<uint64_t>(tilingData->MoeDistributeCombineV3Info.h);
    uint64_t epWorldSize = static_cast<uint64_t>(tilingData->MoeDistributeCombineV3Info.epWorldSize);
    uint64_t k = static_cast<uint64_t>(tilingData->MoeDistributeCombineV3Info.k);
    uint64_t sharedExpertNum = static_cast<uint64_t>(tilingData->MoeDistributeCombineV3Info.sharedExpertNum);
    uint64_t maxBs = static_cast<uint64_t>(tilingData->MoeDistributeCombineV3Info.globalBs)/ epWorldSize;
    ...
    OP_TILING_CHECK((actualSize > maxWindowSizeEp),
        OP_LOGE(nodeName, "HCCL_BUFFSIZE_EP is too SMALL, maxBs = %lu, h = %lu, epWorldSize = %lu,"
            ...
            return ge::GRAPH_FAILED);
    tilingData->MoeDistributeCombineV3Info.totalWinSizeEp = maxWindowSizeEp;
    OP_LOGD(nodeName, "EpwindowSize = %lu", maxWindowSizeEp);

    uint64_t tpWorldSize = static_cast<uint64_t>(tilingData->MoeDistributeCombineV3Info.tpWorldSize);
    if (tpWorldSize == TP_WORLD_SIZE_TWO) {
        ...
        OP_TILING_CHECK(mc2tiling::GetCclBufferSize(groupTpHccl, &maxWindowSizeTp, nodeName) != ge::GRAPH_SUCCESS,
            ...
            return ge::GRAPH_FAILED);
        actualSize = static_cast<uint64_t>(tilingData->MoeDistributeCombineV3Info.a) * (tokenNeedSizeDispatch +
        ...
        OP_LOGE(nodeName, "TP HCCL_BUFFSIZE is too SMALL, A = %u, tokenNeedSizeDispatch = %lu, tokenNeedSizeCombine = %lu,"
            ...
            tilingData->MoeDistributeCombineV3Info.a, tokenNeedSizeDispatch, tokenNeedSizeCombine, actualSize / MB_SIZE + 1UL,
            maxWindowSizeTp / MB_SIZE), return ge::GRAPH_FAILED);
        tilingData->MoeDistributeCombineV3Info.totalWinSizeTp = maxWindowSizeTp;
        OP_LOGD(nodeName, "TpwindowSize = %lu", maxWindowSizeTp);
    }
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus CheckCombineOrARN(gert::TilingContext* context, MoeDistributeCombineV3TilingData &tilingData, const char *nodeName, const CombineV2Config& config)
{
    const gert::StorageShape *expertIdsStorageShape = context->GetInputShape(EXPERT_IDS_INDEX);
    const gert::StorageShape *expandXStorageShape = context->GetInputShape(EXPAND_X_INDEX);
    int64_t expertIdsDim0 = expertIdsStorageShape->GetStorageShape().GetDim(0);
...
{
    const char *nodeName = context->GetNodeName();
    OP_LOGD(nodeName, "Enter MoeDistributeCombineV3 Tiling func");
    MoeDistributeCombineV3TilingData *tilingData = context->GetTilingData<MoeDistributeCombineV3TilingData>();
    ...
    // 获取入参属性
    OP_TILING_CHECK(GetAttrAndSetTilingData(context, *tilingData, nodeName, groupEp, groupTp, commQuantMode, config) == ge::GRAPH_FAILED,
        OP_LOGE(nodeName, "Getting attr failed."), return ge::GRAPH_FAILED);

    const gert::StorageShape *xActiveMaskStorageShape = context->GetOptionalInputShape(config.xActiveMaskIndex);
    isActiveMask = (xActiveMaskStorageShape != nullptr);
    tilingData->MoeDistributeCombineV3Info.isTokenMask = ((isActiveMask) &&
        (xActiveMaskStorageShape->GetStorageShape().GetDimNum() == ONE_DIM));
    tilingData->MoeDistributeCombineV3Info.isExpertMask = ((isActiveMask) &&
        (xActiveMaskStorageShape->GetStorageShape().GetDimNum() == TWO_DIMS));

    // 获取elasticInfo
    const gert::StorageShape *elasticInfoStorageShape = context->GetOptionalInputShape(config.elasticInfoIndex);
    hasElasticInfo = (elasticInfoStorageShape != nullptr);
    tilingData->MoeDistributeCombineV3Info.hasElasticInfo = hasElasticInfo;

    // 获取performanceInfo
    const gert::StorageShape *performanceInfoStorageShape = context->GetOptionalInputShape(config.performanceInfoIndex);
    isPerformance = (performanceInfoStorageShape != nullptr);
    tilingData->MoeDistributeCombineV3Info.isPerformance = isPerformance;

    // 检查输入输出的dim、format、dataType
    uint32_t tpWorldSize = tilingData->MoeDistributeCombineV3Info.tpWorldSize;
    OP_TILING_CHECK(TilingCheckMoeDistributeCombine(context, nodeName, isActiveMask, hasElasticInfo, isPerformance, tpWorldSize, config) !=
                    ge::GRAPH_SUCCESS, OP_LOGE(nodeName, "Tiling check params failed"), return ge::GRAPH_FAILED);

    // 检查属性的取值是否合法
    OP_TILING_CHECK(!CheckAttrs(context, *tilingData, nodeName, localMoeExpertNum, isActiveMask, config),
        OP_LOGE(nodeName, "attr check failed."), return ge::GRAPH_FAILED);

    uint32_t sharedExpertNum = tilingData->MoeDistributeCombineV3Info.sharedExpertNum;
    uint32_t sharedExpertRankNum = tilingData->MoeDistributeCombineV3Info.sharedExpertRankNum;
    uint32_t epRankId = tilingData->MoeDistributeCombineV3Info.epRankId;

    isShared = (epRankId < sharedExpertRankNum);

    // 检查shape各维度并赋值h,k
    OP_TILING_CHECK(!CheckTensorShape(context, *tilingData, nodeName, isShared, isActiveMask, localMoeExpertNum, hasElasticInfo, isPerformance, config),
    ...
    tilingData->MoeDistributeCombineV3Info.aivNum = aivNum;
    tilingData->MoeDistributeCombineV3Info.totalUbSize = ubSize;
...
static ge::graphStatus MoeDistributeCombineV3TilingFuncNew(gert::TilingContext* context, const CombineV2Config& config)
{
...
ge::graphStatus MoeDistributeCombineV3TilingFunc(gert::TilingContext* context)
{
    ...
    if (rstdOutDesc == nullptr) {
        config.tpSendCountsIndex = 5; // 根据combineV2算子原型标志位设置tpSendCounts索引
        config.xActiveMaskIndex = 6; // 根据combineV2算子原型标志位设置xActiveMask索引
        config.activationScaleIndex = 7; // 根据combineV2算子原型标志位设置activationScale索引
        config.weightScaleIndex = 8; // 根据combineV2算子原型标志位设置weightScale索引
        config.groupListIndex = 9; // 根据combineV2算子原型标志位设置groupList索引
        config.sharedExpertXIndex = 11; // 根据combineV2算子原型标志位设置sharedExpertX索引
        config.elasticInfoIndex = 12; // 根据combineV2算子原型标志位设置elasticInfo索引
        config.oriXIndex = 13; // 根据combineV2算子原型标志位设置oriX索引
        config.constExpertAlpha1Index = 14; // 根据combineV2算子原型标志位设置constExpertAlpha1索引
        config.constExpertAlpha2Index = 15; // 根据combineV2算子原型标志位设置constExpertAlpha2索引
        config.constExpertVIndex = 16; // 根据combineV2算子原型标志位设置constExpertV索引
        config.performanceInfoIndex = 17; // 根据combineV2算子原型标志位设置performanceInfo索引
        ...
        config.residualXIndex = 5; // 根据combineARN算子原型标志位设置residualX索引
        config.gammaIndex = 6; // 根据combineARN算子原型标志位设置gamma索引
        config.tpSendCountsIndex = 7; // 根据combineARN算子原型标志位设置tpSendCounts索引
        config.xActiveMaskIndex = 8; // 根据combineARN算子原型标志位设置xActiveMask索引
        config.activationScaleIndex = 9; // 根据combineARN算子原型标志位设置activationScale索引
        config.weightScaleIndex = 10; // 根据combineARN算子原型标志位设置weightScale索引
        config.groupListIndex = 11; // 根据combineARN算子原型标志位设置groupList索引
        config.sharedExpertXIndex = 13; // 根据combineARN算子原型标志位设置sharedExpertX索引
        config.elasticInfoIndex = 14; // 根据combineARN算子原型标志位设置elasticInfo索引
        config.oriXIndex = 15; // 根据combineARN算子原型标志位设置oriX索引
        config.constExpertAlpha1Index = 16; // 根据combineARN算子原型标志位设置constExpertAlpha1索引
        config.constExpertAlpha2Index = 17; // 根据combineARN算子原型标志位设置constExpertAlpha2索引
        config.constExpertVIndex = 18; // 根据combineARN算子原型标志位设置constExpertV索引
        config.performanceInfoIndex =19; // combineARN算子原型没有传入performanceInfoIndex设置虚拟索引
        config.outputYIndex = 0; // 根据combineARN算子原型标志位设置outputY索引
        config.outputRstdIndex = 1; // 根据combineARN算子原型标志位设置outputRstd索引
        config.outputXIndex = 2; // 根据combineARN算子原型标志位设置outputX索引
        config.attrNormEpsIndex = 15; // 根据combineARN算子原型标志位设置attrNormEps索引
        config.attrZeroExpertNumIndex = 16; // 根据combineARN算子原型标志位设置attrZeroExpertNum索引
        config.attrCopyExpertNumIndex = 17; // 根据combineARN算子原型标志位设置attrCopyExpertNum索引
        config.attrConstExpertNumIndex = 18; // 根据combineARN算子原型标志位设置attrConstExpertNum索引
        config.hasAddRmsNorm = true;
    }
    ret = MoeDistributeCombineV3TilingFuncNew(context, config);
    return ret;
}
