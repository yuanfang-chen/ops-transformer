...
//         auto attrs = context->GetAttrs();
//         auto sharedExpertRankNumPtr = attrs->GetAttrPointer<int64_t>(Idx::attr::ATTR_SHARED_EXPERT_RANK_NUM_INDEX);
//         OP_TILING_CHECK(*sharedExpertRankNumPtr != 0, OP_LOGE(nodeName, "sharedExpertX only support Idx::input None "\
...
//         OP_LOGE(nodeName, "param shape of Idx::input tensor is invalid"), return false);

//     OP_TILING_CHECK(!CheckOptionalInputTensorDim(context, nodeName, isActiveMask, hasElasticInfo, isPerformance, tpWorldSize),
//         OP_LOGE(nodeName, "param shape of optional Idx::input tensor is invalid"), return false);

//     OP_TILING_CHECK(!CheckOutputTensorDim(context, nodeName),
//         OP_LOGE(nodeName, "param shape of Idx::output tensor is invalid"), return false);
...
//     // 检查参数dataType信息
//     OP_TILING_CHECK(!CheckTensorDataType(context, nodeName, isActiveMask, hasElasticInfo, isPerformance, tpWorldSize),
    ...
    const char *nodeName, uint32_t localMoeExpertNum)
{
    ...
    OP_TILING_CHECK(
        mc2tiling::GetEpWinSize(context, nodeName, hcclBufferSizeEp, maxWindowSizeEp, Idx::attr::ATTR_GROUP_EP_INDEX) != ge::GRAPH_SUCCESS,
                                                                    ...
                                                                    MoeDistributeCombineA2Info &info, bool isLayered)
{
    ...
    OP_TILING_CHECK(elasticInfoStorageShape != nullptr, OP_LOGE(K_INNER_DEBUG, "current does not support elasticInfo as Idx::input"), return GRAPH_FAILED);
