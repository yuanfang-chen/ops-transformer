...
static gert::TilingContextPara BuildTilingContextPara(const QuantAllReduceTestParam &param) 
{
    ...
    return gert::TilingContextPara tilingContextPara(OP_NAME, inputTensorDesc, outputTensorDesc, attrs, &compileInfo, param.socVersion);
}
...
TEST_F(TestMoeDistributeTeardownTiling, general_cases_multi_thread) {
    TestMultiThread(test_cases, sizeof(test_cases) / sizeof(MoeDistributeDispatchTeardownTilingTestParam), 3);
}

INSTANTIATE_TEST_CASE_P(MoeDistributeDispatchTeardownTilingUT, TestMoeDistributeTeardownTiling, testing::ValuesIn(GetCasesFromCsv<MoeDistributeDispatchTeardownTilingTestParam>(ReplaceFileExtension2Csv(__FILE__))));
