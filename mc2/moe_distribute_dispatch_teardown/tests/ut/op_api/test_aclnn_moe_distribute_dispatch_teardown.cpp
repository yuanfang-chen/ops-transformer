...
static void TestOneParamCase(const MoeDistributeDispatchTeardownAclnnTestParam& param) {
    std::out << "[TEST_CASE]" << param.caseName << std::endl;
    ...
    auto ut = OP_API_UT(aclnnMoeDistributeDispatchTeardown,
                            INPUT(x, y, expert_ids, comm_cmd_info, groupEp, param.epWorldSize, param.epRankId, param.moeExpertNum, param.expertShardType, param.sharedExpertNum, param.sharedExpertRankNum, param.quantMode, param.globalBs, param.expertTokenNumsType, param.commType, commAlg),
                            OUTPUT(expandX, dynamicScales, assistInfoForCombine, expertTokensNums));
    uint64_t workspace_size = 0;
    aclOpExecutor* executor = nullptr;
    aclnnStatus aclRet = ut.TestGetWorkspaceSizeWithNNopbaseInner(&workspace_size, executor);
    EXPECT_EQ(aclRet, param.retStatus);
}
...
TEST_F(test_aclnn_moe_distribute_dispatch_teardown, general_cases_multi_thread) {
    TestMultiThread(test_cases, sizeof(test_cases) / sizeof(MoeDistributeDispatchTeardownAclnnTestParam), 3);
}
}