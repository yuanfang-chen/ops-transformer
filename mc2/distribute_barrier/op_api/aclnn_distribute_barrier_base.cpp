                            ...
                            Mc2MoeContext* mc2Context, int64_t worldSize, void* ctx)
{
    ...
    ret = HcckGetHcclBuffer(hcclHandle, &hcclBufferAddr, &hcclBufferSize);
    if (ret != HCCL_SUCCESS) {
    ...
    for (uint64_t rankId = 0; rankId < worldSize; rankId++) {
        ...
        channelDesc[0].channelProtool = CommProtocol::COMM_PROTOCOL_HCCS;
        ...
        ret = HcclChannelGetHcclBuffer(comm, channels[0], &remoteHcclBufferAddr, &remoteHcclBufferSize);
        if (ret != HCCL_SUCCESS) {
...
aclnnStatus GetMc2Context(const aclTensor* mc2Context, int64_t worldSize) 
{
                                                       ...
                                                       aclOpExecutor** executor)
{
    ...
    retParam = GetMc2Context(mc2Context, worldSize);
 	CHECK_RET(ret == ACLNN_SUCCESS, retParam);
