...
extern "C" aclnnStatus aclnnInnerAlltoAllvGroupedMatMulGetWorkspaceSize(
    ...
    int64_t gmmWeightQuantMode, int64_t mmXQuantMode, int64_t mmWeightQuantMode, const aclTensor *gmmYOut, const aclTensor *mmYOutOptional,
...
static bool CheckNotNull(const aclTensor *gmmX, const aclTensor *gmmWeight, const aclTensor *gmmY,
                         const aclTensor *gmmXScale, const aclTensor *gmmWeightScale,
...
// 检查维度
static bool CheckShape(const aclTensor *gmmX, const aclTensor *gmmWeight, const aclTensor *gmmXScale,
                       const aclTensor *gmmWeightScale, const aclTensor *mmXOptional,
                       const aclTensor *mmWeightOptional, const aclTensor *mmXScaleOptional,
                       ...
                       int64_t epWorldSize)
{
    ...
    if ((((gmmWeight->GetViewShape().GetDim(0)) * epWorldSize) > MAX_EXPERT_SIZE) || ((gmmWeight->GetViewShape().GetDim(0)) > MAX_E_SIZE) || (!((gmmWeight->GetViewShape().GetDim(0)) > ZERO))) {
    ...
    if (mmXOptional != nullptr) {
        ...
        if ((mmXOptional->GetViewShape().GetDim(0) == ZERO) || (mmXOptional->GetViewShape().GetDim(0) == ZERO) || (mmXOptional->GetViewShape().GetDim(1) > MAX_H2_LEN)) {
                        ...
                        const aclTensor *mmWeightScaleOptional, const aclTensor *gmmY, const aclTensor *mmYOptional)
{
    ...
    if (mmXOptional != nullptr) {
        if ((IsPrivateFormat(mmXOptional->GetStorageFormat())) || (IsPrivateFormat(mmXScaleOptional->GetStorageFormat()))) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "aclnnQuantAlltoAllVGroupMatmul, mmXOptional or mmXScaleOptional format %s does not support private format.",
                    op::ToString(mmXOptional->GetStorageFormat()).GetString());
            return false;
        }
    }
    if (mmWeightOptional != nullptr) {
        if ((IsPrivateFormat(mmWeightOptional->GetStorageFormat())) || (IsPrivateFormat(mmWeightScaleOptional->GetStorageFormat()))) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "aclnnQuantAlltoAllVGroupMatmul, mmWeightOptional or mmWeightScaleOptional format %s does not support private format.",
                               ...
                               const aclTensor *mmYOptional, const aclTensor *permuteOutOptional)
{
    ...
    // 3.检查参数是否为空
    CHECK_RET(CheckNotNull(gmmX, gmmWeight, gmmY, gmmXScale, gmmWeightScale, gmmXQuantMode,
                           gmmWeightQuantMode),
              ACLNN_ERR_PARAM_INVALID);
    // 4.检查暂不支持的参数是否为空，不影响场景
    CHECK_RET(CheckNotSupportNull(gmmXOffsetOptional, gmmWeightOffsetOptional, mmXOffsetOptional,
                                  mmWeightOffsetOptional),
              ACLNN_ERR_PARAM_INVALID);
    // 5.检查维度
    CHECK_RET(CheckDimValid(gmmX, gmmWeight, gmmY, gmmXScale, gmmWeightScale, mmXOptional,
                            mmWeightOptional, mmYOptional, mmXScaleOptional, mmWeightScaleOptional),
              ACLNN_ERR_PARAM_INVALID);
    // 5.检查空tensor
    CHECK_RET(CheckNotEmptyTensor(gmmX, gmmWeight, gmmY), ACLNN_ERR_PARAM_INVALID);
    // 检查所有输入/量化数据类型
    CHECK_RET(CheckDtypesValid(gmmX, gmmWeight, gmmXScale, gmmWeightScale, mmXOptional,
                               mmWeightOptional, mmXScaleOptional, mmWeightScaleOptional, gmmY, mmYOptional,
                               permuteOutOptional),
              ACLNN_ERR_PARAM_INVALID);
    // 8.检查Quant
    CHECK_RET(CheckQuantValid(gmmXQuantMode, gmmWeightQuantMode, gmmXScale, gmmWeightScale,
                              mmXQuantMode, mmWeightQuantMode, mmXScaleOptional, mmWeightScaleOptional),
              ACLNN_ERR_PARAM_INVALID);
    // 检查shape
    CHECK_RET(CheckShape(gmmX, gmmWeight, gmmXScale, gmmWeightScale, mmXOptional, mmWeightOptional,
                         mmXScaleOptional, mmWeightScaleOptional, gmmY, mmYOptional, epWorldSize),
              ACLNN_ERR_PARAM_INVALID);
    // 6.检查输入的数据格式是否为ND
    CHECK_RET(CheckFormat(gmmX, gmmWeight, gmmXScale, gmmWeightScale, mmXOptional, mmWeightOptional, mmXScaleOptional, mmWeightScaleOptional, gmmY, mmYOptional), ACLNN_ERR_PARAM_INVALID);
...
extern "C" aclnnStatus aclnnQuantAlltoAllvGroupedMatMulGetWorkspaceSize(
    ...
    const aclIntArray *sendCounts, const aclIntArray *recvCounts, bool transGmmWeight, bool transMmWeight, int64_t groupSize,
    ...
    uint64_t *workspaceSize, aclOpExecutor **executor)
{
    aclnnStatus ret_param = CheckParams(
        ...
        mmXQuantMode, mmWeightQuantMode, group, epWorldSize,
    ...
    aclnnStatus ret = aclnnInnerAlltoAllvGroupedMatMulGetWorkspaceSize(
        ...
        gmmXScale, gmmWeightScale, mmXScaleOptional, mmWeightScaleOptional, group, epWorldSize,
        sendCounts, recvCounts, transGmmWeight, transMmWeight, permuteOutFlag, gmmXQuantMode, gmmWeightQuantMode,
        mmXQuantMode, mmWeightQuantMode, gmmY, mmYOptional, permuteOutOptional,
