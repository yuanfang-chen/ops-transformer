...
/* !
 * \file allto_allv_grouped_mat_mul_tt_quant_tiling.cc
 * \brief
 */
...
#include "allto_allv_grouped_mat_mul_tt_quant_tiling.h"
...
namespace optiling {
bool AlltoAllvGmmTTQuantTiling::IsCapable()
{
    ...
    isHif8_ = true;
    OP_LOGD(context_->GetNodeName(), "AlltoAllvGmmTTQuantTiling is capable.");
    return true;
}

uint64_t AlltoAllvGmmTTQuantTiling::GetTilingKey() const
{
    uint64_t tilingKey = GET_TPL_TILING_KEY(ADD_TPL_HIF8, hasSharedExpertFlag_, transGmmWeight_, transMmWeight_);
    return tilingKey;
}

void AlltoAllvGmmTTQuantTiling::SetTilingParams(Mc2GroupedMatmulTilingData::GMMQuantTilingData &gmmQuantTilingData, uint64_t M, uint64_t N, uint64_t K) const
{
...
ge::graphStatus AlltoAllvGmmTTQuantTiling::CheckGmmDType() const
{
...
ge::graphStatus AlltoAllvGmmTTQuantTiling::CheckMmDType() const
{
...
ge::graphStatus AlltoAllvGmmTTQuantTiling::CheckQuantMode() const
{
    ...
    OP_TILING_CHECK(gmmXQuantMode != PERTENSOR_QUANT_MODE,
        OP_LOGE(context_->GetNodeName(), "gmmXQuantMode should be 1(pertensor mode)."), return ge::GRAPH_FAILED);
    // gmmWeightQuantMode
    ...
    OP_TILING_CHECK(gmmWeightQuantMode != PERTENSOR_QUANT_MODE,
        OP_LOGE(context_->GetNodeName(), "gmmWeightQuantMode should be 1(pertensor mode)."),
        return ge::GRAPH_FAILED);
    ...
    if (hasSharedExpertFlag_) {
        ...
        OP_TILING_CHECK(mmXQuantMode != PERTENSOR_QUANT_MODE,
            OP_LOGE(context_->GetNodeName(), "mmXQuantMode should be 1(pertensor mode)."), return ge::GRAPH_FAILED);
        // mmWeightQuantMode
        ...
        OP_TILING_CHECK(mmWeightQuantMode != PERTENSOR_QUANT_MODE,
            OP_LOGE(context_->GetNodeName(), "mmWeightQuantMode should be 1(pertensor mode)."),
            return ge::GRAPH_FAILED);
        return ge::GRAPH_SUCCESS;
    }
...
REGISTER_OPS_TILING_TEMPLATE(AlltoAllvGroupedMatMul, AlltoAllvGmmTTQuantTiling, 1);
} // namespace optiling