...
{
    OP_LOGE(A_INNER_DEBUG, "CheckerSetMNK 1.");
    maxM_ = maxM;
    maxN_ = maxN;
    maxK_ = maxK;
...
ge::graphStatus AlltoAllvGmmChecker::CheckSendRecvDataVolumn(const gert::TilingContext *context) const
{
    ...
    OP_LOGE(A_INNER_DEBUG, "CheckSendRecvDataVolumn 2.");
...
{
    OP_LOGE(A_INNER_DEBUG, "CheckAttrsShapeRelation start.");
    auto attrs = context->GetAttrs();
    OP_TILING_CHECK(attrs == nullptr, OP_LOGE(A_INNER_DEBUG, "GetAttrs returned null."), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(sendCountsSum != tilingData->taskTilingInfo.BSK,
        OP_LOGE(A_INNER_DEBUG,
            ...
            sendCountsSum,
            tilingData->taskTilingInfo.BSK),
        return ge::GRAPH_FAILED);

    const uint64_t *recvCounts = static_cast<const uint64_t *>(recvCountsPtr->GetData());
    uint64_t recvCountsSum = std::accumulate(recvCounts, recvCounts + recvCountsSize, 0ULL);
    OP_TILING_CHECK(recvCountsSum != tilingData->taskTilingInfo.A,
        OP_LOGE(A_INNER_DEBUG,
...
{
    OP_LOGE(A_INNER_DEBUG, "CheckShapeRelation 1.");
    OP_TILING_CHECK(
    ...
    auto attrs = context->GetAttrs();
    auto transGmmWeightPtr = attrs->GetAttrPointer<bool>(ATTR_TRANS_GMM_WEIGHT_INDEX);
    uint64_t gmmWeightH1 = *transGmmWeightPtr
                               ? context->GetInputShape(GMM_WEIGHT_INDEX)->GetStorageShape().GetDim(NUM_TWO)
                               : context->GetInputShape(GMM_WEIGHT_INDEX)->GetStorageShape().GetDim(1);
    ...
    if (mmXStorageShape != nullptr) {
        ...
        auto transMmWeightPtr = attrs->GetAttrPointer<bool>(ATTR_TRANS_MM_WEIGHT_INDEX);
        uint64_t mmWeightH2 = *transMmWeightPtr
                                  ? context->GetOptionalInputShape(MM_WEIGHT_INDEX)->GetStorageShape().GetDim(1)
                                  : context->GetOptionalInputShape(MM_WEIGHT_INDEX)->GetStorageShape().GetDim(0);
    ...
    OP_LOGE(A_INNER_DEBUG, "CheckShapeRelation 2.");
...
{
    OP_LOGE(A_INNER_DEBUG, "CheckMmShapeDims 1.");
    const gert::StorageShape* mmXStorageShape = context->GetOptionalInputShape(MM_X_INDEX);
    if (mmXStorageShape != nullptr) {
...
ge::graphStatus AlltoAllvGmmChecker::CheckShapeDims(const gert::TilingContext *context)
{
    ...
    OP_LOGE(A_INNER_DEBUG, "CheckShapeDims 2.");
    return ge::GRAPH_SUCCESS;
}
