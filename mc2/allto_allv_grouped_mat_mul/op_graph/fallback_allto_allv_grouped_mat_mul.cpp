    ...
    const bool* permuteOutFlag)
{
    OPS_ERR_IF(gmmX == nullptr,
        OP_LOGE("AlltoAllvGroupedMatMulFallback", "gmmX is null"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(gmmWeight == nullptr,
        OP_LOGE("AlltoAllvGroupedMatMulFallback", "gmmWeight is null"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(group == nullptr,
        OP_LOGE("AlltoAllvGroupedMatMulFallback", "group is null"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(epWorldSize == nullptr,
        OP_LOGE("AlltoAllvGroupedMatMulFallback", "epWorldSize is null"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(transGmmWeight == nullptr,
        OP_LOGE("AlltoAllvGroupedMatMulFallback", "transGmmWeight is null"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(transMmWeight == nullptr,
        OP_LOGE("AlltoAllvGroupedMatMulFallback", "transMmWeight is null"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(permuteOutFlag == nullptr,
        OP_LOGE("AlltoAllvGroupedMatMulFallback", "permuteOutFlag is null"), return ge::GRAPH_FAILED);
    ...
    std::vector<int64_t>& actRecvCountsSeqArray)
{
    OPS_ERR_IF(sendCounts == nullptr,
        OP_LOGE("AlltoAllvGroupedMatMulFallback", "sendCounts is null"), return ge::GRAPH_FAILED);
    OPS_ERR_IF(recvCounts == nullptr,
        OP_LOGE("AlltoAllvGroupedMatMulFallback", "recvCounts is null"), return ge::GRAPH_FAILED);
...
{
    OPS_ERR_IF(gmmY == nullptr, OP_LOGE("AlltoAllvGroupedMatMulFallback", "gmmY is null"), return ge::GRAPH_FAILED);
...
static ge::graphStatus AlltoAllvGroupedMatMulExecuteFunc(gert::OpExecuteContext* host_api_ctx)
{
    ...
    // 获取输入参数
    const gert::Tensor* gmmX = host_api_ctx->GetInputTensor(static_cast<size_t>(ops::AlltoAllvGroupedMatMulInputIdx::K_GMM_X));
    const gert::Tensor* gmmWeight = host_api_ctx->GetInputTensor(static_cast<size_t>(ops::AlltoAllvGroupedMatMulInputIdx::K_GMM_WEIGHT));
    ...
    const gert::Tensor* mmX = host_api_ctx->GetOptionalInputTensor(static_cast<size_t>(ops::AlltoAllvGroupedMatMulInputIdx::K_MM_X));
    const gert::Tensor* mmWeight = host_api_ctx->GetOptionalInputTensor(static_cast<size_t>(ops::AlltoAllvGroupedMatMulInputIdx::K_MM_WEIGHT));
    ...
    const int64_t* epWorldSize = attrs->GetInt(static_cast<size_t>(ops::AlltoAllvGroupedMatMulAttrIdx::K_EP_WORLD_SIZE));
    ...
    const bool* transGmmWeight = attrs->GetBool(static_cast<size_t>(ops::AlltoAllvGroupedMatMulAttrIdx::K_TRANS_GMM_WEIGHT));
    const bool* transMmWeight = attrs->GetBool(static_cast<size_t>(ops::AlltoAllvGroupedMatMulAttrIdx::K_TRANS_MM_WEIGHT));
    const bool* permuteOutFlag = attrs->GetBool(static_cast<size_t>(ops::AlltoAllvGroupedMatMulAttrIdx::K_PERMUTE_OUT_FLAG));

    // 输入参数和属性的校验
    ge::graphStatus ret = CheckInputsAndAttrs(gmmX, gmmWeight, group, epWorldSize, transGmmWeight, transMmWeight, permuteOutFlag);
