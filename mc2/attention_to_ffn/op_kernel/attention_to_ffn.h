...
private:
    ...
    uint32_t aivNumAlignCnt_{0};
    uint32_t syncWorkspaceOffset_{0};
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::InitByTinglingData(const AttentionToFFNTilingData *tilingData)
{
    ...
    aivNumAlignSize_ = Ceil(aivNum_ * sizeof(int32_t), UB_ALIGN) * UB_ALIGN;
    ...
    GM_ADDR layerId, GM_ADDR expertIds, GM_ADDR expertRankTable, GM_ADDR scales, GM_ADDR activeMask, GM_ADDR workspaceGM, TPipe *pipe, const AttentionToFFNTilingData *tilingData)
{
    ...
    if constexpr (isSync) {
 	    ...
 	    tpipe_->InitBuffer(syncStatusWorkspaceBuf_, aivNumAlignSize_);
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SendTokenToFFN()
{
    ...
    if constexpr (isSync) { //异步场景存储token发送状态到workspace
        DataCopyExtParams ffnStatusCopyParams = {static_cast<uint16_t>(ffnNum_), static_cast<uint32_t>(sizeof(int32_t)), 0U,
                                                 static_cast<uint32_t>((syncWorkspaceOffset_ - 1) * sizeof(int32_t)), 0U};
 	    DataCopyPad(syncStatusGMTensor_[aivId_], ffnStatusTensor_, ffnStatusCopyParams);
 	}
}
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SetFFNStatus()
{
    ...
    uint32_t endId = 0;
    uint32_t sentNum = 0;
    syncWorkspaceOffset_ = aivNum_ * WORKSPACE_ELEMENT_OFFSET / sizeof(int32_t);
    aivNumAlignCnt_ = aivNumAlignSize_ / sizeof(int32_t);
    Duplicate<int32_t>(ffnStatusTensor_, static_cast<int32_t>(0), ffnNum_); //异步场景设置状态
    ...
    Duplicate<int32_t>(syncStatusWorkspaceTensor_, static_cast<int32_t>(0), aivNumAlignCnt_); // 初始化workSpace
    SyncFunc<AscendC::HardEvent::V_MTE3>();
    for (uint32_t ffnIdx = startId; ffnIdx < endId; ++ffnIdx) {
        DataCopy(syncStatusGMTensor_[ffnIdx * syncWorkspaceOffset_], syncStatusWorkspaceTensor_, aivNumAlignCnt_);
    }
}
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SetFlagToFFN()
{
    ...
    GM_ADDR toRankAddr;
    DataCopyExtParams syncStatusParams = {1U, static_cast<uint32_t>(aivNum_ * sizeof(int32_t)), 0U, 0U, 0U};
    DataCopyPadExtParams<int32_t> copyPadParams{false, 0U, 0U, 0U};
    SumParams params{1, aivNumAlignCnt_, aivNum_};
    for (uint32_t ffnIdx = startFFNId; ffnIdx < endFFNId; ++ffnIdx) {
        if constexpr (isSync) {
            DataCopyPad(syncStatusWorkspaceTensor_, syncStatusGMTensor_[(ffnIdx - ffnStartRankId_) * syncWorkspaceOffset_], syncStatusParams, copyPadParams);
            SyncFunc<AscendC::HardEvent::MTE2_V>();
            ...
            Sum(ffnStatusTensorFloat, syncStatusWorkspaceTensorFloat, params);
            SyncFunc<AscendC::HardEvent::V_S>();
