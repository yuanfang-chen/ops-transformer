...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SendTokenToFFN()
{
    ...
    if (startId >= totalSendNum_) {
        if constexpr (isSync) {
            ...
            DataCopy(syncStatusGMTensor_[aivId_ * aivWorkspaceOffset_ / sizeof(int32_t)], ffnStatusTensor_, ffnNumAlignCnt_);
        }
        return;
    }
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SetFlagToFFN()
{
    ...
    if constexpr (isSync) {
        tpipe_->Reset();
        tpipe_->InitBuffer(ffnStatusBuf_, ffnNumAlignSize_); // ffnNum_
 	    tpipe_->InitBuffer(syncStatusWorkspaceBuf_, aivNum_ * ffnNumAlignSize_);
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SetFlagInAttn()
{
    ...
    if constexpr (isSync) {
        uint32_t sendNumAlignSize = Ceil(sendNum * sizeof(int32_t), UB_ALIGN) * UB_ALIGN;
        tpipe_->InitBuffer(attnStatusBuf_, sendNumAlignSize);
    }
