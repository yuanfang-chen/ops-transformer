...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SendTokenToFFN()
{
    ...
    if constexpr (isSync) {
        ...
        DataCopy(syncStatusGMTensor_[aivId_ * ffnNumAlignHalfCnt_], ffnStatusTensor_, ffnNumAlignHalfCnt_);
    }
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SetFlagToFFN()
{
    ...
    GM_ADDR toRankAddr;
    DataCopyExtParams syncStatusParams = {static_cast<uint16_t>(aivNum_), static_cast<uint32_t>(sizeof(half)), static_cast<uint32_t>((ffnNumAlignHalfCnt_ - 1) * sizeof(half)), 0U, 0U};
