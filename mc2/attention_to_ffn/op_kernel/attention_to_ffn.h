    ...
    GM_ADDR layerId, GM_ADDR expertIds, GM_ADDR expertRankTable, GM_ADDR scales, GM_ADDR activeMask, GM_ADDR workspaceGM, TPipe *pipe, const AttentionToFFNTilingData *tilingData)
{
    ...
    if constexpr (isSync) {
 	    ...
 	    tpipe_->InitBuffer(syncStatusWorkspaceBuf_, aivNum_ * UB_ALIGN);
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SendTokenToFFN()
{
    ...
    if constexpr (isSync) {
        aivWorkspaceOffset_ = ffnNum_ * WORKSPACE_ELEMENT_OFFSET / sizeof(int32_t);
        ...
        DataCopy(syncStatusGMTensor_[aivId_ * aivWorkspaceOffset_], ffnStatusTensor_, ffnNumAlignCnt_);
    }
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SetFlagToFFN()
{
    ...
    GM_ADDR toRankAddr;
    DataCopyExtParams syncStatusParams = {static_cast<uint16_t>(aivNum_), static_cast<uint32_t>(sizeof(int32_t)), 
                                          static_cast<uint32_t>((aivWorkspaceOffset_ - 1) * sizeof(int32_t)), 0U, 0U};
    DataCopyPadExtParams<int32_t> copyPadParams{false, 0U, 0U, 0U};
    SumParams params{1, static_cast<uint32_t>(aivNum_ * STATUS_REP_STRIDE), aivNum_ * STATUS_REP_STRIDE};
    for (uint32_t ffnIdx = startFFNId; ffnIdx < endFFNId; ++ffnIdx) {
        if constexpr (isSync) {
            ...
            LocalTensor<float> syncStatusWorkspaceTensorFloat = syncStatusWorkspaceTensor_.ReinterpretCast<float>();
 	        LocalTensor<float> ffnFlagTensorFloat = ffnFlagTensor_.ReinterpretCast<float>();
 	        Sum(ffnFlagTensorFloat, syncStatusWorkspaceTensorFloat, params);
            SyncFunc<AscendC::HardEvent::V_S>();
            if (ffnFlagTensor_.GetValue(0) == 0) {
                continue;
            }
        }
        if (winContext_->userMemType == 0) {
