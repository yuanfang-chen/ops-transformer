...
constexpr uint32_t WORKSPACE_ELEMENT_OFFSET = 32;
...
private:
    ...
    __aicore__ inline void FindExpertRank(uint32_t tokenIdx, int32_t expertId);
    ...
    GlobalTensor<half> syncStatusGMTensor_;
    ...
    LocalTensor<half> ffnStatusTensor_;
    LocalTensor<half> syncStatusWorkspaceTensor_;
    ...
    uint64_t ffnNumAlignSizeHalf_{0};
    uint64_t aivNumAlignSizeHalf_{0};
    ...
    uint32_t ffnNumAlignHalfCnt_{0};
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::InitByTinglingData(const AttentionToFFNTilingData *tilingData)
{
    ...
    ffnNumAlignSizeHalf_ = Ceil(ffnNum_ * sizeof(half), UB_ALIGN) * UB_ALIGN;
    aivNumAlignSizeHalf_ = Ceil(aivNum_ * sizeof(half), UB_ALIGN) * UB_ALIGN;
    ...
    GM_ADDR layerId, GM_ADDR expertIds, GM_ADDR expertRankTable, GM_ADDR scales, GM_ADDR activeMask, GM_ADDR workspaceGM, TPipe *pipe, const AttentionToFFNTilingData *tilingData)
{
    ...
    if constexpr (isSync) {
 	  	tpipe_->InitBuffer(ffnStatusBuf_, ffnNumAlignSizeHalf_); // ffnNum_
 	    tpipe_->InitBuffer(syncStatusWorkspaceBuf_, aivNumAlignSizeHalf_);
 	    ffnStatusTensor_ = ffnStatusBuf_.Get<half>();
 	    syncStatusWorkspaceTensor_ = syncStatusWorkspaceBuf_.Get<half>();
...
template <TemplateMC2TypeClass>
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::FindExpertRank(uint32_t tokenIdx, int32_t expertId)
{
    uint64_t expRankTableOffset = expertId * expRankTableM_ + layIdsExpRankTableOffset_;
    if (tokenIdx == 0 || expRankTableOffset >= preExpRankTableOffset_ + EXPERT_TABLE_REP_STRIDE) {
        ...
        preExpRankTableOffset_ = expRankTableOffset;
    }
    ...
    if (expRankTableOffset + rankOffset >= preExpRankTableOffset_ + EXPERT_TABLE_REP_STRIDE) {
        ...
        preExpRankTableOffset_ = expRankTableOffset + rankOffset;
    }
    toRankId_ = expertRankTableGMTensor_.GetValue(expRankTableOffset + rankOffset); // 拿到当前要发送的卡号
    if (expRankTableOffset + rankOffset + 1 >= preExpRankTableOffset_ + EXPERT_TABLE_REP_STRIDE) {
        ...
        preExpRankTableOffset_ = expRankTableOffset + rankOffset + 1;
    }
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SetExpertAndRank(uint32_t tokenIdx, uint32_t tokenId, uint32_t topkId)
{
    ...
    FindExpertRank(tokenIdx, dstExpertId_); // 查表获取当前要发送的rankId以及localExpId
    statusTensor_.SetValue(tokenIdx * STATUS_REP_STRIDE, localExpId_);
    if constexpr (isSync) { // 异步场景记录FFN节点发送状态
        if (tokenIdx == 0) {
            SyncFunc<AscendC::HardEvent::MTE3_S>(); // 等待前面workspace初始化
        }
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SendTokenToFFN()
{
    ...
    ffnNumAlignHalfCnt_ = ffnNumAlignSizeHalf_ / sizeof(half);
    ...
    if constexpr (isSync) {
        aivWorkspaceOffset_ = ffnNum_ * WORKSPACE_ELEMENT_OFFSET / sizeof(half);
        syncStatusGMTensor_.SetGlobalBuffer((__gm__ half*)syncStatusWorkspaceGM_);
        Duplicate<half>(ffnStatusTensor_, static_cast<half>(0), ffnNumAlignHalfCnt_); // 初始化workSpace
        SyncFunc<AscendC::HardEvent::V_MTE3>();
        DataCopy(syncStatusGMTensor_[aivId_ * aivWorkspaceOffset_], ffnStatusTensor_, ffnNumAlignHalfCnt_);
    }
...
__aicore__ inline void AttentionToFFN<TemplateMC2TypeFunc>::SetFlagToFFN()
{
    ...
    GM_ADDR toRankAddr;
    DataCopyExtParams syncStatusParams = {static_cast<uint16_t>(aivNum_), static_cast<uint32_t>(sizeof(half)), static_cast<uint32_t>((aivWorkspaceOffset_ - 1) * sizeof(half)), 0U, 0U};
    DataCopyPadExtParams<half> copyPadParams{false, 0U, 0U, 0U};
    SumParams params{1, static_cast<uint32_t>(aivNumAlignSizeHalf_ / sizeof(half)), aivNum_};
    for (uint32_t ffnIdx = startFFNId; ffnIdx < endFFNId; ++ffnIdx) {
        if constexpr (isSync) {
            DataCopyPad(syncStatusWorkspaceTensor_, syncStatusGMTensor_[ffnIdx - ffnStartRankId_], syncStatusParams, copyPadParams);
            SyncFunc<AscendC::HardEvent::MTE2_V>();
            Sum(ffnStatusTensor_, syncStatusWorkspaceTensor_, params);
            SyncFunc<AscendC::HardEvent::V_S>();
            if (static_cast<int32_t>(ffnStatusTensor_.GetValue(0)) == 0) {
                continue;
            }
        }
        if (winContext_->userMemType == 0) {
