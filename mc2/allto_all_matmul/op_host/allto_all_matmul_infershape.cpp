...
static ge::graphStatus CheckShapeForAlltoAllMatmul(const gert::InferShapeContext* context, AlltoAllMatmulShapeInfo& shape)
{
    ...
    OP_LOGD(INNER_DEBUG, "x1 dim0 is: %ld, x1 dim1 is: %ld, x2 dim0 is: %ld, x2 dim1 is: %ld, transX2 is: %d, ",
            x1Shape->GetDim(0U), x1Shape->GetDim(1U), x2Shape->GetDim(0U), x2Shape->GetDim(1U), transX2);
