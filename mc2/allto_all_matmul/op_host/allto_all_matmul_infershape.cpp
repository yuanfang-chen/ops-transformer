...
{
    OP_LOGI(INNER_DEBUG, "Check x shape in all_to_all_matmul.");
    ...
    OPS_CHECK(x1Shape->GetDimNum() != DIM_TWO, CUBE_INNER_ERR_REPORT(INNER_DEBUG,
              "In AlltoAllMatmul, x1 shape should be %ld, but the actual value is %ld.", DIM_TWO, x1Shape->GetDimNum()),
    ...
    OPS_CHECK(x2Shape->GetDimNum() != DIM_TWO, CUBE_INNER_ERR_REPORT(INNER_DEBUG,
              "In AlltoAllMatmul, x2 shape should be %ld, but the actual value is %ld.", DIM_TWO, x2Shape->GetDimNum()),
              return ge::GRAPH_FAILED);
...
{
    OP_LOGI(INNER_DEBUG, "Check all_to_all_axes shape in all_to_all_matmul.");
    ...
    if (alltoAllAxesPtr != nullptr) {
        ...
        OPS_CHECK((alltoAllAxes[0] != NUM_MINUS_TWO || alltoAllAxes[1] != NUM_MINUS_ONE), CUBE_INNER_ERR_REPORT(INNER_DEBUG,
                  "In AlltoAllMatmul, the value of alltoAllAxes should be [-2, -1], but the actual value is [%ld, %ld].",
 ...
 * @param shape
 */
static ge::graphStatus CheckBiasShapeForAlltoAllMatmul(const gert::InferShapeContext* context, AlltoAllMatmulShapeInfo& shape)
{
    OP_LOGI(INNER_DEBUG, "Check bias shape in all_to_all_matmul.");
    ...
    if (biasShape != nullptr) {
        ...
        OPS_CHECK(biasShape->GetDim(0) != shape.n, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                  "In AlltoAllMatmul, bias dim0 must be the same with matmul axis n, but actual bias dim0 is: %ld, axis n is: %ld",
 ...
 * @param shape
 */
static ge::graphStatus CheckXScaleShapeForAlltoAllMatmul(const gert::InferShapeContext* context, AlltoAllMatmulShapeInfo& shape)
{
    OP_LOGI(INNER_DEBUG, "Check x_scale shape in all_to_all_matmul.");
    ...
    if (x1ShapeScale != nullptr) {
        ...
        if (*x1QuantMode == X1_MXFP8_QUANT_NUM && *x2QuantMode == X2_MXFP8_QUANT_NUM) {
            ...
            OPS_CHECK(x1ShapeScaleDimNum != DIM_ONE, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                      "In AlltoAllMatmul, x1Scale shape must be %ld, but actual value is: %ld", DIM_ONE, x1ShapeScaleDimNum),
                      return ge::GRAPH_FAILED);
        }
        // x1Scale第0维与m轴一致
        OPS_CHECK(x1ShapeScale->GetDim(0) != shape.m, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                  "In AlltoAllMatmul, x1Scale dim0 must be the same with matmul axis m, but actual x1Scale dim0 is: %ld, axis m is: %ld",
    ...
    if (*x1QuantMode == X1_MXFP8_QUANT_NUM && *x2QuantMode == X2_MXFP8_QUANT_NUM) {
        ...
        OPS_CHECK(x2ShapeScaleDimNum != DIM_THREE, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                  "In AlltoAllMatmul, x2Scale dim num must be %ld, but actual value is: %ld", DIM_THREE, x2ShapeScaleDimNum),
                  return ge::GRAPH_FAILED);
        // x2Scale最后一维一定是2
        ...
        OPS_CHECK(x2ShapeScaleDimNum != DIM_ONE, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
                  "In AlltoAllMatmul, x2Scale shape must be %ld, but actual value is: %ld", DIM_ONE, x2ShapeScaleDimNum),
                  return ge::GRAPH_FAILED);
    }
    // x2Scale第0维与n轴一致
    OPS_CHECK(x2ShapeScale->GetDim(0) != shape.n, CUBE_INNER_ERR_REPORT(context->GetNodeName(),
              "In AlltoAllMatmul, x2Scale dim0 must be the same with matmul axis n, but actual x2Scale dim0 is: %ld, axis n is: %ld",
 ...
 * @param shape
 */
static ge::graphStatus CheckAxisKShapeForAlltoAllMatmul(const gert::InferShapeContext* context, AlltoAllMatmulShapeInfo& shape)
{
    OP_LOGI(INNER_DEBUG, "Check axis k shape in all_to_all_matmul.");
 ...
 * @param shape
 */
static ge::graphStatus GetMatmulAxisInfoForAlltoAllMatmul(const gert::InferShapeContext* context, AlltoAllMatmulShapeInfo& shape)
{
    const auto attrs = context->GetAttrs();
...
static ge::graphStatus InferShapeAlltoAllMatmul(gert::InferShapeContext* context)
{
    ...
    if (shape.m != NUM_MINUS_ONE) {
        // 静态shape图校验
        OP_LOGI(INNER_DEBUG, "Check shape for static graph pipeline in all_to_all_matmul.");
