...
static ge::graphStatus InferShapeAlltoAllMatmul(gert::InferShapeContext* context)
{
    ...
    if (all2all_out_flag) {
        if (shape.m != NUM_MINUS_ONE) {
            int64_t all2all_out_first_dim = CeilDiv(shape.m, shape.rankNum);
            int64_t all2all_out_second_dim = shape.k1 * shape.rankNum;
    ...
    if (shape.m == NUM_MINUS_ONE) {
        ...
        int64_t out_first_dim = CeilDiv(shape.m, shape.rankNum);
        int64_t out_second_dim = shape.n;
