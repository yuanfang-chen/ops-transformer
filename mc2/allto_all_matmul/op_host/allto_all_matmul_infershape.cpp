...
static ge::graphStatus CheckShapeForAlltoAllMatmul(const gert::InferShapeContext* context, AlltoAllMatmulShapeInfo& shape)
{
    ...
    if (shape.m != NUM_MINUS_ONE) {
        // 动态图拿到的shape的值都是-1，因此在infershape无法校验x1.k和x2.k/rankSize是否相等
        // 动态图的k轴校验下放到aclnn或tiling中去
        if (shape.k1 != shape.k2 / shape.rankNum) {
            OP_LOGE(context->GetNodeName(),
                    "In matmul_allto_all x1.k must be the same to x2.k / rankSize, but actual get x1.k: %ld, x2.k: %ld, rankSize: %ld",
