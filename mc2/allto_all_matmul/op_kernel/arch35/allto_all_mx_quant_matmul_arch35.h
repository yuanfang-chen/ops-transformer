    ...
    GM_ADDR workspaceGM, AlltoAllMatmulTilingDataType *tilingData, TPipe *tPipe)
{
    ...
    auto &&mc2Tiling_ = tilingData_->alltoAllMxQuantMatmulTilingInfo;
    tPipe_ = tPipe;
    x1_ = x1;
    x2_ = x2;
    y_ = y;
    ...
    if (all2all_out == nullptr) {
        transOutGM_ = commOutGM_ + mc2Tiling_.commLen;
    }
    permuteOutGM_ = commOutGM_ + mc2Tiling_.commLen + mc2Tiling_.permuteLen;
...
AlltoAllMxQuantMatmulArch35<SchedulerType, SchedulerContextType, AlltoAllMatmulTilingDataType>::ProcessTile(uint32_t taskCnt)
{
    ...
    pipeLineContext_.transposeContext->transposeDstOffset = tileMMultiRankK * (uint64_t)mc2Tiling_.rankDim * sizeof(DTYPE_X1);
    ...
    pipeLineContext_computationContext->baseData.aGM = permuteOutGM_;
    pipeLineContext_computationContext->baseData.bGM = x2_;
    pipeLineContext_computationContext->baseData.cGM = y_;
    pipeLineContext_computationContext->baseData.biasGM = bias_;
    pipeLineContext_computationContext->baseData.a_offset = tileMMultiRankK * (uint64_t)mc2Tiling_.rankDim * sizeof(DTYPE_X1);
    pipeLineContext_computationContext->baseData.b_offset = 0UL;
    pipeLineContext_computationContext->baseData.c_offset =
        (uint64_t)mc2Tiling_.tileM * (uint64_t)mc2Tiling_.rankN * sizeof(DTYPE_Y);
    pipeLineContext_computationContext->additionalData.x1_scale = x1Scale_;
    pipeLineContext_computationContext->additionalData.x1_scale_offset = 
        (uint64_t)mc2Tiling_.tileM * CeilDiv(mc2Tiling_.rankK, MXFP_GROUP_SIZE) * NUM_TWO * sizeof(AscendC::fp8_e8m0_t);
    pipeLineContext_computationContext->additionalData.x2_scale = x2Scale_;
    pipeLineContext_.computationContext->tilingDataPtr = &(tilingData_->mc2MxQuantMmTileTilingData);
...
AlltoAllMxQuantMatmulArch35<SchedulerType, SchedulerContextType, AlltoAllMatmulTilingDataType>::ProcessTail(uint32_t taskCnt)
{
    ...
    pipeLineContext_computationContext->baseData.aGM = permuteOutGM_ + tileCntMultitileMMultiRankK * sizeof(DTYPE_X1) * mc2Tiling_.rankDim;
    pipeLineContext_computationContext->baseData.bGM = x2_;
    pipeLineContext_computationContext->baseData.cGM = y_ + (uint64_t)mc2Tiling_.tileCnt * mc2Tiling_.tileM * mc2Tiling_.rankN * sizeof(DTYPE_Y);
    pipeLineContext_computationContext->baseData.a_offset = tailMMultiRankK * (uint64_t)mc2Tiling_.rankDim * sizeof(DTYPE_X1);
    pipeLineContext_computationContext->baseData.b_offset = 0UL;
    pipeLineContext_computationContext->baseData.c_offset = (uint64_t)mc2Tiling_.tailM * mc2Tiling_.rankN * (uint64_t)sizeof(DTYPE_Y);
    pipeLineContext_computationContext->additionalData.x1_scale =
        x1Scale_ + mc2Tiling_.tileCnt * mc2Tiling_.tileM * CeilDiv(mc2Tiling_.rankK, MXFP_GROUP_SIZE) * NUM_TWO * sizeof(AscendC::fp8_e8m0_t);
    pipeLineContext_computationContext->additionalData.x1_scale_offset =
        (uint64_t)mc2Tiling_.tailM * CeilDiv(mc2Tiling_.rankK, MXFP_GROUP_SIZE) * NUM_TWO * sizeof(AscendC::fp8_e8m0_t);
    pipeLineContext_computationContext->additionalData.x2_scale = x2Scale_;
    pipeLineContext_.computationContext->tilingDataPtr = &(tilingData_->mc2MxQuantMmTailTilingData);
