    ...
    GM_ADDR workspaceGM, AlltoAllMatmulTilingDataType *tilingData, TPipe *tPipe)
{
    ...
    // MX场景下，通信数据的存储顺序是commX1Scale、commX1，转置后的存储顺序是transX1ScaleGM1_、transOutGM_
    commX1ScaleGM1_ = workspaceGM;
    commOutGM_ = workspaceGM + CeilDiv(mc2Tiling_.rankK, MXFP_GROUP_SIZE) * NUM_TWO * 
                 (uint64_t)mc2Tiling_.rankM * sizeof(AscendC::fp8_e8m0_t);
    transX1ScaleGM1_ = all2all_out;
    transOutGM_ = all2all_out + CeilDiv(mc2Tiling_.rankK, MXFP_GROUP_SIZE) * NUM_TWO *
                 (uint64_t)mc2Tiling_.rankM * sizeof(AscendC::fp8_e8m0_t);
...
AlltoAllMxQuantMatmulArch35<SchedulerType, SchedulerContextType, AlltoAllMatmulTilingDataType>::ProcessScale(){
    ...
    pipeLineContext_.communicationContext->sendCount = 
        (uint64_t)mc2Tiling_.rankM * CeilDiv(mc2Tiling_.rankK, MXFP_GROUP_SIZE) * NUM_TWO; 
    ...
    pipeLineContext_.transposeContext->innerAxis = CeilDiv(mc2Tiling_.rank, MXFP_GROUP_SIZE);
    pipeLineContext_.transposeContext->transM = (uint64_t)mc2Tiling_.rankM;
    pipeLineContext_.transposeContext->innerOffsetIn = CeilDiv(mc2Tiling_.rankK, MXFP_GROUP_SIZE);
    pipeLineContext_.transposeContext->innerOffsetOut = CeilDiv(mc2Tiling_.rankK, MXFP_GROUP_SIZE) * mc2Tiling_.rankDim;
