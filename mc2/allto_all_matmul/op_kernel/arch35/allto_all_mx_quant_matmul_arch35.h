...
AlltoAllMxQuantMatmulArch35<SchedulerType, SchedulerContextType, AlltoAllMatmulTilingDataType>::ProcessTile(uint32_t taskCnt)
{
    ...
    pipeLineContext_.computationContext->additionalData.x1_scale_offset = 
        (uint64_t)mc2Tiling_.tileM * CeilDiv(mc2Tiling_.rankK * (uint64_t)mc2Tiling_.rankDim, MXFP_GROUP_SIZE) * NUM_TWO * sizeof(AscendC::fp8_e8m0_t);
...
AlltoAllMxQuantMatmulArch35<SchedulerType, SchedulerContextType, AlltoAllMatmulTilingDataType>::ProcessTail(uint32_t taskCnt)
{
    ...
    pipeLineContext_.computationContext->additionalData.x1_scale =
        x1Scale_ + mc2Tiling_.tileCnt * mc2Tiling_.tileM * CeilDiv(mc2Tiling_.rankK * (uint64_t)mc2Tiling_.rankDim, MXFP_GROUP_SIZE) * NUM_TWO * sizeof(AscendC::fp8_e8m0_t);
    pipeLineContext_.computationContext->additionalData.x1_scale_offset =
