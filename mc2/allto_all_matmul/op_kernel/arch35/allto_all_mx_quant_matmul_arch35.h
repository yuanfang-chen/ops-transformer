    ...
    GM_ADDR workspaceGM, AlltoAllMatmulTilingDataType *tilingData, TPipe *tPipe)
{
    ...
    // MX场景下，通信数据的存储顺序是commX1Scale、commX1、transX1ScaleGM1_
    commX1ScaleGM1_ = workspaceGM;
    uint64_t x1ScaleLen = CeilDiv(mc2Tiling_.rankK * (uint64_t)mc2Tiling_.rankDim, MXFP_GROUP_SIZE) * NUM_TWO * 
                 (uint64_t)mc2Tiling_.rankM /  (uint64_t)mc2Tiling_.rankDim * sizeof(AscendC::fp8_e8m0_t);
...
AlltoAllMxQuantMatmulArch35<SchedulerType, SchedulerContextType, AlltoAllMatmulTilingDataType>::ProcessScale(){
    ...
    // x1Scale的通信元素个数为rankM * (rankK * rankDim)整除64 * 2
    pipeLineContext_.communicationContext->sendCount = (uint64_t)mc2Tiling_.rankM / mc2Tiling_.rankDim * rankKdivByGroupSize; 
    ...
    // x1Scale转置相关地址和偏移
    pipeLineContext_.transposeContext->transposeSrcAddr = commX1ScaleGM1_;
    pipeLineContext_.transposeContext->transposeDstAddr = transX1ScaleGM1_;
    pipeLineContext_.transposeContext->nextSrcBlockOffset = (uint64_t)mc2Tiling_.rankM / mc2Tiling_.rankDim * rankKdivByGroupSize;
    pipeLineContext_.transposeContext->nextDstBlockOffset = rankKdivByGroupSize;
    pipeLineContext_.transposeContext->rankCnt = (uint64_t)mc2Tiling_.rankDim;
    pipeLineContext_.transposeContext->innerAxis = rankKdivByGroupSize;
    pipeLineContext_.transposeContext->transM = (uint64_t)mc2Tiling_.rankM / mc2Tiling_.rankDim;
    pipeLineContext_.transposeContext->innerOffsetIn = rankKdivByGroupSize;
    pipeLineContext_.transposeContext->innerOffsetOut = rankKdivByGroupSize * mc2Tiling_.rankDim;
...
AlltoAllMxQuantMatmulArch35<SchedulerType, SchedulerContextType, AlltoAllMatmulTilingDataType>::ProcessTail(uint32_t taskCnt)
{
    ...
    pipeLineContext_.computationContext->baseData.aGM = transOutGM_ + tileCntMultitileMMultiRankK * sizeof(DTYPE_X1) * mc2Tiling_.rankDim;
    pipeLineContext_.computationContext->baseData.bGM = x2_;
    ...
    pipeLineContext_.computationContext->additionalData.x1Scale = 
        transX1ScaleGM1_ + mc2Tiling_.tileCnt * mc2Tiling_.tileM * CeilDiv(mc2Tiling_.rankK * (uint64_t)mc2Tiling_.rankDim, MXFP_GROUP_SIZE) * 
        NUM_TWO * sizeof(AscendC::fp8_e8m0_t);
    pipeLineContext_.computationContext->additionalData.x1ScaleOffset =
        (uint64_t)mc2Tiling_.tailM * CeilDiv(mc2Tiling_.rankK * (uint64_t)mc2Tiling_.rankDim, MXFP_GROUP_SIZE) * NUM_TWO * sizeof(AscendC::fp8_e8m0_t);
    pipeLineContext_.computationContext->additionalData.x2Scale = x2Scale_;
