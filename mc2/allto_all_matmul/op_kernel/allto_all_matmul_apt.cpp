...
#define ALLTO_ALL_MATMUL_A3_FP_IMPL(tilingData, pipe)                                                                 \
    do {                                                                                                               \
        DEFINE_MC2_HCCL_FOR_COMMUNICATION(HcclServerType::HCCL_SERVER_TYPE_AICPU, 0, 1, AlltoAllMatmulTilingDataA3,        \
                                          CommunicationType);                                                          \
        ...
        DEFINE_MC2_MATMUL_FOR_MATMUL_COMPUTATION_A3_FP(Mc2MatmulV3TilingData, ComputationType);                           \
        ...
        AlltoAllMatmulA3<SchedulerType, SchedulerContextType, AlltoAllMatmulTilingDataA3> op(&SchedulerImpl);        \
...
#define ALLTO_ALL_KC_QUANT_MATMUL_IMPL(tilingData, pipe, MMDataTypeX1)                                                 \
    do {                                                                                                               \
        DEFINE_MC2_HCCL_FOR_COMMUNICATION(HcclServerType::HCCL_SERVER_TYPE_CCU, 0, 1, AlltoAllQuantMatmulTilingData, \
                                          CommunicationType);                                                          \
        CommunicationType commImplName(&tilingData);                                                                   \
        DEFINE_MC2_TRANSPOSE_FOR_MATH_COMPUTATION(DTYPE_X1, TransposeType);                                            \
        TransposeType transposeImplName(&pipe);                                                                        \
