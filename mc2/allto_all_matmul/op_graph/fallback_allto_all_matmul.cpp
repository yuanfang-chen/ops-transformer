...
inline ge::graphStatus GetCommonMatmulInputPara(const gert::OpExecuteContext* host_api_ctx, CommonMatmulParas& para)
{
    ...
    if (x1QuantMode == X1_MX_QUANT_MODE_NUM && x2QuantMode == X2_MX_QUANT_MODE_NUM) {
        ...
        const bool x2_trans = (transX2Ptr != nullptr ? *transX2Ptr : false);
        para.x2Acl = ConvertMmType(x2, x2_trans);
    }
...
inline ge::graphStatus GetAttrPara(const gert::OpExecuteContext* host_api_ctx, AttrParas& para)
{
    ...
    const int64_t* x1_quant_dtype_ptr = attrs->GetInt(INDEX_ATTR_X1_QUANT_DTYPE);
    para.x1QuantDtype = (x1_quant_dtype_ptr != nullptr ? *x1_quant_dtype_ptr : static_cast<uint64_t>(ge::DataType::DT_UNDEFINED));
    ...
    const bool* alltoall_out_flag_ptr = attrs->GetBool(INDEX_ATTR_ALLTOALL_OUT_FLAG);
    para.alltoAllOutFlag = (alltoall_out_flag_ptr != nullptr ? *alltoall_out_flag_ptr : true);
...
inline ge::graphStatus GetQuantMatmulPara(const gert::OpExecuteContext* host_api_ctx, QuantMatmulParas& para)
{
    ...
    if (x1QuantMode == X1_MX_QUANT_MODE_NUM && x2QuantMode == X2_MX_QUANT_MODE_NUM) {
        ...
        const bool* transX2Ptr = attrs->GetBool(INDEX_ATTR_TRANS_X2);
        const bool x2_trans = (transX2Ptr != nullptr ? *transX2Ptr : false);
        para.x2ScaleAcl = ConvertMmType(x2Scale, x2_trans);
    }
...
static ge::graphStatus AlltoAllMatmulExecuteFunc(gert::OpExecuteContext* host_api_ctx)
{
    ...
    if (x1QuantMode == 0 && x2QuantMode == 0) {
        const auto ret = EXEC_OPAPI_CMD(aclnnAlltoAllMatmul, mm_para.x1Acl, mm_para.x2Acl, mm_para.bias, actSeqArray, attr_para.group, attr_para.transposeX1,
        ...
        const auto ret = EXEC_OPAPI_CMD(aclnnAlltoAllQuantMatmul, mm_para.x1Acl, mm_para.x2Acl, mm_para.bias, quant_matmul_para.x1ScaleAcl, quant_matmul_para.x2ScaleAcl,
