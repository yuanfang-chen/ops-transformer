...
## 参数说明​
 ...
 <table style="undefined;table-layout: fixed; width: 1576px"><colgroup>
  ...
  <col style="width: 170px">
    ...
    <td>x1_scale</td>
    <td>可选输入</td>
    <td>左矩阵的量化系数，当前仅在全量化场景下需要配置，对应公式中的x1Scale。</td>
    <td>FLOAT32</td>
    <td>ND</td>
    </tr>
    <tr>
    ...
    <td>all2all_axes</td>
    <td>可选属性</td>
    <td>AlltoAll和Pemute数据交换的方向，支持配置空或者[-2, -1]，传入空时默认按[-2, -1]处理，表示将输入由(BS, H)转为(BS / rankSize, H * rankSize)。</td>
    ...
    <td>transpose_x2</td>
    <td>可选属性</td>
    <td>标识右矩阵是否转置过，配置为True时右矩阵Shape为(N，H)，不支持和PyTorch中的.t()方法同时使用。</td>
    <td>bool</td>
    <td>-</td>
    </tr>
    <tr>
...
* 输入x1必须是2维，其shape为(BS, H)，BS必须整除NPU卡数，BS和N的值不得超过2147483647(INT32_MAX)，不支持非连续输入。
* 输入x2必须是2维，其shape为(H*rankSize, N)，H*rankSize范围根据芯片型号和场景不同有不同约束，详见[非量化aclnn约束说明](./docs/aclnnAlltoAllQuantMatmul.md/#约束说明)
  和[量化aclnn约束说明](./docs/aclnnAlltoAllMatmul.md/#约束说明)，支持非连续输入。
* bias和x2_scale若非空，其维度必须为1维，shape为(N，)。
* x1_scale若非空，其维度必须为1维，shape为(BS/rankSize，)。
* all2all_axes为1维数组，shape必须为(2)。
* 目前支持左矩阵perToken量化和perToken动态量化，x1QuantMode=3或7；右矩阵perChannel量化，x2QuantMode=2。
* 目前支持非量化场景下输入左矩阵第一维度为0的空tensor，其它空tensor均不支持。
* 非量化场景x1、x2计算输入的数据类型要和output、alltoAllOutOptional计算输出的数据类型一致，传入的x1、x2或者output不为空指针。
* 量化场景传入的x1、x2、x2Scale或者output不为空指针，且
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：biasOptional不为空指针。
* x1、x2、bias和output计算输入输出的数据类型根据不同场景有不同的限制：
    - 非量化场景下：
        - x1/x2计算输入的数据类型为FLOAT16时，bias计算输入的数据类型支持FLOAT16和FLOAT32；x1/x2计算输入的数据类型为BFLOAT16时，bias计算输入的数据类型支持BFLOAT16和FLOAT32。
    - 量化场景下：
        - 详见[量化aclnn约束说明](./docs/aclnnAlltoAllMatmul.md/#约束说明)。
...
| 调用方式   | 样例代码                                                                                      | 说明                                         |
| ---------------- |-------------------------------------------------------------------------------------------| --------------------------------------------------- |
| aclnn接口  | [test_aclnn_allto_all_matmul.cpp](./examples/test_aclnn_allto_all_matmul.cpp)             | 通过[aclnnAlltoAllMatMul](./docs/aclnnMatmulAlltoAll.md)接口方式调用非量化场景的AlltoAllMatMul算子。 |
| aclnn接口  | [test_aclnn_allto_all_quant_matmul.cpp](./examples/test_aclnn_allto_all_quant_matmul.cpp) | 通过[aclnnAlltoAllQuantMatMul](./docs/aclnnlAlltoAllQuantMatmu.md)接口方式调用量化场景的AlltoAllMatMul算子。 |