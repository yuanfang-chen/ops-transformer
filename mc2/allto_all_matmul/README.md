...
- 计算公式：假设x1输入shape为(BS, H)，mx量化场景下x1Scale输入shape为(BS, ceil(H/64), 2)，rankSize为NPU卡数
    ...
    - <term>Ascend 950PR/Ascend 950DT</term>：
      ...
      - mx量化场景：
        ...
        out[m,n] = \sum_{j=0}^{kLoops-1} ((\sum_{k=0}^{gsK-1} (permutedOutSlice * x2Slice))* (permutedX1Scale[m/gsM, j] * x2Scale[j, n/gsN]))+bias[n]
        $$
        其中，gsM，gsN和gsK分别代表groupSizeM，groupSizeN和groupSizeK；permutedOutSlice代表通信后x1第m行长度为groupSizeK的向量，x2Slice代表x2第n列长度为groupSizeK的向量；K轴均从j*groupSizeK起始切片，j的取值范围[0, kLoops)，kLoops = ceil(K / groupSizeK)，K为K轴长度，等于H/*rankSize，支持最后的切片长度不足groupSizeK。对于mx量化模式，[groupSizeM，groupSizeN，groupSizeK]取值组合仅支持[1，1，32]。
