...
// setup & teardown
class Arch35TilingTest : public testing::TestWithParam<AlltoAllMatmulTestParam> {
protected:
...
TEST_P(Arch35TilingTest, AlltoAllMatmulGeneralCases)
{
    TestOneParamCase(GetParam());
}

TEST_F(Arch35TilingTest, AlltoAllMatmulGeneralCasesMultiThread)
{
    TestExecMultiThread(testCases, sizeof(testCases) / sizeof(AlltoAllMatmulTestParam), 1);
}

INSTANTIATE_TEST_CASE_P(AlltoAllMatmulTilingUT, Arch35TilingTest, testing::ValuesIn(testCases));
