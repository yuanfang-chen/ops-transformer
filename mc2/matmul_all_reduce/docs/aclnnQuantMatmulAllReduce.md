...
- **参数说明**
    ...
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：
        ...
        - x2输入的数据格式支持ND（当前版本仅支持二维输入）和FRACTAL\_NZ格式（当前版本仅支持四维输入）。当x2的数据格式为FRACTAL\_NZ时，配合`aclnnCalculateMatmulWeightSizeV2`和`aclnnTransMatmulWeight`完成数据格式ND到数据格式NZ的转换，非连续的tensor仅支持transpose场景。
