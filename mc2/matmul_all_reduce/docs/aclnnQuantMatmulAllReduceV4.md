...
- **接口功能**：兼容aclnnQuantMatmulAllReduce、aclnnQuantMatmulAllReduceV2、aclnnQuantMatmulAllReduceV3支持的功能，在此基础上新增perblock,pertile,mxfp量化方式的支持。新增x1，x2输入支持dtype为FLOAT8_E4M3FN、FLOAT8_E5M2、HIFLOAT8、FLOAT4_E2M1、FLOAT4_E1M2。
- **计算公式**：
...
- 确定性计算：
  - Atlas A2 训练系列产品/Atlas 800I A2 推理产品/A200I A2 Box 异构组件：aclnnQuantMatmulAllReduceV4默认非确定性实现，支持通过aclrtCtxSetSysParamOpt开启确定性。
  - 昇腾910_95 AI处理器：aclnnQuantMatmulAllReduceV4默认确定性实现。
