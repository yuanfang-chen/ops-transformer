...
struct MatmulAllReduceHostUtParamBase {
    ...
    MatmulAllReduceHostUtParamBase(const csv_map& csvMap)
    {
        ...
        this->is_trans_a = stoi(ReadMap(csvMap, "is_trans_a", "0"));
        this->is_trans_b = stoi(ReadMap(csvMap, "is_trans_b", "0"));
        this->comm_turn = stoi(ReadMap(csvMap, "comm_turn", "0"));
        this->antiquant_group_size = stoi(ReadMap(csvMap, "antiquant_group_size", "0"));
        this->group_size = stoi(ReadMap(csvMap, "group_size", "0"));
        this->y_dtype = stoi(ReadMap(csvMap, "y_dtype", "0"));
        this->comm_quant_mode = stoi(ReadMap(csvMap, "comm_quant_mode", "0"));
        this->expectResult = stoi(ReadMap(csvMap, "expectResult", "1")) ? ge::GRAPH_SUCCESS : ge::GRAPH_FAILED;
    }
};
...
struct MatmulAllReduceTilingUtParam: public MatmulAllReduceHostUtParamBase {
    ...
    gert::TilingContextPara::TensorDescription bias;
    gert::TilingContextPara::TensorDescription x3;
    gert::TilingContextPara::TensorDescription antiquant_scale;
    gert::TilingContextPara::TensorDescription antiquant_offset;
    gert::TilingContextPara::TensorDescription dequant_scale;
    gert::TilingContextPara::TensorDescription pertoken_scale;
    gert::TilingContextPara::TensorDescription comm_quant_scale_1;
    gert::TilingContextPara::TensorDescription comm_quant_scale_2;
        ...
        MatmulAllReduceHostUtParamBase(csvMap)
    {
        ...
        this->outputInstance.emplace_back(
            CreateTensor(csvMap, "y_shape", "y_dtype", "y_format",
                y));
        this->ranksize = stoi(ReadMap(csvMap, "ranksize", "8"));
        this->expectTilingKey = stoi(ReadMap(csvMap, "expectTilingKey", "0"));
        this->expectTilingDataHash = ReadMap(csvMap, "expectTilingDataHash", "default");
    }
...
private:
        ...
        const std::string& formatKey, gert::TilingContextPara::TensorDescription& outTd)
    {
        ...
        std::vector<int64_t> shape = GetShapeArr(shape);
