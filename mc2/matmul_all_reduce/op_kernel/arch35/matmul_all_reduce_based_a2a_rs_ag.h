...
public:
    ...
    __aicore__ inline void PrePareHCCL()
    {
        for (uint32_t i = 0U; i < paramInTiling_->tileCnt; i++){
            ...
            uint64_t alignedIndexOffsetTile = CeilAlign(tileInfo_.cAddrOffset, rankNum_) * i;
        ...
        for (uint32_t i = 0U; i < paramInTiling_->tailCnt; i++){
            ...
            uint64_t alignedIndexOffsetTail = CeilAlign(tileInfo_.cAddrOffset, rankNum_) * paramInTiling_->tileCnt 
                                            + CeilAlign(tailInfo_.cAddrOffset, rankNum_) * i;
    ...
    __aicore__ inline void ReduceSumAndAllGather()
    {
        if ASCEND_IS_AIV {
            ...
            for (int i = 0; i < paramInTiling_->tileCnt; i++){
                ...
                reduceSumInGM_ += CeilAlign(tileInfo_.cAddrOffset, rankNum_);               // reduceSumIn 切块之间是非连续的
                reduceSumOutGM_ += CeilAlign(tileInfo_.cAddrOffset, rankNum_) / rankNum_;   // reduceSumOut 切块之间是非连续的
            }
            ...
            for (int i = 0; i < paramInTiling_->tailCnt; i++){
                ...
                reduceSum_.Init(ceilDataCount, 0, rankNum_, aivNum, reduceSumInGM_, reduceSumOutGM_, tPipe_);
                reduceSum_.ExecuteReduceSum();
                reduceSumInGM_ += CeilAlign(tailInfo_.cAddrOffset, rankNum_);
                reduceSumOutGM_ += CeilAlign(tailInfo_.cAddrOffset, rankNum_) / rankNum_;
            }
        }
        SyncAll();
        if (notifyFlag_) {
