    ...
    __aicore__ inline void ReduceSumAndAllGather()
    {
        if ASCEND_IS_AIV {
            ...
            for (int i = 0; i < paramInTiling_->tileCnt; i++){
                tPipe_->Reset();
                reduceSum_.Init(tileInfo_.cOffset, 0, rankNum_, aivNum, reduceSumInGM_, reduceSumOutGM_, tPipe_);
            ...
            for (int i = 0; i < paramInTiling_->tailCnt; i++){
                tPipe_->Reset();
                reduceSum_.Init(tailInfo_.cOffset, 0, rankNum_, aivNum, reduceSumInGM_, reduceSumOutGM_, tPipe_);
                reduceSum_.ExecuteReduceSum();
