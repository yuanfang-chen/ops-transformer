...
CutResult MatmulAllReduceTilingBase::GetCutResult()
{
    ...
    if ((commQuantScaleShape1 != nullptr) && (commQuantScaleShape2 != nullptr)) { // low-bit comm
        ...
        MMAllReduceFitBalanceTiling allReduceTilingHccl(args_, KernelType::ALL_REDUCE_VIA_ALL_TO_ALL_ALL_GATHER, TopoType::STANDARD_CARD);
...
void MatmulAllReduceTilingBase::DoSplitMTiling()
{
    ...
    if (args_.enableSplitK || isKZero_ || (isPerBlock_ && isNotBatchOne && !is128Aligned) ||
        ...
        CutResult mCutAllreduce = GetCutResult();
