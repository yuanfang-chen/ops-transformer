...
{
    bool isUseAllReduceTwoShot = mc2tiling::isUseAllReduceTwoShot(args_.rankDim, npuArch_);
    OP_TILING_CHECK(
    ...
    if (isUseAllReduceTwoShot && !MutableRCSTilingData().isInputCommQuantScale) {
        uint8_t dataType = static_cast<uint8_t>(mc2tiling::ConvertGeTypeToHcclType(opName_, args_.geCType));
        OP_TILING_CHECK(
...
uint64_t QuantMatmulAllReduceTilingA5::GetTilingKey() const
{
    ...
    bool isUseAllReduceTwoShot = mc2tiling::isUseAllReduceTwoShot(args_.rankDim, npuArch_);
    bool isA2ARSAG = (isUseAllReduceTwoShot && (commDtype == COMMDTPYE_DEFAULT));
...
ge::graphStatus QuantMatmulAllReduceTilingA5::GetWorkspaceSize()
{
    ...
    if (mc2tiling::isUseAllReduceTwoShot(args_.rankDim, npuArch_) && !MutableRCSTilingData().isInputCommQuantScale) {
        OP_TILING_CHECK(
