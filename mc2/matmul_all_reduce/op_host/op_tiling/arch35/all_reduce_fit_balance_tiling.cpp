...
constexpr static double COMM_EXPANSION_TILENUM = 4;
...
{
    return ((value + 128) / 256) * 256; // align to 256
}

void MMAllReduceFitBalanceTiling::SetCommBoundTile()
{
    // Set the tile of comm bound.
    if (tilingM_.cutRes.shortTileAtBack || (ratioCalcComm_ < CALC_COMM_BOUND_BAR) ||
        (matmulPerf_.MatmulTime(mmInfo_.mValue, 1) < MM_EXPANSION_TIME)) {
        return;
    }
