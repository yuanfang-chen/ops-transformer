...
static bool CheckOutPutParam(const gert::TilingContext* context)
{
    ...
    OP_TILING_CHECK(outputDim0 != mValue,
        ...
        "m-axis should be %lu, but output's m-axis is %lu\n", ), return false);
...
static ge::graphStatus AllGatherParamsCheck(const gert::TilingContext* context)
{
    ...
    OP_TILING_CHECK(!CheckOutPutParam(context),
        VECTOR_INNER_ERR_REPORT_TILING(context->GetNodeName(), "the output is invalid"), return ge::GRAPH_FAILED);
