/**
 * Copyright (c) 2025 Huawei Technologies Co., Ltd.
 * This program is free software, you can redistribute it and/or modify it under the terms and conditions of
 * CANN Open Software License Agreement Version 2.0 (the "License").
 * Please refer to the License for details. You may not use this file except in compliance with the License.
 * THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
 * INCLUDING BUT NOT LIMITED TO NON-INFRINGEMENT, MERCHANTABILITY, OR FITNESS FOR A PARTICULAR PURPOSE.
 * See LICENSE in the root of the software repository for the full text of the License.
 */

/*!
 * \file grouped_mat_mul_allto_allv_tiling_key.h
 * \brief Quant Grouped MatMul AlltoAllV TilingKey 定义
 */
#ifndef QUANT_GROUPED_MAT_MUL_ALLTO_ALLV_TILING_KEY_H__
#define QUANT_GROUPED_MAT_MUL_ALLTO_ALLV_TILING_KEY_H__

#include "ascendc/host_api/tiling/template_argument.h"

// 量化模式宏定义
#define QUANT_MODE_NONE         0  // 无量化
#define QUANT_MODE_PER_TENSOR   1  // PerTensor 量化
#define QUANT_MODE_PER_CHANNEL  2  // PerChannel 量化
#define QUANT_MODE_PER_TOKEN    3  // PerToken 量化
#define QUANT_MODE_PER_GROUP    4  // PerGroup 量化
#define QUANT_MODE_PER_BLOCK    5  // PerBlock 量化
#define QUANT_MODE_MX_QUANT     6  // Mx Quant 量化

// 通信量化模式宏定义
#define COMM_QUANT_MODE_NONE    0  // 不量化
#define COMM_QUANT_MODE_INT8    1  // INT8 量化
#define COMM_QUANT_MODE_INT4    2  // INT4 量化

/**
 * TilingKey 定义
 * 使用 ASCENDC_TPL_ARGS_DECL 宏实现 TilingKey 到模板参数的自动转换
 *
 * 参数说明：
 * - TILINGKEY_GMM_WEIGHT_TRANS: GMM 计算权重转置场景 (0=不转置, 1=转置)
 * - TILINGKEY_SHARED_MM_WEIGHT_TRANS: 共享专家 MM 计算权重转置场景 (0=不转置, 1=转置)
 * - TILINGKEY_COMM_QUANT_MODE: 通信量化模式 (0=不量化, 1=INT8, 2=INT4)
 * - TILINGKEY_GMM_QUANT_MODE: GMM 量化模式 (0-6, 当前仅支持1)
 * - TILINGKEY_SHARED_MM_QUANT_MODE: 共享专家 MM 量化模式 (0-6, 当前仅支持1)
 */
ASCENDC_TPL_ARGS_DECL(
    QuantGroupedMatMulAlltoAllv,

    // GMM 计算转置场景
    ASCENDC_TPL_BOOL_DECL(
        TILINGKEY_GMM_WEIGHT_TRANS, 0, 1),

    // 共享专家 MM 计算转置场景
    ASCENDC_TPL_BOOL_DECL(
        TILINGKEY_SHARED_MM_WEIGHT_TRANS, 0, 1),

    // 通信量化模式：0=不量化, 1=INT8, 2=INT4
    ASCENDC_TPL_UINT_DECL(
        TILINGKEY_COMM_QUANT_MODE, ASCENDC_TPL_2_BW, ASCENDC_TPL_UI_LIST,
        COMM_QUANT_MODE_NONE, COMM_QUANT_MODE_INT8, COMM_QUANT_MODE_INT4),

    // GMM 量化模式：0=NONE, 1=PerTensor, 2=PerChannel, 3=PerToken, 4=PerGroup, 5=PerBlock, 6=MxQuant
    ASCENDC_TPL_UINT_DECL(
        TILINGKEY_GMM_QUANT_MODE, ASCENDC_TPL_4_BW, ASCENDC_TPL_UI_LIST,
        QUANT_MODE_NONE, QUANT_MODE_PER_TENSOR, QUANT_MODE_PER_CHANNEL,
        QUANT_MODE_PER_TOKEN, QUANT_MODE_PER_GROUP, QUANT_MODE_PER_BLOCK, QUANT_MODE_MX_QUANT),

    // 共享专家 MM 量化模式
    ASCENDC_TPL_UINT_DECL(
        TILINGKEY_SHARED_MM_QUANT_MODE, ASCENDC_TPL_4_BW, ASCENDC_TPL_UI_LIST,
        QUANT_MODE_NONE, QUANT_MODE_PER_TENSOR, QUANT_MODE_PER_CHANNEL,
        QUANT_MODE_PER_TOKEN, QUANT_MODE_PER_GROUP, QUANT_MODE_PER_BLOCK, QUANT_MODE_MX_QUANT)
);

/**
 * TilingKey 选择器定义
 * 定义所有有效的 TilingKey 组合
 */
ASCENDC_TPL_SEL(
    // 场景1: 无共享专家，GMM不转置，不量化
    // tiling_key num = 1
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_GMM_WEIGHT_TRANS, 0),
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_SHARED_MM_WEIGHT_TRANS, 0),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_COMM_QUANT_MODE, ASCENDC_TPL_UI_LIST, COMM_QUANT_MODE_NONE),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_GMM_QUANT_MODE, ASCENDC_TPL_UI_LIST, QUANT_MODE_PER_TENSOR),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_SHARED_MM_QUANT_MODE, ASCENDC_TPL_UI_LIST, QUANT_MODE_NONE)),

    // 场景2: 有共享专家，GMM不转置，MM转置，不量化
    // tiling_key num = 1
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_GMM_WEIGHT_TRANS, 0),
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_SHARED_MM_WEIGHT_TRANS, 1),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_COMM_QUANT_MODE, ASCENDC_TPL_UI_LIST, COMM_QUANT_MODE_NONE),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_GMM_QUANT_MODE, ASCENDC_TPL_UI_LIST, QUANT_MODE_PER_TENSOR),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_SHARED_MM_QUANT_MODE, ASCENDC_TPL_UI_LIST, QUANT_MODE_PER_TENSOR)),

    // 场景3: 无共享专家，GMM转置，不量化
    // tiling_key num = 1
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_GMM_WEIGHT_TRANS, 1),
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_SHARED_MM_WEIGHT_TRANS, 0),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_COMM_QUANT_MODE, ASCENDC_TPL_UI_LIST, COMM_QUANT_MODE_NONE),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_GMM_QUANT_MODE, ASCENDC_TPL_UI_LIST, QUANT_MODE_PER_TENSOR),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_SHARED_MM_QUANT_MODE, ASCENDC_TPL_UI_LIST, QUANT_MODE_NONE)),

    // 场景4: 有共享专家，GMM转置，MM转置，不量化
    // tiling_key num = 1
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_GMM_WEIGHT_TRANS, 1),
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_SHARED_MM_WEIGHT_TRANS, 1),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_COMM_QUANT_MODE, ASCENDC_TPL_UI_LIST, COMM_QUANT_MODE_NONE),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_GMM_QUANT_MODE, ASCENDC_TPL_UI_LIST, QUANT_MODE_PER_TENSOR),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_SHARED_MM_QUANT_MODE, ASCENDC_TPL_UI_LIST, QUANT_MODE_PER_TENSOR)),

    // 场景5-8: INT8 通信量化模式
    // tiling_key num = 4
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_GMM_WEIGHT_TRANS, 0, 1),
        ASCENDC_TPL_BOOL_SEL(TILINGKEY_SHARED_MM_WEIGHT_TRANS, 0, 1),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_COMM_QUANT_MODE, ASCENDC_TPL_UI_LIST, COMM_QUANT_MODE_INT8),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_GMM_QUANT_MODE, ASCENDC_TPL_UI_LIST, QUANT_MODE_PER_TENSOR),
        ASCENDC_TPL_UINT_SEL(TILINGKEY_SHARED_MM_QUANT_MODE, ASCENDC_TPL_UI_LIST, QUANT_MODE_NONE, QUANT_MODE_PER_TENSOR))
);

#endif // QUANT_GROUPED_MAT_MUL_ALLTO_ALLV_TILING_KEY_H__
