...
#include "../../op_kernel/moe_distribute_dispatch_v3_tiling_key.h"
#include "../../../moe_distribute_dispatch_v2/op_host/op_tiling/moe_distribute_dispatch_v2_tiling_helper.h"
...
namespace {
    ...
    // A2定义
    const char *K_INNER_DEBUG = "MoeDistributeDispatchV3 Tiling Debug";
...
namespace optiling {
static void PrintTilingDataInfo(const char *nodeName, MoeDistributeDispatchV3TilingData &tilingData)
{
    OP_LOGD(nodeName, "epWorldSize is %u.", tilingData.MoeDistributeDispatchV3Info.epWorldSize);
    OP_LOGD(nodeName, "tpWorldSize is %u.", tilingData.MoeDistributeDispatchV3Info.tpWorldSize);
    OP_LOGD(nodeName, "epRankId is %u.", tilingData.MoeDistributeDispatchV3Info.epRankId);
    OP_LOGD(nodeName, "tpRankId is %u.", tilingData.MoeDistributeDispatchV3Info.tpRankId);
    OP_LOGD(nodeName, "expertShardType is %u.", tilingData.MoeDistributeDispatchV3Info.expertShardType);
    OP_LOGD(nodeName, "sharedExpertNum is %u.", tilingData.MoeDistributeDispatchV3Info.sharedExpertNum);
    OP_LOGD(nodeName, "sharedExpertRankNum is %u.", tilingData.MoeDistributeDispatchV3Info.sharedExpertRankNum);
    OP_LOGD(nodeName, "moeExpertNum is %u.", tilingData.MoeDistributeDispatchV3Info.moeExpertNum);
    OP_LOGD(nodeName, "quantMode is %u.", tilingData.MoeDistributeDispatchV3Info.quantMode);
    OP_LOGD(nodeName, "globalBs is %u.", tilingData.MoeDistributeDispatchV3Info.globalBs);
    OP_LOGD(nodeName, "bs is %u.", tilingData.MoeDistributeDispatchV3Info.bs);
    OP_LOGD(nodeName, "k is %u.", tilingData.MoeDistributeDispatchV3Info.k);
    OP_LOGD(nodeName, "h is %u.", tilingData.MoeDistributeDispatchV3Info.h);
    OP_LOGD(nodeName, "aivNum is %u.", tilingData.MoeDistributeDispatchV3Info.aivNum);
    OP_LOGD(nodeName, "totalUbSize is %lu.", tilingData.MoeDistributeDispatchV3Info.totalUbSize);
    OP_LOGD(nodeName, "totalWinSizeEP is %lu.", tilingData.MoeDistributeDispatchV3Info.totalWinSizeEp);
    OP_LOGD(nodeName, "totalWinSizeTP is %lu.", tilingData.MoeDistributeDispatchV3Info.totalWinSizeTp);
    OP_LOGD(nodeName, "hasElastic is %d.", tilingData.MoeDistributeDispatchV3Info.hasElasticInfo);
    OP_LOGD(nodeName, "isPerformance is %d.", tilingData.MoeDistributeDispatchV3Info.isPerformance);
    OP_LOGD(nodeName, "zeroComputeExpertNum is %d", tilingData.MoeDistributeDispatchV3Info.zeroComputeExpertNum);
    OP_LOGD(nodeName, "cumSumUBMinValue is %d", tilingData.MoeDistributeDispatchV3Info.cumSumUBMinValue);
}
...
static ge::graphStatus GetAttrAndSetTilingData(const gert::TilingContext *context, const char *nodeName,
    MoeDistributeDispatchV3TilingData &tilingData, std::string &groupEp, std::string &groupTp, bool &isSetCommAlg)
{
    auto attrs = context->GetAttrs();
    ...
    if (mc2tiling::GetSocVersion(context) != "Ascend950") {
        ...
        OP_LOGD(nodeName, "MoeDistributeDispatchV3 isSetCommAlg = %d\n", isSetCommAlg);
    }

    groupEp = std::string(groupEpPtr);
    tilingData.MoeDistributeDispatchV3Info.epWorldSize = static_cast<uint32_t>(epWorldSize);
    tilingData.MoeDistributeDispatchV3Info.tpWorldSize = static_cast<uint32_t>(*tpWorldSizePtr);
    tilingData.MoeDistributeDispatchV3Info.epRankId = static_cast<uint32_t>(*epRankIdPtr);
    tilingData.MoeDistributeDispatchV3Info.tpRankId = static_cast<uint32_t>(*tpRankIdPtr);
    tilingData.MoeDistributeDispatchV3Info.expertShardType = static_cast<uint32_t>(*expertShardPtr);
    tilingData.MoeDistributeDispatchV3Info.sharedExpertNum = static_cast<uint32_t>(*sharedExpertNumPtr);
    tilingData.MoeDistributeDispatchV3Info.sharedExpertRankNum = static_cast<uint32_t>(sharedExpertRankNum);
    if (tilingData.MoeDistributeDispatchV3Info.sharedExpertRankNum == 0U) {
        if (tilingData.MoeDistributeDispatchV3Info.sharedExpertNum == 1U) {
            tilingData.MoeDistributeDispatchV3Info.sharedExpertNum = 0U;
        }
    }
    tilingData.MoeDistributeDispatchV3Info.moeExpertNum = static_cast<uint32_t>(moeExpertNum);
    tilingData.MoeDistributeDispatchV3Info.quantMode = static_cast<uint32_t>(*quantModePtr);
    tilingData.MoeDistributeDispatchV3Info.expertTokenNumsType = static_cast<uint32_t>(*expertTokenNumsTypePtr);
    tilingData.MoeDistributeDispatchV3Info.zeroComputeExpertNum = static_cast<int32_t>(zeroComputeExpertNum);
    OP_LOGD(nodeName, "MoeDistributeDispatchV3 zeroComputeExpertNum = %d\n",
        tilingData.MoeDistributeDispatchV3Info.zeroComputeExpertNum);
    ...
    if (mc2tiling::GetSocVersion(context) != "Ascend950" && isSetCommAlg) { 
        ...
        tilingData.MoeDistributeDispatchV3Info.cumSumUBMinValue = static_cast<uint32_t>(cumSumUBMinValue); 
        OP_LOGD(nodeName, "lastDim = %d, MoeDistributeDispatchV3 cumSumUBMinValue = %d\n", lastDim, 
            tilingData.MoeDistributeDispatchV3Info.cumSumUBMinValue); 
    }
    return ge::GRAPH_SUCCESS;
}

static bool CheckSharedAttrs(const char *nodeName,
    const MoeDistributeDispatchV3TilingData &tilingData)
{
    uint32_t sharedExpertNum = tilingData.MoeDistributeDispatchV3Info.sharedExpertNum;
    uint32_t sharedExpertRankNum = tilingData.MoeDistributeDispatchV3Info.sharedExpertRankNum;
...
static bool CheckCommAlgAttrs(const gert::TilingContext *context, const char *nodeName,
    const MoeDistributeDispatchV3TilingData &tilingData, bool isSetCommAlg)
{
    uint32_t tpWorldSize = tilingData.MoeDistributeDispatchV3Info.tpWorldSize;
    // 获取bs
    const gert::StorageShape *xStorageShape = context->GetInputShape(X_INDEX);
...
static ge::graphStatus CheckAttrs(const gert::TilingContext *context, const char *nodeName,
    MoeDistributeDispatchV3TilingData &tilingData, uint32_t &localMoeExpertNum, bool isActiveMask, bool isSetCommAlg)
{
    uint32_t epWorldSize = tilingData.MoeDistributeDispatchV3Info.epWorldSize;
    uint32_t tpWorldSize = tilingData.MoeDistributeDispatchV3Info.tpWorldSize;
    uint32_t moeExpertNum = tilingData.MoeDistributeDispatchV3Info.moeExpertNum;
    uint32_t sharedExpertRankNum = tilingData.MoeDistributeDispatchV3Info.sharedExpertRankNum;
    ...
    // 校验tp=2时是否没有动态缩容参数
    OP_TILING_CHECK((tpWorldSize > 1) && (tilingData.MoeDistributeDispatchV3Info.hasElasticInfo), OP_LOGE(nodeName, "Cannot support elasticInfo"
    ...
    tilingData.MoeDistributeDispatchV3Info.bs = static_cast<uint32_t>(xDim0);
    ...
    OP_LOGD(nodeName, "MoeDistributeDispatchV3 *globalBsPtr = %ld, bs = %ld, epWorldSize = %u\n",
    ...
    OP_TILING_CHECK(((*globalBsPtr > (xDim0 * static_cast<int64_t>(epWorldSize))) && isActiveMask),
        ...
        "bs=%ld, epWorldSize=%u.", *globalBsPtr, xDim0, epWorldSize), return ge::GRAPH_FAILED);
    if (*globalBsPtr == 0) {
        tilingData.MoeDistributeDispatchV3Info.globalBs = static_cast<uint32_t>(xDim0) * epWorldSize;
    } else {
        tilingData.MoeDistributeDispatchV3Info.globalBs = static_cast<uint32_t>(*globalBsPtr);
    }
...
static ge::graphStatus CheckTwoDimScalesShape(const gert::TilingContext *context, const char *nodeName,
    MoeDistributeDispatchV3TilingData &tilingData, const int64_t scalesDim0, const int64_t scalesDim1)
{
    uint32_t sharedExpertRankNum = tilingData.MoeDistributeDispatchV3Info.sharedExpertRankNum;
    uint32_t sharedExpertNum = tilingData.MoeDistributeDispatchV3Info.sharedExpertNum; 
    int64_t moeExpertNum = static_cast<int64_t>(tilingData.MoeDistributeDispatchV3Info.moeExpertNum);
    const gert::StorageShape *xStorageShape = context->GetInputShape(X_INDEX);
...
static ge::graphStatus CheckTensorShape(const gert::TilingContext *context, const char *nodeName,
    MoeDistributeDispatchV3TilingData &tilingData, const uint32_t quantMode, const bool isScales,
    const bool isSharedExpert,const bool hasElasticInfo, const bool isPerformance, const int64_t localMoeExpertNum)
{
    ...
    uint32_t A = 0U;
    uint32_t globalBs = tilingData.MoeDistributeDispatchV3Info.globalBs;
    uint32_t sharedExpertNum = tilingData.MoeDistributeDispatchV3Info.sharedExpertNum;
    uint32_t sharedExpertRankNum = tilingData.MoeDistributeDispatchV3Info.sharedExpertRankNum;
    ...
    tilingData.MoeDistributeDispatchV3Info.h = static_cast<uint32_t>(xDim1);
    ...
    tilingData.MoeDistributeDispatchV3Info.k = static_cast<uint32_t>(expertIdsDim1);

    // 校验scales的维度
    uint64_t h = tilingData.MoeDistributeDispatchV3Info.h;
    uint32_t bs = tilingData.MoeDistributeDispatchV3Info.bs;
    ...
    tilingData.MoeDistributeDispatchV3Info.scalesRow = scalesRow;
    tilingData.MoeDistributeDispatchV3Info.scalesCol = scalesCol;
    tilingData.MoeDistributeDispatchV3Info.scalesCount = scalesCount;
    tilingData.MoeDistributeDispatchV3Info.scalesTypeSize = scalesTypeSize;

    uint32_t rankNumPerSharedExpert = 0;
    uint32_t epWorldSizeU32 = tilingData.MoeDistributeDispatchV3Info.epWorldSize;
    ...
    if (hasElasticInfo) {
        ...
        const int64_t epWorldSize = static_cast<int64_t>(tilingData.MoeDistributeDispatchV3Info.epWorldSize);
    ...
    tilingData.MoeDistributeDispatchV3Info.a = A;
    ...
    // 校验expandX的维度
    int64_t tpWorldSize = static_cast<int64_t>(tilingData.MoeDistributeDispatchV3Info.tpWorldSize);
    const gert::StorageShape *expandXStorageShape = context->GetOutputShape(OUTPUT_EXPAND_X_INDEX);
    ...
    int64_t epWorldSize = static_cast<int64_t>(tilingData.MoeDistributeDispatchV3Info.epWorldSize);
    const gert::StorageShape *epRecvCountStorageShape = context->GetOutputShape(OUTPUT_EP_RECV_COUNTS_INDEX);
    const gert::StorageShape *tpRecvCountStorageShape = context->GetOutputShape(OUTPUT_TP_RECV_COUNTS_INDEX);
...
static ge::graphStatus SetHcommCfg(const gert::TilingContext *context, MoeDistributeDispatchV3TilingData *tiling,
    const std::string groupEp, const std::string groupTp, const uint32_t tpWorldSize)
{
    const char *nodeName = context->GetNodeName();
    OP_LOGD(nodeName, "MoeDistributeDispatchV3 groupEp = %s", groupEp.c_str());
    ...
    if (tpWorldSize > 1) {
        OP_LOGD(nodeName, "MoeDistributeDispatchV3 groupTp = %s", groupTp.c_str());
...
static ge::graphStatus CheckWinSize(const gert::TilingContext *context, MoeDistributeDispatchV3TilingData &tilingData,
    const char *nodeName, const bool isSetCommAlg, uint32_t &localMoeExpertNum)
{
    ...
    uint32_t sharedExpertNum = tilingData.MoeDistributeDispatchV3Info.sharedExpertNum;
    uint64_t h = static_cast<uint64_t>(tilingData.MoeDistributeDispatchV3Info.h);
    uint64_t k = static_cast<uint64_t>(tilingData.MoeDistributeDispatchV3Info.k);
    uint64_t epWorldSize = static_cast<uint64_t>(tilingData.MoeDistributeDispatchV3Info.epWorldSize);
    uint64_t maxBs = static_cast<uint64_t>(tilingData.MoeDistributeDispatchV3Info.globalBs) / epWorldSize;
    ...
    tilingData.MoeDistributeDispatchV3Info.totalWinSizeEp = maxWindowSizeEp;
    OP_LOGD(nodeName, "windowSize = %lu", maxWindowSizeEp);

    uint64_t tpWorldSize = static_cast<uint64_t>(tilingData.MoeDistributeDispatchV3Info.tpWorldSize);
    if (tpWorldSize == TP_WORLD_SIZE_TWO) {
        ...
        OP_TILING_CHECK(mc2tiling::GetCclBufferSize(groupTpHccl, &maxWindowSizeTp, nodeName) != ge::GRAPH_SUCCESS,
            ...
            return ge::GRAPH_FAILED);
        actualSize = static_cast<uint64_t>(tilingData.MoeDistributeDispatchV3Info.a) * (tokenNeedSizeDispatch +
        ...
        OP_LOGE(nodeName, "TP HCCL_BUFFSIZE is too SMALL, A = %u, tokenNeedSizeDispatch = %lu, tokenNeedSizeCombine = %lu,"
            ...
            tilingData.MoeDistributeDispatchV3Info.a,  tokenNeedSizeDispatch, tokenNeedSizeCombine, actualSize / MB_SIZE + 1UL,
            maxWindowSizeTp / MB_SIZE), return ge::GRAPH_FAILED);
        tilingData.MoeDistributeDispatchV3Info.totalWinSizeTp = maxWindowSizeTp;
...
{
    const char *nodeName = context->GetNodeName();
    MoeDistributeDispatchV3TilingData *tilingData = context->GetTilingData<MoeDistributeDispatchV3TilingData>();
    ...
    OP_LOGI(nodeName, "Enter MoeDistributeDispatchV3 tiling check func.");
    ...
    tilingData->MoeDistributeDispatchV3Info.isTokenMask = ((isActiveMask) &&
        (xActiveMaskStorageShape->GetStorageShape().GetDimNum() == ONE_DIM));
    tilingData->MoeDistributeDispatchV3Info.isExpertMask = ((isActiveMask) &&
        (xActiveMaskStorageShape->GetStorageShape().GetDimNum() == TWO_DIMS));

    // 获取elasticInfo
    const gert::StorageShape *elasticInfoStorageShape = context->GetOptionalInputShape(ELASTIC_INFO_INDEX);
    hasElasticInfo = (elasticInfoStorageShape != nullptr);
    tilingData->MoeDistributeDispatchV3Info.hasElasticInfo = hasElasticInfo;

    // 获取performanceInfo
    const gert::StorageShape *performanceInfoStorageShape = context->GetOptionalInputShape(PERFORMANCE_INFO_INDEX);
    isPerformance = (performanceInfoStorageShape != nullptr);
    tilingData->MoeDistributeDispatchV3Info.isPerformance = isPerformance;

    quantMode = tilingData->MoeDistributeDispatchV3Info.quantMode;
    ...
    uint32_t epRankId = tilingData->MoeDistributeDispatchV3Info.epRankId;
    uint32_t sharedExpertRankNum = tilingData->MoeDistributeDispatchV3Info.sharedExpertRankNum;
    ...
    tilingData->MoeDistributeDispatchV3Info.totalUbSize = ubSize;
    tilingData->MoeDistributeDispatchV3Info.aivNum = aivNum;
