...
#include "../../../op_kernel/moe_distribute_dispatch_v3.cpp"
#include "../../../op_kernel/moe_distribute_dispatch_v3_tiling_key.h"

class moe_distribute_dispatch_v3_test : public testing::Test {
protected:
    static void SetUpTestCase() {
        std::cout << "moe_distribute_dispatch_v3_test SetUp\n" << std::endl;
    }
    static void TearDownTestCase() {
        std::cout << "moe_distribute_dispatch_v3_test TearDown\n" << std::endl;
    }
};

TEST_F(moe_distribute_dispatch_v3_test, moe_distribute_dispatch_v3_test_1000) {
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    size_t sysWorkspaceSize = 16 * 1024 * 1024;
    size_t usrWorkspaceSize = 0;
    size_t allWorkspaceSize = usrWorkspaceSize + sysWorkspaceSize;
    uint8_t* workspace = (uint8_t*)AscendC::GmAlloc(allWorkspaceSize);
    size_t tilingSize = sizeof(MoeDistributeDispatchV3TilingData);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);

    MoeDistributeDispatchV3TilingData *tiling_data = reinterpret_cast<MoeDistributeDispatchV3TilingData*>(tiling);
    ...
    auto moe_distribute_dispatch_v3_warrper = [] (
        ...
        GM_ADDR expandScalesOut, GM_ADDR workspaceGM, GM_ADDR tilingGM) {
            moe_distribute_dispatch_v3<false, TILINGKEY_NO_QUANT, false, TILINGKEY_NO_FULLMESH, TILINGKEY_TPL_MTE, TILINGKEY_TPL_A3>(
    ...
    ICPU_RUN_KF(moe_distribute_dispatch_v3_warrper, 40, x, expertIds, scales, xActiveMask, expertScales, elasticInfo, 
...
TEST_F(moe_distribute_dispatch_v3_test, moe_distribute_dispatch_v3_test_2000001000) {
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    size_t sysWorkspaceSize = 16 * 1024 * 1024;
    size_t usrWorkspaceSize = 0;
    size_t allWorkspaceSize = usrWorkspaceSize + sysWorkspaceSize;
...
TEST_F(moe_distribute_dispatch_v3_test, moe_distribute_dispatch_v3_test_2000001002) {
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    size_t sysWorkspaceSize = 16 * 1024 * 1024;
    size_t usrWorkspaceSize = 0;
    size_t allWorkspaceSize = usrWorkspaceSize + sysWorkspaceSize;
    ...
    auto moe_distribute_dispatch_v3_warrper = [] (
        ...
        GM_ADDR assistInfoOut, GM_ADDR expertTokenNumsOut, GM_ADDR epSendCountsOut, GM_ADDR tpSendCountsOut, 
        GM_ADDR expandScalesOut, GM_ADDR workspaceGM, GM_ADDR tilingGM) {
            moe_distribute_dispatch_v3<false, TILINGKEY_PERTOKEN_QUANT, false, TILINGKEY_NO_FULLMESH, TILINGKEY_TPL_MTE, TILINGKEY_TPL_A3>(
                x, expertIds, scales, xActiveMask, expertScales, elasticInfo, performanceInfo, expandXOut, 
                dynamicScalesOut, assistInfoOut, expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, 
                expandScalesOut, workspaceGM, tilingGM);
        };
...
TEST_F(moe_distribute_dispatch_v3_test, moe_distribute_dispatch_v3_test_2000001012) {
    AscendC::SetKernelMode(KernelMode::MIX_MODE);
    size_t sysWorkspaceSize = 16 * 1024 * 1024;
    size_t usrWorkspaceSize = 0;
    size_t allWorkspaceSize = usrWorkspaceSize + sysWorkspaceSize;
    ...
    auto moe_distribute_dispatch_v3_warrper = [] (
        ...
        GM_ADDR assistInfoOut, GM_ADDR expertTokenNumsOut, GM_ADDR epSendCountsOut, GM_ADDR tpSendCountsOut, 
        GM_ADDR expandScalesOut, GM_ADDR workspaceGM, GM_ADDR tilingGM) {
            moe_distribute_dispatch_v3<false, TILINGKEY_PERTOKEN_QUANT, true, TILINGKEY_NO_FULLMESH, TILINGKEY_TPL_MTE, TILINGKEY_TPL_A3>(
                x, expertIds, scales, xActiveMask, expertScales, elasticInfo, performanceInfo, expandXOut, 
                dynamicScalesOut, assistInfoOut, expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, 
                expandScalesOut, workspaceGM, tilingGM);
        };
