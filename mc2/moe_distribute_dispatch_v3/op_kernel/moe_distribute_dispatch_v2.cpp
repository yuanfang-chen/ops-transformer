...
{
REGISTER_TILING_DEFAULT(MoeDistributeDispatchV3TilingData);
#if defined(__DAV_C310__)
    GET_TILING_DATA_WITH_STRUCT(MoeDistributeDispatchV3TilingData, tilingData, tilingGM);
#else
    ...
    if constexpr (ArchTag == TILINGKEY_TPL_A5) {
        if constexpr (CommMode == TILINGKEY_TPL_CCU){
            MoeDistributeDispatchA5<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::UNQUANT, false, false> op;
            ...
            MoeDistributeDispatchV3<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::UNQUANT, false, false> op;
    ...
    if constexpr (ArchTag == TILINGKEY_TPL_A5) {
        if constexpr (CommMode == TILINGKEY_TPL_CCU){
            MoeDistributeDispatchA5<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::UNQUANT, true, false> op;
            op.Init(x, expertIds, scales, xActiveMask, expandXOut, dynamicScalesOut, assistInfoOut, 
                    expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
            op.Process();
        } else if constexpr (CommMode == TILINGKEY_TPL_MTE){
            MoeDistributeDispatchV3<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::UNQUANT, true, false> op;
            op.Init(x,expertIds, scales, xActiveMask, elasticInfo, performanceInfo, expandXOut, dynamicScalesOut, assistInfoOut, 
                    expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
            op.Process();
        }
    ...
    if constexpr (ArchTag == TILINGKEY_TPL_A5) {
        if constexpr (QuantMode == TILINGKEY_STATIC_QUANT) {
            if constexpr (CommMode == TILINGKEY_TPL_CCU){
                MoeDistributeDispatchA5<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::STATIC_QUANT, true, false> op;
                op.Init(x, expertIds, scales, xActiveMask, expandXOut, dynamicScalesOut, assistInfoOut, 
                        expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
                op.Process();
            } else if constexpr (CommMode == TILINGKEY_TPL_MTE){
                MoeDistributeDispatchV3<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::STATIC_QUANT, true, false> op;
                op.Init(x,expertIds, scales, xActiveMask, elasticInfo, performanceInfo, expandXOut, dynamicScalesOut, assistInfoOut, 
                        expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
                op.Process();
            }
            ...
            if constexpr (CommMode == TILINGKEY_TPL_CCU){
                MoeDistributeDispatchA5<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::PERTOKEN_DYNAMIC_QUANT, ScaleMode, false> op;
                op.Init(x, expertIds, scales, xActiveMask, expandXOut, dynamicScalesOut, assistInfoOut, 
                        expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
                op.Process();
            } else if constexpr (CommMode == TILINGKEY_TPL_MTE){
                MoeDistributeDispatchV3<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::PERTOKEN_DYNAMIC_QUANT, ScaleMode, false> op;
                op.Init(x,expertIds, scales, xActiveMask, elasticInfo, performanceInfo, expandXOut, dynamicScalesOut, assistInfoOut, 
                        expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
                op.Process();
            }
            ...
            if constexpr (CommMode == TILINGKEY_TPL_CCU){
                MoeDistributeDispatchA5<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::PERGROUP_DYNAMIC_QUANT, ScaleMode, false> op;
                op.Init(x, expertIds, scales, xActiveMask, expandXOut, dynamicScalesOut, assistInfoOut, 
                        expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
                op.Process();
            } else if constexpr (CommMode == TILINGKEY_TPL_MTE){
                MoeDistributeDispatchV3<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::PERGROUP_DYNAMIC_QUANT, ScaleMode, false> op;
                op.Init(x,expertIds, scales, xActiveMask, elasticInfo, performanceInfo, expandXOut, dynamicScalesOut, assistInfoOut, 
                        expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
                op.Process();
            }  
            ...
            if constexpr (CommMode == TILINGKEY_TPL_CCU){
                MoeDistributeDispatchA5<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::MX_QUANT, false, false> op;
                op.Init(x, expertIds, scales, xActiveMask, expandXOut, dynamicScalesOut, assistInfoOut, 
                        expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
                op.Process();
            } else if constexpr (CommMode == TILINGKEY_TPL_MTE){
                MoeDistributeDispatchV3<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::MX_QUANT, false, false> op;
                op.Init(x,expertIds, scales, xActiveMask, elasticInfo, performanceInfo, expandXOut, dynamicScalesOut, assistInfoOut, 
                        expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
                op.Process();
            }
    ...
    if constexpr (ArchTag == TILINGKEY_TPL_A3) {
        if constexpr (FullMesh == TILINGKEY_ENABLE_FULLMESH) {
            GET_TILING_DATA_WITH_STRUCT(MoeDistributeDispatchV3TilingData, tilingData, tilingGM);
            MoeDistributeDispatchV3FullMesh<DTYPE_X, DTYPE_EXPAND_X, false, false, false, false> op;
            op.Init(x,expertIds, scales, xActiveMask, elasticInfo, performanceInfo, expandXOut, dynamicScalesOut, assistInfoOut,
                    expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
            op.Process();
            return;
            ...
            GET_TILING_DATA_WITH_STRUCT(MoeDistributeDispatchV3TilingData, tilingData, tilingGM);
            MoeDistributeDispatchV3<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::UNQUANT, false, HasTp> op;
            op.Init(x,expertIds, scales, xActiveMask, elasticInfo, performanceInfo, expandXOut, dynamicScalesOut, assistInfoOut, 
                    expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
            op.Process();
            return;
    ...
    if constexpr (ArchTag == TILINGKEY_TPL_A3) {
        if constexpr (FullMesh == TILINGKEY_ENABLE_FULLMESH) {
            if constexpr (QuantMode == TILINGKEY_STATIC_QUANT) {
                GET_TILING_DATA_WITH_STRUCT(MoeDistributeDispatchV3TilingData, tilingData, tilingGM);
                MoeDistributeDispatchV3FullMesh<DTYPE_X, DTYPE_EXPAND_X, true, false, false, false> op;
                op.Init(x,expertIds, scales, xActiveMask, elasticInfo, performanceInfo, expandXOut, dynamicScalesOut, assistInfoOut,
                        expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
                op.Process();
                return;
                ...
                GET_TILING_DATA_WITH_STRUCT(MoeDistributeDispatchV3TilingData, tilingData, tilingGM);
                MoeDistributeDispatchV3FullMesh<DTYPE_X, DTYPE_EXPAND_X, false, true, ScaleMode, false> op;
                op.Init(x,expertIds, scales, xActiveMask, elasticInfo, performanceInfo, expandXOut, dynamicScalesOut, assistInfoOut, 
                        expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
                op.Process();
                return;
            ...
            if constexpr (QuantMode == TILINGKEY_STATIC_QUANT) {
                GET_TILING_DATA_WITH_STRUCT(MoeDistributeDispatchV3TilingData, tilingData, tilingGM);
                MoeDistributeDispatchV3<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::STATIC_QUANT, false, HasTp> op;
                op.Init(x,expertIds, scales, xActiveMask, elasticInfo, performanceInfo, expandXOut, dynamicScalesOut, assistInfoOut, 
                        expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
                op.Process();
                return;
                ...
                GET_TILING_DATA_WITH_STRUCT(MoeDistributeDispatchV3TilingData, tilingData, tilingGM);
                MoeDistributeDispatchV3<DTYPE_X, DTYPE_EXPAND_X, MoeDistributeDispatchV3Impl::PERTOKEN_DYNAMIC_QUANT, ScaleMode, HasTp> op;
                op.Init(x,expertIds, scales, xActiveMask, elasticInfo, performanceInfo, expandXOut, dynamicScalesOut, assistInfoOut, 
                        expertTokenNumsOut, epSendCountsOut, tpSendCountsOut, workspaceGM, &pipe, &tilingData);
                op.Process();
                return;
