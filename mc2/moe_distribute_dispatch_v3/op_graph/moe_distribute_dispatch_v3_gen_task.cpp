...
/*!
 * \file moe_distribute_dispatch_v3_gen_task.cpp
 * \brief
 */
#include <vector>
#include <set>
...
#ifdef BUILD_OPEN_PROJECT
ge::Status MoeDistributeDispatchV3CalcParamFunc(gert::ExeResGenerationContext *context)
{
    if ((Mc2GenTaskOpsUtils::IsTargetPlatform(context->GetNodeName(), NPUARCH_A5)) &&
        (Mc2Arch35GenTaskOpsUtils::GetCommAlg(context, ATTR_INDEX_COMM_ALG_DISTRIBUTE_DISPATCH_V2) != COMM_ALG_MTE)) {
...
ge::Status MoeDistributeDispatchV3GenTaskFunc(const gert::ExeResGenerationContext *context,
...
IMPL_OP(MoeDistributeDispatchV3)
    .CalcOpParam(MoeDistributeDispatchV3CalcParamFunc)
    .GenerateTask(MoeDistributeDispatchV3GenTaskFunc);
#else // mc2 gen task utils
ge::Status MoeDistributeDispatchV3CalcParamFunc(gert::ExeResGenerationContext *context)
{
    if ((Mc2A5GenTaskUtils::IsTargetPlatform(context->GetNodeName(), NPUARCH_A5)) &&
        (Mc2A5GenTaskUtils::GetCommAlg(context, ATTR_INDEX_COMM_ALG_DISTRIBUTE_DISPATCH_V2) != COMM_ALG_MTE)) {
        OPS_LOG_D(context->GetNodeName(), "Do A5 ccu calc param.");
...
IMPL_OP_CT(MoeDistributeDispatchV3)
    .CalcOpParam(MoeDistributeDispatchV3CalcParamFunc)
    .GenerateTask(MoeDistributeDispatchV3GenTaskFunc);
REGISTER_EXT_TASK_TYPE(MoeDistributeDispatchV3, fe::ExtTaskType::kAicoreTask);
#endif
} // namespace ops
