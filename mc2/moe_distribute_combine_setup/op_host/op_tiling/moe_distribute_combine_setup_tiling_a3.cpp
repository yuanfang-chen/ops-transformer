...
/*!
 * \file moe_distribute_combine_setup_tiling_a3.cpp
 * \brief
 */
...
void MoeDistributeCombineSetupTilingA3::PrintTilingDataInfo()
{
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckRequiredAttrValue()
{
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::GetRequiredAttrAndSetTilingData()
{
    OP_LOGD(nodeName_, "GetRequiredAttrAndSetTilingData");
    auto attrs = context_->GetAttrs();
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckSharedExpertAttrValue()
{
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckOptionalAttrValue()
{
    auto attrs = context_->GetAttrs();
    ...
    OP_TILING_CHECK(
        ...
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(
        (*commQuantModePtr != COMM_QUANT_NONE),
        OP_LOGE(nodeName_, "commQuantMode only support 0, get %ld.", *commQuantModePtr), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::GetOptionalAttrAndSetTilingData()
{
    OP_LOGD(nodeName_, "GetOptionalAttrAndSetTilingData");
    auto attrs = context_->GetAttrs();
    OP_TILING_CHECK(attrs == nullptr, OP_LOGE(nodeName_, "attrs is null."), return ge::GRAPH_FAILED);
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::GetComplexAttrAndSetTilingData()
{
    ...
    if (epRankId >= sharedExpertRankNum) {
        // MoE 专家卡
        tilingData_->moeDistributeCombineSetupInfo.moeExpertPerRankNum = moeExpertNum / (epWorldSize - sharedExpertNum);
    } else {
        // 共同专家卡
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckOneTensorDim(
    std::string name, TensorType tensortype, uint32_t index, uint32_t dims)
{
    ...
    if (tensortype == INPUT) {
        ...
        OP_LOGE(
            nodeName_, "TensorType Only Support input or output. type:%d, name:%s, index:%u", tensortype, name.c_str(),
            index);
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckInputTensorDim()
{
    OP_TILING_CHECK(
    ...
    OP_TILING_CHECK(
        CheckOneTensorDim("assistInfoForCombine", INPUT, ASSIST_INFO_INDEX, ONE_DIM) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName_, "assistInfoForCombine checkdim failed."), return ge::GRAPH_FAILED);

    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckOutputTensorDim()
{
    OP_TILING_CHECK(
    ...
    OP_TILING_CHECK(
        CheckOneTensorDim("commCmdInfoOut", OUTPUT, COMM_CMD_INFO_OUT_INDEX, ONE_DIM) != ge::GRAPH_SUCCESS,
        OP_LOGE(nodeName_, "commCmdInfoOut checkdim failed."), return ge::GRAPH_FAILED);

    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckTensorDim()
{
    OP_TILING_CHECK(
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckTensorShapeRelation()
{
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE(
            nodeName_, "expandx's dim0[%ld] should be equal to quantexpandxout's dim0[%ld]",
            expandXStorageShape->GetStorageShape().GetDim(0), quantExpandXOutStorageShape->GetStorageShape().GetDim(0)),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckTensorShapeSizeAInMoeRank()
{
    auto expandXStorageShape = context_->GetInputShape(EXPAND_X_INDEX);
    ...
    if (globalBs == 0) {
        // MoE 专家卡 均分
        OP_TILING_CHECK(
            !(A >= (BS * epWorldSize * std::min(localMoeExpertNum, K))),
            OP_LOGE(
                nodeName_,
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckTensorShapeSizeAInSharedRank()
{
    auto expandXStorageShape = context_->GetInputShape(EXPAND_X_INDEX);
    int64_t A = expandXStorageShape->GetStorageShape().GetDim(0U);
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckTensorShapeSizeAndSetTilingData()
{
    auto expandXStorageShape = context_->GetInputShape(EXPAND_X_INDEX);
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE(
            nodeName_, "globalBs[%u] should be >= BS * epWorldSize[%ld] and <= 512 * epWorldSize, or = 0", globalBs,
            BS * epWorldSize),
        return ge::GRAPH_FAILED);
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckCalcTensorShapeSizeAndSetTilingData()
{
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE(
            nodeName_, "quantExpandXOut's dim1[%ld] should be equal to tokenMsgSize[%ld]", tokenMsgSize,
            tokenMsgSizeGolden),
        return ge::GRAPH_FAILED);
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckTensorDataType()
{
    ...
    OP_TILING_CHECK(
        assistInfoForCombineDesc == nullptr, OP_LOGE(nodeName_, "assistInfoForCombine is null."),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(
        quantExpandXOutDesc == nullptr, OP_LOGE(nodeName_, "quantExpandXOut is null."), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(
        commCmdInfoOutDesc == nullptr, OP_LOGE(nodeName_, "commCmdInfoOut is null."), return ge::GRAPH_FAILED);

    OP_TILING_CHECK(
        (expandXDesc->GetDataType() != ge::DT_BF16) && (expandXDesc->GetDataType() != ge::DT_FLOAT16),
        OP_LOGE(
            nodeName_, "expandX dataType is invalid, dataType should be bfloat16 or float16, but got %s",
    ...
    OP_TILING_CHECK(
        ...
        OP_LOGE(
            nodeName_, "expertIds dataType is invalid, dataType should be int32, but got %s",
            Ops::Base::ToString(expertIdsDesc->GetDataType()).c_str()),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(
        (assistInfoForCombineDesc->GetDataType() != ge::DT_INT32),
        OP_LOGE(
            nodeName_, "assistInfoForCombine dataType is invalid, dataType should be int32, but is %s",
            Ops::Base::ToString(assistInfoForCombineDesc->GetDataType()).c_str()),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(
        (quantExpandXOutDesc->GetDataType() != ge::DT_INT8),
        OP_LOGE(
            nodeName_, "quantExpandXOut dataType is invalid, dataType should be int8, but got %s",
            Ops::Base::ToString(quantExpandXOutDesc->GetDataType()).c_str()),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(
        (commCmdInfoOutDesc->GetDataType() != ge::DT_INT32),
        OP_LOGE(
            nodeName_, "commCmdInfoOut dataType is invalid, dataType should be int32, but is %s",
            Ops::Base::ToString(commCmdInfoOutDesc->GetDataType()).c_str()),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}

void MoeDistributeCombineSetupTilingA3::SetTilingKey()
{
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::CheckHcclBuffSize()
{
    const uint64_t hcclBuffSize = mc2tiling::Mc2TilingUtils::GetMaxWindowSize();
    OP_TILING_CHECK(
        hcclBuffSize < MIN_AVAILABLE_BUFF_SIZE,
        OP_LOGE(nodeName_, "HCCL_BUFFSIZE [%lu] < [%lu].", hcclBuffSize, MIN_AVAILABLE_BUFF_SIZE),
        return ge::GRAPH_FAILED);
    ...
    uint32_t& K = tilingData_->moeDistributeCombineSetupInfo.k;
    uint32_t& localExpertNum = tilingData_->moeDistributeCombineSetupInfo.moeExpertPerRankNum;
    uint32_t maxBs = BS;
    if (globalBs != 0) {
...
void MoeDistributeCombineSetupTilingA3::SetPlatformInfo()
{
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::SetWorkspace()
{
...
ge::graphStatus MoeDistributeCombineSetupTilingA3::MoeDistributeCombineSetupTilingFuncImpl()
{
...
bool MoeDistributeCombineSetupTilingA3::IsCapable()
{
    return true;
}

ge::graphStatus MoeDistributeCombineSetupTilingA3::DoOpTiling()
{
