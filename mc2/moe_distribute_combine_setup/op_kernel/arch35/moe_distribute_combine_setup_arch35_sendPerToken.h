...
private:
    ...
    // // URMA通信相关函数
    // __aicore__ inline void InitPICI();
    ...
    GM_ADDR expandXGM_;
    // LocalTensor<int32_t> assistInfoForCombineLocal_;
    // LocalTensor<int32_t> statusLocal_;
    ...
    // TQue<QuePosition::VECIN, 1> expertTokenTmpQueue_; // URMA本卡回环通信拷贝数据Local Memory暂存
    // TBuf<> expertTokenTmpBuf_;                        // URMA本卡回环通信拷贝数据Local Memory暂存
    // TBuf<> statusBuf_;         // URMA本卡状态暂存
    ...
    TBuf<> templateSqeBuf_; // 96B，Write with notify SQE需要96B空间
...
{
    tpipe_->Reset();
    // tpipe_->InitBuffer(assistInfoBuf_,
    ...
    tpipe_->InitBuffer(tokenSqeBuf_, WRITE_WITH_NOTIFY_SQE_SIZE);

    tpipe_->InitBuffer(expertTokenTmpQueue_, 1, static_cast<uint32_t>(expertPerSizeOnWin_));
    // tpipe_->InitBuffer(expertTokenTmpBuf_, Ceil(expertPerSizeOnWin_, UB_ALIGN) * UB_ALIGN);
    // tpipe_->InitBuffer(statusBuf_, UB_ALIGN); // 只用4Bytes，但最低分配32Bytes
    // statusLocal_ = statusBuf_.Get<int32_t>();
    // statusLocal_.SetValue(0, 0x3f800000); // URMA本卡通信状态
}

template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeCombineSetup<TemplateMC2TypeFunc>::AssistInfoLocalCopy()
{
    // assistInfoForCombineLocal = assistInfoBuf_.Get<int32_t>();
...
{
    if (startRankId_ >= moeDistributeCombineSetupInfo_->epWorldSize) { // 空闲核，直接返回
        return;
    }
    ...
    if (unlikely(moeDistributeCombineSetupInfo_->epRankId >= startRankId_ &&
        ...
        for (uint32_t expertIdx = 0U; expertIdx < curRankExpertNum; ++expertIdx) {
            ...
            // expandX在GM上，winIn也属于GM，因此数据需要GM -> local -> winIn
        ...
        SyncFunc<AscendC::HardEvent::MTE2_S>(); // 保证数据写入后再写状态
        // 向本卡状态区写状态
    ...
    for (uint32_t epIdx = startRankId_; epIdx < endRankId_; ++epIdx) {
        ...
        // if (likely(epIdx != moeDistributeCombineSetupInfo_->epRankId)) {

        // URMA 加载SQ CQ
        GetURMASqInfoTensor(sqInfoU8, (GM_ADDR)hcclContext_, epIdx);
        GetURMACqInfoTensor(cqInfoU8, (GM_ADDR)hcclContext_, epIdx);
        // 等sqInfoU8、cqInfoU8的DataCopy(GM->Local)，后续InvalidateCqeStatus、UpdateCommWriteWithNotifySQE标量读
        SyncFunc<AscendC::HardEvent::MTE2_S>();

            // 获取PI CI
            GetPICI((GM_ADDR)hcclContext_, moeDistributeCombineSetupInfo_->epRankId, epIdx, sqPi, sqCi, cqPi, cqCi, isFirstInitCqe);
        ...
        // dstStateAddr = GetWinStateAddrByRankId(moeDistributeCombineSetupInfo_->epRankId) +
        //                static_cast<uint64_t>(epIdx) * stateOffset_; // TODO 联调场景，构造输入，2p场景特定
        ...
        for (uint32_t expertIdx = 0U; expertIdx < curRankExpertNum; ++expertIdx) {
            ...
            // dstAddr = GetWinAddrByRankId(moeDistributeCombineSetupInfo_->epRankId, expertIdx) +
            //           static_cast<uint64_t>(epIdx) *
            //               static_cast<uint64_t>(moeDistributeCombineSetupInfo_->moeExpertPerRankNum) *
            //               expertPerSizeOnWin_; // TODO 联调场景，构造输入，2p场景特定

            // if (true) { // TODO 联调场景，构造输入，2p场景特定
            // if (unlikely(epIdx == moeDistributeCombineSetupInfo_->epRankId)) {
            //     // 跳过本卡URMA通信，使用DataCopy直接拷贝

            //     DataCopyExtParams copyParams{1U, curTokenNum * static_cast<uint32_t>(axisHExpandXTypeSize_), 0U, 0U,
            //                                  0U};
            //     DataCopyPadExtParams<uint8_t> padParams{false, 0U, 0U, 0U};
            ...
            for (uint32_t tokenIdx = 0U; tokenIdx < curTokenNum; ++tokenIdx) {
                // 对token遍历，每个token组装一个WQE
                ++tokenSqeNum;

                // 发送上一个WQE模板
                if (likely(tokenSqeNum > 1U)) {
                    SyncFunc<AscendC::HardEvent::S_V>(); // 等templateSqeU8标量设置完成
                    DataCopy(tokenSqeU8, templateSqeU8, WRITE_WITH_NOTIFY_SQE_SIZE);
                    SyncFunc<AscendC::HardEvent::V_S>();
                    SetCommWriteWithNotifySQE(tokenSqeU8, notifySqeInfo.dataSrcAddr, notifySqeInfo.dataDstAddr,
                                              notifySqeInfo.length, notifySqeInfo.notifyAddr + sizeof(uint64_t),
                                              notifySqeInfo.notifyData, notifySqeInfo.cqe);
                    SyncFunc<AscendC::HardEvent::S_MTE3>(); // 等tokenSqeU8标量写，后续Local->GM
                    // 发数据
                    PutCommNotifySQE(sqInfoU8, cqInfoU8, tokenSqeU8, cqeTensorU8, jfcDoorBellU8, 1, sqPi, sqCi, cqCi);
                ...
                srcAddr += axisHExpandXTypeSize_;
                dstAddr += axisHExpandXTypeSize_;
            }
        }
        ...
        //     GlobalTensor<int32_t> selfStatusTensor;
        //     selfStatusTensor.SetGlobalBuffer((__gm__ int32_t *)dstStateAddr);
        //     selfStatusTensor.SetValue(0, 0x3f800000);
        //     DataCacheCleanAndInvalid<int32_t, CacheLine::SINGLE_CACHE_LINE,
        //     DcciDst::CACHELINE_OUT>(selfStatusTensor);
        ...
        SyncFunc<AscendC::HardEvent::V_S>();
        SetCommWriteWithNotifySQE(tokenSqeU8, notifySqeInfo.dataSrcAddr, notifySqeInfo.dataDstAddr,
                                  notifySqeInfo.length, notifySqeInfo.notifyAddr, 0x3f800000, 1);
        SyncFunc<AscendC::HardEvent::S_MTE3>(); // 等tokenSqeU8标量写，后续Local->GM
        // 发数据
        PutCommNotifySQE(sqInfoU8, cqInfoU8, tokenSqeU8, cqeTensorU8, jfcDoorBellU8, 1, sqPi, sqCi, cqCi);
        // PipeBarrier<PIPE_MTE3>(); // TODO 这个等哪个同步？
        ...
        SendJFSDoorBell(jfsDoorBellU8, sqInfoU8, sqPi);
            ...
            // 更新PI CI
            UpdatePICI((GM_ADDR)hcclContext_, moeDistributeCombineSetupInfo_->epRankId, epIdx, sqPi, sqCi, cqPi, cqCi, true);
        }
    }
...
// template <TemplateMC2TypeClass>
// __aicore__ inline void MoeDistributeCombineSetup<TemplateMC2TypeFunc>::InitPICI()
// {
//     // 软件侧的PI CI，由AIV在windowsExp中维护，HCCL不帮忙开辟内存，headAddr tailAddr 为预留字段
//     // TODO win区里取值
//     sqPi_ = 0;
//     sqCi_ = 0;
//     cqPi_ = 0;
...
//     DataCopyExtParams addrInfoParams = {1U, static_cast<uint32_t>(sizeof(uint32_t)), 0U, 0U, 0U};
//     DataCopyPadExtParams<uint32_t> addrInfoPadParams{false, 0U, 0U, 0U};

//     LocalTensor<uint64_t> sqInfoU64 = urmaSqInfoBuf_.Get<uint64_t>();
//     GlobalTensor<uint32_t> sqHeadAddrGlobalTensor;
//     sqHeadAddrGlobalTensor.SetGlobalBuffer((__gm__ uint32_t *)(sqInfoU64(3)));
//     LocalTensor<uint32_t> sqHeadAddrU32 = sqHeadAddrBuf_.Get<uint32_t>();
//     DataCopyPad(sqHeadAddrU32, sqHeadAddrGlobalTensor, addrInfoParams, addrInfoPadParams);

//     GlobalTensor<uint32_t> sqTailAddrGlobalTensor;
//     sqTailAddrGlobalTensor.SetGlobalBuffer((__gm__ uint32_t *)(sqInfoU64(4)));
//     LocalTensor<uint32_t> sqTailAddrU32 = sqTailAddrBuf_.Get<uint32_t>();
//     DataCopyPad(sqTailAddrU32, sqTailAddrGlobalTensor, addrInfoParams, addrInfoPadParams);


//     LocalTensor<uint64_t> cqInfoU64 = urmaCqInfoBuf_.Get<uint64_t>();
//     GlobalTensor<uint32_t> cqHeadAddrGlobalTensor;
//     cqHeadAddrGlobalTensor.SetGlobalBuffer((__gm__ uint32_t *)(cqInfoU64(3)));
//     LocalTensor<uint32_t> cqHeadAddrU32 = cqHeadAddrBuf_.Get<uint32_t>();
//     DataCopyPad(cqHeadAddrU32, cqHeadAddrGlobalTensor, addrInfoParams, addrInfoPadParams);

//     GlobalTensor<uint32_t> cqTailAddrGlobalTensor;
//     cqTailAddrGlobalTensor.SetGlobalBuffer((__gm__ uint32_t *)(cqInfoU64(4)));
//     LocalTensor<uint32_t> cqTailAddrU32 = cqTailAddrBuf_.Get<uint32_t>();
//     DataCopyPad(cqTailAddrU32, cqTailAddrGlobalTensor, addrInfoParams, addrInfoPadParams);

//     SyncFunc<AscendC::HardEvent::MTE2_S>(); // 等sqHeadAddrBuf_、sqTailAddrBuf_、cqHeadAddrBuf_、cqTailAddrBuf_
//     sqPi_ = sqHeadAddrU32(0);
//     sqCi_ = sqTailAddrU32(0);
//     cqPi_ = cqHeadAddrU32(0);
//     cqCi_ = cqTailAddrU32(0);
//     if (sqPi_ != sqCi_ || cqPi_ != cqCi_) {
//         // TODO 直接退出kernel
//     }
//     sqPiLinear_ = sqPi_;
//     cqCiLinear_ = cqCi_;
