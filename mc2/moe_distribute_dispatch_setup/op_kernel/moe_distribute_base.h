                                       ...
                                       const AscendC::LocalTensor<uint8_t> &sqInfoTensor, uint32_t sqPi)
{
    ...
    // AscendC::printf("sq pi value is %d", sqPi);
}
                                       ...
                                       const AscendC::LocalTensor<uint8_t> &cqInfoTensor, uint32_t cqCi)
{
    ...
    st_dev((cqInfoU32(0) << 32) + cqCi, (__gm__ uint64_t *)(cqInfoU64(5)), 0);
}
                                            ...
                                            uint32_t &outCqCi)
{
    ...
    while (pollTimes < cqDepth - 1) {
        ...
        // AscendC::PipeBarrier<PIPE_ALL>(); // test
    //     for(int i=0; i<100;i++){
    //     AscendC::printf("test");
    // } //test
                                           ...
                                           const AscendC::LocalTensor<uint8_t> &cqeTensor)
{
    ...
    AscendC::Duplicate<uint8_t>(cqeTensor, 0xff, UB_ALIGN * cqeNum); // 初始化为全0
    // SyncFunc<AscendC::HardEvent::V_S>();
                                  ...
                                  uint32_t &outSqPi, uint32_t &outSqCi, uint32_t &outCqCi)
{
    ...
    // AscendC::printf("availableSpace is %d", availableSpace);
    while (availableSpace < sqeCount) {
        // 可用空间不足时，轮询CQ，更新本地的cqCi_ sqCi_和硬件的cqCi
        // AscendC::printf("enter pollcommcq");
...
__aicore__ inline void GetPICI(GM_ADDR hcclContext, uint32_t curRankId, uint32_t dstRankId, uint32_t &outSqPi,
                               uint32_t &outSqCi, uint32_t &outCqPi, uint32_t &outCqCi, bool &temp/*, uint32_t &outSqPiLinear,
...
__aicore__ inline void UpdatePICI(GM_ADDR hcclContext, uint32_t curRankId, uint32_t dstRankId, uint32_t sqPi,
                                  uint32_t sqCi, uint32_t cqPi, uint32_t cqCi, bool temp)
{
    if (hcclContext == nullptr || curRankId >= MAX_RANK_NUM || dstRankId >= MAX_RANK_NUM) {
        ascendc_assert(false, "hcclContext is nullptr or curRankId >= MAX_RANK_NUM or dstRankId >= MAX_RANK_NUM");
        return;
