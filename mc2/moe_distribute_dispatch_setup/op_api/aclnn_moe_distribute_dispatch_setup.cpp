...
enum NnopbaseHcclServerType {
    ...
    // NNOPBASE_HCCL_SERVER_TYPE_URMA
                         ...
                         aclTensor* expandIdxOut, aclTensor* commCmdInfoOut)
{
    ...
    OP_LOGD("aclnn_moe_distribute_dispatch_setup CheckNotNull success");
...
static aclnnStatus CheckParams(const aclTensor* x, const aclTensor* expertIds, const aclTensor* scalesOptional,
                               ...
                               aclTensor* expandIdxOut, aclTensor* commCmdInfoOut)
{
    OP_LOGD("aclnn_moe_distribute_dispatch_setup CheckParams start");
    ...
    OP_LOGD("aclnn_moe_distribute_dispatch_setup CheckParams success");
    ...
    aclTensor* expandIdxOut, aclTensor* commCmdInfoOut, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    OP_LOGD("aclnnMoeDistributeDispatchSetupGetWorkspaceSize end");
    return ret;
}
    ...
    uint64_t& commCmdInfoOutSize)
{
    ...
    if (x->GetViewShape().GetDimNum() != TWO_DIM) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "x's DimNum shoule be 2, but got %lu.", x->GetViewShape().GetDimNum());
        return ACLNN_ERR_PARAM_INVALID;
    }
    if (expertIds->GetViewShape().GetDimNum() != TWO_DIM) {
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "expertIds's DimNum shoule be 2, but got %lu.",
    ...
    if (sharedExpertRankNum >= epWorldSize) {
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "sharedExpertRankNum should less than epWorldSize[%ld], but got %ld.", epWorldSize,
            sharedExpertRankNum);
        return ACLNN_ERR_PARAM_INVALID;
    } else if (epRankId < 0) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "epRankId should not less than 0, but got %ld.", epRankId);
        return ACLNN_ERR_PARAM_INVALID;
    } else {
                                            ...
                                            aclrtStream stream)
{
    ...
    if (NnopbaseSetHcclServerType) {
        OP_LOGD("aclnnMoeDistributeDispatchSetup start0");
        NnopbaseSetHcclServerType(executor, NNOPBASE_HCCL_SERVER_TYPE_MTE);
    }
    OP_LOGD("aclnnMoeDistributeDispatchSetup start1");
    aclnnStatus ret = aclnnInnerMoeDistributeDispatchSetup(workspace, workspaceSize, executor, stream);
    OP_LOGD("aclnnMoeDistributeDispatchSetup end");
    return ret;
}
