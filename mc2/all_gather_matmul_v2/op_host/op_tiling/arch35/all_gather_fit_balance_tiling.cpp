...
{
    if (tilingM_.cutRes.longTileLen * mmInfo_.kValue * commPerf_.GetCommDTypeSize() * rankDim_ <= mc2tiling::ALL_GATHER_HCCL_MEM_LIMIT) {
        return;
    }
    ...
    uint64_t minSplitPart = Ops::Base::CeilDiv(mmInfo_.mValue * mmInfo_.kValue * commPerf_.GetCommDTypeSize() * rankDim_, mc2tiling::ALL_GATHER_HCCL_MEM_LIMIT);
