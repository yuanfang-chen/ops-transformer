...
ge::graphStatus AllGatherQuantBmmTiling::CheckHCCLSize()
{
    ...
    uint64_t sizeOfSplitM = Ops::CeilDiv(args_.mValue, mc2tiling::ALL_GATHER_HCCL_NUM_LIMIT) * sizeOfSingleM;
    OP_TILING_CHECK(sizeOfSingleM > mc2tiling::ALL_GATHER_HCCL_MEM_LIMIT,
...
ge::graphStatus AllGatherQuantBmmTiling::AdjustHCCLLimit()
{
    ...
    uint64_t minSplitPart = Ops::CeilDiv(args_.mValue * args_.kValue * sizeof(args_.geAType) * args_.rankDim, mc2tiling::ALL_GATHER_HCCL_MEM_LIMIT);
    tileMValue_ = Ops::CeilDiv(args_.mValue, minSplitPart);
    MutableRCSTilingDataA5().tileCnt = Ops::FloorDiv(args_.mValue, tileMValue_);
