...
ge::graphStatus MhcPreBaseTiling::GetInputShape()
{
    ...
    //     chunkTSize_ = 192;
    // }
    // chunkTSize_ = chunTSize_ < 2 ? 2 : chunTSize_;
...
{
    constexpr size_t kWorkspaceAlignBytes = 32U;
    size_t xFloatWorkspaceSizeRaw = static_cast<size_t>(matM_) * static_cast<size_t>(matK_) * sizeof(float);
    size_t xFloatWorkspaceSize = (xFloatWorkspaceSizeRaw + kWorkspaceAlignBytes - 1U) /
                                kWorkspaceAlignBytes * kWorkspaceAlignBytes;
    size_t mmResWorkspaceSize = 0;

    // mmRes 直接写外部输出张量 initParams.mm_res，需要在GM额外申请内存
    if (outFlag_ == 0U) {
        uint64_t chunkNd = (matK_ + blockDim_ - 1) / blockDim_;  // blockDim_为32，先天对齐
    ...
    size_t userWorkspaceSize = xFloatWorkspaceSize + mmResWorkspaceSize;
