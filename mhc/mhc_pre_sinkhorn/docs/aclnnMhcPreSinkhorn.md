...
| $\text{softmax}(\cdot, \dim=-1)$ | 在最后一（行）维执行softmax归一化 |
...
|:--- |:--- |:--- |:--- |:--- |:--- |:--- |:--- |
| h_res | 输入 | 待计算数据，mHC层的$\mathbf{H}'_{\text{res}}$矩阵 | 必选参数，不能为空Tensor | FLOAT32 | ND | (B,S,n,n)、(T,n,n) | √ |
| eps | 输入 | 归一化防除零参数 | 建议值：1e-6 | FLOAT32 | - | - | - |
| numIters| 输入 | 迭代次数 | 建议值：20；范围：1~100 | INT64 | - | - | - |
| outFlag| 输入 | 输出标志位 | 0：仅输出sinkhorn结果；1：输出中间结果（当前版本仅支持0） | INT32 | - | - | - |
| h_res_sinkhorn | 输出 | MhcPreSinkhorn变换最终结果（双随机矩阵$\mathbf{H}_{\text{res}}$） | 必选输出，维度与输入h_res一致 | FLOAT32 | ND | (B,S,n,n)、(T,n,n) | √ |
| norm_out| 输出 | 迭代过程中的归一化中间结果 | 可选输出（outFlag=1时有效） | FLOAT32 | ND | [2\*num_iters,n,n,B,S]、[2\*num_iters,n,n,T] | √ |
| sum_out| 输出 | 迭代过程中的求和中间结果 | 可选输出（outFlag=1时有效） | FLOAT32 | ND | [2\*num_iters,n,B,S]、[2\*num_iters,n,T] | √ |
...
| 返回值 | 错误码 | 描述 |
|:--- |:--- |:--- |
| ACLNN_ERR_PARAM_NULLPTR | 161001 | 必选参数（h_res/h_res_sinkhorn）或输出参数（workspaceSize/executor）为空指针。 |
| ACLNN_ERR_PARAM_INVALID | 161002 | 1. h_res的数据类型/格式非FLOAT32/ND；2. numIters超出1~100范围；3. n值非4/6/8；4. outFlag非0。 |
...
1. 输入约束：
   - 输入Tensor `h_res` 为空，报错`ACLNN_ERR_PARAM_NULLPTR`；
...
| 维度数 | 3/4 | 输入h_res支持3维(T,n,n)或4维(B,S,n,n)，其他维度数不支持。 |
| outFlag | 0 | 当前版本仅支持outFlag=0，不支持outFlag=1（中间结果输出）。 |
