 ...
 * THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EITHER EXPRESS OR IMPLIED,
...
/**
 * @brief aclnnMhcPreSinkhornGetWorkspaceSize
 ...
 * @param [out] sum_out: 输出tensor，迭代过程中的求和中间结果（可选），shape为[2*numIters,n,B,S]或[2*numIters,n,T]，数据类型：FP。
 * @param [out] workspaceSize: 返回需要在npu device侧申请的workspace大小。
 * @param [out] executor: 返回op执行器，包含了算子计算流程。
 * @return aclnnStatus: 返回状态码
 */
ACLNN_API aclnnStatus aclnnMhcPreSinkhornGetWorkspaceSize(const aclTensor *h_res, float eps, int64_t numIters,
...
/**
 * @brief aclnnMhcPreSinkhorn
 * @domain aclnn_ops_infer
 * 
 * @param [in] workspace: 在npu device侧申请的workspace内存起址。
 * @param [in] workspaceSize: 在npu device侧申请的workspace大小，由第一段接口aclnnMhcPreSinkhornGetWorkspaceSize获取。
 * @param [in] executor: op执行器，包含了算子计算流程。
 * @param [in] stream: acl stream流。
 * @return aclnnStatus: 返回状态码
 */
ACLNN_API aclnnStatus aclnnMhcPreSinkhorn(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor,
                                        aclrtStream stream);
