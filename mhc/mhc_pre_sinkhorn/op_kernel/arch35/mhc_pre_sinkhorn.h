...
#ifndef ASCENDC_MHC_PRE_SINKHORN_H
#define ASCENDC_MHC_PRE_SINKHORN_H
...
class MhcPreSinkhornSimd {
public:
    __aicore__ inline MhcPreSinkhornSimd(TPipe &pipe, const MhcPreSinkhornTilingData &tilingData)
        : pipe_(pipe), tilingData_(tilingData){};
    __aicore__ inline void Init(GM_ADDR h_res, GM_ADDR h_res_sinkhorn, GM_ADDR norm_out, GM_ADDR sum_out, GM_ADDR tiling);
...
private:
    ...
    GlobalTensor<float> hResSinkhorn_;
    GlobalTensor<float> normOut_;
    GlobalTensor<float> sumOut_;
    TPipe &pipe_;
    const MhcPreSinkhornTilingData &tilingData_;
    int64_t blockIdx_;
    int64_t loop_;
...
__aicore__ inline void MhcPreSinkhornSimd::Init(GM_ADDR h_res, GM_ADDR h_res_sinkhorn, GM_ADDR norm_out, GM_ADDR sum_out,
                                             GM_ADDR tiling)
{
    blockIdx_ = GetBlockIdx();
    hRes_.SetGlobalBuffer((__gm__ float *)(h_res));
    hResSinkhorn_.SetGlobalBuffer((__gm__ float *)(h_res_sinkhorn));
    ...
    tailLoopSize_ = tilingData_.tUbFactorTail;
...
__aicore__ inline void MhcPreSinkhornSimd::CalcSoftmax(__local_mem__ float *inputAddr, __local_mem__ float *outputAddr,
...
__aicore__ inline void MhcPreSinkhornSimd::CalcCol(__local_mem__ float *inputAddr, __local_mem__ float *outputAddr,
                                                __local_mem__ uint32_t *maskAddr, __local_mem__ float *maxAddr,
                                                uint32_t dataLen, uint32_t n, float eps)
{
    __VEC_SCOPE__
    {
        ...
        MicroAPI::Duplicate(duplicateReg1, (int32_t)(INDEX_BLOCK_LEN));
        MicroAPI::Duplicate(duplicateReg2, (int32_t)(n * n));
        MicroAPI::Arange(orderReg, (int32_t)(0));
        MicroAPI::Div(tmpReg1, orderReg, duplicateReg1, maskReg);
        MicroAPI::Mul(tmpReg2, tmpReg1, duplicateReg1, maskReg);
        MicroAPI::Sub(tmpReg2, orderReg, tmpReg2, maskReg);
        MicroAPI::Muls(tmpReg2, tmpReg2, (int32_t)(n), maskReg);
        MicroAPI::Mul(tmpReg3, tmpReg1, duplicateReg2, maskReg);
        MicroAPI::Add(indexReg, tmpReg2, tmpReg3, maskReg);
        MicroAPI::DataCopyGather(gatherReg, inputAddr, (MicroAPI::RegTensor<uint32_t> &)(indexReg), maskReg);
...
__aicore__ inline void MhcPreSinkhornSimd::CalcRow(__local_mem__ float *inputAddr, __local_mem__ float *outputAddr,
                                                __local_mem__ uint32_t *maskAddr, __local_mem__ float *maxAddr,
                                                uint32_t dataLen, uint32_t n, float eps)
{
    __VEC_SCOPE__
...
__aicore__ inline void MhcPreSinkhornSimd::Process()
{
    ...
    for (int64_t i = 0; i < loop_; i++) {
        ...
        DataCopyExtParams dataCopyExtParams{1, (uint32_t)(loopSize * sizeof(float)), 0, 0, 0};
        DataCopyPad(inputLocal, hRes_[inputOffset], dataCopyExtParams, dataCopyPadExtParams);

        auto MTE2ToVEventID = (event_t)(GetTPipePtr()->FetchEventID(HardEvent::MTE2_V));
        ...
        for (uint16_t i = 0; i < repeatTimes; i++) {
            for (uint16_t j = 0; j < tilingData_.n; j++) {
                ...
                CalcSoftmax(curInputAddr, curOutputAddr, maskAddr, maxAddr, (uint32_t)(dataLen),
                            (uint32_t)(tilingData_.n), tilingData_.eps);
            }
        }
        ...
        for (uint16_t i = 0; i < repeatTimes; i++) {
            for (uint16_t j = 0; j < tilingData_.n; j++) {
                ...
                CalcCol(curAddrCol, curAddrCol, maskAddr, maxAddr, (uint32_t)(dataLen),
                        (uint32_t)(tilingData_.n), tilingData_.eps);
            }
        }
        __VEC_SCOPE__
        ...
        for (int64_t iter = 0; iter < tilingData_.num_iters - 1; iter++) {
            for (uint16_t i = 0; i < repeatTimes; i++) {
                for (uint16_t j = 0; j < tilingData_.n; j++) {
                    ...
                    CalcRow(curAddr, curAddr, maskAddr, maxAddr, (uint32_t)(dataLen),
                            (uint32_t)(tilingData_.n), tilingData_.eps);
                }
                __VEC_SCOPE__
                {
                ...
                for (uint16_t j = 0; j < tilingData_.n; j++) {
                    ...
                    CalcCol(curAddr, curAddr, maskAddr, maxAddr, (uint32_t)(dataLen),
                            (uint32_t)(tilingData_.n), tilingData_.eps);
                }
            }
            __VEC_SCOPE__
        ...
        auto VToMTE3EventID = (event_t)(GetTPipePtr()->FetchEventID(HardEvent::V_MTE3));
        SetFlag<HardEvent::V_MTE3>(VToMTE3EventID);
        WaitFlag<HardEvent::V_MTE3>(VToMTE3EventID);

        DataCopyPad(hResSinkhorn_[inputOffset], outputLocal, dataCopyExtParams);
