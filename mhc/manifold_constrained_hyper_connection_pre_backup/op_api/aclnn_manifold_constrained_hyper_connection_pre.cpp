...
bool CheckDtypeValid(const ManifoldConstrainedHyperConnectionParamsBase &params)
{
    // x支持BF16或FP16
...
aclnnStatus CovertDataContiguous(ManifoldConstrainedHyperConnectionParamsBase &params, aclOpExecutor *executor)
{
    // 将输入tensor转换为连续格式
...
static aclnnStatus mHCPreCommonProcess(ManifoldConstrainedHyperConnectionParamsBase &params, aclOpExecutor *executor)
{
    ...
    ret = CovertDataContiguous(params, executor);
    CHECK_RET(ret == ACLNN_SUCCESS, ret);
