...
static int32_t CreatMc2Context(HcclComm &comm, int64_t worldSize, Mc2ContextStru *mc2Context)
{
    ...
    auto getFuncHcclGetRemoteIpcHcclBuffer = GetHcclFwkFuncAddr<_HcclGetRemoteIpcHcclBuffer>("HcclGetRemoteIpcHcclBuffer");
    if (getFuncHcclGetRemoteIpcHcclBuffer == nullptr) {
        printf("getFuncHcclGetRemoteIpcHcclBuffer failed.");
        return -1;
    }
    printf("getFuncHcclGetRemoteIpcHcclBuffer success.");
    uint32_t rankId;
    ...
    std::string algConfig, uint32_t opType)
{
    ...
    printf("try HcclCommGetHanldeWithName groupName %s.", groupName);
