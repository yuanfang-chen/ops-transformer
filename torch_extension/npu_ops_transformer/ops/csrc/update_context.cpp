...
namespace op_api {
using npu_utils = at_npu::native::NpuUtilis;

constexpr static uint8_t COMM_ENGINE_AIV = 4;
constexpr uint32_t OP_TYPE_ALL_TO_ALL = 8;
constexpr uint32_t EP_OP_TYPE = OP_TYPE_ALL_TO_ALL;
cnost std::string EP_ALG_CONFIG = "AlltoAll=level0:fullmesh;level1:pairwise";
...
static int32_t CreatMc2Context(HcclComm &comm, int64_t worldSize, Mc2ContextStru *mc2Context)
{
    ...
    auto getFuncHcclGetHcclBuffer = GetHcclFuncAddr<_HcclGetHcllBuffer>("HcclGetHcclBuffer");
    if (getFuncHcclGetHcclBuffer == nullptr) {
        printf("getFuncHcclGetHcclBuffer failed.");
        return -1;
    }
    printf("getFuncHcclGetHcclBuffer success.");
    auto getFuncHcclGetRemoteHcclBuffer = GetHcclFuncAddr<_HcclGetRemoteHcclBuffer>("HcclGetRemoteHcclBuffer");
    if (getFuncHcclGetRemoteHcclBuffer == nullptr) {
        printf("getFuncHcclGetRemoteHcclBuffer failed.");
        return -1;
    }
    printf("getFuncHcclGetRemoteHcclBuffer success.");
    uint32_t rnakId;
    ...
    for (uint64_t remoteRankId = 0; remoteRankId < worldSize; remoteRankId++) {
        ...
        uint64_t commSize = 0; // 出餐， HCCL_BUFFER_SIZE=, 校验
        HcclResult ret;
    ...
    std::string algConfig, uint32_t opType)
{
    ...
    printf("try HcclCommGetHanldeWithName groupName %d.", groupName);
    ...
    void *opsResCtx;
    ret = static_cast<HcclResult>(getFuncHcclCreateOpResCtx(commHandle, opType, opArgs, &opsResCtx));
    if (ret != 0) {
    ...
    ret = static_cast<HcclResult>(getFuncHcclGetRankId(commHandle, &rnakId));
    if (ret != HCCL_SUCCESS) {
...
static int32_t GetMc2Context(Mc2ContextStru* mc2ContextHost, int64_t epWrldSize, const char* groupEpStr)
{
    ...
    int32_t contextRet = CreateHcclContext(epCommHandle, opArgs, epWrldSize, groupEpStr, EP_ALG_CONFIG, EP_OP_TYPE);
    if (contextRet != 0) {
        printf("CreateHcclContext failed.");
        return contextRet;
    }
    printf("CreateHcclContext success");
    contextRet = CreateMc2Context(epCommHandle, epWrldSize, mc2ContextHost);
    if (contextRet != 0) {
        printf("CreateMc2Context failed.");
        return contextRet;
    }
    printf("CreateMc2Context success");
    
    printf("CreateMc2Context beagin");
