...
class MoeDistributeBuffer:
    ...
    def get_low_latency_ccl_buffer_size(world_size: int, num_max_dispatch_tokens_per_rank: int, hidden: int,
        ...
        torch._check(((num_max_dispatch_tokens_per_rank >= 1) and (num_max_dispatch_tokens_per_rank <= 512)),
                     lambda: (f"num_max_dispatch_tokens_per_rank only support in [1, 512], but got {num_max_dispatch_tokens_per_rank=}."))
        ...
        local_moe_expert_num = num_moe_expert // (world_size - num_shared_expert_ranks) # local_moe_expert_num*world_size仅支持[0，2048]
        torch._check(((local_moe_expert_num * world_size > 0) and (local_moe_expert_num * world_size <= 2048)),
            lambda: (f"local_moe_expert_num * world_size only support in (0, 2048], but got {world_size=} {num_shared_expert_ranks=} "
                     f"local_moe_expert_num = num_moe_expert // (world_size - num_shared_expert_ranks), {local_moe_expert_num=}"))
