...
-   API功能：
    ...
    需与[npu\_low\_latency_dispatch](npu_low_latency_dispatch.md)配套使用，相当于按npu\_low\_latency\_dispatch算子收集数据的路径原路返回。
     - 支持数据整合功能，进行alltoallv通信，将接收的数据整合（乘权重再相加）
-   计算公式：
    - 数据整合功能：

      $ata\_out = AlltoAllv(expend\_x)$
...
```
npu_low_latency_combine(x, topk_idx, topk_weights, assist_info_for_combine, ep_send_counts, handle, *, num_experts = 0, comm_alg: str = "", comm_quant_mode=0, x_active_mask=None, expand_scales=None, shared_expert_x=None, elastic_info=None, ori_x=None, const_expert_alpha_1=None, const_expert_alpha_2=None, const_expert_v=None, zero_expert_num=0, copy_expert_num=0, const_expert_num=0, expert_shard_type=0, shared_expert_num=1, shared_expert_rank_num=0, num_max_dispatch_tokens_per_rank=0) -> Tensor
```

## 参数说明<a name="zh-cn_topic_0000002168254826_section112637109429"></a>

-   **x** (`Tensor`)：必选参数，根据`topk_idx`进行扩展过的token特征，要求为2维张量，shape为\(max\(tp\_world\_size, 1\) \*A, H\)，数据类型支持`bfloat16`、`float16`，数据格式为$ND$，支持非连续的Tensor。

-   **topk\_id** (`Tensor`)：必选参数，每个token的topK个专家索引，要求为2维张量，shape为\(BS, K\)。数据类型支持`int32`，数据格式为$ND$，支持非连续的Tensor。对应[npu\_low\_latency\_dispatch](npu_low_latency_dispatch.md)的`topk_idx`输入，张量里value取值范围为\[0, num\_experts\)，且同一行中的K个value不能重复。
-   **topk\_weights** (`Tensor`)：必选参数，表示每个token的topK个专家的权重，要求为2维张量，shape为\(BS, K\)。数据类型支持`float`，数据格式为$ND$，支持非连续的Tensor。
...
-   **ep\_send\_counts** (`Tensor`)：必选参数，表示本卡每个专家发给EP（Expert Parallelism）域每个卡的token数（token数以前缀和的形式表示），要求为1维张量。数据类型支持`int32`，数据格式为$ND$，支持非连续的Tensor。对应[npu\_low\_latency\_dispatch](npu_low_latency_dispatch.md)的`ep_recv_counts`输出。
    -   <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：要求shape为\(num\_experts+2\*global\_bs\*K\*server\_num, \)，前`num_experts`个数表示在EP通信域内，该卡上每个专家收到来自其他各卡的token数（以前缀和的形式表示），2\*global\_bs\*K\*server\_num用于存储机间和机内通信前，combine可提前做reduce操作的token个数和通信区偏移量，`num_max_dispatch_tokens_per_rank`传入0时此处按照bs\*ep\_world\_size计算。
    -   <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：要求shape为\(ep\_world\_size\*max\(tp\_world\_size, 1\)\*local\_expert\_num, \)。
-   **num\_experts** (`int`)：必选参数，MoE专家数量，取值范围\[1, 1024\]，并且满足以下条件：moe\_expert\_num\%\(ep\_world\_size)\=0。
    -   <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：还需满足moe\_expert\_num\/\(ep\_world\_size) <= 24。
...
-   **expand\_scales** (`Tensor`)：可选参数，对应[npu\_low\_latency\_dispatch]      (npu_low_latency_dispatch.md)的`expand_scales`输出。
...
-   **zero\_expert\_num** (`int`)：可选参数，表示零专家的数量。
    ...
    -   <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：取值范围[0, MAX_INT32)，MAX_INT32 = 2^31 - 1，合法的零专家的ID值是\[num\_experts, num\_experts+zero\_expert\_num\)。

-   **copy\_expert\_num** (`int`)：可选参数，表示拷贝专家的数量。
    -   <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：当前版本不支持，传0即可。
    -   <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：取值范围[0, MAX_INT32)，MAX_INT32 = 2^31 - 1，合法的拷贝专家的ID值是\[num\_experts+zero\_expert\_num, num\_experts+zero\_expert\_num+copy\_expert\_num\)。
...
-   调用接口过程中使用的`num_experts`、`expert_shard_type`、`shared_expert_num`、`shared_expert_rank_num`、`num_max_dispatch_tokens_per_rank`参数取值所有卡需保持一致,`expert_shard_type`、`num_max_dispatch_tokens_per_rank`网络中不同层中也需保持一致，且和[torch\_npu.npu\_moe\_distribute\_dispatch\_v2](torch_npu-npu_low_latency_dispatch.md)对应参数也保持一致。
...
-   参数里Shape使用的变量如下：
    ...
    -   local\_expert\_num：表示本卡专家数量。
        ...
        -   对于MoE专家卡，local\_expert\_num=num\_experts/\(ep\_world\_size-shared\_expert\_rank\_num)，当local\_expert\_num\>1时，不支持TP域通信。
...
-   HCCL_BUFFSIZE:
    -   <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：
        ...
        - comm\_alg配置为"hierarchy": 要求=num\_experts\*BS\*\(H\*sizeof\(dtype_x\)+4\*\(\(K+7\)/8\*8\)\*sizeof\(uint32\)\)+4MB+100MB，不要求moe\_expert\_num\/\(ep\_world\_size - shared\_expert\_rank\_num\) <= 24。
...
-   单算子模式调用
    ...
    from npu_ops_transformer import MoeDistributeBuffer
    ...
    def warm_up_dispatch(rank, group_ep, group_tp):
        ...
        out = torch_npu.npu_low_latency_dispatch(**dispatch_kwargs_before)
        return out
    ...
    def run_npu_process(rank):
        ...
        out = warm_up_dispatch(rank, ep_group, tp_group)

        distribute_buffer = MoeDistributeBuffer(ep_group)

        expand_x, dynamic_scales, assist_info_for_combine, expert_token_nums, ep_recv_counts, = distribute_buffer.npu_low_latency_dispatch(
            x=x,
            topk_idx=expert_ids,
            shared_expert_num=0,
            shared_expert_rank_num=shared_expert_rank_num,
...
-   图模式调用
    ...
    class MOE_DISTRIBUTE_GRAPH_Model(torch.nn.Module):
        ...
        def forward(self, x, expert_ids, group_ep, group_tp, ep_world_size, tp_world_size,
            ...
            output_dispatch_npu = torch_npu.npu_low_latency_dispatch(
                x=x,
                expert_ids=expert_ids,
                group_ep=group_ep,
                group_tp=group_tp,
            ...
            output_combine_npu = torch_npu.npu_low_latency_combine(
    ...
    def warm_up_dispatch(rank, group_ep, group_tp):
        ...
        (
            ...
            expert_token_nums, ep_recv_counts, , _
        ) = torch_npu.npu_low_latency_dispatch(**dispatch_kwargs_before)
        return expand_x, dynamic_scales, expand_idx, expert_token_nums, ep_recv_counts, 
