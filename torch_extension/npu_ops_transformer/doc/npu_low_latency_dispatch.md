...
-   API功能：
    ...
    需与[npu\_low\_latency_combine](npu_low_latency_combine.md)配套使用，完成MoE的并行部署下的token的dispatch和combine接收。
...
-   计算公式：
    - 动态量化场景：
      ...
      若`quant_mode`为`2`，即动态量化场景：
         ...
         \ dynamic\_scales=
         \begin{cases} 
         \ \ alltoall\_dynamic\_scales\_out & \quad
         \end{cases}
         $$
...
-   **top\_idx** (`Tensor`)：必选参数，表示每个token的topK个专家索引，决定每个token要发给哪些专家。要求为2维张量，shape为\(BS, K\)，数据类型支持`int32`，数据格式为$ND$，支持非连续的Tensor。对应[npu\_low\_latency\_combine](npu_low_latency_combine.md)的`expert_ids`输入，张量里value取值范围为\[0, num\_experts\)，且同一行中的K个value不能重复。
-   **num\_experts** (`int`)：必选参数，MoE专家数量，取值范围\[1, 1024\]，并且满足以下条件：num\_experts\%\(ep\_world\_size - shared\_expert\_rank\_num\)\=0。
    -   <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：还需满足num\_experts\/\(ep\_world\_size - shared\_expert\_rank\_num\) <= 24。
...
-   **topk\_weights** (`Tensor`)：可选参数，表示每个token的topK个专家权重。
...
-   **ep\_recv\_counts** (`Tensor`)：表示EP通信域各卡收到的token数（token数以前缀和的形式表示），要求为1维张量，数据类型`int32`，数据格式支持$ND$，支持非连续的Tensor。对应[npu\_low\_latency\_combine](npu_low_latency_combine.md)的`ep_send_counts`输入。
    -   <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：要求shape为\(num\_experts+2\*num_max\_dispatch\_tokens\_per\_rank\*K\*server\_num, \)，前`num_experts`个数表示在EP通信域内，该卡上每个专家收到来自其他各卡的token数（以前缀和的形式表示），2\*num_max\_dispatch\_tokens\_per\_rank\*K\*server\_num用于存储机间和机内通信前，combine可提前做reduce操作的token个数和通信区偏移量，`num_max_dispatch_tokens_per_rank`传入0时此处按照bs\*ep\_world\_size计算。
...
-   参数里Shape使用的变量如下：
    ...
    -   local\_expert\_num：表示本卡专家数量。
        ...
        -   对于MoE专家卡，local\_expert\_num=num\_experts/\(ep\_world\_size-shared\_expert\_rank\_num)，当local_expert_num大于1时，不支持TP域通信。
...
-   HCCL_BUFFSIZE:
    ...
    -   <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：
        ...
        - comm\_alg配置为"hierarchy": 要求=num\_experts\*BS\*\(H\*sizeof\(dtype_x\)+4\*\(\(K+7\)/8\*8\)\*sizeof\(uint32\)\)+4MB+100MB，不要求num\_experts\/\(ep\_world\_size - shared\_expert\_rank\_num\) <= 24。
...
-   图模式调用
    ...
    def warm_up_dispatch(rank, group_ep, group_tp):
        ...
        warm_up_dispatch_buffer = MoeDistributeBuffer(group_ep)
        (
            expand_x, dynamic_scales, expand_idx,
            expert_token_nums, ep_recv_counts, tp_recv_counts, _
        ) = warm_up_dispatch_buffer.npu_low_latency_dispatch(**dispatch_kwargs_before)
