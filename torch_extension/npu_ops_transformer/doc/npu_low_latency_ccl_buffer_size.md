...
-   计算公式：
    ...
    -   计算combine阶段每个token所需大小：
        ...
        其中，full\_mesh\_data\_align为480对齐，win\_addr\_align为512对齐

    -   计算最ccl\_buffer\_size大小：
...
```
get\_low\_latency\_ccl\_buffer\_size(world\_size, num\_max\_dispatch\_tokens\_per\_rank, hidden, num\_moe\_expert, topk, num\_shared\_expert=0, num\_shared\_expert\_ranks=0, comm\_alg="") -> int
```
...
-   **num\_shared\_expert\_ranks** (`int`)：可选参数，表示共享专家卡数量。必须满足`world\_size - num\_shared\_expert\_ranks > 0`，默认值为0。
