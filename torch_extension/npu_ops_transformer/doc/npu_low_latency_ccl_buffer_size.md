...
-   计算公式：
    ...
    -   计算token实际长度：
        ...
        $$token\_actual\_len = Align32(hidden * max_out_dtype_size) + scale_expand_index_buffer$$
    ...
    -   根据`comm_alg`计算dispatch阶段每个token所需大小：
        ...
        $$token\_need\_size\_dispatch = Align480(token\_actual\_len) / full_mesh_data_align * win_addr_align$$
    ...
    -   计算combine阶段每个token所需大小：
        ...
        $$token\_need\_size\_combine = Align512(hidden * max_out_dtype_size)$$
    
        其中，full_mesh_data_align为480对齐，win_addr_align为512对齐

    计算最小缓冲区大小：

    $$dispatch_buffer_size = 2 * num_max_dispatch_tokens_per_rank * token_need_size_dispatch * world_size * local_moe_expert_num$$
    $$combine_buffer_size = 2 * num_max_dispatch_tokens_per_rank * token_need_size_combine * (topk + num_shared_expert)$$
    $$ccl_buffer_size = Align1MB(dispatch_buffer_size + combine_buffer_size) / 1MB$$
