...
#ifndef ATTENTION_WORKER_COMBINE_SPLIT_H_H
#define ATTENTION_WORKER_COMBINE_SPLIT_H_H
...
template <typename T>
__aicore__ inline void KernelAttentionWorkerCombineSplitH<T>::Init(GM_ADDR schedule_context,
                                                                    GM_ADDR expert_scales, GM_ADDR layer_id, GM_ADDR y,
                                                                    GM_ADDR next_layer_id) {
  ...
  srcTokenGm.SetGlobalBuffer((__gm__ T*)(token_data_addr + micro_batch_id * tl_->BS * (tl_->K + 1) * tl_->H * sizeof(T)));
  srcTokenInfoGm.SetGlobalBuffer((__gm__ int32_t*)(token_info_addr + micro_batch_id * tl_->BS * (tl_->K + 1) * sizeof(int32_t)));
...
__aicore__ inline void KernelAttentionWorkerCombineSplitH<T>::Process() {
  ...
  for (int64_t bsLoopId = 0; bsLoopId < bsLoopNum; bsLoopId++) {
    ...
    for (int64_t hLoopId = 0; hLoopId < hLoopNum; hLoopId++) {
      ...
      for (int64_t kLoopId = 0; kLoopId < tl_->K; kLoopId++) {
        ...
        CopyIn(tokenSrcStart + bsLoopId * tokenSrcBsOffset + kLoopId * tl_->H + hLoopId * tl_->HSplitFactor, tl_->HSplitFactor);
        Compute(srcScalesGm(scaleStart + bsLoopId * tl_->K + kLoopId), tl_->HSplitFactor);
      }
      SetFlag<HardEvent::V_MTE2>(eventIDVToMTE2);
      WaitFlag<HardEvent::V_MTE2>(eventIDVToMTE2);
      CopyIn(tokenSrcStart + bsLoopId * tokenSrcBsOffset + tl_->K * tl_->H + hLoopId * tl_->HSplitFactor, tl_->HSplitFactor);
    ...
    for (int64_t kLoopId = 0; kLoopId < tl_->K; kLoopId++) {
      SetFlag<HardEvent::V_MTE2>(eventIDVToMTE2);
      WaitFlag<HardEvent::V_MTE2>(eventIDVToMTE2);
      CopyIn(tokenSrcStart + bsLoopId * tokenSrcBsOffset + kLoopId * tl_->H + hLoopNum * tl_->HSplitFactor, hTailLength);
      Compute(srcScalesGm(scaleStart + bsLoopId * tl_->K + kLoopId), hTailLength);
    }
    SetFlag<HardEvent::V_MTE2>(eventIDVToMTE2);
    WaitFlag<HardEvent::V_MTE2>(eventIDVToMTE2);
    CopyIn(tokenSrcStart + bsLoopId * tokenSrcBsOffset + tl_->K * tl_->H + hLoopNum * tl_->HSplitFactor, hTailLength);
  ...
  if (tl_->needSchedule == 1) {
    ...
    if (blockId == 0) {
      ...
      DataCacheCleanAndInvalid<uint32_t, CacheLine::SINGLE_CACHE_LINE,DcciDst::CACHELINE_ALL>(contextGm0[GET_OFFSET_B32(ScheduleContext, attention.micro_batch_id)]);
    }
  }
}
