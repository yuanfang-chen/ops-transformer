...
#ifndef ATTENTION_WORKER_COMBINE_SPLIT_K_H
#define ATTENTION_WORKER_COMBINE_SPLIT_K_H
...
template <typename T>
__aicore__ inline void KernelAttentionWorkerCombineSplitK<T>::Init(GM_ADDR schedule_context,
...
__aicore__ inline void KernelAttentionWorkerCombineSplitK<T>::Process() {
  ...
  for (int64_t bsLoopId = 0; bsLoopId < bsLoopNum; bsLoopId++) {
    ...
    CopyIn(tokenSrcStart + bsLoopId * tokenSrcBsOffset + tl_->KSplitLoopNum * tokenSrcKInOffset, tl_->KSplitTailFactor + 1);
