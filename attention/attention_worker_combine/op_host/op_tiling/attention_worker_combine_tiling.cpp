...
static std::tuple<int64_t, int64_t> GetShapeTuple(const gert::TilingContext* context, 
                                                  const int64_t index = 0) {
  const gert::StorageShape* shapePtr = context->GetInputShape(index);
  OP_TILING_CHECK(shapePtr == nullptr, OP_LOGE(context->GetNodeName(), "Shape is nullptr."), 
                  return std::make_tuple(0, 0));
  // check shape length is DIM_SIZE
  OP_TILING_CHECK(shapePtr->GetStorageShape().GetDimNum() != DIM_SIZE, 
                  OP_LOGE(context->GetNodeName(), "Shape must be (BS, K)."), return std::make_tuple(0, 0));
  return std::make_tuple(
    shapePtr->GetStorageShape().GetDim(SHAPE_IDX_BS), shapePtr->GetStorageShape().GetDim(SHAPE_IDX_K));
}
...
ge::graphStatus AttentionWorkerCombineTiling::GetPlatformInfo() {
  ...
  if (platformInfo == nullptr) {
    ...
    OP_TILING_CHECK(compileInfoPtr == nullptr, OP_LOGE(context_->GetNodeName(), "CompileInfo is nullptr."),
...
ge::graphStatus AttentionWorkerCombineTiling::GetShapeAttrsInfo() {
  OP_TILING_CHECK(context_ == nullptr, OP_LOGE(context_->GetNodeName(), "context_ can not be nullptr."),
                  return ge::GRAPH_FAILED);
  // Basic info
  ...
  OP_TILING_CHECK(k > K_UPPER_BOUND, OP_LOGE(context_->GetNodeName(), "k should <= 64."), return ge::GRAPH_FAILED);

  // Attrs info
  auto attrs = context_->GetAttrs();
  OPS_CHECK_NULL_WITH_CONTEXT(context_, attrs);
  auto hiddenSize = attrs->GetInt(HIDDEN_SIZE_INDEX);
  OPS_CHECK_NULL_WITH_CONTEXT(context_, hiddenSize);
  tilingData_.set_H(*hiddenSize);
  auto tokenDtype = attrs->GetInt(TOKEN_DTYPE_INDEX);
  OPS_CHECK_NULL_WITH_CONTEXT(context_, tokenDtype);
  tokenDtype_ = *tokenDtype;
  auto needSchedule = attrs->GetInt(NEED_SCHEDULE_INDEX);
  OPS_CHECK_NULL_WITH_CONTEXT(context_, needSchedule);
...
void AttentionWorkerCombineTiling::SelectBestHCore(int64_t hAlign, int64_t hInSplitK, int64_t &bsCoreNum, int64_t &lastCoreNum, int64_t &lastBestHCore) {
  int64_t bsMaxCore = 0;
...
ge::graphStatus AttentionWorkerCombineTiling::DoOpTiling() {
  ...
  int64_t hInFullK = AlignDown(static_cast<int64_t>(ubSize_) / (2 * kAddOne * B16_DTYPE_BYTES + 2 * B16_DTYPE_BYTES + 2 * B32_DTYPE_BYTES), H_ALIGN_SIZE);
  ...
  int64_t hInSplitK = AlignDown(static_cast<int64_t>(ubSize_) / (2 * B16_DTYPE_BYTES + 2 * B16_DTYPE_BYTES + 2 * B32_DTYPE_BYTES), H_ALIGN_SIZE);
  int64_t kIn = 1;
...
ge::graphStatus TilingPrepare4AttentionWorkerCombine(gert::TilingParseContext* context) {
  ...
  auto compileInfo = GetCompileInfoPtr<AttentionWorkerCombineCompileInfo>(context);
  OPS_CHECK_NULL_WITH_CONTEXT(context, compileInfo);
  auto platformInfo = context->GetPlatformInfo();
  OPS_CHECK_NULL_WITH_CONTEXT(context, platformInfo);
  auto ascendcPlatform = platform_ascendc::PlatformAscendC(platformInfo);
  compileInfo->coreNum = ascendcPlatform.GetCoreNumAiv();
  OP_TILING_CHECK(compileInfo->coreNum <= 0, OP_LOGE(context->GetNodeName(), "coreNum must be greater than 0."),
                  return ge::GRAPH_FAILED);
  uint64_t ubSize = 0;
  ascendcPlatform.GetCoreMemSize(platform_ascendc::CoreMemType::UB, ubSize);
  compileInfo->ubSize = ubSize;
  OP_TILING_CHECK(compileInfo->ubSize <= 0, OP_LOGE(context->GetNodeName(), "ubSize must be greater than 0."),
                  return ge::GRAPH_FAILED);
