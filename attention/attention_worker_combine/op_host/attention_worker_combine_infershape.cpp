...
graphStatus InferShape4AttentionWorkerCombine(gert::InferShapeContext* context) {
  ...
  OPS_CHECK_NULL_WITH_CONTEXT(context, scheduleContextInputShape);
  OPS_CHECK_NULL_WITH_CONTEXT(context, expertScalesInputShape);
  OPS_CHECK_NULL_WITH_CONTEXT(context, layerIdInputShape);

  auto attrs = context->GetAttrs();
  OPS_CHECK_NULL_WITH_CONTEXT(context, attrs);
  auto hiddenSize = attrs->GetAttrPointer<int64_t>(IDX_ZERO);
  OPS_CHECK_NULL_WITH_CONTEXT(context, hiddenSize);
  ...
  OPS_CHECK_NULL_WITH_CONTEXT(context, nextLayerIdShape);
...
graphStatus InferDtype4AttentionWorkerCombine(gert::InferDataTypeContext *context) {
  ...
  OPS_CHECK_NULL_WITH_CONTEXT(context, tokenDtype);
