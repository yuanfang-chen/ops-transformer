...
#include "nn_other.h"
#include "../op_graph/attention_worker_combine_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                          \
    ...
    attention_worker_combine_op.set_input_##intputName(placeholder##intputIndex);                       \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                        \
    ...
    attention_worker_combine_op.update_output_desc_##outputName(outputName##outputIndex##_desc);
    ...
    std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto attention_worker_combine_op = op::AttentionWorkerCombine("attention_worker_combine");

    // shape定义
    std::vector<int64_t> schedule_context_shape = {1024};
    std::vector<int64_t> expert_scales_shape = {48, 8};
    std::vector<int64_t> layer_id_shape = {1};
    
    std::vector<int64_t> y_shape = {48, 20480};
    std::vector<int64_t> next_layer_id_shape = {1};
    
    // 添加输入（顺序严格匹配 proto.h）
    ADD_INPUT(1, schedule_context, inDtype, schedule_context_shape);
    ADD_INPUT(2, expert_scales, DT_FLOAT, expert_scales_shape);
    ADD_INPUT(3, layer_id, DT_INT32, layer_id_shape);
    
    // 添加输出（顺序严格匹配 proto.h）
    ADD_OUTPUT(1, y, DT_FLOAT16, y_shape);
    ADD_OUTPUT(2, next_layer_id, DT_INT32, next_layer_id_shape);

    ADD_INPUT_ATTR(hidden_size, 20480)
    ADD_INPUT_ATTR(token_dtype, 1)
    ADD_INPUT_ATTR(need_schedule, 15)

    outputs.push_back(attention_worker_combine_op);
    // 添加完毕
    return SUCCESS;
}
