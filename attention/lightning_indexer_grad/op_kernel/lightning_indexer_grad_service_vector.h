...
__aicore__ inline void LIGVector<LIGT>::InitOutputDkcoreGm(GlobalTensor<float> dkCoreWorkspaceGM, LIGCommon::ConstInfo constInfo, LIGCommon::RunInfo runInfo)
{
    ...
    if constexpr (LIGT::layout == LIG_LAYOUT::BSND) {
        dkCorePerSize =  constInfo.headNumK * constInfo.headDim / 2;
    } else if constexpr (LIGT::layout == LIG_LAYOUT::TND) {
        dkCorePerSize =  runInfo.actualSeqK * constInfo.headDim / 2;
    }
    uint64_t zeroDataTensorSize = dkCorePerSize <= MAX_UB_SIZE ? dkCorePerSize : MAX_UB_SIZE;
    LocalTensor<float> zeroDataTensor = unifiedBuffer.GetWithOffset<float>(zeroDataTensorSize, gatherPingUbOffset);
    uint64_t blockGroupBegin = (GetBlockIdx() % 2 == 0) ? 0 : dkCorePerSize;
    uint64_t looptimes = (dkCorePerSize - 1) / MAX_UB_SIZE + 1;
    AscendC::Duplicate(zeroDataTensor, static_cast<float>(0.0), zeroDataTensorSize);
    AscendC::SetFlag<HardEvent::V_MTE3>(eventIdVToMte3);
    AscendC::WaitFlag<HardEvent::V_MTE3>(eventIdVToMte3);

    for (int i = 0; i < looptimes; i++) {
        AscendC::DataCopy(dkCoreWorkspaceGM[GetBlockIdx() * dkCorePerSize + i * zeroDataTensorSize], zeroDataTensor, zeroDataTensorSize);
    }
    AscendC::SetFlag<HardEvent::MTE3_MTE2>(eventIdMte3ToMte2);
    AscendC::WaitFlag<HardEvent::MTE3_MTE2>(eventIdMte3ToMte2);
}
