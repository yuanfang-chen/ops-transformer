    ...
    GlobalTensor<float> dkWorkSpaceGmTensor, LIGCommon::ConstInfo constInfo, LIGCommon::RunInfo runInfo)
{
    ...
    for (uint64_t i = loopBegin; i < loopEnd; i++) {
        ...
        // [B, S2, N2, D]
        uint64_t dkeyOffset = singleIndice * constInfo.headDim;
        uint64_t dkeyCoreOffset;
        if constexpr (LIGT::layout == LIG_LAYOUT::BSND) {
            dkeyCoreOffset = dkeyOffset + currentCoreIndex / 2 * constInfo.seqlenK * constInfo.headDim;
        } else if constexpr (LIGT::layout == LIG_LAYOUT::TND) {
            dkeyCoreOffset = dkeyOffset + currentCoreIndex / 2 * runInfo.actualSeqK * constInfo.headDim;
        }
        ...
        DataCopy(dkWorkSpaceGmTensor[dkeyCoreOffset], gatherPingPongUb, constInfo.headDim);
