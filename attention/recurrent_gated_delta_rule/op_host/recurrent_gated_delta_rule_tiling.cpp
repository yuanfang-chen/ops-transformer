...
ge::graphStatus RecurrentGatedDeltaRuleTiling::AnalyzeShapesParser()
{
    ...
    const auto &actSeqlensShape = context_->GetInputShape(ACTUAL_SEQ_LENGTHS_INDEX)->GetOriginShape();
    ...
    OP_CHECK_IF(tilingData_.t <=0,
        OP_LOGE(inputParams_.opName, "T should greater than 0, but T is %ld.", tilingData_.t),
            return ge::GRAPH_FAILED);
    // // B>=0
    OP_CHECK_IF((tilingData_.b <= 0U),
        OP_LOGE(inputParams_.opName, "B(actualSeaLengths Dim0) should greater than 0, but B is %ld.", tilingData_.b),
            return ge::GRAPH_FAILED);
    // nk>0 nk<=256
...
ge::graphStatus RecurrentGatedDeltaRuleTiling::AnalyzeOptionalShapes()
{
    ...
    const auto &valueShape = context_->GetInputShape(VALUE_INDEX)->GetOriginShape();
    if (tilingData_.hasGama == 1){
        const auto &gShape = context_->GetOptionalInputShape(G_INDEX)->GetOriginShape();
        if (!CheckDim(gShape, G_DIM_NUM, "g") || 
            !CheckDimEqual(gShape, DIM_0, queryShape, DIM_0, "g", "query", "T dimension") ||
            !CheckDimEqual(gShape, DIM_1, valueShape, DIM_1, "g", "value", "Nv dimension")) {
                return ge::GRAPH_FAILED;
            }
    }
    if (tilingData_.hasGamaK == 1) {
        const auto &gkShape = context_->GetOptionalInputShape(GK_INDEX)->GetOriginShape();
        if (!CheckDim(gkShape, GK_DIM_NUM, "gk") || 
            !CheckDimEqual(gkShape, DIM_0, queryShape, DIM_0, "gk", "query", "T dimension") ||
            !CheckDimEqual(gkShape, DIM_1, valueShape, DIM_1, "gk", "value", "Nv dimension") ||
            !CheckDimEqual(gkShape, DIM_2, keyShape, DIM_2, "gk", "key", "Dk dimension")) {
                return ge::GRAPH_FAILED;
        }
    }
    if (tilingData_.hasAcceptedTokens == 1) {
        const auto &numAcceptedShape = context_->GetOptionalInputShape(NUM_ACCEPTED_TOKENS_INDEX)->GetOriginShape();
