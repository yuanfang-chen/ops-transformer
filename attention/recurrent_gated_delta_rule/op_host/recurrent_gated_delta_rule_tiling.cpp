...
ge::graphStatus RecurrentGatedDeltaRuleTiling::AnalyzeShapesParser()
{
    ...
    OP_CHECK_IF(queryShape.GetDim(DIM_0) <=0,
        OP_LOGE(inputParams_.opName, "T should greater than 0, but T is %ld ", queryShape.GetDim(DIM_0)),
            return ge::GRAPH_FAILED);
    // // B>=0
    // OP_CHECK_IF((cuSeqlensShape->GetShapeSize() <= 0U),
    //     OP_LOGE(inputParams_->opName, "B(actualSeaLengths Dims) should greater than 0, but B is %ld", cuSeqlensDims),
    //         return ge::GRAPH_FAILED);
    // nk>0 nk<=256
...
{
    OP_CHECK_IF(context_->GetInputShape(QUERY_INDEX)->GetShapeSize() == 0,
        OP_LOGE(inputParams_.opName, "Query not support empty tensor."),
            return ge::GRAPH_FAILED);
    OP_CHECK_IF(context_->GetInputShape(KEY_INDEX)->GetShapeSize() == 0,
        OP_LOGE(inputParams_.opName, "Key not support empty tensor."),
            return ge::GRAPH_FAILED);
    OP_CHECK_IF(context_->GetInputShape(VALUE_INDEX)->GetShapeSize() == 0,
        OP_LOGE(inputParams_.opName, "Value not support empty tensor."),
            return ge::GRAPH_FAILED);
    OP_CHECK_IF(context_->GetInputShape(BETA_INDEX)->GetShapeSize() == 0,
        OP_LOGE(inputParams_.opName, "Beta not support empty tensor."),
            return ge::GRAPH_FAILED);
    OP_CHECK_IF(context_->GetInputShape(STATE_INDEX)->GetShapeSize() == 0,
        OP_LOGE(inputParams_.opName, "State not support empty tensor."),
            return ge::GRAPH_FAILED);
    OP_CHECK_IF(cuSeqlensShape = context_->GetInputShape(CUSEQLENS_INDEX)->GetShapeSize() == 0,
        OP_LOGE(inputParams_.opName, "ActualSeqLengths not support empty tensor."),
            return ge::GRAPH_FAILED);
    OP_CHECK_IF(ssmStateShape = context_->GetInputShape(SSM_STATE_INDICES_INDEX)->GetShapeSize() == 0,
        OP_LOGE(inputParams_.opName, "SsmStateIndices not support empty tensor."),
            return ge::GRAPH_FAILED);
    OP_CHECK_IF(context_->GetOutputShape(OUT_INDEX)->GetShapeSize() == 0,
        OP_LOGE(inputParams_.opName, "Out not support empty tensor."),
            return ge::GRAPH_FAILED);
    OP_CHECK_IF(gamaFlag && context_->GetOptionalInputDesc(G_INDEX)->GetShapeSize() == 0,
        OP_LOGE(inputParams_.opName, "G not support empty tensor."),
            return ge::GRAPH_FAILED);
    OP_CHECK_IF(gamaKFlag && context_->GetOptionalInputDesc(GK_INDEX)->GetShapeSize() == 0,
        OP_LOGE(inputParams_.opName, "Gk not support empty tensor."),
            return ge::GRAPH_FAILED);
    OP_CHECK_IF(accFlag && context_->GetOptionalInputDesc(ACC_TO_INDEX)->GetShapeSize() == 0
