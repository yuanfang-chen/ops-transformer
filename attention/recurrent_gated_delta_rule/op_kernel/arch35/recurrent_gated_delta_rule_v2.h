...
private:
    __aicore__ inline void CopyInGamaK(uint32_t tIdxStart, uint32_t tIdxEnd, uint32_t head_i)
    {
        ...
        // DumpTensorForDim2(gamaUb_, 1, (tIdxEnd - tIdxStart) * alignDK_);
    }
    ...
    __aicore__ inline void CopyInQKV(uint32_t tIdxStart, uint32_t tIdxEnd, uint32_t head_i)
    {
        uint32_t kHeadIdx = head_i / (NV_ / NK_);
        uint64_t qkGmOffset = (uint64_t)tIdxStart * NK_ * DK_ + (uint64_t)kHeadIdx * DK_;
        uint64_t vGmOffset = (uint64_t)tIdxStart * NV_ * DV_ + (uint64_t)head_i * DV_;
        ...
        Cast(queryUb_, queryInUb, RoundMode::CAST_NONE, dealRowCount * dstRowStride);
        AscendC::PipeBarrier<PIPE_V>();
        // DumpTensorForDim2(queryInUb, 2, dealRowCount * dstRowStride);
        Muls(queryUb_, queryUb_, scale_, dealRowCount * dstRowStride);
        // DumpTensorForDim2(queryUb_, 3, dealRowCount * dstRowStride);
        inputQueue1_.FreeTensor(queryInUb);
        ...
        inputQueue1_.DeQue<inType>();
        Cast(keyUb_, keyInUb, RoundMode::CAST_NONE, dealRowCount * dstRowStride);
        // DumpTensorForDim2(keyUb_, 4, dealRowCount * dstRowStride);
        inputQueue1_.FreeTensor(keyInUb);

        // CopyValue
        actDataLen = DV_;
        srcRowStride = NV_ * DV_;
        dstRowStride = alignDV_;
        LocalTensor<inType> valueInUb = inputQueue1_.AllocTensor<inType>();
        CopySingleMatrixNDToND(valueInUb, valueGm_[vGmOffset], dealRowCount, actDataLen, srcRowStride, dstRowStride);
        inputQueue1_.EnQue<inType>(valueInUb);
        inputQueue1_.DeQue<inType>();
        Cast(valueUb_, valueInUb, RoundMode::CAST_NONE, dealRowCount * dstRowStride);
        // DumpTensorForDim2(valueUb_, 5, dealRowCount * dstRowStride);
        inputQueue1_.FreeTensor(valueInUb);
    }
    ...
    __aicore__ inline void CopyInState(uint32_t stateBlockIdx, uint32_t head_i, uint32_t dvIdx, uint32_t curSingleV)
    {
        ...
        // DumpTensorForDim2(stateUb_, 6, dealRowCount * dstRowStride);
    }

    __aicore__ inline void UpdateGama(uint32_t bSeqIdx, float gama)
    {
        Adds(gamaUb_[bSeqIdx * alignDK_], gamaUb_[bSeqIdx * alignDK_], gama, alignDK_);
        AscendC::PipeBarrier<PIPE_V>();
        Exp(gamaUb_[bSeqIdx * alignDK_], gamaUb_[bSeqIdx * alignDK_], alignDK_);
    }

    __aicore__ inline void Compute(uint32_t seq0, uint32_t seq1, uint32_t tIdx, uint32_t bSeqIdx, uint32_t head_i, uint32_t dvIdx, uint32_t curSingleV)
    {
        uint32_t stateShape[2] = {curSingleV, DK_};
        // DumpTensorForDim2(stateUb_, 10, curSingleV * alignDK_);
        // VecMulMatForBigRowCount(stateUb_, gamaUb_[bSeqIdx * alignDK_], stateUb_, curSingleV, alignDK_, DK_);
        VecMulMatVF(stateUb_, gamaUb_[bSeqIdx * alignDK_], stateUb_, curSingleV, alignDK_);
        if (dvIdx == 0 && bSeqIdx == 0) {
            CopyInQK(seq0, seq1, head_i);
        }
        AscendC::PipeBarrier<PIPE_V>();
        // DumpTensorForDim2(stateUb_, 11, curSingleV * alignDK_);

        // VecMulMat + RowSum
        LocalTensor<float> tmpUb = tmpBuff2_.Get<float>();
        // VecMulMatForBigRowCount(tmpUb, keyUb_[bSeqIdx * alignDK_], stateUb_, curSingleV, alignDK_, DK_);
        VecMulMatVF(tmpUb, keyUb_[bSeqIdx * alignDK_], stateUb_, curSingleV, alignDK_);
        AscendC::PipeBarrier<PIPE_V>();
        // curSingleV >= 256时有精度问题
        LocalTensor<float> reduceSumResUb = tmpBuff1_.Get<float>();
        // RowSum(reduceSumResUb, tmpUb, curSingleV, alignDK_, DK_);
        ReduceSum<float, Pattern::Reduce::AR, true>(reduceSumResUb, tmpUb, stateShape, true);
        if (dvIdx == 0 && bSeqIdx == 0) {
            CopyInV(seq0, seq1, head_i);
        }
        AscendC::PipeBarrier<PIPE_V>();
        // DumpTensorForDim2(reduceSumResUb, 12, curSingleV);
        ...
        // Outer + Add
        // Brcb(tmpUb, reduceSumResUb, (curSingleV + 7) / 8, {1, 8});
        // AscendC::PipeBarrier<PIPE_V>();
        // Outer<true>(stateUb_, tmpUb, keyUb_[bSeqIdx * alignDK_], curSingleV, alignDK_, DK_);
        OuterAddVF(stateUb_, reduceSumResUb, keyUb_[bSeqIdx * alignDK_], stateUb_, curSingleV, alignDK_);
        AscendC::PipeBarrier<PIPE_V>();
        // // DumpTensorForDim2(stateUb_, 13, curSingleV * alignDK_);

        // CopyOutState
        LocalTensor<outType> stateOutLocal = stateOutQueue_.AllocTensor<outType>();
        Cast(stateOutLocal, stateUb_, AscendC::RoundMode::CAST_RINT, alignDK_ * curSingleV);
        // DumpTensorForDim2(stateOutLocal, 14, curSingleV * alignDK_);
        ...
        stateOutParams.srcStride = (alignDK_ - DK_) / (BYTE_BLOCK / sizeof(outType));
        ...
        // State * query * scale
        // VecMulMatForBigRowCount(tmpUb, queryUb_[bSeqIdx * alignDK_], stateUb_, curSingleV, alignDK_, DK_);
        ...
        ReduceSum<float, Pattern::Reduce::AR, true>(reduceSumResUb, tmpUb, stateShape, true);
        AscendC::PipeBarrier<PIPE_V>();
        // DumpTensorForDim2(reduceSumResUb, 15, curSingleV);

        // CopyAttentionOut
        LocalTensor<outType> attnOutLocal = attnOutQueue_.AllocTensor<outType>();
        Cast(attnOutLocal, reduceSumResUb, AscendC::RoundMode::CAST_RINT, curSingleV);
        AscendC::PipeBarrier<PIPE_V>();
        // DumpTensorForDim2(attnOutLocal, 16, curSingleV);
            ...
            for (uint64_t tIdx = seq0; tIdx < seq1; tIdx++) {
                ...
                if (dvIdx == 0) {
                    if (hasGama_) {
                        ...
                        UpdateGama(tIdx - seq0, gama_);
                        AscendC::PipeBarrier<PIPE_V>();
                    }
                }
                beta_ = Cast(betaGm_.GetValue(gamaBetaGmOffset));
                Compute(seq0, seq1, tIdx, tIdx - seq0, head_i, dvIdx, curSingleV);
            }
        }
    }
