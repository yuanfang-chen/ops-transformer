...
template<typename T>
__simd_vf__ void VecMulMatVFImpl(__ubuf__ T* matrixUb, __ubuf__ T* rowVecUb, __ubuf__ T* outputUb,
...
template<typename T>
__aicore__ inline void VecMulMatVF(const LocalTensor<T>& matrixInUb, const LocalTensor<T>& rowVecInUb,
                                   const LocalTensor<T>& matrixOutUb, const uint16_t row, const uint32_t col)
{
    ...
    VecMulMatVFImpl(matrixUb, rowVecUb, outputUb, floatRepSize, dLoops, dTail, dTailLoop, row, col);
}
