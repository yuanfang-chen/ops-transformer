...
def cpu_recurrent_gated_delta_rule(
    ...
    T, n_heads_v, Dv = q.shape
    B = act_seq_len.shape
    q = q.to(torch.float32)
    k = k.to(torch.float32)
    v = vars.to(torch.float32)
...
def run_recurrent_gated_delta_rule_eager(B, mtp, nk, nv, dk, dv, actual_seq_lengths=None,
    ...
    print(f"act_seq_len: shape {act_seq_len.shape}, dtype: {act_seq_len.dtype}")
    print(f"ssm_state_indices: shape {ssm_state_indices.shape}, dtype: {ssm_state_indices.dtype}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--scale_value', type=int, default=None, help='query scaling factor')
