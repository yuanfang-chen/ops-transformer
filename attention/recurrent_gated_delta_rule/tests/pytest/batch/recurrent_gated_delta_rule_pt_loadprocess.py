...
def test_recurrent_gated_delta_rule_process(filepath, device_id=0):
    ...
    npu_result, npu_state_result = torch.ops.custom.recurrent_gated_delta_rule(
                query.npu(), key.npu(), value.npu(), state.npu(), beta.npu(), scale_value,
                act_seq_len.npu(), ssm_state_indices, num_accepted_tokens, g, gk)
