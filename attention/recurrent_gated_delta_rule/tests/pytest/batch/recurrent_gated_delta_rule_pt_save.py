...
from recurrent_gated_delta_rule_golden import cpu_recurrent_gated_delta_rule, get_seq_used_by_batch
import pandas as pd
import numpy as np
import torch
import torch_npu
...
def load_excel_test_cases(excel_file_path: str, sheetname: str):
    ...
    try:
        ...
        required_columns = [
            ...
            "has_gamma_k", "has_num_accepted_tokens", "scale_value", "num_accepted_tokens", "data_type",
            "query_datarange","key_datarange","value_datarange","gama_datarange","gama_k_datarange",
...
def recurrent_gated_delta_rule_output_single(data_case):
    ...
    gama_datarange = [float(x.strip()) for x in gama_datarange.split(',')]
    gama_k_datarange = [float(x.strip()) for x in gama_k_datarange.split(',')]
    ...
    if has_num_accepted_tokens == 'True' and num_accepted_tokens == None:
        num_accepted_tokens = torch.randint(1, actual_seq_lengths[i] + 1, (B), dtype=torch.int32)
    ...
    query = rand_range((T, nk, dv, dk), query_datarange, data_type)
    key = rand_range((T, nk, dk), key_datarange, data_type)
    value = rand_range((T, nv, dv), value_datarange, data_type)
    g = rand_range((T, nv), gamma_datarange, data_type=torch.float32) if has_gamma == 'True' else None
    gk = rand_range((T, nv, dk), gamma_k_datarange, data_type=torch.float32) if has_gamma_k == 'True' else None
    beta = rand_range((T, nv), beta_datarange, data_type=torch.float32)
    num_accepted_tokens = torch.tensor(num_accepted_tokens) if has_num_accepted_tokens == 'True' else None
    state = rand_range((block_num, nv, dv, dk), state_datarange, data_type)
    act_seq_len = torch.tensor(actual_seq_lengths, data_type=torch.int32)
    ssm_state_indices = torch.tensor(ssm_state_indices, data_type=torch.int32)
    ...
    cpu_result, cpu_state_result = test_operator.forward(
        ...
        num_accepted_tokens=num_accepted_tokens, gamma=g, gamma_k=gk)
