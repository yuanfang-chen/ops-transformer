...
public:
        ...
        uint32_t numElemsAligned)
    {
        ...
        // 512个元素，以128为单位分治求和，512->256->128
        for (uint32_t columnStrideIndex = 2; columnStrideIndex <= loopCount; columnStrideIndex *= 2) {
            ReduceSumByPair(srcUb, numRowsRound, loopCount, columnStrideIndex, dataBlockStride, blockNumPerRow);
            AscendC::PipeBarrier<PIPE_V>();
        }
    ...
    __aicore__ inline
    void RowsumSPECTILE256(const AscendC::LocalTensor<half> &srcUb, const AscendC::LocalTensor<half> &rowsumUb,
        const AscendC::LocalTensor<half> &tvUbTensor, uint32_t numRowsRound, uint32_t numElems,
        uint32_t numElemsAligned)
    {
        // Vector计算单元每个迭代最多处理256Byte数据，因此half低精度场景，每次迭代最多处理256/2=128个元素
        uint32_t loopCount = numElemsAligned / HALF_VECTOR_SIZE; // half低精度场景，每行需要256/128=2次循环处理
        // 每个datablock长度32Byte，因此half低精度场景，每个datablock内有32/2=16个元素
        uint8_t blockNumPerRow = numElemsAligned / BLOCK_SIZE; // half低精度场景，每行共有256/16=16个datablock
        uint8_t dataBlockStride = 1;

        // 256个元素，以128为单位分治求和，256->128
        ReduceSumByPair(srcUb, numRowsRound, loopCount, columnStrideIndex, dataBlockStride, blockNumPerRow);
        AscendC::PipeBarrier<PIPE_V>();
    ...
    __aicore__ inline
    void RowmaxSPECTILE256(const AscendC::LocalTensor<half> &srcUb, const AscendC::LocalTensor<half> &rowmaxUb,
                            const AscendC::LocalTensor<half> &tvUbTensor, uint32_t numRowsRound, uint32_t numElems,
                            uint32_t numElemsAligned)
    {
        // Vector计算单元每个迭代最多处理256Byte数据，因此half低精度场景，每次迭代最多处理256/2=128个元素
