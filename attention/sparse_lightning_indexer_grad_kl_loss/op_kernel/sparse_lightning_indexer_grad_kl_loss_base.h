...
__aicore__ inline void SparseLightningIndexerGradKLLossBase<SLIT>::InitWorkspace(__gm__ uint8_t *workspace)
{
    ...
    if constexpr (LAYOUT_T == SLILayout::TND) {
        scatterAddOffset = actualSeqLengthsKeyGm.GetValue(constInfo.bSize - 1) * constInfo.dSizeQueryIndex * sizeof(float); //batch
    } else {
        scatterAddOffset = constInfo.bSize * constInfo.s2Size * constInfo.dSizeQueryIndex * sizeof(float); //batch
    }
    ...
    scatterAddResPang.SetGlobalBuffer(
        (__gm__ T *)(workspace + totalOffset));
    ...
    if ASCEND_IS_AIV {
        ...
        AscendC::InitOutput(scatterAddResPang[t2Start * constInfo.dSizeQueryIndex], constInfo.dSizeQueryIndex * (t2End - t2Start), static_cast<T>(0));
...
__aicore__ inline void SparseLightningIndexerGradKLLossBase<SLIT>::DeterProcess()
{
    ...
    if ASCEND_IS_AIV {
        ...
        vector2Service.InitVector2GM(scatterAddRes, topKIndexGm, dKeyIndexGm, actualSeqLengthsQueryGm, actualSeqLengthsKeyGm,
            lossRes, lossGm, scatterAddResPang, actualSeqLengthsKeyGm);
