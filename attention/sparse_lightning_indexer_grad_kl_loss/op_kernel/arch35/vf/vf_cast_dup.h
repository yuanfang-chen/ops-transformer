...
__simd_vf__ inline void CastDupBasicVF(__ubuf__ OUT_T * dstUb, __ubuf__ T * srcUb, uint32_t m, uint32_t maskScaler)
{
    ...
    if constexpr (IsSameType<OUT_T, half>::value) {
        ...
        Cast<OUT_T, float, castTraitZero>(vreg_relugrad_bf, vreg_input_new, pregFullExeB16);
        DeInterleave(vreg_dw_bf_even, vreg_dw_bf_odd, vreg_relugrad_bf, vreg_relugrad_bf);
