...
public:
    ...
    __aicore__ inline void InitGlobalBuffer(GlobalTensor<INPUT_T> &queryGm, GlobalTensor<INPUT_T> &queryIndexGm, 
                                            GlobalTensor<INPUT_T> &queryRopeGm, GlobalTensor<T> &bmm2Res, 
                                            GlobalTensor<INPUT_T> &gatherSYResGm, GlobalTensor<OUT_T> &dQueryIndexGm);
    __aicore__ inline void ComputeMm1(Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> &bmm1ResBuf, 
    ...
    __aicore__ inline void ComputeMm2(Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> &bmm2ResBuf,
                                    BuffersPolicyDB<BufferType::L1, SyncType::CROSS_CORE_SYNC_BOTH> &sYL1Buf, 
...
{
    // init l1 buffer
    queryBuf.Init(*l1BufferManagerPtr, 128 * 576 * sizeof(INPUT_T));
    sYQueryL1Buf.Init(*l1BufferManagerPtr, 64 * 128 * sizeof(INPUT_T));
    sYGatherResBuf.Init(*l1BufferManagerPtr, 128 * 128 * sizeof(OUT_T));
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void SligKlLossBlockCube<TEMPLATE_ARGS>::InitGlobalBuffer(GlobalTensor<INPUT_T> &queryGm, GlobalTensor<INPUT_T> &queryIndexGm, 
                                            GlobalTensor<INPUT_T> &queryRopeGm, GlobalTensor<T> &bmm2Res, 
                                            GlobalTensor<INPUT_T> &gatherSYResGm, GlobalTensor<OUT_T> &dQueryIndexGm)
{
    this->queryGm = queryGm;
    this->qRopeGm = queryRopeGm;
    this->queryIndexGm = queryIndexGm;
    this->mm2ResGm = bmm2Res;
    this->gatherSYResGm = gatherSYResGm;
    this->mm4ResGm = dQueryIndexGm;
};
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void SligKlLossBlockCube<TEMPLATE_ARGS>::ComputeMm1(Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> &bmm1ResBuf, 
                                BuffersPolicyDB<BufferType::L1, SyncType::CROSS_CORE_SYNC_BOTH> &sYL1Buf, 
                                SLIGradKLLossRunInfo &runInfo, SLIGradKLLossConstInfo &constInfo, SLIGradKLLossKRunInfo &pRunInfo)
{
    ...
    uint32_t kLoopTimes = CeilDiv(kSize, P_GATHER_SPLIT_SIZE); //循环次数，循环累计kSize行搬出UB  2048 / 128
    uint32_t tailLoopKSize = kSize - (kLoopTimes - 1) * P_GATHER_SPLIT_SIZE; //尾块大小
    ...
    for (uint32_t kIdx = 0; kIdx < kLoopTimes; kIdx++) {
        ...
        CrossCoreWaitFlag<2, PIPE_MTE1>(SYNC_GATHER_TO_MM12_FLAG[kIdx % 2]);
        LocalTensor<INPUT_T> gatherL1Tensor = gatherResL1Buf.GetTensor<INPUT_T>();
        uint64_t l0cOffset = kIdx * P_GATHER_SPLIT_SIZE * constInfo.gSizeQuery;
        ...
        CrossCoreSetFlag<2, PIPE_MTE1>(SYNC_GATHER_TO_MM12_FLAG[kIdx % 2]);
    }
    mm1L0CBuffer.Set<HardEvent::M_FIX>();
    mm1L0CBuffer.Wait<HardEvent::M_FIX>();      
    // fix2ub
    CrossCoreWaitFlag<2, PIPE_FIX>(SYNC_MM2_TO_V1_FLAG[pRunInfo.kTaskId % 3]);
    ...
    fixpipe2UbParams.srcStride = AlignTo(fixpipe2UbParams.mSize, static_cast<uint16_t>(16));
    // mmResUb上两行之间的间隔，单位：element。
    fixpipe2UbParams.dstStride = AlignTo(constInfo.pKBaseSize, static_cast<int32_t>(16));
    ...
    CrossCoreSetFlag<2, PIPE_FIX>(SYNC_MM2_TO_V1_FLAG[pRunInfo.kTaskId % 3]);
};

TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void SligKlLossBlockCube<TEMPLATE_ARGS>::ComputeMm2(Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> &bmm2ResBuf,
                                    BuffersPolicyDB<BufferType::L1, SyncType::CROSS_CORE_SYNC_BOTH> &sYL1Buf, 
                                    SLIGradKLLossRunInfo &runInfo, SLIGradKLLossConstInfo &constInfo,
                                    SLIGradKLLossKRunInfo &syRunInfo)
{
    ...
    uint32_t kLoopTimes = CeilDiv(kSize, SY_GATHER_SPLIT_SIZE); // 循环次数，循环累计kSize行搬出UB
    uint32_t tailLoopKSize = kSize - (kLoopTimes - 1) * SY_GATHER_SPLIT_SIZE; // 尾块大小
    LocalTensor<T> outTensor = bmm2ResBuf.template GetTensor<T>();
    MMParam mmParam = {
        constInfo.gSizeQueryIndex,
        SY_GATHER_SPLIT_SIZE,
    ...
    for (uint32_t kIdx = 0; kIdx < kLoopTimes; kIdx++) {
        ...
        CrossCoreWaitFlag<2, PIPE_MTE1>(SYNC_GATHER_TO_MM12_FLAG[kIdx % 2]);
        LocalTensor<INPUT_T> gatherL1Tensor = gatherResL1Buf.GetTensor<INPUT_T>();
        uint64_t l0cOffset = kIdx * SY_GATHER_SPLIT_SIZE * constInfo.gSizeQueryIndex;
    ...
    // fix2ub
    CrossCoreWaitFlag<2, PIPE_FIX>(SYNC_MM2_TO_V1_FLAG[syRunInfo.kTaskId % 3]);
    FixpipeParamsC310<CO2Layout::ROW_MAJOR> fixpipe2UbParams;
    fixpipe2UbParams.nSize = (kSize + 7) >> 3 << 3;
    fixpipe2UbParams.mSize = (constInfo.gSizeQueryIndex + 1) >> 1 << 1;
    fixpipe2UbParams.srcStride = AlignTo(fixpipe2UbParams.mSize, static_cast<uint16_t>(16));
    // mmResUb上两行之间的间隔，单位：element。
    fixpipe2UbParams.dstStride = AlignTo(constInfo.syKBaseSize, static_cast<int32_t>(16));
    // 双目标模式，按M维度拆分，M / 2 * N写入每个UB, M必须为2的倍数
    fixpipe2UbParams.dualDstCtl = 1;
    fixpipe2UbParams.params.ndNum = 1;
    fixpipe2UbParams.params.srcNdStride = 0;
    ...
    CrossCoreSetFlag<2, PIPE_FIX>(SYNC_MM2_TO_V1_FLAG[syRunInfo.kTaskId % 3]);
    //fix2gm
    ...
    fixpipe2GmParams.srcStride = AlignTo(fixpipe2GmParams.mSize, static_cast<uint16_t>(16));
    fixpipe2GmParams.dstStride = AlignTo(constInfo.kSize, static_cast<uint32_t>(16));
...
{
    uint32_t kSize = pRunInfo.kProcessSize; // 2048 
    uint32_t kLoopTimes = CeilDiv(kSize, P_GATHER_SPLIT_SIZE); //循环次数，循环累计kSize行搬出UB
    uint32_t tailLoopKSize = kSize - (kLoopTimes - 1) * P_GATHER_SPLIT_SIZE; //尾块大小
    ...
    CrossCoreWaitFlag<2, PIPE_MTE1>(SYNC_V6_TO_C3_FLAG);
    for (uint32_t kIdx = 0; kIdx < kLoopTimes; kIdx++) {
        if (kIdx == kLoopTimes - 1) {
            mmParam.singleM = tailLoopKSize;
        }
        reluGradL1Tensor = reluGradResL1Buf.GetTensor<INPUT_T>();
        int64_t reluGradOffset = kIdx * P_GATHER_SPLIT_SIZE * AlignTo(constInfo.gSizeQueryIndex,  static_cast<uint32_t>(C0_SIZE));
        // 拷贝左矩阵
        ...
        fixpipe2UbParams.srcStride = AlignTo(fixpipe2UbParams.mSize, static_cast<uint16_t>(16));
        // mmResUb上两行之间的间隔，单位：element。
        fixpipe2UbParams.dstStride = AlignTo(constInfo.dSizeQueryIndex, static_cast<uint32_t>(16));
        // 双目标模式，按M维度拆分，M / 2 * N写入每个UB, M必须为2的倍数
        fixpipe2UbParams.dualDstCtl = 1;
        fixpipe2UbParams.params.ndNum = 1;
        fixpipe2UbParams.params.srcNdStride = 0;
        ...
        CrossCoreSetFlag<2, PIPE_FIX>(SYNC_C3_TO_V7_FLAG);
    }
                                ...
                                SLIGradKLLossRunInfo &runInfo, SLIGradKLLossConstInfo &constInfo, SLIGradKLLossKRunInfo &pRunInfo, bool isFixOut)
{
    ...
    for (uint32_t kIdx = 0; kIdx < kLoopTimes; kIdx++) { // 2048 / 128
        ...
        // 拷贝右矩阵
        int64_t reluGradOffset = kIdx * P_GATHER_SPLIT_SIZE * AlignTo(constInfo.gSizeQueryIndex,  static_cast<uint32_t>(C0_SIZE));
        int64_t gatherWorkspaceOffset = pRunInfo.kTaskId * constInfo.pKBaseSize * dSize + kIdx * P_GATHER_SPLIT_SIZE * dSize;
    ...
    if (isFixOut) {
        ...
        fixpipe2GmParams.mSize = constInfo.gSizeQueryIndex;
        fixpipe2GmParams.srcStride = AlignTo(fixpipe2GmParams.mSize, static_cast<uint16_t>(16));
        fixpipe2GmParams.dstStride = AlignTo(dSize, static_cast<uint32_t>(16));
        fixpipe2GmParams.dualDstCtl = 0;
        fixpipe2GmParams.params.ndNum = 1;
        fixpipe2GmParams.params.srcNdStride = 0;
        fixpipe2GmParams.params.dstNdStride = 0;
        ...
        Fixpipe<OUT_T, T, PFA_CFG_ROW_MAJOR_GM>(mm4ResGm[runInfo.queryIndexTensorOffset], mm4L0CBuffer.GetTensor<T>(), fixpipe2GmParams); //TODO db?
    }
};
