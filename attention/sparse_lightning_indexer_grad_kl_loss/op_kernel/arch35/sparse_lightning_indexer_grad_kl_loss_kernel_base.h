    ...
    int64_t taskId, int64_t bIdx, int64_t s1Idx)
{
    ...
    for (int32_t s2TaskId = 0; s2TaskId < runInfo.s2LoopTimes + 1; ++s2TaskId) {
        ...
        if (s2TaskId >= 0 && notLastLoop) {
            ...
            for (int32_t loopIdx = 0; loopIdx < kRunInfoNeg0.s2SingleLoopTimes; ++loopIdx) {
                SetSYSingleRunInfo(kRunInfoNeg0, runInfo, s2TaskId, loopIdx);
                if ASCEND_IS_AIV {
                    vecBlock.ProcessVector0(this->sYL1Buf.Get(), runInfo, kRunInfoNeg0);
                }
            }
            if ASCEND_IS_AIC {
                cubeBlock.ComputeMmSy(this->bmm1Buffers.GetCube(), sYL1Buf, runInfo, constInfo, kRunInfoNeg0);
            }
        }

        if (s2TaskId >= 1) {
            Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> bmm1Res = this->bmm1Buffers.GetVec();
            for (int32_t loopIdx = 0; loopIdx < kRunInfoNeg1.s2SingleLoopTimes; ++loopIdx) {
                SetSYSingleRunInfo(kRunInfoNeg1, runInfo, s2TaskId - 1, loopIdx);
                if ASCEND_IS_AIV {
                                                                                   ...
                                                                                   int64_t taskId, int64_t bIdx, int64_t s1Idx)
{
    ...
    for (int32_t s2TaskId = 0; s2TaskId < runInfo.s2LoopTimes + 3; ++s2TaskId) {
        SLIGradKLLossKRunInfo &kRunInfoNeg0 = kRunInfos[s2TaskId % MODE_NUM_4];
        SLIGradKLLossKRunInfo &kRunInfoNeg3 = kRunInfos[(s2TaskId + 1) % MODE_NUM_4];
        SLIGradKLLossKRunInfo &kRunInfoNeg2 = kRunInfos[(s2TaskId + 2) % MODE_NUM_4];
        ...
        if (s2TaskId >= 0 && notLastThreeLoop) {
            ...
            for (int32_t loopIdx = 0; loopIdx < kRunInfoNeg0.s2SingleLoopTimes; ++loopIdx) {
                SetPSingleRunInfo(kRunInfoNeg0, runInfo, s2TaskId, loopIdx);
                if ASCEND_IS_AIV {
                    vecBlock.ProcessVector3(this->sYL1Buf.Get(), runInfo, kRunInfoNeg0);
                }
            }
            if ASCEND_IS_AIC {
                cubeBlock.ComputeMmP(this->bmm1Buffers.GetCube(), sYL1Buf, runInfo, constInfo, kRunInfoNeg0);
            }
        }
        if (s2TaskId >= 3) {
            if ASCEND_IS_AIC {
                isFixOut = !notLastLoop;
                cubeBlock.ComputeMm3(this->bmm3Buffer.Get(), this->reluGradResL1Buf.Get(), runInfo, constInfo, kRunInfoNeg3);
                cubeBlock.ComputeMm4(reluGradResL1Buf.Get(), runInfo, constInfo, kRunInfoNeg3, isFixOut);
            }
            for (int32_t loopIdx = 0; loopIdx < kRunInfoNeg3.s2SingleLoopTimes; ++loopIdx) {
                SetPSingleRunInfo(kRunInfoNeg3, runInfo, s2TaskId - 3, loopIdx);
                if ASCEND_IS_AIV {
                    vecBlock.ProcessVector7(this->bmm3Buffer.Get(), runInfo, kRunInfoNeg3);
                }
            }
        }
        if (s2TaskId >= 2 && notLastLoop) {
            Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> bmm1Res = this->bmm1Buffers.GetVec();
            for (int32_t loopIdx = 0; loopIdx < kRunInfoNeg2.s2SingleLoopTimes; ++loopIdx) {
                SetPSingleRunInfo(kRunInfoNeg2, runInfo, s2TaskId - 2, loopIdx);
                if ASCEND_IS_AIV {
                    vecBlock.ProcessVector4(bmm1Res, runInfo, kRunInfoNeg2);
                    vecBlock.ProcessVector5(runInfo, kRunInfoNeg2);
                    vecBlock.ProcessVector6(this->reluGradResL1Buf.Get(), runInfo, kRunInfoNeg2);
                }
            }
        }
    }
...
{
    kRunInfo.s2SingleIdx = taskId;
    kRunInfo.s2Idx = s2TaskId * constInfo.pKBaseSize * 2 / VEC_SY_BASESIZE + kRunInfo.s2SingleIdx;
    kRunInfo.isS2end = kRunInfo.s2SingleIdx >= kRunInfo.s2SingleLoopTimes - 1 && s2TaskId >= runInfo.s2LoopTimes - 1;
...
SparseLightningIndexerGradKLLossKernelBase<CubeBlockType, VecBlockType>::InitWorkspace(__gm__ uint8_t *workspace)
{
    ...
    if ASCEND_IS_AIV {
        ...
        int64_t avgCost = CeilDiv(constInfo.totalCost, totalCoreNum);
