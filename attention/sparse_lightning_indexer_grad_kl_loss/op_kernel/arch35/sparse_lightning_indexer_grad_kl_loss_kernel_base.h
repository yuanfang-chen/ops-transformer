    ...
    TPipe *tPipe)
{
    ...
    if ASCEND_IS_AIV {
        ...
        cubeBlock.InitGlobalBuffer(queryGm, queryIndexGm, queryRopeGm, reluRes, gatherSYRes, dQueryIndexGm);
    }
}
                                                                                   ...
                                                                                   int64_t taskId, int64_t bIdx, int64_t s1Idx)
{
    ...
    SLIGradKLLossKRunInfo kRunInfos[2];
    if ASCEND_IS_AIV {
    ...
    for (int32_t s2TaskId = 0; s2TaskId < runInfo.s2LoopTimes + 1; ++s2TaskId) {
        ...
        if (s2TaskId >= 0 && notLastLoop) {
            SetSYRunInfo(kRunInfos[s2TaskId % 2], runInfo, s2TaskId);
            for (int32_t loopIdx = 0; loopIdx < kRunInfos[s2TaskId % 2].s2SingleLoopTimes; ++loopIdx) {
                SetSYSingleRunInfo(kRunInfos[s2TaskId % 2], runInfo, s2TaskId, loopIdx);
                if ASCEND_IS_AIV {
                    vecBlock.ProcessVector0(this->sYL1Buf.Get(), runInfo, kRunInfos[s2TaskId % 2]);
                }
            }
            if ASCEND_IS_AIC {
                cubeBlock.ComputeMm2(this->bmm1Buffers.GetCube(), sYL1Buf, runInfo, constInfo, kRunInfos[s2TaskId % 2]);
            }
        }

        if (s2TaskId >= 1) {
            Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> bmm1Res = this->bmm1Buffers.GetVec();
            for (int32_t loopIdx = 0; loopIdx < kRunInfos[(s2TaskId + 1) % 2].s2SingleLoopTimes; ++loopIdx) {
                SetSYSingleRunInfo(kRunInfos[(s2TaskId + 1) % 2], runInfo, s2TaskId - 1, loopIdx);
                if ASCEND_IS_AIV {
                    vecBlock.ProcessVector1(bmm1Res, runInfo, kRunInfos[(s2TaskId + 1) % 2]);
                }
            }
        }
    }
                                                                                   ...
                                                                                   int64_t taskId, int64_t bIdx, int64_t s1Idx)
{
    ...
    SLIGradKLLossKRunInfo kRunInfos[4];
    bool isFixOut = false;
    if ASCEND_IS_AIV {
    ...
    for (int32_t s2TaskId = 0; s2TaskId < runInfo.s2LoopTimes + 3; ++s2TaskId) {
        ...
        if (s2TaskId >= 0 && notLastThreeLoop) {
            SetPRunInfo(kRunInfos[s2TaskId % 4], runInfo, s2TaskId);
            for (int32_t loopIdx = 0; loopIdx < kRunInfos[s2TaskId % 4].s2SingleLoopTimes; ++loopIdx) {
                SetPSingleRunInfo(kRunInfos[s2TaskId % 4], runInfo, s2TaskId, loopIdx);
                if ASCEND_IS_AIV {
                    vecBlock.ProcessVector3(this->sYL1Buf.Get(), runInfo, kRunInfos[s2TaskId % 4]);
                }
            }
            if ASCEND_IS_AIC {
                cubeBlock.ComputeMm1(this->bmm1Buffers.GetCube(), sYL1Buf, runInfo, constInfo, kRunInfos[s2TaskId % 4]);
            }
        }
        if (s2TaskId >= 3) {
            if ASCEND_IS_AIC {
                isFixOut = !notLastLoop;
                cubeBlock.ComputeMm3(this->bmm3Buffer.Get(), this->reluGradResL1Buf.Get(), runInfo, constInfo, kRunInfos[(s2TaskId + 1) % 4]);
                cubeBlock.ComputeMm4(reluGradResL1Buf.Get(), runInfo, constInfo, kRunInfos[(s2TaskId + 1) % 4], isFixOut);
            }
            for (int32_t loopIdx = 0; loopIdx < kRunInfos[(s2TaskId + 1) % 4].s2SingleLoopTimes; ++loopIdx) {
                SetPSingleRunInfo(kRunInfos[(s2TaskId + 1) % 4], runInfo, s2TaskId - 3, loopIdx);
                if ASCEND_IS_AIV {
                    vecBlock.ProcessVector7(this->bmm3Buffer.Get(), runInfo, kRunInfos[(s2TaskId + 1) % 4]);
                }
            }
        }
        if (s2TaskId >= 2 && notLastLoop) {
            Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> bmm1Res = this->bmm1Buffers.GetVec();
            for (int32_t loopIdx = 0; loopIdx < kRunInfos[(s2TaskId + 2) % 4].s2SingleLoopTimes; ++loopIdx) {
                SetPSingleRunInfo(kRunInfos[(s2TaskId + 2) % 4], runInfo, s2TaskId - 2, loopIdx);
                if ASCEND_IS_AIV {
                    vecBlock.ProcessVector4(bmm1Res, runInfo, kRunInfos[(s2TaskId + 2) % 4]);
                    vecBlock.ProcessVector5(runInfo, kRunInfos[(s2TaskId + 2) % 4]);
                    vecBlock.ProcessVector6(this->reluGradResL1Buf.Get(), runInfo, kRunInfos[(s2TaskId + 2) % 4]);
                }
            }
        }
    }
    ...
    SLIGradKLLossRunInfo &runInfo, int64_t taskId, int64_t bIdx, int64_t s1Idx, int64_t s1IdxEnd, bool isLastB)
{
    ...
    runInfo.s2BaseSize = N_WORKSPACE_SIZE;
    runInfo.s2CurSize = 0;
    ...
    if (constInfo.subBlockIdx == 0) {
        runInfo.nIndexSize = AlignTo(constInfo.gSizeQueryIndex, static_cast<uint32_t>(2)) / 2;
        runInfo.nVecSize = AlignTo(constInfo.gSizeQuery, static_cast<uint32_t>(2)) / 2;
    } else {
        runInfo.nIndexSize = constInfo.gSizeQueryIndex - AlignTo(constInfo.gSizeQueryIndex, static_cast<uint32_t>(2)) / 2;
        runInfo.nVecSize = constInfo.gSizeQuery - AlignTo(constInfo.gSizeQuery, static_cast<uint32_t>(2)) / 2;
    }
    ...
    if constexpr (LAYOUT_Q == SLILayout::TND) {
        ...
        runInfo.weightOffset = runInfo.accumS1Idx * constInfo.gSizeQueryIndex + 
            AlignTo(constInfo.gSizeQueryIndex, static_cast<uint32_t>(2)) * constInfo.subBlockIdx / 2;
        runInfo.softmaxInputOffset = runInfo.accumS1Idx * constInfo.gSizeQuery +
            AlignTo(constInfo.gSizeQuery, static_cast<uint32_t>(2)) * constInfo.subBlockIdx / 2;
    } else if constexpr (LAYOUT_Q == SLILayout::BSND) {
        ...
        runInfo.weightOffset = runInfo.bIdx * constInfo.s1Size * constInfo.gSizeQueryIndex +
            runInfo.s1Idx * constInfo.gSizeQueryIndex + AlignTo(constInfo.gSizeQueryIndex, static_cast<uint32_t>(2)) *
            constInfo.subBlockIdx / 2;
        runInfo.softmaxInputOffset = runInfo.bIdx * constInfo.s1Size * constInfo.gSizeQuery +
            runInfo.s1Idx * constInfo.gSizeQuery + AlignTo(constInfo.gSizeQuery, static_cast<uint32_t>(2)) *
            constInfo.subBlockIdx / 2;
    }
    ...
    SLIGradKLLossRunInfo &runInfo)
{
    ...
    runInfo.s2CurSize = 0;
    runInfo.s2BaseSize = N_WORKSPACE_SIZE / 2;
}
...
SparseLightningIndexerGradKLLossKernelBase<CubeBlockType, VecBlockType>::SetSYRunInfo(SLIGradKLLossKRunInfo &kRunInfo,
                                                                                     ...
                                                                                     int64_t s2TaskId)
{
    runInfo.s2TailSize = (runInfo.s2RealSize % N_WORKSPACE_SIZE == 0) ?
        N_WORKSPACE_SIZE : (runInfo.s2RealSize % N_WORKSPACE_SIZE);
    ...
    kRunInfo.s2SingleLoopTimes = CeilDiv(kRunInfo.kProcessSize, N_WORKSPACE_SIZE);
...
SparseLightningIndexerGradKLLossKernelBase<CubeBlockType, VecBlockType>::SetPRunInfo(SLIGradKLLossKRunInfo &kRunInfo,
                                                                                     ...
                                                                                     int64_t s2TaskId)
{
    runInfo.s2TailSize = (runInfo.s2RealSize % (N_WORKSPACE_SIZE / 2) == 0) ?
        (N_WORKSPACE_SIZE / 2) : (runInfo.s2RealSize % (N_WORKSPACE_SIZE / 2));
    kRunInfo.kTaskId = s2TaskId;
    kRunInfo.kProcessSize = (kRunInfo.kTaskId  == runInfo.s2LoopTimes - 1) ? (runInfo.s2RealSize - (runInfo.s2LoopTimes - 1) * constInfo.pKBaseSize) : constInfo.pKBaseSize;
    kRunInfo.s2SingleLoopTimes = CeilDiv(kRunInfo.kProcessSize, N_WORKSPACE_SIZE / 2);
...
SparseLightningIndexerGradKLLossKernelBase<CubeBlockType, VecBlockType>::SetSYSingleRunInfo(SLIGradKLLossKRunInfo &kRunInfo,
                                                                                     ...
                                                                                     int64_t taskId)
{
    kRunInfo.s2Idx = s2TaskId * constInfo.syKBaseSize / N_WORKSPACE_SIZE + taskId;
    kRunInfo.s2SingleIdx = taskId;
    kRunInfo.isS2end = taskId >= kRunInfo.s2SingleLoopTimes - 1 && s2TaskId >= runInfo.s2LoopTimes - 1;
    kRunInfo.s2RealBaseSize = kRunInfo.isS2end ? runInfo.s2TailSize : N_WORKSPACE_SIZE;
    runInfo.s2CurSize = s2TaskId * constInfo.syKBaseSize + taskId * N_WORKSPACE_SIZE;
    kRunInfo.s2SingleCurSize = kRunInfo.s2SingleIdx * N_WORKSPACE_SIZE;
...
SparseLightningIndexerGradKLLossKernelBase<CubeBlockType, VecBlockType>::SetPSingleRunInfo(SLIGradKLLossKRunInfo &kRunInfo,
                                                                                     ...
                                                                                     int64_t taskId)
{
    kRunInfo.s2Idx = s2TaskId * constInfo.pKBaseSize * 2 / N_WORKSPACE_SIZE + taskId;
    kRunInfo.s2SingleIdx = taskId;
    kRunInfo.isS2end = taskId >= kRunInfo.s2SingleLoopTimes - 1 && s2TaskId >= runInfo.s2LoopTimes - 1;
    kRunInfo.s2RealBaseSize = kRunInfo.isS2end ? runInfo.s2TailSize : (N_WORKSPACE_SIZE / 2);
    runInfo.s2CurSize = s2TaskId * constInfo.pKBaseSize + taskId * N_WORKSPACE_SIZE / 2;
    kRunInfo.s2SingleCurSize = kRunInfo.s2SingleIdx * N_WORKSPACE_SIZE / 2;
    kRunInfo.isAlign64 = (!kRunInfo.isS2end) || (kRunInfo.isS2end && runInfo.s2TailSize % 64 == 0);
}
...
__aicore__ inline void SparseLightningIndexerGradKLLossKernelBase<CubeBlockType, VecBlockType>::SetConstInfo()
{
    ...
    // constInfo.syKBaseSize = 256; //目前写死256
    // constInfo.pKBaseSize = 128; //目前写死128
}
