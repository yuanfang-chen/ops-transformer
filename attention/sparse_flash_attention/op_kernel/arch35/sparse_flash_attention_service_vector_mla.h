...
public:
    ...
    __aicore__ inline void InitGlobalBuffer(__gm__ uint8_t *key, __gm__ uint8_t *value, /*__gm__ uint8_t *keyRope,*/ __gm__ uint8_t *sparseIndices,
    ...
    int64_t token0Idx, int64_t token1Idx, const RunInfo &runInfo, ConstInfo &constInfo)
{
    ...
    if (keySrcStride >= INT32_MAX || keySrcStride < 0 || constInfo.sparseBlockSize > 1) {
        ...
        intriParams.blockLen = /*combineBytes*/ constInfo.dSizeNope;
        intriParams.dstStride = 0;
    ...
    const RunInfo &runInfo, ConstInfo &constInfo, int32_t startPos)
{
    ...
    */
    ...
    //if constexpr (IS_PA) {
        ...
        //maxBlockNumPerBatch = maxBlockNumPerBatch;
    //}
    ...
    const RunInfo &runInfo, ConstInfo &constInfo, int32_t startPos)
{
    if (0) {
        ...
        //     DequantKv(kvOutUb, kvInUb, dealRow, s2ProcessBaseSize, constInfo);
        //     stage0InQue.FreeTensor(kvInUb);
        //     stage0OutQue.EnQue(kvOutUb);
        //     kvOutUb = stage0OutQue.DeQue<Q_T>();

        //     // 3、copy kv out, ub -> l1
        //     CopyOutKvUb2L1(outputL1, kvOutUb, 0, dealRow, s2Start, runInfo, constInfo);
        //     s2Start += dealRow;
    ...
    //}
    if constexpr (1 == 0) {  // 不走这里，这里留给后续性能优化，现在只搞搬到GM的  // TEMPLATE_MODE == SASTemplateMode::SCFA_TEMPLATE_MODE && !IS_SPLIT_G
        int64_t s2PerVecLoop = 2LL;
        int64_t vecNum = 2LL;
...
__gm__ uint8_t *value, /*__gm__ uint8_t *keyRope,*/ __gm__ uint8_t *sparseIndices, __gm__ uint8_t *blockTable)
{
    ...
    CVSharedParams &sharedParams, int32_t aicIdx, uint8_t subBlockIdx)
{
    ...
    sharedParams.dSizeVInput = 1152; //sparseAttnSharedkvBaseParams.dSizeVInput; // 拷贝用
