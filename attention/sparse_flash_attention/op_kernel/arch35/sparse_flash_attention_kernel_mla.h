    ...
    __gm__ uint8_t *workspace, const SparseFlashAttentionTilingDataMla *__restrict tiling, TPipe *tPipe)
{
    ...
    //     oriTopkLengthGm.SetGlobalBuffer((__gm__ int32_t *)oriTopkLength);  不要了
...
KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType>::InitMMResBuf(__gm__ uint8_t *workspace)
{
    ...
    //if constexpr (IS_SPLIT_G || TEMPLATE_MODE == SASTemplateMode::ORI_SCFA_TEMPLATE_MODE) {
...
__aicore__ inline void KvQuantSparseFlashAttentionMla<CubeBlockType, VecBlockType>::ProcessMainLoop()
{
                ...
                for (int64_t s2LoopCount = 0; s2LoopCount <= s2LoopLimit; ++s2LoopCount) {
                    if (notLastTwoLoop) {
                        ...
                        if ASCEND_IS_AIC {
                            ...
                            this->vecBlock.ProcessVec0(this->l1RightBuffers.Get(), v0ResGmBuffers.Get(), runInfo1, this->constInfo, 0);
                        }
                    }
                    if (taskId > 0 && notLast) {
    ...
    //wzn add
    constexpr int32_t IS_SPLIT_G = 1;
    for (int64_t bnIdx = bN2StartIdx; bnIdx < bN2EndIdx; bnIdx++) {
        bool lastBN = (bnIdx == bN2EndIdx - 1);
        runParam.boIdx = bnIdx;
        runParam.n2oIdx = 0;
