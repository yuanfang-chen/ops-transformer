...
{
    OP_LOGD(context->GetNodeName(), "Begin to do InferDataTypeChunkGatedDeltaRule");

    // 设置输出的dtype
    ge::DataType sizeDtype = context->GetInputDataType(IDX_0);
    context->SetOutputDataType(IDX_0, sizeDtype);

    OP_LOGD(context->GetNodeName(), "End to do InferDataTypeChunkGatedDeltaRule");
    return GRAPH_SUCCESS;
}

IMPL_OP(ChunkGatedDeltaRule).InferDataType(InferDataTypeChunkGatedDeltaRule);
