...
public:
    ...
    __aicore__ inline void InitLocalBuffers()
    {
        ...
        buffOffset += chunkSize_ * maxLen  * sizeof(float) * paraNum_;
        ...
        inverseUbFloat_ = tmpBuff_.GetWithOffset<float>(
                            static_cast<uint32_t>(halfChunkSize_ * halfChunkSize_ * INVERSE_COUNT), buffOffset); 
...
private:
   ...
   __aicore__ inline void SetChunkTensors(uint64_t nId, uint64_t localChunkId, uint64_t chunkStartRow)
    ...
    __aicore__ inline void ProcessParaChunk(int32_t curParaNum, uint64_t startTaskId, uint64_t localChunkId, uint64_t chunkStartRow)
    {
        ...
        if ASCEND_IS_AIV {
            ...
            for (uint32_t i = 0; i < curParaNum; ++i) {
                ...
                uint64_t bgOffset = chunkStartRow * nv_ + nId;
                betaGm_ = betaBaseGm_[bgOffset];
                BetaCopyInWithStride(betaGm_, subValidLenBatch_[i]);
            }
            ...
            for (uint32_t i = 0; i < curParaNum; ++i) {
                ...
                InverseCompute(attnWsGm_[kOffset]);
            }
            ...
            for (uint32_t i = 0; i < curParaNum; ++i) {
                ...
                valueGm_ = valueBaseGm_[vOffset];
                uint64_t betaUbOffset = i * halfChunkSize_ * sizeof(float);
                uint64_t valueUbOffset = i * chunkSize_ * maxLen  * sizeof(float);
    ...
    __aicore__ inline void QKPreProcess(const GlobalTensor<bfloat16_t>& srcGm, const GlobalTensor<float>& dstGm, 
                                        const GlobalTensor<float>& outKgGm,
                                        LocalTensor<float>& dstBuffer, uint32_t subValidRows, bool kgFlag = false)
    {
        // copyIn
    ...
    __aicore__ inline void GCumExpCompute(const GlobalTensor<float> src, const GlobalTensor<float> dst, uint32_t validLen)
    {
        // Copy g
    ...
    __aicore__ inline void GammaCompute(const LocalTensor<float> gBroadUbFloat, LocalTensor<float> gTransBroadUbFloat)
    {
        ...
        Broadcast<float, BROADCAST_AXIS, 1>(gBroadUbFloat, gCumExpUbFloat_, divShape, gShape);
        Broadcast<float, BROADCAST_AXIS, 0>(gTransBroadUbFloat, gCumExpUbFloat_, divShape, gTransShape);
        PipeBarrier<PIPE_V>();
        // div
        Div(gammaUbFloat_, gBroadUbFloat, gTransBroadUbFloat, chunkSize_ * chunkSize_);
        PipeBarrier<PIPE_V>();
        // mask
    ...
    __aicore__ inline void BetaCopyInWithStride(const GlobalTensor<float> src, uint32_t subValidRows)
    {
    ...
    __aicore__ inline void InverseCompute(const GlobalTensor<float> src)
    {
    ...
    __aicore__ inline void GBKCompute(const GlobalTensor<float> gBKWsGm, const GlobalTensor<float> outKgGm, LocalTensor<float> betaUbFloat, LocalTensor<float> kUbFloatCon)
    {
        if (gOptional_){
            // tmp = -1.0 * beta * g_cum_exp
            Mul(gBUbFloat_, betaUbFloat, gCumExpUbFloat_[subOffset_], halfChunkSize_);
            PipeBarrier<PIPE_V>();
    ...
    __aicore__ inline void QPrimeCompute(const GlobalTensor<float> outQPrimeGm, LocalTensor<float> qUbFloatCon, LocalTensor<float> gCumExpBroadUbFloat)
    {
        ...
        if (gOptional_){
            ...
            Broadcast<float, BROADCAST_AXIS, 1>(gCumExpBroadUbFloat, gCumExpUbFloat_[subOffset_], 
                                                qShape, gCumExpShape);
            PipeBarrier<PIPE_V>();
    ...
    __aicore__ inline void DataCopyOutG(GlobalTensor<float> y, uint64_t length)
    {
        ...
        if (subBlockIdx_ == 0){
            DataCopyExtParams params{static_cast<uint16_t>(1),
                                     ...
                                     0, 0, 0};
            DataCopyPad(y, gCumExpUbFloat_, params);  // stage1 out
        }
    }
