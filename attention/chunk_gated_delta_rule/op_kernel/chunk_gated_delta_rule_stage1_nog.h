...
/*!
 * \file chunk_gated_delta_rule_stage1_nog.h
 * \brief
 */
#ifndef __CHUNK_GATED_DELTA_RULE_STAGE1_NOG_H_
#define __CHUNK_GATED_DELTA_RULE_STAGE1_NOG_H_
...
struct GDRStageOneNoGInitParams {
    // input
...
class GDRStageOneNoG {
public:
    __aicore__ inline GDRStageOneNoG(MT_FP32 &mmFp32) : mmFp32(mmFp32) {}
    __aicore__ inline void SetGlobalTensors(const GDRStageOneNoGInitParams &initParams) {
    ...
    __aicore__ inline void Init(const GDRStageOneNoGInitParams &initParams, TPipe *pipe, const ChunkGatedDeltaRuleTilingData *tilingData)
    {
        pipe_ = pipe;
        tiling_ = tilingData;
        nk_ = tiling_->nk;
...
private:
   ...
   __aicore__ inline void SetChunkTensors(uint64_t nid, uint64_t local_cid, uint64_t chunk_start_row)
    ...
    __aicore__ inline void vBetaCompute()
    {
        ...
        if (validLen_ < halfChunkSize_) {
            Duplicate(valueUbFloat[validLen_ * dv_aligned_], static_cast<float>(0.0f), (halfChunkSize_ - validLen_) * dv_aligned_);
            PipeBarrier<PIPE_V>();
        }
