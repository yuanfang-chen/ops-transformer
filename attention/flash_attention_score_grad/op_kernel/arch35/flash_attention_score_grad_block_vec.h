...
public:
    ...
    __aicore__ inline void DeterComputeDkv(LocalTensor<CALC_TYPE> &mm1ResTensor, LocalTensor<CALC_TYPE> &mm2ResTensor, FagConstInfo &constInfo, 
                                            bool isLastSort, int64_t computeBlockIdx, int64_t remainLoopNum, int16_t *deterPpFlag);
    __aicore__ inline void DeterComputeDqkv(LocalTensor<CALC_TYPE> &mm1ResTensor, LocalTensor<CALC_TYPE> &mm2ResTensor, 
                                            FagConstInfo &constInfo, bool isLastSort, int64_t computeBlockIdx, int64_t remainLoopNum);
                                                                    ...
                                                                    int64_t remainLoopNum, int16_t *deterPpFlag)
{
    ...
    uint32_t allSize =
        constInfo.deterConstInfo.usedCubeCoreNum * constInfo.deterConstInfo.dqEachVectorSize * sizeof(CALC_TYPE);
    uint8_t loopTimes = static_cast<uint8_t>((allSize + DETER_DQ_UB_SIZE - 1) / DETER_DQ_UB_SIZE);
    uint8_t eachLoopBlockCount =
        static_cast<uint8_t>(Min(DETER_DQ_UB_SIZE / (constInfo.deterConstInfo.dqEachVectorSize * sizeof(CALC_TYPE)),
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void FAGBlockVec<TEMPLATE_ARGS>::DeterComputeDkv(LocalTensor<CALC_TYPE> &mm1ResTensor, LocalTensor<CALC_TYPE> &mm2ResTensor, 
                                                                    ...
                                                                    int64_t remainLoopNum, int16_t *deterPpFlag)
{
    ...
    uint32_t allSize = constInfo.deterConstInfo.usedCubeCoreNum * constInfo.deterConstInfo.dkvEachVectorSize * sizeof(CALC_TYPE);
    uint8_t loopTimes = static_cast<uint8_t>((allSize + DETER_DKV_UB_SIZE - 1) / DETER_DKV_UB_SIZE);
    uint8_t eachLoopBlockCount =
        static_cast<uint8_t>(Min(DETER_DKV_UB_SIZE / (constInfo.deterConstInfo.dkvEachVectorSize * sizeof(CALC_TYPE)),
                                 constInfo.deterConstInfo.usedCubeCoreNum));
    uint8_t eachLoopStart = 0;
    uint8_t eachLoopEnd = 0;
    AscendC::SetAtomicAdd<CALC_TYPE>();
    for (uint8_t loopIdx = 0; loopIdx < loopTimes; loopIdx++) {
        LocalTensor<CALC_TYPE> dkDeterBuf = mm1ResBuf[1 - bufId].Get<CALC_TYPE>();
        LocalTensor<CALC_TYPE> dvDeterBuf = mm2ResBuf[1 - bufId].Get<CALC_TYPE>();
        dataCopyParams.blockCount = (loopIdx < loopTimes - 1)
                                        ? eachLoopBlockCount
                                        : constInfo.deterConstInfo.usedCubeCoreNum - loopIdx * eachLoopBlockCount;
        if (loopIdx > 0) {
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void FAGBlockVec<TEMPLATE_ARGS>::DeterComputeDqkv(LocalTensor<CALC_TYPE> &mm1ResTensor, LocalTensor<CALC_TYPE> &mm2ResTensor, 
                                                                    ...
                                                                    int64_t computeBlockIdx, int64_t remainLoopNum)
{
    ...
    allSize = constInfo.deterConstInfo.usedCubeCoreNum * constInfo.deterConstInfo.dkvEachVectorSize * sizeof(CALC_TYPE);
