...
public:
    ...
    constexpr static uint32_t DETER_DQ_UB_SIZE_FP32_D256 = 32 * 1024;
                                                                    ...
                                                                    int64_t computeBlockIdx, int64_t remainLoopNum, int16_t *deterPpFlag)
{
    ...
    eachLoopBlockCount = static_cast<uint8_t>(DETER_DKV_UB_SIZE / (constInfo.deterConstInfo.dkvEachVectorSize * sizeof(CALC_TYPE)));
    loopTimes = static_cast<uint8_t>((constInfo.deterConstInfo.usedCubeCoreNum + eachLoopBlockCount - 1) / eachLoopBlockCount);
    eachLoopStart = 0;
    eachLoopEnd = 0;
