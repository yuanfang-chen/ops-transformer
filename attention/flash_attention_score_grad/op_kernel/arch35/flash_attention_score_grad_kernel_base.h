...
__aicore__ inline void FlashAttentionScoreGradKernelBase<ChildClass, CubeBlockType, VecBlockType>::SetSwizzleConstInfo()
{
    ...
    if (enableDenseSwizzle || enableCasualSwizzle) {
        if (constInfo.commonConstInfo.s1Size >= M_SWIZZLE_SIZE) {
    ...
    while (true) {
        ...
        if (loopIdx < constInfo.leftTotalRound) {
            ...
            if (globalS1Idx == constInfo.mSwizzleBlockNum) {
                isEnd =
                    GetLocalS1S2Idx(rTmp, constInfo.mSwizzleBlockNumTail, N_SWIZZLE_BLOCK_SIZE, localS1Idx, localS2Idx);
            } else {
                isEnd = GetLocalS1S2Idx(rTmp, M_SWIZZLE_BLOCK_SIZE, N_SWIZZLE_BLOCK_SIZE, localS1Idx, localS2Idx);
            }
        } else {
            ...
            if (globalS1Idx == constInfo.mSwizzleBlockNum) {
                isEnd = GetLocalS1S2Idx(rTmp, constInfo.mSwizzleBlockNumTail, constInfo.nSwizzleBlockNumTail,
                                        localS1Idx, localS2Idx);
            } else {
                isEnd =
                    GetLocalS1S2Idx(rTmp, M_SWIZZLE_BLOCK_SIZE, constInfo.nSwizzleBlockNumTail, localS1Idx, localS2Idx);
            }
        }
        swizzleLoopIdx++;
        if (isEnd) {
    ...
    FagRunInfo &runInfo, int64_t loopIdx)
{
    while (true) {
        ...
        if (loopIdx < constInfo.leftTotalRound) {
            ...
            // 求根公式求解二次方程
            int64_t a = (-constInfo.leftDownTotalRound / 2);
            int64_t b = constInfo.leftDownTotalRound / 2 - tmp;
            int64_t c = tmp - loopIdx - 1;
            globalS2Idx = Ceil<int64_t>(-b - sqrt(b * b - 4 * a * c), constInfo.leftDownTotalRound);
            loopIdx = loopIdx - (globalS2Idx * (constInfo.leftUpTotalRound + constInfo.rightUpTotalRound) +
                                 (2 * constInfo.mSwizzleBlockNum - 1 - globalS2Idx) * globalS2Idx / 2 *
                                     constInfo.leftDownTotalRound);
            if (loopIdx < constInfo.leftUpTotalRound) {
                globalS1Idx = globalS2Idx;
                isEnd = GetLocalS1S2Idx(loopIdx, M_SWIZZLE_BLOCK_SIZE, M_SWIZZLE_BLOCK_SIZE, localS1Idx, localS2Idx);
                ...
                loopIdx = loopIdx - (constInfo.leftUpTotalRound +
                                     constInfo.leftDownTotalRound * (constInfo.mSwizzleBlockNum - 1 - globalS2Idx));
                isEnd = GetLocalS1S2Idx(loopIdx, constInfo.mSwizzleBlockNumTail, M_SWIZZLE_BLOCK_SIZE, localS1Idx,
                                        localS2Idx);
            }
        } else {
            globalS1Idx = constInfo.mSwizzleBlockNum;
            ...
            loopIdx = loopIdx - constInfo.leftTotalRound;
            isEnd = GetLocalS1S2Idx(loopIdx, constInfo.mSwizzleBlockNumTail, constInfo.mSwizzleBlockNumTail, localS1Idx,
                                    localS2Idx);
        }
        swizzleLoopIdx++;
        if (isEnd) {
...
template __aicore__ inline bool
FlashAttentionScoreGradKernelBase<ChildClass, CubeBlockType, VecBlockType>::GetLocalS1S2Idx(int64_t loopIdx, int64_t m,
                                                                                            int64_t n,
                                                                                            int64_t &localS1Idx,
                                                                                            int64_t &localS2Idx)
{
    ...
    if constexpr (IS_DENSE) {
        ...
        if (globalIdx >= m * (2 * m - n + 1) / 2) {
            return true;
        }
        localS2Idx = Ceil((2 * m - 1) - sqrt((2 * m - 1) * (2 * m - 1) + 8 * (m - 1 - globalIdx)), 2);
        localS1Idx = globalIdx - (2 * m - localS2Idx - 1) * localS2Idx / 2;
    }
    return false;
}
