...
FlashAttentionScoreGradKernelDeter<CubeBlockType, VecBlockType>::CalCausalDeterIndex(uint32_t roundId, CoordinateInfo &coordinateInfo)
{
    ...
    if constexpr (!BaseClass::IS_TND) {
        ...
        if (this->constInfo.sparseMode == RIGHT_DOWN_CAUSAL && m > n) {
            mGap = (this->constInfo.commonConstInfo.s1Size - this->constInfo.commonConstInfo.s2Size) / BaseClass::DETER_CUBE_BASEM;
            m -= mGap;
        ...
        if constexpr (BaseClass::IS_N_EQUAL) {
            ...
            CalTNDGQACausalIndex<BaseClass::DETER_CUBE_BASEM, BaseClass::DETER_CUBE_BASEN>(this->actualSeqQlenAddr, this->actualSeqKvlenAddr, 
    ...
    if constexpr (BaseClass::DETER_TILING_SPLIT_MODE == 1) {
        useEven = initialRoundId % 2 == 1;
        TransTilingSplitModeBack<BaseClass::DETER_TILING_SPLIT_MODE, BaseClass::IS_TND>(coordinateInfo, this->constInfo, useEven);
    }
    ...
    uint32_t roundId, CoordinateInfo &coordinateInfo)
{
    ...
    if constexpr (!BaseClass::IS_TND) {
        ...
        if constexpr (BaseClass::IS_N_EQUAL) {
            CalTNDBandIndex<BaseClass::DETER_CUBE_BASEM, BaseClass::DETER_CUBE_BASEN>(this->actualSeqQlenAddr, this->actualSeqKvlenAddr,
...
FlashAttentionScoreGradKernelDeter<CubeBlockType, VecBlockType>::CalDeterMaxLoopNum()
{
    ...
    if constexpr (BaseClass::DETER_TILING_SPLIT_MODE == 1 && BaseClass::DETER_SPARSE_TYPE == DETER_CAUSAL) {
        m = Ceil<int64_t>(m, 2);
    }
    InitCoordinateInfo(m, this->constInfo.s2Outer, 0, 0, this->coordinateInfos[0]);
    InitCoordinateInfo(m, this->constInfo.s2Outer, 0, 0, this->coordinateInfos[1]);
    ...
    if constexpr (BaseClass::DETER_SPARSE_TYPE == DETER_BAND) {
        ...
        if constexpr(BaseClass::IS_N_EQUAL) {
            ...
            deterLoopMax = Max(this->tilingData->s1s2BNGS1S2SplitCoreParams.deterMaxRound / 2, this->bandInfo.rm2);
        } else {
        ...
        InitCoordinateInfo(s1Outer, s2Outer, mOffset, nOffset, this->coordinateInfos[0]);
        InitCoordinateInfo(s1Outer, s2Outer, mOffset, nOffset, this->coordinateInfos[1]);
