...
__aicore__ inline void FlashAttentionScoreGradKernel<CubeBlockType, VecBlockType>::Process()
{
    ...
    while (true) {
        ...
        if (taskId > 0) {
            ...
            // if (unlikely(this->constInfo.isSink && IS_DROP && (BaseClass::IS_DQ_WRITE_UB ||
            //     BaseClass::IS_DK_WRITE_UB || IS_DV_WRITE_UB))) {
            //     if ASCEND_IS_AIV {
            //         CrossCoreSetFlag<SYNC_MODE, PIPE_V>(SYNC_SINK_UB_REUSE_FLAG);
            //     }
            //     if ASCEND_IS_AIC {
            //         CrossCoreWaitFlag<SYNC_MODE, PIPE_FIX>(SYNC_SINK_UB_REUSE_FLAG);
            //         CrossCoreWaitFlag<SYNC_MODE, PIPE_FIX>(16 + SYNC_SINK_UB_REUSE_FLAG);
            //     }
            // }
