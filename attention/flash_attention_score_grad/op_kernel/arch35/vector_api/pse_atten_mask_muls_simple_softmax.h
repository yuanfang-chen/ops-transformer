                                    ...
                                    GlobalTensor<T2> &sumGm)
{
    ...
    if (constInfo.tndMaxSumLayout == MAX_SUM_TND) { // TND + TND
        maxSumGmOffset =
            (runInfo.commonRunInfo.queryOffset / constInfo.commonConstInfo.dSize +
             runInfo.commonRunInfo.firstHalfS1RealSize * GetSubBlockIdx() * constInfo.commonConstInfo.n2G) *
            MAX_SUM_REDUCE_AXIS_SIZE / sizeof(T2);
    } else if (constInfo.commonConstInfo.layoutType == TND && constInfo.tndMaxSumLayout == 0) { // TND + BNS8
        int64_t tndS1PrefixSum =
            (runInfo.commonRunInfo.boIdx == 0 ? 0 :
                                                ((gm int64_t *)constInfo.seqS1_addr)[runInfo.commonRunInfo.boIdx - 1]);
        ...
        if (unlikely(runInfo.commonRunInfo.boIdx == 0)) {
            actualS1Len = ((gm int64_t *)constInfo.seqS1_addr)[0];
        } else {
            actualS1Len = ((gm int64_t *)constInfo.seqS1_addr)[runInfo.commonRunInfo.boIdx] -
                          ((gm int64_t *)constInfo.seqS1_addr)[runInfo.commonRunInfo.boIdx - 1];
        }
        maxSumGmOffset +=
