...
__global__ __aicore__ void flash_attention_score_grad(
    ...
    __gm__ uint8_t *queryRope, __gm__ uint8_t *keyRope, __gm__ uint8_t *key_sink, __gm__ uint8_t *value_sink, __gm__ uint8_t *softmax_lse, __gm__ uint8_t *dsScale, __gm__ uint8_t *pScale,
    __gm__ uint8_t *dq, __gm__ uint8_t *dk, __gm__ uint8_t *dv, __gm__ uint8_t *dpse,
    __gm__ uint8_t *dqRope, __gm__ uint8_t *dkRope, __gm__ uint8_t *dk_sink, __gm__ uint8_t *dv_sink, __gm__ uint8_t *workspace, __gm__ uint8_t *tiling_data)
    ...
    if constexpr (IsEmptyTensor) {
        ...
        RegbaseFAG<SplitAxis, InputDType, IsTnd, IsDrop, IsPse, IsAttenMask, S1TemplateNum, S2TemplateNum, DTemplateNum, DeterType, IsNEqual, IsBn2MultiBlk,
            IsDNoEqual, IsRope, OutDType, FP8_OPEN_TSCM, IsTndSwizzle, IsRegbase>(
                ...
                deqScaleQ, deqScaleK, deqScaleV, deqScaleDy, dsScale, pScale, queryRope, keyRope, key_sink, value_sink, softmax_lse,
                dq, dk, dv, dpse, dqRope, dkRope, dk_sink, dv_sink, workspace, tiling_data);
    }
}