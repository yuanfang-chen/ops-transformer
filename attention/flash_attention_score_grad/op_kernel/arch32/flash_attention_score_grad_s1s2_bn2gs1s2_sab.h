...
public:
    ...
    __aicore__ inline void Cube1CopyOut(GlobalTensor<float> dstTensor, 
                                        ...
                                        const int32_t nSize,
                                        DBParams& dbParam);
    __aicore__ inline void Cube3CopyOut(GlobalTensor<float> dstTensor, 
                                        LocalTensor<float> srcTensor, 
                                        const int32_t mSize, 
                                        const int32_t nSize,
                                        DBParams& dbParam);
...
__aicore__ inline void FlashAttentionScoreGradS1s2Bn2gs1s2SameAB<FAGT>::ComputeMM1(DBParams& dbParam)
{
    ...
    if (DTEMPLATETYPE == DTemplateType::Aligned64){
        ...
        for (uint32_t m = 0; m < mLoops; m++) {
            ...
            LoadDataAToL0((*l0_a_tensor), dxL1Tensor[aL1Offset], value_d, subMSizeAct);
            for (uint32_t n = 0; n < nLoops; n++) {
                subNSizeAct = n == (nLoops - 1) ? nTail : nSplitSize;
                subNSizeActAlign = CeilDiv(subNSizeAct, C0_SIZE) * C0_SIZE;
                uint64_t bL1Offset = n * nSplitSize * value_dAlign;
                ...
                if constexpr (MM_OUT_FORMAT == CubeFormat::NZ) {
                    ...
                    Cube1CopyOut(mm1WorkspaceGm[pingpongIdx * cubeBaseMN + m * mSplitSize * dbParam.s2CvExtend +
        ...
        for (uint32_t m = 0; m < mLoops; m++) {
            ...
            LocalTensor<T1> *l0_a_tensor = ping_pong_flag_l0_a_ ? &l0_a_pong_tensor : &l0_a_ping_tensor;
            CopyGmToL1(qL1Tensor[aL1Offset],
                queryGm[specify_for_v_aTensorOffsetCv + m * mSplitSize * dbParam.s1Stride / d * d], subMSizeAct,
                d, dbParam.s1Stride / d * d);
            AscendC::SetFlag<HardEvent::MTE2_MTE1>(eventIdMte2ToMte1);
            AscendC::WaitFlag<HardEvent::MTE2_MTE1>(eventIdMte2ToMte1);
            
            LoadDataAToL0((*l0_a_tensor), qL1Tensor[aL1Offset], d, subMSizeAct);
            for (uint32_t n = 0; n < nLoops; n++) {
                subNSizeAct = n == (nLoops - 1) ? nTail : nSplitSize;
                subNSizeActAlign = CeilDiv(subNSizeAct, C0_SIZE) * C0_SIZE;
                uint64_t bL1Offset = n * nSplitSize * dAlign;
                LocalTensor<T1> *l0_b_tensor = ping_pong_flag_l0_b_ ? &l0_b_pong_tensor : &l0_b_ping_tensor;
                LocalTensor<float> *l0_c_tensor = ping_pong_flag_l0_c_ ? &l0_c_pong_tensor : &l0_c_ping_tensor;
                // B矩阵
                if (m == 0) {
                    CopyGmToL1(kL1Tensor[bL1Offset],
                        keyGm[specify_for_v_bTensorOffsetCv + n * nSplitSize * dbParam.s2Stride / d * d],
                ...
                if constexpr (MM_OUT_FORMAT == CubeFormat::NZ) {
                    ...
                    Cube1CopyOut(mm2WorkspaceGm[pingpongIdx * cubeBaseMN + m * mSplitSize * dbParam.s2CvExtend +
                                                  n * nSplitSize], *l0_c_tensor, subMSizeAct, subNSizeAct, dbParam);
                }
                ping_pong_flag_l0_b_ = 1 - ping_pong_flag_l0_b_;
                ping_pong_flag_l0_c_ = 1 - ping_pong_flag_l0_c_;
...
template <typename FAGT>
__aicore__ inline void FlashAttentionScoreGradS1s2Bn2gs1s2SameAB<FAGT>::Cube3CopyOut(GlobalTensor<float> dstTensor,
                                                                                 LocalTensor<float> srcTensor,
                                                                                 const int32_t mSize,
                                                                                 const int32_t nSize,
                                                                                 DBParams& dbParam)
    ...
    DBParams& dbParam, int64_t nextBlockId)
{
    ...
    ///////////////////////////////////////////////////////////////
    event_t eventIdMToFixpipe = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::M_FIX));
    event_t eventIdFixpipeToMte2 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::FIX_MTE2));
    AscendC::SetFlag<HardEvent::M_MTE1>(eventIdMToMte1_ID3);
    AscendC::SetFlag<HardEvent::M_MTE1>(eventIdMToMte1_ID4);
    AscendC::SetFlag<HardEvent::M_MTE1>(eventIdMToMte1_ID5);
    AscendC::SetFlag<HardEvent::M_MTE1>(eventIdMToMte1_ID6);
    ...
    AscendC::SetFlag<HardEvent::FIX_MTE2>(eventIdFixpipeToMte2);
    AscendC::WaitFlag<HardEvent::FIX_MTE2>(eventIdFixpipeToMte2);

    for (uint32_t n = 0; n < nLoops; n++) { // s2
        subNSizeAct = n == (nLoops - 1) ? nTail : nSplitSize;
        subNSizeActAlign = CeilDiv(subNSizeAct, C0_SIZE) * C0_SIZE;
        LocalTensor<float> *l0_c_tensor = ping_pong_flag_l0_c_ ? &l0_c_pong_tensor : &l0_c_ping_tensor;
        for (uint32_t m = 0; m < mLoops; m++) {
            subMSizeAct = m == (mLoops - 1) ? mTail : mSplitSize;
            subMSizeActAlign = CeilDiv(subMSizeAct, C0_SIZE) * C0_SIZE;
            uint64_t aL1StartOffset = (n % 2) * mSplitSize * value_dAlign * 4;
            uint64_t aL1Offset = aL1StartOffset + m * mSplitSize * value_dAlign;
            uint64_t bL1Offset = m * mSplitSize * value_dAlign;
            LocalTensor<T1> *l0_a_tensor = ping_pong_flag_l0_a_ ? &l0_a_pong_tensor : &l0_a_ping_tensor;
            LocalTensor<T1> *l0_b_tensor = ping_pong_flag_l0_b_ ? &l0_b_pong_tensor : &l0_b_ping_tensor;
            // A矩阵
            uint16_t blockCount = subNSizeActAlign / C0_SIZE;
            ...
            AscendC::WaitFlag<HardEvent::MTE2_MTE1>(eventIdMte2ToMte1);
            // LoadDataAToL0
            AscendC::WaitFlag<HardEvent::M_MTE1>(ping_pong_flag_l0_a_ + 3 + 2);
            uint32_t m_c0_loop = (subMSizeAct + 15) / 16;
            uint32_t n_c0_loop = (subNSizeAct + 15) / 16;
            commonLoadData2dParamsTranspose.repeatTimes = m_c0_loop * n_c0_loop;
            commonLoadData2dParamsTranspose.srcStride = 1;
            AscendC::LoadData((*l0_a_tensor), dsL1Tensor[aL1Offset], commonLoadData2dParamsTranspose);
            AscendC::SetFlag<HardEvent::MTE1_M>(eventIdMte1AToM);
            AscendC::WaitFlag<HardEvent::MTE1_M>(eventIdMte1AToM);
            // LoadDataBToL1
            if (n == 0 || (nLoops > mLoops && n > m)) {
            ...
            // LoadDataBToL0
            AscendC::WaitFlag<HardEvent::M_MTE1>(ping_pong_flag_l0_b_ + 3);
            commonLoadData2dParamsTranspose.repeatTimes = value_d / C0_SIZE;
            commonLoadData2dParamsTranspose.srcStride = subMSizeActAlign / C0_SIZE;
            for (int32_t i = 0; i < subMSizeActAlign / C0_SIZE; i++) {
                AscendC::LoadData((*l0_b_tensor)[i * value_d * C0_SIZE],
                                    dxL1Tensor[bL1Offset + i * C0_SIZE * C0_SIZE],
            ...
            int unit_flag = 0b10;
            bool init_c = (m == 0);
            bool out_c = (m == (mLoops - 1));
            if (out_c) {
                unit_flag = 0b11;
            }
            commonMadParams.m = subNSizeAct == 1 ? 2 : subNSizeAct;
            commonMadParams.n = value_d;
            ...
            AscendC::SetFlag<HardEvent::M_MTE1>(ping_pong_flag_l0_a_ + 3 + 2);
            AscendC::SetFlag<HardEvent::M_MTE1>(ping_pong_flag_l0_b_ + 3);
            ping_pong_flag_l0_a_ = 1 - ping_pong_flag_l0_a_;
            ping_pong_flag_l0_b_ = 1 - ping_pong_flag_l0_b_;
        ...
        AscendC::WaitFlag<HardEvent::M_FIX>(eventIdMToFixpipe);
        if constexpr (MM2_OUT_FORMAT == CubeFormat::NZ) {
            Cube3CopyOut(dvWorkSpaceGm[dvOffset + n * nSplitSize * C0_SIZE], *l0_c_tensor, subNSizeAct,
                value_d, dbParam);
        } else {
            Cube3CopyOut(dvWorkSpaceGm[dvOffset + n * nSplitSize * dbParam.s1Stride], *l0_c_tensor,
                subNSizeAct, value_d, dbParam);
        }
        ping_pong_flag_l0_c_ = 1 - ping_pong_flag_l0_c_;
    }
