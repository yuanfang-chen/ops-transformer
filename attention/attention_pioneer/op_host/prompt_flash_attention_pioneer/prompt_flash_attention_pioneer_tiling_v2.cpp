...
bool PromptFlashAttentionPioneerTilingV2::CheckSinkLength(ContextParamsForPFATiling& contextKeyParams)
{
    ...
    //     sinkLength = 0;
    // }
    // bool sinkLengthCheck = false;
    // sinkLengthCheck = ((sinkLength != 0) && (sinkLength != 128));
...
bool PromptFlashAttentionPioneerTilingV2::CheckSinkLengthCrossver(ContextParamsForPFATiling& contextKeyParams, PFAShapeInfo& queryShapeInfo)
{
    // int64_t sinkLength = 0;
    // if (contextKeyParams.keySinkInputShape != nullptr) {
    //     sinkLength = contextKeyParams.keySinkInputShape->GetStorageShape().GetDim(0);
    ...
    //         "sinklength only support 128"), return false);
    // OP_CHECK_IF(contextKeyParams.actualSequenceLengthKV < sinkLength, OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, 
    //         "actualSequenceLengthKV must greater than or equal to sinklength"), return false);
    ...
    // OP_CHECK_IF((!(enablePFAMLA && (queryShapeInfo.d == 192)) || (enableIFAMLA &&(queryShapeInfo.d == 512))), OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
    //         "PFA rope should be combined and transmitted, and IFA rope should be separated and transmitted"),return false);
    ...
    PFAShapeInfo& queryShapeInfo, PromptFlashAttentionPioneerTilingData& tilingData) 
{
    ...
    if (!CheckSinkLengthCrossver(contextKeyParams, queryShapeInfo)) {
        return ge::GRAPH_FAILED;
    }
