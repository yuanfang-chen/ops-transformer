...
#include "attention_pioneer_inner.h"
...
aclnnStatus aclnnAttentionPioneerGetMaxWorkspaceSize(
    ...
    const aclTensor *attentionOut, const aclTensor *softmaxLse, uint64_t *workspaceSize, aclOpExecutor **executor)
{
    OP_LOGD("start aclnnAttentionPioneerGetMaxWorkspaceSize");
    const aclTensorList *tensorListKey = key;
    const aclTensorList *tensorListValue = value;
    AP_TensorPreProcess(tensorListKey, tensorListValue);
    ...
    AP_PrefixTensorPreProcess(tensorKeySharedPrefixOptional, tensorValueSharedPrefixOptional);
    ...
    aclnnStatus ret = AP_FakeArray(actualSeqLengthsOptional, fakeActualSeqLengthsOptional);
    CHECK_RET_CODE(ret, "Try alloc fake actualSeqLengthsOptional failed");

    ret = AP_FakeArray(actualSeqLengthsKvOptional, fakeActualSeqLengthsKvOptional);
    ...
    ret = AP_FakeArray(actualSharedPrefixLenOptional, fakeActualSharedPrefixLenOptional);
    if (ret != ACLNN_SUCCESS) {
    ...
    ret = AP_FakeArray(qStartIdxOptional, fakeQStartIdxOptional);
    if (ret != ACLNN_SUCCESS) {
    ...
    ret = AP_FakeArray(kvStartIdxOptional, fakeKVStartIdxOptional);
    if (ret != ACLNN_SUCCESS) {
    ...
    ret = aclnnInnerAttentionPioneerTensorGetWorkspaceSize(
    ...
    const aclTensor *attentionOut, const aclTensor *softmaxLse, uint64_t *workspaceSize, aclOpExecutor **executor)
{
    ...
    aclnnStatus ret = aclnnInnerAttentionPioneerGetWorkspaceSize(
...
{
    return aclnnInnerAttentionPioneer(workspace, workspaceSize, executor, stream);
}
