...
# aclnnAttentionPioneer
[📄 查看源码](https://gitcode.com/cann/ops-transformer/tree/master/attention/attention_pioneer)
...
-  接口功能：适配decode & prefill场景的FlashAttention算子，既可以支持prefill计算场景（PromptFlashAttention），也可支持decode计算场景（IncreFlashAttention）。
    ...
    相比于FusedInferAttentionScoreV4，本接口（AttentionPioneer）新增qStartIdxOptional、kvStartIdxOptional、pseType参数。
...
算子执行接口为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnAttentionPioneerGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnAttentionPioneer”接口执行计算。
...
```c++
aclnnStatus aclnnAttentionPioneer(
    void                *workspace, 
    uint64_t            workspaceSize, 
    aclOpExecutor       *executor, 
...
## aclnnAttentionPioneerGetWorkspaceSize
...
- **参数说明：**
    ...
    <tbody>
        ...
        <td>workspaceSize</td>
        <td>输入</td>
        <td>在Device侧申请的workspace大小，由第一段接口aclnnAttentionPioneerGetWorkspaceSize获取。</td>
        </tr>
        <tr>
        <td>executor</td>
        <td>输入</td>
...
示例代码如下，仅供参考，具体编译和执行过程请参考[编译与运行样例](../../../docs/zh/context/编译与运行样例.md)。
  ...
  #include "aclnnop/aclnn_attention_pioneer.h"
  ...
  int main() {
      ...
      aclOpExecutor* executor;
      // 调用第一段接口
      ret = aclnnAttentionPioneerGetWorkspaceSize(queryTensor, tensorKeyList, tensorValueList, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, numHeads, scaleValue, preTokens, nextTokens, layerOut, numKeyValueHeads, sparseMode, innerPrecise, blockSize, antiquantMode, softmaxLseFlag, keyAntiquantMode, valueAntiquantMode, queryAntiquantMode, pseType, outTensor, nullptr, &workspaceSize, &executor);
      CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAttentionPioneerGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
      ...
      // 调用第二段接口
      ret = aclnnAttentionPioneer(workspaceAddr, workspaceSize, executor, stream);
      CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAttentionPioneer failed. ERROR: %d\n", ret); return ret);
