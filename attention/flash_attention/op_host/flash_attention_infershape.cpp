...
/*!
 * \file flash_attention_infershape.cpp
 * \brief FlashAttention算子InferShape实现
 *        根据输入shape和layout属性推导attention_out和softmax_lse的输出shape。
 */
...
// 属性索引（与flash_attention_def.cpp中的属性声明顺序一致）
...
ge::graphStatus InferShapeFlashAttention(gert::InferShapeContext *context)
{
    ...
    // ----------------------------------------------------------------
    // 从v shape解析 headDimV（MLA/MQA场景v的D可能与q不同）
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    // 推导attention_out的shape（layout由layoutOut决定，D维用headDimV）
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    // 推导softmax_lse的shape
    //   TND layout:   (T_q, N_q)  -- 2维，每个token每个head一个lse值
    //   其他layout:   (B, N_q, S_q) -- 3维
    // ----------------------------------------------------------------
