...
/*!
 * \file flash_attention_template_tiling_key.h
 * \brief FlashAttention TilingKey定义（非量化，仅FP16/BF16）
 *
 * 相比flash_attention_score，简化了以下维度：
 *   - 移除PSE相关（无PSE）
 *   - 移除HasDrop（无dropout）
 *   - 移除HasRope（无rope）
 *   - 移除FP8相关dtype
 *   - 新增IsPA维度（分页注意力PA_ND/PA_Nz）
 *   - 新增IsSoftmaxLse维度（控制训练/推理模式）
 */

#ifndef TEMPLATE_TILING_KEY_FLASH_ATTENTION_H_
#define TEMPLATE_TILING_KEY_FLASH_ATTENTION_H_
...
ASCENDC_TPL_ARGS_DECL(FlashAttention,
    ...
    // bit:7-4  Layout (Q的数据布局): 1=BSND, 2=BNSD, 3=TND (与flash_attention_score保持一致)
...
ASCENDC_TPL_SEL(
    // FP16/BF16  AA_HIGH_PRECISION  BSND/BNSD/TND  128_128  D均一
    ...
    // FP16/BF16  AA_HIGH_PRECISION  BSND/BNSD/TND  128_128  DvTemplateType != DTemplateType (MLA)
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_UINT_SEL(KernelTypeKey, ASCENDC_TPL_UI_LIST, 0),
        ASCENDC_TPL_UINT_SEL(ImplMode, ASCENDC_TPL_UI_LIST, 0),
        ASCENDC_TPL_UINT_SEL(Layout, ASCENDC_TPL_UI_LIST, 1, 2, 3),
    ...
    // FP16/BF16  AA_HIGH_PRECISION  BSND/BNSD/TND  64_128  小batch场景
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_UINT_SEL(KernelTypeKey, ASCENDC_TPL_UI_LIST, 0),
        ASCENDC_TPL_UINT_SEL(ImplMode, ASCENDC_TPL_UI_LIST, 0),
        ASCENDC_TPL_UINT_SEL(Layout, ASCENDC_TPL_UI_LIST, 1, 2, 3),
    ...
    // FP16/BF16  AA_INVALID_LINE_HIGH_PRECISION  BSND/BNSD/TND  128_128
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_UINT_SEL(KernelTypeKey, ASCENDC_TPL_UI_LIST, 0),
    ...
    // FP16/BF16  PA（分页注意力）  BSND/BNSD/TND
    ASCENDC_TPL_ARGS_SEL(
        ASCENDC_TPL_UINT_SEL(KernelTypeKey, ASCENDC_TPL_UI_LIST, 0),
        ASCENDC_TPL_UINT_SEL(ImplMode, ASCENDC_TPL_UI_LIST, 0, 1),
        ASCENDC_TPL_UINT_SEL(Layout, ASCENDC_TPL_UI_LIST, 1, 2, 3),
        ASCENDC_TPL_UINT_SEL(S1TemplateType, ASCENDC_TPL_UI_LIST, 64, 128),
        ASCENDC_TPL_UINT_SEL(S2TemplateType, ASCENDC_TPL_UI_LIST, 64, 128),
        ASCENDC_TPL_UINT_SEL(DTemplateType, ASCENDC_TPL_UI_LIST, 64, 128, 192, 256),
        ASCENDC_TPL_UINT_SEL(DvTemplateType, ASCENDC_TPL_UI_LIST, 0),
        ASCENDC_TPL_UINT_SEL(HasAtten, ASCENDC_TPL_UI_LIST, 0, 1),
        ASCENDC_TPL_UINT_SEL(IsPA, ASCENDC_TPL_UI_LIST, 1),
        ASCENDC_TPL_UINT_SEL(IsSoftmaxLse, ASCENDC_TPL_UI_LIST, 0),
        ASCENDC_TPL_UINT_SEL(Regbase, ASCENDC_TPL_UI_LIST, 1),
        ASCENDC_TPL_TILING_STRUCT_SEL(optiling::FlashAttentionScoreSimplifiedTilingData)
    ),
    #endif
...
#endif // TEMPLATE_TILING_KEY_FLASH_ATTENTION_H_
