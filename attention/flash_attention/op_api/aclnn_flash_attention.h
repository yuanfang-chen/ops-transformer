...
#ifndef OP_API_INC_LEVEL2_ACLNN_FLASH_ATTENTION_H_
#define OP_API_INC_LEVEL2_ACLNN_FLASH_ATTENTION_H_
 ...
 *
 * 该接口实现训练推理归一的FlashAttention计算，支持：
 *   - 训练正向过程（returnSoftmaxLse=1时输出softmax_lse，供反向传播使用）
 *   - 推理过程（支持分页KV缓存，通过block_table管理KV Block）
 *   - 变长序列（通过cu_seqlens_q/kv或seqused_q/kv指定实际序列长度）
 *   - 预计算tiling方案（通过metadata输入，避免推理时重复计算tiling）
 *   - 仅支持非量化场景，q/k/v数据类型为FLOAT16或BFLOAT16
 *
...
#endif  // OP_API_INC_LEVEL2_ACLNN_FLASH_ATTENTION_H_
