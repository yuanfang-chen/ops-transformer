...
ge::graphStatus LIInfoParser::CheckShapeDim()
{
    ...
    OP_CHECK_IF(outShapeDim != qExpectShapeDim,
               ...
               return ge::GRAPH_FAILED);
    OP_CHECK_IF(valuesOutShapeDim != qExpectShapeDim,
    ...
    OP_CHECK_IF(!(weightsShapeDim == qExpectShapeDim - 1),
               OP_LOGE(opName_, "the dim num of weights's shape should be %u, but now is %u", qExpectShapeDim - 1,
                weightsShapeDim),
               return ge::GRAPH_FAILED);
    OP_CHECK_IF(valuesOutShapeDim != 0 && (!(*opParamInfo_.returnValue)),
               OP_LOGE(opName_, "when return value is false, the dim num of sparse_values's shape should be null, but now is %u",
                outShapeDim),
               return ge::GRAPH_FAILED);
...
ge::graphStatus LIInfoParser::ValidateInputShapesMatchQtnd()
{
    ...
    OP_CHECK_IF((opParamInfo_.weights.shape->GetStorageShape().GetDim(0) != qTsize) ||
                ...
                OP_LOGE(opName_, "TND case input query, weights, sparse_indices, sparse_values dim 0 are %u, %ld, %ld, %ld respectively, they must be same.",
...
ge::graphStatus LIInfoParser::ValidateInputShapesMatchQbsnd()
{
    ...
    OP_CHECK_IF((opParamInfo_.weights.shape->GetStorageShape().GetDim(0) != bSize_) ||
                ...
                OP_LOGE(opName_, "BSND case input query, weight, sparse_indices, sparse_values dim 0 are %u, %ld, %ld, %ld respectively, they must be same.",
    ...
    OP_CHECK_IF((opParamInfo_.weights.shape->GetStorageShape().GetDim(1) != s1Size_) ||
                ...
                (opParamInfo_.valuesOut.shape->GetStorageShape().GetDim(0) != s1Size_),
                OP_LOGE(opName_, "BSND case input query, weight, sparse_indices, sparse_values dim 1 are %u, %ld, %ld, %ld, they must be same.",
                    s1Size_, opParamInfo_.weights.shape->GetStorageShape().GetDim(1),
                    opParamInfo_.attenOut.shape->GetStorageShape().GetDim(1),
                    opParamInfo_.valuesOut.shape->GetStorageShape().GetDim(1)),
                return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus LIInfoParser::ValidateInputShapesMatch()
{
    ...
    */
    ...
    OP_CHECK_IF((opParamInfo_.attenOut.shape->GetStorageShape().GetDim(outN2Dim) != n2Size_) ||
               ...
               OP_LOGE(opName_, "input query, output sparse_indices and sparse_values shape n2 dim must be same,"
                       "but now they are %u, %ld, %ld respectively.",
                       n2Size_, opParamInfo_.attenOut.shape->GetStorageShape().GetDim(outN2Dim),
                       opParamInfo_.valuesOut.shape->GetStorageShape().GetDim(outN2Dim)),
               return ge::GRAPH_FAILED);
    // -----------------------check sparse_count-------------------
    OP_CHECK_IF((opParamInfo_.attenOut.shape->GetStorageShape().GetDim(outN2Dim + 1) != *opParamInfo_.sparseCount) ||
               ...
               OP_LOGE(opName_, "output sparse_indices and sparse_values shape last dim must be same as attr sparse_count,"
                       "but now they are %u, %ld, %ld respectively.", *opParamInfo_.sparseCount,
