 ...
 * \brief use buffer for matmul l1, better pipeline
 */
...
public:
    ...
    // FP16/BF16: KEY_BUF_NUM 从3减到2（FP16比FP8大一倍，L1放不下3份）
    ...
    static constexpr uint32_t QUERY_MTE1_MTE2_EVENT = EVENT_ID4;         // KEY_MTE1_MTE2_EVENT + KEY_BUF_NUM;
    ...
    // FP16/BF16 NZ格式K轴块大小为16（FP8为32）
    static constexpr uint64_t BLOCK_CUBE_K = 16;
...
{
    pipe->InitBuffer(bufUB_, 2 * CeilDiv(constInfo_.mBaseSize, 2) * constInfo_.s2BaseSize * sizeof(float));
                                                    ...
                                                    const LICommon::RunInfo &runInfo)
{
    ...
    while (s2L1Offset < s2L1RealSize) {
        ...
        nd2nzPara.srcNdMatrixStride = 0;
        nd2nzPara.dstNzMatrixStride = 0;
        // FP16 NZ format: BLOCK_CUBE_K=16（而非FP8的32）
        DataCopy(keyL1_[(keyL1BufIdx_ % KEY_BUF_NUM) * KEY_BUFFER_OFFSET + s2L1Offset * BLOCK_CUBE_K],
                                                     ...
                                                     uint64_t s1gL0RealSize, const LICommon::RunInfo &runInfo)
{
    ...
    loadData2DParamsV2.kStep = CeilDiv(constInfo_.headDim, BLOCK_CUBE_K);
...
template <typename LIT>
__aicore__ inline void LIMatmul<LIT>::ComputeL0c(uint64_t s1gL0RealSize, uint64_t s2L0RealSize,
                                                const LICommon::RunInfo &runInfo)
{
