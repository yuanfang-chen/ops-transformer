...
template <typename LIT>
__aicore__ inline void QLIMatmul<LIT>::InitParams(const ConstInfo &constInfo)
{
...
template <typename LIT>
__aicore__ inline void QLIMatmul<LIT>::InitBuffers(TPipe *pipe)
{
...
QLIMatmul<LIT>::InitMm1GlobalTensor(const GlobalTensor<int32_t> &blkTableGm, const GlobalTensor<K_T> &keyGm,
                                   const GlobalTensor<Q_T> &queryGm)
{
...
template <typename LIT>
__aicore__ inline void QLIMatmul<LIT>::ComputeMm1(const LICommon::RunInfo &runInfo)
{
    CrossCoreWaitFlag<LICommon::ConstInfo::QLI_SYNC_MODE4, PIPE_FIX>(LICommon::ConstInfo::CROSS_VC_EVENT + runInfo.loop % 2);
    CrossCoreWaitFlag<LICommon::ConstInfo::QLI_SYNC_MODE4, PIPE_FIX>(LICommon::ConstInfo::CROSS_VC_EVENT + runInfo.loop % 2 + LICommon::ConstInfo::AIV0_AIV1_OFFSET);
    ...
    CrossCoreSetFlag<LICommon::ConstInfo::QLI_SYNC_MODE4, PIPE_FIX>(LICommon::ConstInfo::CROSS_CV_EVENT + runInfo.loop % 2); 
    CrossCoreSetFlag<LICommon::ConstInfo::QLI_SYNC_MODE4, PIPE_FIX>(LICommon::ConstInfo::CROSS_CV_EVENT + runInfo.loop % 2 + LICommon::ConstInfo::AIV0_AIV1_OFFSET); 
}

template <typename LIT>
__aicore__ inline void QLIMatmul<LIT>::KeyNd2Nz(uint64_t s2L1RealSize, uint64_t s2GmOffset,
                                                    const LICommon::RunInfo &runInfo)
{
...
// blkNum, blkSize, N2, D
template <typename LIT>
__aicore__ inline void QLIMatmul<LIT>::KeyNd2NzForPA(uint64_t s2L1RealSize, uint64_t s2GmOffset,
                                                    const LICommon::RunInfo &runInfo)
{
...
// batch, s1, n2, g, d
template <typename LIT>
__aicore__ inline void QLIMatmul<LIT>::QueryNd2Nz(uint64_t s1gL1RealSize, uint64_t s1gGmOffset,
                                                 const LICommon::RunInfo &runInfo)
{
    Nd2NzParams nd2nzPara;
    nd2nzPara.ndNum = 1;
...
template <typename LIT>
__aicore__ inline void QLIMatmul<LIT>::LoadQueryToL0a(uint64_t s1gGmOffset, uint64_t s1gL1Offset, uint64_t s1gL1RealSize,
                                                     uint64_t s1gL0RealSize, const LICommon::RunInfo &runInfo)
{
...
template <typename LIT>
__aicore__ inline void QLIMatmul<LIT>::LoadKeyToL0b(uint64_t s2L1Offset, uint64_t s2L1RealSize, uint64_t s2L0RealSize,
                                                   const LICommon::RunInfo &runInfo)
{
    LoadData2DParamsV2 loadData2DParamsV2;
...
template <typename LIT>
__aicore__ inline void QLIMatmul<LIT>::ComputeL0c(uint64_t s1gL0RealSize, uint64_t s2L0RealSize,
                                                const LICommon::RunInfo &runInfo)
{
...
template <typename LIT>
__aicore__ inline void QLIMatmul<LIT>::Fixp(uint64_t s1gGmOffset, uint64_t s2GmOffset, uint64_t s1gL0RealSize,
                                           uint64_t s2L0RealSize, const LICommon::RunInfo &runInfo)
{
    ...
    if constexpr (std::is_same_v<float, float>) {
        ...
        Fixpipe<float, float, QLI_CFG_ROW_MAJOR_UB>(mm1ResUB_[(runInfo.loop % 2) * (UB_BANK_STRIDE / sizeof(float))], // 未考虑s1gGmOffset和s2GmOffset
                                                    cL0_[(l0BufIdx_ % L0_BUF_NUM) * l0cBufferOffset_ + mSize / 2 * 16], fixpipeParams); // 将matmul结果从L0C搬运到UB
    }
}

template <typename LIT>
__aicore__ inline void QLIMatmul<LIT>::AllocEventID()
{
...
template <typename LIT>
__aicore__ inline void QLIMatmul<LIT>::FreeEventID()
{
