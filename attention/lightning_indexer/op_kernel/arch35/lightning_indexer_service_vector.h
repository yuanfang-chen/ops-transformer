...
template <typename LIT>
__aicore__ inline void LIVector<LIT>::InitVecInputTensor(GlobalTensor<W_T> weightsGm, GlobalTensor<int32_t> indiceOutGm,
                                                         GlobalTensor<K_T> valueOutGm, GlobalTensor<int32_t> blockTableGm)
{
...
template <typename LIT>
__aicore__ inline void LIVector<LIT>::InitVecWorkspaceTensor(GlobalTensor<uint16_t> scoreGm)
{
...
template <typename LIT>
__aicore__ inline void LIVector<LIT>::ProcessVec1(const LICommon::RunInfo &info)
{
    ...
    if (curAivS1ProcNum == 0) {
        CrossCoreWaitFlag<LICommon::ConstInfo::QLI_SYNC_MODE4, PIPE_V>(LICommon::ConstInfo::CROSS_CV_EVENT + pingpong);  // V核等C核计算完mm1，mm1Res已搬运到UB
        CrossCoreSetFlag<LICommon::ConstInfo::QLI_SYNC_MODE4, PIPE_V>(LICommon::ConstInfo::CROSS_VC_EVENT + pingpong);   // V核处理完，通知C核可以把mm1Res搬运到UB
        return;
    }
    ...
    //CV同步
    CrossCoreWaitFlag<LICommon::ConstInfo::QLI_SYNC_MODE4, PIPE_V>(LICommon::ConstInfo::CROSS_CV_EVENT + info.loop % 2);   //V核等C核计算完mm1，mm1Res已搬运到UB
    ...
    int64_t vec1OutGmOffset = blockId_ % 2 == 0 ? curS2Idx : 
                            s1BaseSizePerAIV * LICommon::Align((uint64_t)constInfo_.kSeqSize, (uint64_t)s2BaseSize_) + curS2Idx;
    ...
    copyOutParams.dstStride = (LICommon::Align((uint64_t)constInfo_.kSeqSize, (uint64_t)s2BaseSize_) - s2BaseSize_) * sizeof(uint16_t);
...
template <typename LIT>
__aicore__ inline void LIVector<LIT>::ProcessTopK(const LICommon::RunInfo &info)
{
    ...
    for (uint32_t i = 0; i < curAivS1ProcNum; i++) {
        ...
        if (validS2Len >= topkCount_) {
            ...
            if (s2LoopNum == 1) {
                ...
                AscendC::DataCopyPadExtParams<uint16_t> padParams{true, 0, 0, 0};
                AscendC::DataCopyPad(mrgValueLocal_, scoreGm[vecOffset * LICommon::Align((uint64_t)constInfo_.kSeqSize, (uint64_t)s2BaseSize_)], copyInParams, padParams);
                ...
                for (uint32_t loopIdx = 0; loopIdx < s2LoopNum; loopIdx++) {
                    if (loopIdx == 0) {
                        ...
                        SetFlag<HardEvent::MTE2_V>(TOPK_MTE2_V_EVENT);
                        WaitFlag<HardEvent::MTE2_V>(TOPK_MTE2_V_EVENT);
                        topkOp_(mrgValueLocal_, indicesOutLocal_, scoreOutLocal_, trunkLen_, loopIdx, s2LoopNum, returnValueFlag);
                        continue;
                    }
                    ...
                    uint32_t offset = vecOffset * LICommon::Align((uint64_t)constInfo_.kSeqSize, (uint64_t)s2BaseSize_) + loopIdx * trunkLen_;
                    ...
                    if ((topkCountAlign256_ + validTrunkLen) % 256 != 0) {
                        Duplicate(mrgValueLocal_[topkCountAlign256_ + validTrunkLen / 256 * 256], zero, LICommon::Align(validTrunkLen, (uint32_t)256) - validTrunkLen / 256 * 256);
                        SetFlag<HardEvent::V_MTE2>(V_MTE2_EVENT);
                        WaitFlag<HardEvent::V_MTE2>(V_MTE2_EVENT);
                    }
                    WaitFlag<HardEvent::V_MTE2>(V_MTE2_EVENT1);
                    ...
                    SetFlag<HardEvent::MTE2_V>(TOPK_MTE2_V_EVENT);
                    WaitFlag<HardEvent::MTE2_V>(TOPK_MTE2_V_EVENT);
                    topkOp_(mrgValueLocal_, indicesOutLocal_, scoreOutLocal_, LICommon::Align(topkCountAlign256_ + validTrunkLen, (uint32_t)256), loopIdx, s2LoopNum, returnValueFlag);
                    SetFlag<HardEvent::V_MTE2>(V_MTE2_EVENT1);
                }
            }
        } else {
        ...
        if (returnValueFlag) {
            ...
            if (validS2Len < topkCount_) {
                ...
                mask[0] = mask[0] << (validS2Len % 8);
                PipeBarrier<PIPE_V>();
                Duplicate(valueOutLocal_[validS2Len / 8 * 8], zeroFloat, mask, 1, 1, 0);
            }
        
            if (validS2Len / 8 * 8 + 64 < topkCount_) {
                PipeBarrier<PIPE_V>();
                Duplicate(valueOutLocal_[validS2Len / 8 * 8 + 64], zeroFloat, topkCount_ - (validS2Len / 8 * 8 + 64));
            }
            ...
            // 搬运到GM
            AscendC::DataCopyPad(valueOutGm[info.valueOutOffset + (curS1Idx + rowIdx) * topkCount_], valueOutLocal_, copyOutValueParams);
        }        
    }
}
}  // namespace LIKernel
