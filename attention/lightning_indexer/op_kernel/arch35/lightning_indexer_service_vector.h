 ...
 * \brief
 */
#ifndef LIGHTNING_INDEXER_SERVICE_VECTOR_H_ARCH35
#define LIGHTNING_INDEXER_SERVICE_VECTOR_H_ARCH35
...
__aicore__ inline void LIVector<LIT>::InitBuffers(TPipe *pipe)
{
    ...
    // Topk
    pipe->InitBuffer(mrgValueBuf_, (topkCountAlign256_ + trunkLen_) * sizeof(uint16_t));
...
__aicore__ inline void LIVector<LIT>::ProcessVec1(const LICommon::RunInfo &info)
{
    ...
    if (curAivS1ProcNum == 0) {
        CrossCoreWaitFlag<LICommon::ConstInfo::QLI_SYNC_MODE4, PIPE_V>(LICommon::ConstInfo::CROSS_CV_EVENT + pingpong);
        CrossCoreSetFlag<LICommon::ConstInfo::QLI_SYNC_MODE4, PIPE_V>(LICommon::ConstInfo::CROSS_VC_EVENT + pingpong);
        return;
    }
    ...
    //CV同步
    CrossCoreWaitFlag<LICommon::ConstInfo::QLI_SYNC_MODE4, PIPE_V>(LICommon::ConstInfo::CROSS_CV_EVENT + info.loop % 2);
