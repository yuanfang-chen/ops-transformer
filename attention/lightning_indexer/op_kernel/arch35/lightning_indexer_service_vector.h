...
template <typename LIT>
__aicore__ inline void QLIVector<LIT>::InitBuffers(TPipe *pipe)
{
...
template <typename LIT>
__aicore__ inline void QLIVector<LIT>::InitParams(const struct LICommon::ConstInfo &constInfo,
                                                   const LITilingData *__restrict tilingData)
{
...
template <typename LIT>
__aicore__ inline void QLIVector<LIT>::InitVecInputTensor(GlobalTensor<W_T> weightsGm, GlobalTensor<int32_t> indiceOutGm,
                                                           GlobalTensor<int32_t> blockTableGm)
{
...
template <typename LIT>
__aicore__ inline void QLIVector<LIT>::InitVecWorkspaceTensor(GlobalTensor<uint16_t> scoreGm)
{
    this->scoreGm = scoreGm; // resucesum*k
}

template <typename LIT>
__aicore__ inline void QLIVector<LIT>::AllocEventID()
{
...
template <typename LIT>
__aicore__ inline void QLIVector<LIT>::FreeEventID()
{
...
template <typename LIT>
__aicore__ inline void QLIVector<LIT>::CleanInvalidOutput(int64_t invalidS1Offset)
{
...
template <typename LIT>
__aicore__ inline void QLIVector<LIT>::ProcessVec1(const LICommon::RunInfo &info)
{
...
template <typename LIT>
__aicore__ inline void QLIVector<LIT>::ProcessTopK(const LICommon::RunInfo &info)
{
        ...
        if (validS2Len >= topkCount_) {
            ...
            if (s2LoopNum == 1) {
                ...
                for (uint32_t loopIdx = 0; loopIdx < s2LoopNum; loopIdx++) {
                    ...
                    SetFlag<HardEvent::MTE2_V>(TOPK_MTE2_V_EVENT);
                    WaitFlag<HardEvent::MTE2_V>(TOPK_MTE2_V_EVENT);
                    topkOp_(mrgValueLocal_, indicesOutLocal_, scoreOutLocal_, LICommon::Align(topkCountAlign256_ + validTrunkLen, (uint32_t)256), loopIdx, s2LoopNum);
                    SetFlag<HardEvent::V_MTE2>(V_MTE2_EVENT1);
                }
            }
        } else {
