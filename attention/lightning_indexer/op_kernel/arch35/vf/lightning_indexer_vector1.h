...
__simd_vf__ inline void CastWeightTypeFP16(__ubuf__ half* weight_,
                                           uint32_t weightStride,
                                           __ubuf__ float* weightFloat_,
                                           const int batch)
{
...
// float in uint16 out
// LI版本: 移除了kScale和qScale乘法，直接计算 Score = W ⊙ ReLU(Q @ K^T)
                                                  ...
                                                  const int batch)
{
    ...
    if constexpr (std::is_same<W_T, bfloat16_t>::value) {
        CastWeightType(weight, weightStride, weightFloat, batch);
    } else if constexpr (std::is_same<W_T, half>::value) {
        CastWeightTypeFP16(weight, weightStride, weightFloat, batch);
    }
    // W_T == float: weightFloat already points to weight (no cast needed)
    ...
    if (batch == 2) {
        ...
        MulWeightAndReduceSum(out, qk, qkVLStride, weightFloat, gSize);
    }
}
