...
__simd_callee__ inline void UIntToSortableKey(AscendC::MicroAPI::RegTensor<FloatT>& outKey,
                                              AscendC::MicroAPI::RegTensor<typename UIntSortConstCtx<FloatT>::UInt>& inVal,
                                              ...
                                              AscendC::MicroAPI::MaskReg& maskAll)
{
    ...
    AscendC::MicroAPI::Xor((AscendC::MicroAPI::RegTensor<UInt>&)outKey, (AscendC::MicroAPI::RegTensor<UInt>&)outKey, regMask, maskAll);
}
                                              ...
                                              const uint32_t topK)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for(uint16_t i = 0; i < topkLoopNum; ++i) {
            ...
            AscendC::MicroAPI::StoreAlign<bfloat16_t, AscendC::MicroAPI::StoreDist::DIST_NORM>(outBuf + i * 128, regOut, maskAllB16);
        }
...
__aicore__ inline void CastWeightType(const LocalTensor<half> &weight_,
                                     uint32_t weightStride,
                                     const LocalTensor<float> &weightFloat_,
                                     const int batch)
{
                                                  ...
                                                  const int batch)
{
    ...
    if constexpr (sizeof(W_T) != sizeof(float)) {
    ...
    if (batch == 2) {
        ...
        MulWeightAndReduceSum(out_, qk_, qkVLStride, weightFloat_, gSize);
    }
}
