...
public:
    ...
    {
        // 2 * LICommon::Align(topK, (uint32_t)256):两块hisIndexLocal；3 * 256：histogramsLocal idxHighLocal idxLowLocal；64：nkValueLocal
                                      ...
                                      uint32_t s2LoopNum, bool returnValueFlag)
    {
        ...
        if (loopIdx == 0) {
            ...
            topkb16gather::LiTopKGatherVF(hisIndexLocal[(loopIdx + 1) % 2], hisValueLocal, mrgValueLocal, tmpIndexLocal, hisIndexLocal[loopIdx % 2], 
                                    topK, loopIdx * trunkLen - LICommon::Align(topK, (uint32_t)256), s2SeqLen);
            if (loopIdx == s2LoopNum - 1) {
                PipeBarrier<PIPE_V>();
                AscendC::DataCopy(indicesOutLocal, hisIndexLocal[(loopIdx + 1) % 2], LICommon::Align(topK, (uint32_t)256));
            }
        }
    }
private:
