 ...
 * \brief arch35 kernel for lightning_indexer (non-quantized FP16/BF16)
 */

#ifndef LIGHTNING_INDEXER_KERNEL_H_ARCH35
#define LIGHTNING_INDEXER_KERNEL_H_ARCH35
...
__aicore__ inline void LIPreload<LIT>::InitTilingData(const LITilingData *__restrict tilingData)
{
    ...
    constInfo.keyBlockStride = constInfo.kCacheBlockSize * constInfo.kHeadNum * constInfo.headDim;
...
template <typename LIT>
__aicore__ inline void LIPreload<LIT>::GetS1S2ActualSeqLen(uint32_t bIdx, uint32_t &actS1Size, uint32_t &actS2Size, uint32_t &actS2SizeOrig)
{
...
template <typename LIT>
__aicore__ void inline LIPreload<LIT>::SplitCore(uint32_t curCoreIdx, uint32_t &coreNum,
                                              ...
                                              const LITilingData *__restrict tiling, TPipe *tPipe)
{
    ...
    uint64_t singleCoreScoreSize = constInfo.s1BaseSize * LICommon::Align((uint64_t)constInfo.kSeqSize, (uint64_t)constInfo.s2BaseSize)  * sizeof(uint16_t);
    ...
    if ASCEND_IS_AIV {
        ...
        weightsGm.SetGlobalBuffer((__gm__ Q_T *)weights);
...
#endif  // LIGHTNING_INDEXER_KERNEL_H_ARCH35
