...
template <int DT_W_FLAG, int DT_Q, int DT_K, int DT_OUT, int PAGE_ATTENTION, int LAYOUT_T, int K_LAYOUT_T>
                                             ...
                                             __gm__ uint8_t *tiling)
{
    ...
    if constexpr (DT_Q == LI_TPL_FP16 && DT_K == LI_TPL_FP16 && DT_OUT == LI_TPL_INT32) {
        INVOKE_LI_NO_KFC_OP_IMPL(LIPreload, DT_W_FLAG, half, half, int32_t, PAGE_ATTENTION, 
                                 LI_LAYOUT(LAYOUT_T), LI_LAYOUT(K_LAYOUT_T));
    } else {
        INVOKE_LI_NO_KFC_OP_IMPL(LIPreload, DT_W_FLAG, bfloat16_t, bfloat16_t, int32_t, PAGE_ATTENTION, 
                                 LI_LAYOUT(LAYOUT_T), LI_LAYOUT(K_LAYOUT_T));
    }
#endif
}
