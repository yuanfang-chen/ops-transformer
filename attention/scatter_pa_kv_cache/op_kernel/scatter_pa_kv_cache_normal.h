...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int64_t i = 0; i < curBlockFactor_; i++) {
            ...
            CopyoCache<T1>(kOffsetIn, kOffsetOut, kUbLoop, kUbTail, keyInGm_, keyCacheOutGm_);
            CopyoCache<T2>(vOffsetIn, vOffsetOut, vUbLoop, vUbTail, valueInGm_, valueCacheOutGm_);
        }
    }
    ...
    __aicore__ inline void ProcessFullyLoad()
    {
        ...
        uint64_t kTokenSizeAlign = (kTokenSize + kAlign - 1) / kAlign * kAlign; // Key一个token大小对齐
        ...
        uint64_t vTokenSizeAlign = (vTokenSize + vAlign - 1) / vAlign * vAlign; // Value一个token大小对齐
...
private:
    template <typename T>
    __aicore__ inline void CopyoCache(int64_t offsetIn, int64_t offsetOut, int64_t ubLoop, int64_t ubTail,
