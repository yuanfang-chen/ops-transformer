...
ge::graphStatus ScatterPaKvCacheMembaseTiling::GetInputDtype()
{
    if (params_.templateType == TEMPLATE_SISO || params_.templateType == TEMPLATE_SISO_NCT) {
        ...
        if (params_.templateType == TEMPLATE_NORMAL || params_.templateType == TEMPLATE_NORM_NCT ||
            params_.templateType == TEMPLATE_NZ || params_.templateType == TEMPLATE_ALIBI) {
            if (!(inputKeyDtype_ == ge::DT_FLOAT16 || inputKeyDtype_ == ge::DT_BF16 || inputKeyDtype_ == ge::DT_INT8)) {
                OP_LOGE(context_, "input key dtype must be float16 or bfloat16 or int8.");
                return ge::GRAPH_FAILED;
            }
            ...
            if (!(inputKeyDtype_ == ge::DT_FLOAT16 || inputKeyDtype_ == ge::DT_BF16)) {
                OP_LOGE(context_, "input key dtype must be float16 or bfloat16.");
                return ge::GRAPH_FAILED;
            }
            if (!(inputValueDtype_ == ge::DT_FLOAT16 || inputValueDtype_ == ge::DT_BF16)) {
                OP_LOGE(context_, "input value dtype must be float16 or bfloat16.");
                return ge::GRAPH_FAILED;
            }
        }
    }
...
ge::graphStatus ScatterPaKvCacheMembaseTiling::CheckInputShapeNz()
{
    ...
    if (inputKeyCacheInShape_.GetDimNum() == 4) {
        ...
        OP_CHECK_IF((static_cast<uint64_t>(numBlocks) * params_.blockSize < params_.numTokens),
                    OP_LOGE(context_, "numBlocks * blockSize should larger than numTokens."), return ge::GRAPH_FAILED);
        OP_CHECK_IF((lastDimK != inputKeyCacheInShape_.GetDim(DIM_3)),
                    OP_LOGE(context_, "dim3 of keyCache should be same as lastDimK."), return ge::GRAPH_FAILED);
        OP_CHECK_IF((params_.numHead * params_.kHeadSize / lastDimK != inputKeyCacheInShape_.GetDim(DIM_1)),
                    ...
                    return ge::GRAPH_FAILED);
        OP_CHECK_IF((lastDimV != inputValueCacheInShape_.GetDim(DIM_3)),
                    OP_LOGE(context_, "dim3 of valueCache should be same as lastDimV."), return ge::GRAPH_FAILED);
        ...
        OP_CHECK_IF((static_cast<uint64_t>(numBlocks) * params_.blockSize < params_.numTokens),
                    OP_LOGE(context_, "numBlocks * blockSize should larger than numTokens."), return ge::GRAPH_FAILED);
        OP_CHECK_IF((lastDimK != inputKeyCacheInShape_.GetDim(DIM_4)),
                    OP_LOGE(context_, "dim4 of keyCache should be same as lastDimK."), return ge::GRAPH_FAILED);
        ...
        OP_CHECK_IF((params_.kHeadSize / lastDimK != inputKeyCacheInShape_.GetDim(DIM_2)),
                    ...
                    return ge::GRAPH_FAILED);
        OP_CHECK_IF((lastDimV != inputValueCacheInShape_.GetDim(DIM_4)),
                    OP_LOGE(context_, "dim4 of valueCache should be same as lastDimV."), return ge::GRAPH_FAILED);
...
{
    bool canLoad = ((static_cast<uint64_t>(params_.numHead) * params_.kHeadSize * params_.typeByteK +
    ...
    if (canLoad && params_.numTokens < SMALL_TOKEN) {
...
{
    bool canLoad = (static_cast<uint64_t>(params_.numHead) * params_.kHeadSize * params_.typeByteK <= params_.ubSize);
