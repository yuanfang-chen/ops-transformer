...
def judge_value_by_isclose(real_data, data_compe):
    ...
    # print('fulfill_percent:', fulfill_percent, 'pct_thd:', pct_thd)
...
def compare_topk_valid(cur_cpu, cur_npu, topk_value, bsn, diff_npu, diff_cpu,
    ...
    if is_equivalent:
        ...
        for diff_idx in range(len(only_in_npu_list)):
            ...
            print(f'b_idx={b_idx} n2_idx={n2_idx} s1_idx={s1_idx}')
            print(f'    仅在npu的元素 idx={only_in_npu_list[diff_idx]} value={element_npu} AE={npu_ae} RE={npu_re}')
            print(f'    仅在cpu的元素 idx={only_in_cpu_list[diff_idx]} value={element_cpu} AE={cpu_ae} RE={cpu_re}')
            print(f'    bm_value={value_bm}')
            
            if npu_re > thres or cpu_re > thres: #TODO:标准2：万分之一
                if return_value_flag:
                    #TODO:标准3
                    # print(f'    该点位re超限,cpu_re{cpu_re},npu_re{npu_re}  接下来比较cpu/npu value')
                    # print('标准3...')
...
def compare_topk_invalid(cur_npu, cur_cpu, bsn, diff_npu, diff_cpu):
    b_idx, s1_idx, n2_idx = bsn
    npu_pass = True
    for i in range(len(cur_cpu)):
        element_npu = cur_npu[i]
        element_cpu = cur_cpu[i]
        if element_npu != -1:
            print(f'b_idx={b_idx} n2_idx={n2_idx} s1_idx={s1_idx} invalid部分不全是-1')
            print(f'    npu={element_npu}, cpu={element_cpu}')
            npu_pass = False
            diff_npu.append(element_npu)
        # if element_cpu != -1:
        #     npu_pass = False
...
def check_result(expect, result, topk_value, params):
    ...
    for t_id in rows:
        ...
        npu_pass_t, max_re_t = compare_topk_valid(cpu_reshape[t_id, :valid_len], npu_reshape[t_id, :valid_len],
                                                    ...
                                                    return_value_flag)
        # npu_pass_t = compare_topk_invalid(cpu_reshape[t_id, valid_len + 1 :], npu_reshape[t_id, valid_len + 1 :],
        #                                   bsn, diff_npu, diff_cpu)
        if not npu_pass_t:
            npu_pass = False
    end_time = time()
    print(f"耗时：{end_time - start_time:.6f} 秒")
    topk_precision = not diff_npu and not diff_cpu # TODO:均为空，topk_precision为True
    if topk_precision:
