...
__aicore__ inline void QLIMatmul<QLIT>::ComputeMm1(const QLICommon::RunInfo &runInfo)
{
    ...
    int64_t s1L0LoopCnt = CeilDiv(runInfo.actMBaseSize / constInfo_.gSize, constInfo.s1BaseSize / 2);  // 2 :一次取constInfo.s1BaseSize的一半
    int64_t s1gL1Offset[2] = {0, static_cast<int64_t>(constInfo_.gSize * constInfo.s1BaseSize / 2)};
    int64_t s1gL0RealSize[2] = {s1L0LoopCnt > 1 ? static_cast<int64_t>(constInfo_.gSize * constInfo.s1BaseSize / 2) : runInfo.actMBaseSize,
