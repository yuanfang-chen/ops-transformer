...
__aicore__ inline void QLIVector<QLIT>::InitBuffers(TPipe *pipe)
{
    ...
    pipe->InitBuffer(weightBuf_, 2 * CeilDiv(s1BaseSize_, 2) * gSize_* sizeof(bfloat16_t));    
    weightUB_ = weightBuf_.Get<bfloat16_t>();//weight
    pipe->InitBuffer(weightFloatBuf_, 2 * CeilDiv(s1BaseSize_, 2) * gSize_* sizeof(float));   // 大小：2(开dB) * 2 * 64 * 2 = 0.5KB 
    weightFloatUB_ = weightFloatBuf_.Get<float>();//weight bf16 -> float
    ...
    // Topk
    pipe->InitBuffer(mrgValueBuf_, (topkCount_ + trunkLen_) * sizeof(SCORE_T));     // 大小：(topkCount_ + 每次排序长度) * sizeof(SCORE_T)
    mrgValueLocal_ = mrgValueBuf_.Get<SCORE_T>();
    
    pipe->InitBuffer(indicesOutBuf_, (topkCount_ + 64) * sizeof(uint32_t));         // 大小：(topkCount_ + 64) * 4  64:duplicate刷-1需要额外空间
...
{	 
    ...
    wDataCopyExtParams.blockLen = curAivS1ProcNum * gSize_* sizeof(bfloat16_t);	 
    ...
    AscendC::Cast(weightFloatUB_, weightUB_, RoundMode::CAST_NONE, curAivS1ProcNum * gSize_* sizeof(bfloat16_t));
    //qScaleGm  -->  qScaleUB_	 
    DataCopyPadExtParams<float> padQScaleParams{false, 0, 0, 0};	 
    DataCopyExtParams qDataCopyExtParams;	 
    qDataCopyExtParams.blockCount = 1;	 
    qDataCopyExtParams.blockLen = curAivS1ProcNum * gSize_* sizeof(float);	 
    ...
    auto weightBase = weightFloatUB_[pingpong * s1BaseSizePerAIV * gSize_];	 
