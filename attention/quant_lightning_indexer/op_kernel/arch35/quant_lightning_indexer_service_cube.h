...
__aicore__ inline void QLIMatmul<QLIT>::ComputeMm1(const QLICommon::RunInfo &runInfo)
{
    ...
    for (uint64_t s2GmOffset = 0; s2GmOffset < s2ProcessSize; s2GmOffset += S2_BASIC_BLOCK) {
        ...
        for (uint64_t s1gGmOffset = 0; s1gGmOffset < s1gProcessSize; s1gGmOffset += constInfo.mBaseSize) {
            uint64_t s1gL1RealSize =
                s1gGmOffset + constInfo.mBaseSize > s1gProcessSize ? s1gProcessSize - s1gGmOffset : constInfo.mBaseSize;
            ...
            if (runInfo.isFirstS2InnerLoop && s2GmOffset == 0) {
                ...
                queryL1Mte1BufIdx_ =
                    queryL1Mte2BufIdx_ - (CeilDiv(s1gProcessSize, constInfo.mBaseSize) - 1 - (s1gGmOffset > 0));
            }
            for (uint64_t s2L1Offset = 0; s2L1Offset < s2L1RealSize; s2L1Offset += S2_BASIC_BLOCK_L0) {
                ...
                for (uint64_t s1gL1Offset = 0; s1gL1Offset < s1gL1SizeAlign2G; s1gL1Offset += constInfo.mBaseSize) {
                    WaitFlag<HardEvent::M_MTE1>(M_MTE1_EVENT + l0BufIdx_ % L0_BUF_NUM);
                    uint64_t s1gL0RealSize =
                        s1gL1Offset + constInfo.mBaseSize > s1gL1SizeAlign2G ? s1gL1SizeAlign2G - s1gL1Offset : constInfo.mBaseSize;
