...
template <typename T>	 
__simd_callee__ inline void BroadcastLane(AscendC::MicroAPI::RegTensor<T>& dst,	 
...
// W * Relu(ScaleQ * Q * (ScaleK * K)^T) ==> W * ScaleQ * Relu(Q * K^T) * ScaleK 
// float in uint32 out 
__aicore__ inline void MulWeightAndReduceSum(const LocalTensor<uint32_t> &out_,   // out    [S2Base]     [128   ] 
...
{ 
    ...
    auto out = (__local_mem__ uint32_t*)out_.GetPhyAddr(); 
    __VEC_SCOPE__ 
    { 
        ...
        DuplicateZero(regSum0, maskAll); 
        DuplicateZero(regSum1, maskAll); 
        // interleave load 
        ...
        for (uint16_t i = (uint16_t)(0); i < (uint16_t)(gSize); i += 2) {  
            ...
            WeightedAccum(regSum0, regQK, regwBrc, maskAll); 
            // interleave load 
            MicroAPI::LoadAlign<float, MicroAPI::LoadDist::DIST_DINTLV_B32>(regQK[0], regQK[1], qk + 128 * i + 128); 
            BroadcastLane(regwBrc, regW, i + 1); 
            WeightedAccum(regSum1, regQK, regwBrc, maskAll); 
        } 
        AscendC::MicroAPI::Add(regSum0[0], regSum0[0], regSum1[0], maskAll); 
        AscendC::MicroAPI::Add(regSum0[1], regSum0[1], regSum1[1], maskAll); 
        AscendC::MicroAPI::Mul(regSum0[0], regSum0[0], regKScale[0], maskAll); 
        AscendC::MicroAPI::Mul(regSum0[1], regSum0[1], regKScale[1], maskAll); 
        AscendC::MicroAPI::RegTensor<uint32_t> regOut[2]; 
        FloatX2ToSortableKey<float>(regOut[0], regOut[1], regSum0[0], regSum0[1], fp32Ctx, maskAll); 
        // interleave store 
        AscendC::MicroAPI::StoreAlign<uint32_t, MicroAPI::StoreDist::DIST_INTLV_B32>(out, regOut[0], regOut[1], maskAll); 
    } 
} 
...
{ 
    ...
    {	 
        ...
        // unroll2	 
        for (uint16_t i = (uint16_t)(0); i < (uint16_t)(gSize); i += 2) {	 
            MicroAPI::LoadAlign<float>(regQK[0], qk + 128 * i);	 
...
{	 
    ...
    {	 
        ...
        AscendC::MicroAPI::RegTensor<float> regKScale[2]; 
        AscendC::MicroAPI::RegTensor<float> regSum0[4]; 
        AscendC::MicroAPI::RegTensor<float> regSum1[4]; 
        AscendC::MicroAPI::MaskReg maskAllB32 = AscendC::MicroAPI::CreateMask<float, AscendC::MicroAPI::MaskPattern::ALL>(); 
        AscendC::MicroAPI::MaskReg maskAllB16 = AscendC::MicroAPI::CreateMask<bfloat16_t, AscendC::MicroAPI::MaskPattern::ALL>(); 
        FloatSortConstCtx<bfloat16_t> bf16Ctx; 
        InitFloatSortConstCtx(bf16Ctx, maskAllB16); 
        ...
        for (uint16_t i = (uint16_t)(0); i < (uint16_t)(gSize); i++) { 
            ...
            AscendC::MicroAPI::Gather(regwBrc[0], regW[0], brcGatherIndex); 
            AscendC::MicroAPI::Gather(regwBrc[1], regW[1], brcGatherIndex); 
        ...
        // Convert to bfloat16 and store first output channel 
        AscendC::MicroAPI::RegTensor<bfloat16_t> regSumBF16_0, regSumBF16_1; 
        // Process first output channel 
        AscendC::MicroAPI::DeInterleave(regSum0[0], regSum0[1], regSum0[0], regSum0[1]); 
        AscendC::MicroAPI::Cast<bfloat16_t, float, castTraitF32ToF16_ODD>(regSumBF16_0, regSum0[1], maskAllB32); 
        AscendC::MicroAPI::Cast<bfloat16_t, float, castTraitF32ToF16_EVEN>(regSumBF16_0, regSum0[0], maskAllB32); 
        AscendC::MicroAPI::RegTensor<uint16_t> regOut0; 
        FloatToSortableKey<bfloat16_t>(regOut0, regSumBF16_0, bf16Ctx, maskAllB16); 
        AscendC::MicroAPI::StoreAlign<uint16_t, AscendC::MicroAPI::StoreDist::DIST_NORM>(out0, regOut0, maskAllB16); 
        // Process second output channel 
        AscendC::MicroAPI::DeInterleave(regSum1[0], regSum1[1], regSum1[0], regSum1[1]); 
        AscendC::MicroAPI::Cast<bfloat16_t, float, castTraitF32ToF16_ODD>(regSumBF16_1, regSum1[1], maskAllB32); 
        AscendC::MicroAPI::Cast<bfloat16_t, float, castTraitF32ToF16_EVEN>(regSumBF16_1, regSum1[0], maskAllB32); 
        AscendC::MicroAPI::RegTensor<uint16_t> regOut1; 
        FloatToSortableKey<bfloat16_t>(regOut1, regSumBF16_1, bf16Ctx, maskAllB16); 
        AscendC::MicroAPI::StoreAlign<uint16_t, AscendC::MicroAPI::StoreDist::DIST_NORM>(out1, regOut1, maskAllB16); 
    } 
} 
...
{ 
    ...
    { 
        ...
        for (uint16_t i = (uint16_t)(0); i < (uint16_t)(gSize); i++) {	 
            AscendC::MicroAPI::Duplicate(brcGatherIndex, i);	 
            // interleave load	 
            MicroAPI::LoadAlign<float, MicroAPI::LoadDist::DIST_DINTLV_B32>(regQK0[0], regQK0[1], qk0 + 128 * i);	 
            MicroAPI::LoadAlign<float, MicroAPI::LoadDist::DIST_DINTLV_B32>(regQK1[0], regQK1[1], qk1 + 128 * i);	 
            AscendC::MicroAPI::Gather(regwBrc[0], regW[0], brcGatherIndex);	 
            AscendC::MicroAPI::Gather(regwBrc[1], regW[1], brcGatherIndex);	 
            AscendC::MicroAPI::Relu(regQK0[0], regQK0[0], maskAllB32);	 
            AscendC::MicroAPI::Relu(regQK0[1], regQK0[1], maskAllB32);	 
...
// float in uint16 out	 
__aicore__ inline void BatchMulWeightAndReduceSum(const LocalTensor<uint16_t> &out_,   // out    [B, S2Base]     [128   ]	 
                                                uint32_t outStride,	 
                                                const LocalTensor<float> &qk_,       // q*k^t  [B, G, S2Base]  [64 128]	 
                                                uint32_t qkStride,	 
                                                const LocalTensor<float> &weight_,   // w      [B, G]          [64    ]	 
                                                uint32_t weightStride,	 
                                                const LocalTensor<float> &kScale_,   // kScale [S2Base]        [128   ]	 
                                                uint32_t kScaleStride,	 
                                                const LocalTensor<float> &qScale_,   // qScale [B, G]          [64    ]	 
                                                uint32_t qScaleStride,	 
                                                const int gSize,                     // G 64	 
                                                const int batch)	 
{	 
    if (batch == 2) {	 
        ...
        MulWeightAndReduceSum(out_, qk_, weight_, kScale_, qScale_, gSize);	 
    }	 
}	 

// // float in uint16 out(qk bank优化)	 
__aicore__ inline void BatchMulWeightAndReduceSum(const LocalTensor<uint16_t> &out_,   // out    [S2Base]     [128   ]	 
                                                uint32_t outStride,	 
                                                const LocalTensor<float> &qk_,       // q*k^t  [G, S2Base]  [64 128]	 
                                                uint32_t qkVLStride,	 
                                                uint32_t qkStride,	 
