 ...
 * \brief Vanilla kernel-execution perf benchmark for FIAS V4 on ascend910b.
 *        Times only the device-side Execute call via aclrtEvent; host-side
 *        tiling (GetWorkspaceSize) is excluded from the measurement.
 */
...
// Run FIAS V4 with the given shapes; time only the second-stage kernel call.
                      ...
                      int warmup, int iters, aclrtStream stream)
{
    ...
    // Workspace size is shape-dependent only, so size it once and reuse the
    ...
    for (int i = 0; i < warmup; ++i) {
        ...
        CHECK_RET(aclnnFusedInferAttentionScoreV4(wsAddr, ws, exec, stream));
    }
...
int main()
{
    ...
    printf("=== FIAS V4 vanilla kernel perf (fp16, BNSD, ascend910b) ===\n");
