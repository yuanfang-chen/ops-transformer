...
/*!
 * \file test_fused_infer_attention_score_vx.cpp
 * \brief
 */
...
int main() {
    ...
    ret = aclnnFusedInferAttentionScoreVXGetWorkspaceSize(
    ...
    if (!CHECK_RET(ret == ACL_SUCCESS)) {
        LOG_PRINT("aclnnFusedInferAttentionScoreVXGetWorkspaceSize failed. ERROR: %d\n", ret);
        return ret;
    }
    ...
    if (!CHECK_RET(ret == ACL_SUCCESS)) {
        LOG_PRINT("aclnnFusedInferAttentionScoreVX failed. ERROR: %d\n", ret); 
        return ret;
    }
