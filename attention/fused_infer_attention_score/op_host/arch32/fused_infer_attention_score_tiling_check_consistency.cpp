...
ge::graphStatus FiaTilingCheck::CheckActualSeqLensKv() const
{
    ...
    if (kvLayout_ == FiaLayout::TND) {
        if (opParamInfo_.actualSeqLengthsQ.tensor != nullptr &&
            ...
            OP_LOGE(opName_, "when kv layout is TND, %s shape size is %u, it should be equal to %s shape size(%u).",
        ...
        if (actualSeqLengthsKvSize_ != bSize_ && actualSeqLengthsKvSize_ != 1U) {
            OP_LOGE(opName_, "when kv layout is TND, %s shape size is %u, it should be equal to batch size(%u) or equal to 1.",
