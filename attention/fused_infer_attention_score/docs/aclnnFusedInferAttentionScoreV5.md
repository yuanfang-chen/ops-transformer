...
- 接口功能：能适配decode & prefill场景的FlashAttention算子，既可以支持prefill计算场景（PromptFlashAttention），也可支持decode计算场景（IncreFlashAttention）。相比于FusedInferAttentionScoreV4，本接口新增splitfuse接口的alibimask能力(暂不支持)，及相关alibiCoeffOptional、alibiLeftAlign、isAlibiMaskSqrt参数。
