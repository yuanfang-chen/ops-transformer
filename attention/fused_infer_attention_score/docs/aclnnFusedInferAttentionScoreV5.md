...
- 单参数约束
  ...
  - 全量化场景
    ...
    - per-block 全量化
      ...
      - 入参 quantScale1的shape为（1），dtype为FLOAT32
    - per-tensor 全量化
...
- 特性交叉约束
  ...
  - 全量化场景
    ...
    - per-block 全量化
      ...
      - query、key、value 的dtype为FLOAT8_E4M3FN时，quantScale1的取值范围满足值[1,448]；query、key、value 的dtype为HIFLOAT8时，quantScale1的取值范围满足值[1,32768]
    - per-tensor 全量化
...
- 特性交叉约束
  ...
  - 伪量化
    ...
    - 当 keyAntiquantMode 和 valueAntiquantMode 为 per-token 或 per-token 使用 page attention 管理 scale/offset，query 数据类型为 FP16/BF16 且 key/value 数据类型为FLOAT8_E4M3FN，不支持叠加后量化
