...
- **参数说明：**
    ...
    <tr>
        ...
        <ul>
            ...
            <li>sparseMode = 9时：
                <ul>
                    <li>inputLayout为BSND、BNSD时，attenMaskOptional的shape输入支持(B, Q_S, Q_S)</li>
...
<summary><a id="Mask"></a>Mask</summary>
    &nbsp;&nbsp;<table style="undefined;table-layout: fixed; width: 1150px"><colgroup>
        ...
        <tr>
            ...
            <td>非量化支持GQA和MLA场景，全量化仅支持MLA场景。不支持左padding、pseShift、sharedPrefix。输出dtype不支持INT8。每个batch需满足Q_S≤KV_S。inputLayout为BSND/BNSD时mask shape为(B,Q_S,Q_S)；inputLayout为TND时mask shape为(∑Q_Si²,)。</td>
        </tr>
        </tbody>
    </table>
        ...
        <td>
            ...
            <li><strong>GQA/MQA场景</strong>（numHeads是numKeyValueHeads的整数倍且不相等）：
                ...
                <li>sparse模式：
                    ...
                    <li>sparse=9（传入tree mask，inputLayout为BSND/BNSD时shape为(B,Q_S,Q_S)，inputLayout为TND时shape为(∑Q_Si²,)）</li>
                    </ul>
                </li>
...
&nbsp;&nbsp;<table style="undefined;table-layout: fixed; width: 983px"><colgroup>
    ...
    <tr>
        ...
        <td colspan="3">当MTP等于0时，支持sparseMode=0且attenMask为nullptr；当MTP大于0、小于16时，支持sparseMode=3（传入优化后的attenMask矩阵，shape为2048*2048）或sparseMode=9（传入tree mask，inputLayout为BSND时shape为(B,Q_S,Q_S)，inputLayout为TND时shape为(∑Q_Si²,)）；</td>
    </tr>
    <tr>
        <td rowspan="9">伪量化</td>
    ...
    <col style="width: 750px"></colgroup>
        ...
        <tbody>
            ...
            <tr>
                ...
                <li>sparseMode = 9时，非量化支持GQA和MLA场景，全量化仅支持MLA场景。attenMaskOptional不能为空。inputLayout为BSND/BNSD时shape为(B,Q_S,Q_S)；inputLayout为TND时shape为(∑Q_Si²,)。不支持左padding、pseShift、sharedPrefix。输出dtype不支持INT8。每个batch需满足Q_S≤KV_S。</li>
