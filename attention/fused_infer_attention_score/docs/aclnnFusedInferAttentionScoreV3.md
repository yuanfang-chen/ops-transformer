...
- TND、TND_NTD、NTD_TND场景下query，key，value输入的综合限制：
  ...
  - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：
    - sparse模式仅支持sparse=0且attentionMask为nullptr，或sparse=3且attentionMask不为nullptr，或sparse=4且传入attentionMask不为nullptr；
    - 当query的d等于512时：
      - 支持TND、TND_NTD;
    ...
    - 当query的d不等于512时：
      ...
      - TND场景，当head配比为GQA/MQA时（即必须完整传入numHeads和numKeyValueHeads参数，且numHeads是numKeyValueHeads的整数倍，且二者不相等），有如下约束：
        - 当数据类型为FLOAT16、BFLOAT16时，支持sparse=0且attentionMask为nullptr，或sparse=3且传入优化后的attentionMask：
      ...
      - TND场景，当head配比为MHA时，有如下约束：
        - 当数据类型为FLOAT16时，仅支持sparse=0且attentionMask为nullptr；
        - 当数据类型为FLOAT16时，仅支持innerPrecise=1；
        - 当数据类型为FLOAT16时，不支持page attention；
        - 当数据类型为BFLOAT16时，支持sparse=0且attentionMask为nullptr，或sparse=3，4且传入优化后的attentionMask；
  ...
  - 输入queryRope和keyRope时，仅支持如下特性：
    ...
    - <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>：
      ...
      - 当query的d等于512时：
        ...
        - sparse：Q_S等于1时只支持sparse=0且attentionMask为nullptr，Q_S大于1时只支持sparse=3且传入mask；
...
- **当Q_S大于1时**：
  ...
  - 参数sparseMode当前仅支持值为0、1、2、3、4的场景，取其它值时会报错。
    ...
    - sparseMode = 2、3、4时，attenMask的shape需要为S,S或1,S,S或1,1,S,S,其中S的值需要固定为2048，且需要用户保证传入的attenMask为下三角，attentionMask为nullptr或者传入的shape不正确报错。
