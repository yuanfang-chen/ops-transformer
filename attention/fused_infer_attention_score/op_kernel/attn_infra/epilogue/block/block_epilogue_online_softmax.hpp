...
public:
        ...
        uint32_t qHeads, int64_t offsetFull, int64_t pseQ, int64_t pseKv)
    {
        ...
        if (proTokenNum != 0) {
            ...
            // AscendC::printf("proTokenNum gFullOffset:%d\n",gFullOffset);
            AscendC::DataCopyPad(
        ...
        for (uint32_t headIdx = 0; headIdx < integralHeadNum; headIdx++) {
            ...
            gFullOffset = BIdx * qHeads * pseQ * pseKv + qNStartIdxVec * pseQ * pseKv + offsetFull;
            // AscendC::printf("integralHeadNum gFullOffset:%d\n",gFullOffset);
            AscendC::DataCopyPad(
        ...
        if (epiTokenNum != 0) {
            ...
            gFullOffset = BIdx * qHeads * pseQ * pseKv + qNStartIdxVec * pseQ * pseKv + offsetFull;
            // AscendC::printf("epiTokenNum gFullOffset:%d\n",gFullOffset);
            AscendC::DataCopyPad(
                fullUbTensor16[innerUbRowOffset], gFull[gFullOffset],
                AscendC::DataCopyExtParams(
        ...
        uint32_t kvSEndIdx, uint32_t qNStartIdx, uint32_t BIdx, uint32_t qHeads, int64_t pseQ, int64_t pseKv, bool isLastStackTile)
    {
        ...
        // AscendC::printf("rowNum:%d,columnNum:%d,kvSStartIdx:%d,kvSEndIdx:%d,qSBlockSize:%d,rowOffer:%d,maskStride:%d\n",rowNum,columnNum,kvSStartIdx,kvSEndIdx,qSBlockSize,rowOffer,maskStride);
        ...
        // AscendC::printf("BIdx:%d,qNStartIdxVec:%d,qNSplitSubBlock:%d,maskOffsetThisSubBlock:%d\n",BIdx,qNStartIdxVec,qNSplitSubBlock,maskOffsetThisSubBlock);
         ...
         for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoopNum + preLoad; rowLoopIdx++) {
             if (rowLoopIdx < rowLoopNum) {
                ...
                if (rowLoopIdx == 0) {
                    ...
                    // AscendC::printf("proTokenNum:%d,integralHeadNum:%d,epiTokenNum:%d\n",proTokenNum,integralHeadNum,epiTokenNum);
                    AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);
                    CopyFullGmToUb(
                            gFull,
