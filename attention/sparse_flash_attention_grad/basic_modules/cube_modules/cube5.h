...
CubeOp<T1>::cube5LoadDy(LocalTensor<T1>& l1Dy, const int64_t dyGmOffset, const int32_t dIdx, 
                        const uint32_t perLoopDSize, const bool reloadDy)
{
    if (reloadDy) {
        ...
        l1Dy = l1_common_tensors[ping_pong_flag_l1_common_];
        int64_t currentDyGmOffset = dyGmOffset + dIdx * perLoopDSize;
        CopyGmToL1(l1Dy, attentionGradGm[currentDyGmOffset], dimG, perLoopDSize, dimDv);
    } else {
        l1Dy = l1_dy_tensor[dIdx * perLoopDSize * dimGAlign];
    }
}

template <typename T1>
__aicore__ inline __attribute__((always_inline)) void
CubeOp<T1>::cube5ProcessDLoop(LocalTensor<T1>& l1P, const int64_t dyGmOffset, const uint32_t dLoopTimes,
                               const uint32_t perLoopDSize, const uint32_t mmParamM, const int64_t mm5ResOutOffset,
                               const bool reloadDy)
{
    for (int32_t dIdx = 0; dIdx < dLoopTimes; dIdx++) {
        LocalTensor<T1> l1Dy;
        cube5LoadDy(l1Dy, dyGmOffset, dIdx, perLoopDSize, reloadDy);
        ...
        MmadInnerWithSync<T1>(l0cTensor, l1P, l1Dy,
...
template <typename T1>
__aicore__ inline __attribute__((always_inline)) void
CubeOp<T1>::cube5ProcessMLoop(const int64_t pGmOffset, const int64_t dyGmOffset, const int32_t blkCntOffset,
                               const int64_t mm5ResOutBaseOffset, const bool reloadDy)
{
    ...
    for (int32_t mIdx = blkCntOffset; mIdx < blkCntOffset + selectedCntOffset; mIdx += blockOffset) {
        LocalTensor<T1> l1P = l1_p_tensors[ping_pong_flag_l1_p_];
        ...
        CopyGmToL1(l1P, pWorkspaceGm[pGmOffset + (mIdx - blkCntOffset) * selectedBlockSize], dimG, mmParamM, PER_LOOP_BLOCK_SIZE);
        
        cube5ProcessDLoop(l1P, dyGmOffset, dLoopTimes, perLoopDSize, mmParamM, mm5ResOutOffset, reloadDy);
                         ...
                         const int64_t outGmOffset, const int32_t blkCntOffset, const int32_t mmPingPongIdx, const RunInfo &runInfo)
{
    ...
    cube5ProcessMLoop(pGmOffset, dyGmOffset, blkCntOffset, mm5ResOutBaseOffset, reloadDy);
}
