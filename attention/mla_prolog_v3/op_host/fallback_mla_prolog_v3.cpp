...
graphStatus GetMlaPrologV3Attr(const OpExecuteContext *ctx, MlaPrologV3FallBackParam &param)
{
    ...
    const int *getWeightQuantMode = ctx->GetAttrs()->GetAttrPointer<int64_t>(ATTR_WEIGHT_QUANT_MODE_INDEX);
    const int *getKvCacheQuantMode = ctx->GetAttrs()->GetAttrPointer<int64_t>(ATTR_KV_CACHE_QUANT_MODE_INDEX);
    const int *getQueryQuantMode = ctx->GetAttrs()->GetAttrPointer<int64_t>(ATTR_QUERY_QUANT_MODE_INDEX);
    const int *getCkvkrRepoMode = ctx->GetAttrs()->GetAttrPointer<int64_t>(ATTR_CKVKR_REPO_MODE_INDEX);
    const int *getQuantScaleRepoMode = ctx->GetAttrs()->GetAttrPointer<int64_t>(ATTR_QUANT_SCALE_REPO_MODE_INDEX);
    const int *getTileSize = ctx->GetAttrs()->GetAttrPointer<int64_t>(ATTR_TILE_SIZE_INDEX);
