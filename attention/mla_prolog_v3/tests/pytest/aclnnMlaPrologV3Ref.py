...
def dynamic_quant_without_smooth_scale(inputs, out_deqq_shape_shape, dtype_max=INT8_DTYPE_MAX,
...
def get_param(torch_tensor_list, params):
    ...
    if res['flaglist'][23]:
        ...
        res["out_deq_qnorm_dtype"] = trans_input_dtype(params['dtype_output'][output_opt_idx])
        output_opt_idx += 1
...
def cal_mlaprolog(mla_param):
    ...
    out_qnorm = None
    out_deq_qnorm = None
    full_quant_mode_config = _get_full_quant_mode_config(mla_param["weight_quant_mode"])
    mode2_family_full_quant = mla_param["weight_quant_mode"] in (
    ...
    # matmul1后处理
    if mode2_family_full_quant: # x_dtype == 'int8':
        if mla_param["device"] == "gpu":
    ...
    elif mla_param["weight_quant_mode"] in (
            WEIGHT_QUANT_MODE_FULL_FP8_E4M3,
            WEIGHT_QUANT_MODE_FULL_HIF8):
    ...
    if enable_quant_output:
        if mla_param["weight_quant_mode"] == WEIGHT_QUANT_MODE_MXFP8_FULL:
            ...
            out1, deq_scale_q_nope = dynamic_quant_without_smooth_scale(
                ...
                dtype_max=full_quant_mode_config["dtype_max"],
                quant_dtype=full_quant_mode_config["query_quant_dtype"],
            )
        else:
            out1 = out1.to(torch.bfloat16).to(torch.float32)
    ...
    if kv_dtype in ["int8", "float8_e4m3fn", "hifloat8"]:
        scatter_size = 32
    elif kv_dtype == "bf16":
        scatter_size = 16
    ...
    if mla_param['ckvkr_repo_mode'] == 1:
        ...
        if kr_dtype in ["int8", "float8_e4m3fn", "hifloat8"]:
            scatter_size = 32
        elif kr_dtype == "bf16":
            scatter_size = 16
...
def aclnn_op_func_cpu(torch_tensor_list, params):
    ...
    out3 = mla_param["kv_cache_tensor"].clone() if isinstance(mla_param["kv_cache_tensor"], torch.Tensor) \
        else torch.from_numpy(np.asarray(mla_param["kv_cache_tensor"]))
    out4 = mla_param["kr_cache_tensor"].clone() if isinstance(mla_param["kr_cache_tensor"], torch.Tensor) \
        else torch.from_numpy(np.asarray(mla_param["kr_cache_tensor"]))
