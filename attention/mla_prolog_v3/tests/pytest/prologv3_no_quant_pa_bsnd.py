...
class GeneralizedPrologV3:
    ...
    def forward(self, vars_list):
        ...
        (token_x, w_dq, w_uq_qr, w_uk, w_dkv_kr, gamma_cq, gamma_ckv, sin, cos,
         index_table, kv_cache, kr_cache) = vars_list
        ...
        x_shape = "(B*S1,He)"
        print(
            f"[INFO]matmul1 start. token_x{x_shape}:{tuple(token_x.shape)}|{token_x.dtype}"
            f" w_dq(He,Hcq):{tuple(w_dq.shape)}|{w_dq.dtype} matmul1_dtype:{matmul1_dtype}")
        ...
        gamma1 = gamma_cq
        print(
            f"[INFO]rmsnorm1 start. matmul1_res{matmul1_res_shape}:{tuple(matmul1_res.shape)}|{matmul1_res.dtype}"
            f" gamma_cq(Hcq):{tuple(gamma1.shape)}|{gamma1.dtype}")
        ...
        # matmul2计算
        print(
            f"[INFO]matmul2 start. norm1_res{matmul1_res_shape}:{tuple(norm1_res.shape)}|{norm1_res.dtype}"
            f" w_uq_qr(Hcq,N*(D+Dr)):{tuple(w_uq_qr.shape)}|{w_uq_qr.dtype} matmul2_dtype:{matmul2_dtype}")
        ...
        print(
            f"[INFO]splitD1 end. splitd1_res1{splitd1_res1_shape}:{tuple(splitd1_res1.shape)}"
            f" splitd1_res2{splitd1_res2_shape}:{tuple(splitd1_res2.shape)}")
        ...
        # matmul3计算
        print(
            f"[INFO]matmul3 start. splitd1_res1(B,S1,N,D):{tuple(splitd1_res1.shape)}|{splitd1_res1.dtype}"
            f" w_uk(N,D,Hckv):{tuple(w_uk.shape)}|{w_uk.dtype} matmul3_dtype:{matmul3_dtype}")
        ...
        cos_shape = "(B*S1,Dr)"
        print(
            f"[INFO]rotary1 start. splitd1_res2{splitd1_res2_shape}:{tuple(splitd1_res2.shape)}|{splitd1_res2.dtype}"
            f" cos{cos_shape}:{tuple(cos.shape)}|{cos.dtype}")
        ...
        # matmul4计算
        print(
            f"[INFO]matmul4 start. token_x{x_shape}:{tuple(token_x.shape)}|{token_x.dtype}"
            f" w_kv_kr(He,Hckv+Dr):{tuple(w_kv_kr.shape)}|{w_kv_kr.dtype} matmul4_dtype:{matmul4_dtype}")
        ...
        print(
            f"[INFO]splitD2 end. splitd2_res1{splitd2_res1_shape}:{tuple(splitd2_res1.shape)},"
            f" splitd2_res2{splitd2_res2_shape}:{tuple(splitd2_res2.shape)}")
        ...
        gamma2 = gamma_ckv
        print(
            f"[INFO]rmsnorm2 start. splitd2_res1{splitd2_res1_shape}:{tuple(splitd2_res1.shape)}|{splitd2_res1.dtype}"
            f" gamma_ckv(Hckv):{tuple(gamma2.shape)}|{gamma2.dtype}")
        ...
        # -------------------------------------------------------------------------------------------------------
        # scatter1 : norm2_res(B*S1,Hckv) * kv_cache -> out3
        # -------------------------------------------------------------------------------------------------------
        ...
        print(
            f"[INFO]scatter1 start. norm2_res{splitd2_res1_shape}:{tuple(norm2_res.shape)}|{norm2_res.dtype}"
            f" kv_cache{out3_info}:{tuple(kv_cache.shape)}|{kv_cache.dtype} scatter_size:{scatter_size}")
        ...
        # -------------------------------------------------------------------------------------
        print(
            f"[INFO]rotary2 start. splitd2_res2{splitd2_res2_shape}:{tuple(splitd2_res2.shape)}|{splitd2_res2.dtype}"
            f" cos{cos_shape}:{tuple(cos.shape)}|{cos.dtype}")
        ...
        # ----------------------------------------------------------------------------------------------
        # scatter2 : rotary2_res(B*S1,Dr) * kr_cache -> out4
        # ----------------------------------------------------------------------------------------------
        ...
        print(
            f"[INFO]scatter2 start. rotary2_res{splitd2_res2_shape}:{tuple(rotary2_res.shape)}|{rotary2_res.dtype}"
            f" kr_cache{out4_info}:{tuple(kr_cache.shape)}|{kr_cache.dtype} scatter_size:{scatter_size}")
        if cache_mode == "PA_BSND":
            B_num = out4_shape[0]
...
def test_prologv3_no_quant(params):
    ...
    w_uq_qr = torch.rand(Hcq, q_head_num * (head_dim + rope_head_dim),
                          dtype=torch.bfloat16, generator=generator).npu()
    ...
    kv_cache = torch.rand(block_num, block_size, kv_head_num, Hckv,
                          dtype=torch.bfloat16, generator=generator).npu()
    kr_cache = torch.rand(block_num, block_size, kv_head_num, rope_head_dim,
                          dtype=torch.bfloat16, generator=generator).npu()

    # 完成cpu侧逻辑及调用
    vars_list = [token_x, w_dq, w_uq_qr, w_uk, w_dkv_kr, rmsnorm_gamma_cq,
                 rmsnorm_gamma_ckv, rope_sin, rope_cos, cache_index, kv_cache, kr_cache]
    test_prologv3 = GeneralizedPrologV3(params)
    expect = test_prologv3.forward(vars_list)
    ...
    result = torch_npu.npu_mla_prolog_v3(
        ...
        rope_sin, rope_cos, kv_cache, kr_cache,
