...
- **参数说明**
  ...
  | queryNormFlag     | 输入      | 表示是否输出queryNormOutOptional、dequantScaleQNormOutOptional，Host侧参数。  | - false表示不输出，true表示输出，默认值为false | BOOL  | BOOL| -- | --    |-   |
  ...
  | ckvkrRepoMode     | 输入      | 表示kvCache和krCache的存储模式，Host侧参数。  | - 0表示kvCache和krCache分别存储，1表示kvCache和krCache合并存储，默认值为0| INT64  | INT64 | -- | --    |-   |
