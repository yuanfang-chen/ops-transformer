...
- 输入query、key、value的
  ...
  - inputLayout必须一致。
- 关于数据shape的约束：
  - 当前只支持全计算，输入inputLayout仅支持BSND，shape[B, S, N, D]仅支持[1, 54000, 10, 128]、[1, 9360, 80, 128]两组取值。
  - 不支持任何可选输入。
...
int main() {
  ...
  // 调用aclnnQuantFlashAttentionScore第一段接口
    ret = aclnnQuantFlashAttentionScoreGetWorkspaceSize(
            q, k, v, dScaleQ, dScaleK, dScaleV, attenmask, scaleValue, preTokens, nextTokens, headNum, layOut, sparseMode, pScale,
