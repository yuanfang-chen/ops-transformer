...
static Status ParseOpToGraphMultiHeadAttention(const ge::Operator& op, ge::Graph& graph) {
  ...
  auto AttentionScore = ge::op::FlashAttentionScore((ori_name + "_FlashAttentionScore").c_str())
      ...
      .set_attr_keep_prob(keep_prob).set_attr_pre_tockens(pre_tockens)
      .set_attr_next_tockens(next_tockens).set_attr_inner_precise(inner_precise)
      .set_attr_sparse_mode(sparse_mode).set_attr_head_num(head_num);
