...
protected:
    ...
    uint64_t GetTilingKey() const override
    {
        ...
        OP_LOGD(opName, "TND TilingKey info is implMode:%d, s1TemplateType:%d, s2TemplateType:%d, dTemplateType:%d,"
            "pseMode:%d, hasAttenMask:%d, hasDropOut:%d, hasRope:%d, regbase:%d, optionalDn:%d", static_cast<uint8_t>(implMode),
    ...
    {
        if (hasRope || (hasAttenMask && attenMaskCompressMode != static_cast<uint8_t>(AttenMaskCompressMode::LEFT_UP_CAUSAL_MODE)
            && attenMaskCompressMode != static_cast<uint8_t>(AttenMaskCompressMode::RIGHT_DOWN_CAUSAL_MODE)) ||
            (hasPse && pseType != static_cast<int64_t>(PseType::PSE_INNER_MUL_ADD_TYPE) &&
            pseType !=  static_cast<int64_t>(PseType::PSE_INNER_MUL_ADD_SQRT_TYPE)) ||
            (hasDropOut && dropMaskOuter) || inputDtypeBytes == DATA_TYPE_FP32 || dTemplateType > DTemplateType::ALIGNED_256) {
            return;
        }
        optionalDn = true;
    }
