...
constexpr static uint32_t MLA_NEGATIVE_MIN_VALUE_FP32 = 0xFF7FFFFF;
constexpr static uint32_t MLA_NEGATIVE_MIN_VALUE_FP16 = 0xFBFF;
...
template <typename T> __aicore__ inline void GetExtremeValue(T &negativeScalar, T &positiveScalar)
{
    if constexpr (IsSameType<T, float>::value) {
        uint32_t tmp1 = MLA_NEGATIVE_MIN_VALUE_FP32;
        uint32_t tmp2 = MLA_POSITIVE_MAX_VALUE_FP32;
        negativeScalar = *((float *)&tmp1);
        positiveScalar = *((float *)&tmp2);
    } else {
        uint16_t tmp1 = MLA_NEGATIVE_MIN_VALUE_FP16;
