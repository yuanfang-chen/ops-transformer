...
void CopyTmpResult(FAMetaData &tmpRes, FAMetaData &splitRes)
{
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        splitRes.headFdDataIdx[i] = tmpRes.headFdDataIdx[i];
        splitRes.fdRes.fdBalanceMSplitNum[i] = tmpRes.fdRes.fdBalanceMSplitNum[i];
        splitRes.fdRes.fdBalanceMTailSize[i] = tmpRes.fdRes.fdBalanceMTailSize[i];
    }
}
...
void ClearTmpResult(FAMetaData &tmpResult)
{
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        tmpResult.headFdDataIdx[i] = 0U;
        tmpResult.fdRes.fdBalanceMSplitNum[i] = 0U;
        tmpResult.fdRes.fdBalanceMTailSize[i] = 0U;
    }
}
...
void RecordFDInfo(const SplitContext &splitContext, const AssignContext &assignContext, FAMetaData &result)
{
    ...
    result.fdRes.fdBalanceMSplitNum[result.fdRes.fdNum] = curFdS1gSplitPart;
    result.fdRes.fdBalanceMTailSize[result.fdRes.fdNum] = curFdS1gLastPartSize;
    result.fdRes.fdNum++;
}
...
void CalcSplitPlan(uint32_t coreNum, int64_t costLimit, const SplitContext &splitContext, FAMetaData &result)
{
    ...
    for (uint32_t i = 0; i < coreNum; ++i) {
        ...
        result.headFdDataIdx[assignContext.curCoreIdx] = assignContext.curKvSplitPart - 1U;
...
void SplitFD(FAMetaData &result)
{
    ...
    for (uint32_t i = 0; i < result.fdRes.fdNum; i++) {
        totalFDLoad += result.fdRes.fdS2SplitNum[i] * result.fdRes.fdBalanceMSplitNum[i];
        totalFDHeadSplit += result.fdRes.fdBalanceMSplitNum[i];
    }
    ...
    for (uint32_t i = 0; i < result.fdRes.fdNum; i++) {
        uint32_t fDKVSplitNum = result.fdRes.fdS2SplitNum[i];
        for (uint32_t gS1SplitIdx = 0; gS1SplitIdx < result.fdRes.fdBalanceMSplitNum[i]; gS1SplitIdx++) {
            ...
            if (fDKVSplitNum > remainSpace * FD_TOLERANCE_RATIO) {
                result.fdRes.fdBalanceEndIdx1[curCoreIndex] = preTmpFDIndexEndOfFdHead;
                result.fdRes.fdBalanceEndIdx2[curCoreIndex] = preTmpFDIndexEndOfFdHeadSplit;
                curCoreIndex += 1U;
    ...
    result.fdRes.fdBalanceEndIdx1[curCoreIndex] = preTmpFDIndexEndOfFdHead;
    result.fdRes.fdBalanceEndIdx2[curCoreIndex] = preTmpFDIndexEndOfFdHeadSplit;
    result.fdRes.fdUsedVecNum = curCoreIndex + 1;
}
