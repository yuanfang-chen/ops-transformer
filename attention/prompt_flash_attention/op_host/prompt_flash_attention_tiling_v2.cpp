                                                       ...
                                                       const PFAShapeInfo &valueShapeInfo) const
{
    ...
    bool supportLayout = layoutString == "BSH" || layoutString == "BSND" || layoutString == "BNSD" || layoutString == "BNSD_BSND";
    if (quantScale2Dim == 1) {
        if(quantScale2ShapeSize == quantScale2ShapeSizePerChannel && supportLayout){
            //supportLayout + post quant perChannel场景 shape 乘积为qN*vD即可
            return true;
        }else{
