...
bool PromptFlashAttentionTilingV2::CheckPATypeAndShape(ContextParamsForPFATiling& contextKeyParams,
    ...
    if (enableIFAMLAFullQuant) {
        ...
        OP_CHECK_IF(!(queryShapeInfo.s > 1 && (queryShapeInfo.d != 64 && queryShapeInfo.d != 128)) && (*blockSize % BLOCK_SIZE_BASE != 0 || *blockSize < BLOCK_SIZE_BASE || *blockSize > BLOCK_SIZE_MAX),
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                "block size(%d) should be a multiple of %d, and should be in range of [%d, %d] when PA enable with condition: q_s is greater than 1 and query/key/value d(%d) is not 64 or 128",
