...
#define PRINTF_NAME(x) printf(#x);printf(" is %d\n", x);
...
void PromptFlashAttentionTilingV2::InferSplitCoreMode(PromptFlashAttentionTilingData& tilingData) 
{
    uint32_t thresholdS2Size = 1024;
    // thres = L2CacheSize / (bnSize * (D + Dv) * TypeSize * 2);  // 乘2表示K、V
    uint32_t s1 = maxActualseqQ;
    uint32_t S1 = tilingData.promptAttentionBaseParams.get_seqSize();
    uint32_t s2 = maxActualseqKV;

    uint32_t batchSize = tilingData.promptAttentionBaseParams.get_batchSize();
    uint32_t numHead = tilingData.promptAttentionBaseParams.get_headNumSize();
    int64_t bnSize = std::min(batchSize * numHead, aicNum);
    uint32_t sparseMode = tilingData.promptAttentionBaseParams.get_sparseMode();


    if((S2 >= thresholdS2Size && !isKVHasPrefix && !enableLeftPadding && !enableAlibiPse &&
        !enableKVAntiquant && !enablePostQuant && !enablePertensorQuant && !enablePerblockQuant && !enableIFA && !enableIFAMLA) &&
      ((sparseMode == SPARSE_MODE_LEFT_UP) ||
      ((sparseMode == SPARSE_MODE_RIGHT_DOWN) ||
       (sparseMode == SPARSE_MODE_ALL_MASK) || 
       (sparseMode == SPARSE_MODE_NO_MASK && !enableMask)))) {
        splitCoreMode = SplitCoreMode::SPLIT_S1OUT_CUBE;
        printf("zzyzzy SPLIT_S1OUT_CUBE -------------------------------------------------------------------------- \n");
    } else {
    ...
    printf("zzyzzy s1:%d, S1:%d, s2:%d, S2:%d, maxActualseqKV:%d, thresholdS2Size:%d, splitCoreMode:%d\n", s1, S1, s2, S2, maxActualseqKV, thresholdS2Size, splitCoreMode);
}
...
void PromptFlashAttentionTilingV2::PromptFlashAttentionSplitOutSeq(PromptFlashAttentionTilingData& tilingData,
    std::vector<int64_t>& actualSeqLengths, std::vector<int64_t>& actualSeqLengthsKV, bool isAttenMaskUsed) 
{
    printf("zzyzzy PromptFlashAttentionSplitOutSeq ===================\n");
    PromptAttentionBaseParams* baseParams = &tilingData.promptAttentionBaseParams;
    PromptAttentionSingleCoreParams* singleCoreParams = &tilingData.promptAttentionSingleCoreParams;
    uint32_t curCoreNum = coreNum;
    uint32_t batchSize = baseParams->get_dimNumOfseq();
    ...
    for (uint32_t sIdx = 0; sIdx < batchSize; sIdx++) {
        ...
        totalBlockNumsOneHead += GetCalcBlockNumsOneHead(actualSeqLengthsTmp, sInnerSize, sOuterSize,
            sInnerSize, preTokensLeftUp, nextTokensLeftUp, true);


        int64_t outerBlockNums = (actualSeqLengthsTmp + static_cast<int64_t>(sOuterSize) - 1) /
            static_cast<int64_t>(sOuterSize);
        totalSize += outerBlockNums * headNumSize;
        printf("totalSize:%d, headNumSize:%d, outerBlockNums:%d, actualSeqLengthsTmp:%d, sOuterSize:%d, nextTokensLeftUp\n", totalSize, headNumSize, outerBlockNums, actualSeqLengthsTmp, sOuterSize, nextTokensLeftUp);
    }
    singleCoreParams->set_multiSmaxsInnerLoopTimes(multiSmaxsInnerLoopTimes);
    PRINTF_NAME(multiSmaxsInnerLoopTimes)
    PRINTF_NAME(totalBlockNumsOneHead)

    // Amount of computation per core
    int64_t s1OuterSize = (baseParams->get_seqSize() + sOuterSize - 1) / sOuterSize;
    faTilingAdapter.multiCoreParamsRegbase.set_s1OuterSize(s1OuterSize);
    PRINTF_NAME(s1OuterSize)
    PRINTF_NAME(sOuterSize)


    uint32_t actualUsedCoreNum = std::min(totalBlockNumsOneHead, static_cast<int64_t>(curCoreNum));
    // actualUsedCoreNum = 1;
    singleCoreParams->set_actualCoreNums(actualUsedCoreNum);
    int64_t sinnerBlocknum = (baseParams->get_seqInnerSize() + sInnerSize - 1) / sInnerSize;
    // int64_t totalSize = batchSize * s1OuterSize * headNumSize;
    // int64_t totalSize = totalBlockNumsOneHead * headNumSize;
    PRINTF_NAME(totalBlockNumsOneHead * headNumSize)
    PRINTF_NAME(totalSize);

    uint64_t firstFullLoadS1OuterIdx = - 1;
    uint32_t sparseMode = tilingData.promptAttentionBaseParams.get_sparseMode();
    if (sparseMode == SPARSE_MODE_LEFT_UP) {
        firstFullLoadS1OuterIdx = (std::min(maxActualseqQ, maxActualseqKV) + sOuterSize -1 ) / sOuterSize - 1;
    } else if (sparseMode == SPARSE_MODE_RIGHT_DOWN) {
        firstFullLoadS1OuterIdx = s1OuterSize - 1;
        uint32_t s1SparseValidSize = baseParams->get_seqSize();
        uint32_t s2SparseValidSize = std::min(AlignUp(static_cast<int32_t>(S2 - baseParams->get_seqSize()), static_cast<int32_t>(sOuterSize)), static_cast<int32_t>(S2));
        baseParams->set_s1SparseValidSize(s1SparseValidSize);
        baseParams->set_s2SparseValidSize(s2SparseValidSize);
        printf("maxActualseqQ:%d, maxActualseqKV:%d, sOuterSize:%d\n", maxActualseqQ, maxActualseqKV, sOuterSize);
        printf("s1SparseValidSize:%d, s2SparseValidSize:%d\n", s1SparseValidSize, s2SparseValidSize);
    } else if (sparseMode == SPARSE_MODE_NO_MASK || sparseMode == SPARSE_MODE_ALL_MASK) {
        firstFullLoadS1OuterIdx = -1;
    }
    printf("sparseMode:%d\n", sparseMode);

    PRINTF_NAME(firstFullLoadS1OuterIdx);
    PRINTF_NAME(actualUsedCoreNum);
    PRINTF_NAME(sinnerBlocknum);
    SetMultiCoreParamsRegbase(totalSize, actualUsedCoreNum);
    faTilingAdapter.multiCoreParamsRegbase.set_firstFullLoadS1OuterIdx(firstFullLoadS1OuterIdx);
    PRINTF_NAME(faTilingAdapter.multiCoreParamsRegbase.get_firstFullLoadS1OuterIdx());
}
    ...
    PromptFlashAttentionTilingData& tilingData) 
{
    ...
    if (splitCoreMode == SplitCoreMode::SPLIT_S1OUT_CUBE) {
        PromptFlashAttentionSplitOutSeq(tilingData, actualSeqLengths, actualSeqLengthsKV, isAttenMaskUsed);
...
void PromptFlashAttentionTilingV2::PFATilingDataconvert(PromptFlashAttentionTilingData& tilingData) 
{
    ...
    inputParams.set_s1SparseValidSize(tilingData.promptAttentionBaseParams.get_s1SparseValidSize());
    inputParams.set_s2SparseValidSize(tilingData.promptAttentionBaseParams.get_s2SparseValidSize());
