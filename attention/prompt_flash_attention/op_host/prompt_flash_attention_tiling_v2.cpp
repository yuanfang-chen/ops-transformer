...
bool PromptFlashAttentionTilingV2::CheckPerTensorQuantParams(const ContextParamsForPFATiling& contextKeyParams,
    ...
    OP_CHECK_IF((queryQuantMode == nullptr || keyAntiquantMode == nullptr || valueAntiquantMode == nullptr),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
            "queryQuantMode, keyAntiquantMode or valueAntiquantMode is nullptr in per-tensor quant scenario."),
            return false);
    OP_CHECK_IF((*queryQuantMode != static_cast<int64_t>(AntiquantTypeEnum::PER_CHANNEL) ||
        *keyAntiquantMode != static_cast<int64_t>(AntiquantTypeEnum::PER_CHANNEL)||
        *valueAntiquantMode != static_cast<int64_t>(AntiquantTypeEnum::PER_CHANNEL)),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
            "queryQuantMode(%ld), keyAntiquantMode(%ld) or valueQuantMode(%ld) is not supported "
            "in per-tensor quant scenario.! Only support Per-Tensor(0).",
            *queryQuantMode, *keyAntiquantMode, *valueAntiquantMode), return false);
