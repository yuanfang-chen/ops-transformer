...
#ifndef SPLIT_CORE_35_H
#define SPLIT_CORE_35_H
...
struct FlashDecodeResult {
    ...
    std::vector<uint32_t> fdBalanceEndIdx2 {};   // FD负载均衡阶段，每个vector的二级索引，脚标为vector ID，值为归约任务的m轴切分ID
    // 3、每个core处理的第1个归约任务的数据应存放的workspace位置
    FlashDecodeResult(uint32_t coreNum, uint32_t vecCubeRatio) :
        fdBN2Idx(coreNum),
        fdMIdx(coreNum),
        fdS2SplitNum(coreNum),
        fdBalanceMSplitNum(coreNum),
        fdBalanceMTailSize(coreNum),
        fdBalanceEndIdx1(coreNum * vecCubeRatio),
        fdBalanceEndIdx2(coreNum * vecCubeRatio){}
};
