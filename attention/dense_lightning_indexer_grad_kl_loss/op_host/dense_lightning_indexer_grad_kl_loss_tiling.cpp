...
int64_t DenseLightningIndexerGradKLLossTilingBase::FindBIndex(int64_t bIndex, int64_t curIndex, int64_t &accumulateLen)
{
    ...
    return dliGradkllossMultiCoreParams_->get_totalSize() >= curIndex ? bSize : -1;
}

void DenseLightningIndexerGradKLLossTilingBase::CalcMultiCoreOffset(int64_t &bStartIdx, int64_t &s1StartIdx,
                                                                    int64_t &bEndIdx, int64_t &s1EndIdx)
{
    int64_t actualSum = 0;
    int64_t aicIdx = 0;

    int64_t bS1Index = dliGradkllossMultiCoreParams_->get_bS1Ptr()[aicIdx];
    int64_t bS1EndIndex = aicIdx + 1 < optiling::MAX_CORE_NUM ?
            dliGradkllossMultiCoreParams_->get_bS1Ptr()[aicIdx + 1] : dliGradkllossMultiCoreParams_->get_totalSize();
    if (tilingKeyLayout == LayoutType::LAYOUT_TND) {
...
void DenseLightningIndexerGradKLLossTilingBase::CalcMaxLoop()
{
    ...
    dliGradkllossBaseParams_->set_maxLoopSize(taskId);
    OP_LOGD(context_->GetNodeName(), "CalcMaxLoop maxLoopSize = %d.", taskId);
}
