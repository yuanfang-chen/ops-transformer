...
public:
    ...
    __aicore__ inline void InitCommonGlobalBuffer(
        ...
        __gm__ uint8_t *deqScaleV, __gm__ uint8_t *postQuantScale, __gm__ uint8_t *prefix, __gm__ uint8_t *attenMask,
    ...
    __gm__ uint8_t *learnableSink, __gm__ uint8_t *&workspace, ConstInfo<isInfer, hasRope> &constInfo)
{
    if ASCEND_IS_AIV {
        ...
        if constexpr (isFp8) {
            ...
            pScaleGm.SetGlobalBuffer((__gm__ float *)postQuantScale);
        }
    ...
    ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    constInfo.pScale = this->pScale.GetValue(0);
...
{
    if constexpr (IsSameType<T, float>::value && !isFp8) {
