    ...
    const ConstInfo<isInfer, hasRope> &constInfo, int32_t bIdx, __gm__ int64_t *actualSeqKvlenAddr)
{
    ...
    if constexpr (layout == LayOutTypeEnum::LAYOUT_BSH) {
        ...
        if constexpr (isPa) {
            ...
            actualSeqKVLen = (bIdx == 0) ? actualSeqKvlenAddr[0] : actualSeqKvlenAddr[bIdx] -
                actualSeqKvlenAddr[bIdx - 1];
            valueInnerOffsetSize = (bIdx == 0) ? 0 : actualSeqKvlenAddr[bIdx - 1] * constInfo.dSizeV;
        }
        runParam.valueCoreOffset = valueInnerOffsetSize + runParam.n2oIdx * constInfo.bSize * actualSeqKVLen *
            constInfo.dSizeV;
    } else {
