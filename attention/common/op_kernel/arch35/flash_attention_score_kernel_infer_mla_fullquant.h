    ...
    for (uint32_t bnIdx = bnStartIdx; bnIdx < bnEndIdx; ++bnIdx) {
        ...
        for (int64_t gS1Index = gS1StartIdx; gS1Index < tempGS1End; ++gS1Index) {
            ...
            for (int64_t s2LoopCount = 0; s2LoopCount <= s2LoopLimit; ++s2LoopCount) {
                // ==================== Pipeline Stage 0: BMM1 (Q × K^T) ====================
                // AIC核执行: 计算attention scores
                // if (notLastThreeLoop): 确保不是流水线收尾阶段
                if (notLastThreeLoop) {
                ...
                // ==================== Pipeline Stage 1: Vec1 (Softmax) ====================
                // AIV核执行: 对BMM1结果做softmax归一化
                // if (taskId > 0): 确保有前序BMM1结果
                // if (notLastTwoLoop): 确保不是倒数第二次GS1迭代
                ...
                // ==================== Pipeline Stage 2: BMM2 (P × V) ====================
                // AIC核执行: 计算最终输出
                // if (taskId > 1): 确保有前序Vec1结果
                // if (notLast): 确保不是最后一次GS1迭代
                if (taskId > 1 && notLast) {
                    if ASCEND_IS_AIC {
                ...
                // ==================== Pipeline Stage 3: Vec2 (Post-Quant) ====================
                // AIV核执行: 对BMM2结果做后量化并输出
                // if (taskId > 2): 确保有前序BMM2结果
                if (taskId > 2) {
                    if ASCEND_IS_AIV {
