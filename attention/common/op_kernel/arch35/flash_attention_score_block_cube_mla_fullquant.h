 ...
 * 
 * Traits模式: 在编译期提取类的类型信息和常量
 * 允许Kernel层通过统一的接口访问CubeBlock的模板参数
 */
template <typename T>
 ...
 * 用于提取CubeBlock的类型别名
 * 
 * 例如: GEN_TRAIT_TYPE(INPUT_T, ...) → using INPUT_T_TRAITS = INPUT_T;
 */
 ...
 * 用于提取CubeBlock的编译期常量
 * 
 * 例如: GEN_TRAIT_CONST(s1BaseSize, uint32_t, ...) → static constexpr uint32_t s1BaseSizeTraits = s1BaseSize;
 */
 ...
 * 
 * 使用宏展开生成Traits结构体，实现编译期反射:
 * 
 * 1. CUBE_BLOCK_TRAITS_TYPE_FIELDS(GEN_TRAIT_TYPE)
 *    - 展开为: using INPUT_T_TRAITS = INPUT_T;
 *              using OUTPUT_T_TRAITS = OUTPUT_T;
 *              using T_TRAITS = T;
 *              ...
 * 
 * 2. CUBE_BLOCK_TRAITS_CONST_FIELDS(GEN_TRAIT_CONST)
 *    - 展开为: static constexpr uint32_t s1BaseSizeTraits = s1BaseSize;
 *              static constexpr uint32_t s2BaseSizeTraits = s2BaseSize;
 *              static constexpr bool isFp8Traits = isFp8;
 *              ...
 * 
 * 使用示例:
 *   CubeBlockTraits<FABlockCubeMlaFullquant<ARGS>>::s1BaseSizeTraits
 *   // 等价于访问 FABlockCubeMlaFullquant<ARGS>::s1BaseSize
 * 
