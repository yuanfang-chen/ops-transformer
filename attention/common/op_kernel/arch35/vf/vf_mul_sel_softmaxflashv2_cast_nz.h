    ...
    if constexpr (useNz) {
        ...
        if constexpr (oriNRange == GT_128_AND_LTE_256) {
            ...
            ProcessVec1NoUpdateImpl128<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode,
                ...
                dstTensor, indexesTensor, expSumTensor, maxTensor, srcTensor, expMaxTensor, inExpSumTensor, inMaxTensor, maskTensor,
                pseTensor, dropTensor, sharedTmpBuffer, m, originN, pseStride, slopes, posShift, scale, dScaleQK, minValue, keepProb,
                queryScaleUb, deSCaleKValue, sinkValue);
    ...
    if constexpr (useNz) {
        ...
        if constexpr (oriNRange == GT_128_AND_LTE_256) {
            ...
            ProcessVec1UpdateGeneralImpl128<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode,
                ...
                scale, dScaleQK, minValue, keepProb, queryScaleUb, deSCaleKValue, sinkValue);
        }
    }
}
