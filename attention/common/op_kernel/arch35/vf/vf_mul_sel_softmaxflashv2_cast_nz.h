...
__aicore__ inline void ProcessVec1NoUpdate(
    ...
    const float pScale = 1.0f, const bool hasSink = false, const float sinkValue = 0)
{
    if constexpr (useNz) {
...
__aicore__ inline void ProcessVec1Update(
    ...
    const LocalTensor<T>& queryScaleUb = LocalTensor<T>(), const float deSCaleKValue = 1.0f, const float pScale = 1.0f, const bool hasSink = false, const float sinkValue = 0)
{
    if constexpr (useNz) {
        if constexpr (oriNRange == GT_256_AND_LTE_512) {
            LocalTensor<uint8_t> indexesTensor;
