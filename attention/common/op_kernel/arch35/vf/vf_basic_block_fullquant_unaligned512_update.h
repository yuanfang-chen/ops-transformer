...
__simd_vf__ void ProcessVec1UpdateGeneralImpl512GqaFullquantVF(
    ...
    const uint16_t m, const uint32_t n, const T minValue)
{
    ...
    RegTensor<T2> vreg_exp_0_1;
    RegTensor<T2> vreg_exp_2_1;
    RegTensor<T2> vreg_exp_1_1;
    RegTensor<T2> vreg_exp_3_1;
    RegTensor<T2> vreg_exp_0_2;
    RegTensor<T2> vreg_exp_2_2;
    RegTensor<T2> vreg_exp_1_2;
    RegTensor<T2> vreg_exp_3_2;

    RegTensor<T2> vreg_exp_merge_tmp_1_1;  // 前8行
    RegTensor<T2> vreg_exp_merge_tmp_1_2;
    RegTensor<T2> vreg_exp_merge_1;
    RegTensor<T2> vreg_exp_merge_tmp_2_1;  // 后8行
    RegTensor<T2> vreg_exp_merge_tmp_2_2;
    RegTensor<T2> vreg_exp_merge_2;

    RegTensor<uint8_t> vreg_exp_merge_indexes;
    ...
    for (uint16_t i = 0; i < 4; ++i) {
        ...
        // [TODO-ZD]:512不对齐的情况，是否使用preg_ori_tail_n,其他所有分支待改
        Select(vreg_input_x_1, vreg_input_x_1, vreg_min, preg_all);
        Select(vreg_input_x_unroll_1, vreg_input_x_unroll_1, vreg_min, preg_all);
        Select(vreg_input_x_2, vreg_input_x_2, vreg_min, preg_all);
        Select(vreg_input_x_unroll_2, vreg_input_x_unroll_2, vreg_min, preg_all);

        StoreAlign<half, MicroAPI::StoreDist::DIST_NORM_B16>(
            (__ubuf__ half *&)srcUb1 + i * 16 * 16, vreg_input_x_1, preg_all);
        StoreAlign<half, MicroAPI::StoreDist::DIST_NORM_B16>(
            (__ubuf__ half *&)srcUb1 + i * 16 * 16 + 8 * 16, vreg_input_x_unroll_1, preg_all);
        StoreAlign<half, MicroAPI::StoreDist::DIST_NORM_B16>(
            (__ubuf__ half *&)srcUb2 + i * 16 * 16, vreg_input_x_2, preg_all);
        StoreAlign<half, MicroAPI::StoreDist::DIST_NORM_B16>(
            (__ubuf__ half *&)srcUb2 + i * 16 * 16 + 8 * 16, vreg_input_x_unroll_2, preg_all);
        ...
        for (uint16_t j = 1; j < n / 16; ++j) {  // 每个256内部，第j个[64, 16]，n / 16为NZ shape的第一个维度
            ...
            StoreAlign<half, MicroAPI::StoreDist::DIST_NORM_B16>(
                (__ubuf__ half *&)srcUb1 + i * 16 * 16 + j * 64 * 16, vreg_input_x_1, preg_all);
            StoreAlign<half, MicroAPI::StoreDist::DIST_NORM_B16>(
                (__ubuf__ half *&)srcUb1 + i * 16 * 16 + j * 64 * 16 + 8 * 16, vreg_input_x_unroll_1, preg_all);
            StoreAlign<half, MicroAPI::StoreDist::DIST_NORM_B16>(
                (__ubuf__ half *&)srcUb2 + i * 16 * 16 + j * 64 * 16, vreg_input_x_2, preg_all);
            StoreAlign<half, MicroAPI::StoreDist::DIST_NORM_B16>(
                (__ubuf__ half *&)srcUb2 + i * 16 * 16 + j * 64 * 16 + 8 * 16, vreg_input_x_unroll_2, preg_all);
    ...
    for (uint16_t i = 0; i < 4; ++i) {
        ...
        for (uint16_t j = 0; j < n / 32; ++j) { 
            ...
            Cast<T2, float, castTraitZero>(vreg_exp_0_1, vreg_exp_0_1, preg_all);
            Cast<T2, float, castTraitTwo>(vreg_exp_2_1, vreg_exp_2_1, preg_all);
            Cast<T2, float, castTraitOne>(vreg_exp_1_1, vreg_exp_1_1, preg_all);
            Cast<T2, float, castTraitThree>(vreg_exp_3_1, vreg_exp_3_1, preg_all);

            Or((RegTensor<uint8_t>&)vreg_exp_merge_tmp_1_1, (RegTensor<uint8_t>&)vreg_exp_0_1, (RegTensor<uint8_t>&)vreg_exp_2_1, preg_all_b8);
            Or((RegTensor<uint8_t>&)vreg_exp_merge_tmp_1_2, (RegTensor<uint8_t>&)vreg_exp_1_1, (RegTensor<uint8_t>&)vreg_exp_3_1, preg_all_b8);
            Or((RegTensor<uint8_t>&)vreg_exp_merge_1, (RegTensor<uint8_t>&)vreg_exp_merge_tmp_1_1, (RegTensor<uint8_t>&)vreg_exp_merge_tmp_1_2, preg_all_b8);

            LoadAlign(vreg_exp_merge_indexes, indexesUb);
            Gather(vreg_exp_merge_1, vreg_exp_merge_1, vreg_exp_merge_indexes);
            StoreAlign(expUb1 + i * 16 * 32 + j * 64 * 32, vreg_exp_merge_1, preg_all_b8);

            // 16行中的后8行
            Cast<T2, float, castTraitZero>(vreg_exp_0_2, vreg_exp_0_2, preg_all);
            Cast<T2, float, castTraitTwo>(vreg_exp_2_2, vreg_exp_2_2, preg_all);
            Cast<T2, float, castTraitOne>(vreg_exp_1_2, vreg_exp_1_2, preg_all);
            Cast<T2, float, castTraitThree>(vreg_exp_3_2, vreg_exp_3_2, preg_all);

            Or((RegTensor<uint8_t>&)vreg_exp_merge_tmp_2_1, (RegTensor<uint8_t>&)vreg_exp_0_2, (RegTensor<uint8_t>&)vreg_exp_2_2, preg_all_b8);
            Or((RegTensor<uint8_t>&)vreg_exp_merge_tmp_2_2, (RegTensor<uint8_t>&)vreg_exp_1_2, (RegTensor<uint8_t>&)vreg_exp_3_2, preg_all_b8);
            Or((RegTensor<uint8_t>&)vreg_exp_merge_2, (RegTensor<uint8_t>&)vreg_exp_merge_tmp_2_1, (RegTensor<uint8_t>&)vreg_exp_merge_tmp_2_2, preg_all_b8);

            Gather(vreg_exp_merge_2, vreg_exp_merge_2, vreg_exp_merge_indexes);
            StoreAlign(expUb1 + i * 16 * 32 + j * 64 * 32 + 256, vreg_exp_merge_2, preg_all_b8);
        ...
        for (uint16_t j = 0; j < n / 32; ++j) { 
            ...
            LoadAlign(vreg_exp_merge_indexes, indexesUb);
            Gather(vreg_exp_merge_1, vreg_exp_merge_1, vreg_exp_merge_indexes);
            StoreAlign(expUb2 + i * 16 * 32 + j * 64 * 32, vreg_exp_merge_1, preg_all_b8);
            ...
            Gather(vreg_exp_merge_2, vreg_exp_merge_2, vreg_exp_merge_indexes);
            StoreAlign(expUb2 + i * 16 * 32 + j * 64 * 32 + 256, vreg_exp_merge_2, preg_all_b8);
        }
