    ...
    const float slopes, const float posShift, const T scale, const T minValue)
{
    ...
    RegTensor<float> vreg_sink_input1;
    RegTensor<float> vreg_sink_input2;
    RegTensor<float> vreg_sink_input3;
    RegTensor<float> vreg_sink_input4;
    // bfloat16_t
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        if constexpr (hasAtten == 1) {
            ...
            if constexpr (hasSink) {
                LoadAlign(vreg_sink_input1, sinkUb + i * s2BaseSize);
                LoadAlign(vreg_sink_input2, sinkUb + floatRepSize + i * s2BaseSize);
                LoadAlign(vreg_sink_input3, sinkUb + floatRepSize * 2 + i * s2BaseSize);
                LoadAlign(vreg_sink_input4, sinkUb + floatRepSize * 3 + i * s2BaseSize);
                Max(vreg_max_tmp1, vreg_sink_input1, vreg_sink_input2, preg_all);
                Max(vreg_max_tmp2, vreg_sink_input3, vreg_sink_input4, preg_all);
                Max(vreg_max_tmp3, vreg_max_tmp1, vreg_max_tmp2, preg_all);
                Reduce<MicroAPI::ReduceType::MAX, float, float, MicroAPI::MaskMergeMode::ZEROING>(
                    vreg_input_max, vreg_max_tmp3, preg_all);
            }
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        if constexpr (hasSink) {
            LoadAlign<T, MicroAPI::LoadDist::DIST_DINTLV_B32>(
                vreg_sink_input1, vreg_sink_input2, sinkUb + i * s2BaseSize);
            LoadAlign<T, MicroAPI::LoadDist::DIST_DINTLV_B32>(
                vreg_sink_input3, vreg_sink_input4, sinkUb + floatRepSize * 2 + i * s2BaseSize);
            ExpSub(vreg_sink_input1, vreg_sink_input1, vreg_max_brc, preg_all);
            ExpSub(vreg_sink_input2, vreg_sink_input2, vreg_max_brc, preg_all);
            ExpSub(vreg_sink_input3, vreg_sink_input3, vreg_max_brc, preg_all);
            ExpSub(vreg_sink_input4, vreg_sink_input4, vreg_max_brc, preg_all);
        }
        ...
        if constexpr (hasSink) {
            Add(vreg_sink_input1, vreg_sink_input2, preg_all);
            Add(vreg_sink_input3, vreg_sink_input4, preg_all);
            Add(vreg_sink_input1, vreg_sink_input3, preg_all);
            Reduce<MicroAPI::ReduceType::SUM, float, float, MicroAPI::MaskMergeMode::ZEROING>(
                vreg_exp_sum3, vreg_sink_input1, preg_all);
        }
