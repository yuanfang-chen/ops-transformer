...
__simd_vf__ void ProcessVec1UpdateImpl128VF(
    ...
    const float posShift, const T scale, const float dScaleQK, const T minValue, const float deSCaleKValue = 1.0f, const float sinkValue = 0.0f)
{
...
__aicore__ inline void ProcessVec1UpdateImpl128(
    ...
    const LocalTensor<T>& queryScaleUb = LocalTensor<T>(), const float deSCaleKValue = 1.0f, const float sinkValue = 0.0f)
{
    ...
    ProcessVec1UpdateImpl128VF <T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop, isMlaSgd, isMlaFullQuant, hasSink>(
