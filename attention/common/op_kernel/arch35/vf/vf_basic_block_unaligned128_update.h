...
template <typename T, typename T2, typename pseShiftType, uint32_t s1BaseSize = 128, uint32_t s2BaseSize = 128, bool hasAtten = 0,
    PseTypeEnum pseMode = PseTypeEnum::PSE_NONE_TYPE, bool hasDrop = 0, bool isMlaSgd = false, bool isMlaFullQuant = false, bool hasSink = false>
__simd_vf__ void ProcessVec1UpdateGeneralImpl128VF(
    ...
    const float slopes, const float posShift, const T scale, const float dScaleQK, const T minValue, const float deSCaleKValue = 1.0f, const float sinkValue = 0.0f)
{
...
__aicore__ inline void ProcessVec1UpdateGeneralImpl128(
    ...
    const T minValue, float keepProb, const LocalTensor<T>& queryScaleUb = LocalTensor<T>(), const float deSCaleKValue = 1.0f, const float sinkValue = 0.0f)
{
    ...
    ProcessVec1UpdateGeneralImpl128VF<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop, isMlaSgd, isMlaFullQuant, hasSink>(
