...
__simd_vf__ void ProcessVec1UpdateGeneralImpl128VF(
    ...
    const float slopes, const float posShift, const T scale, const float dScaleQK, const T minValue, const float deSCaleKValue = 1.0f, float sinkValue = 0.0f)
{
...
__aicore__ inline void ProcessVec1UpdateGeneralImpl128(
    ...
    const T minValue, float keepProb, const LocalTensor<T>& queryScaleUb = LocalTensor<T>(), const float deSCaleKValue = 1.0f, float sinkValue = 0.0f)
{
