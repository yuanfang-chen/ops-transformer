...
__simd_vf__ void ProcessVec1NoUpdateImpl128VF(
    ...
    const float dScaleQK, const T minValue, const float deSCaleKValue = 1.0f, float sinkValue = 0.0f)
{
    ...
    float sinkValue = 0.0f)
{
    ...
    ProcessVec1NoUpdateImpl128VF<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop, isMlaSgd, isMlaFullQuant, hasSink>(
        expUb, x_expUb, pseUb, expSumUb, maxUb, maxUbStart, srcUb, qScaleUb, indexesUb, maskUb, maskUbUnroll, dropMaskUb,
        divValue, blockStride, repeatStride, dScale, m, pseStride, slopes, posShift, scale, dScaleQK, minValue, deSCaleKValue, sinkValue);
}
} // namespace
