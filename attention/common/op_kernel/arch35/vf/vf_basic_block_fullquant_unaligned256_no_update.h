...
__simd_vf__ void ProcessVec1NoUpdateGeneralImpl256GqaFullquantVF(
    ...
    const uint16_t m, const uint32_t n, const T minValue, const uint32_t tailN, const int64_t tailNOffset, const T logPScale)
{
    ...
    Duplicate(vreg_p_scale, logPScale);
    ...
    for (uint16_t i = 0; i < 4; ++i) {
        ...
        Sub(vreg_max_tmp, vreg_max_tmp, vreg_p_scale, preg_all);
        Sub(vreg_max_tmp_unroll, vreg_max_tmp_unroll, vreg_p_scale, preg_all);
    ...
    const uint16_t m, const uint32_t originN, const T scale, const float dScaleQK, const T minValue, const float pScale)
{
    ...
    const T logPScale = static_cast<half>(std::log(pScale));
    ...
    ProcessVec1NoUpdateGeneralImpl256GqaFullquantVF<T, T2, OUTPUT_T, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop>(
        ...
        m, n, minValue, tailN, tailNOffset, logPScale);
}
} // namespace
