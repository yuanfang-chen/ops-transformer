...
template <int DT_Q, int DT_K, int DT_OUT, int LAYOUT_T>
                                                           ...
                                                           __gm__ uint8_t *tiling)
{
    ...
    if constexpr (DT_Q == TPL_FP16 && DT_K == TPL_FP16 && DT_OUT == TPL_FP32) {
        DENSE_LI_SOFTMAX_LSE_OP_IMPL(DenseLISoftmaxLse, half, half, float, LAYOUT(LAYOUT_T));
    } else {
        DENSE_LI_SOFTMAX_LSE_OP_IMPL(DenseLISoftmaxLse, bfloat16_t, bfloat16_t, float, LAYOUT(LAYOUT_T));
    }
#endif
}