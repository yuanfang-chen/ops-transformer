...
```Cpp
aclnnStatus aclnnCausalConv1dAdd(
  void          *workspace,
  uint64_t       workspaceSize,
  aclOpExecutor *executor,
  aclrtStream    stream)
...
## aclnnCausalConv1dAddGetWorkspaceSize
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnCausalConv1dAddGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
int main()
{
    ...
    std::vector<int16_t> xHostData(cuSeqLen * dim, 1);
    std::vector<int16_t> weightHostData(K * dim, 1);
    std::vector<int16_t> convStatesHostData(numSlots * stateLen * dim, 0);
    std::vector<int32_t> queryStartLocHostData = {0, 5, 8, 15, 19};
    std::vector<int32_t> cacheIndicesHostData = {0, 3, 1, 5};
    std::vector<int32_t> initialStateModeHostData = {1, 0, 2, 1};
    std::vector<int16_t> biasHostData(dim, 0);
    std::vector<int32_t> numAcceptedTokensHostData(batch, 0);
    std::vector<int16_t> yHostData(cuSeqLen * dim, 0);
    ...
    ret = CreateAclTensor(initialStateModeHostData, initialStateModeShape, &initialStateModeDeviceAddr, aclDataType::ACL_INT32, &initialStateMode);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建bias aclTensor
    ret = CreateAclTensor(biasHostData, biasShape, &biasDeviceAddr, aclDataType::ACL_FLOAT16, &bias);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建numAcceptedTokens aclTensor
    ret = CreateAclTensor(numAcceptedTokensHostData, numAcceptedTokensShape, &numAcceptedTokensDeviceAddr, aclDataType::ACL_INT32, &numAcceptedTokens);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建y aclTensor
    ...
    int64_t padSlotId = -1;
    int64_t runMode = 0;
    int64_t residualConnection = 1;

    // 调用aclnnCausalConv1dAdd第一段接口
    ret = aclnnCausalConv1dAddGetWorkspaceSize(
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnCausalConv1dAddGetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
    ...
    // 调用aclnnCausalConv1dAdd第二段接口
    ret = aclnnCausalConv1dAdd(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnCausalConv1dAdd failed. ERROR: %d\n", ret); return ret);
