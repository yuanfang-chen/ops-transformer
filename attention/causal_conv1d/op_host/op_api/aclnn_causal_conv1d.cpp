                               ...
                               const aclTensor* y)
{
    ...
    OP_CHECK_NULL(queryStartLoc, return ACLNN_ERR_PARAM_NULLPTR);
    OP_CHECK_NULL(cacheIndices, return ACLNN_ERR_PARAM_NULLPTR);
    OP_CHECK_NULL(hasInitialState, return ACLNN_ERR_PARAM_NULLPTR);
    OP_CHECK_NULL(y, return ACLNN_ERR_PARAM_NULLPTR);

    OP_CHECK_DTYPE_NOT_SAME(weight, x, return ACLNN_ERR_PARAM_INVALID);
    OP_CHECK_DTYPE_NOT_SAME(convStates, x, return ACLNN_ERR_PARAM_INVALID);
    ...
    OP_CHECK_DTYPE_NOT_MATCH(queryStartLoc, DataType::DT_INT32, return ACLNN_ERR_PARAM_INVALID);
    OP_CHECK_DTYPE_NOT_MATCH(cacheIndices, DataType::DT_INT32, return ACLNN_ERR_PARAM_INVALID);
    OP_CHECK_DTYPE_NOT_MATCH(hasInitialState, DataType::DT_BOOL, return ACLNN_ERR_PARAM_INVALID);

    // output shape should match x (prefill/extend: y(cu_seqlen, dim))
...
{
    L2_DFX_PHASE_1(aclnnCausalConv1d,
                   DFX_IN(x, weight, biasOptional, convStates, queryStartLoc, cacheIndices, hasInitialState,
    ...
    auto ret = CheckParams(x, weight, biasOptional, convStates, queryStartLoc, cacheIndices, hasInitialState, y);
    CHECK_RET(ret == ACLNN_SUCCESS, ret);

    auto yInfer = l0op::CausalConv1d(x, weight, biasOptional, convStates, queryStartLoc, cacheIndices, hasInitialState,
                                    activationMode, padSlotId, uniqueExecutor.get());
