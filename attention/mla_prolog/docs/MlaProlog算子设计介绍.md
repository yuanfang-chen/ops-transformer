...
MlaProlog融合算子包含Vector计算和Cube计算，Vector侧和Cube侧的计算存在依赖关系，因此需要进行CV流水设计，
...
1、MatmulCq计算时，对Hcq进行了分核，单核不会计算完整的token，因此在RmsNormCq计算前，
需要做AIC与AIV之间的同步控制（SYNC_MMCQ_NORMCQ）

2、MatmulCkvKr计算时，同样对Hcq进行了分核，单核不会计算完整的token，因此在RmsNormCkv计算前，也
需要做AIC与AIV之间的同步控制（SYNC_MMCKVKR_NORMCKV）

3、由于RmsNormCq按行切分到不同核上，因此在执行MatmulQcQr计算时，也需要AIV与AIC的同步控制（SYNC_NORMCQ_MMQCQR）
