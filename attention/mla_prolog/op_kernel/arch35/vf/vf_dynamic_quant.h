...
__aicore__ inline void ComputeVF(__local_mem__ T* xAddr, __local_mem__ O* yAddr, __local_mem__ float* scaleAddr,
    uint32_t rowIndex, uint32_t col)
{
    ...
    constexpr float maxValue = static_cast<float>(1.0) / FP8_E4M3FN_MAX_VALUE;
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t j = 0; j < vfLoop; j++) {
            ...
            AscendC::MicroAPI::DataCopy<T, AscendC::MicroAPI::LoadDist::DIST_UNPACK_B16>(vreg1, xAddr + rowIndex * rowCount + j * VL);
        ...
        AscendC::MicroAPI::ReduceMax(vreg5, vreg4, preg1);
        AscendC::MicroAPI::Muls(vreg6, vreg5, maxValue, preg1);
        AscendC::MicroAPI::Duplicate(vreg7, vreg6, preg1);
        AscendC::MicroAPI::DataCopyUnAlign<float, AscendC::MicroAPI::PostLiteral::POST_MODE_UPDATE>(scaleAddr, vreg6, ureg0, 1);
        ...
        for (uint16_t j = 0; j < vfLoop; j++) {
            ...
            AscendC::MicroAPI::DataCopy<O, AscendC::MicroAPI::StoreDist::DIST_PACK4_B32>(addr, vreg9, preg2);
        }
...
__aicore__ inline void ComputeData(__ubuf__ T* srcAddr, __ubuf__ uint16_t* halfScaleLocalAddr,
    __ubuf__ int8_t* outLocalAddr, uint32_t totalCountInUB, uint16_t loopNum, uint16_t vecLen)
{
    ...
    const LocalTensor<T>& inputLocal, const LocalTensor<uint8_t>& tmpLocal, uint32_t row, uint32_t col)
{
    ...
    ComputeMaxExp<T, U>(srcAddr, maxExpAddr, totalCountInUB, loopNum, vecLen);
    ComputeScale<T, U>(maxExpAddr, mxScaleLocalAddr, halfScaleLocalAddr, totalScaleInUB, loopNumScale, vecLen);
    ...
    ComputeData<T, U>(srcAddr, halfScaleLocalAddr, outLocalAddr, totalCountInUB, loopNum, vecLen);
}
