    ...
    const uint32_t floatRepSize, uint32_t dLoops, uint32_t dTail, uint32_t dTailLoop, uint32_t row, uint32_t col, uint32_t stride)
{
    ...
    constexpr static AscendC::MicroAPI::CastTrait castTraitBf16ToFp32 = {
        AscendC::MicroAPI::RegLayout::ZERO, AscendC::MicroAPI::SatMode::UNKNOWN,
        AscendC::MicroAPI::MaskMergeMode::ZEROING, AscendC::RoundMode::UNKNOWN};
    constexpr static AscendC::MicroAPI::CastTrait castTraitFp32ToInt8 = {
    ...
    for (uint32_t j = 0; j < dLoops; j++) {
        AscendC::MicroAPI::LoadAlign<C, AscendC::MicroAPI::LoadDist::DIST_BRC_B32>(vregQuantScale, quantScaleAddr + colOffset);
        rowOffset = colOffset;
        for (uint32_t i = 0; i < row; i++) {
            AscendC::MicroAPI::LoadAlign<T, AscendC::MicroAPI::LoadDist::DIST_NORM>(vregInput, xAddr + rowOffset);
            AscendC::MicroAPI::Cast<float, T, castTraitBf16ToFp32>(vregInputFp32, vregInput, fullMask);
            AscendC::MicroAPI::Mul(vregInputFp32, vregInputFp32, vregQuantScale, fullMask);
            AscendC::MicroAPI::Cast<O, float, castTraitFp32ToInt8>(vregOutput, vregInputFp32, fullMask);
            AscendC::MicroAPI::StoreAlign<O, AscendC::MicroAPI::StoreDist::DIST_NORM_B32>(yAddr + rowOffset, vregOutput, fullMask);  
            rowOffset += stride;
        }
        ...
        if (dTailLoop) {
        rowOffset = 0;
        AscendC::MicroAPI::LoadAlign<C, AscendC::MicroAPI::LoadDist::DIST_BRC_B32>(vregQuantScale, quantScaleAddr + dLoops * floatRepSize);
        for (uint32_t i = 0; i < row; i++) {
            AscendC::MicroAPI::LoadAlign<T, AscendC::MicroAPI::LoadDist::DIST_NORM>(vregInput, xAddr + dLoops * floatRepSize + rowOffset);
            AscendC::MicroAPI::Cast<float, T, castTraitBf16ToFp32>(vregInputFp32, vregInput, tailMask);
            AscendC::MicroAPI::Mul(vregInputFp32, vregInputFp32, vregQuantScale, tailMask);
            AscendC::MicroAPI::Cast<O, float, castTraitFp32ToInt8>(vregOutput, vregInputFp32, tailMask);
            AscendC::MicroAPI::StoreAlign<O, AscendC::MicroAPI::StoreDist::DIST_NORM_B32>(yAddr + rowOffset, vregOutput, tailMask);  
            rowOffset += stride;
        }
    }
}
    ...
    const LocalTensor<C> &quantScaleLocal, const uint32_t row, const uint32_t col, const uint32_t stride)
{
    ...
    __ubuf__ C * quantScaleUb = (__ubuf__ T *)quantScaleLocal.GetPhyAddr();
    ...
    QuantChannelVFImpl(outputUb, inputUb, quantScaleLocal, floatRepSize, dLoops, dTail, dTailLoop, row, col, stride);
}
} // namespace MlaProlog
#endif