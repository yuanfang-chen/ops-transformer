    ...
    const uint32_t floatRepSize, uint32_t dLoops, uint32_t dTail, uint32_t dTailLoop, uint32_t row, uint32_t col, uint32_t stride)
{
    ...
    for (uint32_t j = 0; j < dLoops; j++) {
        AscendC::MicroAPI::LoadAlign<C, AscendC::MicroAPI::LoadDist::DIST_NORM_B32>(vregQuantScale, quantScaleAddr + colOffset);
        rowOffset = 0;
        for (uint32_t i = 0; i < row; i++) {
            ...
            AscendC::MicroAPI::Cast<half, float, castTraitF32ToHalf>(vregOutputHalf, vregInputFp32, fullMask);
            AscendC::MicroAPI::Cast<O, half, castTraitPack2>(vregOutput, vregOutputHalf, fullMask);
            AscendC::MicroAPI::StoreAlign<O, AscendC::MicroAPI::StoreDist::DIST_NORM_B32>(yAddr + colOffset + rowOffset, vregOutput, fullMask);
            rowOffset += stride;
        }
        ...
        if (dTailLoop > 0) {
        rowOffset = 0;
        AscendC::MicroAPI::LoadAlign<C, AscendC::MicroAPI::LoadDist::DIST_NORM_B32>(vregQuantScale, quantScaleAddr + dLoops * floatRepSize);
        for (uint32_t i = 0; i < row; i++) {
            ...
            AscendC::MicroAPI::Cast<half, float, castTraitF32ToHalf>(vregOutputHalf, vregInputFp32, tailMask);
            AscendC::MicroAPI::Cast<O, half, castTraitPack2>(vregOutput, vregOutputHalf, tailMask);
            AscendC::MicroAPI::StoreAlign<O, AscendC::MicroAPI::StoreDist::DIST_NORM_B32>(yAddr + dLoops * floatRepSize + rowOffset, vregOutput, tailMask);
            rowOffset += stride;
        }
    }
}
