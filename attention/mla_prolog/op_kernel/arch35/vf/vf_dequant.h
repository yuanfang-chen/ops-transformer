...
template<typename T, typename C, typename O> // M=0为fp8全量化PerToken量化；M=1为fp8全量化pertile量化
__simd_vf__ void DequantVFImpl (__ubuf__ O* yAddr, __ubuf__ T* xAddr, __ubuf__ C * scalePerChannelAddr, __ubuf__ C * scalePerTokenAddr,
    const uint16_t floatRepSize, const uint16_t fp32BlockElementNum, uint16_t dLoops, uint16_t dTail, uint16_t dTailLoop, uint16_t row, uint16_t col, uint16_t stride)
{
    ...
    tailMask = AscendC::MicroAPI::UpdateMask<float>(static_cast<uint32_t>(dTail));

    uint16_t colOffset = 0;
    uint16_t rowOffset = 0;
    uint16_t scaleOffset = 0;
    for (uint16_t j = 0; j < dLoops; j++) {       
        AscendC::MicroAPI::LoadAlign<C, AscendC::MicroAPI::LoadDist::DIST_BRC_B32>(vregscalePerChannel, scalePerChannelAddr + colOffset);
        rowOffset = colOffset;
        scaleOffset = 0;
        for (uint16_t i = 0; i < row; i++) {
            ...
            AscendC::MicroAPI::Cast<O, float>(vregOutput, vregInputFp32, fullMask);
    ...
    if (dTailLoop) {
        rowOffset = 0;
        scaleOffset = 0;
        AscendC::MicroAPI::LoadAlign<C, AscendC::MicroAPI::LoadDist::DIST_BRC_B32>(vregscalePerChannel, scalePerChannelAddr + dLoops * floatRepSize);
        for (uint16_t i = 0; i < row; i++) {
            ...
            AscendC::MicroAPI::Cast<O, float>(vregOutput, vregInputFp32, tailMask);
...
__aicore__ inline void DequantVf(const LocalTensor<O>& outputLocal, const LocalTensor<T>& inputLocal, const LocalTensor<C>& scalePerChannelLocal,
    const LocalTensor<C>& scalePerTokenLocal, const uint32_t row, const uint32_t col, const uint32_t stride)
{   
    ...
    const uint16_t fp32BlockElementNum = 8;
    uint16_t dLoops = col / floatRepSize;
    uint16_t dTail = col % floatRepSize;
