...
__aicore__ inline void MulQr_VF(const LocalTensor<T> &outputLocal, const LocalTensor<T> &inputLocal, const LocalTensor<T> &dequantScaleBrcbLocal,
    ...
    __VEC_SCOPE__ {
        ...
        for(uint16_t i = 0; i < repeatTimes; i++) {
            ...
            // broadcast量化系数
            MicroAPI::DataCopy<T, MicroAPI::LoadDist::DIST_BRC_B32>(vregDequantScaleVrcb, dequantScaleBrcbBuf + dequantLoopOffset);
            ...
            MicroAPI::DataCopy<T, MicroAPI::StoreDist::DIST_NORM>(outputBuf + loopOffset, vregRes, pregAll);
        }
    }
}
} // namespace MlaProlog
