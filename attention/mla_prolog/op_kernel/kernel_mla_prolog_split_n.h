...
#if __CCE_AICORE__ == 310
    GlobalTensor<float> cqPartialsGm_;
    GlobalTensor<float> ckvKrPartialsGm_;
...
__aicore__ inline void MlaPrologVecS1CubS2<MLAPT>::WorkspaceInit(__gm__ uint8_t *workspace) {
    ...
    if (baseParams_->mm1KnKGroups > 0) {
        cqPartialsGm_.SetGlobalBuffer((__gm__ float *)(workspace + workspaceOffset));
        workspaceOffset += static_cast<int64_t>(baseParams_->mm1KnKGroups)
                         * static_cast<int64_t>(baseParams_->stepBatchSize)
                         * static_cast<int64_t>(baseParams_->headSizeCq) * sizeof(float);
    }
    if (baseParams_->mm2KnKGroups > 0) {
        ckvKrPartialsGm_.SetGlobalBuffer((__gm__ float *)(workspace + workspaceOffset));
...
__aicore__ inline void MlaPrologVecS1CubS2<MLAPT>::AicProcess(AicOffset &aicOffset, int64_t batchOffset, int64_t mmQnLoops) {
    ...
    // Ascend 950: Split-KN for MM1 (MatmulCq)
    {
        ...
        mmCqKn.n = singleN1;
        mmCqKn.k = singleK1;
        MatmulSplitN<mmInputType, float, dequantScaleType>(
    ...
    // Ascend 950: Split-KN for MM2 (MatmulCkvKr)
    {
        ...
        MatmulSplitN<mmInputType, float, dequantScaleType, true>(
...
__aicore__ inline void MlaPrologVecS1CubS2<MLAPT>::AivProcess(AivOffset &aivOffset, int64_t batchOffset, int64_t curStepBatchSize, int64_t numHeadOffset, int64_t mmQnLoops) {
    ...
    if (baseParams_->mm1KnKGroups > 0) {
        ...
        if (nStart < baseParams_->headSizeCq) {
            AccumulateKnPartials<float, mmCqOutputType>(
                mmCqResGm_,
                cqPartialsGm_,
    ...
    if (baseParams_->mm2KnKGroups > 0) {
        ...
        if (nStart < ckvKrN) {
            AccumulateKnPartials<float, mmCkvKrOutputType>(
                mmCkvKrResGm_,
                ckvKrPartialsGm_,
