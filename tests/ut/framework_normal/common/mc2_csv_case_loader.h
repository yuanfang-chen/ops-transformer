...
std::vector<T> GetCasesFromCsv(const std::string& csvPath)
{
    ...
    for (int lineNum = 1; std::getline(csvFile, line); ++lineNum) {
        size_t valuesSize = std::count(line.begin(), line.end(), ',') + 1;
        if (keys.size() != valuesSize) {
            std::cout << "[ERROR] \"" << csvPath << "\" Line " << lineNum
                      << ": Row data does not match CSV header columns. Row has " << valuesSize
