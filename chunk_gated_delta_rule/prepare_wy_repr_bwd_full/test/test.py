...
if __name__ == "__main__":
    ...
    chunk_indices = prepare_chunk_indices(cu_seqlens, chunk_size = 64)
    test_prepare_wy_repr_bwd_full(B = 1, H = 8, T = 65536, K = 128, V = 128, chunk_size = 64, ktype=torch.float16, btype=torch.float16, cu_seqlens = cu_seqlens.npu(), chunk_indices=chunk_indices.npu())
    # k[0][0][0][1] = 0.5
    # data = {
    #     'k': k,
    #     'v': v,
    ...
    # dk, dv, dbeta, dg=torch_npu.npu_prepare_wy_repr_bwd_full(k.npu(), v.npu(), beta.npu(), A.npu(), dA.npu(), dw.npu(), du.npu(), g.npu(), cu_seqlens=None, chunk_indices=None,chunk_size= chunk_size)

    # torch.save(dk, "npu_dk.pt")
    # torch.save(dv, "npu_dv.pt")
    # torch.save(dbeta, "npu_dbeta.pt")
    # torch.save(dg, "npu_dg.pt")