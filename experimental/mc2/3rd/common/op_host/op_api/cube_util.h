...
inline bool IsCubeSupportFp32() {
    if (op::GetCurrentPlatformInfo().GerCurNpuArch() != NpuArch::DAV_2201 &&
        op::GetCurrentPlatformInfo().GetSocVersion() != NpuArch::DAV_3510) {
        return false;
    }
    return true;
}
...
inline bool IsCubeSupportHf32() {
    if (op::GetCurrentPlatformInfo().GetSocVersion() != NpuArch::DAV_2201 &&
        op::GetCurrentPlatformInfo().GetSocVersion() != NpuArch::DAV_3510) {
        return false;
    }
    return true;
