...
 */
#ifndef OP_API_INC_MOE_DISTRIBUTE_COMBINE_SHMEM_
#define OP_API_INC_MOE_DISTRIBUTE_COMBINE_SHMEM_
...
/**
 * 算子功能：实现MoeDistributeCombineShmem功能。
 * @brief
 * aclnnMoeDistributeCombineShmem的第一段接口，根据具体的计算流程，计算workspace大小。
 * @domain aclnn_ops_infer
 * @param [in] expandX:
 * 计算输入，Tensor，数据类型float16，bfloat16，必须为2维，数据格式支持ND。
 * @param [in] expertIds:
 * 计算输入，Tensor，数据类型int32，必须为2维，数据格式支持ND。
 * @param [in] assistInfoForCombine:
 * 计算输入，Tensor，数据类型int32，必须为1维，数据格式支持ND。
 * @param [in] epSendCounts:
 * 计算输入，Tensor，数据类型int32，必须为1维，数据格式支持ND。
 * @param [in] expertScales:
 * 计算输入，Tensor，数据类型float32，必须为2维，数据格式支持ND。
 * @param [in] tpSendCountsOptional:
 * 计算输入，Tensor，数据类型int32，必须为1维，数据格式支持ND。若有TP域通信需要传参，若无TP域通信，传空指针即可。
 * @param [in] xActiveMaskOptional:
 * 计算输入，Tensor，数据类型bool，必须为1维，数据格式支持ND。
 ...
 * 计算输入，Tensor，数据类型float32，必须为1维，数据格式支持ND。
 * @param [in] sharedExpertXOptional:
 * 计算可选输入，Tensor，数据类型float16，bfloat16，必须为2维，数据格式支持ND。
 * @param [in] elastic_infoOptional:
 * 计算可选输入，Tensor，数据类型int32，必须为1维，数据格式支持ND。
 * @param [in] ori_xOptional:
 * 计算可选输入，Tensor，数据类型float16，bfloat16，必须为2维，数据格式支持ND。
 * @param [in] const_expert_alpha_1Optional:
 * 计算可选输入，Tensor，数据类型float16，bfloat16，必须为2维，数据格式支持ND。
 * @param [in] const_expert_alpha_2Optional:
 * 计算可选输入，Tensor，数据类型float16，bfloat16，必须为2维，数据格式支持ND。
 * @param [in] const_expert_vOptional:
 * 计算可选输入，Tensor，数据类型float16，bfloat16，必须为2维，数据格式支持ND。
 * @param [in] groupEp:
 * 计算输入，str。ep通信域名称，专家并行的通信域。不能和groupTp相同。
 * @param [in] epWorldSize: 计算输入，int。ep通信域size。
 * @param [in] epRankId:
 * 计算输入，int。ep本卡Id。同一个EP通信域中各卡的epRankId不重复。
 ...
 * @param [in] tpRankId:
 * 计算可选输入，int。tp本卡Id。同一个TP通信域中各卡的tpRankId不能重复。
 ...
 * @param [in] outDtype:
 * 计算可选输入，int。输出数据类型。预留参数，暂未使用，传0即可。
 * @param [in] commQuantMode: 计算可选输入，int。通信量化类型。
 * @param [in] groupListType:
 * 计算可选输入，int。groupList格式。预留参数，暂未使用，传0即可。
 ...
 * @param [out] xOut: 计算输出，Tensor，必选输出，数据类型支持float16,
 * bfloat16，仅支持2维，数据格式支持ND。
 * @param [out] workspaceSize: 出参，返回需要在npu device侧申请的workspace大小。
 * @param [out] executor: 出参，返回op执行器，包含了算子计算流程。
 * @return aclnnStatus: 返回值，返回状态码
 *
 */
ACLNN_API aclnnStatus aclnnMoeDistributeCombineShmemGetWorkspaceSize(
...
/**
 * @brief aclnnMoeDistributeCombineShmem的第二段接口，用于执行计算。
 * @param [in] workspace: 在npu device侧申请的workspace内存起址。
 * @param [in] workspace_size: 在npu
 * device侧申请的workspace大小，由第一段接口aclnnMoeDistributeCombineShmemGetWorkspaceSize获取。
 * @param [in] executor: op执行器，包含了算子计算流程。
 * @param [in] stream: acl stream流。
 * @return aclnnStatus: 返回状态码
 */
ACLNN_API aclnnStatus aclnnMoeDistributeCombineShmem(void* workspace,
...
#endif  // OP_API_INC_MOE_DISTRIBUTE_COMBINE_SHMEM_