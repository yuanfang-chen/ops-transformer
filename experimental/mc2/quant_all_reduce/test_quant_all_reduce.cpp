...
{
    int optionIndex = 0;
    int c = getopt_long(argc, argv, "a:b:c:d:e:f:g:h:i:j:", kLongOptions, &optionIndex);
    while (c != -1) {
        switch (c) {
            case 'a':
                rankId = atoi(optarg);
...
int CreatTilingDataAndContext(std::string hcomName, void **deviceTilingAddr, void **deviceContextAddr) {
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("[ERROR] GetTiling mc2CcTiling failed. ret = %d\n", ret);
              return ret);
    // 申请tilingdata设备侧内存
    ret = aclrtMalloc(deviceTilingAddr, sizeof(struct QuantAllReduceTilingData), ACL_MEM_MALLOC_HUGE_FIRST);
...
int LaunchOneThreadQuantAllReduce(Args &args)
{
    ...
    ret = CreatTilingDataAndContext(tilingData, hcomName, &mc2ContextAddr);
    ...
    // 固定写法（创建device侧内存，拷贝host侧内容到device侧）
