...
__aicore__ inline void ExecMoeDistributeCombineV2(
  ...
  GET_TILING_DATA_WITH_STRUCT(MoeDistributeCombineV2TilingData, tilingData,
                              tilingGM);
  MoeDistributeCombineV2<TemplateMC2TypeFunc> op;
  op.Init(shmemContext, expandX, expertIds, assistInfoForCombine, epSendCount,
...

{
  ...
  if (TILING_KEY_IS(10100)) {  // tp=2 IsInt8Quant=0
    ExecMoeDistributeCombineV2<DTYPE_EXPAND_X, DTYPE_X, int32_t, true, false>(
        shmemContext, expandX, expertIds, assistInfoForCombine, epSendCount,
        tpSendCount, scales, xActiveMask, sharedExpertX, elasticInfo, oriX,
        constExpertAlpha1, constExpertAlpha2, constExpertV, XOut, workspaceGM,
        tilingGM, &pipe);
  }
