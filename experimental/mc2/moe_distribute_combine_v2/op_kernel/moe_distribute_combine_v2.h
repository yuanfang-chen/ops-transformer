...
#define TemplateMC2TypeClass                                    \
  typename ExpandXType, typename XType, typename ExpandIdxType, \
      bool IsNeedReduceScatter, bool IsInt8Quant
#define TemplateMC2TypeFunc \
  ExpandXType, XType, ExpandIdxType, IsNeedReduceScatter, IsInt8Quant
...
class MoeDistributeCombineV2 {
 public:
  ...
  __aicore__ inline void Init(
      GM_ADDR shmemContext, GM_ADDR expandX, GM_ADDR expertIds,
      GM_ADDR expandIdx, GM_ADDR epSendCount, GM_ADDR tpSendCount,
      GM_ADDR expertScales, GM_ADDR xActiveMask, GM_ADDR sharedExpertX,
 ...
 private:
  ...
  __aicore__ inline void CustomAdd(LocalTensor<XType> &dst,
...
MoeDistributeCombineV2<TemplateMC2TypeFunc>::TokenMaskCalCnt() {
  // 一维mask, 计算得到有效bs数量
  ...
  DataCopyPad(xActiveMaskTensor, xActiveMaskGM_, xActiveMaskParams,
              xActiveMaskCopyPadParams);
  SyncFunc<AscendC::HardEvent::MTE2_V>();
  LocalTensor<int8_t> xActiveMaskInt8Tensor =
      xActiveMaskTensor.ReinterpretCast<int8_t>();
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::ExpertMaskCalCnt() {
  // 二维mask, 挑选有效token
  uint64_t rsvdCnt = 0;
  ...
  DataCopyPadExtParams<bool> xActiveMaskCopyPadParams{
  ...
  SumParams axisBsSumParams{
      ...
      axisBS_};
  uint32_t calCnt =
      Ceil(axisBS_ * sizeof(half), ALIGNED_LEN) * ALIGNED_LEN / sizeof(half);

  Duplicate<half>(maskTempTensor, (half)0, calCnt);
  DataCopyPad(maskStrideTensor, xActiveMaskGM_, xActiveMaskParams,
              xActiveMaskCopyPadParams);
  SyncFunc<AscendC::HardEvent::MTE2_V>();
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::InitDataStatus(
    const MoeDistributeCombineV2TilingData *tilingData) {
  ...
  datastateLocalTensor_.SetValue(
      2, tilingData->moeDistributeCombineV2Info.epRankId);
  datastateLocalTensor_.SetValue(
      3, tilingData->moeDistributeCombineV2Info.moeExpertNum);
  datastateLocalTensor_.SetValue(
      4, tilingData->moeDistributeCombineV2Info.epWorldSize);
  datastateLocalTensor_.SetValue(
      5, tilingData->moeDistributeCombineV2Info.globalBs);
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::InitInputAndOutput(
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::InitElasticInfo(
    uint32_t &sharedExpertRankNum) {
  DataCacheCleanAndInvalid<int32_t, CacheLine::SINGLE_CACHE_LINE,
                           DcciDst::CACHELINE_OUT>(elasticInfoGM_);
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeCombineV2<TemplateMC2TypeFunc>::InitAttrs(
    const MoeDistributeCombineV2TilingData *tilingData) {
  ...
  enableSpecialExpert_ =
      (constExpertNum_ + zeroExpertNum_ + copyExpertNum_ > 0U);
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::InitInt8Quant() {
  scaleValFloat_ = static_cast<float>(1.0f / SCALE_PARAM);
  uint32_t scaleGranu = static_cast<uint32_t>(
      UB_ALIGN / sizeof(float));  // 计算每个block得到的reducemax结果数量
  scaleNum_ = (hExpandXAlign32Size_ / sizeof(ExpandXType)) /
              scaleGranu;  // 得到有效scale的个数
  repeatNum_ = static_cast<uint32_t>(
      hFloatAlign256Size_ /
      ALIGNED_LEN);  // BlockReduceMax 与 Brcb的重复迭代次数，每次256b参与计算
  mask_ = static_cast<uint32_t>(ALIGNED_LEN / sizeof(float));
  tokenScaleCnt_ = hAlign32Size_ / sizeof(ExpandXType) +
                   scaleNum_;  // int8_align + scale有效个数
}

template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeCombineV2<TemplateMC2TypeFunc>::Init(
    GM_ADDR shmemContext, GM_ADDR expandX, GM_ADDR expertIds, GM_ADDR expandIdx,
  ...
  InitInputAndOutput(expandX, expertIds, expandIdx, epSendCount, expertScales,
                     xActiveMask, sharedExpertX, elasticInfo, oriX,
                     constExpertAlpha1, constExpertAlpha2, constExpertV, XOut);
  ...
  winDataSizeOffset_ =
      ...
      (tilingData->moeDistributeCombineV2Info.totalWinSize / 2UL);
  winStatusOffset_ = COMBINE_STATE_OFFSET +
                     dataState_ * WIN_STATE_OFFSET;  // 前面的预留给dispatch使用
  ...
  for (int tempepRankId = 0; tempepRankId < epWorldSize_; tempepRankId++) {
    OOMCheckAddrRange<XType>(
        (__gm__ XType *)(GetWinAddrByRankId(tempepRankId, EP_DOMAIN)),
        totalWinSize_);
    OOMCheckAddrRange<float>(
        (__gm__ float *)(GetWinStateAddrByRankId(tempepRankId, EP_DOMAIN)),
        STATE_SIZE);
  }
#endif
  ...
  if constexpr (IsNeedReduceScatter) {
    ...
    for (int temptpRankId = 0; temptpRankId < tpWorldSize_; temptpRankId++) {
      OOMCheckAddrRange<XType>(
          (__gm__ XType *)(GetWinAddrByRankId(temptpRankId, TP_DOMAIN)),
          totalWinSize_);
      OOMCheckAddrRange<int32_t>(
          (__gm__ int32_t *)(GetWinStateAddrByRankId(temptpRankId, TP_DOMAIN)),
          STATE_SIZE);
    }
#endif
...
__aicore__ inline void MoeDistributeCombineV2<TemplateMC2TypeFunc>::BuffInit() {
  ...
  if constexpr (IsNeedReduceScatter) {
    tpipe_->InitBuffer(gmTpSendCountInQueue_, BUFFER_NUM,
                       hExpandXAlign32Size_);  // 28K 存储输入拷过来的token
    tpipe_->InitBuffer(xOutQueue_, BUFFER_NUM,
                       hExpandXAlign32Size_);  // 14K 存储输出token
    tpipe_->InitBuffer(winTpSendCountInQueue_, BUFFER_NUM,
                       hExpandXAlign32Size_);  // 14K * 2 存储对端win区token
    if constexpr (AscendC::IsSameType<XType, bfloat16_t>::value) {
      tpipe_->InitBuffer(
          winTpSendCountFloatBuf_,
          hFloatAlign32Size_);  // 28K 参与量化及customAdd中token的v核运算
      tpipe_->InitBuffer(
          gmTpSendCountFloatBuf_,
          hFloatAlign32Size_);  // 28K 参与量化及customAdd中token的v核运算
    ...
    tpipe_->InitBuffer(gmTpSendCountQueue_, BUFFER_NUM,
                       hExpandXAlign32Size_);  // 28K 存储搬入token
    if constexpr (IsInt8Quant) {
      uint32_t tokenScaleAlign32Size =
          Ceil(tokenScaleCnt_ * sizeof(ExpandXType), UB_ALIGN) * UB_ALIGN;
      tpipe_->InitBuffer(xOutQueue_, BUFFER_NUM,
    ...
    if (isScalingDownFlag_) {
      ...
      uint32_t elasticInfoSizeAlign =
          Ceil(elasticInfoSize, UB_ALIGN) * UB_ALIGN;
      ...
      DataCopyPad(elasticInfoTensor_, elasticInfoGM_, elasticInfoParams,
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::MaskAlign() {
  // 扩展后的二维mask通过GM对齐内轴元素个数
  uint32_t calcCnt = Ceil(axisBS_ * axisK_ * sizeof(half), ALIGNED_LEN_256) *
                     ALIGNED_LEN_256 / sizeof(half);
  GlobalTensor<bool> MaskGMTensor;
  MaskGMTensor.SetGlobalBuffer((__gm__ bool *)maskCalcWorkspaceGM_);
  DataCopyExtParams maskCalcParams = {
      1U, static_cast<uint32_t>(calcCnt * sizeof(bool)), 0U, 0U, 0U};
  ...
  DataCopyPadExtParams<bool> xActiveMaskCopyPadParams{
      true, 0U, static_cast<uint8_t>(UB_ALIGN - axisK_), 0U};
  DataCopyPad(maskStrideTensor_, MaskGMTensor, xActiveMaskParams,
              xActiveMaskCopyPadParams);
  SyncFunc<AscendC::HardEvent::MTE2_V>();
  SyncFunc<AscendC::HardEvent::MTE2_S>();
}
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::GenerateActiveMask(half val) {
  ...
  if (isInputTokenMaskFlag_) {
    ...
    uint32_t activeCalcCnt =
        Ceil(activeMaskBsCnt_ * axisK_ * sizeof(half), ALIGNED_LEN_256) *
        ALIGNED_LEN_256 / sizeof(half);
    Duplicate<half>(maskCalcTensor, static_cast<half>(1), activeCalcCnt);
    PipeBarrier<PIPE_V>();
    Cast(maskGenerateTensor_.ReinterpretCast<uint8_t>(), maskCalcTensor,
         RoundMode::CAST_NONE, calcCnt);
  } else {
    // 构造二维全true的mask
    uint32_t calcCnt = Ceil(axisBS_ * axisK_ * sizeof(half), ALIGNED_LEN_256) *
                       ALIGNED_LEN_256 / sizeof(half);
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::MaskSpecialExpert() {
  ...
  DataCopyExtParams expertIdsCntParams = {
      1U, static_cast<uint32_t>(mask * sizeof(int32_t)), 0U, 0U, 0U};
  DataCopyPadExtParams<int32_t> expertIdsCntCopyPadParams{false, 0U, 0U, 0U};
  DataCopyPad(expertIdsTensor_, expertIdsGM_, expertIdsCntParams,
              expertIdsCntCopyPadParams);
  SyncFunc<AscendC::HardEvent::MTE2_V>();
  SyncFunc<AscendC::HardEvent::MTE2_S>();

  // 根据expertId小于moeExpertNum，得到考虑特殊专家后的mask
  uint32_t calcCnt = Ceil(mask * sizeof(int32_t), ALIGNED_LEN_256) *
  ...
  CompareScalar(maskTensor, expertIdsFloat, static_cast<float>(tmpMoeExpertNum),
                AscendC::CMPMODE::LT, calcCnt);
  PipeBarrier<PIPE_V>();
  if (isInputExpertMaskFlag_) {
    Cast(maskCalcTensor, expertMaskTensor_.ReinterpretCast<uint8_t>(),
         RoundMode::CAST_NONE, calcCnt);
  } else {
    Cast(maskCalcTensor, maskGenerateTensor_.ReinterpretCast<uint8_t>(),
         RoundMode::CAST_NONE, calcCnt);
  }
  PipeBarrier<PIPE_V>();
  Select(maskCalcSelectedTensor, maskTensor, maskCalcTensor,
         static_cast<half>(0), SELMODE::VSEL_TENSOR_SCALAR_MODE, calcCnt);
  PipeBarrier<PIPE_V>();
  Cast(maskGenerateTensor_.ReinterpretCast<uint8_t>(), maskCalcSelectedTensor,
       RoundMode::CAST_NONE, calcCnt);
  ...
  uint32_t calCnt =
      Ceil(axisBS_ * sizeof(half), ALIGNED_LEN) * ALIGNED_LEN / sizeof(half);
  LocalTensor<int8_t> maskStrideInt8Tensor =
      maskStrideTensor_.ReinterpretCast<int8_t>();
  activeMaskAlignSize_ =
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::AlltoAllBuffInitAndMaskCal() {
  tpipe_->Reset();
  ...
  if (isInputExpertMaskFlag_ || enableSpecialExpert_) {
    ...
    maxSizeTokenBuf =
        (activeMaskAlignSize_ > hExpandXAlign32Size_ ? activeMaskAlignSize_
                                                     : hExpandXAlign32Size_);
    maxSizeRowTmpFloatBuf =
        (activeMaskAlignHalfSize > hFloatAlign32Size_ ? activeMaskAlignHalfSize
                                                      : hFloatAlign32Size_);
  }
  tpipe_->InitBuffer(expertScalesBuf_,
                     axisBS_ * axisK_ * sizeof(float));  // BS * K * 4 = 32K
  tpipe_->InitBuffer(tokenBuf_,
                     maxSizeRowTmpFloatBuf);  // 16K 用于搬入输入token
  tpipe_->InitBuffer(
      rowTmpFloatBuf_,
      maxSizeRowTmpFloatBuf);  // 32K 用于存储cast之后的fp32 token数据
  tpipe_->InitBuffer(
      mulBuf_,
      hFloatAlign256Size_);  // 32K buffer复用，
                             // 最大用于存储Brcb之后的token，需要256对齐
  tpipe_->InitBuffer(sumFloatBuf_, hFloatAlign32Size_);  // 32K add
  tpipe_->InitBuffer(moeSumQueue_, BUFFER_NUM,
                     hExpandXAlign32Size_);  // 32K 搬入
  tpipe_->InitBuffer(stateBuf_, (flagRcvCount_)*STATE_OFFSET);
  tpipe_->InitBuffer(stateResetBuf_,
                     (flagRcvCount_)*STATE_OFFSET);  // 清理状态区
  ...
  if (isInputExpertMaskFlag_) {
    tpipe_->InitBuffer(tokenTargetTBuf_,
                       Ceil(axisBS_ * sizeof(half), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(validBsIndexTBuf_,
                       Ceil(axisBS_ * sizeof(int32_t), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(
        expertMaskBuf_,
        Ceil(axisBS_ * axisK_ * sizeof(bool), UB_ALIGN) * UB_ALIGN);
    ...
    DataCopyExtParams maskParams{
        1U, static_cast<uint32_t>(axisBS_ * axisK_ * sizeof(bool)), 0U, 0U, 0U};
    DataCopyPad(expertMaskTensor_, xActiveMaskGM_, maskParams,
                maskCopyPadParams);
    SyncFunc<AscendC::HardEvent::V_S>();
  }
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::SplitCoreCal() {
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::ReduceScatterTrans() {
  ...
  GM_ADDR rankGM =
      GetWinAddrByRankId(1 - static_cast<int32_t>(tpRankId_), TP_DOMAIN);
  ...
  DataCopyExtParams expandXCopyParams{
      1U, static_cast<uint32_t>(hExpandXTypeSize_), 0U, 0U, 0U};
  for (uint32_t tokenNumIdx = copyStartIdx; tokenNumIdx < copyEndIdx;
       tokenNumIdx++) {
    tmpUb = moeQueue_.AllocTensor<ExpandXType>();
    DataCopyPad(tmpUb, dataCopyInGM[tokenNumIdx * axisH_], expandXCopyParams,
                copyPadExtParams);
    moeQueue_.EnQue(tmpUb);
    tmpUb = moeQueue_.DeQue<ExpandXType>();
    DataCopyPad(rankWindow_[tokenNumIdx * hAlignWinCnt_], tmpUb,
                expandXCopyParams);
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::SetWaitTpStatusAndDisPatch() {
  PipeBarrier<PIPE_ALL>();
  ...
  if constexpr (IsNeedReduceScatter) {
    ...
    stateGM_ = GetWinStateAddrByRankId(tpToRankId, TP_DOMAIN) +
               coreIdx_ * WIN_ADDR_ALIGN;
    tpStatusWinTensor.SetGlobalBuffer((__gm__ int32_t *)stateGM_);
    DataCopy<int32_t>(tpStatusWinTensor, statusFlagUb,
                      8UL);  // 8是数据大小，按32对齐拷贝
    SyncFunc<AscendC::HardEvent::MTE3_S>();

    GM_ADDR tpStatusWin = GetWinStateAddrByRankId(tpRankId_, TP_DOMAIN) +
                          coreIdx_ * WIN_ADDR_ALIGN;
    ...
    DataCacheCleanAndInvalid<int32_t, CacheLine::SINGLE_CACHE_LINE,
                             DcciDst::CACHELINE_OUT>(selfStatusWinTensor);
  }
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::ExpertAlltoAllDispatchCopyAdd() {
  ...
  for (uint32_t loop = 0; loop < sendCntNum_; loop++) {
    uint32_t tkIndex =
        startTokenId_ + ((loop + epRankId_) % sendCntNum_);  // 错位发送
    uint32_t baseOffset = (tkIndex - startTokenId_) * EXPAND_IDX_INFO;
    uint32_t rankIdExpandIdx =
        static_cast<uint32_t>(expandIdxLocal(baseOffset));  // 位置0是rank_id
    uint32_t toRankId = rankIdExpandIdx;                    // 位置0是rank_id
    uint32_t tokenId = static_cast<uint32_t>(
        expandIdxLocal(baseOffset + 1));  // 位置1是token_id
    uint32_t topkId = static_cast<uint32_t>(
        expandIdxLocal(baseOffset + 2));  // 位置2是topk_id
    if (isScalingDownFlag_) {
      toRankId = elasticInfoTensor_.GetValue(
          ELASTIC_INFO_OFFSET + epWorldSizeOriginal_ + rankIdExpandIdx);
    }
    ...
    DataCopy<float>(stateGMTensor, statusTensor,
                    FLOAT_PER_UB_ALIGN);  // 8是数据大小，按32对齐拷贝
  }
}

template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::Int8QuantProcess() {
  SyncFunc<AscendC::HardEvent::MTE2_V>();
  castLocalTensor_ =
      sendLocalTensor_
          .template ReinterpretCast<int8_t>();  // 长度为int8H_Align + scaleNum
  scaleDivTensor_ = castLocalTensor_[hAlign32Size_]
                        .template ReinterpretCast<XType>();  // 偏移前面的int8

  Cast(winTpSendCountFloatTensor_, gmTpSendCountTensor_, RoundMode::CAST_NONE,
       axisH_);
  PipeBarrier<PIPE_V>();
  Abs(absFloatTensor_, winTpSendCountFloatTensor_,
      axisH_);  // absFloatTensor_ align到256并写0，支持ReduceMax与Brcb
  PipeBarrier<PIPE_V>();
  BlockReduceMax(reduceMaxFloatTensor_, absFloatTensor_, repeatNum_, mask_, 1,
                 1, BLOCK_NUM);  // 32->1 256->8
  PipeBarrier<PIPE_V>();
  Muls(reduceMaxFloatTensor_, reduceMaxFloatTensor_, scaleValFloat_,
       scaleNum_);  // 有效个数
  PipeBarrier<PIPE_V>();
  Cast(scaleDivTensor_, reduceMaxFloatTensor_, RoundMode::CAST_RINT,
       scaleNum_);  // 有效个数
  PipeBarrier<PIPE_V>();
  Brcb(scaleDupLocalTensor_, reduceMaxFloatTensor_, repeatNum_,
       {1, BLOCK_NUM});  // 一次256
  PipeBarrier<PIPE_V>();
  Div(winTpSendCountFloatTensor_, winTpSendCountFloatTensor_,
      scaleDupLocalTensor_, axisH_);  // 有效个数
  PipeBarrier<PIPE_V>();
  Cast(fp16CastTensor_, winTpSendCountFloatTensor_, RoundMode::CAST_RINT,
       axisH_);
  PipeBarrier<PIPE_V>();
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::ExpertAlltoAllDispatchInnerCopyAdd(
  ...
  DataCopyExtParams expandXCopyParams{
      1U, static_cast<uint32_t>(hExpandXTypeSize_), 0U, 0U, 0U};
  DataCopyExtParams xScaleCopyParams{
      1U, static_cast<uint32_t>(tokenScaleCnt_ * sizeof(ExpandXType)), 0U, 0U,
      0U};
  if constexpr (IsNeedReduceScatter) {
    ...
    DataCopyPad(winTpSendCountTensor_, tpRankWindow_[tokenWinOffset],
                expandXCopyParams, copyPadExtParams);
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeCombineV2<TemplateMC2TypeFunc>::CustomAdd(
    LocalTensor<XType> &dst, LocalTensor<XType> &src0,
    LocalTensor<XType> &src1) {
  if constexpr (AscendC::IsSameType<XType, bfloat16_t>::value) {
    ...
    PipeBarrier<PIPE_V>();
    Add(winTpSendCountFloatTensor_, winTpSendCountFloatTensor_,
        gmTpSendCountFloatTensor_, axisH_);
    PipeBarrier<PIPE_V>();
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::WaitDispatch(uint32_t tokenIndex) {
  uint32_t copyCount = flagRcvCount_ * FLOAT_PER_UB_ALIGN;
  uint32_t targetCount = copyCount;
  if (isInputExpertMaskFlag_ ||
      ((zeroExpertNum_ + copyExpertNum_ + constExpertNum_) > 0U)) {
    int32_t tokenTarget =
        static_cast<int32_t>(tokenTargetTensor_.GetValue(tokenIndex)) +
        sharedExpertNum_;
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::Int8DequantProcess(
    LocalTensor<XType> &src) {
  SyncFunc<AscendC::HardEvent::MTE2_V>();
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::ProcessConstantExpert(
  ...
  DataCopyPad(const_v_ub, constExpertVGM_[const_expert_idx * axisH_],
              expandXCopyParams, copyPadExtParams);
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::ProcessCopyExpert(
    uint32_t tokenIndex, float scaleVal) {
  DataCopyPadExtParams<ExpandXType> copyPadExtParams{false, 0U, 0U, 0U};
  DataCopyExtParams expandXCopyParams{
      1U, static_cast<uint32_t>(hExpandXTypeSize_), 0U, 0U, 0U};
  LocalTensor<ExpandXType> tmpUb = moeSumQueue_.AllocTensor<ExpandXType>();
  DataCopyPad(tmpUb, oriXGM_[tokenIndex * axisH_], expandXCopyParams,
              copyPadExtParams);
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::ProcessMoeExpert(
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeCombineV2<TemplateMC2TypeFunc>::LocalWindowCopy() {
  ...
  for (uint32_t curIdx = beginIndex; curIdx < endIndex; curIdx++) {
    ...
    for (uint32_t topkId = 0U; topkId < axisK_; topkId++) {
      ...
      if (isInputExpertMaskFlag_) {
        bool maskExpertFlag =
            expertMaskTensor_.GetValue(tokenIndex * axisK_ + topkId);
        if (!maskExpertFlag) {
          index++;
          continue;
        }
      ...
      if (expert_id < moeExpertNum_) {
        ...
        index++;
                                 ...
                                 constExpertNum_) {
        uint32_t const_expert_idx =
            expert_id - (moeExpertNum_ + zeroExpertNum_ + copyExpertNum_);
        ProcessConstantExpert(tokenIndex, const_expert_idx, scaleVal);
        index++;
      }
    }
    for (uint32_t topkId = axisK_; topkId < (axisK_ + sharedExpertNum_);
         topkId++) {
      ...
      Cast(rowTmpFloatLocal_, tmpUb, AscendC::RoundMode::CAST_NONE, processLen);
      PipeBarrier<PIPE_V>();
      AscendC::Add(sumFloatBufLocal_, sumFloatBufLocal_, rowTmpFloatLocal_,
                   processLen);
      PipeBarrier<PIPE_V>();
      moeSumQueue_.FreeTensor<XType>(tmpUb);
    }
    if (hasSharedExpertX_) {
      ...
      DataCopyPad(rowTmpLocal, sharedExpertXGM_[tokenIndex * axisH_],
                  expandXCopyParams, copyPadExtParams);
      SyncFunc<AscendC::HardEvent::MTE2_V>();
      Cast(rowTmpFloatLocal_, rowTmpLocal, AscendC::RoundMode::CAST_NONE,
           processLen);
      PipeBarrier<PIPE_V>();
      AscendC::Add(sumFloatBufLocal_, sumFloatBufLocal_, rowTmpFloatLocal_,
                   processLen);
