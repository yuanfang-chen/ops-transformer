...
static void PrintTilingDataInfo(const char *nodeName,
  ...
  OP_LOGD(nodeName, "tpWorldSize is %u.",
          tilingData.moeDistributeCombineV2Info.tpWorldSize);
  OP_LOGD(nodeName, "epRankId is %u.",
          tilingData.moeDistributeCombineV2Info.epRankId);
  OP_LOGD(nodeName, "tpRankId is %u.",
          tilingData.moeDistributeCombineV2Info.tpRankId);
  OP_LOGD(nodeName, "expertShardType is %u.",
          tilingData.moeDistributeCombineV2Info.expertShardType);
  OP_LOGD(nodeName, "sharedExpertNum is %u.",
          tilingData.moeDistributeCombineV2Info.sharedExpertNum);
  OP_LOGD(nodeName, "sharedExpertRankNum is %u.",
          tilingData.moeDistributeCombineV2Info.sharedExpertRankNum);
  OP_LOGD(nodeName, "moeExpertNum is %u.",
          tilingData.moeDistributeCombineV2Info.moeExpertNum);
  OP_LOGD(nodeName, "moeExpertPerRankNum is %u.",
          tilingData.moeDistributeCombineV2Info.moeExpertPerRankNum);
  OP_LOGD(nodeName, "globalBs is %u.",
          tilingData.moeDistributeCombineV2Info.globalBs);
  ...
  OP_LOGD(nodeName, "aivNum is %u.",
          tilingData.moeDistributeCombineV2Info.aivNum);
  OP_LOGD(nodeName, "totalUbSize is %lu.",
          tilingData.moeDistributeCombineV2Info.totalUbSize);
  OP_LOGD(nodeName, "totalWinSize is %lu.",
          tilingData.moeDistributeCombineV2Info.totalWinSize);
  OP_LOGD(nodeName, "hasElastic is %d.",
          tilingData.moeDistributeCombineV2Info.hasElasticInfo);
}
...
static ge::graphStatus GetAttrAndSetTilingData(
  ...
  tilingData.moeDistributeCombineV2Info.zeroExpertNum =
      static_cast<uint32_t>(zeroExpertNum);
  tilingData.moeDistributeCombineV2Info.copyExpertNum =
      static_cast<uint32_t>(copyExpertNum);
  tilingData.moeDistributeCombineV2Info.constExpertNum =
...
static bool CheckTensorShape(const gert::TilingContext *context,
  ...
  int64_t moeExpertNum =
      static_cast<int64_t>(tilingData.moeDistributeCombineV2Info.moeExpertNum);
  int64_t zeroExpertNum =
      static_cast<int64_t>(tilingData.moeDistributeCombineV2Info.zeroExpertNum);
  int64_t copyExpertNum =
      static_cast<int64_t>(tilingData.moeDistributeCombineV2Info.copyExpertNum);
  ...
  OP_TILING_CHECK(
      ...
      OP_LOGE(nodeName,
              ...
              expertIdsDim1),
      return false);
  tilingData.moeDistributeCombineV2Info.k =
  ...
  const int64_t epWorldSize =
      static_cast<int64_t>(tilingData.moeDistributeCombineV2Info.epWorldSize);
  if (hasElasticInfo) {
    const gert::StorageShape *elasticInfoStorageShape =
        context->GetOptionalInputShape(ELASTIC_INFO_INDEX);
  ...
  // 校验expandX的维度并设h
  int64_t tpWorldSize =
      static_cast<int64_t>(tilingData.moeDistributeCombineV2Info.tpWorldSize);
  const gert::StorageShape *expandXStorageShape =
      context->GetInputShape(EXPAND_X_INDEX);
  ...
  const gert::StorageShape *sharedExpertXShape =
      context->GetOptionalInputShape(SHARED_EXPERT_X_INDEX);
  tilingData.moeDistributeCombineV2Info.hasSharedExpertX =
...
static bool CheckSharedAttrs(
    const char *nodeName, const MoeDistributeCombineV2TilingData &tilingData) {
  uint32_t sharedExpertNum =
      tilingData.moeDistributeCombineV2Info.sharedExpertNum;
  uint32_t sharedExpertRankNum =
      tilingData.moeDistributeCombineV2Info.sharedExpertRankNum;
...
static bool CheckAttrs(const gert::TilingContext *context,
  ...
  OP_TILING_CHECK(
      ...
      return false);
  tilingData.moeDistributeCombineV2Info.bs =
  ...
  OP_LOGD(
      nodeName,
      "MoeDistributeCombineV2 *globalBsPtr = %ld, bs = %ld, epWorldSize = %u\n",
...
static ge::graphStatus MoeDistributeCombineA3TilingFuncImpl(
  ...
  MoeDistributeCombineV2TilingData *tilingData =
      context->GetTilingData<MoeDistributeCombineV2TilingData>();
  ...
  uint64_t epWorldSize =
      static_cast<uint64_t>(tilingData->moeDistributeCombineV2Info.epWorldSize);
  uint64_t k = static_cast<uint64_t>(tilingData->moeDistributeCombineV2Info.k);
  uint64_t maxBs =
      static_cast<uint64_t>(tilingData->moeDistributeCombineV2Info.globalBs) /
      epWorldSize;
  ...
  uint64_t tpWorldSize =
      static_cast<uint64_t>(tilingData->moeDistributeCombineV2Info.tpWorldSize);
...
static ge::graphStatus MoeDistributeCombineV2TilingFunc(
    gert::TilingContext *context) {
...
ge::graphStatus TilingParseForMoeDistributeCombineV2(
...
IMPL_OP_OPTILING(MoeDistributeCombineV2)
    ...
    .TilingParse<MoeDistributeCombineCompileInfo>(
        TilingParseForMoeDistributeCombineV2);
}  // namespace optiling