...
/*!
 * \file moe_distribute_dispatch_shmem_def.cpp
 * \brief
 */
...
namespace ops {
class MoeDistributeDispatchShmem : public OpDef {
public:
  explicit MoeDistributeDispatchShmem(const char* name) : OpDef(name) {
    this->Input("shmem_space")
        .ParamType(REQUIRED)
    ...
    this->Attr("shmem_size").AttrType(OPTIONAL).Int(0);
...
OP_ADD(MoeDistributeDispatchShmem);
