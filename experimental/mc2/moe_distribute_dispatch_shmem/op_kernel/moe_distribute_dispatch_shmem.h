...
/*!
 * \file moe_distribute_dispatch_shmem.h
 * \brief
 */
#ifndef MOE_DISTRIBUTE_DISPATCH_SHMEM_H
...
namespace MoeDistributeDispatchShmemImpl {
...
class MoeDistributeDispatchShmem {
 public:
  __aicore__ inline MoeDistributeDispatchShmem(){};
  __aicore__ inline void Init(
      GM_ADDR shmemSpace, GM_ADDR x, GM_ADDR expertIds, GM_ADDR scales,
      GM_ADDR xActiveMask, GM_ADDR elasticInfo, GM_ADDR expandXOut,
      ...
      const MoeDistributeDispatchShmemTilingData *tilingData);
 ...
 private:
  ...
  __aicore__ inline void ReduceMaxInplace(const LocalTensor<float> &srcLocal,
                                          uint32_t count);
  ...
  __aicore__ inline void InitElasticInfo(
      GM_ADDR elasticInfo, const MoeDistributeDispatchShmemTilingData *tilingData);
  __aicore__ inline void WaitDispatch();
  __aicore__ inline void GetCumSum(LocalTensor<int32_t> &outLocal,
                                   uint32_t totalCount);
  ...
  __aicore__ inline void SplitToCore(uint32_t curSendCnt, uint32_t curUseAivNum,
                                     uint32_t &startTokenId,
                                     uint32_t &endTokenId,
                                     uint32_t &sendTokenNum,
                                     bool isFront = true);
  __aicore__ inline void FillTriple(LocalTensor<ExpandXOutType> &xOutTensor,
                                    uint32_t tokenIndex, uint32_t k);
  __aicore__ inline void CalTokenSendExpertCnt(uint32_t dstExpertId,
                                               int32_t calCnt,
  ...
  __aicore__ inline GM_ADDR GetWindAddrByRankId(uint8_t ctxIdx,
                                                const int32_t rankId) {
    return (GM_ADDR)GetShmemDataAddr(shmemContextGM_, rankId) + winDataSizeOffset_;
  }

  __aicore__ inline GM_ADDR GetWindStateAddrByRankId(uint8_t ctxIdx,
                                                     const int32_t rankId) {
...
__aicore__ inline int32_t ReduceSumWorkNeedSize(int32_t calCnt) {
  ...
  int32_t iter1AlignEnd =
      ((iter1OutputCount + elementsPerBlock - 1) / elementsPerBlock) *
      elementsPerBlock;
...
uint32_t moeExpertRankNum_{
    0};  // moe专家卡数，等于epWorldSize_ - sharedExpertRankNum_
...
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::InitElasticInfo(
    GM_ADDR elasticInfo, const MoeDistributeDispatchShmemTilingData *tilingData) {
  ...
  DataCopyPad(elasticInfoTensor_, elasticInfoGMTensor_, elasticInfoParams,
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::Init(
    GM_ADDR shmemSpace, GM_ADDR x, GM_ADDR expertIds, GM_ADDR scales,
    GM_ADDR xActiveMask, GM_ADDR elasticInfo, GM_ADDR expandXOut,
    GM_ADDR dynamicScalesOut, GM_ADDR expandIdxOut, GM_ADDR expertTokenNumsOut,
    GM_ADDR sendCountsOut, GM_ADDR tpSendCountsOut, GM_ADDR workspaceGM,
    TPipe *pipe, const MoeDistributeDispatchShmemTilingData *tilingData) {
  tpipe_ = pipe;
  aivId_ = GetBlockIdx();
  epRankId_ = tilingData->moeDistributeDispatchShmemInfo.epRankId;
  epRankIdOriginal_ = tilingData->moeDistributeDispatchShmemInfo.epRankId;
  shmemContextGM_ = shmemSpace;

  // 检查hcclwinsize是否越界
  totalWinSize_ = static_cast<uint64_t>(
      tilingData->moeDistributeDispatchShmemInfo.totalWinSize);

  axisBS_ = tilingData->moeDistributeDispatchShmemInfo.bs;
  axisH_ = tilingData->moeDistributeDispatchShmemInfo.h;
  epWorldSizeOriginal_ = tilingData->moeDistributeDispatchShmemInfo.epWorldSize;
  hasElasticInfoFlag_ = tilingData->moeDistributeDispatchShmemInfo.hasElasticInfo;
  epWorldSize_ = tilingData->moeDistributeDispatchShmemInfo.epWorldSize;
  sharedExpertRankNum_ =
      tilingData->moeDistributeDispatchShmemInfo.sharedExpertRankNum;
  moeExpertNum_ = tilingData->moeDistributeDispatchShmemInfo.moeExpertNum;
  ...
  axisMaxBS_ =
      tilingData->moeDistributeDispatchShmemInfo.globalBs / epWorldSizeOriginal_;
  sharedExpertNum_ = tilingData->moeDistributeDispatchShmemInfo.sharedExpertNum;
  if (sharedExpertNum_ > 0) {
    rankNumPerSharedExpert_ = sharedExpertRankNum_ / sharedExpertNum_;
  }
  expertTokenNumsType_ =
  ...
  tpRankId_ = tilingData->moeDistributeDispatchShmemInfo.tpRankId;
  tpGatherRankId_ = ((tpRankId_ == 0) ? 1 : 0);
  isTokenMaskFlag_ = tilingData->moeDistributeDispatchShmemInfo.isTokenMask;
  isExpertMaskFlag_ = tilingData->moeDistributeDispatchShmemInfo.isExpertMask;
  axisK_ = tilingData->moeDistributeDispatchShmemInfo.k;
  aivNum_ = tilingData->moeDistributeDispatchShmemInfo.aivNum;
  tpWorldSize_ = tilingData->moeDistributeDispatchShmemInfo.tpWorldSize;
  xGMTensor_.SetGlobalBuffer((__gm__ XType *)x);
  xActiveMaskGMTensor_.SetGlobalBuffer((__gm__ bool *)xActiveMask);
  expertIdsGMTensor_.SetGlobalBuffer((__gm__ int32_t *)expertIds);
  dynamicScalesOutGMTensor_.SetGlobalBuffer((__gm__ float *)dynamicScalesOut);
  expertTokenNumsOutGMTensor_.SetGlobalBuffer(
      (__gm__ int64_t *)expertTokenNumsOut);
  ...
  hAlignWinSize_ = Ceil(hScaleIdxSize, WIN_ADDR_ALIGN) *
                   WIN_ADDR_ALIGN;  // win区token起始地址对齐512
  ...
  if (sharedExpertRankNum_ != 0U) {
    sharedUsedAivNum_ =
        (aivNum_ * sharedExpertNum_) / (axisK_ + sharedExpertNum_);
  ...
  datastateLocalTensor_.SetValue(
      2, tilingData->moeDistributeDispatchShmemInfo.epRankId);
  datastateLocalTensor_.SetValue(
      3, tilingData->moeDistributeDispatchShmemInfo.moeExpertNum);
  datastateLocalTensor_.SetValue(
      4, tilingData->moeDistributeDispatchShmemInfo.epWorldSize);
  datastateLocalTensor_.SetValue(
      5, tilingData->moeDistributeDispatchShmemInfo.globalBs);
  ...
  startStatusIndex_ =
      recStatusNumPerCore_ * aivId_;  // + sharedExpertRankNum_, 每个aiv发送的
  if (aivId_ <
      remainderRankNum_) {  // 前remainderRankNum个aiv需要多发1个卡的数据
  ...
  uint32_t statusBufCntAlign = Ceil(Ceil(totalExpertNum_, aivNum_), 8) *
                               8;  // 8 = UB_ALIGN / sizeof(int32_t)
  uint32_t statusBufSize = statusBufCntAlign * UB_ALIGN;
  tpipe_->InitBuffer(statusBuf_, statusBufSize);
  totalUsedUB_ += statusBufSize;
  statusTensor_ =
      statusBuf_
          .Get<int32_t>();  // 保存发送数据量及flag，同时用于计算windows中的偏移
  Duplicate<int32_t>(statusTensor_, 0,
                     recvWinBlockNum_ * 8);  // 8 = UB_ALIGN / sizeof(int32_t)
  ...
  for (int tempepRankId = 0; tempepRankId < epWorldSize_; tempepRankId++) {
    OOMCheckAddrRange<ExpandXOutType>(
        (__gm__ ExpandXOutType *)(GetWindAddrByRankId(COMM_EP_IDX,
                                                      tempepRankId)),
        totalWinSize_);
    OOMCheckAddrRange<float>(
        (__gm__ float *)(GetWindStateAddrByRankId(COMM_EP_IDX, tempepRankId)),
        STATE_SIZE);
  }
#endif
  ...
  uint64_t mask[2] = {
      0x101010101010101,
      0};  // 一次性操作256字节，也是64个int32_t，每8个数将首个设置为0x3F800000
  PipeBarrier<PIPE_V>();
  Duplicate<int32_t>(statusTensor_, 0x3F800000, mask, statusBufCntAlign / 8, 1,
                     8);  // 0x3F800000是float的1

  // 当前tpWin区划分为前后两半区，连续两次dispatch，切换半区, combine
  // 数据区使用前面， 即axisMaxBS_ * (axisK_ + sharedExpertNum_) *
  // hSizeAlignCombine, dispatch使用后面
  uint64_t hSizeAlignCombine =
      Ceil(axisH_ * sizeof(XType), WIN_ADDR_ALIGN) * WIN_ADDR_ALIGN;
  winDataSizeOffset_ =
      dataState_ * (tilingData->moeDistributeDispatchShmemInfo.totalWinSize / 2) +
  ...
  OOMCheckAddrRange<ExpandXOutType>(
      (__gm__ ExpandXOutType *)(winDouble.GetPhyAddr()), totalWinSize_);
#endif
  if constexpr (IsNeedAllgather) {
#if defined(ASCENDC_OOM) && ASCENDC_OOM == 1
    for (int temptpRankId = 0; temptpRankId < tpWorldSize_; temptpRankId++) {
      OOMCheckAddrRange<ExpandXOutType>(
          (__gm__ ExpandXOutType *)(GetWindAddrByRankId(COMM_TP_IDX,
                                                        temptpRankId)),
          totalWinSize_);
      OOMCheckAddrRange<int32_t>((__gm__ int32_t *)(GetWindStateAddrByRankId(
                                     COMM_TP_IDX, temptpRankId)),
                                 STATE_SIZE);
    }
#endif
  ...
  if constexpr (IsNeedAllgather) {
    winTpGatherOutGMTensor_.SetGlobalBuffer(
        (__gm__ ExpandXOutType *)tpWindowGM_);
    fpWinTpGatherOutGMTensor_.SetGlobalBuffer((__gm__ float *)tpWindowGM_);
    winTpEpCntGMTensor_.SetGlobalBuffer(
        (__gm__ int32_t *)(tpStatusWindowGM_ + TP_STATE_SIZE));
  }
  ...
  uint32_t xActivateMaskSize =
      axisBS_ * (Ceil(axisK_ * sizeof(bool), UB_ALIGN) * UB_ALIGN) *
      sizeof(half);
  uint32_t bsAlign256 = Ceil(axisBS_ * sizeof(half), ALIGNED_LEN_256) *
                        ALIGNED_LEN_256 / sizeof(half);
  uint32_t bsKAlign256 = Ceil(expertIdsCnt_ * sizeof(half), ALIGNED_LEN_256) *
                         ALIGNED_LEN_256 / sizeof(half);
  uint32_t expertIdsBufSize =
      expertIdsSize > bsAlign256 ? expertIdsSize : bsAlign256;
  ...
  if (isExpertMaskFlag_ || (zeroComputeExpertNum_ != 0)) {
    ...
    uint32_t validBufferSize =
        expertIdsSize > xActivateMaskSize ? expertIdsSize : xActivateMaskSize;
  ...
  if constexpr (DynamicQuant || StaticQuant) {
    ...
    tpipe_->InitBuffer(xQueue_, bufferNum_,
                       hOutAlignUbSize_);  // 7k*2 + 32 + 12
  }
  ...
  xCopyParams_ = {1U, static_cast<uint32_t>(axisH_ * sizeof(XType)), 0U, 0U,
                  0U};
  hCommuCopyOutParams_ = {
      1U, static_cast<uint32_t>(axisHCommu * sizeof(ExpandXOutType)), 0U, 0U,
      0U};
  expandXCopyParams_ = {
      1U, static_cast<uint32_t>(axisH_ * sizeof(ExpandXOutType)), 0U, 0U, 0U};
}

template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::QuantInit(
    GM_ADDR scales) {
  uint32_t hAlignSize = Ceil(axisH_ * sizeof(XType), UB_ALIGN) * UB_ALIGN;
  tpipe_->InitBuffer(receiveDataCastFloatBuf_,
                     maxSize_);  // max{28K, BS * K * 4B}
  totalUsedUB_ += maxSize_;
  ...
  uint32_t tmpTotalUB =
      totalUsedUB_ + BUFFER_NUM * hAlignSize + hOutAlignUbSize_ * BUFFER_NUM;
  bufferNum_ = tmpTotalUB > MAX_UB_SIZE ? BUFFER_SINGLE : BUFFER_NUM;
  tpipe_->InitBuffer(xInQueue_, bufferNum_, hAlignSize);  // 14K * 2
  tpipe_->InitBuffer(xOutQueue_, bufferNum_,
                     hOutAlignUbSize_);  // 7K * 2 + 32 + 6
}

template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::SplitToCore(
  ...
  if (isFront) {
    ...
    newAivId =
        aivId_ -
        moeUsedAivNum_;  // 由于是后面的核作为发送的共享专家，因此需要换算
  }
  startTokenId = sendTokenNum * newAivId;  // 每个aiv发送时的起始rankid
  if (newAivId <
      remainderTokenNum) {  // 前remainderRankNum个aiv需要多发1个卡的数据
    sendTokenNum += 1;
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::FillTriple(
    LocalTensor<ExpandXOutType> &xOutTensor, uint32_t tokenIndex, uint32_t k) {
  SyncFunc<AscendC::HardEvent::MTE3_S>();
  LocalTensor<int32_t> xOutTint32 =
      xOutTensor.template ReinterpretCast<int32_t>();
...
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::SendToSharedExpert() {
  ...
  for (uint32_t virtualTokenIndex = startTokenId;
    ...
    if constexpr (DynamicQuant || StaticQuant) {
      ...
      DataCopyPad(xInTensor_, xGMTensor_[srcTokenIndex * axisH_], xCopyParams_,
                  copyPadExtParams);
      ...
      DataCopyPad(dstWinGMTensor[tokenIndex * hAlignWinCnt_], xOutTensor_,
                  hCommuCopyOutParams_);
      xOutQueue_.FreeTensor(xOutTensor_);
    } else {
      xTmpTensor_ = xQueue_.AllocTensor<ExpandXOutType>();
      DataCopyPad(xTmpTensor_, xGMTensor_[srcTokenIndex * axisH_],
                  expandXCopyParams_, copyPadExtParams);
      xQueue_.EnQue(xTmpTensor_);
      xTmpTensor_ = xQueue_.DeQue<ExpandXOutType>();
      FillTriple(xTmpTensor_, srcTokenIndex, axisK_ + toSharedExpertIndex);
      DataCopyPad(dstWinGMTensor[tokenIndex * hAlignWinCnt_], xTmpTensor_,
                  hCommuCopyOutParams_);
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::CalTokenSendExpertCnt(
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::SendToMoeExpert() {
  uint32_t startTokenId, endTokenId, sendTokenNum;  // 每个aiv发送时的起始rankid
  SplitToCore(sendToMoeExpTokenCnt_, moeUsedAivNum_, startTokenId, endTokenId,
              sendTokenNum);
  ...
  for (int32_t index = startTokenId; index < endTokenId; ++index) {
    ...
    int32_t toRankId =
        dstExpertId / moeExpertNumPerRank_ + sharedExpertRankNum_;
    ...
    GM_ADDR rankGM =
        (__gm__ uint8_t *)(GetWindAddrByRankId(COMM_EP_IDX, toRankId) +
                           (expertPerSizeOnWin_ *
    ...
    if constexpr (DynamicQuant || StaticQuant) {
      xInTensor_ = xInQueue_.AllocTensor<XType>();
      DataCopyPad(xInTensor_, xGMTensor_[tokenIndex * axisH_], xCopyParams_,
                  copyPadExtParams);
      xInQueue_.EnQue(xInTensor_);
      xInTensor_ = xInQueue_.DeQue<XType>();
      xOutTensor_ = xOutQueue_.AllocTensor<ExpandXOutType>();
      ...
      xTmpTensor_ = xQueue_.AllocTensor<ExpandXOutType>();
      DataCopyPad(xTmpTensor_, xGMTensor_[tokenIndex * axisH_], xCopyParams_,
                  copyPadExtParams);
      xQueue_.EnQue(xTmpTensor_);
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::TokenActiveMaskCal() {
  ...
  DataCopyPad(maskInputTensor, xActiveMaskGMTensor_, maskParams,
              maskCopyPadParams);
  SyncFunc<AscendC::HardEvent::MTE2_V>();
  LocalTensor<int8_t> maskInputInt8Tensor =
      maskInputTensor.ReinterpretCast<int8_t>();
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::CalValidExpIdx(
    LocalTensor<bool> maskInputTensor) {
  uint32_t mask = expertIdsCnt_;
  uint32_t curMaskCnt = axisBS_ * axisK_;
  uint32_t calCnt = Ceil(curMaskCnt * sizeof(half), ALIGNED_LEN_256) *
                    ALIGNED_LEN_256 / sizeof(half);
  ...
  Duplicate<uint32_t>(gatherMaskTensor_, 0,
                      Ceil(expertIdsCnt_, ALIGNED_LEN_256) * ALIGNED_LEN_256 /
                          BITS_PER_BYTE / sizeof(uint32_t));
  PipeBarrier<PIPE_V>();
  CompareScalar(gatherMaskTensorInt8, tempTensor, static_cast<half>(1),
                AscendC::CMPMODE::EQ, calCnt);
  CreateVecIndex(expertsIndexTensor, 0, curMaskCnt);
  PipeBarrier<PIPE_V>();
  GatherMask(validExpertIndexTensor_, expertsIndexTensor, gatherMaskTensor_,
             true, mask, {1, 1, 0, 0}, sendToMoeExpTokenCnt_);
}

template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::CalValidBSCnt(
    LocalTensor<bool> maskStrideTensor) {
  uint64_t rsvdCnt = 0;
  uint32_t mask = axisBS_;
  uint32_t activeMaskAlignSize =
      axisBS_ * (Ceil(axisK_ * sizeof(bool), UB_ALIGN) * UB_ALIGN);
  uint32_t calCnt = Ceil(axisBS_ * sizeof(half), ALIGNED_LEN_256) *
                    ALIGNED_LEN_256 / sizeof(half);
  uint32_t innerAlign = Ceil(axisK_ * sizeof(half), UB_ALIGN) * UB_ALIGN /
                        sizeof(half) * BUFFER_NUM;
  ...
  SumParams axisBsSumParams{
      1,
      static_cast<uint32_t>(Ceil(axisBS_ * sizeof(half), UB_ALIGN) * UB_ALIGN /
                            sizeof(half)),
      axisBS_};

  Duplicate<half>(maskTempTensor, (half)0, calCnt);
  SyncFunc<AscendC::HardEvent::MTE2_V>();
  LocalTensor<int8_t> maskStrideInt8Tensor =
      maskStrideTensor.ReinterpretCast<int8_t>();
  Cast(tempTensor, maskStrideInt8Tensor, RoundMode::CAST_NONE,
       activeMaskAlignSize);
  PipeBarrier<PIPE_V>();
  ...
  PipeBarrier<PIPE_V>();
  GatherMask(validBsIndexTensor_, bsIndexTensor, maskTensorInt32, true, mask,
             {1, 1, 0, 0}, activeMaskBsCnt_);
}

template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::ExpertActiveMaskCal() {
  // 计算当前有效bs数量, stride搬入xActiveMask进行sum计算, 用于moe专家发送
  LocalTensor<bool> maskStrideTensor = dstExpBuf_.Get<bool>();
  DataCopyPadExtParams<bool> maskStrideCopyPadParams{
      true, 0U, static_cast<uint8_t>(UB_ALIGN - axisK_), 0U};
  DataCopyExtParams maskStrideParams{
      static_cast<uint16_t>(axisBS_),
      static_cast<uint32_t>(axisK_ * sizeof(bool)), 0U, 0U, 0U};
  DataCopyPad(maskStrideTensor, xActiveMaskGMTensor_, maskStrideParams,
              maskStrideCopyPadParams);
  CalValidBSCnt(maskStrideTensor);
  // 计算validExpIndexTensor, 连续搬入xActiveMask进行GatherMask计算,
  // 用于moe专家的发送
  LocalTensor<bool> maskInputTensor = dstExpBuf_.Get<bool>();
  DataCopyPadExtParams<bool> maskCopyPadParams{false, 0U, 0U, 0U};
  DataCopyExtParams maskParams{
      1U, static_cast<uint32_t>(expertIdsCnt_ * sizeof(bool)), 0U, 0U, 0U};
  DataCopyPad(maskInputTensor, xActiveMaskGMTensor_, maskParams,
              maskCopyPadParams);
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::MaskZeroComputeExpert(
    uint32_t maskCnt) {
  LocalTensor<int32_t> expertsIndexTensor = dstExpBuf_.Get<int32_t>();
  int32_t maskTensorInt16Cnt = Ceil(expertIdsCnt_, UB_ALIGN / 2);
  LocalTensor<uint8_t> maskTensorInt8 =
      validExpertIndexBuf_.Get<uint8_t>();  // bs*k*1
  LocalTensor<uint32_t> maskTensorInt32 =
      validExpertIndexBuf_.Get<uint32_t>();                        // bs*k*1
  ...
  DataCopyExtParams expertIdsCntParams = {
      1U, static_cast<uint32_t>(expertIdsCnt_ * sizeof(uint32_t)), 0U, 0U, 0U};
  DataCopyPadExtParams<int32_t> expertIdsCntCopyPadParams{false, 0U, 0U, 0U};
  DataCopyPad(expertIdsTensor_, expertIdsGMTensor_, expertIdsCntParams,
              expertIdsCntCopyPadParams);
  SyncFunc<AscendC::HardEvent::MTE2_V>();
  PipeBarrier<PIPE_V>();
  SetDeqScale((half)1.000000e+00f);
  PipeBarrier<PIPE_V>();
  Cast(expertIdsTensorCast, expertIdsTensor_, RoundMode::CAST_NONE,
       expertIdsCnt_);
  PipeBarrier<PIPE_V>();
  Duplicate<uint32_t>(maskTensorInt32, 0, Ceil(expertIdsCnt_, UB_ALIGN));
  PipeBarrier<PIPE_V>();
  uint32_t calcCnt = Ceil(expertIdsCnt_ * sizeof(half), ALIGNED_LEN_256) *
                     ALIGNED_LEN_256 / sizeof(half);
  // 逐元素比较一个tensor中的元素和另一个Scalar的大小，如果比较后的结果为真，则输出结果的对应比特位为1，否则为0。筛掉零计算量专家
  CompareScalar(maskTensorInt8, expertIdsTensorCast,
                static_cast<half>(moeExpertNumInt32), AscendC::CMPMODE::LT,
                calcCnt);
  PipeBarrier<PIPE_V>();
  LocalTensor<uint16_t> maskTensorInt16 =
      validExpertIndexBuf_.Get<uint16_t>();  // 空间bs*k*1
  LocalTensor<uint16_t> gatherMaskTensorint16 =
      gatherMaskTBuf_.Get<uint16_t>();  // 空间bs*k*4
  /* 特殊专家的maskTensorInt16和之前的gatherMaskTensor_结果按位相与，AND
   * 支持uint16， gatherMaskTensor_和gatherMaskTensorint16是同一个地址 */
  And(gatherMaskTensorint16, gatherMaskTensorint16, maskTensorInt16,
      maskTensorInt16Cnt);
  PipeBarrier<PIPE_V>();
  // 再筛一次
  ...
  PipeBarrier<PIPE_V>();
  GatherMask(validExpertIndexTensor_, expertsIndexTensor, gatherMaskTensor_,
             true, maskCnt, {1, 1, 0, 0}, sendToMoeExpTokenCnt_);
  SyncFunc<AscendC::HardEvent::V_S>();
}

template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::GenerateGatherMaskTensor(
    uint32_t maskCnt) {
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::ZeroComputeExpertMaskCal() {
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::AlltoAllDispatch() {
  ...
  if (isExpertMaskFlag_) {
    ...
    DataCopyPad(tmpExpertIdsTensor, expertIdsGMTensor_, expertIdsCntParams,
                expertIdsCntCopyPadParams);
    SyncFunc<AscendC::HardEvent::MTE2_V>();
    GatherMask(expertIdsTensor_, tmpExpertIdsTensor, gatherMaskTensor_, true,
               mask, {1, 1, 1, 0}, rsvdCnt);
  } else {
    DataCopyPad(expertIdsTensor_, expertIdsGMTensor_, expertIdsCntParams,
                expertIdsCntCopyPadParams);
    SyncFunc<AscendC::HardEvent::MTE2_S>();
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::SetStatus() {
  SplitToCore(totalExpertNum_, aivNum_, startExpertId_, endExpertId_,
              sendExpertNum_);

  for (uint32_t curExpertId = startExpertId_; curExpertId < endExpertId_;
       ++curExpertId) {
    int32_t curExpertCnt = 0;
    ...
    if ((curExpertId < sharedExpertRankNum_) &&
        (activeMaskBsCnt_ > 0)) {  // 当前处理专家为共享专家 shared:Cnt -> win
                                   // -> LocalCopy(moe+share)
      if (curExpertId % rankNumPerSharedExpert_ ==
          epRankId_ % rankNumPerSharedExpert_) {
      ...
      CalTokenSendExpertCnt(curMoeExpertId, sendToMoeExpTokenCnt_,
                            curExpertCnt);
    }
  ...
  for (uint32_t expertIndex = startExpertId_; expertIndex < endExpertId_;
       ++expertIndex) {
    uint32_t dstRankId = expertIndex;
    uint32_t offset = stateOffset_ * epRankId_;
    if (expertIndex >= sharedExpertRankNum_) {
      dstRankId = ((expertIndex - sharedExpertRankNum_) / moeExpertNumPerRank_ +
                   sharedExpertRankNum_);
      offset += ((expertIndex - sharedExpertRankNum_) % moeExpertNumPerRank_ *
                 epWorldSize_ * stateOffset_);
    }
    if (isScalingDownFlag_) {
      dstRankId = elasticInfoTensor_.GetValue(ELASTIC_INFO_OFFSET +
                                              epWorldSizeOriginal_ + dstRankId);
    }
    GM_ADDR rankGM =
        (__gm__ uint8_t *)(GetWindStateAddrByRankId(COMM_EP_IDX, dstRankId) +
                           offset);  // 计算地址偏移
    rankGMTensor.SetGlobalBuffer((__gm__ int32_t *)rankGM);
    // 按32对齐拷贝，8是32字节包含的元素个数, 本卡数据需要去掉起始index偏移
    DataCopy<int32_t>(rankGMTensor,
                      statusTensor_[(expertIndex - startExpertId_) * 8], 8UL);
  }
  SyncFunc<AscendC::HardEvent::MTE3_S>();
}
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::ReduceMaxInplace(
  ...
  if (likely(repsFp32 > 1)) {
    // 8 is rep stride
    Max(srcLocal, srcLocal[elemPerRefFp32], srcLocal, elemPerRefFp32,
        repsFp32 - 1, {1, 1, 1, 0, 8, 0});
    PipeBarrier<PIPE_V>();
  }
  if (unlikely(remsFp32 > 0) && unlikely(offsetsFp32 > 0)) {
    Max(srcLocal, srcLocal[offsetsFp32], srcLocal, remsFp32, 1,
        {1, 1, 1, 0, 8, 0});
    PipeBarrier<PIPE_V>();
  }
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::QuantProcess(
  ...
  if constexpr (IsSmoothScaleExist) {
    ...
    DataCopyExtParams scalesCopyInParams{
        1U, static_cast<uint32_t>(axisH_ * sizeof(float)), 0U, 0U, 0U};
    DataCopyPadExtParams<float> copyPadExtParams{false, 0U, 0U, 0U};
    DataCopyPad(smoothScalesTensor_, scalesGMTensor_[expertIndex * axisH_],
  ...
  LocalTensor<int32_t> int32LocalTemp =
      floatLocalTemp.ReinterpretCast<int32_t>();
  ...
  floatLocalTemp.SetValue(hOutSizeAlign_ / sizeof(float),
                          float(1.0) / dynamicScale);  // int8->float32
}

template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::SyncCntOnCore(
  ...
  uint64_t rsvdCnt = 0;
  GatherMask(gatherMaskOutTensor, statusFp32Tensor_, gatherTmpTensor, true,
             mask, {1, (uint16_t)recStatusNumPerCore_, 1, 0}, rsvdCnt);
  PipeBarrier<PIPE_V>();

  // 对当前核对应的专家recv cnt求和
  uint32_t recStatusNumPerCoreInner =
      Ceil(recStatusNumPerCore_ * sizeof(float), UB_ALIGN)  // 对inner要求32对齐
      * UB_ALIGN / sizeof(float);
  SumParams sumParams{1, recStatusNumPerCoreInner, recStatusNumPerCore_};
  Sum(statusSumOutTensor, gatherMaskOutTensor, sumParams);
  SyncFunc<AscendC::HardEvent::V_S>();
  int32_t sumOfRecvCnt =
      statusSumOutTensor.ReinterpretCast<int32_t>().GetValue(0);
  ...
  GM_ADDR wAddr = (__gm__ uint8_t *)(recvCntWorkspaceGM_) +
                  coreOffset * aivId_;  // 写workspace需要按照512字节对齐
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::BufferInit() {
  tpipe_->Reset();
  uint32_t waitStatusBufSize = (((recStatusNumPerCore_ * UB_ALIGN) > 256)
                                    ? (recStatusNumPerCore_ * UB_ALIGN)
                                    : 256);
  tpipe_->InitBuffer(waitStatusBuf_,
                     waitStatusBufSize);  // 1024/24 * 32B = 43 * 32B
                                          // 内存复用，取大
  ...
  uint64_t gatherMaskOutSize = (recStatusNumPerCoreSpace > recvWinBlockNumSpace)
                                   ? recStatusNumPerCoreSpace
                                   : recvWinBlockNumSpace;
  tpipe_->InitBuffer(
      gatherMaskOutBuf_,
      gatherMaskOutSize);  // recStatusNumPerCore_32对齐后大小  * 32B
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::WaitDispatch() {
  BufferInit();
  ...
  LocalTensor<uint32_t> gatherTmpTensor =
      scalarBuf_.GetWithOffset<uint32_t>(UB_ALIGN / sizeof(uint32_t), 0);
  ...
  DataCopyParams intriParams{static_cast<uint16_t>(recStatusNumPerCore_), 1, 0,
                             0};
  SyncFunc<AscendC::HardEvent::S_V>();
  while (sumOfFlag != compareTarget) {
    DataCopy(statusFp32Tensor_,
             windowInstatusFp32Tensor_[startStatusIndex_ * stateOffset_ /
                                       sizeof(float)],
             intriParams);
    SyncFunc<AscendC::HardEvent::MTE2_V>();
  ...
  // 清状态
  SyncFunc<AscendC::HardEvent::MTE3_S>();
  DataCopyParams intriOutParams{static_cast<uint16_t>(recStatusNumPerCore_), 1,
                                0, 0};
  uint64_t duplicateMask[2] = {
      0x101010101010101,
      0};  // 一次性操作256字节，也是64个int32_t，每8个数将首个设置为0
  ...
  SyncFunc<AscendC::HardEvent::V_MTE3>();
  DataCopy(windowInstatusFp32Tensor_[startStatusIndex_ * stateOffset_ /
                                     sizeof(float)],
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::GetCumSum(
    LocalTensor<int32_t> &outLocal, uint32_t totalCount) {
  outLocal = gatherMaskOutBuf_.Get<int32_t>();
  // 获取workspace中每个核的recvcnt
  GM_ADDR wAddr = (__gm__ uint8_t *)(recvCntWorkspaceGM_) +
                  WORKSPACE_ELEMENT_OFFSET * aivId_;
  GlobalTensor<int32_t> sumTensor;
  sumTensor.SetGlobalBuffer((__gm__ int32_t *)wAddr);
  ...
  GatherMask(sumContinueTensor, sumLocalTensor, gatherSumPattern, true, mask,
             {1, copySumNum, 1, 0}, rsvdCnt);
  // height, width(按照32字节对齐padding后总元素个数),
  // nNum，结果矩阵第一列为对应行的求和结果
  uint32_t innerSumParams = (copySumNum * sizeof(float) + UB_ALIGN - 1) /
                            UB_ALIGN * UB_ALIGN / sizeof(float);
  LocalTensor<float> recvCntSumOutTensor =
      scalarBuf_.GetWithOffset<float>(UB_ALIGN / sizeof(float), UB_ALIGN);
  PipeBarrier<PIPE_V>();
  ...
  outLocal.SetValue(0,
                    recvCntSumOutTensor.ReinterpretCast<int32_t>().GetValue(0));
}

template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::LocalWindowCopy() {
  ...
  selfDataStatusTensor_.SetGlobalBuffer(
      (__gm__ uint32_t *)(statusDataSpaceGm_ + STATE_WIN_OFFSET +
                          aivId_ * WIN_ADDR_ALIGN + sizeof(uint32_t)));
  SyncFunc<AscendC::HardEvent::S_MTE3>();
  ...
  if (startExpertId_ >=
      rscvStatusNum_) {  // 分核已与前面的waitDispatch里保持一致
    return;
  }
  ...
  DataCopyExtParams dataCopyExpandIdxParams{
      1U, sizeof(int32_t) * EXPAND_IDX_INFO, 0U, 0U, 0U};
  ...
  for (uint32_t index = startExpertId_; index < endExpertId_; index++) {
    ...
    if (!isShareExpertRankFlag_) {
      if (moeExpertNumPerRank_ >
          1) {  // moe专家卡且一卡多专家场景 转换成数据区的排布偏移
        winOffset =
            index % epWorldSize_ * moeExpertNumPerRank_ + index / epWorldSize_;
      }
    }
    GM_ADDR wAddr =
        (__gm__ uint8_t *)(windowGM_) + winOffset * expertPerSizeOnWin_;
    ...
    for (uint32_t j = 0; j < count; j++) {
      tokGlobal.SetGlobalBuffer(
          (__gm__ ExpandXOutType *)(wAddr + j * hAlignWinSize_));
      // 将数据从Window拷贝到UB
      xTmpTensor_ = xQueue_.AllocTensor<ExpandXOutType>();
      DataCopyPad(xTmpTensor_, tokGlobal, hCommuCopyOutParams_,
                  copyPadExtParams);
      xQueue_.EnQue(xTmpTensor_);
      xTmpTensor_ = xQueue_.DeQue<ExpandXOutType>();
      xTmpTensorInt = xTmpTensor_.template ReinterpretCast<int32_t>();
      DataCopyPad(expandIdxGMTensor_[(beginIdx + j) * EXPAND_IDX_INFO],
                  xTmpTensorInt[tokenQuantAlign_], dataCopyExpandIdxParams);
      if constexpr (DynamicQuant || StaticQuant) {
        xOutFp32Tensor_ = xTmpTensor_.template ReinterpretCast<float>();
        DataCopyPad(dynamicScalesOutGMTensor_[beginIdx + j],
                    xOutFp32Tensor_[hOutSizeAlign_ / sizeof(float)],
                    floatDataCopyParams_);
      }
      if constexpr (IsNeedAllgather) {
        DataCopyPad(winTpGatherOutGMTensor_[(beginIdx + j) * hAlignWinCnt_],
                    xTmpTensor_, hCommuCopyOutParams_);
      }
      expandXOutGlobal.SetGlobalBuffer(
          (__gm__ ExpandXOutType *)(expandXOutGM_) + (beginIdx + j) * axisH_,
          axisH_);
  ...
  if constexpr (IsNeedAllgather) {
    DataCopyPad(winTpEpCntGMTensor_[startExpertId_], outCountLocal,
                dataCopyOutParams);
  }

  GlobalTensor<int32_t> sendCountsGlobal;
  sendCountsGlobal.SetGlobalBuffer(
      reinterpret_cast<__gm__ int32_t *>(sendCountsOutGM_));
  DataCopyPad(sendCountsGlobal[startExpertId_], outCountLocal,
              dataCopyOutParams);
  PipeBarrier<PIPE_MTE3>();
}
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::AllGatherSetStatusAndWait() {
  PipeBarrier<PIPE_ALL>();
  if (startExpertId_ >= totalExpertNum_) {
    return;
  }

  GM_ADDR rankGM = (__gm__ uint8_t *)(GetWindStateAddrByRankId(
                                          COMM_TP_IDX, tpGatherRankId_) +
  ...
  float sumOfFlag = static_cast<float>(-1.0);
  rankGM = (__gm__ uint8_t *)(GetWindStateAddrByRankId(COMM_TP_IDX, tpRankId_) +
                              WIN_ADDR_ALIGN * aivId_);  // 计算地址偏移
  tpwindowInstatusFp32Tensor_.SetGlobalBuffer((__gm__ float *)(rankGM));
  ...
  DataCacheCleanAndInvalid<float, CacheLine::SINGLE_CACHE_LINE,
                           DcciDst::CACHELINE_OUT>(tpwindowInstatusFp32Tensor_);
}

template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::AllgatherProcessOut() {
  if (startExpertId_ >= totalExpertNum_) {
    return;
  }
  ...
  GlobalTensor<float> tpwindowInstatusFp32Tensor_;
  GM_ADDR rankGM =
      (__gm__ uint8_t *)(GetWindStateAddrByRankId(COMM_TP_IDX, tpRankId_) +
                         WIN_ADDR_ALIGN * aivId_);  // 计算地址偏移
  tpwindowInstatusFp32Tensor_.SetGlobalBuffer((__gm__ float *)rankGM);
  LocalTensor<float> statusFp32Tensor_ = statusTensor_.ReinterpretCast<float>();
  DataCopy(statusFp32Tensor_, tpwindowInstatusFp32Tensor_, UB_ALIGN);
  SyncFunc<AscendC::HardEvent::MTE2_S>();
  uint32_t coreGatherCount =
      statusFp32Tensor_.ReinterpretCast<int32_t>().GetValue(1);
  ...
  sendCountsGlobal.SetGlobalBuffer(
      reinterpret_cast<__gm__ int32_t *>(sendCountsOutGM_));
  tpGlobal.SetGlobalBuffer(
      (__gm__ int32_t *)(tpLocalStatusWindowGM_ + TP_STATE_SIZE));
  DataCopyExtParams dataCopyParams{
      1U, static_cast<uint32_t>(sendExpertNum_ * sizeof(int32_t)), 0U, 0U, 0U};
  DataCopyPadExtParams<int32_t> copyPadParams{false, 0U, 0U, 0U};
  tpTmpTensor_ = xQueue_.AllocTensor<int32_t>();
  DataCopyPad(tpTmpTensor_, tpGlobal[startExpertId_], dataCopyParams,
              copyPadParams);
  xQueue_.EnQue(tpTmpTensor_);
  tpTmpTensor_ = xQueue_.DeQue<int32_t>();
  DataCopyPad(sendCountsGlobal[epWorldSize_ + startExpertId_], tpTmpTensor_,
              dataCopyParams);
  ...
  for (uint32_t i = 0; i < coreGatherCount; i++) {
    tokGlobal.SetGlobalBuffer(
        (__gm__ ExpandXOutType *)(tpLocalWindowGM_ +
                                  (preCount + i) * hAlignWinSize_));
    xTmpTensor_ = xQueue_.AllocTensor<ExpandXOutType>();
    DataCopyPad(xTmpTensor_, tokGlobal, hCommuCopyOutParams_, copyPadExtParams);
    xQueue_.EnQue(xTmpTensor_);
    xTmpTensor_ = xQueue_.DeQue<ExpandXOutType>();
    expandXOutGlobal.SetGlobalBuffer(
        (__gm__ ExpandXOutType *)(expandXOutGM_ +
                                  (preCount + totalCnt_ + i) * hOutSize_));
    DataCopyPad(expandXOutGlobal, xTmpTensor_, expandXCopyParams_);
...
template <TemplateMC2TypeClass>
__aicore__ inline void
MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::UpdateTokenNumsOut() {
  ...
  if (aivId_ == lastCore_) {
    ...
    if (moeExpertNumPerRank_ != 1) {
      if (!isShareExpertRankFlag_) {
        ...
        for (uint32_t localMoeIndex = 1; localMoeIndex < moeExpertNumPerRank_;
             ++localMoeIndex) {
          uint32_t preOffset =
              epWorldSize_ * (localMoeIndex - 1) + epWorldSize_ - 1;
    ...
    if constexpr (IsNeedAllgather) {
      ...
      sendTpCountsGlobal.SetGlobalBuffer(
          reinterpret_cast<__gm__ int32_t *>(sendTpCountOutGM_));
      sendTpCountsGlobal.SetValue(tpRankId_, totalCnt_);
      sendTpCountsGlobal.SetValue(tpGatherRankId_, gatherCount_ + preCnt_);
      DataCacheCleanAndInvalid<int32_t, CacheLine::SINGLE_CACHE_LINE,
                               DcciDst::CACHELINE_OUT>(sendTpCountsGlobal);
    }
  }
}
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatchShmem<TemplateMC2TypeFunc>::Process() {
...
// MoeDistributeDispatchShmemImpl
#endif  // MOE_DISTRIBUTE_DISPATCH_Shmem_H