...
namespace npu_ops_transformer_ext
{
    namespace RotaryStride
    {
        ...
        template<typename T>
        class rotary_stride_kernel {
        public:
            __aicore__ inline rotary_stride_kernel() {}
        ...
        private:
            __aicore__ inline void CopyIn(int64_t task_index, int64_t offset_inout, int64_t offset_sincos, int64_t bkn_offset, int64_t bkn_len)
            {
                ...
                copy_pas.dstStride   = (uint32_t)(2 * (bkAlignHalfD_*sizeof(T)/32) - (AlignUp(bkHalfD_*sizeof(T), 32)/32));
                DataCopyPadExtParams<T> pad_pas;

                DataCopyPad(local_in, gmIn_[offset_inout + bkn_offset * stride_], copy_pas, pad_pas);
                DataCopyPad(local_in[bkAlignHalfD_], gmIn_[offset_inout + bkn_offset * stride_ + bkHalfD_], copy_pas, pad_pas);
            ...
            __aicore__ inline void Compute(int64_t task_index, int64_t offset_inout, int64_t offset_sincos, int64_t bkn_offset, int64_t bkn_len)
            {
                ...
                for(int i = 0; i < 2; i++){
                    Mul(lmSinMul_[i*bkAlignHalfD_], local_in_f[i*bkAlignHalfD_], local_sin_f[i*bkAlignHalfD_], mask, (uint8_t)bkn_len, repeat_params);
                    Mul(lmCosMul_[i*bkAlignHalfD_], local_in_f[i*bkAlignHalfD_], local_cos_f[i*bkAlignHalfD_], mask, (uint8_t)bkn_len, repeat_params);
                }
                ...
                Add(local_out_f[bkAlignHalfD_], lmCosMul_[bkAlignHalfD_], lmSinMul_, mask, (uint8_t)bkn_len, repeat_params);        
            ...
            __aicore__ inline void CopyOut(int64_t task_index, int64_t offset_inout, int64_t offset_sincos, int64_t bkn_offset, int64_t bkn_len)
            {
                ...
                copy_pas.blockCount  = (uint16_t)(bkn_len);
                copy_pas.blockLen    = (uint32_t)(bkHalfD_ * sizeof(T));
                copy_pas.srcStride   = (uint32_t)(2*(bkAlignHalfD_*sizeof(T)/32) - (AlignUp(bkHalfD_*sizeof(T), 32)/32));
        ...
        int judge_rotary_stride_launch(const int64_t gbB,  const int64_t gbD,const int64_t ubSize)
        {
            ...
            bkMaxN_ = used_bytes / ((bkAlignHalfD_ * 2 * sizeof(uint16_t) * 2) + (bkAlignHalfD_ * 2 * sizeof(float) * 4));
                ...
                const int64_t stride, int32_t dtype)
        {
            ...
            if(ret == 0){
                rotary_stride_kernel_lanuch(blockDim, stream, in, sin, cos, psql, out, gbB, gbS, gbN, gbD, gbMAXS, stride, dtype);
                return 0;
            }
        ...
        int64_t rotary_stride_npu(int64_t blockDim, torch::Tensor &in, torch::Tensor &sin, torch::Tensor &cos, torch::Tensor &out, const int64_t hiddenDim) {
            ...
            TORCH_CHECK(sin.scalar_type() == at::kBFloat16 || sin.scalar_type() == at::kHalf, "dtype of sin tensor is invalid, only BF16 or FP16 is supported.");
            TORCH_CHECK(cos.scalar_type() == at::kBFloat16 || cos.scalar_type() == at::kHalf, "dtype of cos tensor is invalid, only BF16 or FP16 is supported.");
            ...
            {
                launchStatus = rotary_stride_lanuch(blockDim, stream, in.data_ptr(), sin.data_ptr(), cos.data_ptr(), psql.data_ptr(), out.data_ptr(), in.size(0), in.size(1), in.size(2), hiddenDim, sin.size(0), in.size(3), in.scalar_type() == at::kHalf ? 1 : 27);
                return 0;
            };
