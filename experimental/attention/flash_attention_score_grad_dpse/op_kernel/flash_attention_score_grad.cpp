...
#define INVOKE_FAG_GENERAL_S1S2_BN2GS1S2_IMPL(INPUT_TYPE, IS_ATTEN_MASK, IS_PSE, IS_DROP, MM_OUT_FORMAT, INPUT_LAYOUT, \
    ...
    do {                                                                                                               \
        ...
        FlashAttentionScoreGradPost<INPUT_TYPE, FlashAttentionScoreGradTilingDataS1s2Bn2gs1s2, true, INPUT_LAYOUT,     \
                                    input_format>                                                                      \
            opPost;                                                                                                    \
        opPost.Init(dq, dqRope, dk, dkRope, dv, dpse, actual_seq_qlen, actual_seq_kvlen, dsink, user, tilingData, &pipePost);  \
        opPost.Process();                                                                                              \
    } while (0)
...
#define INVOKE_FAG_GENERAL_S1S2_BN2_IMPL(INPUT_TYPE, MM_CONFIG, CUBE_FORMAT, PSE_CFG, ATTEN_MASK_CFG, DROPOUT_CFG,     \
    ...
    do {                                                                                                               \
        ...
        input_format> opCast;                                                                                          \
        opCast.Init(dq, dqRope, dk, dkRope, dv, dpse, actual_seq_qlen, actual_seq_kvlen, dsink, user, tilingData, &pipeCast);       \
        opCast.Process();                                                                                              \
    } while (0)
...
#define INVOKE_FAG_GENERAL_S1S2_BNGS1S2_B_IMPL(INPUT_TYPE, INPUT_LAYOUT, layout, MM_CONFIG, MM_OUT_FORMAT,             \
    ...
    do {                                                                                                               \
        ...
        opMuls.Init(dq, dqRope, dk, dkRope, dv, dpse, actual_seq_qlen, actual_seq_kvlen, dsink, user, tilingData, &pipeMuls);  \
        opMuls.Process();                                                                                              \
        pipeMuls.Destroy();                                                                                            \
    } while (0)
