...
#include "acl/acl.h"
#include "aclnnop/aclnn_prompt_flash_attention_v4.h"
...
/**
 * @brief Interface to the `aclnnPromptFlashAttentionV4` kernel which performs 
...
at::Tensor npu_prompt_flash_attention(
    ...
    int ret = aclnnPromptFlashAttentionV4GetWorkspaceSize(
    ...
    TORCH_CHECK(ret == ACL_SUCCESS, "aclnnPromptFlashAttentionV4GetWorkspaceSize failed with error: ", ret);
    ...
    TORCH_CHECK(ret == ACL_SUCCESS, "aclnnPromptFlashAttentionV4 execution failed with error: ", ret);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("npu_prompt_flash_attention", &npu_prompt_flash_attention,
        R"DOC(
        Interface to the `aclnnPromptFlashAttentionV4` kernel which performs 
        flash attention computation with prompt support.
