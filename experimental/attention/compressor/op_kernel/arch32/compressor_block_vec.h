...
__aicore__ inline void CompressorBlockVector<COMP>::SetMSplitInfo(const Compressor::RunInfo &info)
{
    ...
    mSplitInfo.vsc1StartOffset = 0;
    mSplitInfo.vsc1ResOffset = info.vsc1ResOffset;

    if (GetBlockIdx() % 2 == 1) {
        mSplitInfo.vsc1ResOffset += GetScSize(mSplitInfo.vecStartB, mSplitInfo.vecStartS,  mSplitInfo.vecEndB,  mSplitInfo.vecEndS) * constInfo_.headDim;
        mSplitInfo.vecStartB = bEnd;
        mSplitInfo.vecStartS = sEnd;
        mSplitInfo.vsc1StartOffset = mSplitInfo.dealTcNum * constInfo_.cmpRatio * constInfo_.dBaseSize * 2;
...
__aicore__ inline void CompressorBlockVector<COMP>::ComputeVec2(const Compressor::RunInfo &info)
{
    ...
    for (uint32_t v2LoopIdx = 0, dealSize = v2MBaseSize; v2LoopIdx < loopCount; ++v2LoopIdx) {
        ...
        ropeOffset = ropeOffset + v2LoopIdx * ropeCount;
        outOffset = outOffset + v2LoopIdx * dealSize * constInfo_.headDim;
