...
| rope\_head\_dim           | 属性      | 表示rope_cos和rope_sin的hidden层最小单元大小，目前仅支持64 | INT32       | -         |
| cmp\_ratio                | 属性      | 用于稀疏计算，表示数据压缩率 | INT32          | -         |
| kv\_block\_table          | 可选输入  | 表示kv\_state存储使用的block映射表 | INT32 | ND         |
| score\_block\_table       | 可选输入  | 表示score\_state存储使用的block映射表 | INT32 | ND         |
...
- shape约束
    ...
    - 若x的维度采用BS合轴，即x的输入shape为[T,H]
        ...
        - kv\_block\_table、score\_block\_table支持输入shape[B,ceil(Smax/block_size)]。Smax为每个Batch中最大的Sequence Length，即Smax=max(start\_pos)+max(cu\_seqlens[n+1] 。
        - cu\_seqlens[n])。当其中元素的值为0时，表示当前位置无需进行更新kv\_state、score\_state操作。
    ...
    - 若x的维度不采用BS合轴，即x的输入shape为[B,S,H]
        ...
        - kv\_block\_table、score\_block\_table支持输入shape[B,ceil(Smax/block_size)]。Smax为每个Batch中最大的Sequence Length，即Smax=max(start\_pos)+S。当其中元素的值为0时，表示当前位置无需进行更新kv\_state、score\_state操作。
