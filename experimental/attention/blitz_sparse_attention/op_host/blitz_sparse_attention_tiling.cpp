...
/*!
 * \file blitz_sparse_attention_tiling.cpp
 * \brief
 */
#include <queue>
#include <vector>
...
#include "../../blitz_sparse_attention/op_host/blitz_sparse_attention_tiling.h"
#include "../../blitz_sparse_attention/op_kernel/blitz_sparse_attention_template_tiling_key.h"
...
void BlitzSparseAttentionTiling::UpdateTilingKeyFlag(ContextParamsForPFATiling& contextKeyParams, uint64_t& tilingKey)
{
...
bool BlitzSparseAttentionTiling::GetApiTmpSize(const uint32_t sOuterFactor, const uint32_t sInnerFactor, const uint32_t typeByteSize)
{
...
size_t BlitzSparseAttentionTiling::GetPFAWorkSpaceSize(BlitzSparseAttentionTilingData* tilingData)
{
...
size_t BlitzSparseAttentionTiling::GetPFABaseApiWorkSpaceSize(uint32_t& blockDimToBeSet) {
    size_t sysWorkspaceSize, workspaceSize;
    const uint64_t defaultSysWorkspaceSize910B = 16U * 1024U * 1024U;
    if (curShortSocName == platform_ascendc::SocVersion::ASCEND310P) {
...
ge::graphStatus BlitzSparseAttentionTiling::TilingGetTilingKeyAttentionAscendC(uint64_t& tilingKey,
    ContextParamsForPFATiling& contextKeyParams, bool useNewTiling, BlitzSparseAttentionTilingData* tilingData) {
...
ge::graphStatus BlitzSparseAttentionTiling::TilingGetBaseApiTilingKeyAttentionAscendC(uint64_t& tilingKey, ContextParamsForPFATiling& contextKeyParams) {
    auto inputDataType = contextKeyParams.inputDataType;  // input q
...
void BlitzSparseAttentionTiling::BlitzSparseAttentionSplitNS(ContextParamsForPFATiling& contextKeyParams,
...
void BlitzSparseAttentionTiling::BlitzSparseAttentionInitOutputSplit(uint64_t totalSize,
    BlitzSparseAttentionTilingData* tilingData, uint32_t curCoreNum)
{
...
void BlitzSparseAttentionTiling::BlitzSparseAttentionInitSoftmaxLseOutputSplit(uint64_t totalSize,
    BlitzSparseAttentionTilingData* tilingData)
{
    PromptAttentionInitOutputParams *initParams = &tilingData->promptAttentionInitOutputParams;
...
void BlitzSparseAttentionTiling::BlitzSparseAttentionSplitNSNew(
    ContextParamsForPFATiling& contextKeyParams, BlitzSparseAttentionTilingData* tilingData,
...
void BlitzSparseAttentionTiling::GetPreNextTokensLeftUp(BlitzSparseAttentionTilingData* tilingData,
...
void BlitzSparseAttentionTiling::SetSplitCoreMode(BlitzSparseAttentionTilingData* tilingData, uint32_t sOuterFactor) {
    PromptAttentionBaseParams* baseParams = &tilingData->promptAttentionBaseParams;
...
void BlitzSparseAttentionTiling::BlitzSparseAttentionSplitSeqOneN(BlitzSparseAttentionTilingData* tilingData,
...
bool BlitzSparseAttentionTiling::EnableMTE2BmmPipe(BlitzSparseAttentionTilingData* tilingData,
    ...
    OP_CHECK_IF(ret != 0,
               OPS_REPORT_VECTOR_INNER_ERR("BlitzSparseAttention", "bmm SetFixSplit failed, ret = %d!", ret),
               return false);
    bool res = bmm.GetTiling(bmmTilingData) != -1;
    return res;
}

void BlitzSparseAttentionTiling::EnableBmmDoubleBuffer(AscendC::tiling::TCubeTiling& bmmTilingData) {
...
void BlitzSparseAttentionTiling::BlitzSparseAttention310PSetBmm1(matmul_tiling::MatmulApiTiling& bmm1)
{
...
void BlitzSparseAttentionTiling::BlitzSparseAttention310PSetBmm2(matmul_tiling::MatmulApiTiling& bmm2)
{
...
ge::graphStatus BlitzSparseAttentionTiling::CheckKeyValueParamsConsistency(const ContextParamsForPFATiling& contextKeyParams) {
...
bool BlitzSparseAttentionTiling::BlitzSparseAttentionCheckBmm1(BlitzSparseAttentionTilingData* tilingData,
    ...
    if (curShortSocName == platform_ascendc::SocVersion::ASCEND310P) {
        BlitzSparseAttention310PSetBmm1(bmm1);
    } else { // 910b
    ...
    if (autoBaseMNK) {
        if (enableMatmulNorm || splitCoreMode == SplitCoreMode::SPLIT_NBS_CUBE || splitCoreMode == SplitCoreMode::SPLIT_ONEN_CUBE) {
            ...
            OP_CHECK_IF(ret != 0,
                       OPS_REPORT_VECTOR_INNER_ERR("BlitzSparseAttention", "bmm1 SetFixSplit failed, ret = %d!", ret),
                       return false);
            ret = bmm1.GetTiling(bmm1TilingData);
        } else {
            uint32_t baseM = std::min(uint32_t(128), sOuterFactor);
...
void BlitzSparseAttentionTiling::GetMatMulType(matmul_tiling::DataType &mmInputType,
...
bool BlitzSparseAttentionTiling::BlitzSparseAttentionCheckBmm2(BlitzSparseAttentionTilingData* tilingData,
    ...
    if (curShortSocName == platform_ascendc::SocVersion::ASCEND310P) {
        BlitzSparseAttention310PSetBmm2(bmm2);
    ...
    if (autoBaseMNK) {
        if (enableMatmulNorm || splitCoreMode == SplitCoreMode::SPLIT_NBS_CUBE || splitCoreMode == SplitCoreMode::SPLIT_ONEN_CUBE) {
            ...
            OP_CHECK_IF(ret != 0,
                       OPS_REPORT_VECTOR_INNER_ERR("BlitzSparseAttention", "bmm2 SetFixSplit failed, ret = %d!", ret),
                       return false);
        }
...
void BlitzSparseAttentionTiling::BlitzSparseAttentionSetTensorSize(
    BlitzSparseAttentionTilingData* tilingData,
...
int64_t BlitzSparseAttentionTiling::BlitzSparseAttentionSetMsdUbSize(BlitzSparseAttentionTilingData* tilingData, PromptAttentionSingleCoreTensorSize& tensorSize, int32_t sInnerFactorTmp) const
{
    int64_t msdUbSize =  0;
    if (enableMsd) {
        if (tilingData->promptAttentionBaseParams.get_headSize() > MSD_BIG_D) {
...
uint32_t BlitzSparseAttentionTiling::CalculateL1SizeUsed(BlitzSparseAttentionTilingData* tilingData, const uint32_t typeByteSize)
{
    if (curShortSocName == platform_ascendc::SocVersion::ASCEND310P) {
...
bool BlitzSparseAttentionTiling::BlitzSparseAttentionCheckArgsLegal(BlitzSparseAttentionTilingData* tilingData,
    ...
    BlitzSparseAttentionSetTensorSize(tilingData, tilingData->promptAttentionTensorSizeRect,
                                        sOuterFactor, sInnerFactor);
    msdUbSize = BlitzSparseAttentionSetMsdUbSize(tilingData, tilingData->promptAttentionTensorSizeRect, static_cast<int32_t>(sInnerFactor));
    ...
    res = (BlitzSparseAttentionCheckBmm1(tilingData, tilingData->bmm1TilingDataRect,
        l1SizeRemain, l0CSize, sOuterFactor, sInnerFactor)) &&
        (BlitzSparseAttentionCheckBmm2(tilingData, tilingData->bmm2TilingDataRect,
        l1SizeRemain, l0CSize, sOuterFactor, sInnerFactor, dSplitFactor));

    OP_CHECK_IF(res == false,
               OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName, "BlitzSparseAttentionCheckBmm1 or BlitzSparseAttentionCheckBmm2 failed."),
               return false);
...
ge::graphStatus BlitzSparseAttentionTiling::BlitzSparseAttentionApiTiling(BlitzSparseAttentionTilingData* tilingData,
...
ge::graphStatus BlitzSparseAttentionTiling::BlitzSparseAttentionSetTilingData(gert::TilingContext* context,
    BlitzSparseAttentionTilingData* tilingData) {
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus BlitzSparseAttentionTiling::GetRectangleFactor(uint32_t seqFactorThreshold,
...
ge::graphStatus BlitzSparseAttentionTiling::SetInputLayout(const char* layout){
    if (layout == nullptr){
...
bool BlitzSparseAttentionTiling::CheckInputDimAndHeadNum(ContextParamsForPFATiling& contextKeyParams, const uint32_t nQAttr, const uint32_t nKVAttr) {
    uint32_t nQ = nQAttr;
...
bool BlitzSparseAttentionTiling::SetTilingHeadNumRatio(ContextParamsForPFATiling& contextKeyParams,
...
bool BlitzSparseAttentionTiling::CheckNonEmptyShapeExceptions(ContextParamsForPFATiling& contextKeyParams,
...
bool BlitzSparseAttentionTiling::CheckBaseApiNonEmptyShapeExceptions(ContextParamsForPFATiling& contextKeyParams,
                                                                     const gert::StorageShape* shape,
                                                                     const std::string &sName) {
    OP_CHECK_IF(shape == nullptr,
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName, "%s shape is null.", sName.c_str()),
...
bool BlitzSparseAttentionTiling::CheckActualSeqLength(ContextParamsForPFATiling& contextKeyParams, uint32_t b, uint32_t sQ, uint32_t sKV,
                                                      const gert::Tensor* actualSeqLenQ, const gert::Tensor* actualSeqLenKV,
                                                      InputLayout inLayout, BlitzSparseAttentionTilingData* tilingData) {
    if (contextKeyParams.fromTilingSink != 0U) {
        return true;
    }
...
bool BlitzSparseAttentionTiling::CheckPseShiftTypeAndShape(ContextParamsForPFATiling& contextKeyParams,
...
ge::graphStatus BlitzSparseAttentionTiling::processPageAttentionInputFlag(ContextParamsForPFATiling& contextKeyParams) {
...
bool BlitzSparseAttentionTiling::checkPABlockSizeAndBlockTable(ContextParamsForPFATiling& contextKeyParams, const gert::Tensor* actualSeqLenKV, 
...
bool BlitzSparseAttentionTiling::culActSeqLenParamsWhenPA(ContextParamsForPFATiling& contextKeyParams, const gert::Tensor* actualSeqLenKV, int64_t& blockNumValid, 
...
bool BlitzSparseAttentionTiling::checkPAKeyValueDimsWhenBBH(ContextParamsForPFATiling& contextKeyParams, int32_t keyDim1, int32_t keyDim2, int32_t keyDim3, 
...
bool BlitzSparseAttentionTiling::checkPAKeyValueDimsWhenBNBD(ContextParamsForPFATiling& contextKeyParams, const gert::StorageShape* keyShape, const gert::StorageShape* valueShape, 
...
bool BlitzSparseAttentionTiling::checkPAKeyValueDimsWhenNZ(ContextParamsForPFATiling& contextKeyParams, const gert::StorageShape* keyShape, const gert::StorageShape* valueShape, 
...
bool BlitzSparseAttentionTiling::CheckPAKeyValueParams(ContextParamsForPFATiling& contextKeyParams, const gert::StorageShape* keyShape, const gert::StorageShape* valueShape, 
...
bool BlitzSparseAttentionTiling::CheckPASparseMode(ContextParamsForPFATiling& contextKeyParams) {
...
bool BlitzSparseAttentionTiling::CheckPAWhenBaseApi(ContextParamsForPFATiling& contextKeyParams, const gert::Tensor* actualSeqLenQ,
...
bool BlitzSparseAttentionTiling::CheckPATypeAndShape(ContextParamsForPFATiling& contextKeyParams,
...
bool BlitzSparseAttentionTiling::CheckAttenMaskShape(ContextParamsForPFATiling& contextKeyParams,
...
bool BlitzSparseAttentionTiling::CheckPAAntiquantSupportScenarios(ContextParamsForPFATiling& contextKeyParams, BlitzSparseAttentionTilingData* tilingData) {
...
bool BlitzSparseAttentionTiling::CheckPerchannelAntiquantParamsShape(ContextParamsForPFATiling& contextKeyParams, const gert::StorageShape* antiquantScaleShape, const gert::StorageShape* antiquantOffsetShape, 
...
bool BlitzSparseAttentionTiling::CheckPerchannelBSNDParamsShape(ContextParamsForPFATiling& contextKeyParams, const gert::StorageShape* antiquantScaleShape, const gert::StorageShape* antiquantOffsetShape,
...
bool BlitzSparseAttentionTiling::CheckAntiquantParamsShape(ContextParamsForPFATiling& contextKeyParams, const gert::StorageShape* antiquantScaleShape,
...
ge::graphStatus BlitzSparseAttentionTiling::CheckPostQuantParams(const ContextParamsForPFATiling& contextKeyParams, uint32_t h, uint32_t n) const {
...
ge::graphStatus BlitzSparseAttentionTiling::AdjustBasicBlock(BlitzSparseAttentionTilingData* tilingData,
...
void BlitzSparseAttentionTiling::Align(uint32_t &num) {
    num = (num + typeByteNum - 1U) / typeByteNum * typeByteNum;
}

// Code for ut, no pratical to use.
ge::graphStatus BlitzSparseAttentionTiling::GetBasicShape310P(uint32_t &b,
                                                              uint32_t &bKV,
                                                              uint32_t &s,
                                                              uint32_t &h,
...
ge::graphStatus BlitzSparseAttentionTiling::GetAndCheckEmptyQueryShape(ContextParamsForPFATiling& contextKeyParams, const gert::StorageShape *queryShape) const {
    OP_CHECK_IF(queryShape == nullptr,
...
void BlitzSparseAttentionTiling::SetBaseApiTilingData(ContextParamsForPFATiling& contextKeyParams, std::vector<int64_t>& actualSeqLengths,
...
void BlitzSparseAttentionTiling::SetBaseApiSeqTilingData(ContextParamsForPFATiling& contextKeyParams, std::vector<int64_t>& actualSeqLengths,
                                                         std::vector<int64_t>& actualSeqLengthsKV) {
    PromptAttentionBaseApiBaseParams* baseParams = &baseApiTilingData.promptAttentionBaseApiBaseParams;
...
ge::graphStatus BlitzSparseAttentionTiling::CheckBaseAPISupportScenarios(ContextParamsForPFATiling& contextKeyParams) {
    const int32_t* sparseMode = contextKeyParams.sparseMode;
...
ge::graphStatus BlitzSparseAttentionTiling::AtbSplitBlock(ContextParamsForPFATiling& contextKeyParams)
{
    PromptAttentionBaseApiBaseParams* baseParams = &baseApiTilingData.promptAttentionBaseApiBaseParams;
...
void BlitzSparseAttentionTiling::SetTensorSizeParams()
{
...
// TND 新增
void BlitzSparseAttentionTiling::SetMultiCoreParamsTND()
{
...
bool BlitzSparseAttentionTiling::InputLayoutIsTNDLike() const
{
    return inputLayout == InputLayout::TND || inputLayout == InputLayout::NTD_TND;
}

int64_t BlitzSparseAttentionTiling::GetTFromInputShape(uint32_t inputIdx, const gert::StorageShape *shape) const
{
...
int64_t BlitzSparseAttentionTiling::GetNFromInputShape(uint32_t inputIdx, const gert::StorageShape *shape) const
{
    bool isKV = (inputIdx != QUERY_INDEX && inputIdx != QUERY_ROPE_INDEX);
...
int64_t BlitzSparseAttentionTiling::GetDFromInputShape(uint32_t inputIdx, const gert::StorageShape *shape) const
{
    bool isKV = (inputIdx != QUERY_INDEX && inputIdx != QUERY_ROPE_INDEX);
    auto nKV = contextKeyParamsPtr->numKeyValueHeads == nullptr ? *contextKeyParamsPtr->headsNumber : *contextKeyParamsPtr->numKeyValueHeads;
    if (enablePA && isKV) {
...
int64_t BlitzSparseAttentionTiling::GetTFromOutputShape(const gert::StorageShape *shape) const
{
    return shape->GetStorageShape().GetDim(0);
}

int64_t BlitzSparseAttentionTiling::GetNFromOutputShape(const gert::StorageShape *shape) const
{
    return shape->GetStorageShape().GetDim(1);
}

void BlitzSparseAttentionTiling::GetActualSeqLenData(int64_t inputIdx,
...
bool BlitzSparseAttentionTiling::BalanceLoad(const std::vector<int64_t> &sparseValidArray,
...
bool BlitzSparseAttentionTiling::InitLoadValue(const std::vector<int64_t> &sparseValidArray, int64_t validAivNum,
...
bool BlitzSparseAttentionTiling::SetSparseStartIdxTND(const std::vector<int64_t> &sparseValidArray, PFAMultiCoreParams &multiCoreParams)
{
    // to avoid buffer overflow, or maybe sometimes we want to only verify single core
    int64_t validAiCoreNum = isSameAB ? std::min(static_cast<int64_t>(multiCoreParams.get_coreNum() / 2), MAX_AIC_NUM)
                                        :std::min(static_cast<int64_t>(multiCoreParams.get_coreNum()), MAX_AIV_NUM);
...
int64_t BlitzSparseAttentionTiling::GetS2RealSize(uint8_t sparseType, int32_t bOutIdx, int64_t s1OutIdx)
{
...
bool BlitzSparseAttentionTiling::InitSparseValidArrayTND(std::vector<int64_t> &sparseValidArray)
{
...
void BlitzSparseAttentionTiling::SetSparseParamsTND()
{
    auto &coreParams = mlaTilingData.PFAcoreParams;
    auto &multiCoreParams = mlaTilingData.PFAmultiCoreParams;
...
uint32_t BlitzSparseAttentionTiling::CalcTschBlockDim(uint32_t sliceNum, uint32_t aicCoreNum, uint32_t aivCoreNum)
{
    uint32_t ration;
...
bool BlitzSparseAttentionTiling::CalcUBSize()
{
    apiMaxUBSize = HIGH_PERF_API_BUFFER_MULTIPLE * s1BasicBlock * s2BasicBlock * sizeof(float);
    return true;
}

void BlitzSparseAttentionTiling::SetSoftMaxTiling()
{
...
bool BlitzSparseAttentionTiling::SetBmm1TilingInput(int64_t tmpS1BasicBlock, int64_t tmpS2BasicBlock,
...
bool BlitzSparseAttentionTiling::SetBmm2TilingInput(int64_t tmpS1BasicBlock, int64_t tmpS2BasicBlock,
...
bool BlitzSparseAttentionTiling::SetMatMulTiling(int64_t tmpS1BasicBlock, int64_t tmpS2BasicBlock,
                                                 matmul_tiling::MatmulApiTiling &bmm1,
                                                 matmul_tiling::MatmulApiTiling &bmm2)
{
...
int64_t BlitzSparseAttentionTiling::CalcMaxS1BasicBlockSize(int64_t actualD, const BufferNum &bufferNum)
{
...
int64_t BlitzSparseAttentionTiling::CalcMaxS2BasicBlockSize(const BufferNum &bufferNum,
...
bool BlitzSparseAttentionTiling::IsBasicBlockInSoftMax(const ge::Shape &shape)
{
    // 2 axes at least
...
void BlitzSparseAttentionTiling::GetBufferNum(BufferNum &bufferNum)
{
    bufferNum.bufferS1S2Num = HIGH_PERF_BUFFER_NUM;
}

void BlitzSparseAttentionTiling::MatchTemplate(uint32_t valueD)
{
...
ge::graphStatus BlitzSparseAttentionTiling::CheckLearnableSinkWhenLayoutIsTND(ContextParamsForPFATiling& contextKeyParams) {
    const gert::StorageShape* queryShape = contextKeyParams.queryInputShape;
    const gert::StorageShape* valueShape = contextKeyParams.valueInputShape;
...
ge::graphStatus BlitzSparseAttentionTiling::CheckInputShapeWhenLayoutIsTND(ContextParamsForPFATiling& contextKeyParams) {
    std::string layoutStr(contextKeyParams.layout);
...
ge::graphStatus BlitzSparseAttentionTiling::CheckActSeqWhenLayoutIsTND(ContextParamsForPFATiling& contextKeyParams) {
    std::string layoutStr(contextKeyParams.layout);
...
ge::graphStatus BlitzSparseAttentionTiling::CheckVarLenPreNextToken(ContextParamsForPFATiling& contextKeyParams,
...
ge::graphStatus BlitzSparseAttentionTiling::RunBigKernelTilingWithParams(ContextParamsForPFATiling& contextKeyParams,
                                            uint64_t& tilingKey,
    ...
    if ((keyShape->GetStorageShape().GetShapeSize() == 0) || (valueShape->GetStorageShape().GetShapeSize() == 0) ||
        ...
        OP_CHECK_IF((contextKeyParams.inputDataType == ge::DT_INT8) || (contextKeyParams.kDataType == ge::DT_INT8) || (contextKeyParams.vDataType == ge::DT_INT8),
                   ...
                   return ge::GRAPH_FAILED);
        BlitzSparseAttentionInitOutputSplit(outShape->GetStorageShape().GetShapeSize(), tilingData, coreNum);
    ...
    if (enableMsd) {
        if (s > CVDIFF_SMALL_QS_THRESHOLDS) {
            OP_LOGE("BlitzSparseAttention", "S of query(%u) is larger than 16, when the key/value's dequant scale is enabled.", s);
            return ge::GRAPH_FAILED;
        }
    }
    ...
    if (splitCoreMode == SplitCoreMode::SPLIT_ONEN_CUBE) {  // Enable N split kernel mode from the perspective of cube in long sequence scenes.
        ...
        BlitzSparseAttentionSplitSeqOneN(tilingData, coreNum, false);
    } else {
        tilingData->promptAttentionInitOutputParams.set_isOneN(0);
        if (useNewTiling) {
            BlitzSparseAttentionSplitNSNew(contextKeyParams, tilingData, coreNum, actualSeqLengths, actualSeqLengthsKV, actualSharedPrefixLen, useBalanceTiling);
        } else {
            BlitzSparseAttentionSplitNS(contextKeyParams, tilingData, coreNum, actualSeqLengths);
        }
    }
    ...
    if (contextKeyParams.isSoftMaxLseEnable) {
        BlitzSparseAttentionInitSoftmaxLseOutputSplit(SoftmaxLseOutShape->GetStorageShape().GetShapeSize(), tilingData);
    }
    ...
    tilingRet = BlitzSparseAttentionApiTiling(tilingData, outputDataTypeSize, sOuterFactor, softmaxSInnerFactor, softmaxSOuterFactor);
    OP_CHECK_IF(tilingRet != ge::GRAPH_SUCCESS,
...
ge::graphStatus BlitzSparseAttentionTiling::CheckIOType(ContextParamsForPFATiling& contextKeyParams, BlitzSparseAttentionTilingData* tilingData, int32_t& outputDataTypeSize) {
...
ge::graphStatus BlitzSparseAttentionTiling::CheckDimNums(ContextParamsForPFATiling& contextKeyParams) {
    std::string layoutStr(contextKeyParams.layout);
    const gert::StorageShape* queryShape = contextKeyParams.queryInputShape;
    const gert::StorageShape* keyShape = contextKeyParams.keyInputShape;
    const gert::StorageShape* valueShape = contextKeyParams.valueInputShape;
...
ge::graphStatus BlitzSparseAttentionTiling::CheckD(ContextParamsForPFATiling& contextKeyParams) {
    std::string layoutStr(contextKeyParams.layout);
...
ge::graphStatus BlitzSparseAttentionTiling::CheckMaskType(ContextParamsForPFATiling &contextKeyParams,
                                                          BlitzSparseAttentionTilingData* tilingData,
...
ge::graphStatus BlitzSparseAttentionTiling::CheckBaseApiMaskVal(ContextParamsForPFATiling& contextKeyParams,
...
ge::graphStatus BlitzSparseAttentionTiling::CheckBaseApiAlibiMask(ContextParamsForPFATiling& contextKeyParams,
                                                                  const gert::StorageShape* pseShiftShape,
...
ge::graphStatus BlitzSparseAttentionTiling::CheckBaseApiNormMask(ContextParamsForPFATiling& contextKeyParams,
                                                                 const gert::StorageShape* pseShiftShape,
...
ge::graphStatus BlitzSparseAttentionTiling::CheckBaseApiMaskBasic(ContextParamsForPFATiling& contextKeyParams,
...
ge::graphStatus BlitzSparseAttentionTiling::SetBaseApiAlibiMaskInfo(ContextParamsForPFATiling &contextKeyParams,
                                                                    const gert::StorageShape* pseShiftShape)
{
    uint32_t maskDim = pseShiftShape->GetStorageShape().GetDimNum();
    PromptAttentionBaseApiBaseParams* baseParams = &baseApiTilingData.promptAttentionBaseApiBaseParams;
...
void BlitzSparseAttentionTiling::SetBaseApiOtherMaskInfo(ContextParamsForPFATiling &contextKeyParams, const gert::StorageShape* pseShiftShape)
{
    uint32_t maskDim = pseShiftShape->GetStorageShape().GetDimNum();
    PromptAttentionBaseApiBaseParams* baseParams = &baseApiTilingData.promptAttentionBaseApiBaseParams;
    uint32_t maxSeqLen = baseParams->get_maxSeqLen();
...
ge::graphStatus BlitzSparseAttentionTiling::SetBaseApiPseInfo(ContextParamsForPFATiling &contextKeyParams, const gert::StorageShape* pseShiftShape)
{
    const int32_t* sparseMode = contextKeyParams.sparseMode;
...
ge::graphStatus BlitzSparseAttentionTiling::CheckBaseApiPse(ContextParamsForPFATiling &contextKeyParams,
                                                            const gert::StorageShape* pseShiftShape) {
    const int32_t* sparseMode = contextKeyParams.sparseMode;
...
void BlitzSparseAttentionTiling::SetMaskSize(const gert::StorageShape* attenMaskShape, BlitzSparseAttentionTilingData* tilingData) {
...
ge::graphStatus BlitzSparseAttentionTiling::CheckShape(ContextParamsForPFATiling& contextKeyParams, const gert::StorageShape* queryShape,
...
ge::graphStatus BlitzSparseAttentionTiling::CheckBaseApiRequiredInput(ContextParamsForPFATiling& contextKeyParams) {
    const gert::StorageShape* queryShape = contextKeyParams.queryInputShape;
    const gert::StorageShape* keyShape = contextKeyParams.keyInputShape;
    const gert::StorageShape* valueShape = contextKeyParams.valueInputShape;
    const gert::StorageShape* attenMaskShape = contextKeyParams.attentionMaskShape;
...
ge::graphStatus BlitzSparseAttentionTiling::CheckBaseApiOptionalInput(ContextParamsForPFATiling& contextKeyParams) {
    const int64_t* innerPrecisePtr = contextKeyParams.innerPrecisePtr;
...
void BlitzSparseAttentionTiling::InferTilingMod(const ContextParamsForPFATiling& contextKeyParams, const std::vector<int64_t>& actualSeqLengths, const std::vector<int64_t>& actualSeqLengthsKV,
...
ge::graphStatus BlitzSparseAttentionTiling::AdjustCVTiling(uint32_t hDivN, uint32_t n, int64_t middleActualSeqLengths,
                                                           int64_t ubSize, int64_t l1Size, int64_t l0CSize,
                                                           uint32_t maskElemSize, uint32_t& sOuterFactor,
                                                           uint32_t& sInnerFactor, BlitzSparseAttentionTilingData* tilingData)
{
    ...
    while (true) {
        ...
        if (BlitzSparseAttentionCheckArgsLegal(tilingData, ubSize, l1Size, l0CSize,
...
ge::graphStatus BlitzSparseAttentionTiling::BlitzSparseAttentionCVDiffSetTensorSize(
    BlitzSparseAttentionTilingData* tilingData,
...
bool BlitzSparseAttentionTiling::BlitzSparseAttentionComputeCVDiffParams(BlitzSparseAttentionTilingData* tilingData,
    int64_t ubSize, int64_t l1Size, int64_t l0CSize, uint32_t typeByteSize,
    uint32_t& sOuterFactor, uint32_t &sInnerFactor, uint32_t maskTypeSize, uint32_t &softmaxSOuterFactor)
{
    ...
    OP_CHECK_IF(res == false,
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName, "BlitzSparseAttentionCheckBmm1 failed!"),
                    return false);

    res = BlitzSparseAttentionCheckBmm2(tilingData, tilingData->bmm2TilingDataRect,
            l1SizeRemain, l0CSize, sOuterFactor, sInnerFactor, dSplitFactorBmm2, true, true);
    OP_CHECK_IF(res == false,
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName, "BlitzSparseAttentionCheckBmm2 failed!"),
                    return false);

    return true;
}

bool BlitzSparseAttentionTiling::FindOptimalTilingSouter(BlitzSparseAttentionTilingData* tilingData,
    ...
    int64_t ubSize, uint32_t typeByteSize, uint32_t maskTypeSize)
{
    ...
    while (ubSizeRemain <= 0 && sOuterFactorTmp > 0) {
        ...
        if (BlitzSparseAttentionCVDiffSetTensorSize(tilingData, tilingData->promptAttentionTensorSizeRect,
                                                sOuterFactorTmp, sInnerFactorTmp, softmaxSOuterFactorTmp) != ge::GRAPH_SUCCESS) {
            return false;
        }

        int64_t msdUbSize = BlitzSparseAttentionSetMsdUbSize(tilingData, tilingData->promptAttentionTensorSizeRect, sInnerFactorTmp);
        queueBufferSize = tilingData->promptAttentionTensorSizeRect.get_attenMaskUbSize();
        pseShiftBufferSize = tilingData->promptAttentionTensorSizeRect.get_pseShiftUbSize();
        apiTmpSize = std::max(softmaxTmpSize, softmaxFlashTmpSize);
...
bool BlitzSparseAttentionTiling::FindOptimalTilingBasicBLock(BlitzSparseAttentionTilingData* tilingData,
    uint32_t& sOuterFactor, uint32_t &sInnerFactor, uint32_t &softmaxSOuterFactor,
    int64_t ubSize, uint32_t typeByteSize, uint32_t maskTypeSize)
{
    auto tmpShape = Shape({softmaxSOuterFactor, sInnerFactor});
    ...
    while (ubSizeRemain <= 0 && sOuterFactorTmp > 0) {
        while ((ubSizeRemain <= 0 && sInnerFactorTmp > 0) || (enableKvAntiquant && ubSizeRemain < minAntiquantUbSizeNeed && sInnerFactorTmp > 0)) {
            ...
            msdUbSize = BlitzSparseAttentionSetMsdUbSize(tilingData, tilingData->promptAttentionTensorSizeRect, sInnerFactorTmp);
...
ge::graphStatus BlitzSparseAttentionTiling::AdjustCVTilingCVDiff(int64_t ubSize, int64_t l1Size, int64_t l0CSize,
    ...
    BlitzSparseAttentionTilingData* tilingData)
{
    ...
    bool res = BlitzSparseAttentionComputeCVDiffParams(tilingData, ubSize, l1Size, l0CSize,
                    softmaxDataTypeSize, minFactor, rectangleFactor, maskElemSize, softmaxSOuterFactor);
    OP_CHECK_IF(res == false,
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName, "BlitzSparseAttentionComputeCVDiffParams failed!"),
                    return ge::GRAPH_FAILED);
...
PFA_EXTERN_C ge::graphStatus TilingBlitzSparseAttention(gert::TilingContext* context) {
    if (context == nullptr) {
        OP_LOGE("BlitzSparseAttention", "tiling context is nullptr!");
        return ge::GRAPH_FAILED;
    }
    if (context->GetRawTilingData() == nullptr) {
        OP_LOGE("BlitzSparseAttention", "tiling context GetRawTilingData is nullptr!");
        return ge::GRAPH_FAILED;
    }

    BlitzSparseAttentionTilingData* tilingData = context->GetTilingData<BlitzSparseAttentionTilingData>();
    ...
    BlitzSparseAttentionTiling flashTiling(nullptr);
    ...
    flashTiling.BlitzSparseAttentionSetTilingData(context, tilingData);
    return ret;
}
}