...
/*!
 * \file blitz_sparse_attention_tiling_register.cpp
 * \brief
 */
...
static ge::graphStatus TilingPrepareForBlitzSparseAttention(gert::TilingParseContext* context) {
    auto platformInfoPtr = context->GetPlatformInfo();
    OP_CHECK_IF(platformInfoPtr == nullptr,
                    OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "platformInfoPtr is null!"),
                    return ge::GRAPH_FAILED);
    auto compileInfoPtr = context->GetCompiledInfo<BlitzSparseAttentionCompileInfo>();
...
IMPL_OP_OPTILING(BlitzSparseAttention)
    .TilingInputsDataDependency({ACTUAL_SEQ_Q_INDEX_PFA, ACTUAL_SEQ_KV_INDEX_PFA})
    .Tiling(TilingBlitzSparseAttention)
    .TilingParse<BlitzSparseAttentionCompileInfo>(TilingPrepareForBlitzSparseAttention); // Register entrance functions to the framework
}  // namespace optiling
