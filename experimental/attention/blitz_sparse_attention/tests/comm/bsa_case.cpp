...
/*!
 * \file bsa_case.cpp
 * \brief BlitzSparseAttention 测试用例.
 */
#include "bsa_case.h"
#include <utility>
...
#include "tiling/bsa/tiling_data.h"
#include "tiling/bsa/tiling_stub.h"
#include "../../op_kernel/blitz_sparse_attention.cpp"
...
// extern "C" __global__ __aicore__ void blitz_sparse_attention PFA_KERNEL_PARAM;

using namespace ops::adv::tests::bsa;
using Tensor = ops::adv::tests::utils::TensorIntf;
using Case = ops::adv::tests::utils::Case;
using Platform = ops::adv::tests::utils::Platform;

bool RunTemplateBlitzSparseAttention(std::function<void(PFA_INPUT_DTYPE)> func,
...
bool RunBlitzSparseAttention(void *func, uint64_t tilingKey, int64_t blockDim, std::vector<Tensor *> &inputs,
                             std::vector<Tensor *> &outputs, uint8_t *workspace, uint8_t *tilingData)
{
    // Kernel 运行
...
extern "C" ge::graphStatus TilingBlitzSparseAttentionStub(gert::TilingContext* context)
{
...
{
    bool rst = mCtx.SetOpName("BlitzSparseAttention");
    ...
    rst = rst && mCtx.SetAttrs({{"num_heads", mParam.numHeads},
        ...
        rst = rst && mCtx.SetKernelRunCbf(RunBlitzSparseAttention);
        rst = rst && mCtx.SetKernelMainFunc((void *)nullptr);
        if(this->mPfaKernelTemplateFunc){
            rst = rst && mCtx.SetKernelRunTemplateCbf(RunTemplateBlitzSparseAttention);
    ...
    pfaTilingFunc = (gert::OpImplRegisterV2::TilingKernelFunc)platform->LoadOpTilingSoSym("TilingBlitzSparseAttention");
    if(pfaTilingFunc == nullptr) {
        LOG_ERR("Can't get origin tiling func, bsa(%p)",pfaTilingFunc);
        return false;
    }
    IMPL_OP(BlitzSparseAttention).Tiling(TilingBlitzSparseAttentionStub);
    return rst;
}
...
{
    this->mOpInfo.mName = "BlitzSparseAttention";
