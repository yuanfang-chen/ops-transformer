...
// custom::graphStatus IFATiling::CheckBaseInputsNull() const
// {
...
//                OP_LOGE(ifaContext_->opName, "Tensor query is empty cause shapesize is 0."), return custom::graphStatus::GRAPH_FAILED);
...
// custom::graphStatus IFATiling::CheckInputParameterFormat() const
// {
...
//     OP_CHECK_IF(
//         (!IsSupportFormat(qFormat)), OP_LOGE(ifaContext_->opName, "Query format %d should be ND/NCHW/NHWC/NCDHW", qFormat), return custom::graphStatus::GRAPH_FAILED);
//     OP_CHECK_IF(
//         (!IsSupportFormat(kFormat)), OP_LOGE(ifaContext_->opName, "Key format %d should be ND/NCHW/NHWC/NCDHW", kFormat), return custom::graphStatus::GRAPH_FAILED);
//     OP_CHECK_IF(
//         (!IsSupportFormat(vFormat)), OP_LOGE(ifaContext_->opName, "Value format %d should be ND/NCHW/NHWC/NCDHW", vFormat), return custom::graphStatus::GRAPH_FAILED);
...
// custom::graphStatus IFATiling::CheckInputAntiquantFormat() const
// {
...
// custom::graphStatus IFATiling::ProcessCheckATBInputWithPage() const
// {
...
//         return custom::graphStatus::GRAPH_FAILED);
//     OP_CHECK_IF((taskNumI64 > UINT32_MAX), OP_LOGE(ifaContext_->opName, "param is invalid"), return custom::graphStatus::GRAPH_FAILED);
//     OP_CHECK_IF((blockSize_ <= 0 || blockSize_ % 16U != 0),
//         OP_LOGE(ifaContext_->opName, "blockSize is invalid"), return custom::graphStatus::GRAPH_FAILED);
//     OP_CHECK_IF((static_cast<uint64_t>(headDim_) * blockSize_ > 128U * 128U),
//         OP_LOGE(ifaContext_->opName, "headDim * blockSize should no greater than 128 * 128"), return custom::graphStatus::GRAPH_FAILED);
//     return custom::graphStatus::GRAPH_SUCCESS;
// }

// custom::graphStatus IFATiling::CheckKeyShapeInput() const
// {
...
// custom::graphStatus IFATiling::CheckValueShapeInput() const
// {
...
// custom::graphStatus IFATiling::ProcessCheckATBInput()
// {
...
//     if (CheckKeyShapeInput() != custom::graphStatus::GRAPH_SUCCESS) {
//         return custom::graphStatus::GRAPH_FAILED;
//     }

//     if (CheckValueShapeInput() != custom::graphStatus::GRAPH_SUCCESS) {
//         return custom::graphStatus::GRAPH_FAILED;
//     }

//     OP_CHECK_IF((headDim_ > 256 || headDim_ <= 0),
//         OP_LOGE(ifaContext_->opName, "headdim is invalid"), return custom::graphStatus::GRAPH_FAILED);
...
// custom::graphStatus IFATiling::CheckInputQKVTypeMatch() const
// {
//     OP_CHECK_IF(
...
//     OP_CHECK_IF(
//         ((!ropeFlag_) && inputQType_ == at::ScalarType::Char && inputKvType_ == at::ScalarType::Char),
//         OP_LOGE(ifaContext_->opName, "When QueryRope/KeyRope is null and Qs(%u) in [1, 16], not support qkv datatype all int8.", qSeqSize_), return custom::graphStatus::GRAPH_FAILED);

//     OP_CHECK_IF(
//         ((inputQType_ == at::ScalarType::Half) && (inputKvType_ != at::ScalarType::Half && inputKvType_ != at::ScalarType::Char && inputKvType_ != ge::DT_INT4)), OP_LOGE(ifaContext_->opName, "when input Q type is fp16, KV type %d should be fp16 or int8 or int4", inputKvType_),
//         return custom::graphStatus::GRAPH_FAILED);

//     OP_CHECK_IF(
//         ((inputQType_ == at::ScalarType::BFloat16) && (inputKvType_ != at::ScalarType::BFloat16 && inputKvType_ != at::ScalarType::Char && inputKvType_ != ge::DT_INT4)), OP_LOGE(ifaContext_->opName, "when input Q type is bf16, KV type %d should be bf16 or int8 or int4", inputKvType_),
//         return custom::graphStatus::GRAPH_FAILED);
...
// custom::graphStatus IFATiling::CheckInputFormatAndLimits()
// {
//     if (CheckInputParameterFormat() != custom::graphStatus::GRAPH_SUCCESS ||
//         CheckInputAntiquantFormat() != custom::graphStatus::GRAPH_SUCCESS ||
//         CheckInputQKVTypeMatch() != custom::graphStatus::GRAPH_SUCCESS) {
//         return custom::graphStatus::GRAPH_FAILED;
//     }
...
// custom::graphStatus IFATiling::CheckKVHeadNum(const gert::StorageShape *inputShape) const
// {
...
// custom::graphStatus IFATiling::CheckKVShape(const size_t &size, const gert::StorageShape *keyTensorInList, const gert::StorageShape *valueTensorInList) const
// {
//     /* kv not continuous */
//     std::string layOutStr = ifaContext_->layOut;
//     if (layOutStr == "BSH") {
...
//         return custom::graphStatus::GRAPH_FAILED);
//     if (CheckKVHeadNum(keyTensorInList) != custom::graphStatus::GRAPH_SUCCESS ||
//         CheckKVHeadNum(valueTensorInList) != custom::graphStatus::GRAPH_SUCCESS) {
//         return custom::graphStatus::GRAPH_FAILED;
//     }
//     return custom::graphStatus::GRAPH_SUCCESS;
// }

// custom::graphStatus IFATiling::CheckQKOutShapeBsh(size_t dimOfQ, size_t dimOfK, size_t dimOfOut,
...
//         outExpectDim, dimOfQ, dimOfK, dimOfOut), return custom::graphStatus::GRAPH_FAILED);
...
//         queryShape->GetStorageShape().GetDim(1)), return custom::graphStatus::GRAPH_FAILED);
...
// custom::graphStatus IFATiling::CheckQKOutShapeTnd(size_t dimOfQ, size_t dimOfK, size_t dimOfOut,
//     const gert::StorageShape *queryShape, const gert::StorageShape *keyShape) const
// {
...
//         dimOfQ, dimOfK, dimOfOut), return custom::graphStatus::GRAPH_FAILED);
...
// custom::graphStatus IFATiling::CheckQKOutShapeBnsdOrBsnd(size_t dimOfQ, size_t dimOfK, size_t dimOfOut,
//     const gert::StorageShape *queryShape, const gert::StorageShape *keyShape) const
// {
...
//         "4th dimOfQ: %lu, 4th dimOfK: %lu, fourth dimOfOut: %lu", dimOfQ, dimOfK, dimOfOut), return custom::graphStatus::GRAPH_FAILED);
...
//         queryShape->GetStorageShape().GetDim(3), keyShape->GetStorageShape().GetDim(3)), return custom::graphStatus::GRAPH_FAILED);
//     return custom::graphStatus::GRAPH_SUCCESS;
// }

// custom::graphStatus IFATiling::CheckQKOutShape() const
// {
...
// custom::graphStatus IFATiling::CheckKeyShapeTensor(const gert::Shape &aShape) const
// {
...
// static bool IsQuant2ShapeValid(const at::IntArrayRef &inputParaShape, uint32_t headnum, uint32_t headsize)
// {
...
// custom::graphStatus IFATiling::CheckQuant2Shape(const at::IntArrayRef &inputParaShape) const
// {
//     auto headsize = headDim_; // D
//     auto headnum = numHeads_; // Q's N
//     bool validShape = IsQuant2ShapeValid(inputParaShape, headnum, headsize);
//     if (!validShape && inputParaShape.size() == DIM_BNSD) {
//         OP_LOGE(ifaContext_->opName,
...
//     if (!validShape && inputParaShape.size() == 3) { // dim is 3
//         OP_LOGE(ifaContext_->opName,
...
//     if (!validShape && inputParaShape.size() == DIM_BH) {
...
//     if (!validShape && inputParaShape.size() == 1) {
...
// custom::graphStatus IFATiling::CheckKVAntiQuantPerHead(const gert::Shape &inputParaShape) const
// {
...
// custom::graphStatus IFATiling::CheckKVAntiQuantPerChannel(const gert::Shape& inputParaShape) const
// {
//     std::string layOutStr = ifaContext_->layOut;
...
// custom::graphStatus IFATiling::CheckGqaAntiQuantPerChannel(const gert::Shape& inputParaShape) const
// {
//     std::string layOutStr = ifaContext_->layOut;
//     if (layOutStr == "BNSD") {
//         OP_CHECK_IF(
...
// custom::graphStatus IFATiling::CheckAntiQuantParam(const gert::Tensor *antiquantScaleTensor,
...
//     if (CheckKVAntiQuantParaShapeLegal(tmpAntiquantScale) == custom::graphStatus::GRAPH_FAILED) {
//         OP_LOGE(ifaContext_->opName, "illegal shape of antiquant scale.");
//         return custom::graphStatus::GRAPH_FAILED;
//     }
//     if (antiquantOffsetTensor != nullptr) {
//         auto tmpAntiquantOffset = antiquantOffsetTensor->GetStorageShape();
//         if (CheckKVAntiQuantParaShapeLegal(tmpAntiquantOffset) == custom::graphStatus::GRAPH_FAILED) {
//             return custom::graphStatus::GRAPH_FAILED;
//         }
//     }
...
// custom::graphStatus IFATiling::CheckAntiquantOffsetType(const gert::Tensor *antiquantOffsetTensor,
//                                                     const gert::CompileTimeTensorDesc *antiquantOffsetDesc,
...
// custom::graphStatus IFATiling::CheckMlaQueryRopeDesc() const
// {
//     if (quantFlag_) {
//         OP_CHECK_IF((ifaContext_->queryRope.desc->GetDataType() !=  at::ScalarType::BFloat16),
...
// custom::graphStatus IFATiling::CheckMlaQueryRopeBsndLayout(const gert::Shape &qRopeShape, const gert::Shape &qShape)
// {
...
// custom::graphStatus IFATiling::CheckMlaQueryRopeBnsdLayout(const gert::Shape &qRopeShape, const gert::Shape &qShape)
// {
//     OP_CHECK_IF((qRopeShape.GetDim(0) != qShape.GetDim(0)) ||
...
// custom::graphStatus IFATiling::CheckMlaQueryRopeNzLayout(const gert::Shape &qRopeShape, const gert::Shape &qShape)
// {
...
// custom::graphStatus IFATiling::CheckMlaQueryRope()
// {
//     if (CheckMlaQueryRopeDesc() != custom::graphStatus::GRAPH_SUCCESS) {
//         return custom::graphStatus::GRAPH_FAILED;
//     }
...
//     return custom::graphStatus::GRAPH_SUCCESS; // never here
// }

// custom::graphStatus IFATiling::CheckMlaAttrs() const
// {
...
// custom::graphStatus IFATiling::CheckMlaKeyRopeDesc() const
// {
//     if (quantFlag_) {
...
// custom::graphStatus IFATiling::CheckMlaKeyRopeShapeMatch(const gert::Shape keyRopeShape, const gert::Shape keyShape) const
// {
...
// custom::graphStatus IFATiling::CheckMlaKeyRopeBsndLayout(const gert::Shape keyRopeShape, const gert::Shape keyShape) const
// {
...
// custom::graphStatus IFATiling::CheckMlaKeyRopeBnsdLayout(const gert::Shape keyRopeShape, const gert::Shape keyShape) const
// {
//     OP_CHECK_IF(
...
// custom::graphStatus IFATiling::CheckMlaKeyRopeNzLayout(const gert::Shape keyRopeShape, const gert::Shape keyShape) const
// {
...
// custom::graphStatus IFATiling::CheckMlaKeyRope() const
// {
//     if (!pageAttentionFlag_) {
//         OP_LOGE(ifaContext_->opName, "only PageAttention KvCache is supported in MLA mode");
//         return custom::graphStatus::GRAPH_FAILED;
//     }
//     if (CheckMlaKeyRopeDesc() != custom::graphStatus::GRAPH_SUCCESS) {
//         return custom::graphStatus::GRAPH_FAILED;
//     }
...
//     if (CheckMlaKeyRopeShapeMatch(keyRopeShape, keyShape) != custom::graphStatus::GRAPH_SUCCESS) {
//         return custom::graphStatus::GRAPH_FAILED;
//     }
...
// custom::graphStatus IFATiling::CheckMlaMisc() const
// {
//     OP_CHECK_IF(antiQuantFlag_,
//         OP_LOGE(ifaContext_->opName, "antiquant is not supported in MLA."),
//         return custom::graphStatus::GRAPH_FAILED);

//     if (CheckDefaultMisc("MLA") != custom::graphStatus::GRAPH_SUCCESS) {
//         return custom::graphStatus::GRAPH_FAILED;
//     }

//     if (antiQuantFlag_) {
//         OP_CHECK_IF(inputQType_ != at::ScalarType::BFloat16,
...
custom::graphStatus IFATiling::CheckGqaAntiquantTensor() const
{
    ...
    // if (CheckGqaAntiquantType() != custom::graphStatus::GRAPH_SUCCESS) {
    //     return custom::graphStatus::GRAPH_FAILED;
    // }
    return custom::graphStatus::GRAPH_SUCCESS;
}

// custom::graphStatus IFATiling::CheckGqaAntiquantType() const
// {
//     // check type
//     if (antiquantMode_ == PER_CHANNEL_MODE) {
//         OP_CHECK_IF(ifaContext_->keyAntiquantScale.desc->GetDataType() != at::ScalarType::BFloat16,
//         OP_LOGE(ifaContext_->opName, "In IFA GQA with KV NZ perchannel antiquant, the key's dequant scale type should be BFLOAT16, but now it's %s!",
//         DataTypeToSerialString(ifaContext_->keyAntiquantScale.desc->GetDataType()).c_str()), return custom::graphStatus::GRAPH_FAILED);
//         OP_CHECK_IF(ifaContext_->valueAntiquantScale.desc->GetDataType() != at::ScalarType::BFloat16,
//         OP_LOGE(ifaContext_->opName, "In IFA GQA with KV NZ perchannel antiquant, the value's dequant scale type should be BFLOAT16, but now it's %s!",
//         DataTypeToSerialString(ifaContext_->keyAntiquantScale.desc->GetDataType()).c_str()), return custom::graphStatus::GRAPH_FAILED);
//     } else if (antiquantMode_ == PER_TOKEN_MODE) {
//         OP_CHECK_IF(ifaContext_->keyAntiquantScale.desc->GetDataType() != ge::DT_FLOAT,
//         OP_LOGE(ifaContext_->opName, "In IFA GQA with KV NZ pertoken antiquant, the key's dequant scale type should be DT_FLOAT, but now it's %s!",
//         DataTypeToSerialString(ifaContext_->keyAntiquantScale.desc->GetDataType()).c_str()), return custom::graphStatus::GRAPH_FAILED);
//         OP_CHECK_IF(ifaContext_->valueAntiquantScale.desc->GetDataType() != ge::DT_FLOAT,
//         OP_LOGE(ifaContext_->opName, "In IFA GQA with KV NZ pertoken antiquant, the value's dequant scale type should be DT_FLOAT, but now it's %s!",
//         DataTypeToSerialString(ifaContext_->keyAntiquantScale.desc->GetDataType()).c_str()), return custom::graphStatus::GRAPH_FAILED);
//     }
//     return custom::graphStatus::GRAPH_SUCCESS;
// }
...
custom::graphStatus IFATiling::CheckGqaTensorEmpty() const
{
    ...
    OP_CHECK_IF(!ifaContext_->lseOut.shape.empty(),
