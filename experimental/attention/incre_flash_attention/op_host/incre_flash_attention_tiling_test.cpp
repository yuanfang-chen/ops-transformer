...
custom::graphStatus IFATiling::ProcessSharedPrefixLen()
{
    ...
    maxActualPrefixLen_ = sMaxPrefix_;
    auto actulLenShape = ifaContext_->actualSharedPrefixLen.data;
...
custom::graphStatus IFATiling::GenTilingKey() const
{
    ...
    printf("fd : %d layout : %d, antiquantMode_ : %d", splitKvVal, layoutVal, static_cast<uint8_t>(antiquantMode_));
...
custom::graphStatus IFATiling::DoSubOpTiling(IFAContext& ifaContext) {
    // OP_LOGE(ifaContext_->opName, " WWWWWWWWWWWWW DEBUG!");
    printf("ifaContext.antiquantOffset has value :%d", ifaContext.antiquantOffset.hasValue);
    printf("ifaContext.keyAntiquantScale has value :%d", ifaContext.keyAntiquantScale.hasValue);
    printf("ifaContext.valueSharedPrefix has value :%d", ifaContext.valueSharedPrefix.hasValue);
    printf("ifaContext.actualSharedPrefixLen empty :%d", ifaContext.actualSharedPrefixLen.data.empty());
    
        
    std::cout << "[DEBUG] key dtype enum = " << static_cast<int>(ifaContext.key.dType)
              << ", name = " << c10::toString(ifaContext.key.dType) << std::endl;

    switch (ifaContext.key.dType) {
        case at::ScalarType::Half:
            printf("ifaContext.key.dType is fp16\n");
            break;
        case at::ScalarType::BFloat16:
            printf("ifaContext.key.dType is bf16\n");
            break;
        case at::ScalarType::Char:
            printf("ifaContext.key.dType is int8\n");
            break;
        default:
            OP_LOGE(ifaContext_->opName, "not support inputKvType%d", ifaContext.key.dType);
            return custom::graphStatus::GRAPH_FAILED;
    }
    inputKvType_ = ifaContext_->key.dType;
    switch (inputKvType_) {
        ...
        case at::ScalarType::Char:
            ...
            break;
        default:
            // printf("not support inputKvType%d\n",inputKvType_);
            OP_LOGE(ifaContext_->opName, "not support inputKvType%d", inputKvType_);
            return custom::graphStatus::GRAPH_FAILED;
    }
    ...
    printf("data: [");
    for (size_t i = 0; i < ifaContext.actualSeqLengthsQ.data.size(); ++i) {
        printf("%ld", ifaContext.actualSeqLengthsQ.data[i]);
        if (i < ifaContext.actualSeqLengthsQ.data.size() - 1) {
            printf(", ");
        }
    }
    printf("]\n");
