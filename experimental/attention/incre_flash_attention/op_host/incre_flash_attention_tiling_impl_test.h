...
struct IFAContext {
    ...
    OptionalParaInfo queryPaddingSize;
    ...
    OptionalParaInfo dequantScaleQuery;
    OptionalParaInfo qStartIdx;
    OptionalParaInfo kvStartIdx;
...
private:
    ...
    // custom::graphStatus SharedPrefixCheckShapes(const gert::Shape &keyShape, const gert::Shape &valueShape) const;

    custom::graphStatus CheckUbSpace();
    custom::graphStatus CheckPABlockSize() const;
    custom::graphStatus SetL2CacheFlag();
    custom::graphStatus SetQuantFlag();

    // custom::graphStatus CheckQuant2Shape(const gert::Shape &inputParaShape) const;
    ...
    // custom::graphStatus CheckKVAntiQuantParamsShapeInPagedAttention(const gert::Shape &inputParaShape) const;
