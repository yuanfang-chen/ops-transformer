...
class GeneralizedSFA:
    def __init__(self, layout_q, layout_kv, q_type, ori_kv_type, cmp_kv_type, B, S1, T1, N1, N2, D, K, block_num1, block_num2,
                 block_size, cu_seqlens_q, seqused_kv, softmax_scale, cmp_ratio, ori_mask_mode, cmp_mask_mode,
        ...
        for i_B in range(B):
            ...
            for i_N2 in range(self.N2):
                ...
                for i_S1 in range(cur_act_q):
                    ...
                    empty_flag, k_sparse = self.gather_cmp_kv(cmp_k_bnsd, topk_id, i_B, i_N2, i_S1, cur_cmp_act_kv,
                                                              cur_act_q)
...
def test_sas_process(params):
    ...
    block_size, cu_seqlens_q, seqused_kv, softmax_scale, cmp_ratio, ori_mask_mode, cmp_mask_mode, ori_win_left, \
    ori_win_right = params
    ...
    if len(seqused_kv) != B:
        ...
        ori_max_block_num_per_batch = math.ceil(ori_max_s2 / block_size)

        cmp_max_s2 = math.floor(ori_max_s2 / cmp_ratio)
        cmp_max_block_num_per_batch = math.ceil(cmp_max_s2 / block_size)
    ...
    for cur_ori_act_kv in seqused_kv:
        cur_ori_kv_block_num = math.ceil(cur_ori_act_kv / block_size)
        ...
        cur_cmp_kv_block_num = math.ceil(cur_cmp_act_kv / block_size)
    ...
    # [B, S, N, D] expand to [B, ori_max_block_num_per_batch * block_size, N, D]
    ori_k_expand = torch.zeros((B, N2, ori_max_block_num_per_batch * block_size, D), dtype=ori_kv_type)
    ori_k_expand[:, :, :ori_max_s2, :] = ori_k_bnsd
    ori_k_in_pa_shape = torch.zeros((block_num1, block_size, N2, D), dtype=ori_kv_type)
            ...
            if cur_block_id == -1:
                ...
                for i_N2 in range(N2):
                    ori_k_in_pa_shape[cur_block_id, :, i_N2, :] = \
                        ori_k_expand[i_B, i_N2, block_start_pos:block_start_pos + block_size, :]

    cmp_k_expand = torch.zeros((B, N2, cmp_max_block_num_per_batch * block_size, D), dtype=cmp_kv_type)
    cmp_k_expand[:, :, :cmp_max_s2, :] = cmp_k_bnsd
    cmp_k_in_pa_shape = torch.zeros((block_num2, block_size, N2, D), dtype=cmp_kv_type)
            ...
            if cur_block_id == -1:
                ...
                for i_N2 in range(N2):
                    cmp_k_in_pa_shape[cur_block_id, :, i_N2, :] = \
                        cmp_k_expand[i_B, i_N2, block_start_pos:block_start_pos + block_size, :]
