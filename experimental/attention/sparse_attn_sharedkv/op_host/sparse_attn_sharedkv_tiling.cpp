...
void SASTilingCheck::Init()
{
    ...
    vHeadDim_ = sasInfo_.vHeadDim;
    oriBlockSize_ = sasInfo_.oriBlockSize;
    cmpBlockSize_ = sasInfo_.oriBlockSize;
...
ge::graphStatus SASTilingCheck::CheckSingleParaOriBlockTable() const
{
    OP_CHECK_IF((oriBlockSize_ <= 0 || oriBlockSize_ > 1024 ||
        ...
        OP_LOGE(opName_, "oriBlockSize_ should be in range [1, 1024], and be aligned to 16 , but got: %ld.",
            oriBlockSize_),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SASTilingCheck::CheckSingleParaSinks() const
{
    OP_CHECK_IF((opParamInfo_.sinks.tensor->GetShapeSize() != n1Size_),
 	    OP_LOGE(opName_, "input metadata dim 0 must be N1."),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SASTilingCheck::CheckSingleParaMetadata() const
{
    OP_CHECK_IF((opParamInfo_.metadata.tensor->GetShapeSize() != METADATA_LIMIT),
 	    OP_LOGE(opName_, "input metadata dim 0 must be 1024."),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SASTilingCheck::CheckSinglePara() const
{
    if (ge::GRAPH_SUCCESS != CheckSingleParaQuery() ||
        ...
        ge::GRAPH_SUCCESS != CheckSingleParaNumHeads() ||            // 空实现
        ge::GRAPH_SUCCESS != CheckSingleParaKvHeadNums() ||          // 空实现
        ...
        ge::GRAPH_SUCCESS != CheckSingleParaCmpRatio() ||            // 空实现
        ge::GRAPH_SUCCESS != CheckSingleParaOriMaskMode() ||         // 空实现
        ge::GRAPH_SUCCESS != CheckSingleParaCmpMaskMode() ||         // 空实现
        ge::GRAPH_SUCCESS != CheckSingleParaOriWinLeft() ||          // 空实现
        ge::GRAPH_SUCCESS != CheckSingleParaOriWinRight()) {         // 空实现
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SASTilingCheck::CheckParaExistence() const
{
    std::map<std::string, const void *> ParamExistMap = {
        ...
        {"cmpBlockTable", opParamInfo_.cmpBlockTable.tensor},
    };
...
ge::graphStatus SASTilingCheck::CheckFeatureShape() const
{
    ...
    OP_CHECK_IF(n1Size_ <= 1 || n1Size_ > 128,
        OP_LOGE(opName_, "q_head_num should be in range [1, 128], but got %u", n1Size_),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(cmpRatio_ < 0 || cmpRatio_ > 128 || (cmpRatio_ & (cmpRatio_ - 1)) != 0U,
        OP_LOGE(opName_, "cmpRatio_ should be in 0, 2, 4, 8, 16, 32, 64, 128, but got %u", cmpRatio_),
        return ge::GRAPH_FAILED);

    OP_CHECK_IF(oriBlockSize_ != 4,
        OP_LOGE(opName_, "oriBlockSize_ should be 4, but got %u", oriBlockSize_),
        return ge::GRAPH_FAILED);

    OP_CHECK_IF(cmpBlockSize_ != 3,
        OP_LOGE(opName_, "cmpBlockSize_ should be 3, but got %u", cmpBlockSize_),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
{
    SetSASShapeCompare();
    if (ge::GRAPH_SUCCESS != CheckOriAndCmpKv() ||
...
ge::graphStatus SASTilingCheck::Process()
{
    ...
    if (CheckSinglePara() != ge::GRAPH_SUCCESS ||
        CheckParaExistence() != ge::GRAPH_SUCCESS || // 没看懂
...
ge::graphStatus SparseAttnSharedkvTiling::DoOpTiling(SASTilingInfo *tilingInfo)
{
    ...
    constexpr uint32_t M_BASE_SIZE = 512;             // m轴基本块大小    // oriBlockSize_ = sasInfo_.oriBlockSize;
