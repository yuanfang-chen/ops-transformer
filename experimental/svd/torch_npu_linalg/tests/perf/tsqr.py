...
def profile_qr(shape_list):
    ...
    with torch_npu.profiler.profile(
        ...
        on_trace_ready=torch_npu.profiler.tensorboard_trace_handler("./perf_result/tsqr"),
...
def benchmark_qr(model):
    with torch.no_grad():
        ...
        for idx, shapes in enumerate(model):
            ...
            # Timing npu_linalg.tsqr on NPU
            ascend_times = []
            for i in range(100):
                npu_x = generate_tensors(shapes)
                torch.npu.synchronize()
                s = time()
                npu_q, npu_r = torch.ops.npu_linalg.tsqr(npu_x)
                torch.npu.synchronize()
                f = time()
...
if __name__ == '__main__':
    ...
    if args.mode == "profile":
        profile_qr(shape_list)
    elif args.mode == "benchmark":
        benchmark_qr(shape_list)
