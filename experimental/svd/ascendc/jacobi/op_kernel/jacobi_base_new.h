...
#define JACOBI_BASE_H
  ...
  // #define DEBUG_MODE
// #define RUNTIME_NORM_GENERATION
#ifdef DEBUG_MODE

#define PRINT_VAL(X)   printf("Variable %s has value %d\n", #X, (int)X)
// #define PRINT_TENSOR(X, SIZE) PipeBarrier<PIPE_ALL>();PRINT_TENSOR_(X,SIZE);PipeBarrier<PIPE_ALL>();
#else
#define PRINT_VAL(X)
#endif
...
private:
        ...
        const LocalTensor<float>& bucket2, const LocalTensor<float>& norms2, const RotateMaskInfo& maskInfo)
    {
...
#ifdef RUNTIME_NORM_GENERATION
        {
            ...
            for (uint16_t rowIdx = 0; rowIdx < maskInfo.leftBucketSize; ++rowIdx) {
                Mul(tmpBucket1[rowIdx * nSizeAligned], bucket1[rowIdx * recordSizeAligned],
                    bucket1[rowIdx * recordSizeAligned], rowMask, rowRepeatsNum, { 1,1,1, repeatStride, repeatStride, repeatStride });
            }
            for (uint16_t rowIdx = 0; rowIdx < maskInfo.rightBucketSize; ++rowIdx) {
                Mul(tmpBucket2[rowIdx * nSizeAligned], bucket2[rowIdx * recordSizeAligned],
                    bucket2[rowIdx * recordSizeAligned], rowMask, rowRepeatsNum, { 1,1,1, repeatStride, repeatStride, repeatStride });
            }
            PipeBarrier<PIPE_V>();
            for (uint16_t rowIdx = 0; rowIdx < maskInfo.leftBucketSize; ++rowIdx) {
        ...
        const LocalTensor<float>& dotProductTmp1, const LocalTensor<float>& dotProductTmp2)
    {
        ...
        for (uint16_t rowIdx = 0; rowIdx < maskInfo.numPairs; ++rowIdx) {
            ...
            Mul(tmpBucket1[rowIdx * nSizeAligned], bucket1[leftRowIdx * recordSizeAligned],
                bucket2[rightRowIdx * recordSizeAligned], rowMask, rowRepeatsNum, { 1,1,1, repeatStride, repeatStride, repeatStride });
        }
        ...
        const LocalTensor<float>& tmpBucket, const LocalTensor<float>& dotProductTmp, const uint8_t(&rowIds)[sizeof(uint64_t)], const uint32_t bucketSize)
    {
        ...
        for (uint16_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
            ...
            Mul(tmpBucket[rowIdx * nSizeAligned], bucket[rowId * recordSizeAligned],
                bucket[rowId * recordSizeAligned], rowMask, rowRepeatsNum, { 1,1,1, repeatStride, repeatStride, repeatStride });
        }
        PipeBarrier<PIPE_V>();
        for (uint16_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
        ...
        const LocalTensor<float>& dotProducts, const LocalTensor<float>& normVector, const LocalTensor<float>& bucket, const uint32_t bucketSize, const RotateMaskInfo& maskInfo)
    {
...
#ifdef RUNTIME_NORM_GENERATION
        {
            ...
            for (uint16_t rowID = 0; rowID < bucketSize; ++rowID) {
                Mul(tmpBucket1[rowID * nSizeAligned], bucket[rowID * recordSizeAligned], bucket[rowID * recordSizeAligned], rowMask, rowRepeatsNum, { 1,1,1, repeatStride, repeatStride, repeatStride });
            }
        ...
        const uint32_t bucket2Size, const LocalTensor<float>& sinTensor, const LocalTensor<float>& cosTensor, const RotateMaskInfo& maskInfo)
    {
        ...
        PipeBarrier<PIPE_V>();
        if constexpr (BUCKET_1_TARGET) {
            #define INIT_MASK float,true
            #define SKIP_INIT_MASK float,false
            uint8_t maskRepeatStride = rowMask * sizeof(float) / 32;
            {
                uint16_t rowIdxWithOffset = maskInfo.rightRowsIds[0];
                Mul<INIT_MASK>(calcTmpTensor1, bucket2[rowIdxWithOffset * recordSizeAligned], sinBroadcastedValues, rowMask, rowRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
                Mul<SKIP_INIT_MASK>(calcTmpTensor2, bucket2[rowIdxWithOffset * recordSizeAligned], cosBroadcastedValues, rowMask, rowRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
            }
            for (uint32_t rowIdx = 1; rowIdx < bucketSize; ++rowIdx) {
                uint16_t rowIdxWithOffset = maskInfo.rightRowsIds[rowIdx];
                Mul<SKIP_INIT_MASK>(calcTmpTensor1[rowIdx * nSizeAligned], bucket2[rowIdxWithOffset * recordSizeAligned], sinBroadcastedValues[rowIdx * rowMask], rowMask, rowRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
                Mul<SKIP_INIT_MASK>(calcTmpTensor2[rowIdx * nSizeAligned], bucket2[rowIdxWithOffset * recordSizeAligned], cosBroadcastedValues[rowIdx * rowMask], rowMask, rowRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
            }
            PipeBarrier<PIPE_V>();
            ...
            for (uint32_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
                MulAddDst(calcTmpTensor1[rowIdx * nSizeAligned], cosBroadcastedValues[rowIdx * rowMask], bucket1[rowIdx * recordSizeAligned], rowMask, rowRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
                MulAddDst(calcTmpTensor2[rowIdx * nSizeAligned], sinBroadcastedValues[rowIdx * rowMask], bucket1[rowIdx * recordSizeAligned], rowMask, rowRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
            }
            PipeBarrier<PIPE_V>();
            for (uint32_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
                uint16_t rowIdxWithOffset = maskInfo.rightRowsIds[rowIdx];
                Copy(bucket1[rowIdx * recordSizeAligned], calcTmpTensor1[rowIdx * nSizeAligned], rowMask, rowRepeatsNum, { 1,1, maskRepeatStride, maskRepeatStride });
                Copy(bucket2[rowIdxWithOffset * recordSizeAligned], calcTmpTensor2[rowIdx * nSizeAligned], rowMask, rowRepeatsNum, { 1,1, maskRepeatStride, maskRepeatStride });
            }
            PipeBarrier<PIPE_V>();
            for (uint32_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
                uint16_t rowIdxWithOffset = maskInfo.rightRowsIds[rowIdx];
                Mul(calcTmpTensor1[rowIdx * uNSizeAligned], sinBroadcastedValues[rowIdx * rowMask], bucket2[rowIdxWithOffset * recordSizeAligned + nSizeAligned], rowMask, uRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
                Mul(calcTmpTensor2[rowIdx * uNSizeAligned], cosBroadcastedValues[rowIdx * rowMask], bucket2[rowIdxWithOffset * recordSizeAligned + nSizeAligned], rowMask, uRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
            }
            PipeBarrier<PIPE_V>();
            ...
            PipeBarrier<PIPE_V>();
            for (uint32_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
                MulAddDst(calcTmpTensor1[rowIdx * uNSizeAligned], cosBroadcastedValues[rowIdx * rowMask], bucket1[rowIdx * recordSizeAligned + nSizeAligned], rowMask, uRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
                MulAddDst(calcTmpTensor2[rowIdx * uNSizeAligned], sinBroadcastedValues[rowIdx * rowMask], bucket1[rowIdx * recordSizeAligned + nSizeAligned], rowMask, uRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
            }
            PipeBarrier<PIPE_V>();
            for (uint32_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
                uint16_t rowIdxWithOffset = maskInfo.rightRowsIds[rowIdx];
                Copy(bucket1[rowIdx * recordSizeAligned + nSizeAligned], calcTmpTensor1[rowIdx * uNSizeAligned], rowMask, uRepeatsNum, { 1,1, maskRepeatStride, maskRepeatStride });
                Copy(bucket2[rowIdxWithOffset * recordSizeAligned + nSizeAligned], calcTmpTensor2[rowIdx * uNSizeAligned], rowMask, uRepeatsNum, { 1,1, maskRepeatStride, maskRepeatStride });
            }
            ...
            uint8_t maskRepeatStride = rowMask * sizeof(float) / 32;
            for (uint32_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
                Mul(calcTmpTensor1[rowIdx * nSizeAligned], bucket2[rowIdx * recordSizeAligned], sinBroadcastedValues[rowIdx * rowMask], rowMask, rowRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
                Mul(calcTmpTensor2[rowIdx * nSizeAligned], bucket2[rowIdx * recordSizeAligned], cosBroadcastedValues[rowIdx * rowMask], rowMask, rowRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
            }
            PipeBarrier<PIPE_V>();
            Muls(calcTmpTensor1, calcTmpTensor1, -1.f, bucketSize * nSizeAligned);
            ...
            for (uint32_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
                ...
                MulAddDst(calcTmpTensor1[rowIdx * nSizeAligned], cosBroadcastedValues[rowIdx * rowMask], bucket1[rowIdxWithOffset * recordSizeAligned], rowMask, rowRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
                MulAddDst(calcTmpTensor2[rowIdx * nSizeAligned], sinBroadcastedValues[rowIdx * rowMask], bucket1[rowIdxWithOffset * recordSizeAligned], rowMask, rowRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
            }
            PipeBarrier<PIPE_V>();
            for (uint32_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
                uint16_t rowIdxWithOffset = maskInfo.leftRowsIds[rowIdx];
                Copy(bucket1[rowIdxWithOffset * recordSizeAligned], calcTmpTensor1[rowIdx * nSizeAligned], rowMask, rowRepeatsNum, { 1,1, maskRepeatStride, maskRepeatStride });
                Copy(bucket2[rowIdx * recordSizeAligned], calcTmpTensor2[rowIdx * nSizeAligned], rowMask, rowRepeatsNum, { 1,1, maskRepeatStride, maskRepeatStride });
            }
            PipeBarrier<PIPE_V>();
            for (uint32_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
                Mul(calcTmpTensor1[rowIdx * nSizeAligned], bucket2[rowIdx * recordSizeAligned + nSizeAligned], sinBroadcastedValues[rowIdx * rowMask], rowMask, uRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
                Mul(calcTmpTensor2[rowIdx * nSizeAligned], bucket2[rowIdx * recordSizeAligned + nSizeAligned], cosBroadcastedValues[rowIdx * rowMask], rowMask, uRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
            }
            PipeBarrier<PIPE_V>();
            ...
            for (uint32_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
                uint16_t rowIdxWithOffset = maskInfo.leftRowsIds[rowIdx];
                MulAddDst(calcTmpTensor1[rowIdx * nSizeAligned], cosBroadcastedValues[rowIdx * rowMask], bucket1[rowIdxWithOffset * recordSizeAligned + nSizeAligned], rowMask, uRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
                MulAddDst(calcTmpTensor2[rowIdx * nSizeAligned], sinBroadcastedValues[rowIdx * rowMask], bucket1[rowIdxWithOffset * recordSizeAligned + nSizeAligned], rowMask, uRepeatsNum,
                    { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
            }
            PipeBarrier<PIPE_V>();
            for (uint32_t rowIdx = 0; rowIdx < bucketSize; ++rowIdx) {
                uint16_t rowIdxWithOffset = maskInfo.leftRowsIds[rowIdx];
                Copy(bucket1[rowIdxWithOffset * recordSizeAligned + nSizeAligned], calcTmpTensor1[rowIdx * nSizeAligned], rowMask, uRepeatsNum, { 1,1, maskRepeatStride, maskRepeatStride });
                Copy(bucket2[rowIdx * recordSizeAligned + nSizeAligned], calcTmpTensor2[rowIdx * nSizeAligned], rowMask, uRepeatsNum, { 1,1, maskRepeatStride, maskRepeatStride });
            }
        }
        PipeBarrier<PIPE_V>();
        ...
        const LocalTensor<float>& cosTensor, const RotateMaskInfo& maskInfo)
    {
        ...
        for (uint32_t pairID = 0; pairID < maskInfo.numPairs; ++pairID) {
            ...
            Mul(calcTmpTensor1[pairID * nSizeAligned], sinBroadcastedValues[pairID * rowMask], bucket[secondRowIdx * recordSizeAligned], rowMask, rowRepeatsNum,
                { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
            Mul(calcTmpTensor2[pairID * nSizeAligned], cosBroadcastedValues[pairID * rowMask], bucket[secondRowIdx * recordSizeAligned], rowMask, rowRepeatsNum,
                { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
        }
        PipeBarrier<PIPE_V>();
        Muls(calcTmpTensor1, calcTmpTensor1, -1.f, bucketSize * nSizeAligned);
        ...
        for (uint32_t pairID = 0; pairID < maskInfo.numPairs; ++pairID) {
            ...
            MulAddDst(calcTmpTensor1[pairID * nSizeAligned], bucket[firstRowIdx * recordSizeAligned], cosBroadcastedValues[pairID * rowMask], rowMask, rowRepeatsNum,
                { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
            MulAddDst(calcTmpTensor2[pairID * nSizeAligned], bucket[firstRowIdx * recordSizeAligned], sinBroadcastedValues[pairID * rowMask], rowMask, rowRepeatsNum,
                { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
        }
        PipeBarrier<PIPE_V>();
        for (uint32_t pairID = 0; pairID < maskInfo.numPairs; ++pairID) {
            uint8_t firstRowIdx = maskInfo.leftRowsIds[pairID];
            uint8_t secondRowIdx = maskInfo.rightRowsIds[pairID];
            Copy(bucket[firstRowIdx * recordSizeAligned], calcTmpTensor1[pairID * nSizeAligned], rowMask, rowRepeatsNum, { 1,1, maskRepeatStride, maskRepeatStride });
            Copy(bucket[secondRowIdx * recordSizeAligned], calcTmpTensor2[pairID * nSizeAligned], rowMask, rowRepeatsNum, { 1,1, maskRepeatStride, maskRepeatStride });
        }
        ...
        for (uint32_t pairID = 0; pairID < maskInfo.numPairs; ++pairID) {
            uint8_t secondRowIdx = maskInfo.rightRowsIds[pairID];
            Mul(calcTmpTensor1[pairID * nSizeAligned], sinBroadcastedValues[pairID * rowMask], bucket[secondRowIdx * recordSizeAligned + nSizeAligned], rowMask, uRepeatsNum,
                { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
            Mul(calcTmpTensor2[pairID * nSizeAligned], cosBroadcastedValues[pairID * rowMask], bucket[secondRowIdx * recordSizeAligned + nSizeAligned], rowMask, uRepeatsNum,
                { 1, 1, 1, maskRepeatStride, 0, maskRepeatStride });
        }
        PipeBarrier<PIPE_V>();
        Muls(calcTmpTensor1, calcTmpTensor1, -1.f, bucketSize * nSizeAligned);
        ...
        for (uint32_t pairID = 0; pairID < maskInfo.numPairs; ++pairID) {
            uint8_t firstRowIdx = maskInfo.leftRowsIds[pairID];
            MulAddDst(calcTmpTensor1[pairID * nSizeAligned], bucket[firstRowIdx * recordSizeAligned + nSizeAligned], cosBroadcastedValues[pairID * rowMask], rowMask, uRepeatsNum,
                { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
            MulAddDst(calcTmpTensor2[pairID * nSizeAligned], bucket[firstRowIdx * recordSizeAligned + nSizeAligned], sinBroadcastedValues[pairID * rowMask], rowMask, uRepeatsNum,
                { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
        }
        PipeBarrier<PIPE_V>();
        for (uint32_t pairID = 0; pairID < maskInfo.numPairs; ++pairID) {
            uint8_t firstRowIdx = maskInfo.leftRowsIds[pairID];
            uint8_t secondRowIdx = maskInfo.rightRowsIds[pairID];
            Copy(bucket[firstRowIdx * recordSizeAligned + nSizeAligned], calcTmpTensor1[pairID * nSizeAligned], rowMask, uRepeatsNum, { 1,1, maskRepeatStride, maskRepeatStride });
            Copy(bucket[secondRowIdx * recordSizeAligned + nSizeAligned], calcTmpTensor2[pairID * nSizeAligned], rowMask, uRepeatsNum, { 1,1, maskRepeatStride, maskRepeatStride });
        }
    ...
    __aicore__ inline void LoadBucket(const LocalTensor<float>& bucketSpace, const LocalTensor<float>& normVector, const uint32_t bucketBegin, const uint32_t bucketEnd, const bool isFirstLoadDataLocal)
    {
        ...
        if (!isFirstLoadDataLocal) {
            ...
            PipeBarrier<PIPE_V>();
            Duplicate(bucketSpace, 0.f, numRows * recordSizeAligned);
            PipeBarrier<PIPE_V>();
            uint32_t nSizeAligned32 = ((((nSize * sizeof(AType)) + 31) / 32) * 32) / sizeof(AType);
            for (uint32_t oneId = bucketBegin; oneId < bucketEnd; ++oneId) {
                uint32_t localBlockOffset = (oneId / rowMask) * rowMask;
                uint32_t onePosition = oneId % rowMask;
                uint64_t localMask = 1;
                localMask <<= onePosition;
                uint64_t mask[2] = { localMask, 0 };
                uint32_t rowOffset = (oneId - bucketBegin) * recordSizeAligned + nSizeAligned;
            ...
            DataCopyPad(bucketSpace, aGm[batchOffsetI + bucketBegin * nSize], { numRows, static_cast<uint16_t>(nSize * sizeof(AType)), 0, static_cast<uint16_t>((recordSizeAligned - nSizeAligned32) * sizeof(AType) / 32) },
    ...
    __aicore__ inline void SaveOutputs(const LocalTensor<float>& bucketSpace, const LocalTensor<float>& normVector, const uint32_t bucketBegin, const uint32_t bucketEnd)
    {
        ...
        for (uint32_t rowIdx = 0; rowIdx < numRows; ++rowIdx) {
            MulAddDst(tmpCurrMaskSpace[rowIdx*rowMask], bucketSpace[rowIdx*recordSizeAligned], bucketSpace[rowIdx*recordSizeAligned], rowMask, rowRepeatsNum, {1, 1, 1, 0, maskRepeatStride, maskRepeatStride});
        }
        PipeBarrier<PIPE_V>();
        ...
        for (uint32_t rowIdx = 0; rowIdx < numRows; ++rowIdx) {
            Div(bucketSpace[rowIdx * recordSizeAligned], bucketSpace[rowIdx * recordSizeAligned], tmpCurrMaskSpace[rowIdx * rowMask],
        ...
        DataCopyPad(uGm[batchOffsetU + bucketBegin * uNSize], bucketSpace[nSizeAligned],
                {static_cast<uint16_t>(numRows), static_cast<uint16_t>(uNSize * sizeof(AType)), 
                 static_cast<uint16_t>((recordSizeAligned - uNSizeAligned32) * sizeof(AType) / 32), 0 });
        ...
        DataCopyPad(vGm[batchOffsetV + bucketBegin * vNSize], bucketSpace,
                    { static_cast<uint16_t>(numRows), static_cast<uint16_t>(vNSize * sizeof(AType)), static_cast<uint16_t>((recordSizeAligned - vNSizeAligned32) * sizeof(AType) / 32),0 });
    }

    __aicore__ inline void GetDiagonal(const GlobalTensor<AType>& source) {
        //ToDo: In future Move saving S matrice to last stage and last phase of applying global stage
        constexpr uint8_t SMNE = SMALL_MASK_NUM_ELEMENTS<float>();
        LocalTensor<float>& squareNorms = tmpBlock1;
        LocalTensor<float>& singularValues = tmpBlock4;
        //TODO: when multipleSaving=true is not support now
        LocalTensor<float>& srcRows = bucketSpace1;
        uint32_t uNSizeAligned32 = (((uNSize * sizeof(float) + 31) / 32) * 32) / sizeof(float);
        uint32_t vNSizeAligned32 = (((vNSize * sizeof(float) + 31) / 32) * 32) / sizeof(float);
        uint32_t numBuckets = (mSize + maxBucketSize - 1) / maxBucketSize;
        const uint8_t maskRepeatStride = rowMask * sizeof(float) / 32;
        const uint8_t maskRepeatStrideForSMNE = SMNE * sizeof(float) / 32;
        Duplicate(singularValues, 0.f, SMNE);
        //TODO: replace on cast

        uint32_t singularValuesOffset = 0;
        for (int bucketIdx = 0; bucketIdx < numBuckets; ++bucketIdx) {
            uint32_t bucketBegin = bucketIdx * maxBucketSize;
            uint32_t bucketEnd = MIN(bucketBegin + maxBucketSize, mSize);
            uint32_t currentBucketSize = bucketEnd - bucketBegin;
            uint32_t sBucketBegin = 0;
            uint32_t sBucketEnd = 0;
            uint32_t sBucketSize = 0;
            if (bucketBegin < sMNSize) {
                sBucketBegin = bucketBegin;
                sBucketEnd = MIN(sMNSize, bucketEnd);
                sBucketSize = sBucketEnd - sBucketBegin;
            }
            event_t evtVToMte2 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_MTE2));
            SetFlag<HardEvent::V_MTE2>(evtVToMte2);
            WaitFlag<HardEvent::V_MTE2>(evtVToMte2);
            LoadBucket(srcRows, tmpBlock32, bucketBegin, bucketEnd, false);

            if (sBucketSize != 0) {
                event_t evtMte2ToV = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE2_V));
                SetFlag<HardEvent::MTE2_V>(evtMte2ToV);
                WaitFlag<HardEvent::MTE2_V>(evtMte2ToV);
                Duplicate(tmpMaskSpace, 0.f, sBucketSize * rowMask);
                for (uint32_t rowIdx = 0; rowIdx < sBucketSize; ++rowIdx) {
                    Mul(bucketSpace2[rowIdx * recordSizeAligned], srcRows[rowIdx * recordSizeAligned], srcRows[rowIdx * recordSizeAligned],
                        rowMask, rowRepeatsNum, { 1, 1, 1, maskRepeatStride, maskRepeatStride, maskRepeatStride });
                }
                PipeBarrier<PIPE_V>();
                for (uint32_t rowIdx = 0; rowIdx < sBucketSize; ++rowIdx) {
                    Add(tmpMaskSpace[rowIdx * rowMask], bucketSpace2[rowIdx * recordSizeAligned], tmpMaskSpace[rowIdx * rowMask],
                        rowMask, rowRepeatsNum, { 1, 1, 1, 0, maskRepeatStride, 0 });
                }
                PipeBarrier<PIPE_V>();
                WholeReduceSum(squareNorms, tmpMaskSpace, rowMask, static_cast<uint8_t>(sBucketSize), 1, 1, maskRepeatStride);
                PipeBarrier<PIPE_V>();
                Sqrt(singularValues, squareNorms, SMNE);
                PipeBarrier<PIPE_V>();
                //Doesnot work in simulator
                Brcb(tmpMemSpace, singularValues, 1, { 1, 8 });
                PipeBarrier<PIPE_V>();
                Brcb(tmpMaskSpace, tmpMemSpace, static_cast<uint8_t>(sBucketSize), { 1, 8 });
                PipeBarrier<PIPE_V>();
                for (uint32_t rowIdx = 0; rowIdx < sBucketSize; ++rowIdx) {
                    Div(srcRows[rowIdx * recordSizeAligned], srcRows[rowIdx * recordSizeAligned], tmpMaskSpace[rowIdx * rowMask],
                        rowMask, rowRepeatsNum, { 1, 1, 1, maskRepeatStride, maskRepeatStride, 0 });
                }
                PipeBarrier<PIPE_V>();
                event_t evtVToMte3 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::V_MTE3));
                ...
                event_t evtMte2ToMte3 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE2_MTE3));
                SetFlag<HardEvent::MTE2_MTE3>(evtMte2ToMte3);
                WaitFlag<HardEvent::MTE2_MTE3>(evtMte2ToMte3);
            }

            DataCopyPad(uGm[bucketBegin * uNSize], srcRows[nSizeAligned],
                { static_cast<uint16_t>(currentBucketSize), static_cast<uint16_t>(uNSize * sizeof(AType)), static_cast<uint16_t>((recordSizeAligned - uNSizeAligned32) * sizeof(AType) / 32),0 });
            if (sBucketSize != 0) {
                uint32_t singularValuesNumAligned32 = (((sBucketSize * sizeof(float) + 31) / 32) * 32) / sizeof(float);
                DataCopyPad(vGm[singularValuesOffset * vNSize], srcRows,
                    { static_cast<uint16_t>(sBucketSize), static_cast<uint16_t>(vNSize * sizeof(AType)), static_cast<uint16_t>((recordSizeAligned - vNSizeAligned32) * sizeof(AType) / 32),0 });
            }
            event_t evtMte3ToMte2 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE3_MTE2));
            SetFlag<HardEvent::MTE3_MTE2>(evtMte3ToMte2);
            WaitFlag<HardEvent::MTE3_MTE2>(evtMte3ToMte2);
            singularValuesOffset += currentBucketSize;
        }
