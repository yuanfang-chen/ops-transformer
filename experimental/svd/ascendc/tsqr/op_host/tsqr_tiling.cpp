...
int64_t allocWorkspace(TsqrTilingData& tilingData, int M, int N, int blockSize, int numLevels, int batchSize, int coreNum, int batchFactor) {
    ...
    int64_t bufferQSize = M * N / 2;
    int64_t maxQrWorkspace = N * blockSize;

    int64_t bufferSize;
    int64_t forwardBufferSize = tmpRSize + maxQrWorkspace * coreNum * 2;
    if (tmpRSize >= bufferQSize || (tmpRSize < bufferQSize && forwardBufferSize >= bufferQSize)) {
        bufferSize = tmpRSize;
    } else {
        bufferSize = bufferQSize;
    }

    int64_t totalWorkspaceSize = (tmpQSize + bufferSize) * batchFactor + maxQrWorkspace * coreNum * 2;
    ...
    tilingData.set_bufferSize(bufferSize);
