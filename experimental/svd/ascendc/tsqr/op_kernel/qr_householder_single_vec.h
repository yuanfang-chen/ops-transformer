...
// static constexpr int32_t FLOAT_MASK = VECTOR_REG_WIDTH / sizeof(float);
...
inline __aicore__ void QRHouseholderSingleVec::InitAndUpdateR() {
    ...
    DataCopyPadParams dataCopyPadParams = {isPad, 0, rightPad, 0};
    ...
    int32_t numIters = (mDim_ - 1) / ubNumRows;
    int32_t tailRows = (mDim_ - 1) % ubNumRows;
    ...
    if (tailRows > 0) {
        int32_t gmOffset = processedRowIdx * kDim_;
        dataCopyParams = {static_cast<uint16_t>(tailRows), static_cast<uint16_t>(kDim_ * sizeof(float)), 0, 0};
        CopyInRow(inputXGm_, gmOffset, dataCopyParams, dataCopyPadParams);
        LocalTensor<float> rows = inQueX_.DeQue<float>();
...
inline __aicore__ void QRHouseholderSingleVec::UpdateR(int32_t rowLen, int32_t startIdx) {
    ...
    int32_t ubNumRows = maxNumElements_ / alignedRowLen;
    int32_t numIters = (mDim_ - (startIdx + 1)) / ubNumRows;
    int32_t tailRows = (mDim_ - (startIdx + 1)) % ubNumRows;
...
inline __aicore__ void QRHouseholderSingleVec::ComputeQ() {
    ...
    for (int32_t i = 0; i < kDim_; i++) {
        ...
        if (ubNumRows <= i) {
            int32_t numRows = i + 1;
            int32_t numIters = numRows / ubNumRows;
            int32_t tailIterLen = numRows % ubNumRows;
            int32_t betaIdx = i;
            for (int32_t j = 0; j < numIters; j++) {
            ...
            if (tailIterLen > 0) {
                DataCopyParams dataCopyTailParams = {static_cast<uint16_t>(tailIterLen), static_cast<uint16_t>(mDim_ * sizeof(float)), 0, 0};
