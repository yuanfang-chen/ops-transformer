...
printSingleCoreParams("increFlashAttentionSingleCoreParams.", tilingData.tilingBase.increFlashAttentionSingleCoreParams);
printTensorSize("increFlashAttentionSingleCoreTensorSize.", tilingData.tilingBase.increFlashAttentionSingleCoreTensorSize);
...
void ConvertContextToParamsIFA(
    ...
    int64_t pre_tokens, int64_t next_tokens, const char* input_layout,
    ...
    const c10::optional<int64_t>& query_dtype, const c10::optional<int64_t>& key_dtype, const c10::optional<int64_t>& value_dtype,
    ...
    c10::optional<int64_t> dequant_scale_value_dtype, c10::optional<int64_t> dequant_scale_key_rope_dtype)
{
    ...
    // workspace
    auto workspaceTensor = at::empty({ifaContext.workSpaceSize}, 
    ...
    at::Tensor actualSeqLengthsTensor;
    if (actual_seq_kvlen.has_value()) {
        auto temp = at::from_blob(
            const_cast<int64_t*>(actual_seq_kvlen->data()),
            {actual_seq_kvlen->size()},
            at::TensorOptions().dtype(at::kLong).device(at::kCPU)
        );
        actualSeqLengthsTensor = temp.to(at::kByte).to(query.options().device());
    }

    at::Tensor actualSeqLengthsQTensor;
    if (actual_seq_qlen.has_value()) {
        auto temp = at::from_blob(
            const_cast<int64_t*>(actual_seq_qlen->data()),
            {actual_seq_qlen->size()},
            at::TensorOptions().dtype(at::kLong).device(at::kCPU)
        );
        actualSeqLengthsQTensor = temp.to(at::kByte).to(query.options().device());
    }
    ...
    if (fdFlag == 0 && layoutVal == 0 && antiquantMode == 0) {
        ...
        auto aclCal = [=]() -> int {
            incre_flash_attention<0, 0, 0><<<blockDim, nullptr, aclstream>>>(
                (GM_ADDR)(query.storage().data()),
                (GM_ADDR)(key.storage().data()),
                (GM_ADDR)(value.storage().data()),
                (GM_ADDR)(pse_shift.storage().data()),
                (GM_ADDR)(atten_mask.storage().data()),
                (GM_ADDR)(actualSeqLengthsQTensor.storage().data()),
                (GM_ADDR)(actualSeqLengthsTensor.storage().data()),
                ...
                (GM_ADDR)(dequant_scale_key.has_value()? dequant_scale_key->storage().data() : nullptr), //keyAntiquantScale
                (GM_ADDR)(nullptr), //keyAntiquantOffset
                (GM_ADDR)(dequant_scale_value.has_value()? dequant_scale_value->storage().data() : nullptr), //valueAntiquantScale
                ...
                (GM_ADDR)(dequant_scale_key_rope.has_value()? dequant_scale_key_rope->storage().data() : nullptr), //keyRopeAntiquantScale
                (GM_ADDR)(dequant_scale_query.has_value()? dequant_scale_query->storage().data() : nullptr), //dequantScaleQuery
