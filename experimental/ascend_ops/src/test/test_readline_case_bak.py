...
class Config:
    ...
    specific_case: Optional[int] = None
...
def build_case_inputs(case: Dict[str, Any], config: Config) -> Dict[str, Any]:
    ...
    q = (torch.rand(convert_str2tuple(case['q_shape']), device="npu", dtype=torch.float32) * (data_range[1] - data_range[0]) + data_range[0]).to(torch.bfloat16)
    # q = torch.rand(convert_str2tuple(case['q_shape']), dtype=torch.bfloat16).npu() * (b - a) + a
...
def compare_outputs(
    ...
    # if True:
    #     output_dir = config.output_dir or "./output_arrays"
    #     import os
    #     os.makedirs(output_dir, exist_ok=True)

    #     path1 = os.path.join(output_dir, "arr1.txt")
    #     path2 = os.path.join(output_dir, "arr2.txt")

    #     np.savetxt(path1, arr1.flat, fmt="%.6f", delimiter="\n")
    #     np.savetxt(path2, arr2.flat, fmt="%.6f", delimiter="\n")

    #     log(f"\n✅ arr1 已保存至: {path1}", config)
    #     log(f"✅ arr2 已保存至: {path2}", config)
    # 检查是否接近（忽略 NaN）
...
def run_single_case(case: Dict[str, Any], case_id: str, config: Config) -> CaseResult:
    ...
    try:
        ...
        # result1, lse1 = run_custom_op(infer_kwargs, config)
...
def main():
    ...
    # 🌟 固定随机种子（关键步骤）
    # ========================
    SEED = 42  # 可以自定义，如 123, 999 等
    torch.manual_seed(SEED)
    torch.npu.manual_seed(SEED)
    torch.npu.manual_seed_all(SEED)  # 若多卡，也建议加
    np.random.seed(SEED)  # 如果使用 numpy 随机数（如这里用到的）
    ...
    config = Config(
        ...
        specific_case = [50]
        # ,50,49,44,43,41,40
        # only_first_n_cases=1,    # 例如设成 1 就只跑第一条
    )
