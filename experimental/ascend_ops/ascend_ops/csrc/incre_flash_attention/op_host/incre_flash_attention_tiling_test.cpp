...
bool IFATiling::CalcUbBmm()
{
    ...
    if (ropeFlag_) {
        ...
        if (slidingFlag_) {
            ...
            printf("msdIterNum_: %d, headDimAlign_ : %d, cubeMSize : %d\n",msdIterNum_ ,headDimAlign_, cubeMSize);
...
custom::graphStatus IFATiling::DoSubOpTiling(IFAContext& ifaContext) {
    ...
    printf("ifaContext.antiquantOffset has value :%d\n", ifaContext.antiquantOffset.hasValue);
    printf("ifaContext.keyAntiquantScale has value :%d\n", ifaContext.keyAntiquantScale.hasValue);
    printf("ifaContext.valueSharedPrefix has value :%d\n", ifaContext.valueSharedPrefix.hasValue);
    printf("ifaContext.actualSharedPrefixLen empty :%d\n", ifaContext.actualSharedPrefixLen.data.empty());
    ...
    if (RunBigKernelTiling(ifaContext, ifaTilingData) == custom::graphStatus::GRAPH_SUCCESS) {
        ...
        case at::ScalarType::Char:
            ...
            break;
        default:
            OP_LOGE(ifaContext.opName, "not support inputKvType%d", inputKvType_);
            return custom::graphStatus::GRAPH_FAILED;
    }
        return custom::graphStatus::GRAPH_SUCCESS;
    }
