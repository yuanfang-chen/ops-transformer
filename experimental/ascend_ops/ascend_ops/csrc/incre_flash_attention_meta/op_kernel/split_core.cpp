...
uint32_t SplitCore::Compute(IncreFlashAttentionMetadataArgs *args)
{
    AcquireParam(args);
    ParamsInit();
    BalanceSchedule();
...
void SplitCore::AcquireParam(IncreFlashAttentionMetadataArgs *args)
{
    ...
    actSeqQLen_ = (int64_t *)args->actSeqQLen;
...
void SplitCore::ParamsInit()
{
    ...
    if (actSeqKvLenDim_ == 1) {
        if (maxActualseq_ < actSeqQLen_[0]) {
            maxActualseq_ = actSeqQLen_[0];
        }
    } else if (actSeqKvLenDim_ != 0) {
        for (int64_t i = 0; i < actSeqKvLenDim_; ++i) {
            if (maxActualseq_ < actSeqKvLen_[i]) {
                maxActualseq_ = actSeqKvLen_[i];
            }
...
bool SplitCore::BalanceSchedule() 
{
    ...
    if (gqaMtpFlag_) {
        if (layoutQuery_ == "BSH_BSND" || layoutQuery_ == "TND") {
...
bool SplitCore::GenMetaData(IncreFlashAttentionMetadataArgs *args)
{
    constexpr uint32_t BYTE_BLOCK = 32UL;
    IncreFlashAttentionMetadata *metaData = (IncreFlashAttentionMetadata  *)args->metaData;
    ...
    for (size_t i = 0; i < MAX_CORE_NUM; ++i) {
