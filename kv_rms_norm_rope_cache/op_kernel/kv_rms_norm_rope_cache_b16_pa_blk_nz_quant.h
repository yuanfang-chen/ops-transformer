...
public:
    ...
    __aicore__ inline void Init(
        ...
        GM_ADDR optional_k_rope, GM_ADDR optional_c_kv, GM_ADDR kpe_scale, GM_ADDR ckv_scale)
    {
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int64_t loopIdx = 0; loopIdx < ubLoop; ++loopIdx) {
            ...
            RoPEWithQuant<KV_DTYPE, true, ROPE_LENGTH>(
                outLocal, quantOutLocal, ropeLocal, cosLocal, sinLocal, workspaceBuffer, localKpeScale, ubFactor);
            ...
            if (tilingData_->isKQuant) {
                ScatterUpdateNZWithQuant<
                    int8_t, KV_DTYPE, isPagedAttention, ROPE_LENGTH, D1_SIZE2_PA_BLK_NZ_QUANT,
                    D0_SIZE32_PA_BLK_NZ_QUANT>(
                    (GlobalTensor<int8_t>&)kCacheGm, kCacheGmNd, outLocal, quantOutLocal, startIdx, ubFactor);
            } else {
                LocalTensor<K_DTYPE> outputLocal = outLocal.template ReinterpretCast<K_DTYPE>();
                ScatterUpdateNZWithQuant<
                    K_DTYPE, KV_DTYPE, isPagedAttention, ROPE_LENGTH, D1_SIZE4_PA_BLK_NZ, D0_SIZE16_PA_BLK_NZ>(
            ...
            RmsNormWithQuant<KV_DTYPE, RMS_NORM_LENGTH>(
                outLocal, quantOutLocal, xLocal, gammaLocalFp32, workspaceBuffer, localCkvScale, ubFactor);
            ...
            if (tilingData_->isVQuant) {
                ScatterUpdateNZWithQuant<
                    int8_t, KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH, D1_SIZE16_PA_BLK_NZ_QUANT,
                    D0_SIZE32_PA_BLK_NZ_QUANT>(
                    (GlobalTensor<int8_t>&)vCacheGm, vCacheGmNd, outLocal, quantOutLocal, startIdx, ubFactor);
            } else {
                LocalTensor<V_DTYPE> outputLocal = outLocal.template ReinterpretCast<V_DTYPE>();
                ScatterUpdateNZWithQuant<
                    V_DTYPE, KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH, D1_SIZE32_PA_BLK_NZ, D0_SIZE16_PA_BLK_NZ>(
        ...
        if (ubTail > 0) {
            ...
            RoPEWithQuant<KV_DTYPE, true, ROPE_LENGTH>(
                outLocal, quantOutLocal, ropeLocal, cosLocal, sinLocal, workspaceBuffer, localKpeScale, ubTail);
            inQueueX.FreeTensor(ropeLocal);
            outQueue.EnQue(outLocal);
            outLocal = outQueue.DeQue<KV_DTYPE>();
            // Scatter Update kCache
            if (tilingData_->isKQuant) {
                ScatterUpdateNZWithQuant<
                    int8_t, KV_DTYPE, isPagedAttention, ROPE_LENGTH, D1_SIZE2_PA_BLK_NZ_QUANT,
                    D0_SIZE32_PA_BLK_NZ_QUANT>(
                    (GlobalTensor<int8_t>&)kCacheGm, kCacheGmNd, outLocal, quantOutLocal, startIdx, ubTail);
            } else {
                LocalTensor<K_DTYPE> outputLocal = outLocal.template ReinterpretCast<K_DTYPE>();
                ScatterUpdateNZWithQuant<
                    K_DTYPE, KV_DTYPE, isPagedAttention, ROPE_LENGTH, D1_SIZE4_PA_BLK_NZ, D0_SIZE16_PA_BLK_NZ>(
            ...
            RmsNormWithQuant<KV_DTYPE, RMS_NORM_LENGTH>(
                outLocal, quantOutLocal, xLocal, gammaLocalFp32, workspaceBuffer, localCkvScale, ubTail);
            inQueueX.FreeTensor(xLocal);
            outQueue.EnQue(outLocal);
            outLocal = outQueue.DeQue<KV_DTYPE>();
            ...
            if (tilingData_->isVQuant) {
                ScatterUpdateNZWithQuant<
                    int8_t, KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH, D1_SIZE16_PA_BLK_NZ_QUANT,
                    D0_SIZE32_PA_BLK_NZ_QUANT>(
                    (GlobalTensor<int8_t>&)vCacheGm, vCacheGmNd, outLocal, quantOutLocal, startIdx, ubTail);
            } else {
                LocalTensor<V_DTYPE> outputLocal = outLocal.template ReinterpretCast<V_DTYPE>();
                ScatterUpdateNZWithQuant<
                    V_DTYPE, KV_DTYPE, isPagedAttention, RMS_NORM_LENGTH, D1_SIZE32_PA_BLK_NZ, D0_SIZE16_PA_BLK_NZ>(
    ...
    __aicore__ inline void RoPEWithQuant(
        ...
        const LocalTensor<float>& scaleLocal, int64_t rows)
    {
