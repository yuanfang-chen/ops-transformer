...
public:
                                ...
                                const L1Params &l1Params, bool isBias, bool dbL0C)
    {
        ...
        scaleBL1OneBuffer_ = baseN_ * Act::Gemm::CeilDiv(scaleKL1_, MXFP_DIVISOR_SIZE) * MXFP_MULTI_BASE_SIZE;
        if (isBias_) {
            biasL1OneBuffer_ = baseN_ * sizeof(BiasType);
        }
        if constexpr (DispatchPolicy::fullLoadMode == 0) {
            aL1OneBuffer_ = baseM_ * Act::Gemm::Align(kL1_, MXFP_DIVISOR_SIZE);
            scaleAL1OneBuffer_ = baseM_ * Act::Gemm::CeilDiv(scaleKL1_, MXFP_DIVISOR_SIZE) * MXFP_MULTI_BASE_SIZE;
            ...
            uint64_t mAlign = Act::Gemm::Align(baseM_, transA ? C0_SIZE : BLOCK_CUBE);
    ...
    __aicore__ inline void InitA1(const AscendC::LocalTensor<AType> &al1Local, TileL1L0Param &tileL1L0Param)
    {
        ...
        if constexpr (!transA) {
            ...
            uint64_t kAlign = Act::Gemm::CeilDiv(tileL1L0Param.curGmAKL1, C0_SIZE) * AscendC::BLOCK_CUBE;
            offset = tileL1L0Param.curAlignM * kAlign;
        } else {
            if (tileL1L0Param.curPadAKL1 == tileL1L0Param.curGmAKL1) {
                return;
            }
            uint64_t m1 = Act::Gemm::CeilDiv(tileL1L0Param.curAlignM, C0_SIZE);
    ...
    __aicore__ inline void InitB1(const AscendC::LocalTensor<BType> &bl1Local, TileL1L0Param &tileL1L0Param)
    {
        ...
        if constexpr (transB) {
            ...
            initConstValueParams.dstGap = 0;
            initConstValueParams.initValue = 0;
            uint64_t kAlign = Act::Gemm::CeilDiv(tileL1L0Param.curGmBKL1, C0_SIZE) * AscendC::BLOCK_CUBE;
            offset = tileL1L0Param.curAlignN * kAlign;
        } else {
            if (tileL1L0Param.curPadBKL1 == tileL1L0Param.curGmBKL1) {
                return;
            }
            uint64_t n1 = Act::Gemm::CeilDiv(tileL1L0Param.curAlignN, C0_SIZE);
                                        ...
                                        uint64_t kL1Offset)
    {
        ...
        uint64_t nDim = transA ? Act::Gemm::CeilDiv(curScaleKL1, MXFP_DIVISOR_SIZE) : curML1;
        uint64_t dDim = transA ? curML1 : Act::Gemm::CeilDiv(curScaleKL1, MXFP_DIVISOR_SIZE);
        ...
        if constexpr (!transA) {
            ...
            dn2nzParams.srcDValue = Act::Gemm::CeilDiv(k_, MXFP_DIVISOR_SIZE);
            dn2nzParams.dstNzC0Stride = Act::Gemm::CeilDiv(curKL1, MXFP_DIVISOR_SIZE);
            ...
            nd2nzParams.dstNzC0Stride = Act::Gemm::CeilDiv(curKL1, MXFP_DIVISOR_SIZE);
            nd2nzParams.dstNzNStride = 1;
                                        ...
                                        uint64_t kL1Offset)
    {
        ...
        uint64_t nDim = transB ? curNL1 : Act::Gemm::CeilDiv(curScaleKL1, MXFP_DIVISOR_SIZE);
        uint64_t dDim = transB ? Act::Gemm::CeilDiv(curScaleKL1, MXFP_DIVISOR_SIZE) : curNL1;
        ...
        if constexpr (transB) {
            ...
            dn2nzParams.srcDnMatrixStride = 0;
            dn2nzParams.srcDValue = Act::Gemm::CeilDiv(k_, MXFP_DIVISOR_SIZE);
            dn2nzParams.dstNzC0Stride = Act::Gemm::CeilDiv(scaleKL1_, MXFP_DIVISOR_SIZE);
            dn2nzParams.dstNzNStride = 1;
            dn2nzParams.dstNzMatrixStride = 0;
            ...
            nd2nzParams.dstNzC0Stride = Act::Gemm::CeilDiv(scaleKL1_, MXFP_DIVISOR_SIZE);
            nd2nzParams.dstNzNStride = 1;
            nd2nzParams.dstNzMatrixStride = 0;
        ...
        uint64_t curScaleKL1)
    {
        ...
        uint64_t m1 = Act::Gemm::CeilDiv(tileL1L0Param.curM, AscendC::BLOCK_CUBE);
        if constexpr (!transA) {
            loadDataParams.mStartPosition = 0;
            loadDataParams.kStartPosition = Act::Gemm::CeilDiv(iter * baseK_, C0_SIZE);
            loadDataParams.mStep = m1;
            loadDataParams.kStep = Act::Gemm::CeilDiv(tileL1L0Param.curKL0, C0_SIZE);
            ...
            if ((m1 & 1) == 0) {
                loadDataParams.mStep = Act::Gemm::CeilAlign(
                    Act::Gemm::CeilDiv(tileL1L0Param.curKL0, AscendC::BLOCK_CUBE), MXFP_MULTI_BASE_SIZE);
            } else {
                loadDataParams.mStep = B8_MIN_STEP;
            }
            loadDataParams.kStep = Act::Gemm::CeilDiv(tileL1L0Param.curM, C0_SIZE);
            loadDataParams.srcStride = Act::Gemm::CeilDiv(tileL1L0Param.curPadAKL1, AscendC::BLOCK_CUBE);
        ...
        loadData2DMxParams.yStep = Act::Gemm::CeilDiv(tileL1L0Param.curKL0, MXFP_DIVISOR_SIZE);
        loadData2DMxParams.srcStride = Act::Gemm::CeilDiv(curScaleKL1, MXFP_DIVISOR_SIZE);
        ...
        if constexpr (transA) {
            if ((m1 & 1) != 0) {
                ...
                uint64_t loadTimes =
                    Act::Gemm::CeilDiv(Act::Gemm::CeilDiv(tileL1L0Param.curKL0, BLOCK_CUBE), B8_MIN_STEP);
                for (uint64_t i = 1; i < loadTimes; i++) {
                    loadDataParams.mStartPosition = B8_MIN_STEP * i + Act::Gemm::CeilDiv(iter * baseK_, BLOCK_CUBE);
                                     ...
                                     TileL1L0Param &tileL1L0Param)
    {
        AscendC::LoadData2DParamsV2 loadDataParams;
        AscendC::LoadData2DMxParams loadData2DMxParams;
        uint64_t n1 = Act::Gemm::CeilDiv(tileL1L0Param.curN, AscendC::BLOCK_CUBE);
        if constexpr (transB) {
            loadDataParams.mStartPosition = 0;
            loadDataParams.kStartPosition = Act::Gemm::CeilDiv(iter * baseK_, C0_SIZE);
            ...
            if ((n1 & 1) == 0) {
                loadDataParams.mStep =
                    Act::Gemm::CeilAlign(Act::Gemm::CeilDiv(tileL1L0Param.curKL0, AscendC::BLOCK_CUBE), EVEN_FACTOR);
            } else {
                loadDataParams.mStep = B8_MIN_STEP;
            }
            loadDataParams.kStep = Act::Gemm::CeilDiv(tileL1L0Param.curN, C0_SIZE);
            loadDataParams.srcStride = Act::Gemm::CeilDiv(tileL1L0Param.curPadBKL1, AscendC::BLOCK_CUBE);
    ...
    __aicore__ inline void UpdateKL1(TileL1L0Param &tileL1L0Param, uint64_t iter0)
    {
        ...
        tileL1L0Param.curPadBKL1 = Act::Gemm::CeilAlign(tileL1L0Param.curGmBKL1, MXFP_DIVISOR_SIZE);
    ...
    {
        if constexpr (transA) {
            tileL1L0Param.curAlignM = Act::Gemm::CeilAlign(tileL1L0Param.curM, C0_SIZE);
        } else {
            tileL1L0Param.curAlignM = Act::Gemm::CeilAlign(tileL1L0Param.curM, BLOCK_CUBE);
        }
        if constexpr (!transB) {
            tileL1L0Param.curAlignN = Act::Gemm::CeilAlign(tileL1L0Param.curN, C0_SIZE);
        } else {
            tileL1L0Param.curAlignN = Act::Gemm::CeilAlign(tileL1L0Param.curN, BLOCK_CUBE);
        }
    }
    ...
    {
        uint64_t kL0Iter = Act::Gemm::CeilDiv(tileL1L0Param.curGmBKL1, baseK_);
        for (uint16_t iter1 = 0; iter1 < kL0Iter; ++iter1) {
            ...
            CopyInC2(biasL1Local_[l1BufferBiasOffset_[biasBufId_] / sizeof(BiasType)], biasBt_[baseN_ * biasBufId_],
                     Act::Gemm::Align(mmadParams.n, AscendC::BLOCK_CUBE), NeedBias(l1Iter, iter1));
            ...
            mmadParams.k = Act::Gemm::CeilAlign(tileL1L0Param.curKL0, MXFP_DIVISOR_SIZE);
            mmadParams.unitFlag =
