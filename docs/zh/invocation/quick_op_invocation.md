...
2. **下载第三方软件依赖**
    ...
    - 方式2：通过[3rd_lib_download.py](../../../scripts/third_party/3rd_lib_download.py)脚本一键下载，该脚本在本项目`scripts/third_party/`目录，下载该脚本并执行如下命令：
    
        ```bash
        python ${scripts_dir}/3rd_lib_download.py
        ```
    \$\{scripts\_dir\}表示脚本存放路径，下载的第三方软件包默认存放在当前脚本所在目录。

3. **编译算子包**

   将下载好的第三方软件上传至离线环境，可存放在指定目录或自定义目录下。**推荐前者，其编译命令与联网编译场景下的命令一致。**

    > **产物说明**：未联网编译和联网编译的产物目录一样，默认存放于build_out目录下。
