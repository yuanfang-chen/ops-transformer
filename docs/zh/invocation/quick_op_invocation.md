...
基于社区版CANN包对算子源码修改时，可采用如下方式进行源码编译。命令一般通过bash.sh实现，
...
1. **检查基础环境是否完备**
    ...
    请确保已按[环境部署](quick_install.md)完成基础环境搭建。若您的编译环境无法访问网络，请先在联网环境中下载源码然后上传至目标环境。
...
2. **下载第三方软件依赖**
    ...
    本项目编译过程依赖的第三方开源软件列表如下，可通过项目根目录cann_3rd_lib_path_download.py安装，命令如下。若从其他地址下载，请确保版本号一致。
...
3. **存放第三方开源软件**
    ...
    - 若通过cann_3rd_lib_path_download.py脚本安装，默认在根目录下生成`cann_3rd_lib_path_download`文件夹，用于存放下载的开源软件包，此时`${cann_3rd_lib_path}`为`./cann_3rd_lib_path_download`。
