...
function(filter_op_files op_name src_list aclnntype filtered_list)
    if (${aclnntype} STREQUAL "aclnn")
        foreach(file_path IN LISTS ${src_list})
            string(REGX MATCH "aclnn_${op_name}.*\\.cpp$" match_result1 "${file_path}")
            string(REGX MATCH "aclnn_${op_name}_v[0-9]+.*\\.cpp$" match_result2 "${file_path}")
        ...
        foreach(file_path IN LISTS ${src_list})
            string(REGX MATCH "aclnnInner_${op_name}.*\\.cpp$" match_result1 "${file_path}")
            string(REGX MATCH "aclnnInner_${op_name}_v[0-9]+.*\\.cpp$" match_result2 "${file_path}")
            if (match_result1 OR match_result2)
                list(APPEND filtered_list "${file_path}")
            endif()
        endforeach()
...
if (base_aclnn_srcs)
    ...
    foreach (_src ${base_aclnn_srcs})
        ...
        if (is_match)
            ...
            set(filtered_list)
            filter_op_files(${_op_name} ACLNN_EXTRA_SRC "aclnn" filtered_list)
            if (filtered_list)
