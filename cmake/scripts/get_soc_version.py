...
class Parser:
    ...
    def main() -> str:
        ...
        args = ps.parse_args()
        logging.debug(args)
        if not Parser.parse_classify_file_soc(file=Path(args.classify[0])):
            return ""
        if not Parser.parse_changed_file_soc(file=Path(args.file[0])):
            return ""
    ...
    @classmethod
    def parse_classify_file_soc(cls, file: Path) -> bool:
    ...
    @classmethod
    def parse_changed_file_soc(cls, file: Path) -> bool:
        file = Path(file).resolve()
        if not file.exists():
