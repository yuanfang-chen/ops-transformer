...
class ParserSoc(Parser):
    ...
    _Modules: List[GetSocModules] = []         # 保存规则文件(tests/test_config.yaml)内设置的模块列表
    ...
    @classmethod
    def _parse_classify_item_soc(cls, name: str, desc: Optional[Dict[str, Any]] = None) -> bool:
        if desc is None:
            logging.error("[%s]'s desc is None.", name)
            return False
        if desc.get('module', False):
            mod_soc = GetSocModules(name=name)
            rst_soc = mod_soc.update_classify_cfg(desc=desc)
            if rst_soc:
                cls._Modules.append(mod_soc)
                short_name = name.split('/')[-1]
                if 'examples' in mod_soc.test_excludes:
                    cls._ExamplesExcludes.append(short_name)
                if 'ut' in mod_soc.test_excludes:
                    cls._UTExcludes.append(short_name)
            return rst_soc
        for k, sub_desc in desc.items():
            if not cls._parse_classify_item_soc(name=name + '/' + k, desc=sub_desc):
                return False
        return True
