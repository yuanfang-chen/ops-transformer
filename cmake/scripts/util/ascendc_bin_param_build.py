...
grep -q \"None of the given tiling keys are in the supported list\"; then\n"
    ...
    def _write_build_cmd(self: any, param_file: str, bin_file: str, index: int, auto_gen_path: str, bisheng_flags: str,
        ...
        if const_var.CHECK_ASC_DEVKIT_VERSION:
            bin_cmd_str = '''res=$(asc_opc $1 --main_func={fun} --input_param={param} --soc_version={soc} --output=$2 --impl_mode={impl} --simplified_key_mode=0 --op_mode=dynamic '''
        else:
            bin_cmd_str = '''res=$(opc $1 --main_func={fun} --input_param={param} --soc_version={soc} --output=$2 --impl_mode={impl} --simplified_key_mode=0 --op_mode=dynamic'''
