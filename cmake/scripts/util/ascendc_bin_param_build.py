...
grep -q \"None of the given tiling keys are in the supported list\"; then\n"
    ...
    def _write_build_cmd(self: any, param_file: str, bin_file: str, index: int, auto_gen_path: str, super_mode=False):
        ...
        build_cmd_var += f'kernel_size=$(stat -c%s "{self.op_file}.o")\n'
        build_cmd_var += f'''echo "Build [{self.soc}] op_file [{self.op_file}] index [{str(index)}] duration [$duration]
                           start_time [$start_time] end_time [$end_time] kernel_size [$kernel_size]" >> {self.op_file}-{str(index)}.txt\n'''
