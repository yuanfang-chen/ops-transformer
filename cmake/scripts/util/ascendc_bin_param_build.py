...
grep -q \"None of the given tiling keys are in the supported list\"; then\n"
    ...
    def _write_build_cmd(self: any, param_file: str, bin_file: str, index: int, auto_gen_path: str, super_mode=False):
        ...
        build_cmd_var += f'kernel_path=$(find "." -type f -name "{self.op_file}/{bin_file}.o" -print | head -n 1)\n'
        build_cmd_var += f'kernel_size=$(stat -c%s "$kernel_path")\n'
        build_cmd_var += f'''echo "Build [{self.soc}] op_name [{self.op_file}] index [{str(index)}] bin_file [{bin_file}] duration [$duration] size [$kernel_size] start_time [$start_time] end_time [$end_time]" >> {self.op_file}-{str(index)}.txt\n'''
