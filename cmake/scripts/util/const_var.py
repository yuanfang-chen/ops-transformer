...
def get_version_from_file(filename):
    try:
        ...
        return None
ASC_DEVKIT_VERSION = get_version_from_file('version.info')
def check_version_requirement(current_ver=ASC_DEVKIT_VERSION, min_version="9.0.0"):
    if version.parse(current_ver) >= version.parse(min_version):
        return True
    else:
        print(f"version is not match: {current_ver} < {min_version}")
        return False
if check_version_requirement(ASC_DEVKIT_VERSION, "9.0.0"):
