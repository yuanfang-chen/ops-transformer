...
def check_asc_devkit_version(min_version="9.0.0"):
    global ASC_DEVKIT_VERSION, VERSION_LOADED
    if not VERSION_LOADED:
        ascend_cann_path = os.environ.get('ASCEND_CANN_PACKAGE_PATH')
        if not ascend_cann_path:
            print("error: env var ASCEND_CANN_PACKAGE_PATH not found")
            return None
        ...
        if not os.path.exists(version_file):
            ...
            return None
        ASC_DEVKIT_VERSION = get_version_from_file(version_file)
        VERSION_LOADED = True
    if version.parse(ASC_DEVKIT_VERSION) >= version.parse(min_version):
        return True
    else:
        return False
