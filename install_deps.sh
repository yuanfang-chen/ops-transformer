...
detect_os() {
    # OS detection, supports debian (uses apt), rhel (uses dnf or yum), euler(uses dnf), macos
...
install_gcc() {
    ...
    if command -v gcc &> /dev/null; then	 
        gcc_curr_ver=$(gcc --version | awk '/^gcc/ {print $4}')	  
    else
        gcc_curr_ver="0.0.0"
    fi

    if command -v g++ &> /dev/null; then	 
        gxx_curr_ver=$(g++ --version | awk '/^g\+\+/ {print $4}')	 
    else
        gxx_curr_ver="0.0.0"
    fi
    ...
    if version_ge "$gcc_curr_ver" "$req_ver"; then
        if version_ge "$gxx_curr_ver" "$req_ver"; then
            echo "GCC and G++ version meets requirements ($curr_ver)"
            return
        fi
    fi
    ...
    if command -v gcc &> /dev/null; then
        gcc_curr_ver=$(gcc --version | awk '/^gcc/ {print $4}')
        gxx_curr_ver=$(g++ --version | awk '/^gcc/ {print $4}')
