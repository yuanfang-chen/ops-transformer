...
ge::graphStatus FfnWorkerBatchingTiling::GetPlatformInfo()
{
    ...
    OP_TILING_CHECK(platformInfo == nullptr,
        VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(), "platformInfo is null"),
        return ge::GRAPH_FAILED);
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(platformInfo);
    uint32_t aivNum = ascendcPlatform.GetCoreNumAiv();
    OP_TILING_CHECK(aivNum == 0,
        VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(), "Get aivNum:%u failed.", aivNum),
        return ge::GRAPH_FAILED);

    uint64_t ubSize = 0;
    ascendcPlatform.GetCoreMemSize(platform_ascendc::CoreMemType::UB, ubSize);
    OP_TILING_CHECK(ubSize == 0,
        VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(), "Get ubSize:%lu failed.", ubSize),
        return ge::GRAPH_FAILED);

    ubBlockSize_ = platform::GetUbBlockSize(context_);
    OP_TILING_CHECK(ubBlockSize_ == 0,
        VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(), "Get ubBlockSize:%u failed.", ubBlockSize_),
        return ge::GRAPH_FAILED);
...
ge::graphStatus FfnWorkerBatchingTiling::CheckInputParam()
{
    ...
    OP_TILING_CHECK(xdtype != ge::DT_INT8,
        VECTOR_INNER_ERR_REPORT_TILIING(
            context_->GetNodeName(), "Input dtype:%s not int8", ops::ToString(xdtype).c_str()),
        return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(xShape.GetDimNum() != 1,
        VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(), "x shape %s dim num not 1",
                                        ops::ToString(xShape).c_str()),
        return ge::GRAPH_FAILED);

    // int64_t dim0 = xShape.GetDim(0);
    // OP_TILING_CHECK(dim0 != X_SHAPE_VALUE,
    //     VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(), "x shape %s dim0 not 1024",
    //                                     ops::ToString(xShape).c_str()),
    //     return ge::GRAPH_FAILED);
...
ge::graphStatus FfnWorkerBatchingTiling::GetAttrsInfo()
{
    ...
    OPS_CHECK_NULL_WITH_CONTEXT(context_, expertNumPtr);
    OP_TILING_CHECK(*expertNumPtr > EXPERT_IDX_MAX || *expertNumPtr <= 0,
        VECTOR_INNER_ERR_REPORT_TILIING(
    ...
    OPS_CHECK_NULL_WITH_CONTEXT(context_, maxOutShapePtr);
    OP_TILING_CHECK(maxOutShapePtr->GetSize() != NUM_FOUR,
        VECTOR_INNER_ERR_REPORT_TILIING(
    ...
    OP_TILING_CHECK((A_ > MAX_SESSION_NUM || A_ <= 0),
        VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(),
            "max_out_shape[0]:%ld should be in range of (0, %ld]", A_, MAX_SESSION_NUM),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(BS_ <= 0,
        VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(), "max_out_shape[1]:%ld should be greater than 0", BS_),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK((K_ > MAX_K_NUM || K_ <= 0),
        VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(),
            "max_out_shape[2]:%ld should be in range of (0, %ld]", K_, MAX_K_NUM),
        return ge::GRAPH_FAILED);
    OP_TILING_CHECK(maxOutShapeArray[INDEX_THREE] <= 0,
        VECTOR_INNER_ERR_REPORT_TILIING(context_->GetNodeName(),
    ...
    if (tokenDtype != nullptr) {
        OP_TILING_CHECK((*tokenDtype < 0 || *tokenDtype > NUM_TWO),
            VECTOR_INNER_ERR_REPORT_TILIING(
    ...
    if (needSchedulePtr != nullptr) {
        OP_TILING_CHECK((*needSchedulePtr < 0 || *needSchedulePtr > 1),
            VECTOR_INNER_ERR_REPORT_TILIING(
    ...
    if (layNumPtr != nullptr) {
        OP_TILING_CHECK((*layNumPtr < 0 || *layNumPtr > *expertNumPtr),
            VECTOR_INNER_ERR_REPORT_TILIING(
...
ge::graphStatus FfnWorkerBatchingTiling::RunFfnWorkerBatchingTiling()
{
    ...
    OPS_CHECK_NULL_WITH_CONTEXT(context_, batchingTilingD);
