    ...
    const gert::TilingContext* context, int64_t batchSize, int64_t numHead, int64_t seqLen, int64_t headSize)
{
    ...
    if (currentCacheMode_ == CacheMode::PA_BLK_NZ || currentCacheMode_ == CacheMode::PA_NZ) {
        ...
        if (!isValidAlign) {
            ...
            "NZ scenario, Dk must be 32B aligned. Alignment value: Quantized (16), Non-quantized(32).");
        }
    }
...
bool KvRmsNormRopeCacheTilingBase::CheckGammaValid(const gert::TilingContext* context, int64_t headSize)
{
    ...
    if (currentCacheMode_ == CacheMode::PA_BLK_NZ || currentCacheMode_ == CacheMode::PA_NZ) {
        ...
        if (!isValidAlign) {
            OP_LOGE(context_->GetNodeName(),
            "NZ scenario, Dv must be 32B aligned. Alignment value: Quantized (16), Non-quantized(32).");
        }
    }
    ...
    const gert::TilingContext* context, int64_t numHead, int64_t headSize)
{
    ...
    if (!isValidAlign) {
        OP_LOGE(context_->GetNodeName(),
            "PA scenario, k_cache blockSize must be 32B aligned. Alignment value: Quantized (16), Non-quantized(32).");
    }
    isValid = isValid && isValidAlign;
    return isValid;
}
    ...
    const gert::TilingContext* context, int64_t numHead, int64_t headSize)
{
    ...
    if (!isValidAlign) {
        OP_LOGE(context_->GetNodeName(),
            "PA scenario, v_cache blockSize must be 32B aligned. Alignment value: Quantized (16), Non-quantized(32).");
    }
    isValid = isValid && isValidAlign;
    return isValid;
}
