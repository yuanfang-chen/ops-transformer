    ...
    for (int i = KV_INDEX; i <= C_KV_OFFSET_IDX; ++i) {
        if (i >= K_ROPE_SCALE_IDX) {
            ...
            if (optionalParamDesc != nullptr) {
                auto optionalParam = context_->GetInputShape(i);
                gert::Shape optionalParamShape = optionalParam->GetStorageShape();
                OP_CHECK_IF(optionalParamShape.GetShapeSize() == 0,
                    OP_LOGE(context_->GetNodeName(), "The optional input k_rope_scale, c_kv_scale, k_rope_offset, c_kv_offset, can not be empty tensor."),
            ...
            OP_CHECK_IF(inputParamShape.GetShapeSize() == 0,
                OP_LOGE(context_->GetNodeName(), "The input kv, gamma, cos, sin, index, k_cache, ckv_cache can not be empty tensor."),
                    return ge::GRAPH_FAILED);
        }
    }
