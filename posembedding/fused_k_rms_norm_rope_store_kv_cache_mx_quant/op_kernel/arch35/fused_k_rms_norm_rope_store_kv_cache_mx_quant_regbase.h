...
public:
    ...
    */
                                        ...
                                        const int64_t copyLineLength)
    {
        ...
        copyOutParams.srcStride = (srcLineLength - copyLineLength) * sizeof(uint8_t);
        copyOutParams.dstStride = 0;
        DataCopyPad(dstGm, srcUb, copyOutParams);
    }

    __aicore__ inline void ScatterUpdateK(const LocalTensor<uint8_t> &outTensor,
                                          const LocalTensor<uint8_t> &outScaleTensor,
                                          const int64_t processSeqLen, const int64_t startTIdx)
    {
