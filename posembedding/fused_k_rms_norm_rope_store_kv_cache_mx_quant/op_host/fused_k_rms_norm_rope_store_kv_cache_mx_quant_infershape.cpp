...
graphStatus InferShape4FusedKRmsNormRopeStoreKvCacheMxQuant(gert::InferShapeContext* context)
{
    ...
    OP_CHECK_IF(vScaleSlotMappingInputShape->GetDimNum() != DIM_ONE,
            OP_LOGE(context, "v_scale_slot_mapping must be 1D tensor [T//32]."), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(kScaleCacheInputShape->GetDimNum() != DIM_THREE + 1,
            OP_LOGE(context, "k_scale_cache must be 4D tensor [Bn, Nk, Bs, D//32]."), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(kScaleCacheInputShape->GetDim(DIM_THREE) <= 0,
            OP_LOGE(context, "k_scale_cache D dimension must be positive."), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(vScaleCacheInputShape->GetDimNum() != DIM_THREE + 1,
            OP_LOGE(context, "v_scale_cache must be 4D tensor [Bn, Nv, Bs//32, D]."), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(vScaleCacheInputShape->GetDim(DIM_TWO) <= 0,
            OP_LOGE(context, "v_scale_cache Bs dimension must be positive."), return ge::GRAPH_FAILED);
