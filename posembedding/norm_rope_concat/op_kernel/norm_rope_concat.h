...
__simd_vf__ inline void NormOperationForward::DoSumVf(__ubuf__ float* outBuf, __ubuf__ float* inBuf, uint32_t heads) {
    ...
    for (uint32_t i = 0; i < heads; ++i) {
        ...
        StoreUnAlign<float, PostLiteral::POST_MODE_NORMAL>(outBuf + i, bufRegTensor, 1);
    }
}
...
*/
__simd_vf__ inline void NormOperationForward::ComputeRstdVF(__ubuf__ float* xBuf, __ubuf__ float* sumXBuf, __ubuf__ float* reducedXBuf, uint32_t heads)
{
