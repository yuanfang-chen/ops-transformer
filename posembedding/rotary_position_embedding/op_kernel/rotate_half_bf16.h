                                                           ...
                                                           uint32_t copyLength)
{
    ...
    if (this->isAligned == true) {
        if (this->layout == LAYOUT_BNSD || this->layout == LAYOUT_NO_BROADCAST || this->layout == LAYOUT_BND ||
            ...
            if (this->layout == LAYOUT_BSND) {
                copyParams.srcGap = (this->bcSecondDim - 1) * this->dBytes / BYTE_OF_BLOCK;   //layout = BSND
            }
            else {
                copyParams.srcStride = (this->bnSize - 1) * this->dBytes / BYTE_OF_BLOCK;   //layout = SBND
            }
...
__aicore__ inline void RotateHalfBf16<OriT, CmpT>::CopyOut(uint64_t yOffset, uint16_t storeSLines, uint32_t copyLength)
{
    ...
    if (this->isAligned == true) {
        ...
        if (this->layout == LAYOUT_BNSD || this->layout == LAYOUT_NO_BROADCAST || this->layout == LAYOUT_BND ||
            ...
            if (this->layout == LAYOUT_BSND) {
                copyParams.dstStride = (this->bcSecondDim - 1) * this->dBytes / BYTE_OF_BLOCK;   //layout = BSND
            }
            else {
                copyParams.dstStride = (this->bnSize - 1) * this->dBytes / BYTE_OF_BLOCK;   //layout = SBND
            }
