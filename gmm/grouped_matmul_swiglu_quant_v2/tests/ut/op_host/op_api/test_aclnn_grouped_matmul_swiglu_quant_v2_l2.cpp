...
void ExpandLeadingBatchForNzCase(vector<vector<int64_t>> &weightShapesVec,
                                 vector<vector<int64_t>> &weightScaleShapesVec)
{
    if (weightShapesVec.size() != 1 || weightScaleShapesVec.size() != 1) {
        return;
    }
    const auto &w = weightShapesVec[0];
    const auto &s = weightScaleShapesVec[0];
    if (w.size() != 5 || s.size() != 2) {
        return;
    }
    if (w[0] <= 1 || w[0] != s[0]) {
        return;
    }
    const int64_t groupCnt = w[0];
    vector<int64_t> perWeightShape = {w[1], w[2], w[3], w[4]};
    vector<int64_t> perScaleShape = {s[1]};

    weightShapesVec.clear();
    weightScaleShapesVec.clear();
    weightShapesVec.reserve(static_cast<size_t>(groupCnt));
    weightScaleShapesVec.reserve(static_cast<size_t>(groupCnt));
    for (int64_t i = 0; i < groupCnt; ++i) {
        weightShapesVec.emplace_back(perWeightShape);
        weightScaleShapesVec.emplace_back(perScaleShape);
    }
}
...
struct SwigluOpApiCase {
    ...
    void Run() const
    {
        ...
        const auto weightFormatVal = ParseFormat(weightFormat);
        auto weightScaleShapesVec = ParseDimsList(weightScaleShapes);
        if (api == "WeightNzV2" && weightFormatVal == ACL_FORMAT_FRACTAL_NZ) {
            ExpandLeadingBatchForNzCase(weightShapesVec, weightScaleShapesVec);
        }
