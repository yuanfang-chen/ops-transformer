...
{
    // Keep descriptor construction conservative for ND tensors in OP-API UT.
    // Passing custom storage/stride with ND can trigger low-level allocator crashes
    // in some environments; UT behavior does not depend on those internals.
...
struct SwigluOpApiCase {
    ...
    void Run() const
    {
        ...
        const auto weightScaleShapesVec = ParseDimsList(weightScaleShapes);
        ...
        if (!hasWeightScale) {
            ...
            auto weightScale = MakeTensorListDesc(weightScaleShapesVec, ParseDtype(weightScaleDtype), ParseFormat(weightScaleFormat),
