...
bool GroupedMatmulSwigluQuantDavidV2Tiling::SetQuantModeForGMMSwigluQuant(const gert::Shape &wScaleShape, const gert::Shape &xScaleShape)
{
    ...
    if (wScaleDims == PRECHANNEL_WEIGHT_SCALE_DIM && static_cast<uint64_t>(wScaleShape.GetDim(wScaleDims - 1)) == inputParams_.nSize &&
    ...
    if (inputParams_.bQuantMode == optiling::QuantMode::PERCHANNEL_MODE && inputParams_.aQuantMode == optiling::QuantMode::PERTOKEN_MODE) {
        return true;
    }
    return false;
}
...
bool GroupedMatmulSwigluQuantDavidV2Tiling::CheckDtypePertoken() {
    ...
    OP_CHECK_IF(!IsB8(inputParams_.outDataDtype),
                OP_LOGE(inputParams_.opName, "Only FLOAT8 or INT8 or HIFLOAT8 output are supported, but out dtype is %s.",
                        ge::TypeUtils::DataTypeToSerialString(inputParams_.outDataDtype).c_str()),
                return false);
...
void GroupedMatmulSwigluQuantDavidV2Tiling::PrintPertokenQuantParams() {
    ...
    oss << "GMMQuantParams: groupNum = " << params.get_groupNum()
        ...
        << ", rowLen = " << params.get_rowLen()
        << ", ubAvail = " << params.get_ubAvail();
    OP_LOGD(inputParams_.opName, "%s", oss.str().c_str());
}
