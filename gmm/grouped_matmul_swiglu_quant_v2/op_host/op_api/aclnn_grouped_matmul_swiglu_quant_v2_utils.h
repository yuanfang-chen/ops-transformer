...
and greater or equal to 4, but actual value is %lu.",
    ...
    bool CheckEmptyTensor() override
    {
        if (gmmDsqParams_.x->GetViewShape().GetDim(1) <= 0) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "The K value in x should be positive, but actual value is %ld",
        ...
        if (((*gmmDsqParams_.weight)[0])->GetViewShape().GetDim(weightKIndex) <= 0) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "The K value in weight should be positive, but actual value is %ld",
