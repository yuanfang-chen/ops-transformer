                                      ...
                                      int32_t k, int32_t n, int32_t baseM, int32_t baseN, int32_t baseK)
{
    ...
    // tiling.mmTilingData.set_M(m);
    // tiling.mmTilingData.set_N(n);
    ...
    gert::TilingContext* context, GroupedMatmulAddTilingData& tiling, int32_t m, int32_t k, int32_t n)
{
    ...
    if (xType == ge::DataType::DT_FLOAT16) {
        ...
        context->SetTilingKey(TILING_KEY_DEFAULT_BF16)4
    } else {
...
static ge::graphStatus TilingCheck4GroupedMatmulAdd(const gert::TilingContext* context)
{
    ...
    auto yShape = context->GetOutputShape(INDEX_OUT_Y)->GetOriginShape();
    ...
    OP_CHECK_IF(
        DIMNUM_3D != yDimNum,
        OP_LOGE(context->GetNodeName(), "groupListShape error yShape.GetDimNum():%lu", yDimNum),
        return ge::GRAPH_FAILED);
    // 检查shape是否可以对应
