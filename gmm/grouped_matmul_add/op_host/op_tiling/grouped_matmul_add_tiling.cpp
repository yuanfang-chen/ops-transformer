...
static ge::graphStatus TilingCheck4GroupedMatmulAdd(const gert::TilingContext* context)
{
    ...
    size_t yRefDimNum = context->GetInputShape(INDEX_IN_YREF)->GetOriginShape().GetDimNum();
    size_t yDimNum = context->GetOutputShape(INDEX_OUT_Y)->GetOriginShape().GetDimNum();
    // 检查输入dim是否符合预期
    OP_CHECK_IF(
        DIMNUM_2D != xDimNum,
    ...
    if (DIMNUM_3D != yDimNum) {
        OP_LOGW(context->GetNodeName(), "yShape error yShape.GetDimNum():%lu", yDimNum);
    }
    // 检查shape是否可以对应
...
static ge::graphStatus Tiling4GroupedMatmulAdd(gert::TilingContext* context)
{
    ...
    tiling.gmmBaseParams.set_coreNum(aicNum * DOUBLE_BUFFER_L0A_L0B);
