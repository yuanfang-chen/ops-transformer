...
constexpr static uint64_t B4_MIN_STEP = 1UL;
...
public:
                                ...
                                bool isBias, bool dbL0C)
    {
        ...
        for (int32_t bufferId = 0; bufferId < l1BufNum_; bufferId++) {
            ...
            uint64_t l1Offset = IS_FP4 ? AscendC::TOTAL_L1_SIZE * (bufferId & 1)
                                     ...
                                     const TileL1L0Param &tileL1L0Param, uint64_t kL0L1Off)
    {
        ...
        if constexpr (!transA) {
            ...
            if (((m1 & 1) == 0 && !(IS_FP4)) ||
                (IS_FP4 && Cgmct::Gemm::CeilDiv(tileL1L0Param.curM, AscendC::BLOCK_CUBE) % B4_MIN_STEP == 0)) {
                                     ...
                                     const TileL1L0Param &tileL1L0Param, uint64_t kL0L1Off)
    {
        ...
        if constexpr (transB) {
            ...
            loadDataParams.kStartPosition = 0;
            if (((n1 & 1) == 0 && !(IS_FP4)) ||
                (IS_FP4 && Cgmct::Gemm::CeilDiv(tileL1L0Param.curN, AscendC::BLOCK_CUBE) % B4_MIN_STEP == 0)) {
                loadDataParams.mStep = Cgmct::Gemm::CeilAlign(
