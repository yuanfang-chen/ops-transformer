                                                                         ...
                                                                         ProblemShape& problemShape)
{
    ...
    if ASCEND_IS_AIV {
        ...
        dequantOutInUBPing_ = AscendC::LocalTensor<float>(AscendC::TPosition::VECIN, offset, CeilDiv(mForSingleVec, FP32_OUTPUT_TIMES) * dequantTiling_->baseN);
        offset += CeilDiv(mForSingleVec, FP32_OUTPUT_TIMES) * dequantTiling_->baseN * sizeof(DataTypeOut);
        dequantOutInUBPong_ = AscendC::LocalTensor<float>(AscendC::TPosition::VECIN, offset, CeilDiv(mForSingleVec, FP32_OUTPUT_TIMES) * dequantTiling_->baseN);
    }
