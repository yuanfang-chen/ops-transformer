...
public:
    ...
    using formatB = BlockMmadBuilder::formatB;
    ...
    __aicore__ inline void UpdateGlobalBuffer(const Params& params)
    {
        ...
        if ASCEND_IS_AIV {
            AscendC::Coord<int64_t, int64_t, int64_t, int64_t, int64_t, int64_t> vecBaseOffset{
                0L, 0L, Get<INDEX_X2SCALE_OFFSET>(baseOffset_),
                Get<INDEX_X1SCALE_OFFSET>(baseOffset_), Get<IDX_LOGIT_OFFSETS>(baseOffset_), Get<IDX_LOGIT_OFFSETS>(baseOffset_)};
    ...
    __aicore__ inline void UpdateOffset(uint32_t groupIdx)
    {
        ...
        if constexpr (formatB == CubeFormat::NZ) {
            Get<INDEX_B_OFFSET>(baseOffset_) = Get<INDEX_B_OFFSET>(baseOffset_) + CeilDiv(n, WEIGHTNZ_N0_32) *
 	                                           CeilDiv(k, WEIGHTNZ_K0_16) * WEIGHTNZ_N0_K0;
        } else if constexpr (formatB == CubeFormat::Zn) {
            Get<INDEX_B_OFFSET>(baseOffset_) = Get<INDEX_B_OFFSET>(baseOffset_) + CeilDiv(k, WEIGHTNZ_K0_32) *
        ...
        // yBaseOffset += (m * n) >> 1
        Get<INDEX_C_OFFSET>(baseOffset_) += m * n;
    }
        ...
        while (bs.GetTileIdx(tileIdx)) {
            ...
            if ASCEND_IS_AIV {
                ...
                AscendC::Std::tuple<int64_t, int64_t, int64_t, int64_t, int64_t, int64_t> epilogueOffset{
                    nOffset, 0, Get<INDEX_X2SCALE_OFFSET>(blockOffset_), Get<INDEX_X1SCALE_OFFSET>(blockOffset_), mOffset, mOffset};
                WaitForCube();
