...
/**
 * @struct Copy
 * @brief Copy struct for DAV_3510 architecture with specific traits
 *
 * This struct is specialized for DAV_3510 architecture and is used to
...
template <class OutputType, class DstTrait, class SrcTrait>
struct Copy<
    Arch::DAV_3510, CopyEnUnitFlagWithLayout, OutputType, DstTrait, SrcTrait,
    AscendC::Std::enable_if_t<PosIsCO1<SrcTrait::tPos>() && PosIsGM<DstTrait::tPos>() && IsNDOrAlign<OutputType>()>
> {
    /**
     * @brief Copy data from source tensor to destination tensor
