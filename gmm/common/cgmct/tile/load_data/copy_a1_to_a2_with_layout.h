...
struct Copy<
    Arch::DAV_2201, CopyWithLayout, AType, DstTrait, SrcTrait,
...
/**
 * @struct Copy
 * @brief Define a Copy structure for tensor copy operations on the DAV_3510 architecture based on position traits
...
template <class AType, class DstTrait, class SrcTrait>
struct Copy<
    Arch::DAV_3510, CopyWithLayout, AType, DstTrait, SrcTrait,
    AscendC::Std::enable_if_t<SrcTrait::tPos == AscendC::TPosition::A1 && DstTrait::tPos == AscendC::TPosition::A2>> {
public:
    using DstTensor = AscendC::LocalTensor<DstTrait>;
    using SrcTensor = AscendC::LocalTensor<SrcTrait>;
