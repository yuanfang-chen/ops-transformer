...
public:
    ...
    void InvokeTilingFunc(optiling::GMMCompileInfo &compileInfo) const
    {
        ...
        vector<int64_t> scaleDims =
            transposeWeight ? vector<int64_t>{groupNum, n, k / 64, 2} : vector<int64_t>{groupNum, k / 64, n, 2};
...
string GetCsvPath()
{
    const string exeDir = GetExeDirPath();
    const vector<string> candidates = {
        exeDir + "../../../../gmm/grouped_matmul/tests/ut/op_host/test_grouped_quant_arch35_tiling.csv",
        "gmm/grouped_matmul/tests/ut/op_host/test_grouped_quant_arch35_tiling.csv",
        "./gmm/grouped_matmul/tests/ut/op_host/test_grouped_quant_arch35_tiling.csv",
    };
...
vector<GroupedQuantArch35TilingTestParam> GetParams(const string &socVersion)
{
    ...
    ifstream csvData(GetCsvPath(), ios::in);
    if (!csvData.is_open()) {
        cout << "cannot open case file " << GetCsvPath() << ", maybe not exist" << endl;
        return params;
    }
