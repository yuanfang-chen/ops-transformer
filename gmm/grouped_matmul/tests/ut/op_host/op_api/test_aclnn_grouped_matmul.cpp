...
struct GroupedMatmulOpApiCase {
    void Run() const
    {
        ...
        auto biasArg = ParseBool(hasBias) ? bias : biasOptional;
        auto scaleArg = ParseBool(hasScale) ? scale : scaleOptional;
        auto perTokenArg = ParseBool(hasPerTokenScale) ? perTokenScale : perTokenScaleOptional;

        if (api == "V3") {
            auto ut = OP_API_UT(aclnnGroupedMatmulV3, INPUT(x, weight, biasArg, scaleArg, offsetOptional,
            ...
            auto ut = OP_API_UT(aclnnGroupedMatmulWeightNz,
                                INPUT(x, weight, biasArg, scaleArg, offsetOptional, antiquantScaleOptional,
                                      antiquantOffsetOptional, perTokenArg, groupList, activationInputOptional,
