...
## aclnnGroupedMatmulWeightNzGetWorkspaceSize
  ...
  - **参数说明：**
    ...
    - <term>Ascend 950PR/Ascend 950DT</term>：
        ...
        - 输入参数`x`、`weight`，输出参数`out`在非量化场景支持最多1024个tensor，在伪量化场景支持最多128个tensor；全量化场景仅支持单tensor（tensorList长度固定为1）。
        - `out`在<term>Ascend 950PR/Ascend 950DT</term>上不支持INT8、INT32。
