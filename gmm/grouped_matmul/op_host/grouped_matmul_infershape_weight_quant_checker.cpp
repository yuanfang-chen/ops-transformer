    ...
    if (IsMxA8W4NZ(xDtype_, weightDtype_)) {
        OP_CHECK_IF(
            ...
            OP_LOGE(context->GetNodeName(),
                    ...
                    MX_GROUP_SIZE * 2, xMDim_, weightKDim_ / MX_GROUP_SIZE / 2, tensorShape->GetDim(0), tensorShape->GetDim(1),tensorShape, tensorShape->GetDim(2)),
            return ge::GRAPH_FAILED);
    }
    return ge::GRAPH_SUCCESS;
}
                                                                         ...
                                                                         const GMMAttrs &gmmAttrs) const
{
    ...
    if (IsNonEmpty(tensorShape)) {
        ...
        } else if (expectedDimNum == ANTIQUANT_PARAM_DIM_NUM_MX) { // MX的场景
            tensorNDimIdx = tensorDimNum - 3;
        }
                                                                ...
                                                                const GMMAttrs &gmmAttrs) const
{
    ...
    int64_t groupNum;
    if (IsMxA8W4NZ(xDtype_, weightDtype_)) {
        groupNum = gmmAttrs.transposeWeight ? antiquantScaleShape->GetDim(antiquantScaleDimNum - 2)
                                                : antiquantScaleShape->GetDim(antiquantScaleDimNum - 3);
    } else {
    ...
    if (IsS8S4NZ(xDtype_, weightDtype_)) {
        ...
        OP_CHECK_IF(groupSize != 128 && groupSize != 256 && groupSize != 512 && groupSize != 192,
                    ...
                    return ge::GRAPH_FAILED);
    } else if(IsMxA8W4NZ(xDtype_, weightDtype_)) {
        groupSize = groupSize / 2; // 最后一维为2
