...
bool GroupedQbmmTiling::CheckCoreNum() const
{
    ...
    if (inputParams_.groupType == SPLIT_K) {
        OP_CHECK_IF(aivNum != 2 * aicNum,
                   OP_LOGE(inputParams_.opName, "When group type is 2, aicNum:aivNum should be 1:2, actual aicNum: %u, aivNum: %u.", aicNum, aivNum),
                   return false);
    }
    if (inputParams_.kernelType == 1 || inputParams_.kernelType == 2) {
        OP_CHECK_IF(aivNum != 2 * aicNum,
                   OP_LOGE(inputParams_.opName, "When kernel type is 1 or equals 2, aicNum:aivNum should be 1:2, actual aicNum: %u, aivNum: %u.", aicNum, aivNum),
                   return false);
    }
    return true;
}
