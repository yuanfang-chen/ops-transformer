...
bool GroupedWeightQuantBatchMatmulTiling::CheckAntiQuantScale(const gert::TilingContext *context) const
{
    if (IsMxA8W4())
    {
        // 检查antiquantScale维度是否正确
        auto antiquantScaleTensor = context->GetDynamicInputTensor(ANTIQUANT_SCALE_IDX, 0);
        OP_CHECK_IF(antiquantScaleTensor == nullptr, OP_LOGE(context->GetNodeName(), "antiquantScaleTensor is nullptr."), return false);
        ...
        OP_CHECK_IF(antiquantScaleDimNum != 4, OP_LOGE(context->GetNodeName(),"antiquantScaleDimNum should be 4."), return false);
        // 检查各个维度值是否正确
        OP_CHECK_IF(antiquantScaleShape.GetDim(0) != static_cast<int64_t>(groupNum_) ||
                    antiquantScaleShape.GetDim(1) != static_cast<int64_t>(nSize_) ||
                    antiquantScaleShape.GetDim(2) != static_cast<int64_t>(kSize_) / (32UL * 2) || // TODO: 32：MxAntiQuantGroupSize
                    antiquantScaleShape.GetDim(3) != 2UL,
                    OP_LOGE(context->GetNodeName(), "Expected shape of antiquantScale scale to be [%lu, %lu, %lu, 2], but actual shape is [%lu,%lu,%lu,%lu]." ,
                        static_cast<int64_t>(groupNum_), static_cast<int64_t>(nSize_), static_cast<int64_t>(kSize_) / (32 * 2),
                        antiquantScaleShape.GetDim(0),antiquantScaleShape.GetDim(1),antiquantScaleShape.GetDim(2),antiquantScaleShape.GetDim(3))
                        , return false);
    }
    return true;
}
...
bool GroupedWeightQuantBatchMatmulTiling::CheckPerTokenScale(const gert::TilingContext *context) const
{
    if (IsMxA8W4())
    {   
        // TODO：perTokenScale是否为必选参数
        auto perTokenScaleTensor = context -> GetDynamicInputTensor(GroupedMatmul::PER_TOKEN_SCALE_INDEX, 0);
        OP_CHECK_IF(perTokenScaleTensor==nullptr, OP_LOGE(context->GetNodeName(),"perTokenScaleTensor is nullptr."), return false);
        gert::Shape perTokenScaleShape = perTokenScaleTensor -> GetStorageShape();
        int64_t perTokenScaleDimNum = perTokenScaleShape.GetDimNum();
        OP_CHECK_IF(perTokenScaleDimNum != 3,OP_LOGE(context -> GetNodeName(),"perTokenScaleShapeDimNum should be 3."),return false);
        OP_CHECK_IF(perTokenScaleShape.GetDim(0) != static_cast<int64_t>(mSize_) ||
                    perTokenScaleShape.GetDim(1) != static_cast<int64_t>(kSize_) / (32UL * 2) ||
                    perTokenScaleShape.GetDim(2) != 2UL, // TODO: 32后续修掉
                    OP_LOGE(context->GetNodeName(), "Expected shape of perTokenScale to be [%lu, %lu, 2], but actual shape is [%lu,%lu,%lu]." ,
                        static_cast<int64_t>(mSize_), static_cast<int64_t>(kSize_) / (32 * 2),perTokenScaleShape.GetDim(0),
                        perTokenScaleShape.GetDim(1),perTokenScaleShape.GetDim(2)), return false);
