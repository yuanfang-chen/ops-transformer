...
bool GroupedWeightQuantBatchMatmulTiling::CheckTensorListSize(const gert::TilingContext *context) const
{
    OP_CHECK_IF(
        ...
        return false);
    if (groupType_ == static_cast<uint64_t>(GroupType::NO_SPLIT)) {
        OP_CHECK_IF(
            numX_ != numWeight_,
            OP_LOGE(context->GetNodeName(),
...
{
    if (groupType_ == static_cast<uint64_t>(GroupType::SPLIT_M)) {
...
bool GroupedWeightQuantBatchMatmulTiling::AnalyzeAttr(const gert::TilingContext *context)
{
    ...
    groupType_ = groupTypePtr != nullptr ? *groupTypePtr : static_cast<uint64_t>(GroupType::NO_SPLIT);
    splitItem_ = splitItemPtr != nullptr ? *splitItemPtr : 0; // 0: 默认split_item
    groupListType_ = groupListTypePtr != nullptr ? *groupListTypePtr : 0;
    isSingleX_ = (groupType_ != static_cast<uint64_t>(GroupType::NO_SPLIT) &&
                  context->GetDynamicInputTensor(X_IDX, 1) == nullptr);
    isSingleWeight_ = (groupType_ != static_cast<uint64_t>(GroupType::NO_SPLIT) &&
...
bool GroupedWeightQuantBatchMatmulTiling::CheckGroupTypeAndSplitItem(const gert::TilingContext *context) const
{
    if (IsA16W4ND()) {
        OP_CHECK_IF(
            (groupType_ != static_cast<uint64_t>(GroupType::NO_SPLIT)) &&
                (groupType_ != static_cast<uint64_t>(GroupType::SPLIT_M)),
