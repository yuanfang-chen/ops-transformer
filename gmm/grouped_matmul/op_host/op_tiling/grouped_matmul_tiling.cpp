...
ge::graphStatus GMMTiling::A8W4Tiling(gert::TilingContext* context, const GMMCompileInfo* compileInfoPtr) {
      ...
      if (useHighPerf) {
        ...
        if (!isMSD && !(biasPtr == nullptr || biasPtr->GetStorageShape().GetShapeSize() == 0)) {
          OP_LOGW(context->GetNodeName(), "GMM W4A8: with-offset mode requires non-empty bias, but got an empty tensor. This will lead to incorrect results.");
        }
        if (!isMSD) {
