...
static aclnnStatus CheckFunctionParams(const gmm::GroupedMatmulParams &gmmParams) {
  ...
  if (op::GetCurrentPlatformInfo().GetCurNpuArch() == NpuArch::DAV_3510) {
    CHECK_COND(isNoActivation, ACLNN_ERR_PARAM_INVALID, "Activation is not supported on DAV_3510 platforms.");
    if (IsQuant(gmmParams.xDtype, weightDtype)) {
      ...
      CHECK_RET(gmm::AclnnGroupedMatmulNoQuantDAV3510Checker(gmmParams).CheckGroupedMatmulNoQuantDAV3510() ==
                    ACLNN_SUCCESS,
                ACLNN_ERR_PARAM_INVALID);
    }
  }
