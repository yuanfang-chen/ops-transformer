...
static aclnnStatus CheckNotNull(const aclTensorList *x, const aclTensorList *weight, const aclTensorList *y)
{
    ...
    CHECK_COND(x->Size() != 0 && CheckTensorListNotNull(x, "x") == ACLNN_SUCCESS, ACLNN_ERR_PARAM_INVALID,
               "X must not be empty tensorlist.");
    CHECK_COND(weight != nullptr, ACLNN_ERR_PARAM_NULLPTR, "Weight must not be nullptr.");
    CHECK_COND(weight->Size() != 0 && CheckTensorListNotNull(weight, "weight") == ACLNN_SUCCESS,
               ACLNN_ERR_PARAM_INVALID, "Weight must not be empty tensorlist.");
    CHECK_COND(y != nullptr, ACLNN_ERR_PARAM_NULLPTR, "Y must not be nullptr.");
    CHECK_COND(y->Size() != 0 && CheckTensorListNotNull(y, "y") == ACLNN_SUCCESS, ACLNN_ERR_PARAM_INVALID,
               "Y must not be empty tensorlist.");
    return ACLNN_SUCCESS;
}
