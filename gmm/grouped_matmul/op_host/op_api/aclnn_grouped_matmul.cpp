...
static aclnnStatus CheckEmptyTensor(const aclTensorList *x, const aclTensorList *weight)
{
  ...
  for (size_t i = 0; i < x->Size(); ++i) {
    ...
    CHECK_COND(xShape.GetDimNum() >= gmm::MIN_FM_DIM && xShape.GetDimNum() <= gmm::MAX_FM_DIM, ACLNN_ERR_PARAM_INVALID,
               "GroupedMatmul x dim num should be in the range [2,6], but actual %d.", xShape.GetDimNum());
  ...
  for (size_t i = 0; i < weight->Size(); ++i) {
    ...
    CHECK_COND(wShape.GetDimNum() >= gmm::MIN_FM_DIM && wShape.GetDimNum() <= gmm::MAX_FM_DIM, ACLNN_ERR_PARAM_INVALID,
               "GroupedMatmul x dim num should be in the range [2,6], but actual %d.", wShape.GetDimNum());
