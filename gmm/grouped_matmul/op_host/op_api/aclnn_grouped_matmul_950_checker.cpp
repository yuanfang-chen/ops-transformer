...
template <typename T>
aclnnStatus AclnnGroupedMatmulDAV3510Checker<T>::CheckInputDtypeForV3Version() const
{   
...
{
    CHECK_COND(CheckInputDtypeForV3Version(), ACLNN_ERR_PARAM_INVALID,
               "CheckInputDtypeForV3Version failed.");
    CHECK_COND(CheckInputShapeForV3Version(), ACLNN_ERR_PARAM_INVALID,
...
{   
    ...
    if (IsQuant(xDtype, weightDtype)) {
        ...
        if (gmmParams_.apiVersion == gmm::GMMApiVersion::V3) {
            CHECK_COND(CheckInputParamsForV3Version(), ACLNN_ERR_PARAM_INVALID,
                       "CheckInputParamsForV3Version failed.");
        }
    }
    return ACLNN_SUCCESS;
}