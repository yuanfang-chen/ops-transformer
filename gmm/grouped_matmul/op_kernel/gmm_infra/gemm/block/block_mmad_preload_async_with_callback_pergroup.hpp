...
>
struct BlockMmad <
    MmadAtlasA2PreloadAsyncWithCallbackPerGroup<
        PRELOAD_STAGES_,
        L1_STAGES_,
        L0A_STAGES_,
        L0B_STAGES_,
...
public:
    // Type Aliases
    using DispatchPolicy = MmadAtlasA2PreloadAsyncWithCallbackPerGroup<
        PRELOAD_STAGES_,
        L1_STAGES_,
        L0A_STAGES_,
        L0B_STAGES_,
    ...
    using LayoutScaleInL1 = typename CopyL1ToFP::LayoutSrc;
    using LayoutScaleInFP = typename CopyL1ToFP::LayoutDst;

    using L1AAlignHelper = Gemm::helper::L1AlignHelper<ElementA, LayoutA>;
    using L1BAlignHelper = Gemm::helper::L1AlignHelper<ElementB, LayoutB>;

    static constexpr float INT4_DATA_TYPE_SIZE = 0.5;
    static constexpr float ELEMENTA_DATA_TYPE_SIZE = INT4_DATA_TYPE_SIZE;
    static constexpr float ELEMENTB_DATA_TYPE_SIZE = INT4_DATA_TYPE_SIZE;
    ...
    static_assert(std::is_same_v<ElementA, AscendC::int4b_t>, "ElementA must int4!");
    static_assert(std::is_same_v<ElementB, AscendC::int4b_t>, "ElementB must int4!");
    // L1 tile size
    static constexpr uint32_t L1A_TILE_SIZE = L1TileShape::M * L1TileShape::K * ELEMENTA_DATA_TYPE_SIZE;
    static constexpr uint32_t L1B_TILE_SIZE = L1TileShape::N * L1TileShape::K * ELEMENTB_DATA_TYPE_SIZE;
    static constexpr uint32_t L1SCALE_TILE_SIZE = L1TileShape::N * sizeof(ElementScale);
    // L0 tile size
    static constexpr uint32_t L0A_TILE_SIZE = L0TileShape::M * L0TileShape::K * ELEMENTA_DATA_TYPE_SIZE;
    static constexpr uint32_t L0B_TILE_SIZE = L0TileShape::K * L0TileShape::N * ELEMENTB_DATA_TYPE_SIZE;
    static constexpr uint32_t L0C_TILE_SIZE = L0TileShape::M * L0TileShape::N * sizeof(ElementAccumulator);
    static constexpr uint32_t FPSCALE_TILE_SIZE = L0TileShape::N * sizeof(ElementScale);

    static constexpr uint32_t MTE2_MTE1_FLAG = 1;
    static constexpr uint32_t MTE1_MTE2_FLAG = 1;
    static constexpr uint32_t M_MTE1_FLAG = 1;
    static constexpr uint32_t MTE1_M_FLAG = 1;
    static constexpr uint32_t FIX_M_FLAG = 1;
    static constexpr uint32_t M_FIX_FLAG = 1;
    static constexpr uint32_t FIX_MTE2_FLAG = 1;
    static constexpr uint32_t MTE2_FIX_FLAG = 1;

    // Check LayoutC
    static_assert(std::is_same_v<LayoutC, layout::RowMajor>, "LayoutC only support RowMajor yet!");

    // Check L1TileShape
    static_assert((L1A_TILE_SIZE + L1B_TILE_SIZE + L1SCALE_TILE_SIZE) * L1_STAGES <= ArchTag::L1_SIZE,
    ...
    static_assert(FPSCALE_TILE_SIZE * L0C_STAGES <= ArchTag::FIXBUF_SIZE, "FPScaleShape exceeding the FIXBUF space!");
    ...
    CATLASS_DEVICE
    BlockMmad(Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart = 0, uint32_t FPBufAddrStart = 0)
    {
        InitL1(resource, l1BufAddrStart);
        InitL0A(resource);
        InitL0B(resource);
        InitL0C(resource);
        InitFP(resource, FPBufAddrStart);
    }
    ...
    void operator()(
        ...
        AscendC::GlobalTensor<ElementScale> const &gmBlockScale, LayoutScale const &layoutScale,
        GemmCoord const &actualShape, bool isAtomicAdd, bool isBeginCallback, bool isEndCallback,
        Callback const &callbackBeforeFixpipe, Callback const &callbackAfterFixpipe
    )
    {
        // Load matrix sacle tile from GM to L1
        ...
        for (uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; ++kLoopIdx) {
            ...
            l1TileMmadParams.isAtomicAdd = isAtomicAdd;
            l1TileMmadParams.isBeginCallback = isBeginCallback;
            l1TileMmadParams.isEndCallback = isEndCallback;
            if (kLoopIdx == 0) {
                l1TileMmadParams.gmBlockScale = gmBlockScale;
                auto scaleTileShape = actualShape.template GetCoordByAxis<1>();
                l1TileMmadParams.layoutScaleInGm = layoutScale.GetTileLayout(scaleTileShape);
...
private:
    ...
    CATLASS_DEVICE
    void InitFP(Arch::Resource<ArchTag> &resource, uint32_t FpAddrStart)
    {
        uint32_t FpOffset = FpAddrStart;
        FPScaleTensor = resource.fpBuf.template GetBufferByByte<ElementScale>(FpOffset);
    }
    ...
    void L1TileMmad(L1TileMmadParams const &params)
    {
        ...
        if (params.isKLoopLast) {
            ...
            if constexpr (!ENABLE_UNIT_FLAG) {
                ...
                copyL0CToGm(params.gmBlockC, l0CTensor, FPScaleTensor, layoutCInGm, layoutCInL0Copy);
                if (params.isAtomicAdd) {
                ...
                copyL0CToGm(params.gmBlockC, l0CTensor, FPScaleTensor, layoutCInGm, layoutCInL0Copy, 0b11);
                if (params.isAtomicAdd) {
                    AscendC::SetAtomicNone();
                }
            }
    ...
    AscendC::LocalTensor<ElementScale> l1ScaleTensorList[L1_STAGES];
    int32_t l1ScaleEventList[L1_STAGES];
    uint32_t l1ScaleListId{0};

    AscendC::LocalTensor<ElementScale> FPScaleTensor;
