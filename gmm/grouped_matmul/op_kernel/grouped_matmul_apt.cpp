...
REGISTER_TILING_DEFAULT(GMMQuantTilingData);
    ...
    if constexpr (QUANT_B_TRANS == GMM_NO_TRANS && QUANT_A_TRANS == GMM_NO_TRANS
        ...
        if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] QUANT_MX: B_NO_TRANS, A_NO_TRANS, DEQUANT_FIXP\n"); }
        GMM_QUANT_IMPL_CLASS(false, false, GmmASWKernel);
    } else if constexpr (QUANT_B_TRANS == GMM_TRANS && QUANT_A_TRANS == GMM_NO_TRANS
        && KERNEL_TYPE == GMM_DEQUANT_FIXP) {
        if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] QUANT_MX: B_TRANS, A_NO_TRANS, DEQUANT_FIXP\n"); }
    ...
    if constexpr (QUANT_B_TRANS == GMM_NO_TRANS && QUANT_A_TRANS == GMM_NO_TRANS
        && KERNEL_TYPE == GMM_DEQUANT_FIXP) {
        if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] QUANT_CUBE/PERTENSOR: B_NO_TRANS, A_NO_TRANS, DEQUANT_FIXP\n"); }
        GMM_QUANT_IMPL_CLASS(false, false, GmmASWKernel);
    } else if constexpr (QUANT_B_TRANS == GMM_TRANS && QUANT_A_TRANS == GMM_NO_TRANS
        && KERNEL_TYPE == GMM_DEQUANT_FIXP) {
        if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] QUANT_CUBE/PERTENSOR: B_TRANS, A_NO_TRANS, DEQUANT_FIXP\n"); }
        GMM_QUANT_IMPL_CLASS(false, true, GmmASWKernel);
    }
#endif
    ...
    if constexpr (QUANT_B_TRANS == GMM_NO_TRANS && QUANT_A_TRANS == GMM_TRANS
        && KERNEL_TYPE == GMM_DEQUANT_FIXP) {
        if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] QUANT_MX/PERTENSOR: B_NO_TRANS, A_TRANS, DEQUANT_FIXP (EmptyTensor)\n"); }
    ...
    if constexpr (QUANT_B_TRANS == GMM_NO_TRANS && QUANT_A_TRANS == GMM_NO_TRANS
        ...
        if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] QUANT_MIX: B_NO_TRANS, A_NO_TRANS, DEQUANT_VECTOR\n"); }
        GMM_QUANT_MIX_IMPL_CLASS(false, false, GQmmMixRegbaseKernel);
    } else if constexpr (QUANT_B_TRANS == GMM_TRANS && QUANT_A_TRANS == GMM_NO_TRANS
        && KERNEL_TYPE == GMM_DEQUANT_VECTOR) {
        if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] QUANT_MIX: B_TRANS, A_NO_TRANS, DEQUANT_VECTOR\n"); }
        ...
        && KERNEL_TYPE == GMM_DEQUANT_VECTOR) {
        if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] QUANT_MIX: B_NO_TRANS, A_TRANS, DEQUANT_VECTOR\n"); }
    ...
    if constexpr (QUANT_B_TRANS == GMM_NO_TRANS && QUANT_A_TRANS == GMM_NO_TRANS
        ...
        if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] QUANT_PERTILE: B_NO_TRANS, A_NO_TRANS, PERGROUP_PERBLOCK\n"); }
        GMM_QUANT_GB_IMPL_CLASS(Cgmct::Gemm::layout::RowMajor, Cgmct::Gemm::layout::RowMajor,
                                Cgmct::Gemm::layout::RowMajorAlign);
    } else if constexpr (QUANT_B_TRANS == GMM_TRANS && QUANT_A_TRANS == GMM_NO_TRANS
        && KERNEL_TYPE == GMM_PERGROUP_PERBLOCK) {
        if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] QUANT_PERTILE: B_TRANS, A_NO_TRANS, PERGROUP_PERBLOCK\n"); }
        GMM_QUANT_GB_IMPL_CLASS(Cgmct::Gemm::layout::RowMajor, Cgmct::Gemm::layout::ColumnMajor,
                                Cgmct::Gemm::layout::RowMajorAlign);
    } else if constexpr (QUANT_B_TRANS == GMM_NO_TRANS && QUANT_A_TRANS == GMM_TRANS
        && KERNEL_TYPE == GMM_PERGROUP_PERBLOCK) {
        if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] QUANT_PERTILE: B_NO_TRANS, A_TRANS, PERGROUP_PERBLOCK\n"); }
    ...
    KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_MIX_AIC_1_2);
        ...
        if constexpr (W_TYPE == WQGMM_FRACTAL_NZ && OFFSET_OR_BIAS_EXIT == WQGMM_ANTIQUANT_OFFSET_NOT_EXIST_BIAS_NOT_EXIST
            ...
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] ANTI_QUANT_INT8: NZ, PER_CH, 512_DEFAULT, RESPLIT, VCV\n"); }
            ...
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] ANTI_QUANT_INT8: NZ, PER_CH, 384_3, RESPLIT, VCV\n"); }
            INVOKE_GMM_WEIGHT_QUANT_VCV_CONTROLLER_OP_IMPL(GMMWeightQuantResplitController, S8S4_NZKN_G,
        ...
        if constexpr (W_TYPE == WQGMM_FRACTAL_NZ && OFFSET_OR_BIAS_EXIT == WQGMM_ANTIQUANT_OFFSET_NOT_EXIST_BIAS_NOT_EXIST
            ...
            && ALGORITHM_SUB_CATEGORY == WQGMM_N_FIRST_TAIL_RESPLIT && ALGORITHM_CATEGORY == WQGMM_VECTOR_ANTIQUANT) {
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] ANTI_QUANT_FP8: NZ, MX, TRANS, DYNAMIC_4, RESPLIT\n"); }
        ...
        if constexpr (W_TYPE == WQGMM_FRACTAL_NZ && OFFSET_OR_BIAS_EXIT == WQGMM_ANTIQUANT_OFFSET_NOT_EXIST_BIAS_NOT_EXIST
            ...
            && ALGORITHM_SUB_CATEGORY == WQGMM_N_FIRST_TAIL_RESPLIT && ALGORITHM_CATEGORY == WQGMM_VECTOR_ANTIQUANT) {
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] ANTI_QUANT_E8M0: NZ, MX, NO_TRANS, 256_4, RESPLIT\n"); }
        ...
        if constexpr (W_TYPE == WQGMM_ND && OFFSET_OR_BIAS_EXIT == WQGMM_ANTIQUANT_OFFSET_NOT_EXIST_BIAS_NOT_EXIST
            ...
            && WQ_A_TRANS == WQGMM_NO_TRANS && TEMPLATE_CUSTOM_SC == WQGMM_MTE2_INNER_SIZE_256_BUF_NUM_4
            && ALGORITHM_SUB_CATEGORY == WQGMM_N_FIRST_TAIL_RESPLIT && ALGORITHM_CATEGORY == WQGMM_VECTOR_ANTIQUANT) {
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] ANTI_QUANT: ND, NO_OFFSET, PER_CH, TRANS, 256_4, RESPLIT\n"); }
            ...
            && WQ_A_TRANS == WQGMM_NO_TRANS && TEMPLATE_CUSTOM_SC == WQGMM_MTE2_INNER_SIZE_256_BUF_NUM_4
            && ALGORITHM_SUB_CATEGORY == WQGMM_N_FIRST_TAIL_RESPLIT && ALGORITHM_CATEGORY == WQGMM_VECTOR_ANTIQUANT) {
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] ANTI_QUANT: ND, HAS_OFFSET, PER_CH, TRANS, 256_4, RESPLIT\n"); }
            ...
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] ANTI_QUANT: ND, NO_OFFSET, PER_CH, TRANS, 256_4, BASIC\n"); }
            static constexpr WqmmConfig wqmmCfg = {false, true, QuantType::PER_CHANNEL, false,
                                                   QuantType::NONE, CubeFormat::ND};
            ...
            && WQ_A_TRANS == WQGMM_NO_TRANS && TEMPLATE_CUSTOM_SC == WQGMM_MTE2_INNER_SIZE_256_BUF_NUM_4
            && ALGORITHM_SUB_CATEGORY == WQGMM_N_FIRST_BASIC_BLOCK && ALGORITHM_CATEGORY == WQGMM_VECTOR_ANTIQUANT) {
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] ANTI_QUANT: ND, HAS_OFFSET, PER_CH, TRANS, 256_4, BASIC\n"); }
            static constexpr WqmmConfig wqmmCfg = {false, true, QuantType::PER_CHANNEL, true,
                                                   QuantType::NONE, CubeFormat::ND};
            INVOKE_GMM_WEIGHT_QUANT_BASIC_CONTROLLER_OP_IMPL(GMMWeightQuantBasicController, wqmmCfg,
                                                             VEC_ANTIQUANT_CONFIG_3);
            ...
            && WQ_A_TRANS == WQGMM_NO_TRANS && TEMPLATE_CUSTOM_SC == WQGMM_MTE2_INNER_SIZE_256_BUF_NUM_4
            && ALGORITHM_SUB_CATEGORY == WQGMM_N_FIRST_BASIC_BLOCK && ALGORITHM_CATEGORY == WQGMM_VECTOR_ANTIQUANT) {
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] ANTI_QUANT: ND, NO_OFFSET, PER_CH, NO_TRANS, 256_4, BASIC\n"); }
            ...
            && WQ_A_TRANS == WQGMM_NO_TRANS && TEMPLATE_CUSTOM_SC == WQGMM_MTE2_INNER_SIZE_256_BUF_NUM_4
            && ALGORITHM_SUB_CATEGORY == WQGMM_N_FIRST_BASIC_BLOCK && ALGORITHM_CATEGORY == WQGMM_VECTOR_ANTIQUANT) {
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] ANTI_QUANT: ND, HAS_OFFSET, PER_CH, NO_TRANS, 256_4, BASIC\n"); }
    ...
    if constexpr (NO_QUANT_B_TRANS == GMM_NO_TRANS && NO_QUANT_A_TRANS == GMM_NO_TRANS) {
        if constexpr (wFormat == CubeFormat::NZ) {
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] NO_QUANT: B_NO_TRANS, A_NO_TRANS, NZ\n"); }
            GmmNoQuantAswt<layout::RowMajor, layout::Nz>(x, weight, bias, groupList, y, tiling);
        } else {
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] NO_QUANT: B_NO_TRANS, A_NO_TRANS, ND\n"); }
        ...
        if constexpr (wFormat == CubeFormat::NZ) {
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] NO_QUANT: B_TRANS, A_NO_TRANS, NZ\n"); }
            GmmNoQuantAswt<layout::RowMajor, layout::Zn>(x, weight, bias, groupList, y, tiling);
        } else {
            if (AscendC::GetBlockIdx() == 0) { AscendC::PRINTF("[GMM] NO_QUANT: B_TRANS, A_NO_TRANS, ND\n"); }
