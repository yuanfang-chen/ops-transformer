...
private:
    ...
    uint64_t kaL1Size_; // mxA8W4场景ka使能多倍载入
                                                              ...
                                                              uint64_t kbL1RealSize, const BasicBlockOffsetParam &param)
{
    ...
    if constexpr (!IsMxA8W4<xType, wqmmConfig.antiQuantType>()) {
        ...
        if (isBias_) {
            mmObj_.SetBias(biasL1_[(aL1BufIdx_ & 1) * biasL1DbOffset_]);
        }
        ...
        basicApiParams.l1KaSize = offsetParams.kaL1Size;
        ...
        mmObj_.Iterate(kbOffset + kbL1RealSize >= param.kSize, kbOffset == 0, aL1_[aL1Offset],
                       ...
                       biasL1_[(aL1BufIdx_ & 1) * biasL1DbOffset_], basicApiParams);
    }
}
                                                                    ...
                                                                    int64_t kaGmOffset)
{
    ...
    int64_t kaL1RealSize = (kaGmOffset + offsetParam.kaL1Size) >= offsetParam.kSize ? offsetParam.kSize - kaGmOffset :
                                                                                      offsetParam.kaL1Size;
    int64_t aGmOffset;
    if constexpr (!wqmmConfig.aTrans) {
    ...
    if constexpr (!IsMxA8W4<xType, wqmmConfig.antiQuantType>()) {
        if (isBias_ && kaGmOffset == 0) {
            DataCopyPad2D(biasL1_[(aL1BufIdx_ & 1) * biasL1DbOffset_], biasGlobal_[param.nOffset], 1, param.nL1Size,
