                                                              ...
                                                              uint64_t cvLoopIdx)
{
    ...
    if constexpr (!IsMxA8W4<xType, wqmmConfig.antiQuantType>()) {
        ...
        mmObj_.Iterate(kbOffset + kbL1RealSize >= param.kSize, kbOffset == 0, aL1_[aL1Offset],
                       ...
                       mxScaleBL1_[(mxScaleBufIdx_ & 1) * mxScaleBL1DbOffset_], biasL1_[(cvLoopIdx & 1) * biasL1DbOffset_],
                       basicApiParams);
    }
}
...
WQBMM_CUBE_COMPUTE_TEMPLATE_PARAM
__aicore__ inline void WQBMM_CUBE_COMPUTE_CLASS::SetScaleMTE1ToMTE2(uint64_t kbL1Offset, const BasicBlockOffsetParam &offsetParam)
{
    if ((kbL1Offset + offsetParam.kbL1Size) % MX_SCALE_K_L1_SIZE == 0 || kbL1Offset + offsetParam.kbL1Size >= offsetParam.kSize) {
                                                                   ...
                                                                   uint64_t kbL1Offset, uint64_t cvLoopIdx)
{
    ...
    uint64_t scaleKL1RealSize =
        (kbL1Offset + MX_SCALE_K_L1_SIZE) > param.kSize ? (param.kSize - kbL1Offset) / MX_GROUPSIZE : scaleKL1StandardLen;
                                                                        ...
                                                                        Dn2NzParams &dn2NzParams)
{
    ...
    dn2NzParams.dstNzC0Stride = CeilDivide(MX_SCALE_L1_SIZE, MX_GROUPSIZE * SCALE_COPY_GROUP_SIZE);
...
__aicore__ inline void WQBMM_CUBE_COMPUTE_CLASS::InitSync()
{
    for (uint64_t i = 0; i < DOUBLE_BUFFER_NUM; i++) {
        ...
        SetFlag<HardEvent::MTE1_MTE2>(cubeEventIdsMxScaleMte1ToMte2_[0]);
    }
...
WQBMM_CUBE_COMPUTE_TEMPLATE_PARAM
__aicore__ inline void WQBMM_CUBE_COMPUTE_CLASS::MxA8W4Init(uint64_t aPrefetchSize, uint64_t l1RemainSize, uint64_t l1StartSize,
                                                            uint64_t mxBiasL1DbOffset,
                                                            const TCubeTiling *__restrict matmulTiling,
                                                            const LocalTensor<biasType> &biasL1)
{
    //  MxA8W4场景空间分配: 其中 weight\Bias为全局分配
