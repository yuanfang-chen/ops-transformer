...
struct MmadMx {
	...
	{
		#ifdef __XBL_PRINT__
        AscendC::printf("[Patch] mad_mx %p,%p,%p,%d,%d,%d,%d,%d,%d,%d",dst.Data().Get(), fm.Data().Get(), filter.Data().Get(), m, k, n, unitFlagCtrl, true, btBuffCtrl,
			initCMatrixCtrl);
		#endif
		mad_mx(
...
struct MmadMxWithBias {
		...
		uint8_t unitFlagCtrl, bool btBuffCtrl, bool initCMatrixCtrl)
	{
		...
		#ifdef __XBL_PRINT__
        AscendC::printf("[Patch] mad_mx %p,%p,%p,%d,%d,%d,%d,%d,%d,%d",(dstType*)xd, fm.Data().Get(), filter.Data().Get(), m, k, n, unitFlagCtrl, true, btBuffCtrl,
			initCMatrixCtrl);
		#endif
		mad_mx(
...
template <>
struct MmadTraits<Cgmct::Gemm::Tile::MmadMx>
	: public MmadTraits<Cgmct::Gemm::Tile::MmadMx, MmadTraitDefault, Cgmct::Gemm::Tile::MmadMx, MmadTraitDefault> {};

template <>
struct MmadTraits<Cgmct::Gemm::Tile::MmadMxWithBias>
	: public MmadTraits<
		Cgmct::Gemm::Tile::MmadMxWithBias, MmadTraitDefault, Cgmct::Gemm::Tile::MmadMxWithBias, MmadTraitDefault> {};
