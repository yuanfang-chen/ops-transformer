...
struct CopyScaleGM2L1 {
        ...
        const T& dst, const U& src, uint16_t nValue, uint32_t dValue, uint64_t srcDValue, uint16_t dstNzC0Stride)
    {
        ...
        #ifdef __XBL_PRINT__
        AscendC::printf("[Patch] copy_gm_to_cbuf_multi_dn2nz %d,%d,%d,%d,%p,%p,%d,%d,%d,%d,%d,%d,%d",loop4DstStride, loop3DstStride, loop2DstStride, dnNum, (__cbuf__ half*)dst.Data().Get(), (__gm__ half*)src.Data().Get(), 0, loop1SrcStride, cacheMode, nValue,
        ...
        const T& dst, const U& src, uint16_t nValue, uint32_t dValue, uint64_t srcDValue, uint16_t dstNzC0Stride)
    {
        ...
        #ifdef __XBL_PRINT__
        AscendC::printf("[Patch] copy_gm_to_cbuf_multi_nd2nz %d,%d,%d,%d,%p,%p,%d,%d,%d,%d,%d,%d,%d scale_gm2l1",loop4DstStride, loop3DstStride, loop2DstStride, ndNum, (__cbuf__ half*)dst.Data().Get(), (__gm__ half*)src.Data().Get(), 0, loop1SrcStride, cacheMode, nValue,
            dValue, loop4SrcStride, false);
        #endif
...
template <>
struct AscendC::Te::CopyTraits<Cgmct::Gemm::Tile::CopyScaleGM2L1>
    : public CopyTraits<
        Cgmct::Gemm::Tile::CopyScaleGM2L1, AscendC::Te::LoadDataTraitDefault, Cgmct::Gemm::Tile::CopyScaleGM2L1,
