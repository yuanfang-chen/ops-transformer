...
private:
    ...
    __aicore__ inline void LoadBL1ToL0B(uint64_t baseK, uint64_t loopId, uint64_t l1KOffset,
                                        const BasicApiParamsV1 &basicApiParams, const LocalTensor<xType> &bL1Tensor_,
                                                              ...
                                                              const LocalTensor<fp8_e8m0_t> &scaleAL1Tensor)
{
    ...
    aL0Load2dParams.mStep = CeilDivide(basicApiParams.l0MSize, static_cast<uint64_t>(AscendC::BLOCK_CUBE)); // 单位16元素
    ...
    aL0Load2dMx.xStep = CeilDivide(basicApiParams.l0MSize, static_cast<uint64_t>(AscendC::BLOCK_CUBE)); // 单位是1个32B分形
    aL0Load2dMx.yStep =
                                                              ...
                                                              const LocalTensor<fp8_e8m0_t> &scaleBL1Tensor)
{
    ...
    bL0Load2dParams.mStep = CeilDivide(basicApiParams.l0NSize, static_cast<uint64_t>(AscendC::BLOCK_CUBE)); // 单位16元素
    ...
    bL0Load2dMx.yStartPosition = CeilDivide((l1KOffset + basicApiParams.gmKOffset) % MX_SCALE_K_L1_SIZE,
                                            MX_GROUP_SIZE * MX_UNIT_BYTES);              // 单位是32B
    bL0Load2dMx.xStep = CeilDivide(basicApiParams.l0NSize, static_cast<uint64_t>(AscendC::BLOCK_CUBE)); // 单位是1个32B分形，
