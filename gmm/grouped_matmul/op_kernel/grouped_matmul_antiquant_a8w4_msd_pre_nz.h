...
constexpr uint32_t DATA_BLOCK_SIZE_32 = 32U;
...
public:
    ...
    __aicore__ inline Init(GM_ADDR x, GM_ADDR y, GM_ADDR groupList, GM_ADDR workspace, const GMMBaseParams& tilingData, TPipe *pipe);
    __aicore__ inline Process();
    __aicore__ inline copyIn(uint32_t mIdx, uint32_t kIdx, uint32_t curVecBaseM, uint32_t curVecBaseK);
    __aicore__ inline compute(uint32_t mIdx, uint32_t kIdx, uint32_t curVecBaseM, uint32_t curVecBaseK);
    __aicore__ inline copyOut(uint32_t mIdx, uint32_t kIdx, uint32_t curVecBaseM, uint32_t curVecBaseK);
...
private:
    ...
    uint32_t vectorBlockSizeAlign{DATA_BLOCK_SIZE_32};
