...
static ge::graphStatus ValidateScaleAndBias(const InferShapeContext *context, const char* op_name, const CheckXandWParams& xAndWParams)
{
    ...
    if (shape_scale->GetDimNum() == twoDimNum) {
        ...
        OP_CHECK_IF((context->GetOptionalInputShape(biasOptionIndex) == nullptr || 
            ...
            OPS_REPORT_CUBE_INNER_ERR(op_name, "bias is not supported.%d %d %d %d",context->GetOptionalInputShape(biasOptionIndex)->GetDim(0),xAndWParams.e,context->GetOptionalInputShape(biasOptionIndex)->GetDim(DIM_ONE),xAndWParams.n), return ge::GRAPH_FAILED);
