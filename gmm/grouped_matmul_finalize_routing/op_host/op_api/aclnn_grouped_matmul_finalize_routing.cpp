...
static inline bool TransposeTensorContiguousProcess(const aclTensor *&contiguousTensor, bool &transpose, aclOpExecutor *executor)
{
    ...
    if (transposeFlag) {
        ...
        transpose = !transpose;
        // FORMAT_FRACTAL_NZ：与 GMM WeightNz SetTransposedTensorListContiguous 一致，交换 view 后写回 storageShape
        if (op::GetCurrentPlatformInfo().GetCurNpuArch() == NpuArch::DAV_3510 &&
    ...
    const aclIntArray *tuningConfigOptional, aclTensor *out, uint64_t *workspaceSize, aclOpExecutor **executor)
{
    ...
    // FORMAT_FRACTAL_NZ：与 GMM WeightNz 在 CreateView/交换 stride 前保存 storageShape、之后写回一致
    if (ge::GetPrimaryFormat(tmpWeight->GetStorageFormat()) == op::Format::FORMAT_FRACTAL_NZ) {
