...
public:
    ...
    bool CheckInputOutShape()
    {
        ...
        if (groupListLen != e) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    "Length of 'groupList' should be equal to the number of experts in weight. Got e=%ld.", e);
            return false;
        }
        if (e > MAX_NUM_EXPERTS) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "In MXFP4/MXFP8, e must be less than 1024. Got e=%ld.", e);
            return false;
        }
        return true;
    }
    ...
    {   
        ...
        if ((xDtype == DataType::DT_FLOAT4_E2M1 || xDtype == DataType::DT_FLOAT4_E1M2) &&
            ...
            if (!(k % MOD2 == 0)) {
                OP_LOGE(ACLNN_ERR_PARAM_INVALID, "In MXFP4 , k must be divisible by 2. Got k=%ld.", k);
                return false;
            }
            if (gmmParams_.transposeX2 == false && (n % MOD2 != 0)) {
                OP_LOGE(ACLNN_ERR_PARAM_INVALID, "In MXFP4 , n must be even when x2 is not transposed. Got n=%ld.", n);
                return false;
            }
            if (k == MOD2) {
