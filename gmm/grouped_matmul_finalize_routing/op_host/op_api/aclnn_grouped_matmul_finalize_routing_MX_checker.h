...
const std::initializer_list<DataType> X_WEIGHT_TYPE_SUPPORT_LIST_FP4 = {op::DataType::DT_FLOAT4_E1M2, op::DataType::DT_FLOAT4_E2M1};
const std::initializer_list<DataType> X_WEIGHT_TYPE_SUPPORT_LIST_FP8 = {op::DataType::DT_FLOAT4_E1M2, op::DataType::DT_FLOAT4_E2M1};
...
public:
    ...
    aclnnStatus CheckInputOutDims()
    {
        ...
        INT64 xscaleExpectDim = quantMode_ == QuantMode::MX ?  THERE_DIM:ONE_DIM;
        INT64 weightscaleExpectDim = quantMode_ == QuantMode::MX ?  FOUR_DIM:TWO_DIM;
        ...
        CHECK_COND(wScaleDimNumber == weightscaleExpectDim, ACLNN_ERR_PARAM_INVALID,
                   "The dim num of scale should be equal %lu, current dim is %lu.", weightscaleExpectDim,wScaleDimNumber);
    ...
    bool CheckInputOutShape()
    {
        ...
        op::Shape xScaleExpectShape =
            quantMode_ == QuantMode::MX ? {m, Ops::Base::CeilDiv(k, GMMFR_SPLIT_SIZE), GMMFR_SPLIT_FACTOR} : {m};
        op::Shape weightScaleExpectShape =
            quantMode_ == QuantMode::MX ? {e, Ops::Base::CeilDiv(k, GMMFR_SPLIT_SIZE), n, GMMFR_SPLIT_FACTOR} : {e, n};
    ...
    bool CheckDtypeValidForPertoken()
    {
        ...
        DataType xDtype = gmmParams_.scale->GetDataType();
        OP_CHECK_DTYPE_NOT_SUPPORT(gmmParams_.scale,
                                   (xDtype == DataType::DT_INT8) ? SCALE_TYPE_SUPPORT_LIST_PERTOKEN_XINT8 :
                                                                   SCALE_TYPE_SUPPORT_LIST_PERTOKEN_XFP8HIFLOAT8,
                                   return false);
        OP_CHECK_DTYPE_NOT_SUPPORT(gmmParams_.rowIndex, ROW_INDEX_TYPE_SUPPORT_LIST, return false);
